...
test('courses page should expose premium cruise classroom flow at first section', async ({ page }) => {
  ...
  await expect(premiumLink).toContainText('柔性之海：豪华邮轮舒适度控制');
  await expect(premiumLink).toContainText('45 分钟标准互动课');
  ...
  await expect(page.getByRole('main').getByRole('heading', { name: '柔性之海：豪华邮轮舒适度控制' })).toBeVisible();
...
test('student demo route should be isolated without teacher-student split screen', async ({ page }) => {
  ...
  await expect(page.getByText('演示模式已开启')).toBeVisible();
  await expect(page.getByText('课堂视角')).toHaveCount(0);
});
