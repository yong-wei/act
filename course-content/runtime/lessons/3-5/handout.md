...
![封面漫画：单元导入](/course-runtime/lessons/3-5/media/3-5-cover-comic.png)
...
![低阶纯极点系统接入零点后的根轨迹变化](/course-runtime/lessons/3-5/media/3-5-rl-01-low-order-zero-compare.png){width=100%}
...
![三阶纯极点系统在不同位置接入零点后的根轨迹重排](/course-runtime/lessons/3-5/media/3-5-rl-02-high-order-zero-compare.png){width=100%}
...
![PD 与测速反馈的结构图](/course-runtime/lessons/3-5/media/3-5-md-01-pd-rate-structure.png){width=88%}
...
![PD 与测速反馈：等效阻尼可相同，但零点结构不同](/course-runtime/lessons/3-5/media/3-5-rl-03-pd-rate-compare.png){width=100%}
...
所以，`PD` 的第一频域作用并不是抽象的“有零点所以会补角”，而是**主动抬升中高频开环幅值**。开环幅值被抬高后，`0 dB` 交叉点通常向右移动，截止频率增大，闭环响应速度随之提高。课堂上应先建立这条链：
...
![PD 与超前：零点位置可以相同，但频域整形方式不同](/course-runtime/lessons/3-5/media/3-5-rl-04-pd-lead-compare.png){width=100%}

图中右下角已经给出校正前、`PD` 后和超前后的开环幅频与相频对照。读取这组图时，课堂顺序应改成“先看右下角频域图，再回头看上方根轨迹图和左下角阶跃响应”。因为对 `PD` 与超前而言，根轨迹差异是结果，频域整形才是起因。
...
这里给的是一般原则和步骤，不展开模块4那种完整频域整定计算。当前课次的任务是让学生先明白“为什么要这样设计”，而不是把全部数值流程一次做完。

### 4.4 频域设计下的适用规律：什么时候优先想 `PD`，什么时候优先想超前

把这一节压缩成工程判断语言，可以得到下面这张表。

| 结构 | 更适合的场景 | 不适合的场景 | 课堂口径 |
| --- | --- | --- | --- |
...
![右半平面零点首次引入：这就是非最小相系统](/course-runtime/lessons/3-5/media/3-5-rl-05-nmp-compare.png){width=100%}
...
### 5.5 [AI融入点] 用 AI 做一次“最小相与非最小相”辨认练习
...
![本讲信息图总结](/course-runtime/lessons/3-5/media/3-5-info.png)
