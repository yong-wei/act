...
![封面漫画：线性主干的边界](/course-runtime/lessons/5-1/media/5-1-cover-comic.png)
...
![非线性边界现象总览](/course-runtime/lessons/5-1/media/5-1非线性边界总览插图.png){width=100%}

导入图把同一条线性信号路径放进多个工程边界中：执行器可能被机械止挡截断，小指令可能落入死区，继电器可能在阈值附近突然通断，滞回与间隙会让回程路径不同，摩擦会吞掉起动动作，速率限制会拖慢变化过程，数字实现还可能把连续量变成分级输出。这些现象共同揭示了线性理论默认的工作范围。
...
边界识别的重点在于先看清线性主干在哪些地方开始失真，再判断问题还能否留在线性框架内修正，或需要进入非线性、复杂链路、数据驱动等新的分析视角。
...
![典型非线性环节的边界特性对照](/course-runtime/lessons/5-1/media/5-1-boundary-static-characteristics.png){width=100%}
...
当 $|v|\le u_{\max}$ 时，饱和环节和比例环节没有区别；当 $|v|>u_{\max}$ 时，再增大输入也不能得到更大的输出。饱和会让线性预测高估执行器能力，也可能在积分控制中引出更严重的累积误差。
...
机械间隙和回差的共同特征是“同一输入值可能对应不同输出值”。输入从小到大经过某点，与输入从大到小回到同一点，输出不一定相同。摩擦则会让小作用先被静摩擦抵消，只有作用超过启动条件后对象才真正运动。它们破坏的不只是比例关系，还把过去的运动方向、接触状态和启动条件带入当前响应。
...
模型失配和强扰动不是单一静态环节，却同样会把线性主干推出有效范围。船舶对象中，航速变化、载荷变化、海况扰动、舵机滞后、执行器老化都会使模型参数偏离原值。若扰动把系统推到饱和、死区、切换或新的工作点附近，响应失真就不再只是参数误差，而是多个边界因素共同出现。
...
这条式子说明了线性主干的合法来源：在工作点 $x_0$ 附近，非线性函数可以用切线近似。只要偏离量足够小，线性模型就能给出主要趋势。问题在于，这个近似需要两个条件：函数在工作点附近有可用斜率，输入扰动也没有把系统推出这个小邻域。下面两个例子分别说明合理与不合理的线性化。
...
![合理局部线性化下的时域与频域对比](/course-runtime/lessons/5-1/media/5-1-local-linearization-smooth-compare.png){width=100%}

左侧把小阶跃、大阶跃和正弦输入的响应分层放在同一坐标中。小阶跃下两条曲线几乎重合，大阶跃下线性化系统给出更高的输出。右侧的 Bode 对比中，黑线是 $G_{\text{lin}}(s)$，蓝线和红线是原始非线性系统在小幅与大幅正弦输入下的一次谐波估计；幅值越大，等效增益越低。这个例子说明：合理线性化可以解释工作点附近的主要动态，但不能自动解释全幅值行为。

### 5.2 不合理例子：把继电器切换强行当成比例斜率
...
在 $v=0$ 附近，$\operatorname{sgn}v$ 不是连续函数，也没有普通意义下的导数。若强行写成
...
这个模型并不是从合法导数得到的局部线性化，而是把开关动作误当成比例环节。它会严重低估小输入带来的切换幅值，也会掩盖继电器输出中的高次谐波和阈值敏感性。

![不合理线性化下的时域与频域对比](/course-runtime/lessons/5-1/media/5-1-local-linearization-relay-bad-compare.png){width=100%}
...
边界识别不能只停在“这是不是非线性”的命名上，还要能说出线性预测会低估什么风险。下面按不同非线性类型给出几个实际场景。

### 6.1 饱和执行器：大指令下响应前段变慢

具体例子是航向舵机角度存在上限。设被控对象仍用一阶近似表示：
...
若忽略饱和，则线性预测直接取 $u(t)=v(t)$。若考虑舵机角度上限，则实际执行器输出为
...
![同一线性设计在小信号与大信号下的边界差异](/course-runtime/lessons/5-1/media/5-1-saturation-response-compare.png){width=100%}
...
![死区执行机构下的忽略预测与实际响应对比](/course-runtime/lessons/5-1/media/5-1-deadzone-feedback-compare.png){width=100%}
...
![滞环继电器下的忽略预测与实际响应对比](/course-runtime/lessons/5-1/media/5-1-relay-hysteresis-feedback-compare.png){width=100%}

忽略继电器时，线性模型预测输出平滑收敛到约 $0.33$，控制量逐渐变成一个小的连续值；实际继电器系统在 $0.5$ 附近来回切换，输出形成周期性摆动，控制量在 $1$ 与 $-1$ 之间跳变。这个例子说明，继电器问题的主要风险不是稳态值算错一点，而是线性预测会完全掩盖开关动作、阈值穿越和周期摆动。

### 6.4 速率限制舵机：快速指令下初段动作被拖慢

具体例子是舵机角度没有越界，但角速度受到限制。对象与控制器为
...
![速率限制舵机下的忽略预测与实际响应对比](/course-runtime/lessons/5-1/media/5-1-rate-limit-feedback-compare.png){width=100%}
...
当响应已经由饱和、死区、滞回、继电器、间隙、摩擦、量化、切换或自持振荡主导时，需要引入边界工具。局部线性化、相平面、描述函数、仿真实验和分段模型都属于这类入口。它们的职责是观察边界，不是直接替代全部控制设计。
...
![封底信息图：线性主干边界识别框架](/course-runtime/lessons/5-1/media/5-1-info.png)
