...
![时域分析典型输入与响应对象示意图（待制作）](/course-runtime/lessons/2-2/media/td-01-time-domain-input-response-overview.svg)
...
![一阶系统单位阶跃响应与时间常数示意图（待制作）](/course-runtime/lessons/2-2/media/td-02-first-order-step-time-constant.svg)
...
![二阶系统四种典型响应形态对比图（待制作）](/course-runtime/lessons/2-2/media/td-03-second-order-response-families.svg)
...
![动态性能指标标注示意图（待制作）](/course-runtime/lessons/2-2/media/td-04-time-domain-indices-annotated.svg)
...
![例题一响应曲线与性能指标示意图（待制作）](/course-runtime/lessons/2-2/media/td-05-example-response-with-indices.svg)
...
![由时域指标反推极点约束区域示意图（待制作）](/course-runtime/lessons/2-2/media/td-06-time-spec-to-pole-region.svg)
...
- `2-3` 将把这种理解迁移到频域，建立正弦稳态响应、频率特性与 `G(jω)` 对象；
- `2-4` 将进一步把频率对象翻译成 Bode 图与 Nyquist 图基础；
- `3-1` 将从纯极点视角讨论稳定与高阶动态；
- `3-3` 将转入根轨迹机制，解释极点怎样迁移、性能如何随之变化；
- `4-1` 则把本课的时域指标正式收束为工程约束和可行域表达。
