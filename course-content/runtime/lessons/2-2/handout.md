...
- **学时**：90分钟
- **知识类型**：[C] + [X]
- **前置**：已掌握拉氏变换、传递函数、典型环节与基本结构表达，能够从标准对象出发描述系统输入输出关系
- **后续**：2-3（频率响应基础）、2-4（Bode 图与 Nyquist 图基础）、3-1（纯极点视角下的稳定与动态基础）、3-3（根轨迹机制与基本法则）、4-1（性能指标体系、工程约束与可行域表达）
- **讲义定位**：这份讲义会带你把“典型输入为什么能作为标准测试对象、一阶与二阶响应怎样建立标准时域语言、动态性能指标怎样完成读图与轻量参数判断”这条主线建立起来，为后面的频域分析、极点解释和性能设计打底。

![封面漫画：单元导入](/course-runtime/lessons/2-2/media/2-2-cover-comic.png){fig-pos="H"}
...
![典型输入信号及响应对象对比曲线](/course-runtime/lessons/2-2/media/2-2-td-01-time-domain-input-response-overview.svg)
...
![一阶系统单位阶跃响应与时间常数对比曲线](/course-runtime/lessons/2-2/media/2-2-td-02-first-order-step-time-constant.svg)
...
![二阶系统四种典型响应形态对比图](/course-runtime/lessons/2-2/media/2-2-td-03-second-order-response-families.svg)
...
![动态性能指标标注示意图](/course-runtime/lessons/2-2/media/2-2-td-04-time-domain-indices-annotated.svg)
...
所以，本节的真正收束不是“把四个公式背下来”，而是先完成下面这个认知转变：

> **看到曲线，能想到指标；看到指标，能想到参数；看到参数，能想到极点位置。**

至于如何把这些量正式转成极点区域、根轨迹迁移和设计约束表达，将在模块3与模块4中再系统展开；本节先把标准时域对象与指标语言建稳。
...
![例题一单位阶跃响应与性能指标标注图](/course-runtime/lessons/2-2/media/2-2-td-05-example-response-with-indices.svg)
...
![由时域指标反推极点约束区域示意图](/course-runtime/lessons/2-2/media/2-2-td-06-time-spec-to-pole-region.svg)
...
![本讲信息图总结](/course-runtime/lessons/2-2/media/2-2-info.png){fig-pos="H"}
