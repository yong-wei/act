...
- **本单元任务**：以同一套跨域证据为基础，整理任务排序、约束边界和证据来源，形成后续结构选择与方案起步的共享输入。
...
本讲的起点是把 `3-8 / 3-9` 已经出现过的同一套跨域证据图重新摆到眼前，解释为什么这些图会导向不同的任务排序——而不是重新给指标概念分类。
...
当前工作点下，系统超调量约为 $32.01\%$，调节时间约为 $83.24\,\text{s}$，上升时间约为 $10.33\,\text{s}$。这说明系统能够跟踪，但跟踪质量还不让人接受：
...
幅频图显示当前截止频率约为 $0.1169\,\text{rad/s}$。这意味着系统的有效工作频带并不高，快速性改善空间有限。对客船场景来说，这不单是”带宽偏低”的分析结论，更是一个设计提醒：
...
当前相角裕度约为 $37.43^\circ$，增益裕度约为 $21.91\,\text{dB}$。这说明系统并非没有储备，但也谈不上宽松。对客船场景，更自然的任务语言是把顺序理清：

1. 先让动态过程进入乘客和船体都能接受的区域；
2. 再谈如何缩短调节时间；
...
这个场景的四格联读不需要像主场景那样展开太细，我们只保留最能说明”排序重排”的部分：
...
到这里，两个场景都已经读过一次。更重要的是下面这张”同图异读”对照表。
...
1. 设计入口的第一步是”同图异读”，而不是重新给指标概念分类。
2. 客船航向控制是本讲主场景，稳定平台只用于确认”任务排序会改变”。
3. 同一套跨域证据会因为任务背景不同而导出不同优先级。
4. 任务排序没写清之前，不应急着进入控制结构选择。

---

## 三、指标不是并列名词，而是任务书的最小语言

前面先确认了”排序会重排”这个判断，现在回头解释：为什么同一套图可以被改写成任务书。关键就在于，指标到了设计阶段不再只是并列名词，而要承担不同角色。

### 3.1 三类常用指标，各自回答不同问题

控制系统设计最常调用的指标，可以先归纳为三组最小语言。
...
设计课里，频域指标最重要的作用是提醒我们：任何速度收益都可能伴随储备代价——而不只是”把频域图再解释一遍”。
...
在 4-1 里，它们的定位是：作为后续比较可行方案时的重要评价语言，留到后面单元再与优化方法结合。
...
这句话的问题在于它**没有排序**。没有排序的任务书，实际上没有给后续设计提供任何真正有用的输入。
...
这也是为什么进入设计前必须先做”同图异读”：如果不先看场景差异，就很容易把所有指标都堆到同一层，最后只得到一张”听起来都重要”的空洞清单。
...
4. 本讲只整理最小指标语言，不把这些指标展开成控制器选型或优化算法专题。

---

## 四、把跨域证据提炼为任务表达卡

到这里，顺序应该反过来：不是先给出一张任务表达卡模板再去找图来填，而是先从跨域证据读出排序，再把这些判断整理为后续课次能接走的输入卡。

### 4.1 一张合格的任务表达卡，至少要写清七项内容

为了让这张卡直接服务后续 `4-2 / 4-3 / 4-4`，任务表达卡采用七个字段来写清完整信息：
...
根据第一章读出来的证据，主场景 A 的任务表达卡可以整理为下面这样。
...
对照案例的作用是进一步确认”任务排序会改变”——不需要和主场景平起平坐。因此这里保留一张更短、更聚焦的对照卡。
...
因此，本讲的直接产物是一张后续所有设计动作都要反复回看的输入卡——而不是某个控制器的名字。

### 4.5 本章小结

1. 任务表达卡应当由证据读出来，而不是先验给出。
2. 为了支撑后续课次，本课至少要把对象、目标、最紧矛盾、硬约束、软目标、观察指标和证据来源写清楚。
3. 主场景 A 负责建立任务表达主线，对照案例 B 只用于确认”任务排序会重排”。
4. 本讲产出的是后续设计共享的输入卡，而不是控制器方案本身。

---

## 五、可行域、满意域与最优域必须分层

任务表达卡写出来之后，我们还需要明确一个边界：

> **可行域不等于满意域，满意域也不等于最优域。**

如果这一点没有确立，很容易把本讲误解成”已经开始求最优参数的课”。
...
2. 本讲的任务是先把”什么不能做、什么当前可接受”写清楚，不在本课求最优。
...
到这里，你已经能说出很多指标名称，也能看懂不少图。但真正进入设计判断时，仍然容易掉进几种固定误区。若这些误区没有提前分清，后面的 `4-2 / 4-3 / 4-4` 就容易把方向写反。
...
1. 设计入口的第一步是”同图异读”，先看同一套跨域证据在不同场景下如何写出不同任务排序。
2. 时域指标、频域指标和积分误差类指标分别回答过程接受度、储备与任务频带、累计代价。
3. 真正进入设计后，指标要被重新写成硬约束、软目标和观察指标。
4. 任务表达卡必须由证据读出来，而不是先验给出。
5. 可行域、满意域和最优域属于三层筛选，本课只负责先把边界写清楚。

### 7.2 本讲产出

本讲输出的是一份后续设计必须共享的输入，它回答：
...
这些问题写清楚之后，后面的选型、起步、验证和优化才会围绕同一份任务书展开。

### 7.3 后续延伸

本单元直接承接 `3-8` 的频域判别语言和 `3-9` 的模块出口综合映射。模块3已经解释了结构变化怎样改变系统行为，这里把这套行为语言改写成”工程上到底想要什么”。接下来的推进顺序是：

- 到 `4-2`，我们将继续回答：既然任务排序已经明确，不同控制结构为什么会更适合不同任务；
- 到 `4-3`，我们将继续回答：在任务卡已经清楚的前提下，初始参数方向应先往哪里试；
- 到 `4-4`，我们将第一次面对”方案落地后为什么失败”，并用本课建立的任务表达卡回头检查失败是破了硬约束，还是只差在软目标。

可以把模块4前半段压缩成一句话：

> 先写清任务，再解释结构，然后开始起步，最后用实际结果检验前面写下的判断。
...
若这五题中有两题以上不能独立说清，说明你现在还停留在”会看图”的层次，建议回到第二章至第四章，把双场景同图异读、指标角色和任务表达卡对照再读一遍。

![封底信息图：设计任务表达的证据链](/course-runtime/lessons/4-1/media/4-1-info.png){fig-pos="H" width=100%}
