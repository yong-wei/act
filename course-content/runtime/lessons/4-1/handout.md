...
- **讲义定位**：本讲以同一套跨域证据为基础，整理任务排序、约束边界和证据来源，形成后续结构选择与方案起步的共享输入。
...
本讲的起点不是重新分类指标概念，而是把 `3-8 / 3-9` 已经出现过的同一套跨域证据图重新摆到眼前，再解释为什么这些图会导向不同任务排序。
...
到这里，两个场景都已经读过一次。更重要的不是多记一套案例数据，而是下面这张“同图异读”对照表。
...
为了让这张卡直接服务后续 `4-2 / 4-3 / 4-4`，任务表达卡采用七个字段，而不是只留抽象标题：
...
到这里，你已经能说出很多指标名称，也能看懂不少图。但真正进入设计判断时，仍然容易掉进几种固定误区。若这些误区没有提前分清，后面的 `4-2 / 4-3 / 4-4` 就很容易把方向写反。
...
本讲输出的是一份后续设计必须共享的输入，而不是提前给出已经整定好的控制器：
...
本讲直接承接 `3-8` 的频域判别语言和 `3-9` 的模块出口综合映射。模块3已经解释了结构变化怎样改变系统行为，这里把这套行为语言改写成“工程上到底想要什么”。接下来的推进顺序是：
...
若这五题中有两题以上不能独立说清，说明你现在还停留在“会看图”的层次，建议回到本讲义第二章至第四章，把双场景同图异读、指标角色和任务表达卡对照再读一遍。
