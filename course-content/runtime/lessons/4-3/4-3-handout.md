...
![经典复合控制的基本结构](/course-runtime/lessons/4-3/media/4-3-classic-compound-control-structure.png)
...
![扰动前馈复合结构](/course-runtime/lessons/4-3/media/4-3-disturbance-feedforward-structure.png)

仿真中参考信号保持为零，扰动信号取 $0.004+0.003\sin(0.08t)$，从而单独观察扰动前馈的作用。图中同时给出无前馈、$75\%$ 前馈强度和 $100\%$ 前馈强度三种结果；后续小节沿用 $75\%$ 前馈强度。

![扰动前馈加入前后的响应对比](/course-runtime/lessons/4-3/media/4-3-disturbance-feedforward-comparison.png)
...
![参考前馈复合结构](/course-runtime/lessons/4-3/media/4-3-reference-feedforward-structure.png)

仿真中参考信号采用余弦速度规划的航向变化，扰动通道和扰动信号与前一小节保持一致。图中同时比较无参考前馈、$75\%$ 参考前馈强度和 $100\%$ 参考前馈强度；后续小节沿用 $75\%$ 强度。

![参考前馈加入前后的响应对比](/course-runtime/lessons/4-3/media/4-3-reference-feedforward-comparison.png)
...
![给定滤波复合结构](/course-runtime/lessons/4-3/media/4-3-setpoint-filter-structure.png)

为了凸显给定滤波的作用，仿真把参考信号改为较陡的阶跃命令，扰动通道和扰动信号仍保持不变。对比只改变是否加入 $Q_f(s)$。

![给定滤波加入前后的响应对比](/course-runtime/lessons/4-3/media/4-3-setpoint-filter-comparison.png)
...
![限幅斜率限制与抗积分饱和结构](/course-runtime/lessons/4-3/media/4-3-antiwindup-implementation-structure.png)

仿真采用较大的航向阶跃，使舵角实际进入饱和区。对比对象是“有限幅和斜率限制但没有抗饱和”与“同样限幅和斜率限制，并加入反算抗饱和”。

![抗积分饱和加入前后的响应对比](/course-runtime/lessons/4-3/media/4-3-antiwindup-comparison.png)
