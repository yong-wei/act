...
![典型控制结构的频率特性对比](/course-runtime/lessons/4-2/media/4-2-controller-frequency-characteristics.png)
...
![控制器结构选型决策树](/course-runtime/lessons/4-2/media/4-2-controller-selection-decision-tree.png)
...
![例题 5.1 频域 PI 校正前后对比](/course-runtime/lessons/4-2/media/4-2-example-5-1-correction-quadrants.png)
...
![例题 5.2 超前校正前后对比](/course-runtime/lessons/4-2/media/4-2-example-5-2-correction-quadrants.png)
...
![例题 5.3 滞后校正前后对比](/course-runtime/lessons/4-2/media/4-2-example-5-3-correction-quadrants.png)
...
![例题 5.4 临界比例实验过程](/course-runtime/lessons/4-2/media/4-2-example-5-4-zn-steps.png)
...
因此最终控制器为
$$
C_{\text{PID}}(s)=4.8\left(1+\frac{1}{2s}+0.5s\right)
=\frac{2.4s^2+4.8s+2.4}{s}
=\frac{2.4(s+1)^2}{s}
$$
Ziegler-Nichols 参数常使响应偏快、超调偏大。若任务更重视平顺性，可把 $K_p$ 降到 $70\%\sim85\%$，并适当增大 $T_i$。

![例题 5.4 Ziegler-Nichols PID 校正前后对比](/course-runtime/lessons/4-2/media/4-2-example-5-4-correction-quadrants.png)

右下角根轨迹对应校正后的开环 $C_{\text{PID}}(s)G_4(s)$。原点处有两个重合极点：一个来自对象 $G_4(s)$ 中的积分环节 $1/s$，另一个来自 PID 控制器中的积分环节 $1/s$。控制器还在 $s=-1$ 处引入两个重合零点，所以根轨迹从原点二重极点和对象其余极点出发，并受到 $s=-1$ 附近零点吸引；这也是校正后根轨迹形状明显不同于纯比例控制的原因。
...
![例题 5.5 扰动前馈补偿前后对比](/course-runtime/lessons/4-2/media/4-2-example-5-5-feedforward-comparison.png)
...
![客船航向控制候选结构的开环 Bode 对比](/course-runtime/lessons/4-2/media/4-2-ship-controller-candidates-bode.png)

Bode 图显示，滞后主要补低频，但会降低相角裕度；超前和超前-滞后显著改善截止频率附近的相位；单独 PI 的保守频域初算没有自动带来更好的抗扰结果，比例增益下降会抵消一部分积分收益。

![客船航向控制候选结构的时域证据](/course-runtime/lessons/4-2/media/4-2-ship-controller-candidates-step-disturbance.png)

时域结果显示，超前和超前-滞后明显降低给定阶跃超调，并缩短调节时间；滞后略微降低低频灵敏度，但给定阶跃超调和调节时间变差；PI 的响应更慢，扰动峰值也没有改善。

![客船候选结构的整定效果比较](/course-runtime/lessons/4-2/media/4-2-ship-controller-candidates-summary.png)
...
![信息图总结](/course-runtime/lessons/4-2/media/4-2-info.png)
