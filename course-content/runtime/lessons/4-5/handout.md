...
![封面漫画：无约束候选冲出边界后，被显式约束拉回可用区域](/course-runtime/lessons/4-5/media/4-5-cover-comic.png)
...
![客船航向保持：无约束候选为什么不可直接交付](/course-runtime/lessons/4-5/media/4-5-ship-heading-unconstrained-failure-compare.png){width=100%}
...
![平衡权重下：无约束最优与带约束最优为什么会分出两组不同解](/course-runtime/lessons/4-5/media/4-5-ship-heading-same-weight-constraint-compare.png){width=100%}
...
![同一结构下，权重改变会重排可行解的收益分布](/course-runtime/lessons/4-5/media/4-5-weight-sweep-constrained-summary.png){width=100%}
...
![客船航向保持：同一性能指标函数下，结构改变会把最优点带向不同区间](/course-runtime/lessons/4-5/media/4-5-ship-heading-pid-vs-leadlag-compare.png){width=100%}
...
仍以本讲的客船航向保持对象为例。若某一候选解表现为“调节时间明显缩短、`ITAE` 下降、控制峰值升至 `8.4`、相角裕度降到 `43^\circ`”，我们应当先判断它触碰了哪几条硬约束，并说明为什么这不是只靠再调权重就能忽略的小波动。
...
要求写出对应的不等式，并指出哪些式子适合写进 `nonlcon`，哪些式子适合直接作为上下界。
...
![信息图：从越界候选到带约束可用解的证据闭环](/course-runtime/lessons/4-5/media/4-5-info.png)
