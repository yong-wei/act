...
面对一阶、二阶系统时，我们已经熟悉两套描述语言：

- 在时域里，看快慢、超调量、调节时间和振荡节奏；
- 在频域里，看主要频段、幅值变化和相位滞后。

这些语言在低阶系统上很好用，但工程对象很少停留在二阶。以船舶航向控制为例，执行机构、船体动力学、测量环节和扰动通道叠加后，模型往往会升到三阶、四阶甚至更高。于是有两个问题必须回答：

1. 高阶系统明明含有更多极点，为什么我们仍常常用低阶模型解释它的主要动态？
2. 忽略某些极点，什么时候会带来明显误差，什么时候又仍然合理？

这里讨论的是**纯极点系统**。我们暂不引入零点，也不改变控制器结构，先把一个基础判断写清楚：

> 高阶系统常可用低阶模型理解，是因为有些模态衰减更快，有些极点在关注频段内影响更小。
...
这些结论只回答了系统会不会失控，还没有说明系统会怎么动。要把极点变成分析工具，还要看它们如何进入时间响应。
...
因此，判断高阶系统能否近似成低阶系统，不能只看极点大致落在左半平面的什么位置，还要看两件事：
...
系统阶数升高后，总响应会变成多个模态的叠加。模态越多，分析通常越复杂；但工程上常见的情况是，某些模态衰减极快，只在起始时刻短暂出现，随后迅速退场。长期决定曲线形态的，往往是最靠近虚轴、衰减最慢的模态。
...
三组模型具有同一对主导极点。把它们的阶跃响应写成显式形式，更容易看清附加极点的影响。共同的主导极点对为
...
参考模型的超调量、峰值时间和调节时间都可以直接由主导二阶对给出。把这些结果与 Octave 数值结果并列，可得到下表：
...
如果只观察阶跃响应，最多只能回答“这条曲线是否仍像一个低阶系统”。工程判断往往还要继续追问：
...
这张图的关键是看**偏差出现在哪些频段**。先把三组模型的幅频表达式写出来：
...
图中的彩色竖线直接来自表达式中的因子结构：主导二阶因子决定主频尺度，一阶附加因子决定新的转折频率。
...
“远离带宽时为什么可以近似忽略”还可以写成更紧凑的表达式。对附加的一阶极点因子
...
这给出了频域低阶近似的表达式证据：当附加极点的转折频率远在主要带宽之外时，它在关注频带里对幅值和相位的影响都很小；一旦落入主要带宽，这个近似就会迅速失效。
...
这里的 Bode 图只用来判断附加极点是否已经改写主要频段。Nyquist 判据、Bode 判稳与裕度分析需要更完整的频域语言，超出本节讨论范围。

## 六、卷积如何把“极点语言”带到输入响应里

### 6.1 阶跃响应不是凭空出现的

极点决定了脉冲响应中的模态结构。接着要问的是：面对阶跃输入、斜坡输入等不同激励时，系统为什么仍表现出同一套由极点主导的动态特征？
...
图中上半部分展示若干个延时脉冲响应分量，下半部分展示这些分量逐步叠加后对阶跃响应的逼近。更值得注意的是三个观察：
...
![信息图总结](/course-runtime/lessons/3-1/media/3-1-info.png)
