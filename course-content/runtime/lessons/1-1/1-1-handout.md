...
![封面漫画：单元导入](/course-runtime/lessons/1-1/media/1-1-cover-comic.png)
...
这条船是一艘需要自动保持航向的客船。风浪推偏它，海流带偏它，舵机有延迟，传感器带着测量偏差。我们的任务只有一个：让它的航向尽可能稳定在目标值附近。下面八个问题，就是你将在这门课上逐一学会回答的问题。
...
传递函数告诉你"输入进去、输出出来"的整体关系，但它不告诉你系统**里面**长什么样——决策环节在哪里？执行器在哪里？测量信息从哪里来？

**方框图**弥补了这一点。它把系统的每个环节画成一个方块，用箭头连接信息流动的方向。一个典型控制系统的结构要素大致是这样的：

![基本控制系统结构要素，包含比较点、决策环节、执行器、对象和传感器五个环节，箭头标注信号流动方向](/course-runtime/lessons/1-1/media/1-1-block-diagram-basic.png){width=85%}
<p align="center"><strong>图1-1-1 基本控制系统结构要素</strong></p>

这张图里的几个名词以后会反复出现。**决策环节**把目标要求转化为作用量；**执行器**把作用量变成真实物理作用，比如舵角、电压或阀门开度；**被控对象**是被作用并产生输出的过程，比如船体、电机或炉温系统；**传感器**负责采集输出信息，它可能带来比例变换、动态滞后或测量偏差。把这些角色分清，后面的结构分析和校正设计才不会混在一起。

方框图让你一眼看到：谁在接收信息、谁在发出指令、测量信息从哪里进入结构。当连接关系更复杂时，**信号流图**和**梅森公式**提供了一个更系统的求解工具——不需要手动化简，直接从图上的前向通路和回路写出总传递函数。这两种工具在课程中作为补充求解器使用，在复杂结构中尤其可靠。
...
![一阶惯性环节的阶跃响应曲线，标注时间常数τ和稳态值K，显示指数收敛到目标值的过程](/course-runtime/lessons/1-1/media/1-1-step-response-first-order.png){width=80%}
...
稳定只是底线。更有趣的问题是：当调大某个参数——比如增益 $K$——系统的极点会怎样**连续移动**？极点移动的方向，直接决定了响应是变快还是更振荡。

把不同 $K$ 对应的极点位置画在同一张复平面上，按 $K$ 增大的顺序连起来，就是**根轨迹**。对当前一阶对象来说，增益增大后极点沿实轴继续向左移动。更复杂的二阶对象会出现分支会合、离开实轴等形态。一行代码生成这张图：

```matlab
rlocus(G);                % 绘制极点随增益 K 变化的迁移路径
```

![一阶对象根轨迹图，显示增益增大时极点从对象极点出发并沿实轴向左移动](/course-runtime/lessons/1-1/media/1-1-root-locus-example.png){width=80%}
...
![一阶惯性环节的Bode图，上半部分为幅频曲线（低频段平坦，高频段以-20dB/dec斜率下降），下半部分为相频曲线（从0°逐渐过渡到-90°），标注截止频率位置](/course-runtime/lessons/1-1/media/1-1-bode-example.png){width=85%}
...
![开环控制结构图：输入指令直接进入前向控制动作，控制动作作用于对象，输出即为最终结果——没有测量、没有比较、没有修正回路](/course-runtime/lessons/1-1/media/1-1-open-loop-block.png){width=70%}
...
![闭环控制结构图：输出经测量环节送回比较端，与期望输出相减得到偏差，偏差信息沿前向通道继续作用于对象——形成一条闭合的信息回路](/course-runtime/lessons/1-1/media/1-1-feedback-loop-block.png){width=75%}
...
![单位负反馈闭环系统方框图，控制器C(s)在前向通道，对象G(s)紧随其后，反馈通道H(s)=1直接连接输出到比较点](/course-runtime/lessons/1-1/media/1-1-closed-loop-block.png){width=75%}
...
![一阶惯性环节校正前后在时域、根轨迹和频域三方面的对比：比例反馈使响应更快、闭环稳态值降低，闭环极点沿实轴左移，环路幅频曲线整体上移](/course-runtime/lessons/1-1/media/1-1-gain-comparison.png){width=90%}
...
![开环阶跃响应，输出持续增长，呈现积分+惯性环节的典型斜坡状响应，无超调，无稳态值](/course-runtime/lessons/1-1/media/1-1-example-openloop-step.png){width=80%}
...
![根轨迹图：两条分支从开环极点0和-2出发，沿实轴相向移动，在-1处会合后离开实轴，上下对称展开](/course-runtime/lessons/1-1/media/1-1-example-root-locus.png){width=80%}
...
![开环Bode图：幅频曲线在低频段以-20dB/dec斜率下降（积分环节），中频段转折为-40dB/dec（惯性环节加入），相频曲线从-90°逐渐过渡到-180°](/course-runtime/lessons/1-1/media/1-1-example-bode.png){width=80%}
...
![校正前后在时域、根轨迹和频域三方面的对比：开环对象无法停在目标值上，加入比例反馈后闭环响应收敛，极点位置和环路幅频曲线同步改变](/course-runtime/lessons/1-1/media/1-1-example-correction-triptych.png){width=95%}
...
![本讲信息图总结](/course-runtime/lessons/1-1/media/1-1-info.png)
