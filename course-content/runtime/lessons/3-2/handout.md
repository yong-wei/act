...
- **核心问题**：面对高阶闭环特征方程，如何不显式求根也判断稳定性、识别临界边界，并把结论转成参数可行域
- **学习重点**：列写普通劳斯表，处理“首位为 0”与“全零行”两类特殊情况，求单参数稳定区间，并把劳斯结论翻译成极点、时域与频域中的边界变化
...
面对高阶系统，我们往往先拿到闭环特征方程，却很难像二阶系统那样直接写出全部根。比起根的精确数值，更先要回答的是：系统是否稳定、右半平面根有几个、参数推到哪里会碰到边界。劳斯判据处理的正是这组问题。
...
对于二阶系统，我们往往可以直接求解根的解析表达式。面对三阶、四阶及更高阶系统时，通常先得到特征方程，再分析根的分布态势。此时需要回答以下四个关键问题：
...
这一工具既适用于高阶系统的初步稳定性分析，也支持参数筛选与优化。根轨迹分析与控制器设计，都以劳斯判据给出的稳定区域为前提。
...
同一对象贯穿全文：固定参数值时，它用于建立普通劳斯表；保留参数 $k$ 后，它又用于分析稳定区间与临界边界。讨论特殊情况和多域对应关系时，也始终围绕这条稳定边界展开。
...
此处直接依据系数建立劳斯表。四阶系统的前两行由偶次项系数和奇次项系数依次排列：
...
劳斯判据的核心优势在于：即便不显式求出所有根的具体位置，系统是否稳定、右半平面根数有多少，仍可直接从第一列的符号变化中读取。
...
这样，后面的稳定区间分析就不会退化为机械的不等式计算。

### 2.4 用数值求根做一次交叉验证

为了验证普通劳斯表的判断结果，我们再用数值方法检验这一组系数的所有根。用 Octave 计算
...
2. 讨论参数变化影响时，我们可以自然建立“第一列符号变化”与“极点穿越虚轴”之间的直观联系。
...
3. 固定了同一主对象，为后面的参数稳定区间分析做好准备。

接着保留特征方程中的参数 $k$，让同一张劳斯表从“稳定判定表”转变为“稳定区间确定表”。
...
这就是参数 $k$ 的稳定可行域。劳斯判据因此不仅能判断某一特定系统是否稳定，还能直接给出使系统保持稳定的参数范围。
...
这两个边界点对应不同的临界状态。根可能压到原点，也可能成对位于虚轴上。有了这一区别，特殊情况处理也更容易分辨。

### 3.4 用三个参数点核对这一结论

再选取三个典型参数值，分别代表稳定区域内、稳定边界上和稳定区域外。
...
这组结果与劳斯表分析完全一致。随着 $k$ 增大，共轭根逐步逼近虚轴，在 $k=18$ 时穿越边界，随后进入右半平面。
...
若要进一步讨论超调、振荡频率、响应速度或工程约束，后续分析都应在这一区间内展开。此时，劳斯判据已经从“稳定性判定工具”进一步转成“参数设计入口”。

---

## 四、特殊情况处理：首位为 0 与全零行

普通劳斯表默认每一步都能利用上一行的首项继续递推计算。特殊情况出现在递推链条中途被打断时。下文只处理最常见的两类：
...
关键信息是：第一列变为 $0$，但该行并未全为零。此时可引入一个极小正数 $\varepsilon$ 代替该位置，继续递推：
...
全零行背后对应多种类型的根结构，它们都具有关于原点成对对称的特征。辅助方程常见地对应以下情形：
...
![两类劳斯特殊情况处理方法卡](/course-runtime/lessons/3-2/media/3-2-special-cases-card.svg)
...
关键在于：劳斯表本身并不直接绘制极点位置，但它提供的符号变化、零行信息和辅助方程解，已经足够推断极点落在左半平面、右半平面、虚轴还是原点附近。
...
这意味着时间响应中保留不衰减模态。系统已压到稳定边界，收敛过程不再完整结束。

首位为 $0$ 的短例 $D_1(s)$ 则有一对右半平面共轭根
...
这张表格应与劳斯表结合记忆。它的每一列都在说明同一件事：稳定性变化会同时反映在复平面、时间响应和频率响应之中。
...
的全部根是否位于左半 $z$ 平面。问题也就回到普通劳斯表的稳定性判定流程。
...
这就是在“极点位于 $\operatorname{Re}(s)<-0.5$”这一更强约束下得到的设计参数区间。
...
这组结果表明：约束越严格，可行域越小；约束条件改变时，只需将目标区域平移回虚轴位置，再重新列写劳斯表分析。
...
加入这一新约束后，图 1 中靠近虚轴的那段蓝色轨迹需要整体排除，仅保留更靠近左侧的部分。这正是代数区间收缩在复平面上的几何意义。
...
对任意 $k$ 均成立，因此 $s=-1$ 始终是一个闭环极点。这意味着，若进一步要求
...
至此，劳斯判据已经完成从“稳定性判定工具”到“参数设计入口”的转换。进行根轨迹分析与校正设计时，参数先落入劳斯判据确定的可行域，再在此基础上讨论动态优化与结构选择。

![参数可行域求解流程图](/course-runtime/lessons/3-2/media/3-2-parameter-range-flow.svg)

图 5. 从特征方程到受区域约束的参数可行域求解流程。若附加复平面区域约束，应先进行变量平移，再回到劳斯表第一列条件的求解。

![单元内容信息图总结](/course-runtime/lessons/3-2/media/3-2-info.png)
...
建议先独立完成下面四项核验，再使用 AI 做结果比对。AI 更适合帮助我们检查漏项、核对推导链，并定位误判位置，而不是直接替代整段推导。

1. 只根据主对象的劳斯表第一列条件，重新推导稳定区间 $-2<k<18$。核验时重点检查自己是否遗漏了 $s^0$ 行给出的条件 $k>-2$，以及是否在处理 $s^1$ 行分式时先判断了分母符号。
2. 将短例 $D_1(s)$ 与 $D_2(s)$ 的处理步骤并排写出，明确区分“首位为 0”与“全零行”的触发条件、处理动作和物理含义。核验时重点看自己是否把两类情况混成同一套算法。
3. 遮住表 4 的后三列，只保留“劳斯现象”一列，自行补写对应的极点结构、时域表现与频域线索。核验时重点看自己能否区分“原点根”和“纯虚根”。
4. 将“全部极点满足 $\operatorname{Re}(s)<-0.5$”转化为平移后的特征多项式，再重新列写劳斯表，检验新区间是否确实比 $(-2,18)$ 更窄。核验时重点确认变量代换写成 $s=z-\sigma$。

完成以上四步后，可再回看主线：劳斯判据先给出稳定底线，再给出边界类型，最后把问题推向“区域约束下的参数可行域”。
...
![稳定判据相关人物与学术脉络配图](/course-runtime/lessons/3-2/media/case-03-figure-01.jpeg){width=50%}
