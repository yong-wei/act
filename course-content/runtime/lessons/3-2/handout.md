...
- **本课产出**：能够列写普通劳斯表，处理“首位为 0”与“全零行”两类特殊情况，求单参数稳定区间，并把劳斯结论翻译成极点、时域与频域中的边界变化
...
选择同一对象家族有两个目的：先用固定参数值建立普通劳斯表，完成高阶系统稳定判定的基本操作；再保留参数 $k$，把同一张表推进到稳定区间与临界边界分析。这样在讨论特殊情况和多域对应关系时，无需频繁切换分析对象，注意力始终聚焦于稳定边界。
...
这里不必急于求解具体的根，直接依据系数建立劳斯表。四阶系统的前两行由偶次项系数和奇次项系数依次排列：
...
下面保留特征方程中的参数 $k$，让同一张劳斯表从“稳定判定表”转变为“稳定区间确定表”。
...
这就是参数 $k$ 的稳定可行域。由此可以看出，劳斯判据不仅能判断某一特定系统是否稳定，还能直接给出使系统保持稳定的参数范围。
...
这两个边界点说明，稳定区间的端点对应多种临界状态。根可能压到原点，也可能成对位于虚轴上。理解这一区别后，再看特殊情况处理会更清楚。
...
若要进一步讨论超调、振荡频率、响应速度或工程约束，后续设计工作都应在这一区间内展开。到这里，劳斯判据已经从“稳定性判定工具”进一步转成“参数设计入口”。

---

## 四、特殊情况处理：首位为 0 与全零行

普通劳斯表默认每一步都能利用上一行的首项继续递推计算。特殊情况出现在递推链条中途被打断时。这里仅处理最常见的两类：
...
这里最关键的一点是：劳斯表本身不直接绘制极点位置，但它提供的符号变化、零行信息和辅助方程解，已经足够推断极点落在左半平面、右半平面、虚轴还是原点附近。
...
的全部根是否位于左半 $z$ 平面。这样问题就回到普通劳斯表的稳定性判定流程。
...
到这里，劳斯判据已经完成从“稳定性判定工具”到“参数设计入口”的转换。进行根轨迹分析与校正设计时，参数首先落入劳斯判据确定的可行域，再在此基础上讨论动态优化与结构选择。
...
建议先独立完成下面四项核验，再使用 AI 做结果比对。AI 的作用是检查漏项、核对推导链并帮助你发现误判位置，而不是直接替你完成整段推导。
...
完成以上四步后，再回看这一讲的主线：劳斯判据先给出稳定底线，再给出边界类型，最后把问题推向“区域约束下的参数可行域”。
