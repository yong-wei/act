...
- 核心问题：如何将“控制器解决问题”的单回路视角扩展为“自主系统链路协同”的系统视角，明确控制在 MASS 中承担的位置、接口与责任边界

![封面漫画：复杂自主系统链路中的控制位置](/course-runtime/lessons/5-3/media/5-3-cover-comic.png)
...
![MASS 感知、估计、规划与控制协同链](/course-runtime/lessons/5-3/media/5-3-mass-coordination-chain.png){width=100%}
...
![控制在复杂自主系统链路中的位置](/course-runtime/lessons/5-3/media/5-3-control-position-in-chain.png){width=100%}
...
传感器噪声的影响可以从一条普通闭环链路中直接看出来。对象实际输出 $y(t)$ 可以是相当平滑的曲线；测量环节叠加噪声后，反馈量 $y_m(t)$ 出现高频抖动，误差 $e(t)=r(t)-y_m(t)$ 随之抖动，控制器输出 $u(t)$ 被迫包含高频成分，执行机构也会出现频繁小幅动作。此时我们在控制器输出端看到的是“控制太忙”，但源头可能只是反馈量质量不足。

![传感器噪声在闭环链路中的传播与滤波缓解](/course-runtime/lessons/5-3/media/5-3-sensor-noise-filter-chain.png){width=100%}
...
![传感器测量延迟导致航向滞后与航迹偏移](/course-runtime/lessons/5-3/media/5-3-sensor-delay-heading-track.png){width=100%}
...
控制语言可以给规划提供约束反馈。例如最大舵角 $\delta_{\max}$、最大舵角速率 $\dot{\delta}_{\max}$、允许横向加速度、最小转弯半径和跟踪误差上界，都可以转化为规划层的可行域。一个合理的自主系统不应先规划出不可执行的轨迹，再要求控制器“强行跟踪”。
...
![规划航迹、航向参考、控制器输出与实际舵角的对比](/course-runtime/lessons/5-3/media/5-3-planning-path-control-comparison.png){width=100%}
...
![执行器饱和与变化率限制对控制量、阶跃响应和航迹的影响](/course-runtime/lessons/5-3/media/5-3-actuator-limits-response-track.png){width=100%}

左侧曲线把同一个理想控制量依次经过幅值饱和和变化率限制。这个仿真故意采用较急的航向阶跃，理想舵角会明显超过 $\pm 16^\circ$ 的舵角边界；叠加 $5^\circ/\mathrm{s}$ 的舵速限制后，实际舵角不仅峰值被截断，上升和回落也明显变慢。中间的阶跃响应说明，同一套控制律在执行约束下会表现出不同的响应速度。右侧航迹说明，执行约束不是“控制信号后处理”的细节，它会改变船体真实运动路径。若规划层和诊断层忽略这一点，就会把可实现性不足误读为控制器本身性能不足。
...
![MASS 链路诊断顺序信息图](/course-runtime/lessons/5-3/media/5-3-diagnostic-sequence-infographic.png){width=100%}
...
![自主避碰中的感知、估计、规划与控制传播场景](/course-runtime/lessons/5-3/media/5-3-autonomous-avoidance-scene.png){width=100%}
...
![不同转弯半径下的舵角指令与实际航迹](/course-runtime/lessons/5-3/media/5-3-turning-radius-saturation-comparison.png){width=100%}
...
同一自动化等级下，感知、估计、规划、控制和执行仍可能有不同实现方式。一个远程控制系统可能拥有充分的感知与估计能力，但规划决策主要由岸基人员完成；一个高度自动化系统可能在航迹规划中使用复杂算法，但底层运动控制仍采用经典反馈控制。我们阅读 MASS 系统时，应同时看自动化等级、责任主体和技术链路。

![MASS 四类自动化程度的场景对比](/course-runtime/lessons/5-3/media/5-3-automation-levels-scene.png){width=100%}
...
复杂自主系统的分析重点是链路责任判断。出现偏差时，应先沿感知、估计、规划、控制、执行和监督逐层定位，再决定是否调整控制器。若参考不可执行、状态不可信或执行器触边，单纯提高控制增益通常会扩大风险。控制工程师在 MASS 场景中的价值，不只是设计一个性能更强的控制器，还包括把控制可行域、稳定裕度和执行边界明确地反馈给系统链路。

![信息图：MASS 链路责任边界](/course-runtime/lessons/5-3/media/5-3-info.png)
