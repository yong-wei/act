...
- **讲义定位**：我们不把“稳态误差”写成公式清单，而是顺着一条完整判断链展开：误差从哪条通道来，为什么型别决定误差阶次，为什么纯增益只能压小部分误差而不能消除结构性误差，以及为什么引入 `PI` 与滞后后还要重新审视时域与频域代价。
...
因此，我们先解决“误差怎样列式、怎样求”，再看为什么必须用积分、`PI` 或滞后去改结构，而不能只停留在调增益上。
...
设单回路负反馈系统中，控制器为 $G_c(s)$，对象为 $G_p(s)$，反馈环节为 $H(s)$。给定输入为 $R(s)$，扰动 $D(s)$ 加在对象输入端，总输出为 $C(s)$。我们先写出比较点误差
...
若原题给出的并不是这种标准骨架，而是较长的方框图或含局部环节的结构图，可以先借助 `2-1` 与“方框图结构表达”中的等效化简工具，把结构整理成“给定通道 + 扰动通道”的统一形式，再进入误差分析。
...
这个例题不仅给出结果，也为后面用型别与静态误差系数法复算同一题做了铺垫。
...
一旦题目显式出现扰动通道，仍应优先回到“扰动作用下的误差传递函数 + 终值定理”这条完整路径。

#### 回到例题 1：再用静态误差系数法复算

由前面的开环传递函数可见，该系统含有两个积分环节，因此是 II 型系统。于是
...
单位反馈下，直接按“给定通道闭环传递函数”和“扰动通道闭环传递函数”的一般式代入，可得
...
因此，面对“给定 + 扰动”共同作用的问题，我们不能只背型别表，更不能把两个信号当作“同一位置的两个输入”机械叠加。
...
这也正是第三章与第四章之间的过渡桥梁：  
这一部分先解决“为什么要改结构”；下一讲再把这些结构变化翻译成完整的频域判断语言。
...
因此，闭环主导极点必须位于阻尼比边界内侧，并落在 $\operatorname{Re}(s)=-0.333$ 左侧。我们的目标不是“把增益调大一点”，而是让补偿后的根轨迹穿过上述可行域，同时把系统型别提高一级。
...
由于原点处的附加极点是固定的，设计自由度主要体现在零点位置。把零点从远左逐步移向原点，观察根轨迹与可行域的交点后，可选定 $s=-0.3$ 作为折中位置。这样做有两层含义：

1. 通过新增积分环节把系统提升为 II 型，从结构上消除斜坡稳态误差；
2. 通过把零点布置在实轴左半平面，重塑根轨迹骨架，使闭环极点落入前面由超调量与调节时间共同确定的可行域。

下图给出了纯增益与 `PI` 设计的根轨迹、阶跃响应和斜坡跟踪对比。
...
若仍只用纯增益，由前面的速度误差系数关系可知需要 $K \ge 10$。
但由前面的闭环极点表达式可知此时阻尼比只有
...
若只用纯增益，由前面的速度误差系数关系，若要满足 $K_v \ge 10$，必须取 $K \ge 10$。
...
把 `PI` 方案与 `PD` 方案的频域核验结果并列起来，可总结为：
...
这正是下一讲 `3-8` 的入口：把这里已经出现的“更准”和“代价”完整翻译成三频段语言，即低频看精度，中频看速度与稳定余量，高频看噪声与执行器负担。
...
我们仍保留 `MATLAB/Octave` 复现脚本，但不在讲义中展开长脚本代码，以免打断正文阅读。
配套资料中保留两类脚本：
