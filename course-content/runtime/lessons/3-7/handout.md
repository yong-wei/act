...
- **讲义定位**：本讲不把“稳态误差”写成公式清单，而是建立一条完整判断链：误差从哪条通道来，为什么型别决定误差阶次，为什么纯增益只能压小部分误差而不能消除结构性误差，以及为什么进入 `PI` 与滞后后必须重新用时域/频域语言审视代价。

![封面漫画：单元导入](/course-runtime/lessons/3-7/media/3-7-cover-comic.png)
...
设单回路负反馈系统中，控制器为 $G_c(s)$，对象为 $G_p(s)$，反馈环节为 $H(s)$。给定输入为 $R(s)$，扰动 $D(s)$ 加在对象输入端，总输出为 $C(s)$。本讲首先讨论比较点误差
...
![给定通道与扰动通道的统一闭环结构](/course-runtime/lessons/3-7/media/3-7-error-dual-channel.png){#fig:3-7-channels width=92%}
...
> 它反映的是控制系统的本质结构。改变的是分子部分，说明分子反映的是选择的信号通道。

因此，后面无论讨论输出还是误差、给定还是扰动，先盯分母看结构，再盯分子看通道，逻辑就不会乱。
...
这个例题的意义不只是算出结果，更重要的是为后面用型别与静态误差系数法复算同一题做铺垫。
...
终值定理给出稳态误差的通用求解路径，型别与静态误差系数法则是在标准结构上做快速判断。  
静态误差系数法只适用于单回路标准负反馈系统；一旦结构中显式出现扰动通道、附加反馈环节或非标准比较点，就应回到终值定理重新列式。
...
这里故意把前向通道拆成两个环节：$G_1(s)$ 表示执行机构，$G_2(s)$ 表示被控对象，扰动加在两者之间。  
它对应的物理意义是：执行器已经按给定发出了作用，但对象入口又额外受到外界推偏。  
...
![例题 2 的双输入结构图](/course-runtime/lessons/3-7/media/3-7-example2-structure.png){#fig:3-7-example2-structure width=82%}
...
因此，面对“给定 + 扰动”共同作用的问题，不能只背型别表，更不能把两个信号当作“同一位置的两个输入”来机械叠加。
...
所以，若题目要求的是结构性误差改善，就必须引入修正结构的控制器。  
本讲后半段的两条典型路径正是：
...
这也正是第三章与第四章之间的过渡桥梁：  
本章先解决“为什么要改结构”；下一章再把这些结构变化翻译成完整的频域判断语言。
...
两者都能增强低频能力，但逻辑并不相同。若把上一课的超前校正也一起纳入比较，则可以统一成一条判别规律：先看新增零极点相对原点的先后顺序，再看系统型别是否变化，就能同时判断它对低频精度和对数幅频折线的影响。
...
![PI、滞后与超前的零极点和转折频率对比](/course-runtime/lessons/3-7/media/3-7-low-frequency-compensators.png){#fig:3-7-lowfreq fig-pos="H" width=96%}
...
这里首次在非阶跃给定下讨论“时域性能指标”，观察对象要从输出 $y(t)$ 切换为误差 $e(t)$。  
更准确地说：超调量与调节时间仍由单位阶跃输出 $y(t)$ 衡量；而斜坡输入下的精度指标，则由误差 $e(t)=r(t)-y(t)$ 的稳态值衡量。
...
先由稳态误差指标判断纯增益法是否在结构上可行。该对象是 I 型系统，纯增益下速度误差系数为
...
这说明：纯增益无论多大，都只能把斜坡误差压小，无法把它变成零。  
...
因此，可行域可概括为：闭环主导极点必须位于阻尼比边界内侧，并落在 $\operatorname{Re}(s)=-0.333$ 左侧。设计目标不是“把增益调大一点”，而是让补偿后的根轨迹穿过上述可行域，同时把系统型别提高一级。
...
![纯增益局限与 PI 时域设计](/course-runtime/lessons/3-7/media/3-7-pi-time-domain-design.png){#fig:3-7-pi-time fig-pos="H" width=96%}
...
> 当题目要求消除结构性误差时，第一步不是调增益，而是判断是否必须提高型别。
...
因此，单纯增益法虽然能把 $K_v$ 拉高，却会把极点推向低阻尼区域。
...
![滞后校正的根轨迹与时域验证](/course-runtime/lessons/3-7/media/3-7-lag-time-domain-design.png){#fig:3-7-lag-time fig-pos="H" width=96%}
...
这说明纯增益法若想获得足够的相位裕度，只能通过降低截止频率来实现；可一旦这样做，又无法同时满足速度误差系数要求。  
因此，本题必须引入积分环节。
...
![PI 频域设计的逐步结果](/course-runtime/lessons/3-7/media/3-7-pi-frequency-design.png){#fig:3-7-pi-frequency fig-pos="H" width=96%}
...
![频域设计下 PI 与 PD 的时域响应对比](/course-runtime/lessons/3-7/media/3-7-pi-pd-comparison.png){#fig:3-7-pi-pd fig-pos="H" width=100%}

从图中可以直接看到：

- `PI` 的阶跃响应更温和，斜坡跟踪最终更准；
- `PD` 的阶跃响应更快，过渡过程更“利落”，但斜坡误差仍保留有限偏差；
- 因而二者并非谁“更高级”，而是分别对应“低频精度优先”和“动态速度优先”的两条设计取向。

### 3.6 本章收束：为什么这会自然过渡到 3-8

`PI` 与滞后都在强调低频；`PD` 与超前则更多作用在中频与相位。  
因此，当我们问“为什么更准”和“代价落在哪里”时，答案已经开始从时域和根轨迹转向频域：

1. 低频决定稳态精度；
2. 中频附近决定截止频率与相位裕度；
3. 补偿器的零极点相对位置，决定了到底是在改善结构性误差，还是在交换速度与阻尼。

这正是下一讲 `3-8` 的入口：把这里已经出现的“更准”和“代价”完整翻译成频域判别语言。
...
6. 频域下的 `PI` 与 `PD` 展现出“低频精度优先”与“动态速度优先”的不同设计取向，下一讲 `3-8` 将把这组差异翻译成统一频域语言。

---

![本讲信息图总结](/course-runtime/lessons/3-7/media/3-7-info.png)
...
本讲仍保留 `MATLAB/Octave` 复现脚本，但不再在讲义中展开长脚本代码，以免打断正文阅读。  
配套资料中保留两类脚本：
