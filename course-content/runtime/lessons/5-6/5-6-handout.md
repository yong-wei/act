...
![封面漫画：单元导入](/course-runtime/lessons/5-6/media/5-6-cover-comic.png)
...
疫苗和生物制剂冷藏仓通常需要把储存温度维持在 $2^\circ\mathrm{C}$ 至 $8^\circ\mathrm{C}$，并通过连续监测、记录和异常处置降低失效风险。这个温区事实来自 CDC 疫苗储存处理资料；WHO 关于受控温度链的资料也强调，离开标准冷链条件必须有明确批准、条件和责任边界。这里只借用这些事实确定任务约束，不把冷链管理扩展成法规课。
...
![48 小时冷链仓库运行日志](/course-runtime/lessons/5-6/media/5-6-temperature-log.png){width=92%}

## 二、二状态热模型与可复现实验

以下数值结论来自本地固定种子仿真，随机种子为 `5606`，采样周期为 $1\ \mathrm{min}$，仿真长度为 $48\ \mathrm{h}$。外部资料只支撑温区与责任背景；曲线、模型、扰动、指标和结论均由本地数据生成。
...
这组参数不代表药品仓库的真实设计参数，只作为比较用的固定对象。它的作用是让三类方法面对完全相同的对象、扰动和评价指标。路线比较时，改变的是控制方法，不改变热模型。

![二状态热模型与控制作用](/course-runtime/lessons/5-6/media/5-6-two-state-thermal-model.png){width=86%}
...
### 3.3 策略学习监督层：四模式阈值策略，避免黑箱控机

这里的策略监督层没有训练一个直接控制压缩机的强化学习策略。当前实现是可解释的四模式监督策略，用于说明策略学习进入安全关键系统时应放在什么层级。它只能读取可观测量，状态向量为

$$
x(k)=\left[T_m(k),\,T_p(k),\,\hat H_{60}(k),\,T_{\mathrm{amb}}(k)\right].
$$

其中 $b(k)$ 是仿真中用于制造传感器偏置的隐藏故障量，不是监督层可直接读取的输入。这个区分很重要：若监督层已经知道传感器偏置，它就不应在 E3 中低估热风险；这里正是用 E3 检查“读数看起来正常，但真实空气温度偏高”时策略监督层的覆盖缺口。
...
这个设计说明了两件事。第一，策略学习不应直接接管压缩机，底层仍然要经过 PI/PID、限幅、回退和温区检查。第二，训练覆盖不足会暴露风险：E3 中传感器读数偏低时，监督层会低估真实热风险；仿真中用“前馈项乘以 0.78”表达这种覆盖缺口。
...
![三路线同题比较](/course-runtime/lessons/5-6/media/5-6-three-route-comparison.png){width=94%}
...
策略学习监督层只在动作模式选择难以用固定规则表达时有价值。这里把策略动作限制为四种上层模式：
...
![风险-验证矩阵](/course-runtime/lessons/5-6/media/5-6-risk-verification-matrix.png){width=86%}

图中的“证据要求等级”是本案例用于比较方法迁移代价的工程证据维度，它不是自动控制领域的标准性能指标。1 级只需基线模型、参数和边界说明即可复核；2 级还需说明数据来源和适用范围；3 级需要独立测试集、压力工况和故障注入；4 级需要安全壳、拒绝条件、上线监控和可回退机制。系统越依赖数据和策略选择，越需要证明数据覆盖、异常行为和人工接管边界。

## 五、方法选择说明卡

本单元的实践产出是一张方法选择说明卡。它不要求写完整控制器代码，而要求把路线选择写成可审查的工程判断。
...
![信息图总结](/course-runtime/lessons/5-6/media/5-6-info.png)
