...
![封面漫画：单元导入](/course-runtime/lessons/5-5/media/5-5-cover-comic.png)
...
这些压力并不意味着显式控制器失去价值。航向稳定、舵角限制、执行器保护、故障降级和安全监控仍然需要可解释的控制结构。需要判断的是：在某些决策或动作选择环节，人工预先写出完整规则的代价是否已经过高，样本和反馈能否提供更可靠的策略形成依据。
...
![显式控制器与学习策略的责任差异](/course-runtime/lessons/5-5/media/5-5-explicit-vs-policy.png){width=100%}
...
学习策略不能被理解为“没有控制器”——策略本身仍是动作生成机制，只是结构和参数来自训练。控制目标、约束、安全边界、异常降级和责任说明并未消失。若这些内容没有写清，策略学习只是把显式控制器中的困难转移到了样本和验证环节。
...
![强化学习小例子：横向误差修正仿真](/course-runtime/lessons/5-5/media/5-5-rl-toy-demo.png){width=100%}

仿真使用固定随机种子 `5505`。左图显示 20 轮平均累计回报从负值逐渐升高，说明策略在状态和动作之间形成了更有利的对应关系。右图从同一起点比较三种行为：随机动作会在误差区域来回游走；显式比例修正直接按照误差方向修正；学习策略经过训练后也能把横向误差压到接近零。这个例子要说明的是“状态、动作、回报、策略”如何在控制语境中落地，而不是证明强化学习优于经典控制。
...
| 验证条件 | 是否具备独立测试、压力测试和故障注入条件 | 训练表现好，闭环部署证据不足 |
...
| 在仿真中探索策略 | 仿真与真实对象存在差距 | 必须完成模型校准和真实部署前验证 |
...
我们以航向保持任务为固定背景来讨论强化学习。目标航向为 $\psi_d$，船舶当前航向为 $\psi_k$，控制器需要让航向尽快、平稳、少打舵地接近目标，并在风浪流扰动下保持稳定。航向误差写成
...
更合理的起步路线通常是后两类，而不是一开始让神经网络完全接管舵机。越靠近舵机，动作越需要可解释、可限制、可降级；越靠近复杂环境适应，策略学习才更容易体现价值。复杂边缘场景因此额外比较一种混合路线：RL 不直接输出舵角，而是在“保守、常规、抗扰、快速”四组 PID 参数之间选择，实际控舵仍由带限幅和舵速约束的 PID 完成。
...
以下仿真使用固定随机种子 `5505`。训练场景随机化 $T$、$K$、$d_r$、初始航向和目标航向，评价场景再引入更强扰动、参数失配和舵效下降，用来观察策略是否只记住了单一场景。
...
![航向控制强化学习训练过程](/course-runtime/lessons/5-5/media/5-5-rl-heading-control-training.png){width=100%}

航向训练与评价采用 $DT=0.5\,\mathrm{s}$、每轮 $600$ 步的仿真时域，单轮覆盖约 $299.5\,\mathrm{s}$。左图显示直接控舵策略的训练回报移动平均值从低位上升，说明策略在边缘工况训练中形成了更有效的状态-动作对应关系。中图把同一策略放入安全外壳后的触发次数同步画出：回报改善不能单独作为成功证据，安全干预是否频繁、是否集中在边界状态，同样需要被记录。右图显示 RL 调度 PID 的训练回报，用于对比“直接输出舵角”和“只选择 PID 参数档位”两条路线的学习难度。
...
工程上还可以采用课程学习：先在静水、低噪声、小角度任务中训练，再加入强扰动、参数漂移、舵机延迟和传感器噪声。这样做的目的是让训练覆盖逐渐接近真实闭环边界，而不是为了让算法看起来更复杂。
...
因此，推荐的工程路线是：RL 先输出参考航向、PID 参数修正或扰动补偿量；若要尝试直接输出舵角，也必须经过安全外壳。仿真中的安全外壳采用如下规则：当策略建议导致舵角越界、舵速越界、误差继续放大或角速度过高时，控制量退回到限幅 PID 输出，并记录一次安全退化。
...
![航向控制边缘场景时间线](/course-runtime/lessons/5-5/media/5-5-rl-heading-control-edge-scenarios.png){width=100%}

边缘场景先单独可视化。上图给出目标航向，中图给出等效偏航扰动，下图给出对象参数漂移。第 $18$ 到 $46$ 秒出现持续侧风或横流，第 $72$ 到 $96$ 秒出现反向扰动，第 $122$ 秒附近出现短时脉冲；同时船体等效时间常数逐渐增大，舵效增益逐渐下降。只有把这些场景画出来，后面的响应曲线才有明确读图依据：同一段误差恢复到底来自目标变化、外界扰动、对象变慢，还是策略自身动作过激。

![航向控制策略评价：响应、舵角与安全外壳](/course-runtime/lessons/5-5/media/5-5-rl-heading-control-evaluation.png){width=100%}

评价场景不再采用理想阶跃航向保持，而是把四种方法放到同一个复杂边缘场景中。固定 PID 使用常规参数，不在评价过程中重新整定；RL 调度 PID 每一步只选择 PID 参数档位，不直接绕过舵角限幅和舵速约束。长时域评价的作用，是让航向响应经历扰动、参数漂移和恢复尾段，而不是在尚未稳定时就截断曲线。

表 9. 航向保持仿真评价指标

| 方法 | RMS误差 | 最大超调 | 调节时间 | 平均舵角 | 平均舵速 | 安全退化 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| PID | 4.43° | 3.68° | 169.0 s | 1.78° | 0.15°/s | 0 |
| RL直接 | 30.62° | 58.04° | 280.5 s | 10.71° | 2.90°/s | 0 |
| RL安全外壳 | 6.94° | 22.12° | 295.0 s | 6.25° | 2.69°/s | 42 |
| RL调度PID | 4.20° | 4.06° | 169.0 s | 2.06° | 0.24°/s | 0 |

这组复杂边缘场景说明三点。第一，直接控舵的 RL 最终能够回到目标附近，但中段超调达到 $58.04^\circ$，RMS 航向误差显著高于固定 PID。这说明“最终误差小”不能替代完整时域评价；若仿真在 60 秒附近截断，反而会误判策略是否正在走向稳定。第二，安全外壳能够把部分危险动作拉回 PID 输出，但它并不能把底层策略自动变成好策略；本例中安全退化触发 $42$ 次，调节时间接近整个评价窗口末端，说明边界状态仍然需要被独立记录。第三，RL 调度 PID 的表现更接近工程入口：它的 RMS 误差低于固定 PID，调节时间与固定 PID 相同，平均舵速只有直接控舵 RL 的约十二分之一，并且实际舵角仍由带限幅和舵速约束的 PID 结构产生。

综合评价仍然支持“RL 调度 PID”作为更合理的工程入口：它让策略学习选择参数档位，把可解释的 PID 结构、执行器约束和长时域验证保留下来。它的不足也清楚：最大超调略高于固定 PID，说明档位切换带来了更积极的动态响应。这个结果不支持简单地用 RL 取代 PID，而支持让 RL 先承担参考生成、参数调度、扰动补偿和异常场景辅助等任务；若要直接接管舵机，训练覆盖、动作解释、安全退化记录和独立验证都必须显著加强。
...
![本讲信息图总结](/course-runtime/lessons/5-5/media/5-5-info.png)
