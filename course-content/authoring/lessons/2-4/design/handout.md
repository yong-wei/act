...
> **学时**：2（100 分钟）
> **知识类型**：[X] 跨域型
> **前置**：2-1（传递函数、典型环节、结构图）；2-3（正弦稳态响应、频率特性、$G(j\omega)$）
> **后续**：3-4（极点作用与根轨迹综合实验中的频域观察）、3-5（零点引入与动态改善）、3-8（频域判别与跨域综合语言）
> **讲义定位**：本讲义承担本主题的唯一事实来源责任。正文先把“为什么频率特性还要进一步画成图、Bode 图到底在画什么、对数频率坐标为什么必要、Nyquist 图为什么是复平面映射图”讲清楚，再进入典型环节的基本绘制法则、手工绘图入门和基础反向识别入口。判据、裕度、补偿设计和完整系统辨识不在本讲展开。
