# 单元 2-4 | Bode图与Nyquist图基础——从频率特性到图形语言

- **层次**：模块2 理论主线 · 第四课
- **学时**：90分钟
- **知识类型**：[X] 跨域型
- **前置**：已掌握传递函数、典型环节和结构图的基本含义，并理解正弦稳态响应、频率特性与 $G(j\omega)$ 的物理意义
- **后续**：3-4（极点作用与根轨迹综合实验中的频域观察）、3-5（零点引入与动态改善）、3-8（频域判别与跨域综合语言）
- **讲义定位**：这份讲义会带你把频率特性正式翻译成 Bode 图和 Nyquist 图的图形语言，学会先看整体趋势、再抓典型环节、最后对标准对象做粗粒度识别。
...
这正是本节课要做的事。  
`2-3` 把频率特性对象建起来；`2-4` 则把这个对象翻译成**图形语言**。从这里开始，频域分析不再只是“代值求一个复数”，而会变成“看图判断一个系统在整个频率范围内的行为”。

所以，这一课的任务不是再发明一套全新的知识，而是完成一次表达升级：

- 上一课回答的是：系统对某个频率怎么响应；
- 这一课回答的是：把所有这些响应放在一起以后，整体形状长什么样。
