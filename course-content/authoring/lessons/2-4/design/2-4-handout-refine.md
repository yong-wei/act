...
- **讲义定位**：本讲将上一讲建立的 Bode 图对象进一步推进到 Nyquist 轨迹、频域指标入口和基础反向识别，使"频率特性"真正成为可读、可画、可反看模型的图形对象。
...
在 `2-3` 中，我们完成了一项重要任务：频率特性 $G(j\omega)$ 不再只是一个抽象的复数，而是能够分解为两张图来观察：
...
然而，工程师不仅关心"幅度如何变化"和"相位如何变化"这两个分离的问题，更想知道：
...
2. 读取纯极点系统 Nyquist 图的起点、终点、方向和转角；
3. 在图上读出截止频率、穿越频率、带宽频率、相位裕度和增益裕度；
4. 进行"先识别对象轮廓，再给出参数粗略判断"的最小反向识别。
...
当 $\omega$ 从低到高连续变化时，复数点就在复平面上连续移动，形成一条轨迹——这条轨迹就是 **Nyquist 图**。

初次接触 Nyquist 图时，首先要建立的不是稳定性判据，而是以下等价关系：
...
对纯极点系统而言，随着频率升高，相位滞后会持续累积。极点数量越多，累积滞后越大，在复平面上的轨迹整体旋转幅度也越大。
...
至此，我们已经能够观察整体轨迹，并进行基本的双图对照。但在工程实践中，还需要更凝练的量来回答：
- 图形距离"危险边界"还有多远？
- 哪个对象的余量更充足，哪个更紧张？
- 哪个频率是读图时最值得关注的参考点？

频域指标正是为此而来。这里只讲解指标的定义、图上读取和基础计算，不将其升级为闭环系统结论、稳定性判据或校正设计准则。
...
此外，**带宽频率** $\omega_b$ 是系统响应速度的一个直观入口。对于闭环频率特性 $T(j\omega)$，通常将幅值下降到低频值的 $\dfrac{1}{\sqrt{2}}$（即 `-3 dB`）处定义为带宽频率。它侧重于"系统能跟上多快的输入变化"这类读图直觉。

**表：频域指标及其解读**

| 指标 | 图上如何查找 | 本讲仅允许达到的程度 |
|------|------------|--------------------|
...
`2-3` 已完成典型环节 Bode 图的初步学习，本讲不重复"为何使用对数轴、为何使用 dB、典型环节的总体趋势"等内容。手工绘图仅涵盖两项：
1. 将典型环节骨架推进到**简单组合对象**；
...
2. 绘制**基准线**。若存在积分环节 $\dfrac{1}{s^r}$，则先用
   ...
   $$
   绘制第一个转折频率之前的幅频骨架。每个积分环节都使幅频图在 $\omega=1\ \text{rad/s}$ 处经过 `0 dB` 线，并从一开始就贡献 `-20 dB/dec` 的斜率；
3. 按从小到大的顺序标出所有转折点；
...
最容易遇到的困难不是"转折后斜率如何调整"，而是"第一笔应从哪条线开始"。只要对象含有积分环节，就应先问自己：

> **是否已先用 $K/s^r$ 确定了 $\omega=1$ 处的基准点和初始斜率？**

### 6.3 Nyquist 图绘图顺序：先画正频率支，再标注关键点与渐近线

对 **Nyquist 图**，首次绘图建议按以下六步进行：
...
关键不在于"计算大量点"，而在于"先将基准项确定正确"。由于对象同时包含常数增益和积分环节，不能将低频幅频图错误地画成水平线，而应先从 $K/s$ 出发建立骨架。
...
这个对象包含积分环节。一旦轨迹具有渐近线，仅观察起点和终点是不够的，必须进一步计算关键点。
...
这组频域指标表明该对象在当前参数下的相位裕度和增益裕度均不宽裕。相位裕度和增益裕度在这里先被视为"距离边界还有多远"的初步表达语言，并非最终裁决依据——更深入的频域稳定性分析留待后续展开。

> **先读懂这些数值来自何处、分别表示什么，是使用它们的前提。**
...
在实际工程中，系统不会只面对单一的理想输入信号。例如，船舶航向控制系统既要跟随驾驶员缓慢的操舵指令，也要应对海浪、风扰和传感器噪声等复杂工况。如果只会逐点计算 $G(j\omega)$ 的数值，虽然也能得到结果，但很难快速回答以下更接近实际工程判断的问题：

- 系统最敏感的频率段是什么？
- 哪个对象的稳定性余量明显更紧张？
- 哪个图形特征已经在提示"此处接近稳定性边界"？
- 如果只得到一张图形，能否先识别出对象的大致类型？

工程师需要同时掌握两种图形方法，正是因为：

- **Bode 图**将问题分解，便于观察幅值与相位各自如何随频率变化；
- **Nyquist 图**将问题综合，便于观察整体轨迹如何靠近关键边界；
- 两者相互配合，才能使频率特性从"会计算一个点"提升为"会解读一个系统"。

表面上在学习绘制图形，实质上是在培养一种更成熟的观察习惯：
...
4. **截止频率、穿越频率、带宽频率、相位裕度和增益裕度等概念在本讲首次引入，仅达到图形读取和基础计算的程度，尚未升级为稳定性判断或设计语言。**
...
进入模块3后，关注点将从"图形如何绘制"转向更深入的问题：为什么极点、零点、系统类型和相位特性会使图形呈现特定变化？这些图形变化如何反过来服务于系统的结构和机理判断？
...
将 **Bode 图**手工绘图视为一个固定的工作流程，建议按以下顺序执行：
