...
| ### 3.1 先选设计点，而非先猜零点 / ### 3.2 用相角条件反推零点位置 / ### 3.3 用模值条件求增益，并用 `Octave` 验证 | formula+design | $$s_d=-1.1\pm j1.67$$，$$G_{PD}(s)=K(1+T_d s)$$，以及 `T_d≈0.35`、`K≈1.00` 的验收链。 | `step-07` | `static+workspace` | 三面板工作区只负责调参、验收和记录，不省略设计点与控制器公式。 | `3-6-pd-design.png` | 设计点、控制器形式和验收指标必须同时可见。 |
