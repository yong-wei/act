...
这正是自动控制原理要回答的问题。从嫦娥探月到深海装备，从智能制造到交通调度，凡是需要在变化中保持目标的工程问题，控制思想都站在背后。1954年，钱学森在《工程控制论》序言中写道，工程控制论的目的，是把分散在各工程领域中的控制问题纳入一个统一的理论框架来讨论。这句话也恰好说明了这门课的定位——它建立的是一套能跨对象迁移的系统思维，后续学习中的模型、曲线、根轨迹、频率响应和校正设计，都在这个统一的框架里找到自己的位置。
...
其中 $\tau$ 是电机的时间常数，$K$ 是增益。这个式子告诉你：转速的变化率 $\dot{y}(t)$ 和当前转速 $y(t)$ 共同决定了下一时刻的转速。微分方程的好处是直观——它直接描述了"当前状态如何推向下一个状态"。
...
在 MATLAB/Octave 中，这样一行代码就能建立这个传递函数对象：

```matlab
G = tf(K, [tau 1]);   % 传递函数 G(s) = K/(τs+1)
```
...
从数学上看，线性系统的时域响应总可以分解为一组基本运动模态的叠加：

$$e^{-\sigma t}\sin(\omega t)$$

$\sigma$ 决定衰减有多快，$\omega$ 决定摆动有多密。这些 $\sigma$ 和 $\omega$ 就是从传递函数分母中解出来的**极点**的实部和虚部——极点是系统行为最早的暴露者。你在后续课程中会反复看到：极点在复平面上的位置，几乎决定了系统的全部主要动态特征。
...
MATLAB 的 `stepinfo` 函数还可以直接从这条曲线中提取数值指标——超调量、调节时间、上升时间，一行命令全部输出。
...
这三个指标构成了一套通用的评价语言。无论对象是船、电机还是温控系统，同一套指标可以跨对象比较。

### 问题五：这条船会不会失控？

性能指标衡量的是"好不好的程度"。但有一个更基本的问题需要先回答：系统会不会在扰动下发散？会不会越调越偏？

答案藏在极点里。如果全部极点都落在复平面的**左半平面**，系统就是稳定的——扰动引起的偏差会逐渐衰减。如果有极点跑到右半平面，偏差就会被持续放大。

对于高阶系统，直接求根很困难。**劳斯判据**提供了一种代数捷径——不需要求解，直接从特征方程的系数判断是否稳定。它本质上是一个安全检查：在你动手调参数之前，先确认系统不会翻车。

### 问题六：如果拧大增益，极点会往哪儿跑？

稳定只是底线。更有趣的问题是：当调大某个参数——比如开环增益 $K$——系统的极点会怎样**连续移动**？极点移动的方向，直接决定了响应是变快还是更振荡。

把不同 $K$ 对应的闭环极点画在同一张复平面上，按 $K$ 增大的顺序连起来，就是**根轨迹**：

![典型二阶系统的根轨迹图，显示增益从0增大到无穷时闭环极点从开环极点出发、沿实轴相向移动、会合后离开实轴上下展开的迁移路径](../media/processed/1-1-root-locus-example.png){width=80%}
...
把前七个问题中的所有分析对着 $G_c(s)$ 重新做一遍，你会看到：增益 $K_p$ 改变了极点的位置，改变了阶跃响应的快慢和超调，改变了根轨迹的走向，改变了 Bode 图上截止频率的位置。

![同一对象在K_p=1和K_p=5两种增益下的阶跃响应和根轨迹对比，增益增大后响应变快但超调增大，极点向虚轴靠近](../media/processed/1-1-gain-comparison.png){width=90%}
...
| 极点位置 | 固定（对象的固有极点） | 随 $K_p$ 移动，$K_p$ 越大极点越向虚轴靠近 |
| 根轨迹 | 不存在——没有可调参数改变极点 | $K_p$ 从 0→∞，闭环极点沿固定路径迁移 |
| Bode 图 | 固定 | 幅频曲线整体上移，截止频率右移，相位裕度减小 |
...
**保持术语统一。** 目标、输出、误差、扰动、开环、闭环——全课使用同一组词。术语漂移会导致整个概念网络松动。
...
`stepinfo` 的输出验证了这一点：上升时间约 1.2s，但没有有限的调节时间和稳态值。
...
开环对象有两个固有特征：积分环节贡献了低频段的高增益——这对稳态精度有利，因为低频扰动会被大幅削弱；但相角从 $-90^\circ$ 起步，随频率增加进一步滞后——留给控制的相位余量从一开始就不宽裕。
...
```matlab
Kp1 = 0.5;  Kp2 = 2;
Gc1 = feedback(Kp1*G, 1);
Gc2 = feedback(Kp2*G, 1);
step(Gc1, Gc2);
legend('K_p=0.5', 'K_p=2');
```

![两种增益下的闭环阶跃响应对比：Kp=0.5时响应缓慢但平稳收敛到1，Kp=2时响应更快但出现约16%的超调和轻微振荡](../media/processed/1-1-example-closed-loop-comparison.png){width=85%}

$K_p=0.5$ 时，闭环响应缓慢但平稳——从诊断中你知道，这是因为两个闭环极点都在负实轴上，靠近虚轴的那一个拖慢了整个过程。$K_p=2$ 时，响应明显加快，但出现了约 16% 的超调和几次摆动才收敛——从根轨迹中你知道，这是因为增益增大后极点离开了实轴，变成了共轭复数对。
...
```matlab
% 1-1 闪电速通完整示例
G = tf(1, [1 2 0]);         % 定义对象 G(s) = 1/(s(s+2))

% 开环诊断
figure(1); step(G);          % 时域：阶跃响应
figure(2); rlocus(G);        % 复域：根轨迹
figure(3); bode(G);          % 频域：Bode图
figure(3); margin(G);        % 频域：稳定裕度（叠加在Bode图上）

% 闭环校正
Kp1 = 0.5;  Kp2 = 2;
Gc1 = feedback(Kp1*G, 1);
Gc2 = feedback(Kp2*G, 1);
figure(4); step(Gc1, Gc2);   % 对比两种增益的闭环响应
legend('K_p=0.5', 'K_p=2');
```

二十行代码，六个函数调用，覆盖了建模、时域、根轨迹、频域和闭环校正的全部基本动作。你现在不需要完全理解每个函数的输出——后续九章会逐一展开。你只需要知道：这些工具存在，它们围绕同一个对象工作，它们各自从不同角度回答同一个问题——**这个系统表现如何，怎样让它更好。**
...
- （a）"增益 $K$ 从 0.5 增大到 2 后，超调量从 0 增加到 16%。"
...
**习题 3**：$K_p$ 从 1 增大到 5 的过程中，闭环响应变快但超调量同步增大，振荡次数增加。$K_p=5$ 时超调量超过 30%。仅靠增大比例增益无法同时获得"快"和"稳"。

**习题 5（c）**：闭环稳态值未到达 1 的原因：比例控制下，闭环传递函数为 $G_c(s)=K_pK/(\tau s+1+K_pK)$。令 $s=0$，稳态值为 $K_pK/(1+K_pK)=3/(1+3)=0.75$，不等于 1。这个残余偏差就是稳态误差——需要积分控制来消除，单元 3-7 将讨论这个问题。
