...
这三个指标构成了一套通用的评价语言。无论对象是船、电机还是温控系统，同一套指标可以跨对象比较。MATLAB 的 `stepinfo` 函数可以从阶跃响应曲线中直接提取这些数值——一行命令全部输出：

```matlab
stepinfo(G)               % 从阶跃响应中提取上升时间、调节时间、超调量等指标
```
...
| 极点位置 | 固定（对象的固有极点） | 随 $K_p$ 移动；在本例中，增益增大后极点离开实轴，虚部增大、阻尼比降低 |
...
info = stepinfo(G)      % 尝试提取指标；若无有限稳态值，需要谨慎解释
```
...
开环对象有两个固有特征：积分环节贡献了低频段的高环路增益——在负反馈回路中，这通常有利于降低低频跟踪误差或抑制特定通道的低频扰动；但相角从 $-90^\circ$ 起步，随频率增加进一步滞后——留给控制的相位余量从一开始就不宽裕。实际扰动能否被削弱，还取决于扰动进入的位置和闭环结构。
