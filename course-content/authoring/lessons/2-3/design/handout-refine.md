...
在上一课中，我们学习了通过阶跃响应曲线来判断系统"快不快、冲不冲、多久能稳定"的方法。这一视角至关重要。然而在实际工程中，我们很快会注意到另一个现象：
...
这说明系统不仅具有"时间维度上的表现"，还展现出"对不同频率成分的偏好"。系统并非对所有变化一视同仁，而是如同一段具有选择性的通道：某些节奏可顺畅通过，另一些会被显著抑制，还有些虽能通过却伴随相位滞后。

上节课讨论的阶跃响应，其实也已隐含了这一现象。阶跃信号可视为多个频率成分叠加的结果，系统呈现出特定的阶跃响应曲线，本质上源自其对不同频率成分的不同处理。

因此，我们的下一个核心问题是：

> 若将输入分解为不同频率的正弦分量，系统将如何分别处理它们？这些处理结果又如何重组为我们熟悉的时域响应？

这正是**频率响应**所要解决的问题。本讲首先建立频率域分析的基础对象与语言：正弦稳态响应、频率特性、幅值变化、相位变化与核心表达 $G(j\omega)$；继而将其翻译为典型环节的 Bode 图直觉与基础手绘骨架。
...
在上一课中，我们利用阶跃信号研究系统随时间展开的响应过程。这种方法特别适用于回答"最终结果是否理想"。但若进一步追问"系统为何呈现此种表现"或"它为何能跟随慢变化而难以跟上快变化"，仅凭时域曲线便显不足。

一个更细致的观察方式自然浮现：先探讨一个基础问题：

> 若输入本身即为频率固定的正弦波，系统将如何响应？

选择正弦波作为起点，是因为它具备两条重要性质：

第一，正弦波天然携带明确的频率 $\omega$，能清晰表达"变化快慢"。$\omega$ 小，表明变化慢；$\omega$ 大，表明变化快。
第二，对线性定常系统，正弦输入在暂态分量消褪后，输出依然保持**同一频率**，仅幅值与相位发生改变。这使得系统对"特定频率"的处理方式可被独立考察。

频域分析并非另辟蹊径，而是将时域曲线中混杂的不同节奏拆解观察。它关注的核心不再是"某一时刻的输出值"，而是：
...
若系统仅面对单一频率的正弦输入，频域分析自然易于理解。但真实工程系统中，输入远不止纯正弦波：阶跃、方波、锯齿波、周期扰动、非周期指令等均属常见。要将频域分析推广至此类复杂信号，需要借助傅里叶思想。

对**周期信号**，傅里叶级数表明：可将其拆解为若干个不同频率的正弦或余弦分量。
对**非周期信号**，傅里叶变换表明：可将其理解为连续频域内各频率分量的叠加积分。

在控制课程中，关键不在于推演完整的积分过程，而是建立下述逻辑脉络：

1. 复杂信号可分解为不同频率分量；
2. 系统对每个频率分量作出独立响应；
3. 各分量响应叠加，构成最终的时域输出。

先考察方波信号。方波虽非单一频率信号，但可以分解出基波及若干奇次谐波。若系统对高频谐波衰减明显，则方波通过系统后，尖锐边缘变得平滑，波形显得圆润。更进一步，阶跃信号可借助"周期趋于无穷的方波极限"来理解，这意味着时域中观察到的"起步陡度、边缘锐利程度、响应延迟"等特性，都与系统如何处理高频分量密切相关。
...
- 若 $|G(j\omega)|>1$，该频率分量被放大；
- 若 $|G(j\omega)|<1$，该频率分量被衰减；
- 若 $\angle G(j\omega)<0$，输出相对于输入滞后；
- 若 $\angle G(j\omega)>0$，输出相对于输入超前。

于是，"系统偏好何种节奏、抗拒何种节奏"得以具体量化。频率响应不再是抽象概念，而成为一张可按频率查阅的"规则表"。
...
系统对每个频率 $\omega$ 均可给出"幅值缩放比 + 相位偏移量"的回应，将这些回应依频率顺序排列，便形成系统的**频率特性**。
...
这是关于频率 $\omega$ 的复变函数，其模与相位对应两个关键量：
...
可以把"频率特性"理解为按频率索引的规则表。这套规则将在本讲首次绘成 `Bode` 图的基本骨架。
...
若频率特性仅停留于公式表列，依然难以对"低频段表现、高频段趋势、转折位置"形成整体判断。最常用方法是绘制 `Bode` 图。
...
`Bode` 图是频率特性作为客观物理对象首次直观可见的入口。至少应能辨识以下三要素：
...
这个骨架的意义，不是替代精确数字绘图，而是让我们第一次把"频率特性"从抽象的公式对象转变为可直接绘制、观察、比较的图形对象。

### 2.5 从 $G(s)$ 到 $G(j\omega)$：为何允许 $s=j\omega$ 的替换

阅读至此，可能产生一个自然疑问：
在 `2-1` 中我们学习拉普拉斯变换，为何到了频域分析，变量变为 $j\omega$？

这个问题若不澄清，后续所有频域公式都可能被看作"经验性替换"。实际上，核心原因并不复杂。拉普拉斯变量可写作
...
当我们关心**正弦稳态响应**时，关注的不再是含有指数增长/衰减的指数项，而是"频率固定、幅值稳定"的纯正弦振荡成分。此时真正需要保留的是虚轴上的频率信息，故归约至
...
理解这一联系，就不会将频域分析视为另辟蹊径，而是明白：

- `2-1` 建立了如何描述系统对象；
- `2-2` 展示了该对象在时间维度的动态行为；
- `2-3` 揭示了该对象对不同频率刺激的响应规则。
...
这表明，时域中的"快慢、平滑度、延迟"并非与频域无关的孤立现象，而是频率分量处理规则在时域上的综合表现。
...
重点不在于复杂运算，而是要将"代入 $j\omega$"这一步骤与"幅值缩放 + 相位偏移"的物理意义对应起来。步骤如下：
...
由于系统线性，输入中各正弦分量均可独立通过系统，输出为各分量输出之和。本题练习的是"分量独立处理、输出重构"的频域思维方式。
...
低频分量几乎完整通过，高频分量则被大幅衰减，且伴随显著相位滞后。故输出波形中，低频成分占据主导地位，高频抖动仅存微量残留。
...
本章虽包含较多公式，但本质上是"构建对象语言"。真正需要掌握的，是一条可迁移至后续课程的思维脉络：
...
控制系统设计不能简单追求"速度越快、灵敏度越高便越好"，而必须明确分辨"哪些变化值得响应、哪些扰动应当滤除"。学习频率特性，本质上是在训练这种具备边界意识的工程判断能力。
...
这即是频率响应能够成立的数学根基，也是后续所有频域"对象语言"的出发点。

---

## 附录 B | 典型环节频率特性速查表与简要计算

在 `2-1` 中已接触典型环节。本章任务是将这些传递函数对象统一翻译为频域表达式。最直接的方式：将各环节中的 $s$ 均替换为 $j\omega$，然后分别求模与相位角。
...
这张表不是用来死记的，而是助我们形成初步判断：何类环节更注重低频、何类环节对高频更敏感、何类环节会逐渐拖延相位。
...
比例环节**无频率选择性**。无论输入节奏快慢，仅按相同比例放大或缩小。
...
频率越低，积分环节放大作用越强；频率越高，其放大作用越弱。所以**积分环节天然对低频分量更敏感**。
...
在后续课程中，上述关系将进一步推进至 `Nyquist` 图形式、双图对照及频域指标量化；但本章已为这些对象语言奠定 `Bode` 图初步的图形化基础。
