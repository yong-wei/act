...
| step-11 | 避碰转弯半径与舵角可行域分析 | `turning_radius_rust_panel_board` | `rust_turning_radius_panel` | 常显题面和公式，用互动面板动态判断规划半径、舵角上限和安全约束 |
...
| 九、避碰转向过急的链路定位分析 | step-11 | 完整题面 -> 启动距离公式 -> 舵角公式 -> 最小半径公式 -> 互动面板 -> 参数提交 | $d_{\mathrm{start}}$，$\delta_d$，$R_{\min}$ | 参数调节提交 |
...
| turning_radius_case | worked_example_with_curve | 九 | step-11 | mandatory_rust_turning_radius_panel | 完整题面、启动距离、舵角、最小半径、单情形动态曲线、动态启动圈 | 题面和公式常显，互动面板不可降级为静态图 | 学生能调节规划半径并提交满足安全约束的半径与最大舵角 |
...
| step-06 | 信息质量表 -> 噪声传播图 -> 延迟航迹图 -> 诊断判断 | 使用两张静态日志图 | 本页图像承载固定日志证据，不是参数探索对象 |
| step-07 | 规划可行性说明 -> 三路径对比图 -> 舵角负担判断 | 使用 `5-3-planning-path-control-comparison.png` | 本页训练读图，不训练重规划算法 |
| step-08 | 执行约束公式 -> 饱和/速率图 -> 日志字段判断 | 使用 `5-3-actuator-limits-response-track.png` | 本页训练约束证据识别 |
| step-10 | 避碰任务书 -> 五层问题链 -> 场景图 -> 证据定位 | 使用 `5-3-autonomous-avoidance-scene.png` | 无参数面板 |
| step-11 | 题面 -> 公式 -> 互动面板 -> 控件调参 -> 参数提交 -> 结论 | 默认视觉样式镜像 `5-3-turning-radius-saturation-comparison.png`，但只显示当前一种半径情形 | 强制互动面板；下方控件调节规划半径，左侧舵角曲线和右侧启动圈动态更新 |
...
### 页面骨架
- 顶部说明：“本页考察闭环误差、反馈质量和执行器限幅三类基础知识。”
- 下方三题客观题组，单题独立提交。
### 静态承载内容
...
- 中部显示题为“MASS 感知到监督的协同链路”的 `5-3-mass-coordination-chain.png`，图片说明信息流和责任流的阅读顺序，图后放六个环节的职责条。
- 底部放角色匹配题。
### 静态承载内容
- 元素出现顺序：MASS 定义 -> 题为“MASS 六环节责任链”的链路公式 -> 协同链路图及说明 -> 六环节职责条 -> 角色匹配题。
- 主要公式：$\mathrm{MASS}=\text{感知}+\text{估计}+\text{规划}+\text{控制}+\text{执行}+\text{监督}$；公式解释为六个责任环节的组合，不作为数学求和理解。
...
### 页面骨架
- 顶部放题为“闭环控制的输入、控制律与对象输出”的闭环公式和符号说明。
- 中部显示题为“控制层在自主系统链路中的位置”的 `5-3-control-position-in-chain.png`，图后说明控制层的输入来自规划参考和估计状态，输出进入舵、推进或姿态执行器。
- 底部放两张半宽作答卡。
### 静态承载内容
- 元素出现顺序：闭环公式及解释 -> 符号说明 -> 控制位置图及说明 -> 三类职责 -> 两张作答卡。
...
- 中部先放表 1：上游信息质量对控制判断的影响；随后常显题为“反馈测量含延迟和噪声”的测量公式，再显示带标题和具体说明的 `5-3-sensor-noise-filter-chain.png` 与 `5-3-sensor-delay-heading-track.png`。
- 底部放两张半宽诊断卡。
### 静态承载内容
- 元素出现顺序：反馈可信度说明 -> 信息质量表及阅读文案 -> 反馈测量公式及解释 -> 噪声传播图及读图说明 -> 噪声滤波解释 -> 延迟航迹图及读图说明 -> 延迟诊断解释 -> 两张诊断卡。
...
- 噪声滤波解释：无滤波时，噪声从反馈测量进入误差信号，控制器输出和执行机构动作都出现高频负担；加入一阶滤波后，高频成分降低，但滤波会引入相位滞后，过强滤波可能降低闭环响应速度。
- 延迟图读图口令：先看无延迟与有延迟的航向跟随差异，再看航迹偏差是否由反馈时间滞后放大。
- 延迟诊断解释：同一控制器参数保持不变，只把反馈测量换成带延迟的航向测量后，航向修正滞后、避障后横向偏差增大；这条对照证据把问题指向反馈测量时间滞后，而不是直接指向控制器增益不足。
### 互动升级点
...
- 中部显示题为“规划航迹到实际舵角的对比证据”的 `5-3-planning-path-control-comparison.png`，图后分三行解释最短折线路径、频繁重规划路径和平滑可行路径。
- 底部放单选题。
### 静态承载内容
...
### 页面骨架
- 顶部放题为“执行器幅值和变化率约束”的执行器约束说明和两条不等式。
- 中部显示题为“执行器约束改变控制量、响应和航迹”的 `5-3-actuator-limits-response-track.png`，图后按“理想控制量、幅值饱和、速率限制、真实航迹”读图。
- 底部放两张半宽作答卡。
### 静态承载内容
- 元素出现顺序：执行约束说明 -> 幅值/速率公式及解释 -> 控制量与航迹图及读图说明 -> 图后解释 -> 两张作答卡。
- 主要公式：$\left|u(t)\right|\le u_{\max}$，$\left|\dfrac{du(t)}{dt}\right|\le \dot{u}_{\max}$。
- 图后解释：左侧曲线先把同一个理想控制量经过 $\pm16^\circ$ 舵角边界截断，再叠加 $5^\circ/\mathrm{s}$ 舵速限制；实际舵角不仅峰值被截断，上升和回落也明显变慢。中间阶跃响应显示同一控制律在执行约束下响应速度改变，右侧航迹显示船体真实路径随之偏离规划意图。
### 互动升级点
...
- 中部放表 2：MASS 链路诊断的责任分层，并给出阅读文案；随后显示题为“MASS 链路诊断顺序”的 `5-3-diagnostic-sequence-infographic.png`。
- 底部放排序题。
### 静态承载内容
- 元素出现顺序：诊断原则 -> 责任分层表及阅读文案 -> 诊断顺序信息图及说明 -> 混合日志排序题。
...
- 中部放五层问题链，再显示题为“自主避碰中的偏差传播场景”的 `5-3-autonomous-avoidance-scene.png`，图片说明中直接包含图后解释。
- 底部放两张作答卡。
### 静态承载内容
- 元素出现顺序：场景任务书 -> 五层问题链 -> 避碰场景图及图内说明 -> 两张作答卡。
- 五层问题链：感知是否识别目标船、岸线、浮标和禁航区域；估计是否给出速度、方位、风险和置信度；规划是否满足规则、安全距离和操纵能力；控制是否跟踪参考并抑制扰动；执行层是否有舵角、舵速和推进余量。
- 图片说明：蓝色规划路径和橙色实际航迹之间的间隔，可能由延迟、约束和控制误差共同留下。
### 互动升级点
...
- 中部放转弯半径互动面板，视觉样式对齐静态图 `5-3-turning-radius-saturation-comparison.png`，但同一时刻只显示一种规划半径对应的舵角指令和航迹证据。
- 面板下方放规划半径控件和参数提交区；学生通过控件调节 $R$，提交满足安全约束的最新规划半径以及对应最大舵角。
### 静态承载内容
- 元素出现顺序：完整题面 -> 题为“避障启动距离、舵角需求与最小半径”的公式卡 -> 四步链路定位显影 -> 互动面板 -> 规划半径控件 -> 参数提交 -> 责任结论。
- 主要公式：
...
### 四步链路定位显影
- 第一步检查上游输入：感知层已经识别障碍物，估计层给出了相对位置和速度；若没有目标状态完全错误的证据，不能先把偏差归因给感知失效。
- 第二步检查规划参考：规划层按当前半径 $R$ 计算启动圈，船舶进入虚线圆后才开始避障；半径越小，启动圈越靠近障碍物，若该半径不可实现，系统会在过近位置才发现自己转不过去。
- 第三步检查控制与执行：由 $\delta_d\approx\arctan(L/R)$ 可知，小半径会要求超过 $18^\circ$ 的名义舵角；控制器强行跟踪时，舵角被饱和截断，航向响应滞后，横向位移来得更晚。
- 第四步形成链路修正：规划层应把最小转弯半径、最大艏向角速度、舵角限幅和安全距离纳入可行域；控制层给出的物理能力边界需要回流到规划层。
### 互动面板设计
...
- 左侧子图标题为“舵角指令”，只显示当前半径下的估计舵角指令虚线和限幅后的实际舵角信号实线，并带时间轴与舵角轴刻度。
- 右侧子图标题为“避障航线”，保留障碍物、安全边界、当前规划半径对应的虚线启动圈、规划航迹和实际航迹。规划半径改变时，虚线启动圈半径和启动位置动态调整；若半径过小，实际航迹应显示横向位移不足或风险边界触碰。
...
### 互动升级点
- 强制实现参数联动互动面板，不允许降级为静态图或只读计算表。
- 学生提交当前选择的规划半径 $R$、对应最大舵角 $\max|\delta|$、名义舵角 $\delta_d$、是否触发舵角饱和、是否满足安全约束、以及一句链路判断。
...
- `release_activity=separate_toggle`；`open_browse=not_applicable`；`teacher_step_reveal=not_applicable`；`reveal_reference_answer=separate_toggle`。
### 学生默认状态
- `default_visibility=hidden`；题面、公式和互动面板常显，参数提交区需教师发放。
### 预览口径
...
### 页面骨架
- 顶部显示课程信息图 `5-3-info.png`，图片说明：“图中把感知、估计、规划、控制、执行和监督串成诊断链，强调偏差判断要同时看上游信息、规划可行性和执行边界。”
