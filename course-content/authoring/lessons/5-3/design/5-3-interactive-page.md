...
- 中部先放表 1：上游信息质量对控制判断的影响；随后常显反馈测量公式，再显示 `5-3-sensor-noise-filter-chain.png` 和 `5-3-sensor-delay-heading-track.png`。
- 底部放两张半宽诊断卡。
### 静态承载内容
- 元素出现顺序：反馈可信度说明 -> 信息质量表 -> 反馈测量公式 -> 噪声传播图 -> 噪声滤波解释 -> 延迟航迹图 -> 延迟诊断解释 -> 两张诊断卡。
...
### 静态承载内容
- 元素出现顺序：完整题面 -> 启动距离公式 -> 舵角需求公式 -> 最小可实现半径公式 -> 四步链路定位显影 -> Rust 面板 -> 规划半径控件 -> 参数提交 -> 责任结论。
- 主要公式：
