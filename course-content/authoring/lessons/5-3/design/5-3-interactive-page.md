...
### 页面骨架
- 页面标题模块说明：“本页考察闭环误差、反馈质量和执行器限幅三类基础知识。”
- 下方三题客观题组，单题独立提交。
### 静态承载内容
...
### 页面骨架
- 页面标题模块说明规划层面对的是任务空间，控制层拿到的是参考航迹、航向或速度剖面。
...
### 页面骨架
- 页面标题模块说明场景任务：一艘自主水面船在狭窄航道保持计划航线，并遇到横穿目标船，需要完成安全避让。
...
- 第三步检查控制与执行：由 $\delta_d\approx\arctan(L/R)$ 可知，小半径会要求超过 $18^\circ$ 的名义舵角；控制器强行跟踪时，舵角被饱和截断，航向响应滞后，横向位移来得更晚。契约中的显影正文必须保留 LaTeX 标记，不能暴露为裸文本近似公式。
...
### 教师控制
- `release_activity=separate_toggle`；`open_browse=not_applicable`；`teacher_step_reveal=teacher_direct`；`reveal_reference_answer=separate_toggle`。
### 学生默认状态
...
### 页面骨架
- 页面标题模块说明自动化等级描述运行形态和责任主体差异，但技术诊断仍需回到链路证据。
- 中部显示 `5-3-automation-levels-scene.png`，图注说明四个场景的责任主体变化，图后放四类自动化程度对照条。
...
### 页面骨架
- 页面标题模块说明本页检查链路诊断、可实现性判断和责任边界意识。
- 三题题组单题独立提交。
### 静态承载内容
