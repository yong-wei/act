...
设自主船以 $v=4\ \mathrm{m/s}$ 航行，规划层为了绕开目标船，给出不同转弯半径的避障路径。若只看几何路径，$R$ 越小，绕行距离越短；但控制层真正要实现的是相应的转向能力。用简化自行车模型近似船舶平面转向，若等效船长取 $L=34\ \mathrm{m}$，半径 $R$ 对应的名义舵角可写成
...
若该船在当前航速、载荷和海况下的舵角边界为 $18^\circ$，则 $R=35\ \mathrm{m}$ 和 $R=65\ \mathrm{m}$ 对应的规划参考已经超过控制和执行的可行域；$R=140\ \mathrm{m}$ 才给舵角留出余量。控制器即便保持稳定，也难以把船舶拉到不可实现的急转路径上。此时合理的诊断结论应写为“规划参考没有吸收控制与执行约束”。

这个判断可以按四步展开。

第一步，检查上游输入。感知层已经识别目标船，估计层也给出了目标船相对速度和位置；没有证据表明目标状态完全错误。

第二步，检查规划参考。由 $\delta_d\approx \arctan(L/R)$ 可知，半径越小，要求的名义舵角越大。$R=35\ \mathrm{m}$ 和 $R=65\ \mathrm{m}$ 对应的名义舵角分别约为 $44.2^\circ$ 和 $27.6^\circ$，都超过 $18^\circ$ 的执行边界。
...
仿真图把半径 $R=35\ \mathrm{m}$、$65\ \mathrm{m}$ 和 $140\ \mathrm{m}$ 的转弯参考放在同一执行器边界下比较。$R=35\ \mathrm{m}$ 与 $R=65\ \mathrm{m}$ 对应的舵角指令会触碰 $18^\circ$ 饱和边界，实际舵角长期贴边，船舶无法按规划曲率转弯，航迹可能侵入名义障碍物风险区；$R=140\ \mathrm{m}$ 时舵角留有余量，实际航迹更容易保持连续避让。这组曲线说明，转弯半径不是规划层的纯几何参数，它直接决定控制与执行层是否需要贴着边界工作。
