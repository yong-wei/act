...
可按“先数 $P$，再数 $N$，最后算 $Z$”的顺序判断：

| 图形 | 典型对象 | $P$ | $N$ | $Z=P-N$ | 结论 |
| --- | --- | --- | --- | --- | --- |
| A | $\dfrac{6}{(s+1)(s+2)(s+4)}$ | 0 | 0 | 0 | 开环稳定，闭环稳定 |
| B | $\dfrac{0.5(s+0.5)}{(s-1)(s+1)(s+3)}$ | 1 | 0 | 1 | 开环不稳定，闭环仍不稳定 |
| C | $\dfrac{8(s+0.5)}{(s-1)(s+1)(s+3)}$ | 1 | 1 | 0 | 开环不稳定，但闭环稳定 |
| D | $\dfrac{10(s-0.5)}{(s+1)(s+2)(s+8)}$ | 0 | 0 | 0 | 含右半平面零点，但闭环仍稳定 |
...
![三频段分工总览](../media/processed/3-8-three-band-overview.png)
...
![航向控制：基线与超前校正后的对比](../media/processed/3-8-heading-case.png)
...
![稳定平台：激进方案、仅降增益与超前校正的对比](../media/processed/3-8-platform-case.png)
