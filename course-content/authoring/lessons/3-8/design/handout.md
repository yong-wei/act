...
![增益提升前后的频域与时域变化](../media/processed/3-8-gain-effect.png)
...
![左半平面零点引入前后的频域与时域变化](../media/processed/3-8-zero-effect.png)
...
![极点增加或积分补偿前后的频域与时域变化](../media/processed/3-8-pole-effect.png)
...
![右半平面零点带来的相位代价与闭环后果](../media/processed/3-8-rhp-zero-effect.png)
...
可按“先数 $P$，再数 $N$，最后算 $Z$”的顺序判断。为了避免对象公式过长把表格挤坏，这里先把判断结果压成紧凑表，再单独列出各图对应对象。

| 图形 | $P$ | $N$ | $Z=P-N$ | 结论 |
| --- | --- | --- | --- | --- |
| A | 0 | 0 | 0 | 开环稳定，闭环稳定 |
| B | 1 | 0 | 1 | 开环不稳定，闭环仍不稳定 |
| C | 1 | 1 | 0 | 开环不稳定，但闭环稳定 |
| D | 0 | 0 | 0 | 含右半平面零点，但闭环仍稳定 |

四个图形对应的典型对象分别为：

1. A：$L_A(s)=\dfrac{6}{(s+1)(s+2)(s+4)}$
2. B：$L_B(s)=\dfrac{0.5(s+0.5)}{(s-1)(s+1)(s+3)}$
3. C：$L_C(s)=\dfrac{8(s+0.5)}{(s-1)(s+1)(s+3)}$
4. D：$L_D(s)=\dfrac{10(s-0.5)}{(s+1)(s+2)(s+8)}$
...
1. **Gain $K$**：调节控制作用强弱，是本案例最先被尝试修改的参数；
2. **Actuator $G_a(s)$**：把控制指令转成执行机构动作，决定响应速度与部分相位滞后；
3. **Platform dynamics $G_p(s)$**：包含平台本体、负载和惯性耦合，是主要动力学来源；
4. **Unity feedback**：把实际俯仰角直接回送到比较点，形成单位负反馈闭环。
