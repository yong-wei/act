...
- 学生页首屏同时看到路径图、主问题、边界
### 脱离讲稿自包含检查
- 当前页已同时给出模块路径位置、核心问题与课程边界；学生脱离讲稿也能知道 `3-8` 不是新章，而是模块 3 的统一翻译入口。
...
- 不借助讲稿也能知道本课主线和判断动作
### 脱离讲稿自包含检查
- 当前页已把统一翻译链、三步判断法、课程目标和边界同页落下；学生离开讲稿后也能知道本课按什么顺序推进。
...
- 四张独立题卡，允许单卡提交
### 作答卡数量说明
- 本页属于前测题组页，四张卡分别对应四类典型误判，必须独立聚合错因，不能压成两道综合题。
### 教师控制
...
- 学生页看到题面完整，不先给答案
### 脱离讲稿自包含检查
- 当前页已给出误判类型、完整题面、理由要求与 AI 锁定语义；学生不看讲稿也知道前测在检查什么、为什么先首答。
...
### 作答卡数量说明
- 三张卡分别承载频带定位、收益判断、代价判断三种不同动作；若合并，会把“先判频带”再次压扁成泛化结论。
### 教师控制
...
### 作答卡数量说明
- 例题页同时要求学生先判频带，再配对收益与代价；拆成三张卡是为了保持解题顺序，并避免一道综合题掩盖具体误判。
### 教师控制
...
- 每个对象单独一张题卡，避免整页统一提交
### 作答卡数量说明
- 本页本质是四对象快判题组，必须保持“一对象一判断卡”，教师才能定位是哪一类图形或哪一步 `P/N/Z` 判断出错。
### 教师控制
- 可按对象逐卡发放答案
### 学生默认状态
- 图表可见；题卡默认隐藏
...
- 学生可以不看讲稿直接完成快判
### 脱离讲稿自包含检查
- 当前页已给出图组、`P/N/Z` 表、边界提醒和四对象独立判断；学生脱离讲稿也可以直接进入快判，不会把右半平面零点误当右半平面极点。
...
### 作答卡数量说明
- 本页三张卡分别对应裕度正负、边界位置、回拉方向，三者不是同一动作；若合并，会削弱 Bode 读图链。
### 教师控制
- 逐卡放答案，允许教师单独显影解题步骤
### 学生默认状态
- 题面与读图顺序可见；步骤与答案默认锁定
...
- 题卡类型至少包含单选、多选、排序三类
### 作答卡数量说明
- 本页需要依次完成首选频带、可动指标、操作顺序三种任务，再进入 AI 对照；三卡拆开才能保留“先自判、后修订”的顺序。
### 教师控制
...
### 脱离讲稿自包含检查
- 当前页已给出目标切换题面、首答区、AI 对照区与修订要求；学生离开讲稿也知道必须先自己判断，再进入 AI 对照。
...
### 作答卡数量说明
- 三张比较卡分别承载主改频带、关键频域量与时域收益读回，正对应“翻译链”的三个环节，不能压成一句口号。
### 教师控制
...
### 作答卡数量说明
- 本页需要分别说明方案不足、主改频带和最终收益；三卡拆开可避免把工程比较压成一句“超前更好”的口号。
### 教师控制
...
- 后测仍采用独立题卡，不使用整页统一提交
### 作答卡数量说明
- 本页属于后测题组页，四张卡分别检查顺序、边界、频带与非最小相直觉，必须独立统计残余错因。
### 教师控制
- 发放、答案、班级薄弱项三类控制分离
### 学生默认状态
- 题卡默认隐藏，发放后显示
