...
function out = effect_payload(base_sys, new_sys, w, t, label_base, label_new, focus_text, gain_text, cost_text)
  ...
  out.root_base = root_locus_to_struct(base_sys, linspace(0, 12, 320));
  out.root_new = root_locus_to_struct(new_sys, linspace(0, 12, 320));
  out.open_poles_base = complex_vector_to_struct(pole(base_sys));
  out.open_zeros_base = complex_vector_to_struct(zero(base_sys));
  out.open_poles_new = complex_vector_to_struct(pole(new_sys));
  out.open_zeros_new = complex_vector_to_struct(zero(new_sys));
  out.closed_poles_base = complex_vector_to_struct(pole(feedback(base_sys, 1)));
  out.closed_poles_new = complex_vector_to_struct(pole(feedback(new_sys, 1)));
