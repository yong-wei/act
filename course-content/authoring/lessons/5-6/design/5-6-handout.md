...
2. 数据驱动预测补偿：保留 PI/PID 主结构，根据历史开门、入库批次、环境温度和最近温度斜率预测热负荷，提前预冷。
...
实验指标包括越界分钟数、最高空气温度、最高货品核心温度、恢复时间、能耗、压缩机切换次数、告警次数和解释/验证负担。恢复时间的判定标准是：主要热负荷结束后，$T_a$ 和 $T_p$ 同时回到 $5.8^\circ\mathrm{C}$ 以下所需的分钟数。能耗按 $\sum u(k)/60$ 计算，表示 48 小时内的归一化压缩机运行小时数。压缩机切换次数按 $u(k)>0.80$ 的高负荷状态进入和离开次数统计。
...
经典路线没有前馈补偿，$u_{\mathrm{ff}}(k)=0$。参数为

| 参数 | 数值 | 作用 |
| --- | ---: | --- |
| $K_p$ | 0.42 | 温度偏高时立即增加制冷 |
| $K_i$ | 0.010 | 每分钟更新积分项，消除慢性偏差 |
...
其中 $\hat H_{60}(k)$ 是未来一小时热负荷分数，数值越大，说明即将出现的开门、入库和高外温叠加越强。这个预测器使用三类可提前获得或可短时估计的信号：开门计划和记录、入库批次热负荷、未来一小时平均外界温度。
...
本单元的策略监督层没有训练一个直接控制压缩机的强化学习策略。当前实现是可解释的四模式监督策略，用于说明策略学习进入安全关键系统时应放在什么层级。它的状态向量为

$$
x(k)=\left[T_m(k),\,T_p(k),\,\hat H_{60}(k),\,T_{\mathrm{amb}}(k),\,b(k)\right].
$$
...
每次仿真按以下顺序执行：先生成 48 小时扰动日志，再让三条路线分别在同一扰动日志上闭环运行，最后从运行轨迹中计算指标。每条轨迹都记录分钟、空气温度、货品核心温度、压缩机占空比、开门热负荷、入库热负荷、外界温度和监督模式。这样可以保证三条路线面对的是同一任务，而不是三组不同难度的案例。

下表读取自本课仿真输出。解释/验证负担按“低、中、高”标记。
...
第二，入库高峰中，经典 PI/PID 仍能守住 $2^\circ\mathrm{C}$ 至 $8^\circ\mathrm{C}$，但扰动结束后恢复时间为 $22\ \mathrm{min}$。数据驱动预测补偿通过提前预冷，把最高空气温度从 $6.93^\circ\mathrm{C}$ 降到 $6.77^\circ\mathrm{C}$，最高货品核心温度从 $7.44^\circ\mathrm{C}$ 降到 $7.27^\circ\mathrm{C}$，恢复时间降到 $0\ \mathrm{min}$。这说明数据驱动路线的收益来自“可预测热负荷”这一具体缺口，不来自泛泛的算法升级。
...
- 误差从哪里来：$e(k)=T_a(k)-T_{\mathrm{set}}$。
...
数据驱动路线适合补足“模型结构大体可信，但某些热负荷难以固定”的部分。在冷链仓库中，开门记录、入库批次、外界温度和最近温度斜率都能为短时热负荷提供可观察信号。数据模块不必替代 PI/PID，只需给出两类有限输出：

| 数据模块职责 | 输入证据 | 输出内容 | 保留的显式责任 |
| --- | --- | --- | --- |
| 热负荷预测 | 开门、入库、环境温度、温度斜率 | 未来一小时热负荷等级 | PI/PID 主结构、压缩机限幅、温区检查 |
...
> 常规日采用经典 PI/PID 作为主路线。模型结构清楚，温区约束直接，固定设定点即可把空气温度和货品核心温度稳定在合格范围内。数据驱动和策略学习没有明显性能收益，反而增加验证负担。入库高峰采用“经典 PI/PID + 数据驱动预测预冷”的混合路线，因为热负荷具有可提前观察的开门和入库信号，仿真中恢复时间从 $22\ \mathrm{min}$ 降到 $0\ \mathrm{min}$。策略学习只保留为监督层候选，原因是未见扰动中的性能优势不足，且需要额外证明训练覆盖、安全壳、拒绝条件和部署监控。
...
2. 入库高峰中，数据驱动预测补偿的价值来自可提前观察的热负荷信号；在仿真中，它把恢复时间从 $22\ \mathrm{min}$ 降到 $0\ \mathrm{min}$。
...
[8]本单元本地仿真数据：`course-content/authoring/lessons/5-6/media/processed/5-6-cold-chain-summary.json`、`5-6-cold-chain-model-params.csv`、`5-6-route-implementation-config.csv` 及三场景运行轨迹文件。该数据仅用于课堂方法比较，不代表任何真实仓库的合规验证。
