...
- 讲义定位：以 `4-4` 的无约束候选解为起点，先复核它越过工程边界的证据，再把这些边界写成硬约束与罚函数，完成一轮带约束参数优化
- 后续：这里得到的带约束可用解将进入 `4-6` 的场景迁移与边界比较

![封面漫画：无约束候选冲出边界后，被显式约束拉回可用区域](../media/processed/4-5-cover-comic.png)
...
按上一课“优先缩短调节时间、抑制拖尾”的偏好，得到一版无约束候选：

$$
C_u(s)=5\frac{10s+1}{2s+1}.
$$

仅看自由目标项，这版候选的调节时间更短、`ITAE` 更小，`ITSE` 也同步下降。问题恰恰在这里。如果在这一步收手，很容易把“自由目标更优”误读为“方案已经可用”。

因此还要补三件事：

- 对 `4-4` 的无约束候选做工程复核；
- 把不能继续含糊处理的边界升格为硬约束；
- 让求解器在这些边界内重新搜索一版可交付的参数解。

## 二、本次课程目标

完成这一轮实践后，我们应能：

1. 指出无约束候选为何会在真实工程边界下失真；
2. 把超调、控制峰值和相角裕度写成硬约束，而非停留于口头提醒；
3. 区分参数范围与输出侧硬约束在模型中承担的不同职责；
4. 写出罚函数表达，并说明它如何把搜索拉回可用区域；
5. 比较起始方案、无约束候选与带约束可用解的差异，说明约束究竟改变了什么。
...
如果只比较这四项，很容易把 `C_u(s)` 看成一版“足够好”的结果。这正是必须做工程复核的原因。

### 3.2 一旦回到工程边界，问题立刻暴露

把无约束候选放回完整证据中，得到下面这组读数：
...
| 相角裕度 / deg | 49.0462 | 47.8274 | 裕度仍在，但未换来更稳妥的整体结果 |

这张表揭示一个关键事实：`C_u(s)` 并非“完全无效”，改进方向本身是对的；但它把自由目标带来的收益建立在越界代价之上，因此不能直接进入工程实现。

### 3.3 图上最先看见的是什么

把起始方案和无约束候选画到同一张图上，问题会更直观：

![客船航向保持：无约束候选为什么不可直接交付](../media/processed/4-5-ship-heading-unconstrained-failure-compare.png){width=100%}

这张图至少暴露三件事：

1. 无约束候选确实让闭环输出更快收敛到目标，说明上一课的加权目标方向无误；
2. 控制量曲线明显冲过可接受的动作区间，说明求解器把执行机构当作可以无限透支的资源；
3. 即使部分频域读数尚未崩坏，如此剧烈的控制动作已经使“更优”失去工程意义。

下一步要做的，是先写清楚哪些边界绝对不能继续含糊处理，而不是再换一组权重重试。

## 四、硬约束第一次被显性提出

### 4.1 哪三条边界必须升格为硬约束

对客船航向保持，至少要把下面三条边界显性写出来：
...
| 边界 | 挡住的风险 | 若不显性写出会怎样 |
| --- | --- | --- |
| $M_p \le 20\%$ | 把越摆当作可无限交换的提速代价 | 响应过程逐步走向激进 |
| $\max |u(t)| \le 7$ | 把执行机构边界当成可透支资源 | 候选解看似更快，实际无法落地 |
| $\phi_m \ge 45^\circ$ | 只看时域变快，忽视稳健性储备 | 结果对模型偏差更脆弱 |

硬约束先把绝不能接受的解挡在外面，它本身并不评判“什么更优”。
...
参数范围的职责是保证搜索停留在当前超前结构的可解释区间内。

新增的三条硬约束职责不同：它们直接裁决响应过程与工程边界是否可被接受。也就是说：

- 参数范围负责“结果还属不属于当前结构”；
- 硬约束负责“这个结果能不能交付”。

若只保留前者而没有后者，求解器仍会在一个“结构上说得通、工程上却可能失真”的区域里搜索。
...
| 调节时间、`ITAE`、控制能量 | 软目标 | 守住边界后继续争取 |
| `ITSE`、截止频率、幅值裕度 | 复核证据 | 求解后用以解释结果是否可信 |

这一步的顺序不能颠倒。应先把哪些边界不能破写清楚，再决定求解器怎样执行；不应先把求解器搭好，再临时往里塞几个约束名词。
...
上一课写下的偏好并未失效。新增的规则只有一条：偏好必须在边界内争取。
...
- 一旦某条边界被突破，总代价迅速放大；
- 越界越严重，求解器越倾向于把搜索拉回边界内。

引入罚函数的意义在于：它把“边界不能只当注释”真正写进模型本体。
...
两条路径都可行。关键不在工具选型，而在“边界是否已经被显性写入模型”。这一步未完成时，再高级的求解器也只是更快地搜出失真解。
...
求解器此时只承担一件事：在已写清的边界内重新搜索更优参数。

### 6.2 `fmincon` 求解流程拆解

对象、结构、目标函数和硬约束已经写清，剩下要做的是把这些量如何交给 `fmincon` 说透。最小实现分五步。
...
第二步，沿用上一课已确定的性能指标函数，不临时改写权重语义。以下面这组平衡权重为例：
...
由此可见，`fmincon` 并未替我们做设计判断。真正决定结果的是三件事：目标函数怎么写、`nonlcon` 里放进了哪些边界、初值与边界区间是否来自前面的设计证据。

### 6.3 同一权重下，无约束与有约束为什么会分出两组不同解

为说清“约束究竟改变了什么”，固定上一课的平衡权重，不再改动偏好：
...
图和表合起来揭示一个关键事实：约束改变的是可行域，不是权重语言。无约束最优并未违背平衡权重的偏好——恰恰相反，它正是过于忠实地沿着这组偏好继续缩短调节时间和累计误差，才把控制峰值推到 `9.901`。引入 `fmincon` 之后，偏好保持不变，但求解器被迫在“峰值不越界”的可行域里重新找点，得到的 `C_{B,c}(s)` 因此调节时间略长，控制能量明显下降。

由此，同一权重下出现两组最优解的根源，不在于目标函数前后不一致，而在于边界是否被写成求解器必须遵守的规则——这一项直接决定最优点能落在哪里。
...
1. $A$ 与 $B$ 的响应速度已经非常接近，说明硬约束一旦主导，继续把权重推向“更快”一侧，并不会持续换来速度收益；
2. $C$ 组将控制能量与控制峰值进一步降低，但调节时间回升至 `25.4 s`——当权重明显倾向动作代价时，可行解会主动退回更保守的时间尺度。

因此权重并非空话。即使硬约束已经固定，可行域内的收益分配仍受权重重排，只是不再允许“越界换优”的假象混入。

### 6.5 同一主案例下：优化 PID 和优化超前结构为什么会分化

在给定结构下，我们已找到一版最合适的参数解，但起初指定的结构未必就是最合适的。若对象、评价函数与硬约束均不变，仅把控制器结构从超前结构改为 `PID`，最优点会如何移动？

仍以客船航向保持为主案例，沿用平衡权重 $B=[0.30,0.30,0.20,0.20]$ 对应的归一化代价函数和同样三条硬约束。得到的两条最优控制器分别为
...
这一步的看点不在谁“赢了”，而在结构如何改写了可达最优区间：

- 在同一代价函数下，优化超前结构的归一化代价约为 `0.620`，优化 `PID` 的归一化代价约为 `8.092`；
- 为守住同样的控制峰值与稳健性边界，`PID` 被迫把积分作用调得很弱，同时把动态过程拖至 `200 s` 的观察窗口边缘；
- 优化 `PID` 的开环相频在低频段贴近 `-180°` 属于合理结果：对象本身已含一个积分环节，`PID` 再叠加一个积分项，低频开环自然呈现近似两个积分环节叠加的相位特征。

由此可见，参数优化只能回答“在给定结构内如何调最好”，无法回答“这个结构本身是否最合适”。后者由后续的结构优化承接。
...
这张表的价值不在某一列谁更小，而在它清楚呈现了约束如何改变了搜索方向：

1. 无约束候选把时间与误差指标降到很低，代价是极高的动作能量和明显越界；
2. 带约束可用解放弃了激进收益，转而在守住边界后让速度、拖尾与动作代价回到更均衡的位置；
3. `ITSE` 从起始方案到带约束可用解几乎不变，说明当前结果并未通过放大前中段误差强度去换取尾段表现。

因此硬约束的作用并非把优化“做保守”，而是把搜索重新拉回可用设计空间。

### 6.7 参数变化为什么仍然合乎结构机理

带约束可用解相对起始方案的参数变化如下：

- $K$ 从 `2.796` 降到 `1.7679`，整体作用强度明显收回；
- $T$ 从 `10` 调到 `10.4676`，零点仍停留在原有的有效工作频段附近；
- $\alpha$ 从 `0.406` 降到 `0.2527`，超前作用增强，用以提高阻尼与稳健性储备。

三条变化共同说明：硬约束并未把结果推离当前固定结构，反而迫使参数沿着结构职责更合理的方向重新校准——不再依靠放大整体作用强度去换取速度，而在动作幅度、动态品质与稳健性之间重新分配余量。
...
3. 改进收益能在统一证据下被读出，而非仅凭单个指标看起来更好。

当前带约束可用解满足这三条标准，已具备进入后续比较的资格。但它仍非终局，原因也很清楚：控制峰值从 `6.8867` 升至 `6.9960`，仍在边界内，但十分贴近 `7` 的上限。说明当前结构在该场景下依然成立，但余量并不宽松。

更重要的是，本讲还额外揭示两点：

- 同一权重下，是否把边界写进求解器，直接改写最优点的位置；
- 同一性能指标函数下，更换控制器结构会让最优点落到完全不同的代价分布上。

到此处的落点其实落在两个新出现的认识上：参数优化的边界已经看清，结构差异的伏笔也已经露头——而不仅仅是“算出了一组参数”。

## 八、小结

本讲完成了六件关键工作：

1. 对 `4-4` 的无约束候选做了完整工程复核；
2. 把超调、控制峰值和相角裕度升格为硬约束；
3. 用 `fmincon` 把目标函数、边界和参数区间衔接成可执行的求解链；
4. 解释了同一权重下无约束解与带约束解为何会分开；
5. 讨论了权重改变时，可行解如何在速度、拖尾和动作代价之间重新分配收益；
6. 用同一主案例下的 `PID` 与超前结构对比，提前暴露了“结构也会改变最优区间”。

对本讲而言，最值得带走的不是某组参数，而是这条链：
...
仍以本讲的客船航向保持对象为例。某一候选解表现为“调节时间明显缩短、`ITAE` 下降、控制峰值升至 `8.4`、相角裕度降至 `43^\circ`”。请判断它触碰了哪几条硬约束，并说明为何这不是再调一次权重就能掩盖的小波动。

### 练习 2：约束翻译

把下面三句话分别改写成可进入求解器的约束形式，并说明它们属于输出侧硬约束还是参数范围：

1. 超调不能超过 `15%`；
2. 执行机构最大动作幅值不能超过 `6.5`；
3. 控制器参数满足 `1.5 \le K \le 4.5`。

请写出对应的不等式，并指出哪些适合写入 `nonlcon`，哪些适合直接作为参数上下界。
...
1. 为什么同一权重下，无约束最优和带约束最优不会落在同一个参数点上；
2. 带约束解放弃了哪一部分收益；
3. 它换回了哪些真正具有工程意义的改进。

![信息图：从越界候选到带约束可用解的证据闭环](../media/processed/4-5-info.png)

## 附录 A：客船航向保持的 `fmincon` 完整代码

下面给出本讲主案例的一份完整 `MATLAB/Octave` 复现脚本，将对象、目标函数、`nonlcon`、`fmincon` 调用与结果回读集中在一个文件中，便于直接复现。
