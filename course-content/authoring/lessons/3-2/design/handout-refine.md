...
对于二阶系统，往往可以直接求解根的解析表达式。面对三阶、四阶及更高阶系统时，我们通常先得到特征方程，再分析根的分布态势。此时需要回答四个关键问题：
...
劳斯判据正是基于特征方程系数直接回答上述问题的数学工具，提供三类信息：
...
选择同一对象家族有两个目的：先用固定参数值建立普通劳斯表，完成高阶系统稳定判定的基本操作；再保留参数 $k$，将同一张表发展为稳定区间与临界边界分析。这样在讨论稳定区间、特殊情况和多域对应关系时，无需频繁切换分析对象，注意力始终聚焦于稳定边界本身。
...
不必急于求解具体的根，直接依据系数建立劳斯表。四阶系统的前两行由偶次项系数和奇次项系数依次排列：
...
所有元素均为正，第一列没有符号变化。由劳斯判据可得：

$$
\text{右半平面根数}=0
$$

因此该系统稳定。即便不显式求出所有根的位置，系统是否稳定、右半平面根数有多少，都可以直接从第一列的符号变化中读取——这是劳斯判据的核心优势。

### 2.3 为什么"看第一列"就够了

普通劳斯表将稳定性问题转化为第一列元素的符号变化分析。根是否跨越虚轴进入右半平面，最终都会反映到第一列的符号上。

本例第一列始终为正，表明根没有穿过虚轴进入右半平面。若某一步第一列出现负号，而前一项仍保持正号，则符号变化次数增加一次，右半平面根数也随之增加一次。
...
四个根均位于左半平面，与劳斯表结论完全一致。这次验证还有一个附带作用：讨论参数变化影响时，可以自然建立"第一列符号变化"与"极点穿越虚轴"之间的直观联系。
...
得到 $k>-2$。综合三项约束，系统稳定区间为

$$
-2<k<18
$$

这就是参数 $k$ 的稳定可行域。劳斯判据的第二层价值在此体现：它不仅能判断某一特定系统是否稳定，还能直接给出使系统保持稳定的参数范围。
...
这两个边界点表明：稳定区间的端点对应多种临界状态，根可能压到原点，也可能成对位于虚轴上。处理特殊情况时，这种差别将变得尤为重要。
...
图 1 展示了参数从 $k=-2$ 扫描至 $k=22$ 时的极点迁移轨迹。图中可直接观察到两个现象：$s=-1$ 始终保持在实轴上；共轭根在 $k=18$ 附近穿过虚轴，这与劳斯表预测的稳定边界完全吻合。
...
若需进一步讨论超调、振荡频率、响应速度或工程约束，所有精细化设计工作都应在此区间内展开。劳斯判据由此从"稳定性判定工具"升级为"参数设计的基础入口"。
...
普通劳斯表默认每一步都能利用上一行的首项继续递推。特殊情况出现在递推链条中途被打断时。最常见的两类是：

1. 某一行第一列为 $0$，但该行并非全零；
2. 某一整行全部为零。

两类现象看似都"导致计算无法继续"，但物理含义不同。第一类对应递推过程中的分母失效问题，第二类则对应系统更深层次的根结构对称性。
...
第一列变为 $0$，但该行并未全为零。引入一个极小正数 $\varepsilon$ 代替该位置，继续递推：
...
处理要点：$\varepsilon$ 的作用是保持符号的连续性，判断时只需关注第一列的符号变化；出现首位为零时，应继续完成递推，再根据第一列符号变化判断稳定性。
...
整行均为零。这种情况下应改用辅助方程法，因为前一行对应的多项式已暴露出系统具有成对对称的根结构。
...
辅助方程的系数全部取自全零行的上一行，幂次按该行对应的奇偶次幂依次递降。
...
全零行背后对应多种类型的根结构，辅助方程常见地对应以下情形：

- 一对符号相反的实根；
- 一对纯虚根；
- 两对实部符号相反、虚部相同的共轭复根。

出现全零行后，必须先求解辅助方程，才能确定根结构类型。
...
第一种：见到首位为零便立刻将系统归类为临界状态。正确做法是先引入 $\varepsilon$ 继续完成表列，再根据第一列符号变化判断右半平面根数。

第二种：见到全零行就直接判定"系统有纯虚根"。正确做法是先建立辅助方程，再由辅助方程的解确定具体根结构。
...
劳斯判据给出的原始信息是"第一列符号变化"和"是否出现特殊情况"。这些结论需要进一步转化，才能与极点分布图、时间响应特性、频率特性等概念建立联系。
...
劳斯表本身不直接绘制极点位置，但它提供的符号变化、零行信息和辅助方程解，已足够推断极点落在左半平面、右半平面、虚轴还是原点的结构信息。

### 5.2 极点结构一旦确定，时域表现就随之确定

极点结构改变后，系统时间响应的主要特征也会相应改变。

先看普通稳定情形。以 $k=12$ 为例，主对象的根为

$$
s=-3.6759,\quad s=-1,\quad s=-0.1621\pm j1.9448
$$

系统包含一个衰减较快的实模态、一个衰减较慢的实模态以及一对衰减振荡模态。时间响应将收敛至稳态值，若复根距离虚轴较近，还会出现较明显的振荡与超调现象。

再看全零行短例。短例 $D_2(s)$ 的辅助方程给出 $s=\pm j$，时间响应中保留等幅振荡分量，振荡不会自行衰减，系统处于临界边界状态。

主对象在 $k=-2$ 时包含原点根 $s=0$，时间响应中保留不衰减模态，收敛过程不再完整结束。

首位为 $0$ 的短例 $D_1(s)$ 有一对右半平面共轭根

$$
s=0.3002\pm j1.6248
$$

时间响应呈现振荡包络逐渐放大的特征，幅值随时间增长，系统失稳。

图 3 将这几种状态呈现在同一张单位阶跃响应图中。蓝线对应稳定情形，黄色曲线对应临界边界，红线对应失稳情形。极点越靠近虚轴，振荡衰减越慢；极点进入右半平面后，振荡包络开始放大。
...
频域中的许多现象，本质上仍由极点位置决定。

当全部极点位于左半平面时，系统对正弦输入的响应能形成稳定的稳态输出。若存在靠近虚轴的共轭复根，幅频曲线常在对应频率附近抬高，表现出明显的共振趋势。

当极点恰位于虚轴上时，分母在某一频率点变为零。短例 $D_2(s)$ 中 $s=\pm j$ 对应频率 $\omega=1\ \text{rad/s}$，系统在该频率处出现理想共振，幅值趋于无穷。
...
这张表格应与劳斯表结合记忆。每一列都在诠释同一件事：稳定性变化会同步反映在复平面、时间响应和频率响应之中。
...
原先的竖线 $\operatorname{Re}(s)=-\sigma$ 在新变量 $z$ 平面中平移至虚轴位置 $\operatorname{Re}(z)=0$。问题转化为：新特征多项式
...
仅要求系统稳定时，参数区间为 $-2<k<18$。加入 $\operatorname{Re}(s)<-0.5$ 约束后，区间收缩为 $-\frac{3}{8}<k<4$。约束越严格，可行域越小；约束条件改变时，只需将目标区域平移回虚轴位置，再重新列写劳斯表进行分析。

图 1 中的蓝色稳定段展示了稳定可行域对应的极点分布范围。加入 $\operatorname{Re}(s)<-0.5$ 这一新约束后，图 1 中靠近虚轴的那段蓝色轨迹需整体排除，仅保留更靠近左侧的部分——这正是代数区间收缩在复平面上的几何意义。
...
全部位于左半 $z$ 平面，因此原系统所有极点均满足 $\operatorname{Re}(s)<-0.5$。
...
出现右半平面根，表明原系统已有部分极点位于 $\operatorname{Re}(s)=-0.5$ 这条竖线的右侧。

### 6.4 为何这标志着进入设计层面

从 $-2<k<18$ 到 $-\frac{3}{8}<k<4$ 的变化，核心在于设计目标的提升，计算方法本身保持不变。
...
对任意 $k$ 均成立，因此 $s=-1$ 始终是一个闭环极点。若进一步要求 $\operatorname{Re}(s)<-1$，则可行域为空集。变量平移法同样能准确揭示这种"无解"结论。

至此，劳斯判据完成了从"稳定性判定工具"到"参数设计入口"的功能升级。后续进行根轨迹分析与校正设计时，参数首先落入劳斯判据确定的可行域，再在此基础上探讨动态优化与结构选择。
...
这段科学史对控制课程具有直接意义。工程师面对的是"系统是否稳定"这一现实问题，劳斯处理的是"高阶多项式根如何分布"这一数学问题。两者在此处完成了交汇：数学工具进入工程设计的核心领域，工程问题反过来也推动了数学表达方式的演进。如今计算机求根已十分便捷，但劳斯判据仍保有重要价值，因为它训练的是从系统结构、边界条件与可行域角度理解控制问题的能力。
