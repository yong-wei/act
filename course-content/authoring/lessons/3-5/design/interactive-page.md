...
| `### 2.2 例 2：三阶纯极点对象，零点位置不同，主导分支被拉走的方式也不同` | figure+comparison | 三阶纯极点对象在不同零点位置下的根轨迹重排。 | `step-05` | `static+interactive` | 比较两种零点位置下主导分支的变化。 | `3-5-rl-02-high-order-zero-compare.png` | 图与比较句式必须同屏。 |
| `### 2.3 从这两组图立住第一组对照 / ### 2.4 如果把零点加到右半平面，会发生什么` | conclusion+risk | 左半平面零点常带来动态改善趋势，但右半平面零点不能直接照搬这套语言。 | `step-06` | `static+interactive` | 风险预测卡只记录对右半平面零点的第一判断。 | `对照表` | “改善趋势”和“边界问号”必须同时出现。 |
...
| `### 3.2 不先看名字，先看等效阻尼相同的后果` | formula+example | $$\zeta_{PD}=\zeta+\frac{1}{2}K_d\omega_n,\qquad \zeta_v=\zeta+\frac{1}{2}K_t\omega_n$$ 与 $K_d=K_t=0.3$ 的例子。 | `step-08` | `static+interactive` | 工作区负责代入与填写，不替代公式本体。 | `公式卡` | 两个公式、变量含义与例子必须同时出现。 |
| `### 3.3 在根轨迹上看见的，是同样增加阻尼但轨迹骨架不同 / ### 3.4 这一组实例应该留下的课堂结论` | comparison+conclusion | `PD` 与测速反馈的根轨迹、时域、频域对照与三句结论。 | `step-09`, `step-10` | `static+interactive` | 对照卡与结论重组分别承接证据和收束。 | `3-5-rl-03-pd-rate-compare.png` | 三域图和三句结论必须完整显式。 |
| `### 4.1 PD 为什么会让系统更快` | formula+principle | $$|G_{PD}(j\omega)|=\sqrt{1+(\omega T_d)^2},\qquad \phi_{PD}(\omega)=\arctan(\omega T_d)$$ 以及“抬中高频、推交叉、代价是高频放大”。 | `step-11` | `static+interactive` | 给频带打标签，辨认低频/拐点后/高频的作用。 | `PD 幅相特性卡` | 公式、链条与代价必须同屏。 |
| `### 4.2 超前为什么更像在关键位置补角` | formula+principle | $$G_{\text{lead}}(s)=\frac{Ts+1}{\alpha Ts+1},0<\alpha<1$$、$$\omega_m=\frac{1}{T\sqrt{\alpha}}$$、$$\phi_m=\sin^{-1}\left(\frac{1-\alpha}{1+\alpha}\right)$$ 与“相位峰”解释。 | `step-12` | `static+interactive` | 找出相位峰频带并判断补的是相角裕度。 | `超前标准式卡` | 标准式、峰值频率和解释句必须完整出现。 |
| `### 4.3 频域下的一般设计原则 / ### 4.4 频域设计下的适用规律` | design+matrix | `PD` 频域设计原则、超前设计步骤与适用规律表。 | `step-13` | `static+interactive` | 情境排序矩阵只负责把原则用到场景。 | `设计矩阵` | 原则、步骤和场景边界必须同屏。 |
| `### 5.1 先把最小相与非最小相写成一对镜像对象 / ### 5.2 非最小相最先暴露出来的，不是慢，而是先往反方向动 / ### 5.3 频域里为什么更难控制` | concept+figure | 镜像对象、逆响应、额外相位滞后与“更快会更早撞上边界”。 | `step-14` | `static+interactive` | 术语解释区只负责让学生把定义和现象连起来。 | `3-5-rl-05-nmp-compare.png` | 镜像对象式、逆响应图和定义卡必须同屏。 |
| `### 5.4 如何控制不能只说原则 / ### 5.5 AI 融入点 / ### 7.2 让 AI 替你找一句最危险的误判` | risk+assessment+ai | 保守带宽实例、后测纠错、AI 只用于对照危险误判。 | `step-15` | `static+interactive` | 后测后才开放 AI 对照，不代替学生写纠正句。 | `增益对照表` | 后测、风险表和 AI 边界必须同时落页。 |
| `### 6.1 本课到底建立了什么 / ### 6.2 和前后课程怎么接 / ### 6.3 小结 / 附录 A` | summary+checklist | 本课判断语言、四个观察量、信息图总结与 `3-6` 统一对象实验预告。 | `step-16` | `static+reflection` | 退出反思只收一句最重要的观察量。 | `3-5-info.png` | 信息图、四个观察量与下一课去向必须同屏。 |
