...
- 工作区固定要求：
  ...
  - 控件区只保留 `基线 / 添加零点` 两种模式；
  - 基线中的极点固定不可拖动；
  - 添加零点模式中的零点标注可直接在根轨迹面板中拖动；
  - 切换模式后恢复讲义默认位置。

### 互动升级点
- 在统一根轨迹工作区中切换基线与添加零点、拖动零点位置，再完成“被拉走的分支 + 被重排的实轴区段”记录。
...
- 固定公式还必须按名称显式落页：
  ...
  $$
- 页面上需给出对应名称：`被控对象`、`相应的闭环传递函数`、`PD 等效特征方程`、`测速反馈等效特征方程`。
...
### 静态承载内容
- 固定传函：
  先给出 `沿用上一步对象` 与 `理想 PD 控制器`：
  $$
  G_p(s)=\frac{4}{s(s+0.8)},\qquad G_{PD}(s)=1+T_d s
  $$
- 然后按揭示顺序给出 `频率特性`、`幅频特性`、`相频特性`：
  $$
  G_{PD}(j\omega)=1+j\omega T_d,\qquad
...
- 固定公式必须给出明确名称：
  ...
  $$
- 页面上需分别标出：`超前网络标准形式`、`幅频特性`、`相频特性`、`最大超前角出现频率`、`最大超前角`。
...
### 静态承载内容
- 页面顺序固定为：`最小相开环对象 / 非最小相开环对象` 公式在最上方，随后是 `3-5-rl-05-nmp-compare.png` 对照图与现象分析，最后才是 `5.4 保守带宽实例表`。
...
### 静态承载内容
- 固定使用 `3-5-info.png`。
- 信息图区必须把四个观察量收成紧凑卡片：`根轨迹观察量`、`时域观察量`、`频域观察量`、`结构判断观察量`。
