...
- **讲义定位**：这份讲义从"结构变化"的角度理解零点，而不是把它看成一个孤立的名词。我们沿着 `同一对象对照 -> 根轨迹改变 -> 时域变化 -> 频域解释 -> 风险边界` 这条主线，看清零点如何带来动态改善，以及这种改善背后的代价。
...
在 `3-4` 中，我们已经能够沿着一张既定的根轨迹做出完整判断：

- 哪些参数还在稳定窗口内；
- 哪些参数更接近可接受窗口；
- 哪些参数虽然没有失稳，却已经开始用更小的裕量去换取更快的响应。

这条判断链已经很有力量，能让我们在一条既有轨迹上做出选择，却还不能回答另一个更靠近控制设计的问题：

> 如果沿着原来的轨迹继续调增益，已经很难同时兼顾速度、振荡和裕量，我们能不能让轨迹本身换一种走法？

上一课的主语还是"同一条轨迹上的不同点"。这一课的主语开始变成"结构变了以后，轨迹为什么会一起变"。结构变化里最先登场的角色，就是零点。
...
零点的作用之所以重要，是因为它改变的不是某个单一指标，而是三条彼此联动的链：
...
> 零点为什么会让系统的动态过程出现纯增益调整所没有的新变化？

### 1.3 统一对象与四个版本

全文固定围绕同一个基准对象展开，先不代入复杂的船舶整定参数，把注意力集中在"结构变化"本身。
...
前三个版本承担主比较任务。右半平面零点版本只作为边界提醒，不在正文里展开成长篇设计讨论。
...
这三项产出连起来，构成一条完整推进路线：先认出是不是在引入零点，再解释零点怎样改变轨迹和动态，最后判断这种变化值不值得要。

### 1.5 [AI融入点] 先写下你的直觉

进入正文前，可以先自己写下三条判断，再请 AI 和你做对照：

1. 如果只想减小超调，你会先调增益，还是先考虑改变结构？
2. `PD` 和测速反馈看上去都能"增大阻尼"，你会默认它们只是名字不同吗？
3. 如果一个零点能让系统更快，你会直接推断"零点越靠近右侧越好"吗？

先把直觉写下来，再拿给 AI 讨论。AI 适合帮你暴露自己没有说清的假设，但真正的依据仍然要回到后面的根轨迹、时域和频域证据上。
...
这个对象只有两个开环极点 $0$ 和 $-1$，没有零点。根轨迹的两条分支从这两个极点出发，在实轴区间 $(-1,0)$ 上相向运动，然后离开实轴进入复平面——这正是 `3-3/3-4` 中已经熟悉的"纯极点基线"。

现在不改对象，只改零点位置。先看两种最典型的加零点情形：
...
> 纯增益调整只是在既有轨迹上移动；零点引入会改写轨迹的终点分配和实轴区段，因此闭环极点可达区域会跟着一起变。

### 2.2 例 2：三阶对象，主导分支被"拉走"的方式也不同
...
- 同一阶次、同一个对象，只因为零点位置不同，`3-4` 中建立的"关键节点、主导分支、可接受窗口"判断语言就必须重写。

讲"零点吸引根轨迹"，不要停在相角条件的抽象层面。更准确的说法是：
...
这是后面所有"动态改善"讨论的前提。如果这一步没有通过图像真正看懂，后面讲 `PD`、测速反馈和超前时，我们很容易重新滑回"是不是只是另一种调增益"的误判。

### 2.4 如果把零点加到右半平面，会发生什么

前面两组图讨论的都是左半平面零点。如果零点放到右半平面，轨迹还会不会继续朝"更有利的动态区域"重排？输出会不会仍然老老实实地朝目标方向起步？频域里的相位会不会继续像左半平面零点那样给我们帮忙？

先把这三个问题记住。读完第五节的镜像零点实例后再回头看这里，会更容易理解"非最小相"这个概念为什么不是换个名字而已。

## 三、实例 1：`PD` 与测速反馈都能增大阻尼，但零点副作用不同

### 3.1 固定同一个对象，并列比较两种结构

为了避免"对象变化"和"控制结构变化"搅在一起，固定被控对象为
...
此时 $\omega_n=2$，$\zeta=0.2$。校正前的系统本来就偏振荡、偏慢，是很典型的"需要改善动态过程"的对象。

并列比较两种校正方式：

1. `PD` 校正：$G_{PD}(s)=1+K_d s$；
2. 测速反馈：保留外环单位负反馈，同时把输出经测速发电机环节 $K_t s$ 回送到对象输入端，构成输出微分反馈。

两种结构的方框图先放在一起看。上图把误差信号 $E(s)$ 分成"直接通道"和 $K_d s$ 通道，再在对象前相加；下图保留单位负反馈外环，并把输出速度项 $K_t s\,C(s)$ 反馈到对象前的局部加法点：

![PD 与测速反馈的结构图](../media/processed/3-5-md-01-pd-rate-structure.png){width=88%}

这里要特别避免一个常见误解：测速反馈不是"只剩一个 $K_t s$ 的局部反馈环"，而是"外环单位负反馈 + 内部速度项负反馈"同时存在的双层结构。也正因为这一点，测速反馈是通过输出微分信号来调节阻尼，而不是把零点直接放进前向通道。

两种控制都通过一个比例系数来决定"微分作用有多强"：

- `PD` 里由 $K_d$ 决定；
- 测速反馈里由 $K_t$ 决定。

对这个对象，两者都把闭环特征方程改写成同一类形式：
...
> 无论是 `PD` 还是测速反馈，只要调的是对应的微分比例系数，都可以通过同一条公式去调节闭环阻尼。
...
这时两者都会把分母的一次项从 $0.8$ 提高到 $2.0$。但闭环传递函数并不相同：
...
差别立刻出现了：`PD` 在参考输入到输出的通道里显式增加了零点 $z=-3.33$，测速反馈没有。

### 3.2 "等效阻尼相同"的后果

![PD 与测速反馈：等效阻尼可相同，但零点结构不同](../media/processed/3-5-rl-03-pd-rate-compare.png){width=100%}

这张图里，上半部分固定 $K_d=K_t=0.3$，比较的是**校正结构确定之后，外环增益 $K$ 变化时的真实根轨迹**；下半部分把这种"相同的阻尼调节能力"翻译成不同的时域和频域后果。

在相同对象、相同等效阻尼比 $\zeta_{\mathrm{eq}}=0.5$ 下，三条阶跃曲线给出了非常具体的对比：
...
测速反馈和 `PD` 都能把极点推向更合适的阻尼区域，但 `PD` 还在输入通道里额外放入了一个零点，因此前段响应更积极，超调也比测速反馈略大。

频域也要落到指标上。三条闭环频率特性的主要差异：
...
从频率特性看，测速反馈并不是"没有微分作用"——它同样能通过提高阻尼压低共振峰，但因为没有显式前向零点，不像 `PD` 那样把输入通道一起推得更积极。

### 3.3 在根轨迹上看见的，是"同样增加阻尼"但轨迹骨架不同

图中的两个上方面板解释这一点。

- 左图对应 `PD` 校正后的开环传函 $L_{PD}(s)=K\dfrac{4(1+0.3s)}{s(s+0.8)}$，开环有两个实极点 $0,-0.8$，还有一个零点 $-3.33$；
- 右图对应测速反馈等效后的外环开环传函 $L_v(s)=K\dfrac{4}{s(s+2.0)}$，开环仍然只有两个实极点 $0,-2.0$，没有零点。

两种结构的共同点和差异可以同时看清：

- 共同点：二者都能通过 $K_d=K_t=0.3$ 提高阻尼，使同一对象的阶跃振荡明显减弱；
- 差异：`PD` 的一条分支会被左半平面零点拉向 $-3.33$，测速反馈则没有这个终点，两个分支从实轴断开后向无穷远伸展。

这就是为什么不能把测速反馈说成"另一种 `PD`"：从"阻尼结果"看两者可能很像；从"零点进了哪里、根轨迹怎样改写"看，两者并不一样。
...
`PD` 的第一频域作用并不是抽象的"有零点所以会补角"，而是**主动抬升中高频开环幅值**。开环幅值被抬高后，`0 dB` 交叉点通常向右移动，截止频率增大，闭环响应速度随之提高：

> `PD` 先把中高频幅值往上托，交叉频率才有机会右移；交叉频率右移后，系统才表现为"更快"。

这也是 `PD` 和单纯比例放大的关键差别。比例增益是把整条幅频曲线整体平移，`PD` 则是在中高频段继续把斜率往上抬，"更快"带有明显的频段选择性。

但这个好处带着直接代价：理想 `PD` 的高频增益会一直上升，高频噪声、测量抖动和未建模高频动态也会被一并放大。工程上常使用带小惯性环节的近似微分，而不直接保留理想 `PD` 的无限高频放大特性。

### 4.2 超前为什么更像"在关键位置补角"：相位提升只集中在一段频带
...
图中右下角已经给出校正前、`PD` 后和超前后的开环幅频与相频对照。读取这组图时，推荐顺序是"先看右下角频域图，再回头看上方根轨迹图和左下角阶跃响应"——对 `PD` 与超前而言，根轨迹差异是结果，频域整形才是起因。
...
这里给的是一般原则和步骤，不展开完整频域整定计算。目标是先明白"为什么要这样设计"，而不是把全部数值流程一次做完。

### 4.4 适用规律：什么时候优先想 `PD`，什么时候优先想超前
...
两者有同样的极点，零点到虚轴的距离也相同，唯一差别在于：

- 左半平面零点 $z=-1$：最小相；
- 右半平面零点 $z=+1$：非最小相。

如果后面出现的差异很大，就不能再说成"零点只是位置不同而已"。

"非最小相"这个名字在这里一次说清。对稳定对象来说，如果幅频特性已经固定，那么所有零点都在左半平面时，系统所带来的相位滞后是最小的，这类对象叫**最小相**。把零点镜像到右半平面，幅频特性不变，但相位会额外再滞后一截，于是它就不再是"相位最小"的那一类系统，这就是**非最小相**这个名称的来源。
...
这个"先往反方向动一下"的现象，就是**逆响应**。它不是数值偶然，而是右半平面零点把输出初始趋势改写之后的直接后果：

> 稳定系统只要含有右半平面零点，就属于非最小相系统；其最典型的时域信号，就是可能出现逆响应。

### 5.3 频域里为什么更难控制

再看右下角的开环频域图。与最小相对象相比，非最小相对象在相同增益下会出现额外相位滞后。这意味着如果你还想把交叉频率继续往右推，系统会更早遇到相角裕度不足的问题。
...
这组实例把"保守带宽、先保相位、接受速度上限"这组工程动作立住了，完整设计流程留待后续展开。

### 5.5 [AI融入点] 用 AI 做一次"最小相 / 非最小相"辨认练习

把这对镜像对象交给 AI，请它回答：

> 两个对象的极点完全相同，为什么一个可以单调跟踪，另一个却会先反向运动？请不要只说定义，要同时解释时域和频域后果。

看完 AI 的回答后，回到本节的三张图自己核对：

- 它有没有明确指出右半平面零点就是非最小相零点？
- 它有没有把逆响应和额外相位滞后同时说出来？
- 它有没有意识到"更快"在这里会更早撞上带宽上限？

## 六、课末收束：动态改善线到此为止

### 6.1 本课建立了什么

走到这里，建立的不是"控制器种类表"，而是一条稳定的判断语言：

- 当纯增益调整已经很难兼顾速度、振荡和裕量时，应该想到是不是需要改变结构；
...
把本课放回模块3主线里看，它正好处在一个转折点上。

- `3-3` 回答的是"轨迹为什么这样形成"；
- `3-4` 回答的是"怎样沿既有轨迹做读图判断"；
- `3-5` 回答的是"怎样通过零点让轨迹本身出现新的动态可能"。

下一课 `3-6` 会把今天的结论真正拿去做统一对象实验，比较基准、`PD`、测速反馈和简单超前四个版本在三域里的差异。`3-7` 会把主线切换到稳态改善——那时会看到，动态改善和稳态改善不是同一条线，零点和积分环节也不是同一类武器。
...
6. 本课只建立"零点动态改善线"的机理语言，不展开完整超前校正设计，也不提前进入模块4的整定流程。
...
### 7.2 [AI融入点] 让 AI 替你找一句最危险的误判

把本讲的三组核心对照发给 AI，并请它回答：

> 在"纯增益调整 vs 零点引入""PD vs 测速反馈""左半平面零点 vs 右半平面零点"这三组比较里，哪一句我们最容易说错？请只给一句误判，再给一句纠正。

看完 AI 的回答后，不要直接照抄。先自己写出那句误判为什么错，再用课堂里的根轨迹、时域和频域语言把它纠正过来。

![本讲信息图总结](../media/processed/3-5-info.png)
