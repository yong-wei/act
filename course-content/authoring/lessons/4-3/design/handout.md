...
- **讲义定位**：当 `4-2` 的单结构起步卡已经写出来，但仍不足以支撑当前任务时，我们要把它推进成一张**首版可验证方案卡**，并把第一次验证前的预期改善、主要风险和优先怀疑点写清楚。

> `4-1` 把分析证据压成任务书，`4-2` 围绕主矛盾完成单结构起步，`4-3` 把“会选结构”推进到“会写方案”，`4-4` 再用第一次验证检查这张方案卡到底站不站得住。

---

## 一、为什么 `4-3` 必须出现

到 `4-2` 结束时，我们已经能写出一张单结构起步卡。卡片里至少已经有下面几项内容：
...
这一步解决的是“先动哪条机制线”的问题。它把“凭感觉喊控制器名字”推进到了“能讲出结构理由”。但工程设计还要再向前走一步，因为单结构往往只能先抓住最紧的那条矛盾，还没有回答后面两个问题：
...
本讲不追求把参数一次算完。本讲要做到的是：**结构骨架、参数方向和验证假设要同时出现。**
...
这三条证据已经足够说明：当前对象不是“完全做不到”，而是“已经能工作，但工作得不够好”。这类对象最容易让人误判，因为系统有响应、有跟踪、有稳定结论，于是很容易误以为继续沿着原结构慢慢加力就够了。
...
这就是 `4-3` 真正的起点。单结构起步卡已经抓住了第一条机制线，但它还无法覆盖第二条机制线。
...
这组式子现在不需要我们沿着完整计算流程再推一遍。对本讲最有价值的是它释放出的结构信息：
...
`4-2` 的单结构起步写的是 `PI/滞后`，而这里的典型候选骨架更接近“积分环节 + 超前整理”。这不是偷换结论，而是因为客船案例已经多了一条新的要求：

- 第一条线仍然要托住低频能力；
- 第二条线必须明确负责中频整理。

只要把“中频整理”写成明确机制线，最自然的工程表达就会靠近超前或 `PID` 中的微分侧。若我们此刻仍只停在“滞后”表述上，结构骨架就还没有把案例里的第二条矛盾写出来。

这里仍然保留抽象骨架 $C_{\mathrm{LF}}(s)\cdot C_{\mathrm{MF}}(s)$，也是为了提醒我们：本课关心的是机制分工，不是提前把最终参数锁死。

### 2.9 参数方向怎样写，才真正可验证

进入 `4-3` 以后，参数方向不再是数值表，而是设计语言。它至少要回答：
...
某船减摇鳍控制系统的目标不是跟踪一个外部给定角度，而是通过鳍产生的力矩去抵消海浪等随机扰动，从而减小船体横摇。这个任务的要点有三条：
...
这就是案例 B 的难点：**任务主线天然是抗扰，但扰动本身并不直接可测，所以控制器必须借助系统可测状态来间接构造对抗力矩。**
...
这条式子很重要，因为它一下子说明了三件事：

1. 这里本来就不是单变量补偿问题；
2. 结构里天然会同时出现“位置项、速度项、加速度项”；
3. 这类任务中的复合结构，不是因为设计者贪多，而是因为单一结构根本无法覆盖全部力矩平衡要求。
...
这说明控制作用等效于把干扰力矩压缩了 $(1 + P)$ 倍。也就是说，案例 B 的复合结构骨架首先服务的是**抗扰通道重写**，而不是“跟踪过程更快”。
...
因此，案例 B 的“为什么需要复合结构”并不是来自“低频和中频都想兼顾”，而是来自**任务本身同时要求多个物理分量参与构造控制力矩。**
...
这时我们会发现，工程上熟悉的 `PID` 形式出现在这里，但它的来历并不是“控制器大全里常见，所以默认用它”，而是：

- 积分项对应一个长期作用方向；
- 比例项对应一个直接的阻尼/恢复分量；
- 微分项对应一个更快的动态响应分量。

换句话说，案例 B 中 `PID` 的价值首先在于它是一个**复合结构骨架**，而不是一个需要背参数表的名字。
...
| 微分相关作用 | 在谐振附近提供更快的动态抑制 | 压住峰值响应，改善抗扰利落度 | 高频噪声更敏感 |
...
## 四、两个案例放在一起，我们才真正看清 `4-3`

只看案例 A，我们会把 `4-3` 理解成“低频和中频怎样一起安排”；只看案例 B，我们又容易把 `4-3` 理解成“多物理分量怎样一起构造力矩”。把两者并排，`4-3` 的边界才会真正清楚。
...
经过两个案例以后，我们可以更准确地理解 `PID` 在本讲中的位置。

它既不是默认答案，也不是禁用答案。它在本讲中的真实位置是：
...
到这里为止，我们已经不需要任何作者态提示，也能直接总结出本讲的方法流程。
...
工程上最危险的方案，往往不是明显很差的方案，而是“只写收益、不写代价”的方案。因为这样的方案一上机，我们只会盯着自己最想看到的那条曲线，而忽略：
...
AI 在本讲里可以参与，但只能放在辅助位置。更合适的用法有两类。
