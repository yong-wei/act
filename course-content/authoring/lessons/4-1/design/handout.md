...
- **讲义定位**：这份讲义不再把重点放在“再背一遍指标名称”上，而是训练你面对同一套跨域证据，先写清任务排序、约束边界和证据来源，再进入后续的结构选择与方案起步。
...
但到了设计阶段，真正难的问题不再是“我看到了什么”，而是“我先保什么，再争什么”。

模块4的入口课要先把一个动作立住：

> **面对同一套跨域证据，不同工程场景会写出不同任务书。**

这正是“分析课”与“设计课”的分界线。分析课回答“系统当前怎样”，设计课回答“系统必须达到什么水平、哪些底线不能破、哪些性能可以在底线之上继续争取”。
...
从这里开始，本课不急着讨论应该选 `PI`、`PD`、超前校正还是复合结构。先把下面四件事写清楚：
...
新版 4-1 的入口动作，不是先把指标概念重新分类，而是先把 `3-8 / 3-9` 已经出现过的同一套跨域证据图重新摆到眼前。我们先看图，再解释为什么这些图会导向不同任务排序。
...
图中 $R(s)$ 表示给定航向，$C(s)$ 表示实际航向，$N(s)$ 表示风浪等扰动因素。这个对象最适合承担模块4前半段的主场景，因为它能把“系统稳定”和“任务可接受”明确分开。
...
| 当前先观察，不急着当结论 | 截止频率、带宽、摆舵频繁程度、积分误差指标 |
...
这个对象不是本课的主场景，但它必须作为对照案例保留下来。原因不是它更复杂，而是它最能说明：
...
这个场景的四格联读不需要像主场景那样铺得很开，本课只保留它真正服务于“排序重排”的部分：
...
| 当前先观察，不急着当结论 | 截止频率、谐振峰值、关键工作区极点变化 |
...
到这里，两个场景都已经读过一次。真正要被学生带走的，不是多记一套案例数据，而是下面这张“同图异读”对照表。
...
第一章先把“排序会重排”做成了入口动作。现在才回头解释：为什么同一套图可以被改写成任务书。关键就在于，指标到了设计课里不再只是并列名词，而要承担不同角色。
...
为了让这张卡真正服务后续 `4-2 / 4-3 / 4-4`，本课建议把任务表达卡固定成七项，而不是只留抽象标题：
...
1. 任务表达卡应当由证据读出来，而不是教师先验给出。
2. 为了支撑后续课次，入口课至少要把对象、目标、最紧矛盾、硬约束、软目标、观察指标和证据来源写清楚。
...
继续用客船主场景来说明。如果用参数向量 $\theta$ 表示某一类控制方案，那么入口课中的可行域可以先写成
...
2. 入口课还没展开具体结构选择，无法严肃比较不同结构之间的最优性；
3. 许多真正的优化问题要等到 `4-5 / 4-6`，结合多目标权衡和失败诊断之后才能合理讨论。

所以，4-1 的教学任务是：**先把边界写清楚，不假装已经求到了最优。**
...
| 最优 | 在当前评价口径下，方案进一步表现更优 | 以为入口课就能直接得到最优参数 |
...
到了这里，学生通常已经能说出很多指标名称，也能看懂不少图。但真正进入设计判断时，仍然容易掉进几种固定误区。4-1 必须提前把它们掐住，否则后面的 `4-2 / 4-3 / 4-4` 会持续把方向写反。

### 6.1 常见误判一：把“系统稳定”等同于“任务已经完成”

这是模块4入口最常见的认知滑坡。稳定只说明系统没有发散，不代表过程已经可接受，更不代表它已经适合当前任务。
...
5. 可行域、满意域和最优域属于三层筛选，入口课只负责先把边界写清楚。

### 7.2 在整张课程地图中的位置

回到模块4主线，本课承担的是“任务表达入口”这一环。它输出的是一份后续设计必须共享的输入，而不是提前给出已经整定好的控制器：
