...
生成时间：2026-04-24T15:36:37+08:00
...
|---|---|---|---|---|---|---|
| step-01 | 从场景配置对比到迁移问题：为什么这次不能只说“再调一调参数” | `migration_boundary_hero_board` | `none` | 公式卡、讲义表 1、失配图、问题聚焦与桥接结论 | `src/lib/unit-4-6-course.ts`；共享 manifest renderer | 严格实现 |
| step-02 | 本次课程目标：完成这轮迁移判断后应能做到什么 | `objective_chain_board` | `none` | 目标标题、五条目标、桥接句 | 同上 | 严格实现 |
| step-03 | 迁移场景任务重排：客船与驱逐舰到底换了什么 | `scenario_reorder_compare_board` | `activity_card_set` | 迁移公式、表 1 后的判断说明、双作答卡 | 共享内容渲染 + 共享活动渲染 | 严格实现 |
| step-04 | 固定结构迁移后的失配信号：拖尾、航迹偏离与职责过载如何同时出现 | `mismatch_evidence_board` | `teacher_reveal_only` | 旧权重、旧总代价、符号说明表、失配图、三层显影 | 共享内容渲染 + 教师显影控制 | 严格实现 |
| step-05 | 驱逐舰专用代价函数：为什么比较对象必须重写 | `objective_rewrite_board` | `activity_card_set` | 新旧目标、含符号和计算方法的五项职责表、双作答卡 | 共享内容渲染 + 共享活动渲染 | 严格实现 |
| step-06 | 何时把结构选择正式抬成问题本身 | `structure_entry_board` | `teacher_reveal_only` | 过载链、三层引入条件、五类结构表达式/参数范围、解码映射例 | 共享内容渲染 + 教师显影控制 | 严格实现 |
...
| step-11 | 总结：从任务重排到结构编码入口的完整判断链 | `summary_exit_board` | `none` | 五条总结、信息图、主线链条、4-7 去向 | 共享内容渲染 | 严格实现 |
