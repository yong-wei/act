...
这一讲只讨论**纯极点系统**。我们暂不引入零点，也不改变控制器结构，先把一个基础判断写清楚：

> 高阶系统之所以常能用低阶模型理解，不是因为高阶信息不重要，而是因为有些模态衰减更快、有些极点在关注频段内影响更小。

因此，这一讲不再增加零散结论，而是集中回答三个更根本的问题：
...
这一点也划清了本讲与后续课次的边界：本讲允许借助 Bode 图说明近似为什么成立，但**不进入 Nyquist 判据、Bode 判稳和裕度设计语言**。这些内容会留到 `3-8` 再统一处理。
