...
> **当前阶段目标**：围绕“误差分析总入口课”补齐本课正式媒体，统一支撑“先把非标准结构整理成双通道骨架，再进入型别判断、纯增益局限、`PI`/滞后时域设计，以及频域下 `PI` 与 `PD` 的对比”这一完整判断链。
> **命名前缀**：全部统一使用 `3-7-...`
...
2. 结构图必须体现给定、扰动、输出与反馈四个关键位置，且要能支撑“先化简成标准双通道骨架，再进入误差分析”的课堂口径。
