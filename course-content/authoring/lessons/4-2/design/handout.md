...
这类问题需要再往前退一步。选型首先要回答的是**当前最紧的矛盾到底落在哪里**。例如：
...
如果这些问题还没想清，控制器名称就会退化成经验口号。学生因此容易出现三种典型误判：

1. 把 `PID` 当作"更高级所以默认更好"的答案；
2. 只会说"这个结构能改善"，却说不清它主要改善了哪一段频率行为；
3. 只盯着收益，不说明代价从哪里来。

因此，这节课要完成的是：把模块3中学过的极点、零点、积分、相位、三频段与稳定裕度语言，重新整理成适合设计环节的判断语句：

> 这类单结构为什么对当前任务更合适，第一步先往哪里推，它优先改善什么，又会牺牲什么。

### 1.1 从 `4-1` 接过来的是一张矛盾清单

任务表达卡交给本节的输入，是下面这五个判断，而不是“最后该用什么控制器”的结论：
...
一旦这五项写清，控制结构的优先级就可以由理由来支撑，首轮起步也才有明确依据。
...
那么 `4-1` 的现状分析已经表明：系统可以跟踪航向，但超调约为 $32.01\%$，调节时间约为 $83.24\,\text{s}$，而且还要长期面对风浪扰动。这个例子里，教师首先关心的是低频保持能力和慢扰动抑制是否充足，因此第一轮判断更自然地会落在 `PI/滞后` 上。
...
则 `4-1` 已经读到：系统调节时间约为 $0.17\,\text{s}$，说明速度并不慢；更值得警惕的是超调约为 $37.55\%$，以及中频储备仍不够从容。这个例子里，首轮更适合先整理相位、阻尼和超调，因此更接近 `PD/超前` 的任务。
...
`4-2` 先完成“为什么选、第一步往哪里起”的说明，`4-3` 再继续展开“单结构不够时怎样长成可验证方案”。
