...
本讲义的航向训练与评价采用 $DT=0.5\,\mathrm{s}$、每轮 $600$ 步的仿真时域，单轮覆盖约 $299.5\,\mathrm{s}$。左图显示直接控舵策略的训练回报移动平均值从低位上升，说明策略在边缘工况训练中形成了更有效的状态-动作对应关系。中图把同一策略放入安全外壳后的触发次数同步画出：回报改善不能单独作为成功证据，安全干预是否频繁、是否集中在边界状态，同样需要被记录。右图显示 RL 调度 PID 的训练回报，用于对比“直接输出舵角”和“只选择 PID 参数档位”两条路线的学习难度。
...
综合评价仍然支持“RL 调度 PID”作为更合理的工程入口。它不是用策略学习取代控制器，而是让策略学习选择参数档位，把可解释的 PID 结构、执行器约束和长时域验证保留下来。它的不足也清楚：最大超调略高于固定 PID，说明档位切换带来了更积极的动态响应。这个结果不支持简单地用 RL 取代 PID，而支持让 RL 先承担参考生成、参数调度、扰动补偿和异常场景辅助等任务；若要直接接管舵机，训练覆盖、动作解释、安全退化记录和独立验证都必须显著加强。
