...
这套版式让"先看哪一域、再用哪一域复核"成为一个稳定的阅读习惯。
...
这个对象有三个特征，使它适合作为综合分析的载体：

1. 开环中含有原点极点，稳态误差与型别问题天然存在。
2. 极点在快慢时间尺度上分布明显，根轨迹和阶跃响应都比较典型。
3. 增益、零点、积分和低频补偿一旦变化，三种语言同步变化，无需为每一域另换案例。

读图时需注意：对于基准版和零点线补强版，左上角使用**单位阶跃响应**，因为这两类方案主要比较动态。对于积分家族，左上角改为**单位斜坡跟踪响应或误差响应**——基准对象本来就是 I 型系统，面对单位阶跃时稳态误差已为零；如果只盯着阶跃终值，就看不出积分方案到底改善了什么。
...
阶跃曲线先冲高，再缓慢衰减到稳态。超调量接近三分之一，说明系统已有动态积极性；但调节时间超过 80 s，这种积极性并未换来利索的收束。它更像"会动，但收得不够干净"的基线对象。
...
相角裕度约 37.43°，增益裕度约 21.91 dB。系统稳定，但远未达到"宽裕"的程度。若只靠提高增益追求更快，最先被压缩的就是相角裕度。

### 3.3 基准结论

基准版本的任务标签判为**综合折中基线**。它具备可接受的稳定性，但暴露出两条限制：

1. 优先追求动态改善时，第一风险出现在相位余量与超调。
2. 优先追求稳态改善时，仅靠当前低频增益不够，必须重新分配低频资源。
...
选择这组控制器有两条依据。第一，基准版的相角裕度只有 37.43°，继续追求更快响应时，中频相位必须先补出来；第二，新增零点放在 `-0.08`，与基准闭环共轭极点实部的量级接近，能更直接地把根轨迹向左拉开，同时再放一个高频极点 `-0.5`，避免高频放大过早失控。

设计过程概括为三步：

1. 保留原增益 2.25 作为比较基线，不让"增益变化"掩盖"零点补偿"本身的作用。
2. 把零点放在基准共轭极点实部附近，让主要振荡模态优先左移，动态改善在根轨迹上直接看见。
...
阶跃曲线的超调几乎被压平，调节时间缩短到原来的三分之一左右。上升更快，收束也更干净，"更快"可以直接从曲线读出来。

#### 左下角：根轨迹

新增零点把主要振荡极点对向左拉开，闭环极点从基准版的 `-0.0456 ± j0.1259` 移到 `-0.2314 ± j0.2581`。系统还保留一个更慢的实极点 `-0.0727`，因此更准确的说法是：主振荡模态左移、阻尼提高，动态品质优先改善。
...
相角裕度提升到 67.58°。这条补强路线通过整理中频附近的相位和幅值获得更好的动态表现。注意它没有改变系统型别，稳态精度任务仍保留在原水平上。

### 4.3 零点线补强的任务标签

零点线补强的任务标签判为**更偏动态改善**。它优先解决"快"和"收得漂亮"，不直接解决"长期误差小不小"。这条路线适合回答"更快一些该怎么办"，不适合独自承担"更准一些该怎么办"。
...
这组 PI 取较大的积分时间常数 $T_i=200$，把积分作用控制在"刚开始看见趋势"的水平。设计依据不是立刻追求误差最快归零，而是先回答一个更基础的问题：只要系统多背一枚积分极点，低频收益和慢极点代价会怎样同时出现。
...
开环在原点多了一个积分极点，同时在 $s=-0.005$ 附近引入零点。闭环新增了一个很靠近虚轴的慢极点 `-0.0051`。这带来两件同时发生的事情：

1. 斜坡跟踪误差开始持续下降，400 s 时已从基准版的 5.56 压到 0.71；
2. 过渡过程被这枚慢极点拖长，超调从 31.95% 增加到 37.22%，调节时间也拉长到 90.62 s。

从频域看，弱积分并没有显著提高交叉频率，增益交叉点仍在 0.1169 rad/s 附近，但相角裕度已经从 37.43° 降到 34.97°——低频收益已经开始出现，代价先在相位余量上出现。
...
强积分沿用同一比例增益，但把积分时间常数压缩到 $T_i=40$。弱积分已经证明"增加积分会让误差继续下降"，这里故意把积分作用做强，看低频收益能否更快兑现，以及代价会不会显著放大。

设计过程很直接：

1. 以弱积分为起点，只加强积分，不引入额外中频整理。
2. 观察单位斜坡误差是否在更短时间内逼近零。
3. 对照校正前后的根轨迹、时域和 Bode 图，判断相位余量和超调被压缩到什么程度。

图中蓝色曲线表示校正前，橙色曲线表示校正后。

![强积分 2x2 跨域图](../media/processed/3-9-integral-strong-quad.png)

积分零点移到 $s=-0.025$，附加积分的作用大得多。单位斜坡误差在 200 s 左右已接近于零，说明它在低频确实更有效；但根轨迹同时告诉我们，主要振荡极点仍贴近虚轴，且多出来的慢极点已经移动到 `-0.0281` 附近，系统把更多动态负担放到了低阻尼模式上。
...
强积分把"更准"这件事做得更彻底，但也把"更冒进、更难收"这件事同时放大了。
...
积分校正在强积分基础上继续做。强积分已经把单位斜坡误差几乎压到零，但 25.09° 的相角裕度太紧，必须补一个中频超前环节，把动态和裕度拉回可用区间。超前零点放在 `-0.05`，极点放在 `-0.5`，让相位提升集中作用在当前交叉频率附近。
...
闭环极点结构也出现了明显分层：一对复极点来到 $-0.2167 \pm j0.3701$，而另一对较慢极点收在 $-0.0334 \pm j0.0072$。校正动作不再只是"继续堆低频"，而是在低频目标不丢的前提下，重新分配中频相位和动态阻尼；同时也提醒我们，决定长时间尾部的并不只是一对快速左移的复极点。

**判断句**：积分校正的任务标签是"更偏综合折中"。积分路线可以继续保留，但必须配合中频整理；否则系统会越来越准，却也越来越难用。
...
滞后方案的目标是：在不改变型别的前提下把低频增益提高一倍左右。零点放在 `-0.025`、极点放在 `-0.0125`，低频被抬高，而交叉频率附近的形状变化相对温和。这正适合拿来和积分路线对照：同样想改善稳态，它为什么只能把误差压小，而不能像积分那样把单位斜坡误差压到零。
...
这组方案没有增加额外积分极点，系统型别没有变化，单位斜坡稳态误差不会降到零，但会从基准版的 5.56 降到 2.78 左右。滞后把低频能力提高了，却没有把"误差阶次"改掉。
...
面对"我想更准一些"的任务，第一反应不该是直接背控制器名称，而应先回答：

1. 究竟要把误差降到多低；
2. 是否必须改变型别；
3. 是否愿意为此付出更紧的相位余量和更长的过渡过程；
4. 如果不愿意，是否需要额外的中频校正。
...
4. 真正能带入下一阶段的判断是：这条路线优先解决什么，同时会先牺牲什么。
...
到这里为止，模块 3 已完成它的任务：能够围绕同一对象，说清结构变化、根轨迹、时域结果和频域后果之间的联系。模块 4 会在这个基础上继续讨论性能指标体系、可行域和整定策略。

---

## 本讲小结

1. 本讲的核心是把极点语言、时域语言和频域语言收束成同一张判断图。
