...
- 已按最新 `syllabus-refactor` 与资源融合框架重新审视 `2-2`：
  - 课堂主线收束为“标准时域对象建立 + 指标读图 + 轻量参数判断”；
  - `pptx/6` 与 `pptx/7.1` 固定为主骨架资源；
  - 船舶 `3.1-3.3` 固定为对象对照补强资源；
  - 思政案例本轮明确不强行接入。
- 已同步回写 `module2.md`、`handout-framework.md`、`boppps.md`、`interactive-page.md`、`multimedia.md` 的设计约束。
