...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Generate 2-2 td-03 second-order response families SVG.')
    parser.add_argument(
        '--output',
        type=Path,
        default=Path(__file__).resolve().parent.parent / 'processed' / '2-2-td-03-second-order-response-families.svg',
    )
