...
- **后续衔接**：2-3（频率响应基础）、2-4（Bode 图与 Nyquist 图基础）、3-1（纯极点视角下的稳定与动态基础）
...
上一单元解决了"如何描述对象"的问题——将物理系统转化为传递函数，并从结构图或信号流图中得到系统总传递函数。一旦这些代数工作完成，一个全新的工程问题随之浮现：

> 如果两个系统都稳定，它们的控制品质就一定相同吗？

答案是否定的。以船舶航向控制为例，驾驶台发出新的航向指令，要求船头快速转向目标方向并稳定保持。真正关心的不只是"最终能否到达"，而是以下过程性问题：
...
这些问题有一个共同特征：都只能在**时间轴**上回答。系统的"表现"不是某个静态数值，而是一条完整的输出曲线。研究这条曲线随时间演化的过程，就是**时域分析**。

模块1中我们已观察过一些直观现象：有的系统缓慢逼近目标，有的来回摆动，有的逐渐偏离。现在，将这种感觉提升为规范语言——以一阶系统和二阶系统的单位阶跃响应为起点，建立一套能够解答"快慢、超调、稳定时间及其机理"的时域分析框架。

> **核心问题**：给定一个闭环传递函数，如何从其单位阶跃响应中判断系统的快慢、振荡和稳定品质？

---

## 二、核心内容

> 本节按照"一阶系统 → 二阶系统 → 动态性能指标 → 跨域桥接"的顺序展开。

### 2.1 时域分析的研究对象、任务与典型输入

**时域分析**指直接在时间变量 $t$ 上讨论系统输出 $c(t)$ 如何随输入和时间演化。对于线性定常系统，若已知传递函数 $G(s)$ 并给定输入 $r(t)$，可通过：
...
时域分析是建模结果的首次"应用"，需完成三项任务：

1. **求响应表达式**：从 $G(s)$ 和 $R(s)$ 得到 $c(t)$；
2. **判响应形态**：判断其属于单调收敛、振荡收敛、持续振荡还是发散；
3. **提性能指标**：从曲线中提取"快慢、超调、稳定时间"等可度量信息。

为便于比较不同系统，通常采用一组标准测试输入。常见的有单位脉冲、单位阶跃、单位斜坡和单位加速度信号，本节重点关注**单位阶跃输入**：

$$
r(t)=1(t), \qquad R(s)=\frac{1}{s}
$$

选择阶跃输入，不仅因其简单，更因其最接近实际工程中的"指令突变"场景。例如，船舶航向指令从 $10^\circ$ 突变为 $20^\circ$，电机转速设定值突然提高，伺服位置突移至新目标，这些都近似为阶跃输入。
...
因此，后续定义的上升时间、峰值时间、超调量和调节时间，默认均围绕**单位阶跃响应**展开。

![典型输入信号及响应对象对比曲线](../media/processed/2-2-td-01-time-domain-input-response-overview.svg){width=100%}

### 2.2 一阶系统的单位阶跃响应与时间常数

一阶系统是曲线解读语言的第一课：它既无振荡，也无超调，只保留了一件事——**系统究竟快还是慢**。先用它建立衡量"快慢"的第一把尺子。
...
这个结果告诉我们，一阶系统的阶跃响应由"终值 $K$"和"指数逼近过程 $e^{-t/T}$"共同构成——响应从零开始，按照指数规律逐渐逼近稳态值。一阶系统的基本性质：
...
**经过一个时间常数后，一阶系统已达到终值的 63.2%**。$T$ 不是抽象符号，而是衡量"系统推进速度"的直接刻度。

工程上常将 $T$ 视作一阶系统快慢的核心参数，常用近似如下：

- 5% 误差带调节时间：$t_s \approx 3T$
- 2% 误差带调节时间：$t_s \approx 4T$
- 10% 到 90% 上升时间：$t_r \approx 2.2T$

这些近似帮助形成快速判断：当时间常数变为原来的 2 倍时，响应曲线整体拉长；当时间常数减小时，曲线整体加速——不会改变形状，也不会突然产生振荡。

> **快慢可由一个参数稳定控制，且该参数会在整条响应曲线上留下统一的痕迹。**

![一阶系统单位阶跃响应与时间常数对比曲线](../media/processed/2-2-td-02-first-order-step-time-constant.svg){width=100%}

若希望亲自复现"时间常数增大，曲线整体拉长"的对比现象，可运行附录 C 中的 `(2.2.1.first-order-step.m)`。

### 2.3 二阶系统标准型与单位阶跃响应的严格表达

一阶系统只能告诉我们"快还是慢"，却无法解释工程中更常见的现象：**为何有些系统会先冲过头，再摆回？** 这时必须进入二阶系统。二阶系统首次将"快慢"与"振荡"同时纳入同一模型。
...
一阶系统只有"逼近终值的快慢"，二阶系统额外拥有"围绕终值摆动的方式"。

#### 二阶系统四种典型响应形态

按阻尼比取值，二阶系统可分为四种典型响应形态：
...
3. **临界阻尼**（$\zeta=1$）
   极点重合于负实轴，响应无振荡，通常是"不振荡前提下最快"的情形。

4. **过阻尼**（$\zeta>1$）
   极点为两个不同的负实根，响应单调但较慢。

这四类情形对应模块1中见过的"四种响应家族"。区别在于，现在不只是看图识现象，而是能将"为何出现这种图"表述为参数条件：

- 不是"它看起来会振荡"，而是"因为 $0<\zeta<1$"；
- 不是"它看起来收得较慢"，而是"因为阻尼比或自然频率落在此范围"；
- 不是"它像是最稳"，而是"因为极点结构已变为负实轴上的不同位置"。

> **看到一条二阶响应曲线，不只说它"像什么"，还要能指出背后是哪个参数在起主导作用。**

![二阶系统四种典型响应形态对比图](../media/processed/2-2-td-03-second-order-response-families.svg){width=100%}

### 2.4 动态性能指标的定义、公式与物理意义

掌握了响应曲线和决定其形状的参数语言，还需要一组可用于比较、计算和交流的量化语言——**动态性能指标**。
...
这四个指标将原本"只能观察"的响应曲线，翻译为"可计算、可比较、可设定要求"的工程语言。

为使后续推导更紧凑，先将欠阻尼二阶系统的响应改写为相角形式。令：
...
四个指标本质上就是从这两层结构中分别提取最有代表性的读数。

#### 2.4.1 上升时间 $t_r$

**上升时间**回答：系统从开始动作到"首次真正达到目标"需要多久。
...
- $\omega_n$ 越大，整体时间尺度越短，$t_r$ 通常越小；
- $\zeta$ 也会影响 $t_r$，因为阻尼会改变曲线首次到达终值的位置。

上升时间不是"仅由一个量决定"的简单指标，但它非常适合回答起步灵敏度问题。

#### 2.4.2 峰值时间 $t_p$

**峰值时间**回答：如果系统会超调，第一个最高点出现在何时。
...
系统内部摆动越快，第一个波峰出现越早。$t_p$ 本质上测量的是"振荡频率"。

#### 2.4.3 超调量 $M_p$

**超调量**描述系统第一次冲过目标值时究竟超出多少，定义为：
...
这是较严格的估计式。工程上，在常见阻尼比范围内，对数项通常可近似为 4，得到最常用的工程公式：
...
调节时间反映的是"指数包络压下去需要多久"，主要由极点实部控制：
...
四个指标协同工作，将一条响应曲线翻译为可交流、可设定、可约束的时域语言。

![动态性能指标标注示意图](../media/processed/2-2-td-04-time-domain-indices-annotated.svg){width=100%}

若希望亲自比较不同阻尼比对 $M_p$、$t_p$、$t_s$ 的影响，可运行附录 C 中的 `(2.4.1.damping-ratio-scan.m)`。

### 2.5 从时域指标到参数与极点趋势

将时域分析理解为"对着曲线测量几个数值"还不够。前面计算出的 $t_r$、$t_p$、$M_p$、$t_s$ 并非彼此独立的四个数字，它们共同指向同一事实：
...
这将"曲线语言"翻译成了"极点语言"。三个核心跨域结论有了明确的几何对应：
...
时域响应曲线不再是孤立图像，而是复平面极点结构在时间轴上的投影结果。
...
求该系统的阻尼比 $\zeta$、自然频率 $\omega_n$、阻尼振荡频率 $\omega_d$，并求出单位阶跃响应的上升时间 $t_r$、峰值时间 $t_p$、超调量 $M_p$ 和 2% 误差带下的调节时间 $t_s$。最后说明该系统的动态品质。
...
该系统属于典型的"反应积极、平稳性一般"的欠阻尼系统。这体现了一个核心事实：**响应速度与超调大小往往难以同时达到最优。**

还需注意：精确从响应曲线读取的 $t_s$ 与工程近似式给出的 $2\,\text{s}$ 并不完全相等。这不是推导错误，而是因为：

$$
t_s \approx \frac{4}{\zeta\omega_n}
$$

本身就是面向初步设计的经验公式，优点是快速估算，代价是引入近似误差。**公式适用于快速判断，仿真适用于精确核对。**

![例题一单位阶跃响应与性能指标标注图](../media/processed/2-2-td-05-example-response-with-indices.svg){width=100%}
...
若阻尼比低于约 $0.591$，系统超调必将超过 10%，这是一条不可突破的下界。
...
满足超调约束后，自然频率不能任意小，否则系统虽然"无冲"，却会"收得太慢"。
...
这两个条件合在一起，实际上是最简单的设计区间问题：

- 第一条给出"不能太冲"的边界；
- 第二条给出"不能太慢"的边界。

若将这些条件映射到 $s$ 平面，就会得到"阻尼线 + 等调节时间线"共同界定的可行极点区域。

对本题的关键边界值快速核对：当 $\zeta = 0.591155...$ 时，由超调公式计算得 $M_p \approx 10\%$；当再取 $\omega_n \approx 3.3832$ 时，有：
...
**时域指标要求可直接转为参数约束，而参数约束又可进一步转为极点约束。**

![由时域指标反推极点约束区域示意图](../media/processed/2-2-td-06-time-spec-to-pole-region.svg){width=100%}

若希望亲自复现边界参数与可行区域的对应关系，可运行附录 C 中的 `(3.2.1.spec-boundary-check.m)`。

### 3.3 例题后的方法总结

将前两道例题并列来看，方法主线已形成完整闭环：

- **正向分析**：已知系统参数 → 求响应指标；
- **逆向设计**：已知响应指标 → 反推参数区域；
- **跨域迁移**：将参数区域进一步转写为极点位置、根轨迹区域和频域裕度要求。

本节虽有大量公式与计算，但核心是构建一套中间语言：
...
只要这条链条建立起来，后续进入根轨迹、频域图形或设计约束时，就不会感到在"换一门课"，而是在处理同一系统，只是切换了观察坐标系。
...
例如，船舶航向调整时，若系统超调过大，可能导致航向来回摆动；在狭窄航道、编队航行或恶劣海况下，这不仅影响效率，更可能直接威胁航行安全。控制工程中的参数选择不是"将某一指标极致化"，而是在安全、稳定、快速、能耗之间寻求负责任的平衡。
...
- 2-3 将把"响应对象"推广至频率响应对象，建立正弦稳态响应、频率特性与 $G(j\omega)$ 的解释；
- 后续模块3将从极点视角解释更一般的稳定与动态问题；
- 模块4将把时域指标正式收束为工程约束和可行域表达，进入设计视角。

> **一句话总结**：本节将"会看响应曲线"提升为"会用指标和极点解释响应曲线"；时域分析不再仅是观察，而是开始成为设计语言。
...
3. 想重新核对相角形式来源；
4. 后续在模块3或模块4中再次调用此套公式时，需回到最初出处。

---

## 附录C｜MATLAB/Octave 复现代码

> 以下代码仅保留最基本复现逻辑，便于直接修改参数、重新运行并观察结果。若使用 Octave，请先确认已安装并加载 `control` 包。
