...
![图 2 根轨迹读图示意图](assets/T3/T3-2-root-locus-reading.png)

某单位负反馈系统只采用比例控制，实际控制器增益记为 $K_c$，且只考虑 $K_c\ge 0$。教师给出的完整根轨迹主图采用“作图增益” $K_r$，两者关系为 $K_r=4K_c$。根轨迹读图示意如图 2。
...
3. 分离后形成一对共轭复极点；当 $K_r=6$ 时，这对极点穿过虚轴，交点约为 $s=\pm j2.4$。

4. 当 $K_r>6$ 时，这对共轭极点进入右半平面。

5. 当 $K_r=2$ 时，主导极点约为 $s=-0.72\pm j0.78$；当 $K_r=5$ 时，主导极点约为 $s=-0.18\pm j1.92$。
...
（3）现把控制器增益从 $K_c=0.50$ 增大到 $K_c=1.25$。说明根轨迹上的极点总体向哪个方向迁移，并结合读点解释闭环响应在振荡程度、超调趋势和衰减快慢上的变化。
