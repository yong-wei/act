...
某单位负反馈系统只采用比例控制，实际控制器增益记为 $K_c$，且只考虑 $K_c \ge 0$。根轨迹主图采用“作图增益” $K_r$，两者关系为
$$
K_r = 4K_c
$$
根轨迹读图示意如下。该图由开环极点 $0,-2,-6$、无开环零点的真实根轨迹数据绘制。请依据图中标注完成读图、换算和趋势解释。

![根轨迹读图示意图（20260623-160841）](assets/T3/T3-2-root-locus-reading-20260623-160841.png)

请回答：

1. 写出闭环稳定的 $K_r$ 取值窗口，并换算为实际控制器增益 $K_c$ 的稳定窗口。
2. 若工程上要求“工作增益不超过稳定边界的 80%”，求允许的 $K_c$ 上限。
3. 现把控制器增益从 $K_c=0.50$ 增大到 $K_c=1.50$。说明根轨迹上的极点总体向哪个方向迁移，并结合图中 $K_r=2$ 与 $K_r=6$ 附近的读点，解释闭环响应在振荡程度、超调趋势和衰减快慢上的变化。

### 标准答案

设题中作图增益与实际控制器增益满足 $K_r=4K_c$。

1. 由图 2 可读得：当 $K_r=96$ 时根轨迹穿过虚轴，而当 $K_r>96$ 时共轭极点进入右半平面，因此严格稳定时必须满足
   $$
   0<K_r<96.\tag{4分}
   $$
   再由 $K_r=4K_c$ 换算，得
   $$
   0<K_c<\frac{96}{4}=24.\tag{4分}
   $$
   所以实际比例增益的稳定窗口为 $0<K_c<24$。

2. “不超过稳定边界的 80%”表示
   $$
   K_r\le 0.8\times 96=76.8.\tag{2分}
   $$
   代回换算关系可得
   $$
   K_c\le \frac{76.8}{4}=19.2.\tag{2分}
   $$
   故允许的 $K_c$ 上限为 $19.2$。
...
3. 当 $K_c=0.50$ 时，
   ...
   当 $K_c=1.50$ 时，
   $$
   K_r=4\times 1.50=6.\tag{1分}
   $$
   因此增益增大时，靠近原点的实根先沿实轴向左移动，和来自 $s=-2$ 的分支在分离点附近形成共轭复根。总体表现为主导极点由实根变为共轭复根。（2分）

   由图 2 中 $K_r=2$ 与 $K_r=6$ 的读点可知，主导极点由 $K_r=2$ 时的实根 $s\approx -0.190$ 变为 $K_r=6$ 时的共轭复根 $-0.886\pm j0.422$：
   - 主导极点实部从 $-0.190$ 左移到 $-0.886$，说明主要衰减速度变快。（2分）
   - 极点由实根变为共轭复根，说明响应开始出现振荡成分，超调趋势增强。（2分）

   综合来看，把 $K_c$ 从 $0.50$ 增大到 $1.50$ 后，系统仍稳定，响应从非振荡主导逐步转向带振荡的衰减过程，速度提高但超调趋势增强。

本题判断均来自图 2 的根轨迹读数与题设增益换算关系。

### 分步评分标准

| 步骤 | 分值 | 判分标准 |
| --- | --- | --- |
| 稳定边界与稳定窗口 | 8分 | 正确判断 $K_r=96$ 为稳定边界，并给出严格稳定区间 $0<K_r<96$；再正确换算出 $0<K_c<24$。若边界点误并入稳定区间，扣 `1-2` 分。 |
| `80%` 边界换算 | 4分 | 先得到 $K_r\le 76.8$，再换算出 $K_c\le 19.2$。只写出其中一步正确给 `1` 分。 |
| 增益迁移换算 | 2分 | 能把 $K_c=0.50,1.50$ 正确换算为 $K_r=2,6$，并说明是沿图中根轨迹分支继续迁移。两者只对一个给 `1` 分。 |
| 动态趋势解释 | 6分 | 基于读点指出主导极点由实根转为共轭复根，衰减速度提高但振荡和超调趋势增强。若只说更振荡而未解释快慢或衰减，最多给 `2-3` 分。 |
