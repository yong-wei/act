...
## Subject
- Title: λ₁, (modeling_db7aabf07d906f63b237e10d)
- Concept kind: theoretical_construct
- One-line definition (source of truth): 如果n阶微分方程的特征根是λ₁, λ₂, ..., λₙ且无重根，则把函数e^(λ₁t), e^(λ₂t), ..., e^(λₙt)称为该微分方程所描述运动的模态，也称为振型。
- Core intuition: 每一种模态代表一种类型的运动形态，齐次微分方程的通解则是它们的线性组合。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 如果n阶微分方程的特征根是λ₁, λ₂, ..., λₙ且无重根，则把函数e^(λ₁t), e^(λ₂t), ..., e^(λₙt)称为该微分方程所描述运动的模态，也称为振型。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「λ₁」
