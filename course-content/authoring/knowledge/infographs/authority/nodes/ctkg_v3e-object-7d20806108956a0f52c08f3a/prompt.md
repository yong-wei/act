...
## Subject
- Title: Proportional plu (Proportional plus integral (PI) controller)
- Concept kind: unknown
- One-line definition (source of truth): Proportional plu：A widely used form of controller with a transfer function G_c(s) = K_p + K_I/s.
- Core intuition: 在图谱邻接中可把握：后续 → Integration compensator。
- Relation summary: 后续 → Integration compensator
- Extended explanation (do not paste verbatim on image): A widely used form of controller with a transfer function G_c(s) = K_p + K_I/s.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Proportiona…」、「一种·Integr…」
