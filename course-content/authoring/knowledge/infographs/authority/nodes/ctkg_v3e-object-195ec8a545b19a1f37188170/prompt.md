...
## Subject
- Title: Disturbance Rejection Bandwidth (ω DRB) (Disturbance Rejection Bandwidth (ω_DRB))
- Concept kind: unknown
- One-line definition (source of truth): Disturbance Rejection Bandwidth (ω DRB)：The maximum frequency at which the sensitivity function S is below a certain threshold (e.g., -3 dB), indicating the sy…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The maximum frequency at which the sensitivity function S is below a certain threshold (e.g., -3 dB…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Disturbance…」
