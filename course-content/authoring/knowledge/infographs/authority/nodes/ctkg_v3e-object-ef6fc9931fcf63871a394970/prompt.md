...
## Subject
- Title: Closed-loop System with Nonlinearity
- Concept kind: unknown
- One-line definition (source of truth): Closed-loop System with Nonlinearity：A negative feedback control system where a nonlinear element f precedes a linear dynamic system G(s), with input R and…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A negative feedback control system where a nonlinear element f precedes a linear dynamic system G(s…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Closed-loop…」
