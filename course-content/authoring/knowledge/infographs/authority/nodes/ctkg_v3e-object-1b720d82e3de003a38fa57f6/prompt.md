...
## Subject
- Title: Cascade Compensator
- Concept kind: unknown
- One-line definition (source of truth): Cascade Compensator：A compensator placed in the feedforward path of a simple, single-loop feedback control system.
- Core intuition: 在图谱邻接中可把握：前置 → Phase-Lag Compensator、Cascade compensation design approach · 后续 → Compensator、Cascade compensation。
- Relation summary: 前置 → Phase-Lag Compensator、Cascade compensation design approach · 后续 → Compensator、Cascade compensation
- Extended explanation (do not paste verbatim on image): A compensator placed in the feedforward path of a simple, single-loop feedback control system.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Cascade Com…」、「Cascade Comp」、「一种·Compen…」、「一种·Phase-…」、「应用于·Casca…」、「关联·Cascad…」
