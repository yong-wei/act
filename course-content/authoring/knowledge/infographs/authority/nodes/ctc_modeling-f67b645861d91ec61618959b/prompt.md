...
## Subject
- Title: system whose motion is (modeling_f67b645861d91ec61618959b)
- Concept kind: theoretical_construct
- One-line definition (source of truth): system whose motion is：A system whose motion is described by a partial differential equation because mass elements are continuously distribute…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A system whose motion is described by a partial differential equation because mass elements are con…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「system whos…」
