...
- One-line definition (source of truth): Lyapunov Function是自动控制原理权威图谱中的领域概念。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → Positive Definite Function、Lyapunov Stability
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Lyapunov Fu…」、「Lyapunov Fun」、「一种·Positi…」、「用于分析·Lyap…」
