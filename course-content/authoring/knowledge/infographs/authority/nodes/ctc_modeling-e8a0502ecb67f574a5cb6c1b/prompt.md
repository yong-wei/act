...
## Subject
- Title: 流图余因子式 Δ_k (modeling_e8a0502ecb67f574a5cb6c1b)
- Concept kind: theoretical_construct
- One-line definition (source of truth): 流图余因子式 Δ_k，等于流图特征式中除去与第 k 条前向通路相接触的回路增益项（包括回路增益的乘积项）以后的余项式。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 流图余因子式 Δ_k，等于流图特征式中除去与第 k 条前向通路相接触的回路增益项（包括回路增益的乘积项）以后的余项式。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「流图余因子式 Δ_k」
