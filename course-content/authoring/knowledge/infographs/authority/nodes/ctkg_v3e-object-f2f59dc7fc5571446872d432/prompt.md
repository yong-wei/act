...
- One-line definition (source of truth): 系统完全可控。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 秩判据、格拉姆矩阵判据 · 后续 → 可控性
- Extended explanation (do not paste verbatim on image): 系统完全可控
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「系统完全可控」、「是一种·可控性」、「包含组件·秩判据」、「应用于·格拉姆矩阵…」
