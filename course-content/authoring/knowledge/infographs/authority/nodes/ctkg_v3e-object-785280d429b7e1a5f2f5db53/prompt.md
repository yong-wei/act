...
- One-line definition (source of truth): Nyquist stability criterion：The Nyquist stability criterion relates the open-loop frequency response to the number of closed-loop poles of the syst…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The Nyquist stability criterion relates the open-loop frequency response to the number of closed-lo…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Nyquist sta…」
