...
- One-line definition (source of truth): 幂级数法求解齐次状态方程。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → 齐次状态方程
- Extended explanation (do not paste verbatim on image): 幂级数法求解齐次状态方程
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「幂级数法求解齐次状态方程」、「用于分析·齐次状态…」
