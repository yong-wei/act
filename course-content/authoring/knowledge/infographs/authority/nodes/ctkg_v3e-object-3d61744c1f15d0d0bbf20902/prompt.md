...
- One-line definition (source of truth): Precision：The degree of exactness or discrimination with which a quantity is stated.
- Core intuition: 在图谱邻接中可把握：前置 → Finite word length · 后续 → Finite word length。
- Relation summary: 前置 → Finite word length · 后续 → Finite word length
- Extended explanation (do not paste verbatim on image): The degree of exactness or discrimination with which a quantity is stated.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Precision」、「应用于·Finit…」、「关联·Finite…」
