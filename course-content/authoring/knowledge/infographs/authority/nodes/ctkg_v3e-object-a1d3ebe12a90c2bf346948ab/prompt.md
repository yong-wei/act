...
- One-line definition (source of truth): Matrix Exponential：The matrix exponential e^{At} is a matrix function used to solve linear state equations.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The matrix exponential e^{At} is a matrix function used to solve linear state equations.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Matrix Expo…」、「Matrix Expon」
