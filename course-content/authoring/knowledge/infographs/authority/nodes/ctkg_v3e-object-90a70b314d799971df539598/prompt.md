...
## Subject
- Title: 否成立 (Output controllability rank test)
- Concept kind: unknown
- One-line definition (source of truth): 构造输出可控性矩阵S0=[CB CAB ... CA^{n-1}B D]，检验rank S0=q是否成立。
- Core intuition: 在图谱邻接中可把握：后续 → 输出可控性矩阵 (output…。
- Relation summary: 后续 → 输出可控性矩阵 (output…
- Extended explanation (do not paste verbatim on image): 构造输出可控性矩阵S0=[CB CAB ... CA^{n-1}B D]，检验rank S0=q是否成立。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「否成立」、「包含组件·输出可控…」
