...
## Subject
- Title: 对于具有连续变化的非线性特性函数 (modeling_57fe9fb7d5fdd731b82b9408)
- Concept kind: analysis_method
- One-line definition (source of truth): 对于具有连续变化的非线性特性函数，在一个很小的范围内，将非线性特性用一段直线来代替。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 对于具有连续变化的非线性特性函数，在一个很小的范围内，将非线性特性用一段直线来代替。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「对于具有连续变化的非线…」
