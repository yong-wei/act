...
## Subject
- Title: 反映系统内部变量和输入变量间因… (modeling_ef0228881c8874509244b517)
- Concept kind: theoretical_construct
- One-line definition (source of truth): 反映系统内部变量和输入变量间因果关系的数学表达式，常具有微分方程或差分方程的形式。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 反映系统内部变量和输入变量间因果关系的数学表达式，常具有微分方程或差分方程的形式。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「反映系统内部变量和输入…」
