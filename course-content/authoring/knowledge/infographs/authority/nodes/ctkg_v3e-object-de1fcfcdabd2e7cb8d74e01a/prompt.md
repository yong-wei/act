...
- One-line definition (source of truth): 形如 \dot{x}(t) = A x(t) 的状态方程，不含输入项。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 幂级数法求解齐次状态方程、拉普拉斯变换法求解齐次状态方程
- Extended explanation (do not paste verbatim on image): 形如 \dot{x}(t) = A x(t) 的状态方程，不含输入项
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「齐次状态方程」、「用于分析·幂级数法…」、「用于分析·拉普拉斯…」
