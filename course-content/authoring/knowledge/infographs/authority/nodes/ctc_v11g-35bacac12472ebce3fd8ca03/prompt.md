...
## Subject
- Title: lumped parameter circuit (lumped_parameter_circuit)
- Concept kind: system_component
- One-line definition (source of truth): lumped parameter circuit：A circuit with normal attenuation of 20n dB/decade, in contrast to distributed RC networks.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A circuit with normal attenuation of 20n dB/decade, in contrast to distributed RC networks.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「lumped para…」、「lumped param」
