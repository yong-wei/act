...
## Subject
- Title: Autonomous estimator
- Concept kind: unknown
- One-line definition (source of truth): Autonomous estimator：A configuration where M and N̄ are chosen so that the state estimator error equation is independent of the reference in…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A configuration where M and N̄ are chosen so that the state estimator error equation is independent…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Autonomous …」、「Autonomous e」
