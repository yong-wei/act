...
- One-line definition (source of truth): 容许控制。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → 状态可控
- Extended explanation (do not paste verbatim on image): 容许控制
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「容许控制」、「应用于·状态可控」
