...
## Subject
- Title: Fast pole and slow pole
- Concept kind: theoretical_construct
- One-line definition (source of truth): Fast pole and slow pole：Fast poles and slow poles refer to relative rate of signal decay.
- Core intuition: 在图谱邻接中可把握：后续 → Natural response、极点。
- Relation summary: 后续 → Natural response、极点
- Extended explanation (do not paste verbatim on image): Fast poles and slow poles refer to relative rate of signal decay.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Fast pole a…」、「Fast pole an」、「关联·Natura…」、「关联·极点」
