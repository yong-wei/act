...
## Subject
- Title: Settling time with 2% criterion
- Concept kind: unknown
- One-line definition (source of truth): Settling time with 2% criterion：The settling time (with a 2% criterion) requirement is T_s = 4/(ζ ω_n).
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The settling time (with a 2% criterion) requirement is T_s = 4/(ζ ω_n).
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Settling ti…」
