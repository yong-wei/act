...
- One-line definition (source of truth): 系统在平衡工作点处，当输入信号为零时输出信号亦为零，在扰动作用下偏离平衡点后，系统输出量增量随时间变化趋于原平衡点的性质。
- Core intuition: 对于线性系统，运动稳定性与平衡状态稳定性等价。
- Relation summary: 后续 → 稳定性、运动稳定性
- Extended explanation (do not paste verbatim on image): 系统在平衡工作点处，当输入信号为零时输出信号亦为零，在扰动作用下偏离平衡点后，系统输出量增量随时间变化趋于原平衡点的性质。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「平衡状态稳定性」、「是一种·稳定性」、「关联·运动稳定性」
