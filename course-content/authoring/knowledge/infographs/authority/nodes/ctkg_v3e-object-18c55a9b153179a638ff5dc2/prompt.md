...
## Subject
- Title: Evaluating effec (Evaluating effects of gain changes using frequency-response data)
- Concept kind: unknown
- One-line definition (source of truth): Evaluating effec：Indicate on the figure where |KG(jω)|=1 for selected trial values of K without redrawing magnitude or phase information.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): Indicate on the figure where |KG(jω)|=1 for selected trial values of K without redrawing magnitude…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Evaluating …」
