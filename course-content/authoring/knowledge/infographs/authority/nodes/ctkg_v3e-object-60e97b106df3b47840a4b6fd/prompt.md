...
- One-line definition (source of truth): 衰减比。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 衰减曲线法 · 后续 → 衰减曲线法
- Extended explanation (do not paste verbatim on image): 衰减比
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「衰减比」、「包含组件·衰减曲线法」、「关联·衰减曲线法」
