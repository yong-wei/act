...
- One-line definition (source of truth): 如果控制系统中有一处或几处信号是一串脉冲或数码，换句话说，这些信号仅定义在离散时间上，则这样的系统称为离散时间系统，简称离散系统。
- Core intuition: 在图谱邻接中可把握：前置 → 采样控制系统、数字控制系统 · 后续 → 脉冲序列、连续时间系统。
- Relation summary: 前置 → 采样控制系统、数字控制系统 · 后续 → 脉冲序列、连续时间系统
- Extended explanation (do not paste verbatim on image): 如果控制系统中有一处或几处信号是一串脉冲或数码，换句话说，这些信号仅定义在离散时间上，则这样的系统称为离散时间系统，简称离散系统。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「离散时间系统」、「是一种·采样控制系统」、「是一种·数字控制系统」、「包含组件·脉冲序列」、「关联·连续时间系统」
