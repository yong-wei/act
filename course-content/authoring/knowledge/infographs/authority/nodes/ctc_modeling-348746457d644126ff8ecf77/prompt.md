...
## Subject
- Title: 一个含约当块的矩阵 (modeling_348746457d644126ff8ecf77)
- Concept kind: theoretical_construct
- One-line definition (source of truth): 当传递函数除含单实极点之外还含有重实极点时，可化为约当标准型动态方程，其A阵是一个含约当块的矩阵。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 当传递函数除含单实极点之外还含有重实极点时，可化为约当标准型动态方程，其A阵是一个含约当块的矩阵。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「一个含约当块的矩阵」
