...
## Subject
- Title: successive-approximation technique
- Concept kind: unknown
- One-line definition (source of truth): successive-approximation technique：An A/D conversion technique based on successively comparing the input voltage to reference levels representing the vari…
- Core intuition: 在图谱邻接中可把握：后续 → A/D converter。
- Relation summary: 后续 → A/D converter
- Extended explanation (do not paste verbatim on image): An A/D conversion technique based on successively comparing the input voltage to reference levels r…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「successive-…」、「关联·A/D co…」
