...
## Subject
- Title: tf2ss: transfer  (tf2ss: transfer function to state-space conversion)
- Concept kind: unknown
- One-line definition (source of truth): tf2ss: transfer ：Matlab command to convert transfer function to state-space representation in control canonical form.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): Matlab command to convert transfer function to state-space representation in control canonical form.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「tf2ss」
