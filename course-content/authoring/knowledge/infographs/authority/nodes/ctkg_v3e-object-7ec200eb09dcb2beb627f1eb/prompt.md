...
## Subject
- Title: Duality-Based Es (Duality-Based Estimator Design Using acker/place)
- Concept kind: unknown
- One-line definition (source of truth): Duality-Based Es：Use the duality relationships (A→Aᵀ, B→Cᵀ, C→Bᵀ) to apply control design tools (acker, place) for estimator gain comput…
- Core intuition: 在图谱邻接中可把握：后续 → Duality-Based Estimator Design。
- Relation summary: 后续 → Duality-Based Estimator Design
- Extended explanation (do not paste verbatim on image): Use the duality relationships (A→Aᵀ, B→Cᵀ, C→Bᵀ) to apply control design tools (acker, place) for e…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Duality-Bas…」、「一种·Dualit…」
