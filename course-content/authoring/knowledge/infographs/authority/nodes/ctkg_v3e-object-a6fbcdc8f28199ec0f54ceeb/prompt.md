...
## Subject
- Title: Method 1: Decoup (Method 1: Decoupling estimation error from reference input)
- Concept kind: unknown
- One-line definition (source of truth): Method 1: Decoup：Select M = B N so that the estimation error e(t) is independent of the reference input r(t). Then determine N to achiev…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): Select M = B N so that the estimation error e(t) is independent of the reference input r(t). Then d…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Method 1」
