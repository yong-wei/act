...
- One-line definition (source of truth): 超前网络相角达到最大值的频率，ωm=1/(T√a)。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 超前网络相角达到最大值的频率，ωm=1/(T√a)。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「最大超前角频率」
