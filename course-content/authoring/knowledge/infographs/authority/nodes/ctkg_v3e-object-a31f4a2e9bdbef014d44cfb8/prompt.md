...
- One-line definition (source of truth): 反馈校正装置接在系统局部反馈通路之中。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → 校正装置
- Extended explanation (do not paste verbatim on image): 反馈校正装置接在系统局部反馈通路之中。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「反馈校正」、「是一种·校正装置」
