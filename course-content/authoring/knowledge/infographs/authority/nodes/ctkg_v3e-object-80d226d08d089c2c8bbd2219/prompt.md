...
- One-line definition (source of truth): 通过将微分方程转化为可分离变量的形式，然后积分得到ẋ和x的解析关系式。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → 相轨迹
- Extended explanation (do not paste verbatim on image): 通过将微分方程转化为可分离变量的形式，然后积分得到ẋ和x的解析关系式。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「直接积分法求相轨迹」、「用于分析·相轨迹」
