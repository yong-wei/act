...
## Subject
- Title: The operational amplifier in (modeling_954f858741527b57a495472c)
- Concept kind: theoretical_construct
- One-line definition (source of truth): The operational amplifier in：The operational amplifier in an analog computer has a sign change that gives it a negative gain.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The operational amplifier in an analog computer has a sign change that gives it a negative gain.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「The operati…」
