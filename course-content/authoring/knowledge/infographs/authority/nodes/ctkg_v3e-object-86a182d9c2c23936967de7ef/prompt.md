...
## Subject
- Title: Safety margin in phase-lead design
- Concept kind: unknown
- One-line definition (source of truth): Safety margin in phase-lead design：A small amount of safety is allowed when determining the necessary additional phase lead φ_m.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A small amount of safety is allowed when determining the necessary additional phase lead φ_m.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Safety marg…」
