...
## Subject
- Title: Duality-Based Estimator Design
- Concept kind: unknown
- One-line definition (source of truth): Duality-Based Estimator Design：Use control design tools (acker, place) with substitutions A→A^T, B→C^T, C→B^T to compute L^T, then transpose to get L.
- Core intuition: 在图谱邻接中可把握：前置 → Duality-Based Es。
- Relation summary: 前置 → Duality-Based Es
- Extended explanation (do not paste verbatim on image): Use control design tools (acker, place) with substitutions A→A^T, B→C^T, C→B^T to compute L^T, then…
