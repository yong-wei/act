...
## Subject
- Title: 合同明确要求 composit… (v5c_closed_loop_transfer_function_companion)
- Concept kind: representation_kind
- One-line definition (source of truth): 合同明确要求 composite/has_formula。
- Core intuition: 现有 Canonical 是 Formula，权威来源成员锚定式 (2-73)，同一原始公式块还完整给出式 (2-74)。
- Relation summary: 前置 → 闭环极点、闭环传递函数
- Extended explanation (do not paste verbatim on image): 合同明确要求 composite/has_formula。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「合同明确要求 comp…」、「属于·闭环极点」、「关联·闭环传递函数」
