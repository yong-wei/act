...
- One-line definition (source of truth): 非周期函数的频谱由傅氏变换 F(jω) 表示，为连续谱，描述信号谐波的分布特性。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 非周期函数的频谱由傅氏变换 F(jω) 表示，为连续谱，描述信号谐波的分布特性。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「非周期函数的频谱」
