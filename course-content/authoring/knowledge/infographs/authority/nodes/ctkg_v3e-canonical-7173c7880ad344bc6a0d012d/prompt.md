...
## Subject
- Title: Frequency respon (Frequency response matching method for order reduction)
- Concept kind: analysis_method
- One-line definition (source of truth): Frequency respon：A more sophisticated approach attempts to match the frequency response of the reduced-order transfer function with the…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A more sophisticated approach attempts to match the frequency response of the reduced-order transfe…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Frequency r…」
