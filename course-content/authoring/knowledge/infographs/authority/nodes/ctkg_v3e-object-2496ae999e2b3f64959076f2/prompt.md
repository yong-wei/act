...
## Subject
- Title: Graph determinant in signal-flow graph
- Concept kind: unknown
- One-line definition (source of truth): Graph determinant in signal-flow graph：The graph determinant Δ(s) is used in Mason's gain formula to evaluate the state transition matrix from the signal-flow…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The graph determinant Δ(s) is used in Mason's gain formula to evaluate the state transition matrix…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Graph deter…」
