...
## Subject
- Title: 信号流图中每个单独回路的增益 (modeling_405a39dbba1f6754b8e08afc)
- Concept kind: theoretical_construct
- One-line definition (source of truth): 信号流图中每个单独回路的增益。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 信号流图中每个单独回路的增益。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「信号流图中每个单独回路…」
