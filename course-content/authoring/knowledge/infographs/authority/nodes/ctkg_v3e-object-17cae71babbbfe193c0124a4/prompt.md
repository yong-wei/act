...
## Subject
- Title: Integration compensator
- Concept kind: unknown
- One-line definition (source of truth): Integration compensator：Integration compensator
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → Proportional plu、根轨迹 · 后续 → Compensator
- Extended explanation (do not paste verbatim on image): Integration compensator
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Integration…」、「Integration 」、「一种·Compen…」、「一种·Propor…」、「用于分析·根轨迹」
