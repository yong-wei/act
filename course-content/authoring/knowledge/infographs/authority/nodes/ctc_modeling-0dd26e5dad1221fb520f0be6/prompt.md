...
## Subject
- Title: 偏差向量至反馈向量之间的传递矩… (modeling_0dd26e5dad1221fb520f0be6)
- Concept kind: theoretical_construct
- One-line definition (source of truth): 偏差向量至反馈向量之间的传递矩阵 H(s)G(s)。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 偏差向量至反馈向量之间的传递矩阵 H(s)G(s)。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「偏差向量至反馈向量之间…」
