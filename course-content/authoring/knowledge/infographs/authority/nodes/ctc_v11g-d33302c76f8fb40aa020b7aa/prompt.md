...
## Subject
- Title: angle of the asymptotes (angle_of_the_asymptotes)
- Concept kind: mathematical_object
- One-line definition (source of truth): angle of the asymptotes：The angle φ_A that the asymptote makes with respect to the real axis.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The angle φ_A that the asymptote makes with respect to the real axis.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「angle of th…」、「angle of the」
