...
- One-line definition (source of truth): Equilibrium Point：A state where the system can remain at rest
- Core intuition: 在图谱邻接中可把握：前置 → Small-signal analysis、Lyapunov's First Method、Lyapunov function。
- Relation summary: 前置 → Small-signal analysis、Lyapunov's First Method、Lyapunov function
- Extended explanation (do not paste verbatim on image): A state where the system can remain at rest
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Equilibrium…」、「Equilibrium 」、「用于分析·Smal…」、「用于分析·Lyap…」、「应用于·Lyapu…」、「应用于·李雅普诺夫…」
