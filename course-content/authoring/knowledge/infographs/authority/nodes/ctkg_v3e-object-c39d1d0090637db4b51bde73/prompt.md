...
## Subject
- Title: 绘制概略开环幅相特性曲线方法
- Concept kind: unknown
- One-line definition (source of truth): 根据系统开环频率特性的表达式，通过取点、计算和作图等方法绘制系统开环幅相特性曲线。
- Core intuition: 概略开环幅相特性曲线应反映开环频率特性的三个重要因素：起点和终点、与实轴的交点、变化范围。
- Relation summary: 后续 → 频率特性
- Extended explanation (do not paste verbatim on image): 根据系统开环频率特性的表达式，通过取点、计算和作图等方法绘制系统开环幅相特性曲线。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「绘制概略开环幅相特性曲…」、「用于分析·频率特性」
