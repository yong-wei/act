...
## Subject
- Title: Tracking-error estimator
- Concept kind: unknown
- One-line definition (source of truth): Tracking-error estimator：A configuration where M and N̄ are chosen so that only the tracking error e = r - y is used in the control.
- Core intuition: 在图谱邻接中可把握：后续 → Tracking error。
- Relation summary: 后续 → Tracking error
- Extended explanation (do not paste verbatim on image): A configuration where M and N̄ are chosen so that only the tracking error e = r - y is used in the…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Tracking-er…」、「Tracking-err」、「应用于·Track…」
