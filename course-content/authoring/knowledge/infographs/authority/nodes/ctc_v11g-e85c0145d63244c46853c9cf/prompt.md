...
## Subject
- Title: lead compensation (lead_compensation)
- Concept kind: design_method
- One-line definition (source of truth): lead compensation：A compensation scheme that approximates PD control, used to speed up response by lowering rise time and decreasing tran…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A compensation scheme that approximates PD control, used to speed up response by lowering rise time…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「lead compen…」、「lead compens」
