...
## Subject
- Title: An ideal current source (modeling_ec763ecf3151981eb739b352)
- Concept kind: theoretical_construct
- One-line definition (source of truth): An ideal current source：An ideal current source delivers a specified current i_s regardless of voltage across its terminals.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): An ideal current source delivers a specified current i_s regardless of voltage across its terminals.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「An ideal cu…」
