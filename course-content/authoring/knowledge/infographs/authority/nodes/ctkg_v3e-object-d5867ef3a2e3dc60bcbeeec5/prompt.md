...
- One-line definition (source of truth): 系统运动的相变量（状态变量），即 x(t) 和 ẋ(t)。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 相平面、相轨迹 · 后续 → 相平面
- Extended explanation (do not paste verbatim on image): 系统运动的相变量（状态变量），即 x(t) 和 ẋ(t)。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「相变量」、「属于·相平面」、「包含组件·相平面」、「派生自·相轨迹」
