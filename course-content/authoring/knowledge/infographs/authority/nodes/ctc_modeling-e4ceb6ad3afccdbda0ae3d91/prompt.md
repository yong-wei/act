...
## Subject
- Title: 初始条件为零时 (modeling_e4ceb6ad3afccdbda0ae3d91)
- Concept kind: theoretical_construct
- One-line definition (source of truth): 初始条件为零时，输出向量的拉氏变换式与输入向量的拉氏变换式之间的传递关系。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 初始条件为零时，输出向量的拉氏变换式与输入向量的拉氏变换式之间的传递关系。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「初始条件为零时」
