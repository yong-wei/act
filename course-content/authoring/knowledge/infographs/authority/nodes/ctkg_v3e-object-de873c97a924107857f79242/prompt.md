...
- One-line definition (source of truth): 谐波输入下，输出响应中与输入同频率的谐波分量与谐波输入的幅值之比A(ω)。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 频率特性、相频特性
- Extended explanation (do not paste verbatim on image): 谐波输入下，输出响应中与输入同频率的谐波分量与谐波输入的幅值之比A(ω)。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「幅频特性」、「关联·频率特性」、「关联·相频特性」
