...
- One-line definition (source of truth): Stable System：A dynamic system with a bounded response to a bounded input.
- Core intuition: 在图谱邻接中可把握：前置 → Marginally Stable System、Bounded Input Bo、Neutral Stability。
- Relation summary: 前置 → Marginally Stable System、Bounded Input Bo、Neutral Stability
- Extended explanation (do not paste verbatim on image): A dynamic system with a bounded response to a bounded input.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Stable Syst…」、「Stable Syste」、「一种·Margin…」、「一种·Bounde…」、「一种·Neutra…」、「一种·Absolu…」、「一种·小范围稳定系统」
