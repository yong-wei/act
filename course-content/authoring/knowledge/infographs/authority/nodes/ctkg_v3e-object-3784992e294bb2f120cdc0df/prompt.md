...
- One-line definition (source of truth): Controllability matrix：A matrix used to determine whether a system is completely controllable
- Core intuition: 在图谱邻接中可把握：前置 → Ackermann's form · 后续 → Controllability。
- Relation summary: 前置 → Ackermann's form · 后续 → Controllability
- Extended explanation (do not paste verbatim on image): A matrix used to determine whether a system is completely controllable
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Controllabi…」、「Controllabil」、「包含组件·Acke…」、「用于分析·Cont…」
