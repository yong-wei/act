...
## Subject
- Title: Root locus metho (Root locus method for PI compensator design)
- Concept kind: unknown
- One-line definition (source of truth): Root locus metho：Adjusting the compensator zero so that the negative real part of the complex roots is zeta*omega_n=0.75, and determinin…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): Adjusting the compensator zero so that the negative real part of the complex roots is zeta*omega_n=…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Root locus …」
