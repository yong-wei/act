...
## Subject
- Title: Reduced-order estimator design method
- Concept kind: unknown
- One-line definition (source of truth): Reduced-order estimator design method：A method to design an estimator of reduced order by partitioning the state into measured and unmeasured parts, establis…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A method to design an estimator of reduced order by partitioning the state into measured and unmeas…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Reduced-ord…」
