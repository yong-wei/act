...
## Subject
- Title: Saturation Nonlinearity
- Concept kind: unknown
- One-line definition (source of truth): Saturation Nonlinearity：Saturation Nonlinearity
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → memoryless nonlinearity
- Extended explanation (do not paste verbatim on image): Saturation Nonlinearity
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Saturation …」、「Saturation N」、「一种·memory…」
