...
- One-line definition (source of truth): Input Compensation：A compensation scheme where the compensator is placed at the input.
- Core intuition: 在图谱邻接中可把握：后续 → Input compensation (as a scheme)、Compensation。
- Relation summary: 后续 → Input compensation (as a scheme)、Compensation
- Extended explanation (do not paste verbatim on image): A compensation scheme where the compensator is placed at the input.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Input Compe…」、「Input Compen」、「一种·Input」、「一种·Compen…」
