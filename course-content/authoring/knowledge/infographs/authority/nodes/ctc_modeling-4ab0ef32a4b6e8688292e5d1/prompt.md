...
## Subject
- Title: signal that is zero (modeling_4ab0ef32a4b6e8688292e5d1)
- Concept kind: theoretical_construct
- One-line definition (source of truth): signal that is zero：A signal δ(t) that is zero for t≠0 and has unit area.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A signal δ(t) that is zero for t≠0 and has unit area.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「signal that…」
