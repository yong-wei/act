...
- One-line definition (source of truth): 通过构造非奇异变换矩阵，将系统状态空间表达式变换为可观测规范分解形式的方法。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → 可观测性规范分解
- Extended explanation (do not paste verbatim on image): 通过构造非奇异变换矩阵，将系统状态空间表达式变换为可观测规范分解形式的方法
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「可观测性规范分解方法」、「一种·可观测性规范…」、「应用于·可观测性规…」
