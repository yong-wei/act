...
## Subject
- Title: Arc at infinity in Nyquist plot
- Concept kind: unknown
- One-line definition (source of truth): Arc at infinity in Nyquist plot：An arc at infinity in the Nyquist plot occurs when there is a pole on the imaginary axis, such as at ω=0.
- Core intuition: 在图谱邻接中可把握：后续 → C1 contour。
- Relation summary: 后续 → C1 contour
- Extended explanation (do not paste verbatim on image): An arc at infinity in the Nyquist plot occurs when there is a pole on the imaginary axis, such as a…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Arc at infi…」、「应用于·C1 co…」
