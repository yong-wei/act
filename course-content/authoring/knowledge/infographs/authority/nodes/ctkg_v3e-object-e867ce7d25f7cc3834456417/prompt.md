...
- One-line definition (source of truth): 采样角频率 ω_s = 2π/T。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 采样角频率 ω_s = 2π/T。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「采样角频率」
