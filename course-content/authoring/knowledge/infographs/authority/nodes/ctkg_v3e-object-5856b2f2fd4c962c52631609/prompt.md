...
## Subject
- Title: 阻尼比 (Damping ratio)
- Concept kind: unknown
- One-line definition (source of truth): 阻尼比：The overshoot to a step input can be related to M_pω through the damping ratio ζ.。
- Core intuition: 在图谱邻接中可把握：前置 → Resonant peak · 后续 → Resonant frequency、Resonant magnitude。
- Relation summary: 前置 → Resonant peak · 后续 → Resonant frequency、Resonant magnitude
- Extended explanation (do not paste verbatim on image): The overshoot to a step input can be related to M_pω through the damping ratio ζ.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「阻尼比」、「Damping rati」、「关联·Resona…」
