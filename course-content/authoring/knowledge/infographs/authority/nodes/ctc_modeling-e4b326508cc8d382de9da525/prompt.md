...
## Subject
- Title: When the numerator degree (modeling_e4b326508cc8d382de9da525)
- Concept kind: theoretical_construct
- One-line definition (source of truth): When the numerator degree：When the numerator degree m is less than the denominator degree n, the transfer function approaches zero as s approache…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): When the numerator degree m is less than the denominator degree n, the transfer function approaches…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「When the nu…」
