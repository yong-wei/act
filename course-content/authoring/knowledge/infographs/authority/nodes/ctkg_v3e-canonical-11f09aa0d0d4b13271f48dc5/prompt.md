...
- One-line definition (source of truth): 串级控制系统中的主调节器，传递函数为Gc1(s)。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 主回路 · 后续 → 主回路
- Extended explanation (do not paste verbatim on image): 串级控制系统中的主调节器，传递函数为Gc1(s)。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「主调节器」、「属于·主回路」、「包含组件·主回路」
