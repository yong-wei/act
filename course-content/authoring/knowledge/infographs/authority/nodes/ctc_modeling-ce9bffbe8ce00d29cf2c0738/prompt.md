...
## Subject
- Title: 既有输入支路又有输出支路的节点 (modeling_ce9bffbe8ce00d29cf2c0738)
- Concept kind: theoretical_construct
- One-line definition (source of truth): 既有输入支路又有输出支路的节点。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 既有输入支路又有输出支路的节点。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「既有输入支路又有输出支…」
