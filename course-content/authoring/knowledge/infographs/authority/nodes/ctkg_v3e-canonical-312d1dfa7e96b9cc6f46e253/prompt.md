...
- One-line definition (source of truth): 单位阶跃响应。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → 单位阶跃函数
- Extended explanation (do not paste verbatim on image): 单位阶跃响应
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「单位阶跃响应」、「派生自·单位阶跃函数」
