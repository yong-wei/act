...
- One-line definition (source of truth): 通过对z变换函数E(z)直接作综合除法，得到按z^{-1}升幂排列的幂级数展开式，从而得到采样脉冲序列的脉冲强度。
- Core intuition: 在图谱邻接中可把握：后续 → 反演积分法。
- Relation summary: 后续 → 反演积分法
- Extended explanation (do not paste verbatim on image): 通过对z变换函数E(z)直接作综合除法，得到按z^{-1}升幂排列的幂级数展开式，从而得到采样脉冲序列的脉冲强度。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「幂级数法」、「关联·反演积分法」
