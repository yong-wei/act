...
- One-line definition (source of truth): Laplace transform：Laplace transform
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → z-transform
- Extended explanation (do not paste verbatim on image): Laplace transform
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Laplace tra…」、「Laplace tran」、「关联·z-tran…」
