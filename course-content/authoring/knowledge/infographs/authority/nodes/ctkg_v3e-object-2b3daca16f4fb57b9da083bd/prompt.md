...
## Subject
- Title: Frequency-Respon (Frequency-Response Based Stability Analysis)
- Concept kind: unknown
- One-line definition (source of truth): Frequency-Respon：External stability analysis using frequency response methods such as the describing function and circle criterion.
- Core intuition: 在图谱邻接中可把握：前置 → Circle Criterion · 后续 → Describing Function Method。
- Relation summary: 前置 → Circle Criterion · 后续 → Describing Function Method
- Extended explanation (do not paste verbatim on image): External stability analysis using frequency response methods such as the describing function and ci…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Frequency-R…」、「一种·Circle」、「包含组件·Desc…」
