...
- One-line definition (source of truth): 复合控制系统是在系统的反馈控制回路中加入前馈通路，组成一个前馈控制与反馈控制相结合的系统。
- Core intuition: 在图谱邻接中可把握：后续 → 前馈通路。
- Relation summary: 后续 → 前馈通路
- Extended explanation (do not paste verbatim on image): 复合控制系统是在系统的反馈控制回路中加入前馈通路，组成一个前馈控制与反馈控制相结合的系统。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「复合控制系统」、「包含组件·前馈通路」
