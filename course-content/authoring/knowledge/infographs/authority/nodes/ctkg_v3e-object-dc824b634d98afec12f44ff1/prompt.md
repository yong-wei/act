...
## Subject
- Title: Control canonical form
- Concept kind: unknown
- One-line definition (source of truth): Control canonical form：A canonical form of state-space representation where the system matrix is in upper companion form, with the coefficient…
- Core intuition: 在图谱邻接中可把握：前置 → Pole placement b。
- Relation summary: 前置 → Pole placement b
- Extended explanation (do not paste verbatim on image): A canonical form of state-space representation where the system matrix is in upper companion form,…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Control can…」、「Control cano」、「用于分析·Pole…」
