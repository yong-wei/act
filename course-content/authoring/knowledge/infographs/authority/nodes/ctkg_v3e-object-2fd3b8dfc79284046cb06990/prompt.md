...
- One-line definition (source of truth): A/D 转换器是把连续的模拟信号转换为离散数字信号的装置。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 模拟信号 · 后续 → 数字信号
- Extended explanation (do not paste verbatim on image): A/D 转换器是把连续的模拟信号转换为离散数字信号的装置。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「A/D转换器」、「关联·模拟信号」、「关联·数字信号」
