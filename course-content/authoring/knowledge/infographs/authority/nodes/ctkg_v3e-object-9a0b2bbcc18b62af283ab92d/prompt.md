...
## Subject
- Title: Computing zeros  (Computing zeros via determinant condition)
- Concept kind: unknown
- One-line definition (source of truth): Computing zeros ：Solve det([zI - A, -B
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): Solve det([zI - A, -B
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Computing z…」
