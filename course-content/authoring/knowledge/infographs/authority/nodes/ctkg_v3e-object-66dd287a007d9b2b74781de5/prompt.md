...
- One-line definition (source of truth): 把系统中的离散信号是脉冲序列形式的离散系统，称为采样控制系统或脉冲控制系统。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 误差采样控制、开环采样系统、线性采样系统 · 后续 → 离散时间系统、采样过程
- Extended explanation (do not paste verbatim on image): 把系统中的离散信号是脉冲序列形式的离散系统，称为采样控制系统或脉冲控制系统。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「采样控制系统」、「是一种·误差采样控制」、「是一种·离散时间系统」、「是一种·开环采样系统」、「是一种·线性采样系统」、「是一种·闭环采样系统」
