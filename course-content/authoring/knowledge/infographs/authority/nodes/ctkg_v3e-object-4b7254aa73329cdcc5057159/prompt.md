...
- One-line definition (source of truth): Separation principle：The principle that the full-state feedback law and the observer can be designed independently
- Core intuition: 在图谱邻接中可把握：后续 → Full-state feedback design procedure。
- Relation summary: 后续 → Full-state feedback design procedure
- Extended explanation (do not paste verbatim on image): The principle that the full-state feedback law and the observer can be designed independently
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Separation …」、「Separation p」、「关联·Full-s…」
