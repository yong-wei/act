...
## Subject
- Title: 自然频率 (Natural frequency)
- Concept kind: unknown
- One-line definition (source of truth): 自然频率：The bandwidth of a system ω_B, as indicated on the frequency response, can be approximately related to the natural freq…
- Core intuition: 在图谱邻接中可把握：后续 → 带宽。
- Relation summary: 后续 → 带宽
- Extended explanation (do not paste verbatim on image): The bandwidth of a system ω_B, as indicated on the frequency response, can be approximately related…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「自然频率」、「Natural freq」、「关联·带宽」
