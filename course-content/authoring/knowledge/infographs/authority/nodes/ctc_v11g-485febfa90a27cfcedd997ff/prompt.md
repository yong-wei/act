...
## Subject
- Title: ultimate period (ultimate_period)
- Concept kind: performance_metric
- One-line definition (source of truth): ultimate period：The period of the sustained oscillations, denoted by P_U, used in the closed-loop Ziegler-Nichols tuning method.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The period of the sustained oscillations, denoted by P_U, used in the closed-loop Ziegler-Nichols t…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「ultimate pe…」、「ultimate per」
