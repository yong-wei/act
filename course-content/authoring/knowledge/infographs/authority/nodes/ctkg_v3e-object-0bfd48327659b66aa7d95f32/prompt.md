...
## Subject
- Title: Determining GM from root locus
- Concept kind: unknown
- One-line definition (source of truth): Determining GM from root locus：Determining GM from root locus
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → Gain Margin (GM) · 后续 → Gain Margin (GM)、Determining GM from Bode plot
- Extended explanation (do not paste verbatim on image): Determining GM from root locus
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Determining…」、「派生自·Gain …」、「关联·Gain M…」、「关联·Determ…」
