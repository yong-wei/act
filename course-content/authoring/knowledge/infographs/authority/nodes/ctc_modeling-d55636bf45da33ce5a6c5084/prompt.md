...
## Subject
- Title: The column vector that (modeling_d55636bf45da33ce5a6c5084)
- Concept kind: theoretical_construct
- One-line definition (source of truth): The column vector that：The column vector x that contains n elements for an nth-order system.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The column vector x that contains n elements for an nth-order system.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「The column …」
