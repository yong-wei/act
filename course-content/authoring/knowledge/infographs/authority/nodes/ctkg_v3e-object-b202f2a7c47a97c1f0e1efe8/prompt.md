...
- One-line definition (source of truth): 滞后网络的分度系数b，表示滞后程度，b<1。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → 分度系数
- Extended explanation (do not paste verbatim on image): 滞后网络的分度系数b，表示滞后程度，b<1
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「滞后深度」、「关联·分度系数」
