...
## Subject
- Title: 构造可观测性矩阵V=[C (Observability rank test method)
- Concept kind: unknown
- One-line definition (source of truth): 构造可观测性矩阵V=[C; CA; ...; CA^{n-1}]，检验rank V=n是否成立。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 构造可观测性矩阵V=[C; CA; ...; CA^{n-1}]，检验rank V=n是否成立。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「构造可观测性矩阵V=[C」
