...
- One-line definition (source of truth): Command following：An important aspect of feedback design involving the tracking of a reference input.
- Core intuition: 在图谱邻接中可把握：前置 → Reference input。
- Relation summary: 前置 → Reference input
- Extended explanation (do not paste verbatim on image): An important aspect of feedback design involving the tracking of a reference input.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Command fol…」、「Command foll」、「关联·Refere…」
