...
- One-line definition (source of truth): Discrete signal：Discrete signal
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → Discrete equivalent、Sampled-data system、Digital-to-analog (D/A) converter · 后续 → Sampled data、Sample period、Sample rate
- Extended explanation (do not paste verbatim on image): Discrete signal
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Discrete si…」、「Discrete sig」、「一种·Sample…」、「包含组件·Samp…」、「包含组件·Disc…」、「派生自·Sampl…」
