...
## Subject
- Title: 比例-微分控制改善系统性能的方法
- Concept kind: analysis_method
- One-line definition (source of truth): 适当选择开环增益和微分器时间常数，既可减小系统在斜坡输入时的稳态误差，又可使系统在阶跃输入时有满意的动态性能。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 适当选择开环增益和微分器时间常数，既可减小系统在斜坡输入时的稳态误差，又可使系统在阶跃输入时有满意的动态性能。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「比例-微分控制改善系统…」
