...
## Subject
- Title: 开环传递函数中时间常数形式表示… (open_loop_gain)
- Concept kind: mathematical_object
- One-line definition (source of truth): 开环传递函数中时间常数形式表示的增益，与根轨迹增益仅相差一个比例常数。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 开环传递函数中时间常数形式表示的增益，与根轨迹增益仅相差一个比例常数。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「开环传递函数中时间常数…」、「open loop ga」
