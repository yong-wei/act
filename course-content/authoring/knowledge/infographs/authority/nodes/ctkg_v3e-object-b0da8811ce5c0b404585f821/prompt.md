...
- One-line definition (source of truth): Stability boundary：The jω axis is the stability boundary between asymptotically stable and unstable response.
- Core intuition: 在图谱邻接中可把握：后续 → Neutral Stability。
- Relation summary: 后续 → Neutral Stability
- Extended explanation (do not paste verbatim on image): The jω axis is the stability boundary between asymptotically stable and unstable response.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Stability b…」、「Stability bo」、「关联·Neutra…」
