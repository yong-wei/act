...
- One-line definition (source of truth): Sampler：A switch that closes every T seconds for one instant of time.
- Core intuition: 在图谱邻接中可把握：前置 → Ideal sampler、Sampling period。
- Relation summary: 前置 → Ideal sampler、Sampling period
- Extended explanation (do not paste verbatim on image): A switch that closes every T seconds for one instant of time.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Sampler」、「一种·Ideal」、「应用于·Sampl…」
