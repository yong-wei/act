...
## Subject
- Title: 由幂级数 $\mathrm{e… (modeling_f7f31cc3da9f59e28b9239f5)
- Concept kind: theoretical_construct
- One-line definition (source of truth): 由幂级数 $\mathrm{e}^{A t}=\boldsymbol{I}+\boldsymbol{A} t+\frac{1}{2} \boldsymbol{A}^{2} t^{2}+\cdots+\frac{1}{k!} \boldsy…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 由幂级数 $\mathrm{e}^{A t}=\boldsymbol{I}+\boldsymbol{A} t+\frac{1}{2} \boldsymbol{A}^{2} t^{2}+\cdots+…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「由幂级数 $\math…」
