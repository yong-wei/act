...
## Subject
- Title: 常用的确定一阶系统时间常数的方…
- Concept kind: analysis_method
- One-line definition (source of truth): 初始斜率特性是常用的确定一阶系统时间常数的方法之一。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 初始斜率特性是常用的确定一阶系统时间常数的方法之一。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「常用的确定一阶系统时间…」
