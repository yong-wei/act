...
## Subject
- Title: 前向通路增益 (forward_path_gain)
- Concept kind: mathematical_object
- One-line definition (source of truth): 前向通路传递函数中时间常数形式表示的增益，记为K_G。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 前向通路传递函数中时间常数形式表示的增益，记为K_G。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「前向通路增益」、「forward path」
