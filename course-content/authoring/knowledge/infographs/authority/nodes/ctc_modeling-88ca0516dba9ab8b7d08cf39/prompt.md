...
## Subject
- Title: Time constant of the (modeling_88ca0516dba9ab8b7d08cf39)
- Concept kind: theoretical_construct
- One-line definition (source of truth): Time constant of the：Time constant of the armature circuit in a DC motor, defined as tau_a = L_a / R_a.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): Time constant of the armature circuit in a DC motor, defined as tau_a = L_a / R_a.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Time consta…」
