...
- One-line definition (source of truth): 当ζ=0时，系统的振荡频率为ω_n，称为无阻尼振荡频率。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 当ζ=0时，系统的振荡频率为ω_n，称为无阻尼振荡频率。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「无阻尼振荡频率」
