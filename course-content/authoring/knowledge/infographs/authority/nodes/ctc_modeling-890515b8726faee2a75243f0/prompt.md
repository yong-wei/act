...
## Subject
- Title: general element with an (modeling_890515b8726faee2a75243f0)
- Concept kind: theoretical_construct
- One-line definition (source of truth): general element with an：A general element with an excitation (through-) variable x(t) and a response (across-) variable y(t), where the relatio…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A general element with an excitation (through-) variable x(t) and a response (across-) variable y(t…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「general ele…」
