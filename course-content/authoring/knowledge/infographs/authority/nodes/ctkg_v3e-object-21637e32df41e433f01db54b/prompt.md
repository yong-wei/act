...
- One-line definition (source of truth): 等倾线法是求取相轨迹的一种作图方法，不需求解微分方程。
- Core intuition: 先确定相轨迹的等倾线，进而绘出相轨迹的切线方向场，然后从初始条件出发，沿方向场逐步绘制相轨迹。
- Relation summary: 前置 → 等倾线 · 后续 → 等倾线
- Extended explanation (do not paste verbatim on image): 等倾线法是求取相轨迹的一种作图方法，不需求解微分方程。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「等倾线法」、「属于·等倾线」、「包含组件·等倾线」
