...
- One-line definition (source of truth): s平面上的等ω水平线映射到z平面上的轨迹是一簇从原点出发的射线，其相角∠z=ωT从正实轴计量。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): s平面上的等ω水平线映射到z平面上的轨迹是一簇从原点出发的射线，其相角∠z=ωT从正实轴计量。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「等ω线映射」
