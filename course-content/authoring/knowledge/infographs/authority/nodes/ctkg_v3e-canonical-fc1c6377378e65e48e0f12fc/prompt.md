...
- One-line definition (source of truth): Time constant tau：The time when the response is 1/e times the initial value. It is a measure of the rate of decay.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The time when the response is 1/e times the initial value. It is a measure of the rate of decay.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Time consta…」、「Time constan」
