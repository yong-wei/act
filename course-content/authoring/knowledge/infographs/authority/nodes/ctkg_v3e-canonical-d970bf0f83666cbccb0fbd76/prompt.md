...
- One-line definition (source of truth): Steady state：The component y2(t) is called the steady state and equals the response given by Eq. (3.29).
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The component y2(t) is called the steady state and equals the response given by Eq. (3.29).
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Steady state」
