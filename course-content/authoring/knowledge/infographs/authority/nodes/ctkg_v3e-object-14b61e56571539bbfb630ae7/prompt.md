...
- One-line definition (source of truth): 当系统方程解析且工作在某一数值附近的较小范围内时，可运用小偏差法将非线性模型线性化。
- Core intuition: 在图谱邻接中可把握：后续 → 非线性系统。
- Relation summary: 后续 → 非线性系统
- Extended explanation (do not paste verbatim on image): 当系统方程解析且工作在某一数值附近的较小范围内时，可运用小偏差法将非线性模型线性化。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「小偏差线性化法」、「用于分析·非线性系统」、「应用于·非线性系统」
