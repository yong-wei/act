...
- One-line definition (source of truth): loop：v3T accepted candidate for loop.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → signal flow graph
- Extended explanation (do not paste verbatim on image): v3T accepted candidate for loop.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「loop」、「属于·signal…」
