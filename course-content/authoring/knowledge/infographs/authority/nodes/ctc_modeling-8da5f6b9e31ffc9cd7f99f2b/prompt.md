...
## Subject
- Title: 将严格有理真分式N(s)/D(… (modeling_8da5f6b9e31ffc9cd7f99f2b)
- Concept kind: analysis_method
- One-line definition (source of truth): 将严格有理真分式N(s)/D(s)串联分解为两部分，选取状态变量为 x1=z, x2=dz/dt, ..., xn=z^(n-1)，其中z为中间变量。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 将严格有理真分式N(s)/D(s)串联分解为两部分，选取状态变量为 x1=z, x2=dz/dt, ..., xn=z^(n-1)，其中z为中间变量。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「将严格有理真分式N(s…」
