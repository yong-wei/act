...
## Subject
- Title: two step parameter design method (two_step_parameter_design_method)
- Concept kind: design_method
- One-line definition (source of truth): two step parameter design method：A method to determine the effect of two parameters by repeating the root locus approach twice: first for one parameter,…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A method to determine the effect of two parameters by repeating the root locus approach twice: firs…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「two step pa…」
