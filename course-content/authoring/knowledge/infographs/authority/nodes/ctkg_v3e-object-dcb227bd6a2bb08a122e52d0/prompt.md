...
- One-line definition (source of truth): 复合校正方式是在反馈控制回路中，加人前馈校正通路，组成一个有机整体。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → 校正装置
- Extended explanation (do not paste verbatim on image): 复合校正方式是在反馈控制回路中，加人前馈校正通路，组成一个有机整体。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「复合校正」、「是一种·校正装置」
