...
## Subject
- Title: Position Error Constant (K p) (Position Error Constant (K_p))
- Concept kind: theoretical_construct
- One-line definition (source of truth): Position Error Constant (K p)：For a Type 0 system (no integrator in the loop transfer function), the constant defined as K_p = G D_cl(0). It determin…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): For a Type 0 system (no integrator in the loop transfer function), the constant defined as K_p = G…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Position Er…」
