...
## Subject
- Title: mathematical model (mathematical_model)
- Concept kind: theoretical_construct
- One-line definition (source of truth): mathematical model：v3T accepted candidate for mathematical model.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): v3T accepted candidate for mathematical model.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「mathematica…」、「mathematical」
