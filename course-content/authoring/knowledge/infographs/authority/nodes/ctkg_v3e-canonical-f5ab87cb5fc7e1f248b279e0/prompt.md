...
## Subject
- Title: Zeros at infinity
- Concept kind: theoretical_construct
- One-line definition (source of truth): Zeros at infinity：Zeros at infinity
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): Zeros at infinity
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Zeros at in…」、「Zeros at inf」
