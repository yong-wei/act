...
## Subject
- Title: 根轨迹增益 (root_locus_gain)
- Concept kind: mathematical_object
- One-line definition (source of truth): 在根轨迹绘制中，所研究的变化参数是根轨迹增益 K*。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → 增益
- Extended explanation (do not paste verbatim on image): 在根轨迹绘制中，所研究的变化参数是根轨迹增益 K*。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「根轨迹增益」、「root locus g」、「是一种·增益」
