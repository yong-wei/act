...
- One-line definition (source of truth): conformal mapping：The relationship z = e^{sT} defines a conformal mapping from the s-plane to the z-plane.
- Core intuition: 在图谱邻接中可把握：前置 → z-transform。
- Relation summary: 前置 → z-transform
- Extended explanation (do not paste verbatim on image): The relationship z = e^{sT} defines a conformal mapping from the s-plane to the z-plane.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「conformal m…」、「conformal ma」、「关联·z-tran…」
