...
## Subject
- Title: 流出端负载阀门的阻力 (modeling_caf9d1bfa4acd33993e1d039)
- Concept kind: theoretical_construct
- One-line definition (source of truth): 流出端负载阀门的阻力，定义为R=Δh/ΔQo，由非线性关系Qo=Ao√(2gh)在平衡点线性化得到。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 流出端负载阀门的阻力，定义为R=Δh/ΔQo，由非线性关系Qo=Ao√(2gh)在平衡点线性化得到。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「流出端负载阀门的阻力」
