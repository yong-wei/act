...
- One-line definition (source of truth): 对数频率特性曲线由对数幅频曲线和对数相频曲线组成，横坐标按lgω分度，对数幅频曲线纵坐标按L(ω)=20lg|G(jω)|线性分度，对数相频曲线纵坐标按φ(ω)线性分度。
- Core intuition: 在图谱邻接中可把握：前置 → 对数频率特性曲线。
- Relation summary: 前置 → 对数频率特性曲线
- Extended explanation (do not paste verbatim on image): 对数频率特性曲线由对数幅频曲线和对数相频曲线组成，横坐标按lgω分度，对数幅频曲线纵坐标按L(ω)=20lg|G(jω)|线性分度，对数相频曲线纵坐标按φ(ω)线性分度。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「对数频率特性曲线绘制方法」、「关联·对数频率特性…」
