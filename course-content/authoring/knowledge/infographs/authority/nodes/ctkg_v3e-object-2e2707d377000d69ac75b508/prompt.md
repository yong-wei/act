...
- One-line definition (source of truth): 以横轴为实轴、纵轴为虚轴构成的复数平面上，以频率ω为参变量绘制的频率特性曲线。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → 幅相频率特性曲线绘制方法
- Extended explanation (do not paste verbatim on image): 以横轴为实轴、纵轴为虚轴构成的复数平面上，以频率ω为参变量绘制的频率特性曲线。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「幅相频率特性曲线」、「关联·幅相频率特性…」
