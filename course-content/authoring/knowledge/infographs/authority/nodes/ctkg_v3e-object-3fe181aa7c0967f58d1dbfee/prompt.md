...
- One-line definition (source of truth): 具有比例控制规律的控制器，称为P控制器，实质上是一个具有可调增益的放大器，只改变信号的增益而不影响其相位。
- Core intuition: 在图谱邻接中可把握：前置 → P控制器 · 后续 → 比例系数。
- Relation summary: 前置 → P控制器 · 后续 → 比例系数
- Extended explanation (do not paste verbatim on image): 具有比例控制规律的控制器，称为P控制器，实质上是一个具有可调增益的放大器，只改变信号的增益而不影响其相位。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「比例控制规律」、「包含组件·P控制器」、「包含组件·比例系数」、「关联·P控制器」
