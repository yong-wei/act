...
- One-line definition (source of truth): s平面上从-ωs/2到ωs/2的周期带称为主要带，其余周期带称为次要带，均映射到z平面上的单位圆及单位圆内的负实轴。
- Core intuition: 在图谱邻接中可把握：后续 → s平面到z平面的映射。
- Relation summary: 后续 → s平面到z平面的映射
- Extended explanation (do not paste verbatim on image): s平面上从-ωs/2到ωs/2的周期带称为主要带，其余周期带称为次要带，均映射到z平面上的单位圆及单位圆内的负实轴。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「主要带与次要带」、「属于·s平面到z平…」
