...
- One-line definition (source of truth): 带宽：The bandwidth, ω_B, is a measure of a ability of the system to faithfully reproduce an input signal.。
- Core intuition: 在图谱邻接中可把握：前置 → 自然频率、Resonant frequency。
- Relation summary: 前置 → 自然频率、Resonant frequency
- Extended explanation (do not paste verbatim on image): The bandwidth, ω_B, is a measure of a ability of the system to faithfully reproduce an input signal.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「带宽」、「Bandwidth」、「关联·自然频率」、「关联·Resona…」
