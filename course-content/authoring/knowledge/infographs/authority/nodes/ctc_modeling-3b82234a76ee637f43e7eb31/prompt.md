...
## Subject
- Title: 1) 确定输入量和输出量 (modeling_3b82234a76ee637f43e7eb31)
- Concept kind: analysis_method
- One-line definition (source of truth): 1) 确定输入量和输出量；2) 列写微分方程；3) 消去中间变量。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 1) 确定输入量和输出量；2) 列写微分方程；3) 消去中间变量。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「1) 确定输入量和输出量」
