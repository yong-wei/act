...
- One-line definition (source of truth): 凡以一阶微分方程作为运动方程的控制系统，称为一阶系统。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 一阶系统的单位脉冲响应
- Extended explanation (do not paste verbatim on image): 凡以一阶微分方程作为运动方程的控制系统，称为一阶系统。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「一阶系统」、「派生自·一阶系统的…」
