...
## Subject
- Title: preloaded spring
- Concept kind: unknown
- One-line definition (source of truth): preloaded spring：preloaded spring
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → coulomb plus viscous friction · 后续 → memoryless nonlinearity
- Extended explanation (do not paste verbatim on image): preloaded spring
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「preloaded s…」、「preloaded sp」、「一种·memory…」、「关联·coulom…」
