...
- One-line definition (source of truth): 已校正系统中频区（-20dB/dec斜率）占据的频率范围宽度。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 已校正系统中频区（-20dB/dec斜率）占据的频率范围宽度。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「中频区宽度」
