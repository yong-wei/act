...
## Subject
- Title: 反应曲线参数 (reaction_curve_parameters)
- Concept kind: mathematical_object
- One-line definition (source of truth): 反应曲线参数：The reaction curve is characterized by the transport delay ΔT and the reaction rate R, which are estimated from the rec…
- Core intuition: 在图谱邻接中可把握：前置 → 反应速率、反应曲线 · 后续 → 传输延迟。
- Relation summary: 前置 → 反应速率、反应曲线 · 后续 → 传输延迟
- Extended explanation (do not paste verbatim on image): The reaction curve is characterized by the transport delay ΔT and the reaction rate R, which are es…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「反应曲线参数」、「关联·反应速率」、「关联·反应曲线」、「关联·传输延迟」
