...
## Subject
- Title: Nichols chart for compensated system
- Concept kind: unknown
- One-line definition (source of truth): Nichols chart for compensated system：The Nichols chart is used to check the closed-loop bandwidth and the reshaping of the frequency response locus.
- Core intuition: 在图谱邻接中可把握：后续 → Compensated system。
- Relation summary: 后续 → Compensated system
- Extended explanation (do not paste verbatim on image): The Nichols chart is used to check the closed-loop bandwidth and the reshaping of the frequency res…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Nichols cha…」、「用于分析·Comp…」
