...
## Subject
- Title: monic polynomial (monic_polynomial)
- Concept kind: mathematical_object
- One-line definition (source of truth): monic polynomial：A polynomial whose coefficient of the highest power of s is 1.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A polynomial whose coefficient of the highest power of s is 1.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「monic polyn…」、「monic polyno」
