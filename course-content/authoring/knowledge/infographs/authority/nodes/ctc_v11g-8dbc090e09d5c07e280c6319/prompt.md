...
## Subject
- Title: 静态速度误差系数 (static_velocity_error_constant)
- Concept kind: performance_metric
- One-line definition (source of truth): 对于I型系统，根轨迹上的K值就是静态速度误差系数。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → I型系统
- Extended explanation (do not paste verbatim on image): 对于I型系统，根轨迹上的K值就是静态速度误差系数。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「静态速度误差系数」、「关联·I型系统」
