...
## Subject
- Title: Negative Real Part of Pole
- Concept kind: theoretical_construct
- One-line definition (source of truth): Negative Real Part of Pole：The real part of a complex pole, denoted by σ, which determines the exponential decay rate of the transient response.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The real part of a complex pole, denoted by σ, which determines the exponential decay rate of the t…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Negative Re…」
