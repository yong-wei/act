...
## Subject
- Title: break frequency (corner frequency)
- Concept kind: unknown
- One-line definition (source of truth): break frequency (corner frequency)：The frequency ω=1/τ is often called the break frequency or corner frequency.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The frequency ω=1/τ is often called the break frequency or corner frequency.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「break frequ…」
