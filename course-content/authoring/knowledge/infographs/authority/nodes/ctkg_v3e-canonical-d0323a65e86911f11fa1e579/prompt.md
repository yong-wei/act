...
- One-line definition (source of truth): 串级控制系统中，由副调节器、被控对象和测量变送器构成的内部回路。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 副调节器、串级控制方法、串级控制系统 · 后续 → 副调节器
- Extended explanation (do not paste verbatim on image): 串级控制系统中，由副调节器、被控对象和测量变送器构成的内部回路。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「副回路」、「属于·副调节器」、「包含组件·串级控制…」、「包含组件·副调节器」
