...
## Subject
- Title: Cover-up method  (Cover-up method for partial fraction expansion)
- Concept kind: analysis_method
- One-line definition (source of truth): Cover-up method ：This process is called the cover-up method because, in the factored form of F(s), we can cover up the individual denomi…
- Core intuition: 在图谱邻接中可把握：后续 → Cover-up method。
- Relation summary: 后续 → Cover-up method
- Extended explanation (do not paste verbatim on image): This process is called the cover-up method because, in the factored form of F(s), we can cover up t…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Cover-up me…」、「一种·Cover-…」
