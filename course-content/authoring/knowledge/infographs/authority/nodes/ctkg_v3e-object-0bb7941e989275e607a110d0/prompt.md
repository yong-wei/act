...
## Subject
- Title: Using ss2tf to c (Using ss2tf to compute transfer function from state-space model)
- Concept kind: unknown
- One-line definition (source of truth): Using ss2tf to c：Using ss2tf to compute transfer function from state-space model
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): Using ss2tf to compute transfer function from state-space model
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Using ss2tf…」
