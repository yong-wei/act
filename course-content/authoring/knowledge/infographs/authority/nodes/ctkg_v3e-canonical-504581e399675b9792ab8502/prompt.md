...
- One-line definition (source of truth): Bode plots：Presenting frequency response in this manner (that is, on a log-log scale) was originated by H. W. Bode
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): Presenting frequency response in this manner (that is, on a log-log scale) was originated by H. W.…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Bode plots」
