...
- One-line definition (source of truth): 描述稳定的系统在单位阶跃函数作用下，动态过程随时间 t 的变化状况的指标。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → 峰值时间、调节时间、上升时间
- Extended explanation (do not paste verbatim on image): 描述稳定的系统在单位阶跃函数作用下，动态过程随时间 t 的变化状况的指标。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「动态性能指标」、「包含组件·峰值时间」、「包含组件·调节时间」、「包含组件·上升时间」、「包含组件·超调量」
