...
- One-line definition (source of truth): Time Constant：For a first-order system, the time constant is τ=1/σ.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): For a first-order system, the time constant is τ=1/σ.
