...
- One-line definition (source of truth): 测量元件、执行元件和被控对象是模拟元件，其输入和输出是连续信号，即时间上和幅值上都连续的信号，称为模拟信号。
- Core intuition: 在图谱邻接中可把握：前置 → 阶梯信号、采样控制系统、数字控制系统 · 后续 → A/D转换器、D/A转换器。
- Relation summary: 前置 → 阶梯信号、采样控制系统、数字控制系统 · 后续 → A/D转换器、D/A转换器
- Extended explanation (do not paste verbatim on image): 测量元件、执行元件和被控对象是模拟元件，其输入和输出是连续信号，即时间上和幅值上都连续的信号，称为模拟信号。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「模拟信号」、「是一种·阶梯信号」、「包含组件·采样控制…」、「包含组件·数字控制…」、「关联·采样过程」、「关联·A/D转换器」
