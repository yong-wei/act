...
- One-line definition (source of truth): 如果控制系统中的所有信号都是时间变量的连续函数，换句话说，这些信号在全部时间上都是已知的，则这样的系统称为连续时间系统，简称连续系统。
- Core intuition: 在图谱邻接中可把握：前置 → 离散时间系统。
- Relation summary: 前置 → 离散时间系统
- Extended explanation (do not paste verbatim on image): 如果控制系统中的所有信号都是时间变量的连续函数，换句话说，这些信号在全部时间上都是已知的，则这样的系统称为连续时间系统，简称连续系统。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「连续时间系统」、「关联·离散时间系统」
