...
- One-line definition (source of truth): 如果信息之间的间隔是时变的，或随机的，则称为非周期采样，或随机采样。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 周期采样
- Extended explanation (do not paste verbatim on image): 如果信息之间的间隔是时变的，或随机的，则称为非周期采样，或随机采样。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「非周期采样」、「关联·周期采样」
