...
## Subject
- Title: variable parameter k (variable_parameter_k)
- Concept kind: mathematical_object
- One-line definition (source of truth): variable parameter k：A variable parameter in the characteristic equation 1+KG(s)=0, where 0 ≤ K < ∞.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A variable parameter in the characteristic equation 1+KG(s)=0, where 0 ≤ K < ∞.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「variable pa…」、「variable par」
