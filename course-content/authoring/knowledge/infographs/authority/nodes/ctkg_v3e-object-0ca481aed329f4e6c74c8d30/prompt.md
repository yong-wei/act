...
## Subject
- Title: Experimental fre (Experimental frequency response determination)
- Concept kind: unknown
- One-line definition (source of truth): Experimental fre：Excite the system with a sinusoid varying in frequency
- Core intuition: 在图谱邻接中可把握：后续 → Frequency Response。
- Relation summary: 后续 → Frequency Response
- Extended explanation (do not paste verbatim on image): Excite the system with a sinusoid varying in frequency
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Experimenta…」、「用于分析·Freq…」
