...
- One-line definition (source of truth): 将输入序列 r(n) 变换为输出序列 c(n) 的一种变换关系，称为离散系统。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 线性离散系统
- Extended explanation (do not paste verbatim on image): 将输入序列 r(n) 变换为输出序列 c(n) 的一种变换关系，称为离散系统。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「离散系统」、「是一种·线性离散系统」
