...
- One-line definition (source of truth): Lyapunov Direct Method是自动控制原理权威图谱中的领域概念。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → Positive Definite Function、state space model、Lyapunov Stability
- Extended explanation (do not paste verbatim on image): 权威图谱尚未提供足够描述，本卡仅作占位，待补描述后重写。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Lyapunov Di…」、「Lyapunov Dir」、「一种·Positi…」、「用于分析·stat…」、「用于分析·Lyap…」、「应用于·Equil…」
