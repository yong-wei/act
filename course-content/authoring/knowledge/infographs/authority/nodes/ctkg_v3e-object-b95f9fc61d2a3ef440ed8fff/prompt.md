...
## Subject
- Title: Truncating numer (Truncating numerator polynomial to remove extraneous roots)
- Concept kind: unknown
- One-line definition (source of truth): Truncating numer：Truncating numerator polynomial to remove extraneous roots
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): Truncating numerator polynomial to remove extraneous roots
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Truncating …」
