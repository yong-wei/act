...
- One-line definition (source of truth): 绝对式轴角编码。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → 多极双通道旋转变压器
- Extended explanation (do not paste verbatim on image): 绝对式轴角编码
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「绝对式轴角编码」、「包含组件·多极双通…」
