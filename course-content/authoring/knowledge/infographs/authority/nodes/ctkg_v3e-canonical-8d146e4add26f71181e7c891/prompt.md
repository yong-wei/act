...
## Subject
- Title: Right half-plane (RHP)
- Concept kind: theoretical_construct
- One-line definition (source of truth): Right half-plane (RHP)：Right half-plane (RHP)
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → Left half-plane (LHP)
- Extended explanation (do not paste verbatim on image): Right half-plane (RHP)
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Right half-…」、「Right half-p」、「关联·Left h…」
