...
## Subject
- Title: dynamic system (dynamic_system)
- Concept kind: theoretical_construct
- One-line definition (source of truth): dynamic system：v3T accepted candidate for dynamic system.
- Core intuition: 在图谱邻接中可把握：后续 → 在时间域描述系统动态性能的数学…、state space model、传递函数。
- Relation summary: 后续 → 在时间域描述系统动态性能的数学…、state space model、传递函数
- Extended explanation (do not paste verbatim on image): v3T accepted candidate for dynamic system.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「dynamic sys…」、「dynamic syst」、「有表示·在时间域描…」、「有表示·state…」、「有表示·传递函数」
