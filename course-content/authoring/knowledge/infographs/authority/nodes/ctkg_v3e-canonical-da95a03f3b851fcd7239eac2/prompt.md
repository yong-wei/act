...
## Subject
- Title: Design Synthesis (Design Synthesis Using Time-Domain Specifications)
- Concept kind: analysis_method
- One-line definition (source of truth): Design Synthesis：A method to specify t_r, M_p, and t_s and determine where poles need to be so that actual responses are less than or eq…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A method to specify t_r, M_p, and t_s and determine where poles need to be so that actual responses…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Design Synt…」
