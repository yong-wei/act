...
- One-line definition (source of truth): Reduced-order observer：An observer that provides estimates only for the states not directly measured, as opposed to a full-state observer.
- Core intuition: 在图谱邻接中可把握：后续 → Full-state observer。
- Relation summary: 后续 → Full-state observer
- Extended explanation (do not paste verbatim on image): An observer that provides estimates only for the states not directly measured, as opposed to a full…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Reduced-ord…」、「Reduced-orde」、「关联·Full-s…」
