...
- One-line definition (source of truth): 当特征根为一对具有负实部的共轭复根时，奇点为稳定焦点。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → 奇点
- Extended explanation (do not paste verbatim on image): 当特征根为一对具有负实部的共轭复根时，奇点为稳定焦点。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「稳定焦点」、「是一种·奇点」
