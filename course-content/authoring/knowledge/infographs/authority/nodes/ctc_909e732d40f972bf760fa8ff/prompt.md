...
## Subject
- Title: v5c disturbance targeted (v5c_disturbance_targeted)
- Concept kind: signal_role
- One-line definition (source of truth): v5c disturbance targeted：In spacecraft attitude control, disturbances are torques that are ignored in the dynamic model and treated as disturban…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): In spacecraft attitude control, disturbances are torques that are ignored in the dynamic model and…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「v5c disturb…」、「v5c disturba」
