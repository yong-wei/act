...
- One-line definition (source of truth): 在控制工程中，为简化惯性环节、一阶微分环节、振荡环节和二阶微分环节的对数幅频曲线的作图，常用低频和高频渐近线近似表示对数幅频曲线，称之为对数幅频渐近特性曲线。
- Core intuition: 在图谱邻接中可把握：前置 → 交接频率、低频段 · 后续 → 截止频率。
- Relation summary: 前置 → 交接频率、低频段 · 后续 → 截止频率
- Extended explanation (do not paste verbatim on image): 在控制工程中，为简化惯性环节、一阶微分环节、振荡环节和二阶微分环节的对数幅频曲线的作图，常用低频和高频渐近线近似表示对数幅频曲线，称之为对数幅频渐近特性曲线。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「对数幅频渐近特性曲线」、「属于·交接频率」、「用于分析·截止频率」、「应用于·低频段」
