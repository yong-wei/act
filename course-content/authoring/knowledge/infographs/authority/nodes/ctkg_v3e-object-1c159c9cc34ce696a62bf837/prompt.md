...
- One-line definition (source of truth): 开环频率特性在截止频率处的相角与-180°的差值，记为γ。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 相频特性、利用开环频域指标估算时域性能方法 · 后续 → 截止频率、利用开环频域指标估算时域性能方法
- Extended explanation (do not paste verbatim on image): 开环频率特性在截止频率处的相角与-180°的差值，记为γ
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「相角裕度」、「用于分析·相频特性」、「用于分析·利用开环…」、「关联·截止频率」、「关联·利用开环频域…」
