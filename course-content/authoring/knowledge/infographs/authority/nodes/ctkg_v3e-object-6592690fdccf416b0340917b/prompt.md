...
- One-line definition (source of truth): 系统闭环幅频特性下降到某一阈值时的频率，记为ω_b。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 得到闭环对数幅频和相频曲线、谐振峰值、截止频率
- Extended explanation (do not paste verbatim on image): 系统闭环幅频特性下降到某一阈值时的频率，记为ω_b
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「带宽频率」、「用于分析·得到闭环…」、「关联·谐振峰值」、「关联·得到闭环对数…」、「关联·截止频率」、「关联·谐振频率」
