...
## Subject
- Title: Neglecting insig (Neglecting insignificant poles for order reduction)
- Concept kind: analysis_method
- One-line definition (source of truth): Neglecting insig：One relatively simple way to delete a certain insignificant pole of a transfer function is to note a pole that has a ne…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): One relatively simple way to delete a certain insignificant pole of a transfer function is to note…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Neglecting …」
