...
## Subject
- Title: Nonlinear Element with Sinusoidal Input
- Concept kind: unknown
- One-line definition (source of truth): Nonlinear Element with Sinusoidal Input：Nonlinear Element with Sinusoidal Input
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → Describing Function · 后续 → Describing Function
- Extended explanation (do not paste verbatim on image): Nonlinear Element with Sinusoidal Input
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Nonlinear E…」、「应用于·Descr…」、「关联·Descri…」
