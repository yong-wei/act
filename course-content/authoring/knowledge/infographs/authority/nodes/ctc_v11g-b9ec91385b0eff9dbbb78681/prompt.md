...
## Subject
- Title: borderline stability (borderline_stability)
- Concept kind: system_property
- One-line definition (source of truth): borderline stability：The condition at which the system is on the verge of instability, characterized by purely imaginary roots.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The condition at which the system is on the verge of instability, characterized by purely imaginary…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「borderline …」、「borderline s」
