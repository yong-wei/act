...
- One-line definition (source of truth): ζ = 1/(2√(T_m K)) 为阻尼比（或相对阻尼系数）。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 测速反馈控制
- Extended explanation (do not paste verbatim on image): ζ = 1/(2√(T_m K)) 为阻尼比（或相对阻尼系数）。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「阻尼比」、「关联·测速反馈控制」
