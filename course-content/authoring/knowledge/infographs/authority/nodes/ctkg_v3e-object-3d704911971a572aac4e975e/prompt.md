...
- One-line definition (source of truth): 将非线性特性视为一个环节，环节的输入为x，输出为y，按照线性系统中比例环节的描述，定义非线性环节输出y和输入x的比值为等效增益。
- Core intuition: 在图谱邻接中可把握：后续 → 摩擦特性、间隙特性、饱和特性。
- Relation summary: 后续 → 摩擦特性、间隙特性、饱和特性
- Extended explanation (do not paste verbatim on image): 将非线性特性视为一个环节，环节的输入为x，输出为y，按照线性系统中比例环节的描述，定义非线性环节输出y和输入x的比值为等效增益。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「等效增益」、「关联·摩擦特性」、「关联·间隙特性」、「关联·饱和特性」、「关联·死区特性」
