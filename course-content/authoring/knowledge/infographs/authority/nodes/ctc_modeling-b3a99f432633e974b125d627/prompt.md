...
## Subject
- Title: 可控标准型与可观测标准型之间的… (modeling_b3a99f432633e974b125d627)
- Concept kind: theoretical_construct
- One-line definition (source of truth): 可控标准型与可观测标准型之间的对偶关系。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 可控标准型与可观测标准型之间的对偶关系。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「可控标准型与可观测标准…」
