...
- One-line definition (source of truth): Convolution：The response of an LTI system can be expressed as the convolution of the input with the unit impulse response of the sy…
- Core intuition: 在图谱邻接中可把握：前置 → Computing response via convolution。
- Relation summary: 前置 → Computing response via convolution
- Extended explanation (do not paste verbatim on image): The response of an LTI system can be expressed as the convolution of the input with the unit impuls…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Convolution」、「包含组件·Comp…」
