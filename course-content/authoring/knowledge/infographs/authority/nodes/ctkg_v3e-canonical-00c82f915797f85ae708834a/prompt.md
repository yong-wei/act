...
- One-line definition (source of truth): 在复合控制系统中，前馈通路是加入反馈控制回路中的前向通路，用于组成前馈控制与反馈控制相结合的系统。
- Core intuition: 在图谱邻接中可把握：前置 → 前馈控制、复合控制系统。
- Relation summary: 前置 → 前馈控制、复合控制系统
- Extended explanation (do not paste verbatim on image): 在复合控制系统中，前馈通路是加入反馈控制回路中的前向通路，用于组成前馈控制与反馈控制相结合的系统。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「前馈通路」、「包含组件·前馈控制」、「包含组件·复合控制…」
