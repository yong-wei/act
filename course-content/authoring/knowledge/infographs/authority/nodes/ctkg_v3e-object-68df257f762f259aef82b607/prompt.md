...
## Subject
- Title: Full-Order Estimator
- Concept kind: unknown
- One-line definition (source of truth): Full-Order Estimator：A method of estimating the state by constructing a full-order model of the plant dynamics.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A method of estimating the state by constructing a full-order model of the plant dynamics.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Full-Order …」、「Full-Order E」
