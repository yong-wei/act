...
## Subject
- Title: 当闭环极点为复数极点时 (underdamped_system)
- Concept kind: system_kind
- One-line definition (source of truth): 当闭环极点为复数极点时，系统为欠阻尼系统，单位阶跃响应为阻尼振荡过程。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 当闭环极点为复数极点时，系统为欠阻尼系统，单位阶跃响应为阻尼振荡过程。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「当闭环极点为复数极点时」、「underdamped 」
