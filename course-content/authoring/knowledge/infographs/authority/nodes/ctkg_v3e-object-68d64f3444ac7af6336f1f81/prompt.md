...
- One-line definition (source of truth): PI控制器。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → 比例-积分控制规律
- Extended explanation (do not paste verbatim on image): PI控制器
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「PI控制器」、「关联·比例-积分控…」
