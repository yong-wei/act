...
- One-line definition (source of truth): 数字信号。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 数字控制系统、A/D转换器 · 后续 → D/A转换器
- Extended explanation (do not paste verbatim on image): 数字信号
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「数字信号」、「包含组件·数字控制…」、「关联·A/D转换器」、「关联·D/A转换器」
