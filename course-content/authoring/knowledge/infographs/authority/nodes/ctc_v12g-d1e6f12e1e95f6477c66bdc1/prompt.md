...
## Subject
- Title: break in point (break_in_point)
- Concept kind: phenomenon
- One-line definition (source of truth): break in point：A point on the root locus where two or more roots come into the real axis.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A point on the root locus where two or more roots come into the real axis.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「break in po…」、「break in poi」
