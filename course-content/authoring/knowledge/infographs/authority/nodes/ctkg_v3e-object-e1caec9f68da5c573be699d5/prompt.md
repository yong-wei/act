...
- One-line definition (source of truth): 串联校正装置一般接在系统误差测量点之后，串接于系统前向通道之中。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → 校正装置
- Extended explanation (do not paste verbatim on image): 串联校正装置一般接在系统误差测量点之后，串接于系统前向通道之中。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「串联校正」、「是一种·校正装置」
