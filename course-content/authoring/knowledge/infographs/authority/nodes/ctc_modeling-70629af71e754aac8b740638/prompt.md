...
## Subject
- Title: procedure to obtain set (modeling_70629af71e754aac8b740638)
- Concept kind: analysis_method
- One-line definition (source of truth): procedure to obtain set：A procedure to obtain a set of linearized equations describing a nonlinear system by expanding the nonlinear functions…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A procedure to obtain a set of linearized equations describing a nonlinear system by expanding the…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「procedure t…」
