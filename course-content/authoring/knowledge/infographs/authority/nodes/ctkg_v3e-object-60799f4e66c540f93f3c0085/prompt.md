...
- One-line definition (source of truth): 线性定常离散系统中，单位脉冲响应序列K(nT)的专有名称，表示对连续信号采样时每一时刻的脉冲值等于该时刻的函数值。
- Core intuition: 在图谱邻接中可把握：后续 → 线性定常离散系统。
- Relation summary: 后续 → 线性定常离散系统
- Extended explanation (do not paste verbatim on image): 线性定常离散系统中，单位脉冲响应序列K(nT)的专有名称，表示对连续信号采样时每一时刻的脉冲值等于该时刻的函数值。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「加权序列」、「关联·线性定常离散…」
