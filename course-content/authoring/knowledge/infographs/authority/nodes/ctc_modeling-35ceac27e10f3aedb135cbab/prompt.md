...
## Subject
- Title: An matrix in the (modeling_35ceac27e10f3aedb135cbab)
- Concept kind: theoretical_construct
- One-line definition (source of truth): An matrix in the：An n x 1 matrix in the state-variable form.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): An n x 1 matrix in the state-variable form.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「An matrix i…」
