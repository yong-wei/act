...
- One-line definition (source of truth): 如果离散系统满足叠加原理，则称为线性离散系统。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 线性定常离散系统 · 后续 → 离散系统
- Extended explanation (do not paste verbatim on image): 如果离散系统满足叠加原理，则称为线性离散系统。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「线性离散系统」、「是一种·离散系统」、「一种·线性定常离散…」
