...
- One-line definition (source of truth): Sampling period：The fixed period at which all numbers enter or leave the computer.
- Core intuition: 在图谱邻接中可把握：后续 → Sampler。
- Relation summary: 后续 → Sampler
- Extended explanation (do not paste verbatim on image): The fixed period at which all numbers enter or leave the computer.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Sampling pe…」、「Sampling per」、「应用于·Sampl…」
