...
## Subject
- Title: Frequency-domain performance measures
- Concept kind: unknown
- One-line definition (source of truth): Frequency-domain performance measures：Performance measures used to describe control system behavior in the frequency domain, including peak of closed-loop fr…
- Core intuition: 在图谱邻接中可把握：前置 → Frequency response method。
- Relation summary: 前置 → Frequency response method
- Extended explanation (do not paste verbatim on image): Performance measures used to describe control system behavior in the frequency domain, including pe…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Frequency-d…」、「应用于·Frequ…」
