...
## Subject
- Title: gain with dead zone
- Concept kind: unknown
- One-line definition (source of truth): gain with dead zone：gain with dead zone
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → memoryless nonlinearity
- Extended explanation (do not paste verbatim on image): gain with dead zone
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「gain with d…」、「gain with de」、「一种·memory…」
