...
## Subject
- Title: Relay Nonlinearity
- Concept kind: unknown
- One-line definition (source of truth): Relay Nonlinearity：Relay Nonlinearity
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): Relay Nonlinearity
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Relay Nonli…」、「Relay Nonlin」
