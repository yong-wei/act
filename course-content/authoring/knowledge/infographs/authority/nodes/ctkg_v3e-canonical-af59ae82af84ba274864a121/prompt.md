...
- One-line definition (source of truth): 一阶系统在理想单位脉冲函数输入下的输出响应，表达式为c(t)=(1/T)e^{-t/T}。
- Core intuition: 在图谱邻接中可把握：后续 → 一阶系统。
- Relation summary: 后续 → 一阶系统
- Extended explanation (do not paste verbatim on image): 一阶系统在理想单位脉冲函数输入下的输出响应，表达式为c(t)=(1/T)e^{-t/T}
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「一阶系统的单位脉冲响应」、「派生自·一阶系统」
