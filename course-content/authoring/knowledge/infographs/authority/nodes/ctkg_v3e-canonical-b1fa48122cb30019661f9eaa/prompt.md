...
## Subject
- Title: Peak time T p (Peak time T_p)
- Concept kind: theoretical_construct
- One-line definition (source of truth): Peak time T p：The time required for the response to reach the first peak of the overshoot.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The time required for the response to reach the first peak of the overshoot.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Peak time T…」、「Peak time T 」
