...
- One-line definition (source of truth): Feedback Compensation：A compensation scheme where the compensator is placed in the feedback path.
- Core intuition: 在图谱邻接中可把握：前置 → Feedback compensation (as a scheme) · 后续 → Compensation。
- Relation summary: 前置 → Feedback compensation (as a scheme) · 后续 → Compensation
- Extended explanation (do not paste verbatim on image): A compensation scheme where the compensator is placed in the feedback path.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Feedback Co…」、「Feedback Com」、「一种·Feedba…」、「一种·Compen…」
