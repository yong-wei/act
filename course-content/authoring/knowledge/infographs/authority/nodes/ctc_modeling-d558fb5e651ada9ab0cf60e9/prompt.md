...
## Subject
- Title: 离散化系统中用于描述状态转移的… (modeling_d558fb5e651ada9ab0cf60e9)
- Concept kind: theoretical_construct
- One-line definition (source of truth): 离散化系统中用于描述状态转移的矩阵，记为 Φ(k)，满足 x(k) = Φ(k) x(0)。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 离散化系统中用于描述状态转移的矩阵，记为 Φ(k)，满足 x(k) = Φ(k) x(0)。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「离散化系统中用于描述状…」
