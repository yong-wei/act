...
- One-line definition (source of truth): 把数字序列形式的离散系统，称为数字控制系统或计算机控制系统。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → 离散时间系统、模拟信号、数字信号
- Extended explanation (do not paste verbatim on image): 把数字序列形式的离散系统，称为数字控制系统或计算机控制系统。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「数字控制系统」、「是一种·离散时间系统」、「包含组件·模拟信号」、「包含组件·数字信号」
