...
## Subject
- Title: Velocity constant
- Concept kind: unknown
- One-line definition (source of truth): Velocity constant：The steady-state error for a ramp input is specified in terms of K_v, the velocity constant.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The steady-state error for a ramp input is specified in terms of K_v, the velocity constant.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Velocity co…」、「Velocity con」
