...
- One-line definition (source of truth): random-access memory (RAM)：random-access memory (RAM)
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → read-only memory (ROM)
- Extended explanation (do not paste verbatim on image): random-access memory (RAM)
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「random-acce…」、「关联·read-o…」
