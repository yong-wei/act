...
## Subject
- Title: 闭环传递函数 (closed_loop_transfer_function)
- Concept kind: representation_kind
- One-line definition (source of truth): 闭环传递函数：The transfer function describing the dynamic performance of a closed-loop control system, given by T(s)=Y(s)/R(s)=p(s)/…
- Core intuition: 在图谱邻接中可把握：后续 → 合同明确要求 composit…。
- Relation summary: 后续 → 合同明确要求 composit…
- Extended explanation (do not paste verbatim on image): The transfer function describing the dynamic performance of a closed-loop control system, given by…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「闭环传递函数」、「关联·合同明确要求…」
