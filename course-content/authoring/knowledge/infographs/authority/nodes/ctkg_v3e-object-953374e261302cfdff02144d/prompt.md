...
- One-line definition (source of truth): 以 x(t) 为横坐标，ẋ(t) 为纵坐标构成的直角坐标平面称为相平面。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 等倾线、极限环、相轨迹
- Extended explanation (do not paste verbatim on image): 以 x(t) 为横坐标，ẋ(t) 为纵坐标构成的直角坐标平面称为相平面。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「相平面」、「属于·等倾线」、「属于·极限环」、「属于·相轨迹」、「属于·奇线」、「属于·奇点」
