...
## Subject
- Title: feedback loop in signal-flow (modeling_44e603669ad0e6bf331b1511)
- Concept kind: theoretical_construct
- One-line definition (source of truth): feedback loop in signal-flow：A feedback loop in a signal-flow graph that starts and ends at the same node.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A feedback loop in a signal-flow graph that starts and ends at the same node.
