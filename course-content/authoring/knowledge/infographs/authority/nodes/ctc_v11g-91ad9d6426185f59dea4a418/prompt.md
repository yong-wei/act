...
## Subject
- Title: 稳态误差 (steady_state_error)
- Concept kind: performance_metric
- One-line definition (source of truth): 稳态误差：The finite error that remains after the output response follows a ramp input.。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The finite error that remains after the output response follows a ramp input.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「稳态误差」、「steady state」
