...
## Subject
- Title: Partial-fraction (Partial-fraction expansion method for distinct poles)
- Concept kind: analysis_method
- One-line definition (source of truth): Partial-fraction：Partial-fraction expansion method for distinct poles
- Core intuition: 在图谱邻接中可把握：后续 → Partial-fraction expansion。
- Relation summary: 后续 → Partial-fraction expansion
- Extended explanation (do not paste verbatim on image): Partial-fraction expansion method for distinct poles
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Partial-fra…」、「一种·Partia…」
