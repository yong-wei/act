...
## Subject
- Title: Closed-loop system transfer function
- Concept kind: unknown
- One-line definition (source of truth): Closed-loop system transfer function：Closed-loop system transfer function
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → Steady-state error e ss
- Extended explanation (do not paste verbatim on image): Closed-loop system transfer function
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Closed-loop…」、「关联·Steady…」
