...
## Subject
- Title: coulomb plus viscous friction
- Concept kind: unknown
- One-line definition (source of truth): coulomb plus viscous friction：coulomb plus viscous friction
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → memoryless nonlinearity、preloaded spring
- Extended explanation (do not paste verbatim on image): coulomb plus viscous friction
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「coulomb plu…」、「一种·memory…」、「关联·preloa…」
