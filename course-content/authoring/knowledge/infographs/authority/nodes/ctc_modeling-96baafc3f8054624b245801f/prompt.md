...
## Subject
- Title: 纯延迟时间 (modeling_96baafc3f8054624b245801f)
- Concept kind: theoretical_construct
- One-line definition (source of truth): 调节阀开度变化所引起的流入量变化需要经过一段传输时间才能对液位产生影响，该时间称为纯延迟时间。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 调节阀开度变化所引起的流入量变化需要经过一段传输时间才能对液位产生影响，该时间称为纯延迟时间。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「纯延迟时间」
