...
## Subject
- Title: Dominant Second-Order Poles Method
- Concept kind: unknown
- One-line definition (source of truth): Dominant Second-Order Poles Method：A pole-placement design approach that selects closed-loop poles for a higher-order system as a desired pair of dominant…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A pole-placement design approach that selects closed-loop poles for a higher-order system as a desi…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Dominant Se…」
