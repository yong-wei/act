...
## Subject
- Title: Zerodegree root locus
- Concept kind: unknown
- One-line definition (source of truth): Zerodegree root locus：A root locus where the number of zeros equals the number of poles, resulting in no asymptotes.
- Core intuition: 在图谱邻接中可把握：前置 → Nonminimum-Phase Compensator。
- Relation summary: 前置 → Nonminimum-Phase Compensator
- Extended explanation (do not paste verbatim on image): A root locus where the number of zeros equals the number of poles, resulting in no asymptotes.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Zerodegree …」、「Zerodegree r」、「关联·Nonmin…」
