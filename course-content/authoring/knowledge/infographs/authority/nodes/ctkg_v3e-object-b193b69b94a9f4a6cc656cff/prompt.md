...
## Subject
- Title: Zero steady-state error
- Concept kind: unknown
- One-line definition (source of truth): Zero steady-state error：The condition where the steady-state tracking error is zero, achieved by including an internal model of the reference i…
- Core intuition: 在图谱邻接中可把握：前置 → Internal model design、Asymptotic tracking。
- Relation summary: 前置 → Internal model design、Asymptotic tracking
- Extended explanation (do not paste verbatim on image): The condition where the steady-state tracking error is zero, achieved by including an internal mode…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Zero steady…」、「Zero steady-」、「关联·Intern…」、「关联·Asympt…」
