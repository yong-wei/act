...
## Subject
- Title: 开环极点 (open_loop_poles)
- Concept kind: mathematical_object
- One-line definition (source of truth): 开环传递函数的极点，在根轨迹法中为已知量。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → root of the denominator
- Extended explanation (do not paste verbatim on image): 开环传递函数的极点，在根轨迹法中为已知量。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「开环极点」、「open loop po」、「一种·root」
