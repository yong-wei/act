...
## Subject
- Title: Routh approximat (Routh approximation method for order reduction)
- Concept kind: analysis_method
- One-line definition (source of truth): Routh approximat：Another novel and useful method for reducing the order is the Routh approximation method based on the idea of truncatin…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): Another novel and useful method for reducing the order is the Routh approximation method based on t…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Routh appro…」
