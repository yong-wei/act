...
## Subject
- Title: percent overshoot (percent_overshoot)
- Concept kind: performance_metric
- One-line definition (source of truth): percent overshoot：The percentage by which the output response exceeds the final value in response to a step input.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The percentage by which the output response exceeds the final value in response to a step input.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「percent ove…」、「percent over」
