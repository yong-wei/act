...
## Subject
- Title: Transformation t (Transformation to Observer Canonical Form)
- Concept kind: unknown
- One-line definition (source of truth): Transformation t：Find a transformation to take a given system to observer canonical form if and only if the system is observable (observ…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): Find a transformation to take a given system to observer canonical form if and only if the system i…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Transformat…」
