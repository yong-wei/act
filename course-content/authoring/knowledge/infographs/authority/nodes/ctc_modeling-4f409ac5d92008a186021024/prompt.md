...
## Subject
- Title: 控制系统中广泛应用的一种小功率… (modeling_4f409ac5d92008a186021024)
- Concept kind: theoretical_construct
- One-line definition (source of truth): 具有重量轻、惯性小、加速特性好的优点，是控制系统中广泛应用的一种小功率交流执行机构，由互相垂直配置的两相定子线圈和一个高电阻值的转子组成。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 具有重量轻、惯性小、加速特性好的优点，是控制系统中广泛应用的一种小功率交流执行机构，由互相垂直配置的两相定子线圈和一个高电阻值的转子组成。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「控制系统中广泛应用的一…」
