...
- One-line definition (source of truth): ζ > 1，特征方程有两个不相等的负实根，单位阶跃响应非周期地趋于稳态输出，但响应速度比临界阻尼情况缓慢。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): ζ > 1，特征方程有两个不相等的负实根，单位阶跃响应非周期地趋于稳态输出，但响应速度比临界阻尼情况缓慢。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「过阻尼」
