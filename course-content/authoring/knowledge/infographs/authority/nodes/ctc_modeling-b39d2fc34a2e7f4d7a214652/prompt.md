...
## Subject
- Title: 测速发电机输出斜率Kt (modeling_b39d2fc34a2e7f4d7a214652)
- Concept kind: theoretical_construct
- One-line definition (source of truth): 测速发电机输出斜率Kt，表示单位角速度的输出电压。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 测速发电机输出斜率Kt，表示单位角速度的输出电压。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「测速发电机输出斜率Kt」
