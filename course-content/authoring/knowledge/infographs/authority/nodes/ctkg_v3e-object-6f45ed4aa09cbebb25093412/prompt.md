...
## Subject
- Title: Using roots to c (Using roots to compute zeros from numerator polynomial)
- Concept kind: unknown
- One-line definition (source of truth): Using roots to c：Using roots to compute zeros from numerator polynomial
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): Using roots to compute zeros from numerator polynomial
