...
- One-line definition (source of truth): 由对数幅频曲线和对数相频曲线组成，横坐标按lgω分度，纵坐标分别为20lg|G(jω)|（dB）和φ(ω)（度）。
- Core intuition: 在图谱邻接中可把握：后续 → 对数频率特性曲线绘制方法。
- Relation summary: 后续 → 对数频率特性曲线绘制方法
- Extended explanation (do not paste verbatim on image): 由对数幅频曲线和对数相频曲线组成，横坐标按lgω分度，纵坐标分别为20lg|G(jω)|（dB）和φ(ω)（度）。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「对数频率特性曲线」、「关联·对数频率特性…」
