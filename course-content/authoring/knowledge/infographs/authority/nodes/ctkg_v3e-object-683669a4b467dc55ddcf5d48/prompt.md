...
- One-line definition (source of truth): 直接在z域内应用的稳定性判据，根据离散系统的闭环特征方程D(z)=0的系数，判别其根是否位于z平面上的单位圆内。
- Core intuition: 在图谱邻接中可把握：前置 → 朱利稳定判据、Unit circle in z-plane · 后续 → 朱利阵列。
- Relation summary: 前置 → 朱利稳定判据、Unit circle in z-plane · 后续 → 朱利阵列
- Extended explanation (do not paste verbatim on image): 直接在z域内应用的稳定性判据，根据离散系统的闭环特征方程D(z)=0的系数，判别其根是否位于z平面上的单位圆内。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「朱利稳定判据」、「包含组件·朱利阵列」、「用于分析·朱利稳定…」、「用于分析·Unit…」
