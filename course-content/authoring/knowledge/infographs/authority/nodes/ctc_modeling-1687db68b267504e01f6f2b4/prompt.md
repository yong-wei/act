...
## Subject
- Title: 先由系统原理图画出系统方框图 (modeling_1687db68b267504e01f6f2b4)
- Concept kind: analysis_method
- One-line definition (source of truth): 先由系统原理图画出系统方框图，分别列写各元件的微分方程，然后消去中间变量。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 先由系统原理图画出系统方框图，分别列写各元件的微分方程，然后消去中间变量。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「先由系统原理图画出系统…」
