...
## Subject
- Title: The impulse response is (modeling_d1d778fef91f78f0d3490f87)
- Concept kind: theoretical_construct
- One-line definition (source of truth): The impulse response is：The impulse response h(t) is defined as the limit of the response to a short pulse p_Δ(t) as Δ approaches 0.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The impulse response h(t) is defined as the limit of the response to a short pulse p_Δ(t) as Δ appr…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「The impulse…」
