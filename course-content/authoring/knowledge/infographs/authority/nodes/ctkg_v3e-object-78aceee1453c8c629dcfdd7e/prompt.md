...
- One-line definition (source of truth): 低速爬行现象。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → 摩擦特性
- Extended explanation (do not paste verbatim on image): 低速爬行现象
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「低速爬行现象」、「关联·摩擦特性」
