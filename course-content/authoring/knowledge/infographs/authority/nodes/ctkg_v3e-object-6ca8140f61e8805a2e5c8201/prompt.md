...
## Subject
- Title: Matched Pole-Zero (MPZ) Method
- Concept kind: unknown
- One-line definition (source of truth): Matched Pole-Zero (MPZ) Method：A digitization method that applies the relation z=e^{sT} to the poles and zeros of a transfer function. Steps: 1) Map p…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A digitization method that applies the relation z=e^{sT} to the poles and zeros of a transfer funct…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Matched Pol…」
