...
- One-line definition (source of truth): Open-loop system poles：Poles of the open-loop system.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → Open-loop system poles location
- Extended explanation (do not paste verbatim on image): Poles of the open-loop system.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Open-loop s…」、「Open-loop sy」、「有表示·Open-…」
