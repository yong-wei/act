...
## Subject
- Title: 用一对共轭复数主导极点对应的二…
- Concept kind: analysis_method
- One-line definition (source of truth): 工程上常采用闭环主导极点的概念对高阶系统进行近似分析，即用一对共轭复数主导极点对应的二阶系统动态性能指标来估算高阶系统的动态性能。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 工程上常采用闭环主导极点的概念对高阶系统进行近似分析，即用一对共轭复数主导极点对应的二阶系统动态性能指标来估算高阶系统的动态性能。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「用一对共轭复数主导极点…」
