...
- One-line definition (source of truth): External Stability：Stability analysis based on input-output behavior, as in frequency-response methods.
- Core intuition: 在图谱邻接中可把握：前置 → Circle Criterion。
- Relation summary: 前置 → Circle Criterion
- Extended explanation (do not paste verbatim on image): Stability analysis based on input-output behavior, as in frequency-response methods.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「External St…」、「External Sta」、「用于分析·Circ…」
