...
- One-line definition (source of truth): 采样器是离散系统中的一种元件，它对连续信号进行采样，将连续时间信号转换为离散时间信号。
- Core intuition: 采样器不影响开环脉冲传递函数的极点，但影响零点，从而影响闭环离散系统的动态性能。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 采样器是离散系统中的一种元件，它对连续信号进行采样，将连续时间信号转换为离散时间信号。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「采样器」
