...
## Subject
- Title: feedback connection (feedback_connection)
- Concept kind: theoretical_construct
- One-line definition (source of truth): feedback connection：A block diagram configuration where two blocks are connected in a feedback arrangement so that each feeds into the othe…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A block diagram configuration where two blocks are connected in a feedback arrangement so that each…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「feedback co…」、「feedback con」
