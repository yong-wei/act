...
- One-line definition (source of truth): Sampled-data system：A system where part of the system acts on sampled data.
- Core intuition: 在图谱邻接中可把握：前置 → Digital-to-analog converter · 后续 → Sample period、Sample rate、Discrete signal。
- Relation summary: 前置 → Digital-to-analog converter · 后续 → Sample period、Sample rate、Discrete signal
- Extended explanation (do not paste verbatim on image): A system where part of the system acts on sampled data.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Sampled-dat…」、「Sampled-data」、「属于·Digita…」、「包含组件·Samp…」、「包含组件·Disc…」
