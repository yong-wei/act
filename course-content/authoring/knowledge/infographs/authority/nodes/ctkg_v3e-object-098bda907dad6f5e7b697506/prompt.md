...
- One-line definition (source of truth): Limit Cycle：Limit Cycle
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → Describing Function、Describing Function Method
- Extended explanation (do not paste verbatim on image): Limit Cycle
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Limit Cycle」、「用于分析·Desc…」
