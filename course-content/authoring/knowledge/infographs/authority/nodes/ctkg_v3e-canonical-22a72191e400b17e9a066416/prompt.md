...
## Subject
- Title: 用矩形脉动函数近似单位脉冲函数…
- Concept kind: analysis_method
- One-line definition (source of truth): 常用具有一定脉宽 b 和有限幅度的矩形脉动函数来代替理想单位脉冲函数，要求实际脉动函数的宽度 b 远小于系统的时间常数 T，一般规定 b<0.1T。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 常用具有一定脉宽 b 和有限幅度的矩形脉动函数来代替理想单位脉冲函数，要求实际脉动函数的宽度 b 远小于系统的时间常数 T，一般规定 b<0.1T。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「用矩形脉动函数近似单位…」
