...
- One-line definition (source of truth): 饱和特性。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 等效增益
- Extended explanation (do not paste verbatim on image): 饱和特性
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「饱和特性」、「关联·等效增益」
