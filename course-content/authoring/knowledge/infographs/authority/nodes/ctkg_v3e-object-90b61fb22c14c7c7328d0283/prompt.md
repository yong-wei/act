...
## Subject
- Title: Evaluation of st (Evaluation of state transition matrix via signal-flow graph)
- Concept kind: unknown
- One-line definition (source of truth): Evaluation of st：Set U(s)=0, use the signal-flow graph with initial conditions, and apply Mason's signal-flow gain formula to obtain the…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): Set U(s)=0, use the signal-flow graph with initial conditions, and apply Mason's signal-flow gain f…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Evaluation …」
