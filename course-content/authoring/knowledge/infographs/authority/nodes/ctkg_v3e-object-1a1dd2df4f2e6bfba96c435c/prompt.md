...
- One-line definition (source of truth): 开环相频特性为-180°时的频率，记为ω_x。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 阻尼自然频率 · 后续 → 幅值裕度
- Extended explanation (do not paste verbatim on image): 开环相频特性为-180°时的频率，记为ω_x
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「穿越频率」、「关联·阻尼自然频率」、「关联·幅值裕度」
