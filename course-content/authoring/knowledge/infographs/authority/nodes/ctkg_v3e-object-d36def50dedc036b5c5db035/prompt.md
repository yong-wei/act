...
- One-line definition (source of truth): Pole placement：A full-state feedback design technique that places closed-loop system poles at desired locations, requiring complete co…
- Core intuition: 在图谱邻接中可把握：后续 → Observability。
- Relation summary: 后续 → Observability
- Extended explanation (do not paste verbatim on image): A full-state feedback design technique that places closed-loop system poles at desired locations, r…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Pole placem…」、「Pole placeme」、「用于分析·Obse…」
