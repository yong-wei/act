...
- One-line definition (source of truth): asymptote：The path the root locus follows as the parameter becomes very large and approaches ∞.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The path the root locus follows as the parameter becomes very large and approaches ∞.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「asymptote」
