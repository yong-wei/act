...
- One-line definition (source of truth): Crossover frequency：The frequency at which the magnitude of the open-loop transfer function |KG(jω)| equals 1 (0 dB).
- Core intuition: 在图谱邻接中可把握：前置 → Determining PM from Bode plot。
- Relation summary: 前置 → Determining PM from Bode plot
- Extended explanation (do not paste verbatim on image): The frequency at which the magnitude of the open-loop transfer function |KG(jω)| equals 1 (0 dB).
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Crossover f…」、「Crossover fr」、「用于分析·Dete…」
