...
- One-line definition (source of truth): microcontroller：A unit that contains both a microprocessor and most other needed functions, including A/D and D/A conversion, used in m…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A unit that contains both a microprocessor and most other needed functions, including A/D and D/A c…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「microcontro…」、「microcontrol」
