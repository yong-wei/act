...
## Subject
- Title: 用于测量角速度并将它转换成电压… (modeling_42946f0d5ec8b73ea7158b0d)
- Concept kind: theoretical_construct
- One-line definition (source of truth): 用于测量角速度并将它转换成电压量的装置，在控制系统中常用的有直流和交流测速发电机。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 用于测量角速度并将它转换成电压量的装置，在控制系统中常用的有直流和交流测速发电机。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「用于测量角速度并将它转…」
