...
## Subject
- Title: Bandwidth frequency
- Concept kind: unknown
- One-line definition (source of truth): Bandwidth frequency：The frequency at which the frequency response has declined 3 dB from its low-frequency value.
- Core intuition: 在图谱邻接中可把握：前置 → Resonant frequency。
- Relation summary: 前置 → Resonant frequency
- Extended explanation (do not paste verbatim on image): The frequency at which the frequency response has declined 3 dB from its low-frequency value.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Bandwidth f…」、「Bandwidth fr」、「关联·Resona…」
