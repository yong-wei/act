...
- One-line definition (source of truth): 包含比例和积分控制作用的控制器。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 包含比例和积分控制作用的控制器。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「比例-积分控制器」
