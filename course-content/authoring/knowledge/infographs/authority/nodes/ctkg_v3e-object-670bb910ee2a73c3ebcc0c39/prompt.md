...
## Subject
- Title: Weak controllability
- Concept kind: unknown
- One-line definition (source of truth): Weak controllability：A situation where a system is mathematically controllable, but certain modes are so weakly affected by the control inpu…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A situation where a system is mathematically controllable, but certain modes are so weakly affected…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Weak contro…」、「Weak control」
