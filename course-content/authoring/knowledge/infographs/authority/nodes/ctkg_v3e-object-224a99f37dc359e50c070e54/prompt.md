...
- One-line definition (source of truth): Tracking error：The difference between the system output and the reference input, y(t) - r(t).
- Core intuition: 在图谱邻接中可把握：前置 → Method 2: Using 、Asymptotic tracking、Tracking-error estimator · 后续 → Reference input。
- Relation summary: 前置 → Method 2: Using 、Asymptotic tracking、Tracking-error estimator · 后续 → Reference input
- Extended explanation (do not paste verbatim on image): The difference between the system output and the reference input, y(t) - r(t).
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Tracking er…」、「Tracking err」、「包含组件·Meth…」、「应用于·Refer…」、「应用于·Metho…」、「应用于·Asymp…」、「应用于·Track…」
