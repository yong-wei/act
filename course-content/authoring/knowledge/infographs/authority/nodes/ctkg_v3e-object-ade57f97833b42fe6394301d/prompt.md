...
## Subject
- Title: 否成立 (PBH rank test method for controllability)
- Concept kind: unknown
- One-line definition (source of truth): 对矩阵A的所有特征值检验rank[λI-A B]=n是否成立。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → PBH秩判据
- Extended explanation (do not paste verbatim on image): 对矩阵A的所有特征值检验rank[λI-A B]=n是否成立。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「否成立」、「是一种·PBH秩判据」
