...
## Subject
- Title: filter compensator (filter_compensator)
- Concept kind: system_component
- One-line definition (source of truth): filter compensator：A filter with transfer function (αs+1)/(βs+1) used in a feedback control system, often called a compensator, with desig…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A filter with transfer function (αs+1)/(βs+1) used in a feedback control system, often called a com…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「filter comp…」、「filter compe」
