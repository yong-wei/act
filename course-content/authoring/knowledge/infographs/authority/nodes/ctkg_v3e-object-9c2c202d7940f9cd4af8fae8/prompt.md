...
- One-line definition (source of truth): 每隔 T 秒与电位器接触一次，T 称为采样周期。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 采样控制系统 · 后续 → 采样持续时间
- Extended explanation (do not paste verbatim on image): 每隔 T 秒与电位器接触一次，T 称为采样周期。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「采样周期」、「包含组件·采样控制…」、「关联·采样持续时间」
