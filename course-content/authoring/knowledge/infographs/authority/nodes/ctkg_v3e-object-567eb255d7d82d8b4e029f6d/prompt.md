...
- One-line definition (source of truth): 对于式（9－76）所示线性时变系统，若存在能将状态 x(t0)=0 转移到 x(tf)=xf 的控制作用，则称状态 xf 是 t0 时刻可达的。
- Core intuition: 若 xf 对所有时刻都是可达的，则称 xf 为完全可达或一致可达。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 对于式（9－76）所示线性时变系统，若存在能将状态 x(t0)=0 转移到 x(tf)=xf 的控制作用，则称状态 xf 是 t0 时刻可达的。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「状态可达」
