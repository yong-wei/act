...
## Subject
- Title: 领域概念 (modeling_8bf044c43e3936166dff5b1c)
- Concept kind: theoretical_construct
- One-line definition (source of truth): 领域概念：A feedback configuration where the feedback signal is subtracted from the input at the summing junction.。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A feedback configuration where the feedback signal is subtracted from the input at the summing junc…
