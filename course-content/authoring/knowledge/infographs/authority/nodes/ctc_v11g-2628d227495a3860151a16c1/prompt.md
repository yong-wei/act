...
## Subject
- Title: imaginary axis crossing (imaginary_axis_crossing)
- Concept kind: mathematical_object
- One-line definition (source of truth): imaginary axis crossing：The points at which the root locus crosses the imaginary axis.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The points at which the root locus crosses the imaginary axis.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「imaginary a…」、「imaginary ax」
