...
- One-line definition (source of truth): 开环零极点位于s右半平面的典型环节，包括比例环节(K<0)、不稳定惯性环节、不稳定一阶微分环节、不稳定振荡环节、不稳定二阶微分环节五种。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 开环零极点位于s右半平面的典型环节，包括比例环节(K<0)、不稳定惯性环节、不稳定一阶微分环节、不稳定振荡环节、不稳定二阶微分环节五种。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「非最小相位环节」
