...
## Subject
- Title: Bode plots (logarithmic plots)
- Concept kind: unknown
- One-line definition (source of truth): Bode plots (logarithmic plots)：Bode plots (logarithmic plots)
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → Bode diagram · 后续 → Bode diagram
- Extended explanation (do not paste verbatim on image): Bode plots (logarithmic plots)
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Bode plots …」、「关联·Bode d…」
