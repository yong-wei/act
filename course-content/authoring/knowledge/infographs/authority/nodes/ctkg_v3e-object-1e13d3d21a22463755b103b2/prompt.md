...
## Subject
- Title: Ackermann's Formula for Estimator
- Concept kind: unknown
- One-line definition (source of truth): Ackermann's Formula for Estimator：Compute L using L = α_e(A) O^{-1} [0 ... 0 1]^T, where O is the observability matrix.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): Compute L using L = α_e(A) O^{-1} [0 ... 0 1]^T, where O is the observability matrix.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Ackermann's…」
