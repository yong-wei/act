...
- One-line definition (source of truth): 当时间 t 趋于无穷时，误差响应 e(t) 的稳态值称为稳态误差，以 e_ss(∞) 标志。
- Core intuition: 在图谱邻接中可把握：前置 → 误差响应。
- Relation summary: 前置 → 误差响应
- Extended explanation (do not paste verbatim on image): 当时间 t 趋于无穷时，误差响应 e(t) 的稳态值称为稳态误差，以 e_ss(∞) 标志。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「稳态误差」、「关联·误差响应」
