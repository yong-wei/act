...
- One-line definition (source of truth): Observer poles：Poles of the observer, located furthest to the left for faster convergence.
- Core intuition: 在图谱邻接中可把握：前置 → Observer poles location。
- Relation summary: 前置 → Observer poles location
- Extended explanation (do not paste verbatim on image): Poles of the observer, located furthest to the left for faster convergence.
