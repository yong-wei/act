...
- One-line definition (source of truth): 理想脉冲。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 采样控制系统 · 后续 → 脉冲序列
- Extended explanation (do not paste verbatim on image): 理想脉冲
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「理想脉冲」、「是一种·脉冲序列」、「包含组件·采样控制…」
