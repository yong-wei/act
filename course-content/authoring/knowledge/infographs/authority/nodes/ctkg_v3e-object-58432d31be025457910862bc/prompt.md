...
- One-line definition (source of truth): 对于离散时间系统，可控性指从任意非零状态转移至零状态，可达性指从零状态转移至任意非零状态，两者在G非奇异时等价。
- Core intuition: 在图谱邻接中可把握：后续 → 可控性。
- Relation summary: 后续 → 可控性
- Extended explanation (do not paste verbatim on image): 对于离散时间系统，可控性指从任意非零状态转移至零状态，可达性指从零状态转移至任意非零状态，两者在G非奇异时等价。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「离散系统的可控性与可达性」、「包含组件·可控性」
