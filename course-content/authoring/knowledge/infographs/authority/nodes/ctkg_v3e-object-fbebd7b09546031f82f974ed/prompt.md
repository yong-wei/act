...
- One-line definition (source of truth): 前馈校正装置的一种形式，接在系统给定值之后及主反馈作用点之前的前向通道上，相当于对给定值信号进行整形或滤波。
- Core intuition: 在图谱邻接中可把握：前置 → 前馈校正 · 后续 → 前馈校正。
- Relation summary: 前置 → 前馈校正 · 后续 → 前馈校正
- Extended explanation (do not paste verbatim on image): 前馈校正装置的一种形式，接在系统给定值之后及主反馈作用点之前的前向通道上，相当于对给定值信号进行整形或滤波。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「前置滤波器」、「是一种·前馈校正」、「关联·前馈校正」
