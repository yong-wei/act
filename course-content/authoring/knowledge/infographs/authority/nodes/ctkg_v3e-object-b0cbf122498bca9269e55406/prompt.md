...
- One-line definition (source of truth): 输出量经恒定延时后不失真地复现输入量变化的环节。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 输出量经恒定延时后不失真地复现输入量变化的环节。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「延迟环节」
