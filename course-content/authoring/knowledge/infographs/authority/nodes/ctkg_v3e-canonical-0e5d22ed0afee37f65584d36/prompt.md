...
- One-line definition (source of truth): 正弦函数。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 典型输入信号
- Extended explanation (do not paste verbatim on image): 正弦函数
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「正弦函数」、「包含组件·典型输入…」
