...
- One-line definition (source of truth): 响应超过其终值到达第一个峰值所需的时间。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 动态性能指标
- Extended explanation (do not paste verbatim on image): 响应超过其终值到达第一个峰值所需的时间。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「峰值时间」、「包含组件·动态性能…」
