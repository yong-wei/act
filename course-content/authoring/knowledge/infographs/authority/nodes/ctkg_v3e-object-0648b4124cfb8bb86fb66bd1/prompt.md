...
- One-line definition (source of truth): Sample period：Sample period
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → Zero-order hold (ZOH)、Discrete equivalent、Sampled-data system · 后续 → Digital-to-analog (D/A) converter
- Extended explanation (do not paste verbatim on image): Sample period
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Sample peri…」、「Sample perio」、「包含组件·Zero…」、「包含组件·Disc…」、「包含组件·Samp…」、「应用于·Digit…」
