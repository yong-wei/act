...
## Subject
- Title: 离散系统结构图中的元件 (modeling_799ffe58feb1471183f23786)
- Concept kind: theoretical_construct
- One-line definition (source of truth): 离散系统结构图中的元件，用 z^{-1} 表示，其输入为 (k+1)T 时刻的状态，输出为延迟一个采样周期的 kT 时刻的状态。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 离散系统结构图中的元件，用 z^{-1} 表示，其输入为 (k+1)T 时刻的状态，输出为延迟一个采样周期的 kT 时刻的状态。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「离散系统结构图中的元件」
