...
- One-line definition (source of truth): Free running：Free running
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → Sample period
- Extended explanation (do not paste verbatim on image): Free running
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Free running」、「应用于·Sampl…」
