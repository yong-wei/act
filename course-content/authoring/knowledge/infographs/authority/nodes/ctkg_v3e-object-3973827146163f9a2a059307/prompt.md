...
## Subject
- Title: Desired closed-loop poles location
- Concept kind: unknown
- One-line definition (source of truth): Desired closed-loop poles location：Desired poles of the closed-loop system are positioned further left in the stable region, roughly at (-1, 0), (-8, 6),…
- Core intuition: 在图谱邻接中可把握：后续 → Desired closed-loop poles。
- Relation summary: 后续 → Desired closed-loop poles
- Extended explanation (do not paste verbatim on image): Desired poles of the closed-loop system are positioned further left in the stable region, roughly a…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Desired clo…」、「有表示·Desir…」
