...
- One-line definition (source of truth): 一种早期控制，可在出现位置误差前，提前产生修正作用，从而达到改善系统性能的目的。
- Core intuition: 系统输出量同时受误差信号及其速率的双重作用。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 一种早期控制，可在出现位置误差前，提前产生修正作用，从而达到改善系统性能的目的。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「比例-微分控制」
