...
- One-line definition (source of truth): 反馈控制系统稳定的充分必要条件是半闭合曲线Γ_GH不穿过(-1, j0)点且逆时针包围临界点(-1, j0)点的圈数R等于开环传递函数的正实部极点数P。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 反馈控制系统稳定的充分必要条件是半闭合曲线Γ_GH不穿过(-1, j0)点且逆时针包围临界点(-1, j0)点的圈数R等于开环传递函数的正实部极点数P。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「奈奎斯特稳定判据（奈氏…」
