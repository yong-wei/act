...
## Subject
- Title: Percent overshoot P.O.
- Concept kind: theoretical_construct
- One-line definition (source of truth): Percent overshoot P.O.：The amount that the response overshoots the final value, expressed as a percentage of the final value.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The amount that the response overshoots the final value, expressed as a percentage of the final val…
