...
- One-line definition (source of truth): Uncompensated system：The system with loop transfer function L(s)=K G(s) when the controller is a constant gain controller G_c(s)=K.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The system with loop transfer function L(s)=K G(s) when the controller is a constant gain controlle…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Uncompensat…」、「Uncompensate」
