...
- One-line definition (source of truth): Damped Natural Frequency：The frequency of oscillation of a second-order system's transient response, given by ω_d = ω_n sqrt(1-ζ^2).
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The frequency of oscillation of a second-order system's transient response, given by ω_d = ω_n sqrt…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Damped Natu…」、「Damped Natur」
