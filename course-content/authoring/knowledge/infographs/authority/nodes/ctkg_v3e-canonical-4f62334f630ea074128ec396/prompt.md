...
- One-line definition (source of truth): 单位阶跃函数。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 典型输入信号、单位阶跃响应
- Extended explanation (do not paste verbatim on image): 单位阶跃函数
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「单位阶跃函数」、「包含组件·典型输入…」、「派生自·单位阶跃响应」
