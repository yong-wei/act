...
## Subject
- Title: memoryless nonlinearity
- Concept kind: unknown
- One-line definition (source of truth): memoryless nonlinearity：A nonlinearity that has no dynamics and is well approximated as a gain that varies as the size of its input signal vari…
- Core intuition: 在图谱邻接中可把握：前置 → gain with dead zone、Saturation Nonlinearity、relay with dead zone。
- Relation summary: 前置 → gain with dead zone、Saturation Nonlinearity、relay with dead zone
- Extended explanation (do not paste verbatim on image): A nonlinearity that has no dynamics and is well approximated as a gain that varies as the size of i…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「memoryless …」、「memoryless n」、「一种·gain」、「一种·Satura…」、「一种·relay」、「是一种·relay」、「一种·coulom…」
