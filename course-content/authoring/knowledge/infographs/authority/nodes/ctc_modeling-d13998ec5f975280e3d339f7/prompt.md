...
## Subject
- Title: component that scales an (modeling_d13998ec5f975280e3d339f7)
- Concept kind: theoretical_construct
- One-line definition (source of truth): component that scales an：A component that scales an input signal by a gain k, where 0 ≤ k ≤ 1.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A component that scales an input signal by a gain k, where 0 ≤ k ≤ 1.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「component t…」
