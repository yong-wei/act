...
- One-line definition (source of truth): 当矩阵A的特征值两两相异时，通过线性变换化为对角线规范型，根据B或C矩阵的行或列是否全为零来判断可控性或可观测性。
- Core intuition: 在图谱邻接中可把握：前置 → 否包含元素全… · 后续 → 系统不可观测。
- Relation summary: 前置 → 否包含元素全… · 后续 → 系统不可观测
- Extended explanation (do not paste verbatim on image): 当矩阵A的特征值两两相异时，通过线性变换化为对角线规范型，根据B或C矩阵的行或列是否全为零来判断可控性或可观测性。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「对角线规范型判据」、「一种·否包含元素全…」、「应用于·系统不可观测」
