...
## Subject
- Title: diagram used to determine (modeling_257734be226a35d7124a77c8)
- Concept kind: analysis_method
- One-line definition (source of truth): diagram used to determine：A diagram used to determine the forces on a body by showing all forces acting on the body and the acceleration.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A diagram used to determine the forces on a body by showing all forces acting on the body and the a…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「diagram use…」
