...
- One-line definition (source of truth): Quantization error：Quantization error
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → Amplitude quantization error
- Extended explanation (do not paste verbatim on image): Quantization error
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Quantizatio…」、「Quantization」、「一种·Amplit…」
