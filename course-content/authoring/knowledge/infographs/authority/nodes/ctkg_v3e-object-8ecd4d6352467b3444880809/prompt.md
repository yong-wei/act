...
## Subject
- Title: Reduced-Order Estimator
- Concept kind: unknown
- One-line definition (source of truth): Reduced-Order Estimator：An estimator that reduces the order by the number of sensed outputs, estimating only the unmeasured state variables.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): An estimator that reduces the order by the number of sensed outputs, estimating only the unmeasured…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Reduced-Ord…」、「Reduced-Orde」
