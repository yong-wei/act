...
## Subject
- Title: 常规根轨迹 (conventional_gain_root_locus)
- Concept kind: representation_kind
- One-line definition (source of truth): 负反馈系统中根轨迹增益 K* 变化时的根轨迹称为常规根轨迹。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → 参数根轨迹、广义根轨迹
- Extended explanation (do not paste verbatim on image): 负反馈系统中根轨迹增益 K* 变化时的根轨迹称为常规根轨迹。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「常规根轨迹」、「关联·参数根轨迹」、「关联·广义根轨迹」
