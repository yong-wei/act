...
- One-line definition (source of truth): 当系统中含有一个或多个具有非线性特性的元件时，该系统称为非线性系统。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 小偏差线性化法、相平面法分析非线性系统、描述函数法 · 后续 → 自激振荡
- Extended explanation (do not paste verbatim on image): 当系统中含有一个或多个具有非线性特性的元件时，该系统称为非线性系统。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「非线性系统」、「用于分析·小偏差线…」、「用于分析·相平面法…」、「用于分析·描述函数法」、「用于分析·相平面法」、「用于分析·平衡点附…」
