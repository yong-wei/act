...
- One-line definition (source of truth): C1 contour：A closed contour in the s-plane that encloses the right half-plane, used in the Nyquist stability criterion.
- Core intuition: 在图谱邻接中可把握：前置 → Arc at infinity in Nyquist plot。
- Relation summary: 前置 → Arc at infinity in Nyquist plot
- Extended explanation (do not paste verbatim on image): A closed contour in the s-plane that encloses the right half-plane, used in the Nyquist stability c…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「C1 contour」、「应用于·Arc a…」
