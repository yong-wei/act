...
- One-line definition (source of truth): 在校正控制器中引入参考输入的内模，即参考输入信号的生成模型，以实现零稳态误差跟踪。
- Core intuition: 在图谱邻接中可把握：前置 → 内模控制器。
- Relation summary: 前置 → 内模控制器
- Extended explanation (do not paste verbatim on image): 在校正控制器中引入参考输入的内模，即参考输入信号的生成模型，以实现零稳态误差跟踪。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「内模」、「包含组件·内模控制器」
