...
- One-line definition (source of truth): Transmission zero：A value of s such that the system can have a nonzero input and state yet produce zero output.
- Core intuition: 在图谱邻接中可把握：后续 → Transmission zeros。
- Relation summary: 后续 → Transmission zeros
- Extended explanation (do not paste verbatim on image): A value of s such that the system can have a nonzero input and state yet produce zero output.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Transmissio…」、「Transmission」、「一种·Transm…」
