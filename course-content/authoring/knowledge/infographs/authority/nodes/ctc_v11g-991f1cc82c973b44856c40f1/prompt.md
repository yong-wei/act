...
## Subject
- Title: angle of arrival (angle_of_arrival)
- Concept kind: mathematical_object
- One-line definition (source of truth): angle of arrival：The angle at which a locus arrives at a complex zero in the s-plane.
- Core intuition: 在图谱邻接中可把握：后续 → 零点。
- Relation summary: 后续 → 零点
- Extended explanation (do not paste verbatim on image): The angle at which a locus arrives at a complex zero in the s-plane.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「angle of ar…」、「angle of arr」、「关联·零点」
