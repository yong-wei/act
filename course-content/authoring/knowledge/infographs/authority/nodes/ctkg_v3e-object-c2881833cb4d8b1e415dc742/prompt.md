...
- One-line definition (source of truth): 开环频率特性在相角为-180°时的幅值倒数，记为h(dB)。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 穿越频率
- Extended explanation (do not paste verbatim on image): 开环频率特性在相角为-180°时的幅值倒数，记为h(dB)
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「幅值裕度」、「关联·穿越频率」
