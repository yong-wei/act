...
## Subject
- Title: Desired closed-loop poles
- Concept kind: unknown
- One-line definition (source of truth): Desired closed-loop poles：Poles of the closed-loop system positioned further left in the stable region.
- Core intuition: 在图谱邻接中可把握：前置 → Desired closed-loop poles location。
- Relation summary: 前置 → Desired closed-loop poles location
- Extended explanation (do not paste verbatim on image): Poles of the closed-loop system positioned further left in the stable region.
