...
- One-line definition (source of truth): 非线性环节的负倒描述函数，记为 -1/N(A)，用于在复平面上与 G(jω) 曲线比较以分析系统稳定性。
- Core intuition: 在图谱邻接中可把握：后续 → 描述函数。
- Relation summary: 后续 → 描述函数
- Extended explanation (do not paste verbatim on image): 非线性环节的负倒描述函数，记为 -1/N(A)，用于在复平面上与 G(jω) 曲线比较以分析系统稳定性。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「负倒描述函数」、「派生自·描述函数」
