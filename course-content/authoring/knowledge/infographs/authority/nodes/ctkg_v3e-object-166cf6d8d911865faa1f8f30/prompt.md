...
## Subject
- Title: Digital-to-analog (D/A) converter
- Concept kind: unknown
- One-line definition (source of truth): Digital-to-analog (D/A) converter：Digital-to-analog (D/A) converter
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → Sample period · 后续 → Zero-order hold (ZOH)、Discrete equivalent、Discrete signal
- Extended explanation (do not paste verbatim on image): Digital-to-analog (D/A) converter
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Digital-to-…」、「包含组件·Zero…」、「应用于·Sampl…」、「关联·Discre…」
