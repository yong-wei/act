...
## Subject
- Title: 为避免状态方程中出现输入导数项 (modeling_591b33e87b9709fd8e130365)
- Concept kind: analysis_method
- One-line definition (source of truth): 为避免状态方程中出现输入导数项，按规则选择状态变量：x1 = y - h0 u, xi = dx_{i-1}/dt - h_{i-1} u (i=2,3,...,n)，其中h0,...,h_{n-1}为待定常数。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 为避免状态方程中出现输入导数项，按规则选择状态变量：x1 = y - h0 u, xi = dx_{i-1}/dt - h_{i-1} u (i=2,3,...,n)，其中h0,...,h_{n-1…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「为避免状态方程中出现输…」
