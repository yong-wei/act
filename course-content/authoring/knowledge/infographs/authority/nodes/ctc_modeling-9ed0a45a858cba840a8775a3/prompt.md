...
## Subject
- Title: 在控制系统中广泛用作执行机构 (modeling_9ed0a45a858cba840a8775a3)
- Concept kind: theoretical_construct
- One-line definition (source of truth): 在控制系统中广泛用作执行机构，用来对被控对象的机械运动实现快速控制。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 在控制系统中广泛用作执行机构，用来对被控对象的机械运动实现快速控制。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「在控制系统中广泛用作执…」
