...
- One-line definition (source of truth): Marginal stability：If the jω-axis roots of the characteristic equation are simple, the system is neither stable nor unstable
- Core intuition: 在图谱邻接中可把握：后续 → 稳定性。
- Relation summary: 后续 → 稳定性
- Extended explanation (do not paste verbatim on image): If the jω-axis roots of the characteristic equation are simple, the system is neither stable nor un…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Marginal st…」、「Marginal sta」、「是一种·稳定性」
