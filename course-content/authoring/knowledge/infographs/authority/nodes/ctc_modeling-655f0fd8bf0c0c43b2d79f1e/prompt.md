...
## Subject
- Title: device composed of electric (modeling_655f0fd8bf0c0c43b2d79f1e)
- Concept kind: theoretical_construct
- One-line definition (source of truth): device composed of electric：A device composed of electric components designed to simulate ordinary differential equations (ODEs).
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A device composed of electric components designed to simulate ordinary differential equations (ODEs…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「device comp…」
