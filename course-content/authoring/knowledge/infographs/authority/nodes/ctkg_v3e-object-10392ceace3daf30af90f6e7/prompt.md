...
- One-line definition (source of truth): 若已知差分方程，并且给定输出序列的初值，则可以利用递推关系，在计算机上一步一步地算出输出序列。
- Core intuition: 在图谱邻接中可把握：后续 → 线性定常离散系统。
- Relation summary: 后续 → 线性定常离散系统
- Extended explanation (do not paste verbatim on image): 若已知差分方程，并且给定输出序列的初值，则可以利用递推关系，在计算机上一步一步地算出输出序列。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「迭代法」、「用于分析·线性定常…」
