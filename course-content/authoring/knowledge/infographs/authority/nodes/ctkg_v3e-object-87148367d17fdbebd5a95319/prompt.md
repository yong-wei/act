...
- One-line definition (source of truth): 如果采样器位于系统闭合回路之外，或者系统本身不存在闭合回路，则称为开环采样系统。
- Core intuition: 在图谱邻接中可把握：前置 → 采样控制系统 · 后续 → 采样控制系统。
- Relation summary: 前置 → 采样控制系统 · 后续 → 采样控制系统
- Extended explanation (do not paste verbatim on image): 如果采样器位于系统闭合回路之外，或者系统本身不存在闭合回路，则称为开环采样系统。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「开环采样系统」、「是一种·采样控制系统」、「关联·采样控制系统」
