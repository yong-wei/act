...
## Subject
- Title: Evaluation of st (Evaluation of state transition matrix via matrix inversion)
- Concept kind: unknown
- One-line definition (source of truth): Evaluation of st：Determine Phi(s) by evaluating Phi(s) = [sI - A]^{-1} through matrix inversion, then obtain Phi(t) via inverse Laplace…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): Determine Phi(s) by evaluating Phi(s) = [sI - A]^{-1} through matrix inversion, then obtain Phi(t)…
