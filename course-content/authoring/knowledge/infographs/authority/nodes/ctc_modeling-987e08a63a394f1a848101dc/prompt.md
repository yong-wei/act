...
## Subject
- Title: 只有输入支路而没有输出支路的节点 (modeling_987e08a63a394f1a848101dc)
- Concept kind: theoretical_construct
- One-line definition (source of truth): 只有输入支路而没有输出支路的节点，一般代表系统的输出变量。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → signal flow graph
- Extended explanation (do not paste verbatim on image): 只有输入支路而没有输出支路的节点，一般代表系统的输出变量。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「只有输入支路而没有输出…」、「包含组件·sign…」
