...
- One-line definition (source of truth): 死区。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → 间隙特性、死区特性
- Extended explanation (do not paste verbatim on image): 死区
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「死区」、「关联·间隙特性」、「关联·死区特性」
