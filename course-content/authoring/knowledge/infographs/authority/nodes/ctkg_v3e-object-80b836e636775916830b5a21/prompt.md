...
- One-line definition (source of truth): 相变量从初始时刻 t₀ 对应的状态点 (x₀, ẋ₀) 起，随着时间 t 的推移，在相平面上运动形成的曲线称为相轨迹。
- Core intuition: 在图谱邻接中可把握：前置 → 奇线、相平面图、MATLAB绘制相轨迹 · 后续 → 相平面图、相平面、相变量。
- Relation summary: 前置 → 奇线、相平面图、MATLAB绘制相轨迹 · 后续 → 相平面图、相平面、相变量
- Extended explanation (do not paste verbatim on image): 相变量从初始时刻 t₀ 对应的状态点 (x₀, ẋ₀) 起，随着时间 t 的推移，在相平面上运动形成的曲线称为相轨迹。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「相轨迹」、「是一种·奇线」、「属于·相平面图」、「属于·相平面」、「包含组件·相平面图」、「用于分析·MATL…」
