...
- One-line definition (source of truth): Tracking Effectiveness：An absolute lower bound on the sample rate is set by a specification to track a command input with a certain frequency…
- Core intuition: 在图谱邻接中可把握：后续 → Sample-Rate Selection。
- Relation summary: 后续 → Sample-Rate Selection
- Extended explanation (do not paste verbatim on image): An absolute lower bound on the sample rate is set by a specification to track a command input with…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Tracking Ef…」、「Tracking Eff」、「属于·Sample…」
