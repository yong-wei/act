...
- One-line definition (source of truth): Neutral Stability：A condition where a closed-loop root falls on the imaginary axis, i.e., s = jω, and the system is on the boundary betwe…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A condition where a closed-loop root falls on the imaginary axis, i.e., s = jω, and the system is o…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Neutral Sta…」、「Neutral Stab」
