...
- One-line definition (source of truth): 当输入信号 R(s) 与主反馈信号 B(s) 不等时，比较装置的输出 E(s)=R(s)-H(s)C(s) 称为误差信号，简称误差（亦称偏差）。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 当输入信号 R(s) 与主反馈信号 B(s) 不等时，比较装置的输出 E(s)=R(s)-H(s)C(s) 称为误差信号，简称误差（亦称偏差）。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「误差信号」
