...
- One-line definition (source of truth): Observability：A system is completely observable if and only if there exists a finite time T such that the initial state x(0) can be d…
- Core intuition: 在图谱邻接中可把握：前置 → Pole placement、Kalman state-space decomposition · 后续 → Observer Canonical Form。
- Relation summary: 前置 → Pole placement、Kalman state-space decomposition · 后续 → Observer Canonical Form
- Extended explanation (do not paste verbatim on image): A system is completely observable if and only if there exists a finite time T such that the initial…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Observabili…」、「Observabilit」、「用于分析·Pole…」、「用于分析·Kalm…」、「关联·Observ…」
