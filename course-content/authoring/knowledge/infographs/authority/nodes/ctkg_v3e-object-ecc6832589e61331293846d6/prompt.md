...
- One-line definition (source of truth): 理想采样器好像是一个载波为 δ_T(t) 的幅值调制器。
- Core intuition: 采样过程可以看成是一个幅值调制过程。
- Relation summary: 后续 → 采样器
- Extended explanation (do not paste verbatim on image): 理想采样器好像是一个载波为 δ_T(t) 的幅值调制器。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「理想采样器」、「是一种·采样器」
