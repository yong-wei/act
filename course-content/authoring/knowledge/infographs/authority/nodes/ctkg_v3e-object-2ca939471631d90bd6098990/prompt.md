...
- One-line definition (source of truth): 在脉冲序列和连续信号之间要用保持器，实现两种信号的转换。
- Core intuition: 实现复现过程的装置称为保持器。
- Relation summary: 前置 → 零阶保持器、一阶保持器、信号复现滤波器 · 后续 → 采样控制系统、阶梯信号
- Extended explanation (do not paste verbatim on image): 在脉冲序列和连续信号之间要用保持器，实现两种信号的转换。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「保持器」、「是一种·零阶保持器」、「是一种·一阶保持器」、「一种·信号复现滤波器」、「属于·采样控制系统」、「包含组件·采样控制…」
