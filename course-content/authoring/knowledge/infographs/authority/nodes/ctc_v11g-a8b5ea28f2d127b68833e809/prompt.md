...
## Subject
- Title: transponder tags (transponder_tags)
- Concept kind: system_component
- One-line definition (source of truth): transponder tags：Tags placed on the floor to task guided vehicles (e.g., to speed up or slow down) at key locations.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): Tags placed on the floor to task guided vehicles (e.g., to speed up or slow down) at key locations.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「transponder…」、「transponder 」
