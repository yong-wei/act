...
- One-line definition (source of truth): 若在某点处 f(x, ẋ) 和 ẋ 同时为零，即有 dẋ/dx = 0/0 的不定形式，则称该点为相平面的奇点。
- Core intuition: 在图谱邻接中可把握：前置 → 稳定焦点、不稳定节点、稳定节点 · 后续 → 相平面。
- Relation summary: 前置 → 稳定焦点、不稳定节点、稳定节点 · 后续 → 相平面
- Extended explanation (do not paste verbatim on image): 若在某点处 f(x, ẋ) 和 ẋ 同时为零，即有 dẋ/dx = 0/0 的不定形式，则称该点为相平面的奇点。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「奇点」、「是一种·稳定焦点」、「是一种·不稳定节点」、「是一种·稳定节点」、「是一种·不稳定焦点」、「是一种·实奇点」
