...
- One-line definition (source of truth): 复系数c_k的幅值集合，用于表示周期信号的频谱。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → 周期信号的频谱
- Extended explanation (do not paste verbatim on image): 复系数c_k的幅值集合，用于表示周期信号的频谱
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「幅值谱」、「关联·周期信号的频谱」
