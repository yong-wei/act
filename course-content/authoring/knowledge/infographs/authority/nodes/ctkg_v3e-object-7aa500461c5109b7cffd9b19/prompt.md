...
- One-line definition (source of truth): Routh array：An array used in the Routh-Hurwitz stability criterion, constructed from the coefficients of the characteristic equatio…
- Core intuition: 在图谱邻接中可把握：前置 → Auxiliary polynomial、First column of Routh array、Routh-Hurwitz stability criterion · 后续 → Routh-Hurwitz stability criterion、特征方程。
- Relation summary: 前置 → Auxiliary polynomial、First column of Routh array、Routh-Hurwitz stability criterion · 后续 → Routh-Hurwitz stability criterion、特征方程
- Extended explanation (do not paste verbatim on image): An array used in the Routh-Hurwitz stability criterion, constructed from the coefficients of the ch…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Routh array」、「属于·Auxili…」、「属于·First …」、「属于·Routh-…」、「包含组件·Rout…」、「派生自·特征方程」
