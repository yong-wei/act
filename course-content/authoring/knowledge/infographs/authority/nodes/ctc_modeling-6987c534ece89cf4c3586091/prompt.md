...
## Subject
- Title: 每一个输入量影响所有输出量 (modeling_6987c534ece89cf4c3586091)
- Concept kind: theoretical_construct
- One-line definition (source of truth): 每一个输入量影响所有输出量，每一个输出量也受所有输入量影响的系统。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 每一个输入量影响所有输出量，每一个输出量也受所有输入量影响的系统。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「每一个输入量影响所有输…」
