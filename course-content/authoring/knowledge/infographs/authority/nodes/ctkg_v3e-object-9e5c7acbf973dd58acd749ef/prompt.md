...
- One-line definition (source of truth): 分隔线。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 极限环、相平面图
- Extended explanation (do not paste verbatim on image): 分隔线
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「分隔线」、「是一种·极限环」、「包含组件·相平面图」
