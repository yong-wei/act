...
## Subject
- Title: state variable (state_variable)
- Concept kind: theoretical_construct
- One-line definition (source of truth): state variable：v3T accepted candidate for state variable.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): v3T accepted candidate for state variable.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「state varia…」、「state variab」
