...
- One-line definition (source of truth): 当 t → ∞ 时，起始于极限环内部或外部的相轨迹均卷向极限环，则该极限环称为稳定的极限环。
- Core intuition: 极限环内部的相轨迹发散至极限环，说明极限环的内部是不稳定区域。
- Relation summary: 前置 → 双极限环 · 后续 → 极限环、自振荡
- Extended explanation (do not paste verbatim on image): 当 t → ∞ 时，起始于极限环内部或外部的相轨迹均卷向极限环，则该极限环称为稳定的极限环。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「稳定极限环」、「是一种·极限环」、「包含组件·双极限环」、「关联·自振荡」
