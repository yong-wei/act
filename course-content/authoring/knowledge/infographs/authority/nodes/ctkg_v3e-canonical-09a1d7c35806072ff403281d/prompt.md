...
- One-line definition (source of truth): 串级控制系统中，由主调节器、副回路等效环节、被控对象和测量变送器构成的外部回路。
- Core intuition: 在图谱邻接中可把握：前置 → 主调节器 · 后续 → 主调节器。
- Relation summary: 前置 → 主调节器 · 后续 → 主调节器
- Extended explanation (do not paste verbatim on image): 串级控制系统中，由主调节器、副回路等效环节、被控对象和测量变送器构成的外部回路。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「主回路」、「属于·主调节器」、「包含组件·主调节器」
