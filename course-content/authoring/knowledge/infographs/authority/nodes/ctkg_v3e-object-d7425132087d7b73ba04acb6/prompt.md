...
- One-line definition (source of truth): flash memory：flash memory
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → read-only memory (ROM)
- Extended explanation (do not paste verbatim on image): flash memory
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「flash memory」、「一种·read-o…」
