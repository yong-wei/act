...
- One-line definition (source of truth): 误差采样控制。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 采样控制系统 · 后续 → 采样控制系统
- Extended explanation (do not paste verbatim on image): 误差采样控制
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「误差采样控制」、「是一种·采样控制系统」、「包含组件·采样控制…」
