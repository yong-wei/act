...
## Subject
- Title: Open-Loop Estimator
- Concept kind: unknown
- One-line definition (source of truth): Open-Loop Estimator：An estimator that uses a model of the plant without feedback correction, relying on correct initial conditions.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): An estimator that uses a model of the plant without feedback correction, relying on correct initial…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Open-Loop E…」、「Open-Loop Es」
