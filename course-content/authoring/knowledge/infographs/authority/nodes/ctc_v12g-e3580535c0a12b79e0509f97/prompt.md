...
## Subject
- Title: negative gain root locus (negative_gain_root_locus)
- Concept kind: theoretical_construct
- One-line definition (source of truth): negative gain root locus：A root locus plotted for negative values of the gain parameter K, also referred to as a 0° root locus.
- Core intuition: 在图谱邻接中可把握：前置 → nonminimum phase system、正反馈内回路 · 后续 → 增益。
- Relation summary: 前置 → nonminimum phase system、正反馈内回路 · 后续 → 增益
- Extended explanation (do not paste verbatim on image): A root locus plotted for negative values of the gain parameter K, also referred to as a 0° root loc…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「negative ga…」、「negative gai」、「关联·增益」、「关联·nonmin…」、「关联·正反馈内回路」
