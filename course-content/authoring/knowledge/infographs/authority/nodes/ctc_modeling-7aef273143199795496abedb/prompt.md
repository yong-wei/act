...
## Subject
- Title: 移动引出点或比较点 (modeling_7aef273143199795496abedb)
- Concept kind: analysis_method
- One-line definition (source of truth): 移动引出点或比较点，交换比较点，进行方框运算将串联、并联和反馈连接的方框合并。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 移动引出点或比较点，交换比较点，进行方框运算将串联、并联和反馈连接的方框合并。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「移动引出点或比较点」
