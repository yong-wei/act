...
## Subject
- Title: Knowledge of the initial (modeling_d978f6c1e3903da5373e65e3)
- Concept kind: theoretical_construct
- One-line definition (source of truth): Knowledge of the initial：Knowledge of the initial values of the state variables x(t0) = (x1(t0), x2(t0), ..., xn(t0)) at the initial time t0, an…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): Knowledge of the initial values of the state variables x(t0) = (x1(t0), x2(t0), ..., xn(t0)) at the…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Knowledge o…」
