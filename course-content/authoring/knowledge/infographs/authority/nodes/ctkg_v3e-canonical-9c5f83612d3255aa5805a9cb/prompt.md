...
## Subject
- Title: Natural frequency ω n (Natural frequency ω_n)
- Concept kind: theoretical_construct
- One-line definition (source of truth): Natural frequency ω n：The frequency of oscillation of the system without damping.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The frequency of oscillation of the system without damping.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Natural fre…」、「Natural freq」
