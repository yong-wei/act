...
- One-line definition (source of truth): DSP chips：Computers originally designed for digital signal-processing applications, giving the control designer full freedom to i…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): Computers originally designed for digital signal-processing applications, giving the control design…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「DSP chips」
