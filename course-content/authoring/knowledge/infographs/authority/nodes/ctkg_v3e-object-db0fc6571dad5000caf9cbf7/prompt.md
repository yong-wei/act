...
## Subject
- Title: Strophoid plane curve
- Concept kind: unknown
- One-line definition (source of truth): Strophoid plane curve：The shape of the Nyquist plot for G(s)=1/(s(s+1)^2) is a translated strophoid plane curve, meaning 'a belt with a twist…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The shape of the Nyquist plot for G(s)=1/(s(s+1)^2) is a translated strophoid plane curve, meaning…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Strophoid p…」、「Strophoid pl」
