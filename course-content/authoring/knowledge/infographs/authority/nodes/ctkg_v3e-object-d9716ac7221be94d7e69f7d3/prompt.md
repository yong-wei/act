...
## Subject
- Title: Kalman state-space decomposition
- Concept kind: unknown
- One-line definition (source of truth): Kalman state-space decomposition：A mechanism for partitioning the state-space to expose controllable/uncontrollable or observable/unobservable subspaces.
- Core intuition: 在图谱邻接中可把握：后续 → Observability。
- Relation summary: 后续 → Observability
- Extended explanation (do not paste verbatim on image): A mechanism for partitioning the state-space to expose controllable/uncontrollable or observable/un…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Kalman stat…」、「用于分析·Obse…」
