...
- One-line definition (source of truth): 采用一阶多项式外推的保持器，m=1。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 零阶保持器 · 后续 → 保持器
- Extended explanation (do not paste verbatim on image): 采用一阶多项式外推的保持器，m=1。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「一阶保持器」、「是一种·保持器」、「关联·零阶保持器」
