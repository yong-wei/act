...
## Subject
- Title: Determining GM from Bode plot
- Concept kind: unknown
- One-line definition (source of truth): Determining GM from Bode plot：Measure vertical distance between |KG(jω)| curve and magnitude=1 line at frequency where ∠G(jω)=-180°.
- Core intuition: 在图谱邻接中可把握：前置 → Gain Margin (GM)、Determining GM from root locus。
- Relation summary: 前置 → Gain Margin (GM)、Determining GM from root locus
- Extended explanation (do not paste verbatim on image): Measure vertical distance between |KG(jω)| curve and magnitude=1 line at frequency where ∠G(jω)=-18…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Determining…」、「派生自·Gain …」、「关联·Determ…」、「关联·Gain M…」
