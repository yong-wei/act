...
## Subject
- Title: Symmetrical root pairs
- Concept kind: unknown
- One-line definition (source of truth): Symmetrical root pairs：Symmetrical root pairs
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → Auxiliary polynomial
- Extended explanation (do not paste verbatim on image): Symmetrical root pairs
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Symmetrical…」、「Symmetrical 」、「包含组件·Auxi…」、「关联·Auxili…」
