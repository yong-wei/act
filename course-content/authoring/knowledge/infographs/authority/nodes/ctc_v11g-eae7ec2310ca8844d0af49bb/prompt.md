...
## Subject
- Title: 传输延迟 (transport_delay)
- Concept kind: performance_metric
- One-line definition (source of truth): 传输延迟：The time delay parameter characterizing the reaction curve in the open-loop Ziegler-Nichols tuning method.。
- Core intuition: 在图谱邻接中可把握：前置 → 反应曲线、反应曲线参数。
- Relation summary: 前置 → 反应曲线、反应曲线参数
- Extended explanation (do not paste verbatim on image): The time delay parameter characterizing the reaction curve in the open-loop Ziegler-Nichols tuning…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「传输延迟」、「transport de」、「关联·反应曲线」、「关联·反应曲线参数」
