...
- One-line definition (source of truth): Ideal sampler：Ideal sampler
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → Sampler
- Extended explanation (do not paste verbatim on image): Ideal sampler
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Ideal sampl…」、「Ideal sample」、「一种·Sampler」
