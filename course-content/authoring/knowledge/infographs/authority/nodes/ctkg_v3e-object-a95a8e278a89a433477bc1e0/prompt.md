...
- One-line definition (source of truth): 纵坐标为L(ω)单位为分贝，横坐标为φ(ω)单位为度，均为线性分度，频率ω为参变量。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 纵坐标为L(ω)单位为分贝，横坐标为φ(ω)单位为度，均为线性分度，频率ω为参变量。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「纵坐标为L(ω)单位为…」
