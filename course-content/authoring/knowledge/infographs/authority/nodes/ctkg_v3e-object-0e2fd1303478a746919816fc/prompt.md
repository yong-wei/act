...
- One-line definition (source of truth): 直接根据z变换的定义，将式（7－23）写成展开形式，从而得到z变换的级数展开式。
- Core intuition: 在图谱邻接中可把握：前置 → z变换。
- Relation summary: 前置 → z变换
- Extended explanation (do not paste verbatim on image): 直接根据z变换的定义，将式（7－23）写成展开形式，从而得到z变换的级数展开式。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「级数求和法」、「包含组件·z变换」
