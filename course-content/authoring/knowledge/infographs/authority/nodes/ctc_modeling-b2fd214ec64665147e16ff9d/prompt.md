...
## Subject
- Title: device whose input is (modeling_b2fd214ec64665147e16ff9d)
- Concept kind: theoretical_construct
- One-line definition (source of truth): device whose input is：A device whose input is the derivative of its output
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A device whose input is the derivative of its output
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「device whos…」
