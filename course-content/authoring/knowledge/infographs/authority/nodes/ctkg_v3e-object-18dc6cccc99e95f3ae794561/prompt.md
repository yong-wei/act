...
- One-line definition (source of truth): aliasing：The phenomenon where high-frequency noise is modulated to a lower frequency by the sampling process, occurring when the…
- Core intuition: 在图谱邻接中可把握：前置 → anti-alias prefilter · 后续 → Nyquist-Shannon sampling theorem。
- Relation summary: 前置 → anti-alias prefilter · 后续 → Nyquist-Shannon sampling theorem
- Extended explanation (do not paste verbatim on image): The phenomenon where high-frequency noise is modulated to a lower frequency by the sampling process…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「aliasing」、「用于分析·anti…」、「应用于·Nyqui…」
