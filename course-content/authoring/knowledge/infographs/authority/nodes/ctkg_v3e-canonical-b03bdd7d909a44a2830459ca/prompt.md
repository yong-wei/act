...
- One-line definition (source of truth): 根据系统常遇到的输入信号形式，在数学描述上加以理想化的一些基本输入函数。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → 正弦函数、单位阶跃函数、单位斜坡函数
- Extended explanation (do not paste verbatim on image): 根据系统常遇到的输入信号形式，在数学描述上加以理想化的一些基本输入函数。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「典型输入信号」、「包含组件·正弦函数」、「包含组件·单位阶跃…」、「包含组件·单位斜坡…」、「包含组件·单位加速…」、「包含组件·单位脉冲…」
