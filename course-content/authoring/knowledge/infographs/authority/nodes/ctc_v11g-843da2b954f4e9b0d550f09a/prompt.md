...
## Subject
- Title: root_locus_contr (root_locus_controller_parameter_selection_method)
- Concept kind: design_method
- One-line definition (source of truth): root_locus_contr：A method to select controller parameters by sketching the root locus as a function of a parameter and determining root…
- Core intuition: 在图谱邻接中可把握：后续 → 阻尼比。
- Relation summary: 后续 → 阻尼比
- Extended explanation (do not paste verbatim on image): A method to select controller parameters by sketching the root locus as a function of a parameter a…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「root locus …」、「用于分析·阻尼比」
