...
## Subject
- Title: 特征方程 (characteristic_equation)
- Concept kind: mathematical_object
- One-line definition (source of truth): 特征方程：The equation whose closed-loop roots in the s-plane determine the relative stability and transient performance of a clo…
- Core intuition: 在图谱邻接中可把握：前置 → 根轨迹法、给定稳定度检验方法、Routh array · 后续 → 根轨迹、根轨迹法、稳定性。
- Relation summary: 前置 → 根轨迹法、给定稳定度检验方法、Routh array · 后续 → 根轨迹、根轨迹法、稳定性
- Extended explanation (do not paste verbatim on image): The equation whose closed-loop roots in the s-plane determine the relative stability and transient…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「特征方程」、「characterist」、「用于分析·根轨迹法」、「应用于·给定稳定度…」、「派生自·Routh…」、「派生自·Norma…」、「关联·根轨迹」
