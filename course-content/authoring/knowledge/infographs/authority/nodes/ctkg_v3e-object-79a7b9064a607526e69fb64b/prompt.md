...
- One-line definition (source of truth): 一种校正控制器设计方法，通过在控制器中引入参考输入信号的内模，实现零稳态误差渐近跟踪各类参考输入信号（如阶跃、斜坡、正弦信号等）。
- Core intuition: 在图谱邻接中可把握：后续 → 内模控制器。
- Relation summary: 后续 → 内模控制器
- Extended explanation (do not paste verbatim on image): 一种校正控制器设计方法，通过在控制器中引入参考输入信号的内模，实现零稳态误差渐近跟踪各类参考输入信号（如阶跃、斜坡、正弦信号等）。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「内模控制器设计方法」、「应用于·内模控制器」
