...
## Subject
- Title: K1 (modeling_cd450f490c2d0e12795560a9)
- Concept kind: theoretical_construct
- One-line definition (source of truth): 电位器电刷单位角位移对应的输出电压，记为K1，等于电源电压E与最大工作角度θ_max的比值。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 电位器电刷单位角位移对应的输出电压，记为K1，等于电源电压E与最大工作角度θ_max的比值。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 
