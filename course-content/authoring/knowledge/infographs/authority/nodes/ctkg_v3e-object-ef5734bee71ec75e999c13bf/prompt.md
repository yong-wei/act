...
## Subject
- Title: Computing poles  (Computing poles via characteristic equation)
- Concept kind: unknown
- One-line definition (source of truth): Computing poles ：Solve det(sI - A) = 0 to find the poles (eigenvalues) of the system.
- Core intuition: 在图谱邻接中可把握：前置 → Eigenvalue · 后续 → Eigenvalue。
- Relation summary: 前置 → Eigenvalue · 后续 → Eigenvalue
- Extended explanation (do not paste verbatim on image): Solve det(sI - A) = 0 to find the poles (eigenvalues) of the system.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Computing p…」、「用于分析·Eige…」、「关联·Eigenv…」
