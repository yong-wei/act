...
- One-line definition (source of truth): Eigenvector：A vector t_i such that A t_i = p_i t_i, where p_i is the corresponding eigenvalue.
- Core intuition: 在图谱邻接中可把握：前置 → Eigenvalue、Transformation t。
- Relation summary: 前置 → Eigenvalue、Transformation t
- Extended explanation (do not paste verbatim on image): A vector t_i such that A t_i = p_i t_i, where p_i is the corresponding eigenvalue.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Eigenvector」、「关联·Eigenv…」、「关联·Transf…」
