...
## Subject
- Title: breakaway point (breakaway_point)
- Concept kind: mathematical_object
- One-line definition (source of truth): breakaway point：A point on the root locus where multiple roots meet and then move away from the real axis.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A point on the root locus where multiple roots meet and then move away from the real axis.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「breakaway p…」、「breakaway po」
