...
- One-line definition (source of truth): Duality：The property that estimation and control problems are mathematically equivalent.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The property that estimation and control problems are mathematically equivalent.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Duality」
