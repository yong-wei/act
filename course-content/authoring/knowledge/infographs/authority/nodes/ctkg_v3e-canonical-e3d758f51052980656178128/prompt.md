...
- One-line definition (source of truth): System Type：A classification of systems according to the degree of polynomial input that they can track with a constant steady-stat…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A classification of systems according to the degree of polynomial input that they can track with a…
