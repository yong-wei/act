...
## Subject
- Title: SRL estimator design method
- Concept kind: unknown
- One-line definition (source of truth): SRL estimator design method：A method using the symmetric root locus equation 1 + q G_e(-s) G_e(s) = 0 to select optimal estimator poles based on th…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A method using the symmetric root locus equation 1 + q G_e(-s) G_e(s) = 0 to select optimal estimat…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「SRL estimat…」
