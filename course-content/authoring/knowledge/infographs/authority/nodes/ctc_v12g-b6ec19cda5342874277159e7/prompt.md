...
## Subject
- Title: real axis root locus (real_axis_root_locus)
- Concept kind: mathematical_object
- One-line definition (source of truth): real axis root locus：The root locus lying in a section of the real axis to the left of an odd number of poles and zeros.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The root locus lying in a section of the real axis to the left of an odd number of poles and zeros.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「real axis r…」、「real axis ro」
