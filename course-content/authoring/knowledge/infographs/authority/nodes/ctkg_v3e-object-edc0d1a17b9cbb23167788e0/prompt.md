...
- One-line definition (source of truth): 谐波线性化。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 描述函数法 · 后续 → 描述函数
- Extended explanation (do not paste verbatim on image): 谐波线性化
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「谐波线性化」、「包含组件·描述函数法」、「用于分析·描述函数」
