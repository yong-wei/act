...
- One-line definition (source of truth): z-transform：A transform defined as Z{r(t)} = Z{r*(t)} = sum_{k=0}^{∞} r(kT) z^{-k}, where z = e^{sT}.
- Core intuition: 在图谱邻接中可把握：前置 → First-Order Plant Discrete Model、开环脉冲传递函数 · 后续 → Laplace transform、conformal mapping。
- Relation summary: 前置 → First-Order Plant Discrete Model、开环脉冲传递函数 · 后续 → Laplace transform、conformal mapping
- Extended explanation (do not paste verbatim on image): A transform defined as Z{r(t)} = Z{r*(t)} = sum_{k=0}^{∞} r(kT) z^{-k}, where z = e^{sT}.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「z-transform」、「派生自·First…」、「派生自·开环脉冲传…」、「关联·Laplac…」、「关联·confor…」
