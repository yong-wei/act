...
## Subject
- Title: time delay (time_delay)
- Concept kind: phenomenon
- One-line definition (source of truth): time delay：A delay in a control system, often arising from delays in the process itself or from delays in the processing of sensed…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A delay in a control system, often arising from delays in the process itself or from delays in the…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「time delay」
