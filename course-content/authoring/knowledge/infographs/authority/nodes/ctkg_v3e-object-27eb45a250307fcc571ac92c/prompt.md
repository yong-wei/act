...
- One-line definition (source of truth): Sample rate：Sample rate
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → Discrete equivalent、Sampled-data system、Discrete signal · 后续 → Difference equations、Sample period、Discrete signal
- Extended explanation (do not paste verbatim on image): Sample rate
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Sample rate」、「包含组件·Disc…」、「包含组件·Samp…」、「应用于·Diffe…」、「派生自·Sampl…」、「派生自·Discr…」
