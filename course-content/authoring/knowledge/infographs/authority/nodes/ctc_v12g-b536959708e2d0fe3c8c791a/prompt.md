...
## Subject
- Title: 自然频率 (natural_frequency)
- Concept kind: performance_metric
- One-line definition (source of truth): 自然频率：The natural frequency is denoted by ω_n and appears in the characteristic equation Δ(s)=s^2+2ζω_n s+ω_n^2=0. It is also…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The natural frequency is denoted by ω_n and appears in the characteristic equation Δ(s)=s^2+2ζω_n s…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「自然频率」、「natural freq」
