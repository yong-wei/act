...
## Subject
- Title: 输出方程中D不为零的系统 (modeling_42c9620057d98bfd31df8233)
- Concept kind: theoretical_construct
- One-line definition (source of truth): 输出方程中D不为零的系统。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 输出方程中D不为零的系统。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「输出方程中D不为零的系统」
