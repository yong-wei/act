...
## Subject
- Title: Ackermann's formula
- Concept kind: unknown
- One-line definition (source of truth): Ackermann's formula：A compact formula for computing the state feedback gain matrix K for pole placement, given by K = [0 ... 0 1] * C^{-1}…
- Core intuition: 在图谱邻接中可把握：前置 → Pole placement b · 后续 → Pole placement b。
- Relation summary: 前置 → Pole placement b · 后续 → Pole placement b
- Extended explanation (do not paste verbatim on image): A compact formula for computing the state feedback gain matrix K for pole placement, given by K = […
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Ackermann's…」、「Ackermann's 」、「派生自·Pole …」、「关联·Pole p…」
