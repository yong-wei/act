...
## Subject
- Title: 根轨迹 (root_locus)
- Concept kind: representation_kind
- One-line definition (source of truth): 根轨迹：The locus of roots of the characteristic equation in the s-plane as a parameter is varied.。
- Core intuition: 在图谱邻接中可把握：前置 → 根轨迹法、特征方程 · 后续 → 闭环极点。
- Relation summary: 前置 → 根轨迹法、特征方程 · 后续 → 闭环极点
- Extended explanation (do not paste verbatim on image): The locus of roots of the characteristic equation in the s-plane as a parameter is varied.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「根轨迹」、「root locus」、「应用于·根轨迹法」、「关联·特征方程」、「有表示·闭环极点」
