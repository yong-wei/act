...
## Subject
- Title: 根据结构图的信号线明确传递的信… (modeling_82926f44dbd08d4b847f084c)
- Concept kind: analysis_method
- One-line definition (source of truth): 根据结构图的信号线明确传递的信号得到节点，用标有传递函数的线段代替结构图中的方框得到支路。
- Core intuition: 应尽量精简节点数目，支路增益为1的相邻两个节点一般可合并，但源节点或阱节点不能合并。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 根据结构图的信号线明确传递的信号得到节点，用标有传递函数的线段代替结构图中的方框得到支路。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「根据结构图的信号线明确…」
