...
## Subject
- Title: Lyapunov's First Method
- Concept kind: unknown
- One-line definition (source of truth): Lyapunov's First Method：A method for studying stability based on the linear approximation: if the linear approximation is strictly stable (all…
- Core intuition: 在图谱邻接中可把握：后续 → Nonlinear System、Equilibrium Point。
- Relation summary: 后续 → Nonlinear System、Equilibrium Point
- Extended explanation (do not paste verbatim on image): A method for studying stability based on the linear approximation: if the linear approximation is s…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Lyapunov's …」、「Lyapunov's F」、「用于分析·Nonl…」、「用于分析·Equi…」
