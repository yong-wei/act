...
## Subject
- Title: Log-magnitude-phase diagram
- Concept kind: unknown
- One-line definition (source of truth): Log-magnitude-phase diagram：A graphical presentation of frequency response plotting logarithmic magnitude in dB versus phase angle for a range of f…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A graphical presentation of frequency response plotting logarithmic magnitude in dB versus phase an…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Log-magnitu…」
