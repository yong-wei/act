...
## Subject
- Title: signal flow graph (signal_flow_graph)
- Concept kind: representation_kind
- One-line definition (source of truth): signal flow graph：v3T accepted candidate for signal flow graph.
- Core intuition: 在图谱邻接中可把握：前置 → forward path、loop、formula for evaluating the · 后续 → 只有输入支路而没有输出支路的节点。
- Relation summary: 前置 → forward path、loop、formula for evaluating the · 后续 → 只有输入支路而没有输出支路的节点
- Extended explanation (do not paste verbatim on image): v3T accepted candidate for signal flow graph.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「signal flow…」、「signal flow 」、「属于·forwar…」、「属于·loop」、「包含组件·只有输入…」、「应用于·formu…」
