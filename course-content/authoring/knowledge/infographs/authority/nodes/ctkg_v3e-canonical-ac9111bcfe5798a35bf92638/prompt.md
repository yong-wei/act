...
- One-line definition (source of truth): T_m = J / F，称为机电时间常数。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): T_m = J / F，称为机电时间常数。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「机电时间常数」
