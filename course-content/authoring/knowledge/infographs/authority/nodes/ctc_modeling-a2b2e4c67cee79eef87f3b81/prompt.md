...
## Subject
- Title: forward path (forward_path)
- Concept kind: theoretical_construct
- One-line definition (source of truth): forward path：v3T accepted candidate for forward path.
- Core intuition: 在图谱邻接中可把握：后续 → signal flow graph。
- Relation summary: 后续 → signal flow graph
- Extended explanation (do not paste verbatim on image): v3T accepted candidate for forward path.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「forward path」、「属于·signal…」
