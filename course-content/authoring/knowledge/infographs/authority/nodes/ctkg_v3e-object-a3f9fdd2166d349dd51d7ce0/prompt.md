...
- One-line definition (source of truth): 将z平面上的单位圆内区域映射成w平面上的左半平面的线性变换，变换公式为z=(w+1)/(w-1)或w=(z+1)/(z-1)。
- Core intuition: 在图谱邻接中可把握：前置 → w域中的劳斯稳定判据。
- Relation summary: 前置 → w域中的劳斯稳定判据
- Extended explanation (do not paste verbatim on image): 将z平面上的单位圆内区域映射成w平面上的左半平面的线性变换，变换公式为z=(w+1)/(w-1)或w=(z+1)/(z-1)。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「w变换（双线性变换）」、「派生自·w域中的劳…」
