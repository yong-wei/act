...
- One-line definition (source of truth): 在连续信号和脉冲序列之间要用采样器，实现信号的转换。
- Core intuition: 实现采样的装置称为采样器，或称采样开关。
- Relation summary: 前置 → 理想采样器、采样控制系统、采样过程
- Extended explanation (do not paste verbatim on image): 在连续信号和脉冲序列之间要用采样器，实现信号的转换。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「采样器」、「是一种·理想采样器」、「包含组件·采样控制…」、「关联·采样过程」
