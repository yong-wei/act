...
- One-line definition (source of truth): 响应曲线的最大偏离量与稳态值之比。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 动态性能指标
- Extended explanation (do not paste verbatim on image): 响应曲线的最大偏离量与稳态值之比。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「超调量」、「包含组件·动态性能…」
