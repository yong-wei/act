...
## Subject
- Title: Velocity Error Constant (K v) (Velocity Error Constant (K_v))
- Concept kind: theoretical_construct
- One-line definition (source of truth): Velocity Error Constant (K v)：For a Type 1 system (one integrator in the loop transfer function), the constant defined as K_v = lim_{s->0} s G D_cl(s…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): For a Type 1 system (one integrator in the loop transfer function), the constant defined as K_v = l…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Velocity Er…」
