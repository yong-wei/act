...
## Subject
- Title: 基于系统内部结构分析的数学模型 (modeling_70434ed48871b3f245571e56)
- Concept kind: theoretical_construct
- One-line definition (source of truth): 基于系统内部结构分析的数学模型，由状态方程和输出方程组成。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 基于系统内部结构分析的数学模型，由状态方程和输出方程组成。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「基于系统内部结构分析的…」
