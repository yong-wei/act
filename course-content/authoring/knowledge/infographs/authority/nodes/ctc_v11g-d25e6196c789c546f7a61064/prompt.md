...
## Subject
- Title: 正反馈内回路 (positive_feedback_inner_loop)
- Concept kind: system_component
- One-line definition (source of truth): 在复杂控制系统设计中，由于某种性能指标要求而必须包含的正反馈内回路。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → negative gain root locus
- Extended explanation (do not paste verbatim on image): 在复杂控制系统设计中，由于某种性能指标要求而必须包含的正反馈内回路。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「正反馈内回路」、「关联·negati…」
