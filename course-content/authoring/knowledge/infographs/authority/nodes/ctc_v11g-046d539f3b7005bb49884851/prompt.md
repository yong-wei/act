...
## Subject
- Title: self loop transmittance l n (self_loop_transmittance_l_n)
- Concept kind: mathematical_object
- One-line definition (source of truth): self loop transmittance l n：The value of the nth self-loop transmittance in Mason's signal-flow gain formula.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The value of the nth self-loop transmittance in Mason's signal-flow gain formula.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「self loop t…」
