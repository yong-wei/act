...
## Subject
- Title: method to expand rational (modeling_eac6404c25ea4a8f89cc3f5b)
- Concept kind: analysis_method
- One-line definition (source of truth): method to expand rational：A method to expand a rational Laplace transform as a sum of simpler terms for inverse transformation.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A method to expand a rational Laplace transform as a sum of simpler terms for inverse transformatio…
