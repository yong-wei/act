...
## Subject
- Title: Regulator problem
- Concept kind: unknown
- One-line definition (source of truth): Regulator problem：A control design problem where the reference input is zero and the goal is to drive all initial conditions to zero in a…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A control design problem where the reference input is zero and the goal is to drive all initial con…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Regulator p…」、「Regulator pr」
