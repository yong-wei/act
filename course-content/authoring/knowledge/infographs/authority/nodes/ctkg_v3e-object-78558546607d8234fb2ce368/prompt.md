...
- One-line definition (source of truth): 复系数 c_k 的集合称为周期信号的频谱，表示周期信号各次谐波分量的幅值和相位分布。
- Core intuition: 在图谱邻接中可把握：前置 → 幅值谱。
- Relation summary: 前置 → 幅值谱
- Extended explanation (do not paste verbatim on image): 复系数 c_k 的集合称为周期信号的频谱，表示周期信号各次谐波分量的幅值和相位分布。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「周期信号的频谱」、「关联·幅值谱」
