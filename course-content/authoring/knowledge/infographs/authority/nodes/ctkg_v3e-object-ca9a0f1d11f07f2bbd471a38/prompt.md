...
## Subject
- Title: Bode图 (Bode plot)
- Concept kind: unknown
- One-line definition (source of truth): Bode图：A plot of frequency response consisting of two curves: the logarithm of magnitude versus log ω and the phase versus log…
- Core intuition: 在图谱邻接中可把握：前置 → Frequency response method。
- Relation summary: 前置 → Frequency response method
- Extended explanation (do not paste verbatim on image): A plot of frequency response consisting of two curves: the logarithm of magnitude versus log ω and…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Bode图」、「Bode plot」、「关联·Freque…」
