...
## Subject
- Title: 将传递函数分解为严格有理真分式… (modeling_43db7af217e7530329f7d3ee)
- Concept kind: analysis_method
- One-line definition (source of truth): 将传递函数分解为严格有理真分式 $N(s)/D(s)$，然后通过串联分解选取状态变量，导出状态空间表达式的方法。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 将传递函数分解为严格有理真分式 $N(s)/D(s)$，然后通过串联分解选取状态变量，导出状态空间表达式的方法。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「将传递函数分解为严格有…」
