...
- One-line definition (source of truth): 开环幅相特性曲线在ω=∞处的点，取决于开环传递函数分子、分母多项式中最小相位环节和非最小相位环节的阶次和。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 开环幅相特性曲线在ω=∞处的点，取决于开环传递函数分子、分母多项式中最小相位环节和非最小相位环节的阶次和。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「开环幅相特性曲线终点」
