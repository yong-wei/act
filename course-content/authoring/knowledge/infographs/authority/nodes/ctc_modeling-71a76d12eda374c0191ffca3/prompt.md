...
## Subject
- Title: 两相伺服电动机时间常数Tm=J… (modeling_71a76d12eda374c0191ffca3)
- Concept kind: theoretical_construct
- One-line definition (source of truth): 两相伺服电动机时间常数Tm=Jm/(fm+Cω)。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 两相伺服电动机时间常数Tm=Jm/(fm+Cω)。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「两相伺服电动机时间常数…」
