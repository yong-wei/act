...
## Subject
- Title: continuation locus (continuation_locus)
- Concept kind: analysis_method
- One-line definition (source of truth): continuation locus：A trick used to compute angles of arrival and departure from a point of multiple roots by plotting a root locus for an…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A trick used to compute angles of arrival and departure from a point of multiple roots by plotting…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「continuatio…」、「continuation」
