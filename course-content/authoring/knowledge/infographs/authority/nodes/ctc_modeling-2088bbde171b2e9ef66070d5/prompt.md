...
## Subject
- Title: block diagram reduction (block_diagram_reduction)
- Concept kind: analysis_method
- One-line definition (source of truth): block diagram reduction：v3T accepted candidate for block-diagram reduction.
- Core intuition: 在图谱邻接中可把握：后续 → block diagram。
- Relation summary: 后续 → block diagram
- Extended explanation (do not paste verbatim on image): v3T accepted candidate for block-diagram reduction.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「block diagr…」、「block diagra」、「应用于·block…」
