...
## Subject
- Title: 系统临界稳定时的开环增益 (critical_open_loop_gain)
- Concept kind: mathematical_object
- One-line definition (source of truth): 根轨迹与虚轴交点处的K值，即系统临界稳定时的开环增益。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 根轨迹与虚轴交点处的K值，即系统临界稳定时的开环增益。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「系统临界稳定时的开环增益」、「critical ope」
