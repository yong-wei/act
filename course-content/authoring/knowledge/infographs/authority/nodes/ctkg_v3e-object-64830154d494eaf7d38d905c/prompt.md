...
## Subject
- Title: 高阶系统时域指标估算经验公式法
- Concept kind: unknown
- One-line definition (source of truth): 利用式(5-107)和(5-108)估算高阶系统超调量和调节时间。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 利用式(5-107)和(5-108)估算高阶系统超调量和调节时间
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「高阶系统时域指标估算经…」
