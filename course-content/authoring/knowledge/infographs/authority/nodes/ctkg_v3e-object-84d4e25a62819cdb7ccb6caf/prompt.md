...
## Subject
- Title: Type-zero system
- Concept kind: unknown
- One-line definition (source of truth): Type-zero system：A system with no poles at the origin in the loop transfer function.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A system with no poles at the origin in the loop transfer function.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Type-zero s…」、「Type-zero sy」
