...
## Subject
- Title: 以n个状态变量作为基底所组成的… (modeling_7eda83cdbc9e753480c9189c)
- Concept kind: theoretical_construct
- One-line definition (source of truth): 以n个状态变量作为基底所组成的n维空间。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 以n个状态变量作为基底所组成的n维空间。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「以n个状态变量作为基底…」
