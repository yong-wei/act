...
## Subject
- Title: 用于简化离散化输入矩阵 G(T… (modeling_49366083a7c2d25157dff636)
- Concept kind: analysis_method
- One-line definition (source of truth): 用于简化离散化输入矩阵 G(T) 计算的方法，令 (k+1)T - τ = τ' 进行变量置换。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 用于简化离散化输入矩阵 G(T) 计算的方法，令 (k+1)T - τ = τ' 进行变量置换。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「用于简化离散化输入矩阵…」
