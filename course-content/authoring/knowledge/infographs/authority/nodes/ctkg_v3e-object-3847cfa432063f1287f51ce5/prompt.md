...
## Subject
- Title: Frequency-Response Stability Test
- Concept kind: unknown
- One-line definition (source of truth): Frequency-Response Stability Test：A method to determine closed-loop stability by evaluating the frequency response of the open-loop transfer function KG(…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A method to determine closed-loop stability by evaluating the frequency response of the open-loop t…
