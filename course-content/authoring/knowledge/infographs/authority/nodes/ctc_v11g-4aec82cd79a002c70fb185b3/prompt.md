...
## Subject
- Title: loop transfer function l s (loop_transfer_function_l_s)
- Concept kind: representation_kind
- One-line definition (source of truth): loop transfer function l s：The transfer function defined as L(s)=b(s)/a(s) used in the root-locus form of the characteristic equation.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The transfer function defined as L(s)=b(s)/a(s) used in the root-locus form of the characteristic e…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「loop transf…」
