...
- One-line definition (source of truth): Sampled data：Data obtained for the system variables only at discrete intervals.
- Core intuition: 在图谱邻接中可把握：前置 → Discrete signal。
- Relation summary: 前置 → Discrete signal
- Extended explanation (do not paste verbatim on image): Data obtained for the system variables only at discrete intervals.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Sampled data」、「一种·Discre…」
