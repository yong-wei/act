...
- One-line definition (source of truth): 对于无源超前网络，a=(R1+R2)/R2>1；对于无源滞后网络，b=R2/(R1+R2)<1。
- Core intuition: 对于无源滞后网络，b=R2/(R1+R2)<1。
- Relation summary: 前置 → 滞后深度
- Extended explanation (do not paste verbatim on image): 对于无源超前网络，a=(R1+R2)/R2>1；对于无源滞后网络，b=R2/(R1+R2)<1。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「分度系数」、「关联·滞后深度」
