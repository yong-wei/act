...
- One-line definition (source of truth): 一类校正控制器，通过在控制器中引入参考输入信号的内模，能以零稳态误差渐近跟踪各类参考输入信号。
- Core intuition: 在图谱邻接中可把握：前置 → 内模控制器设计方法 · 后续 → 内模。
- Relation summary: 前置 → 内模控制器设计方法 · 后续 → 内模
- Extended explanation (do not paste verbatim on image): 一类校正控制器，通过在控制器中引入参考输入信号的内模，能以零稳态误差渐近跟踪各类参考输入信号。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「内模控制器」、「包含组件·内模」、「应用于·内模控制器…」
