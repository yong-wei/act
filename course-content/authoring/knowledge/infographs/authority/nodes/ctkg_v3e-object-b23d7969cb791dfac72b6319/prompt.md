...
- One-line definition (source of truth): 如果采样器位于系统闭合回路之内，则称为闭环采样系统。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 采样控制系统 · 后续 → 采样控制系统
- Extended explanation (do not paste verbatim on image): 如果采样器位于系统闭合回路之内，则称为闭环采样系统。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「闭环采样系统」、「是一种·采样控制系统」、「关联·采样控制系统」
