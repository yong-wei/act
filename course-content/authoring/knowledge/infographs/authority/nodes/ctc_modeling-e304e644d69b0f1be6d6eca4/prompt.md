...
## Subject
- Title: The output of system (modeling_e304e644d69b0f1be6d6eca4)
- Concept kind: theoretical_construct
- One-line definition (source of truth): The output of system：The output of a system when the input is a unit impulse
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The output of a system when the input is a unit impulse
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「The output …」
