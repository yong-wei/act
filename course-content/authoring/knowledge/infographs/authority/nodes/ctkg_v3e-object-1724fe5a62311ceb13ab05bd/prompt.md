...
- One-line definition (source of truth): 线性定常连续系统完全可控的充分必要条件是，存在时刻 t1>0，使格拉姆矩阵 W(0, t1) 为非奇异。
- Core intuition: 在图谱邻接中可把握：后续 → 系统完全可控。
- Relation summary: 后续 → 系统完全可控
- Extended explanation (do not paste verbatim on image): 线性定常连续系统完全可控的充分必要条件是，存在时刻 t1>0，使格拉姆矩阵 W(0, t1) 为非奇异。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「格拉姆矩阵判据」、「应用于·系统完全可控」
