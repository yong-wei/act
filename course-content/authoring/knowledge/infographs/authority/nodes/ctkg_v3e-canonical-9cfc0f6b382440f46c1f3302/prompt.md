...
## Subject
- Title: Rise time T r (Rise time T_r)
- Concept kind: theoretical_construct
- One-line definition (source of truth): Rise time T r：A measure of the swiftness of response
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A measure of the swiftness of response
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Rise time T…」、「Rise time T 」
