...
- One-line definition (source of truth): 描述函数法是基于频域分析法和非线性特性谐波线性化的一种图解分析方法。
- Core intuition: 该方法对于满足结构要求的一类非线性系统，通过谐波线性化，将非线性特性近似表示为复变增益环节，然后推广应用频率法，分析非线性系统的稳定性或自激振荡。
- Relation summary: 后续 → 描述函数、谐波线性化、非线性系统
- Extended explanation (do not paste verbatim on image): 描述函数法是基于频域分析法和非线性特性谐波线性化的一种图解分析方法。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「描述函数法」、「包含组件·描述函数」、「包含组件·谐波线性化」、「用于分析·非线性系统」、「用于分析·极限环」、「用于分析·自激振荡」
