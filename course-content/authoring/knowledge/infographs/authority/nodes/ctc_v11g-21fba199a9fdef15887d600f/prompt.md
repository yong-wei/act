...
- One-line definition (source of truth): lag compensation：A compensation scheme that approximates PI control, used to improve steady-state accuracy of the system.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A compensation scheme that approximates PI control, used to improve steady-state accuracy of the sy…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「lag compens…」、「lag compensa」
