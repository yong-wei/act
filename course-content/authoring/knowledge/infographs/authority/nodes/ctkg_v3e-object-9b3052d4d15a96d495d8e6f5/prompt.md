...
- One-line definition (source of truth): 具有积分控制规律的控制器，称为I控制器，其输出信号u(t)与其输入信号e(t)的积分成正比。
- Core intuition: 在图谱邻接中可把握：前置 → I控制器。
- Relation summary: 前置 → I控制器
- Extended explanation (do not paste verbatim on image): 具有积分控制规律的控制器，称为I控制器，其输出信号u(t)与其输入信号e(t)的积分成正比。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「积分控制规律」、「关联·I控制器」
