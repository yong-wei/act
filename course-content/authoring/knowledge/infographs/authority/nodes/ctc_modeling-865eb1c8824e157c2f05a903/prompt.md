...
## Subject
- Title: 传递函数 (transfer_function)
- Concept kind: representation_kind
- One-line definition (source of truth): 传递函数：v3T accepted candidate for transfer function.。
- Core intuition: 在图谱邻接中可把握：前置 → evans root locus construction method、根轨迹法、The ratio of the · 后续 → 在时间域描述系统动态性能的数学…、impulse response。
- Relation summary: 前置 → evans root locus construction method、根轨迹法、The ratio of the · 后续 → 在时间域描述系统动态性能的数学…、impulse response
- Extended explanation (do not paste verbatim on image): v3T accepted candidate for transfer function.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「传递函数」、「transfer fun」、「应用于·evans…」、「应用于·根轨迹法」、「派生自·在时间域描…」、「派生自·The r…」、「派生自·零点」
