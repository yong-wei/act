...
## Subject
- Title: Pole placement b (Pole placement by coefficient matching in control canonical form)
- Concept kind: unknown
- One-line definition (source of truth): Pole placement b：A method for designing a full-state feedback control law by transforming the system to control canonical form, then equ…
- Core intuition: 在图谱邻接中可把握：前置 → Ackermann's formula · 后续 → Control canonical form、Ackermann's formula。
- Relation summary: 前置 → Ackermann's formula · 后续 → Control canonical form、Ackermann's formula
- Extended explanation (do not paste verbatim on image): A method for designing a full-state feedback control law by transforming the system to control cano…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Pole placem…」、「用于分析·Cont…」、「派生自·Acker…」、「关联·Ackerm…」
