...
## Subject
- Title: 根据系统机理建立微分或差分方程… (modeling_14dc921a2171d3adbe16bbf1)
- Concept kind: analysis_method
- One-line definition (source of truth): 建立状态空间表达式的两种主要方法：一是根据系统机理建立微分或差分方程并选择状态变量；二是由已知的其他数学模型转化得到。
- Core intuition: 二是由已知的其他数学模型转化得到。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 建立状态空间表达式的两种主要方法：一是根据系统机理建立微分或差分方程并选择状态变量；二是由已知的其他数学模型转化得到。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「根据系统机理建立微分或…」
