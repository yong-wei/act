...
- One-line definition (source of truth): 利用超前网络或PD控制器进行串联校正的基本原理，是利用超前网络或PD控制器的相角超前特性。
- Core intuition: 在图谱邻接中可把握：后续 → 频率响应法校正设计。
- Relation summary: 后续 → 频率响应法校正设计
- Extended explanation (do not paste verbatim on image): 利用超前网络或PD控制器进行串联校正的基本原理，是利用超前网络或PD控制器的相角超前特性。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「串联超前校正」、「属于·频率响应法校…」
