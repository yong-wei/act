...
## Subject
- Title: 离散化状态方程中用于从当前状态… (modeling_88803865ceb9957c00782479)
- Concept kind: theoretical_construct
- One-line definition (source of truth): 离散化状态方程中用于从当前状态计算下一时刻状态的矩阵，记为 Φ(T)，与连续系统状态转移矩阵 Φ(t) 的关系为 Φ(T) = Φ(t)|_{t=T}。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 离散化状态方程中用于从当前状态计算下一时刻状态的矩阵，记为 Φ(T)，与连续系统状态转移矩阵 Φ(t) 的关系为 Φ(T) = Φ(t)|_{t=T}。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「离散化状态方程中用于从…」
