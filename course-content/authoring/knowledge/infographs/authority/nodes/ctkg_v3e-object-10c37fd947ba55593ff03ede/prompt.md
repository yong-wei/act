...
- One-line definition (source of truth): 进行控制系统的校正设计，除了应已知系统不可变部分的特性与参数外，还需要已知对系统提出的全部性能指标。
- Core intuition: 性能指标通常是由使用单位或被控对象的设计制造单位提出的。
- Relation summary: 前置 → 系统带宽 · 后续 → 校正问题
- Extended explanation (do not paste verbatim on image): 进行控制系统的校正设计，除了应已知系统不可变部分的特性与参数外，还需要已知对系统提出的全部性能指标。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「性能指标」、「是一种·系统带宽」、「应用于·校正问题」
