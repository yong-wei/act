...
- One-line definition (source of truth): 利用静态位置误差系数 K_p、静态速度误差系数 K_v 和静态加速度误差系数 K_a，根据输入信号形式（阶跃、斜坡、加速度）计算稳态误差。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 利用静态位置误差系数 K_p、静态速度误差系数 K_v 和静态加速度误差系数 K_a，根据输入信号形式（阶跃、斜坡、加速度）计算稳态误差。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「静态误差系数法求稳态误差」
