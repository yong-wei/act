...
## Subject
- Title: System Type for  (System Type for Regulation and Disturbance Rejection)
- Concept kind: theoretical_construct
- One-line definition (source of truth): System Type for ：A classification of a system based on its ability to reject polynomial disturbance inputs, analogous to the classificat…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A classification of a system based on its ability to reject polynomial disturbance inputs, analogou…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「System Type…」
