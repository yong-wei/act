...
- One-line definition (source of truth): Asynchronous Sampling：As noted in the previous paragraphs, divorcing the prefilter design from the control-law design may require using a fas…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): As noted in the previous paragraphs, divorcing the prefilter design from the control-law design may…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Asynchronou…」、「Asynchronous」
