...
## Subject
- Title: 在时间域描述系统动态性能的数学… (v5c_differential_equation_model_targeted)
- Concept kind: representation_kind
- One-line definition (source of truth): 控制系统的微分方程是在时间域描述系统动态性能的数学模型。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 传递函数、dynamic system
- Extended explanation (do not paste verbatim on image): 控制系统的微分方程是在时间域描述系统动态性能的数学模型
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「在时间域描述系统动态性…」、「派生自·传递函数」、「有表示·dynam…」
