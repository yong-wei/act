...
## Subject
- Title: 只有信号输出支路而没有信号输入… (modeling_4df124031c5a8ee564df38c9)
- Concept kind: theoretical_construct
- One-line definition (source of truth): 只有信号输出支路而没有信号输入支路的节点，一般代表系统的输入变量。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 只有信号输出支路而没有信号输入支路的节点，一般代表系统的输入变量。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「只有信号输出支路而没有…」
