...
- One-line definition (source of truth): 由一簇相轨迹所组成的图形称为相平面图。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 相轨迹 · 后续 → 相轨迹、分隔线
- Extended explanation (do not paste verbatim on image): 由一簇相轨迹所组成的图形称为相平面图。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「相平面图」、「属于·相轨迹」、「包含组件·相轨迹」、「包含组件·分隔线」
