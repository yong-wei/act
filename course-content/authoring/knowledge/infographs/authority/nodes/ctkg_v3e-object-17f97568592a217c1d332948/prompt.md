...
- One-line definition (source of truth): Compensated system：The system after adding the phase-lag compensator.
- Core intuition: 在图谱邻接中可把握：前置 → Frequency response method、Nichols chart for compensated system。
- Relation summary: 前置 → Frequency response method、Nichols chart for compensated system
- Extended explanation (do not paste verbatim on image): The system after adding the phase-lag compensator.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Compensated…」、「Compensated 」、「用于分析·Freq…」、「用于分析·Nich…」
