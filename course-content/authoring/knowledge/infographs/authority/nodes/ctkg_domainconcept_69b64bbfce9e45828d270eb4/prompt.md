...
## Subject
- Title: First-Order Plant Discrete Model
- Concept kind: unknown
- One-line definition (source of truth): First-Order Plant Discrete Model是自动控制原理权威图谱中的领域概念。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → z-transform
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「First-Order…」、「派生自·z-tra…」
