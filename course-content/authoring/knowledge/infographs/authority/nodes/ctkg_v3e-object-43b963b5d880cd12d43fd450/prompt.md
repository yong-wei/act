...
## Subject
- Title: Using vector margin to assess stability
- Concept kind: unknown
- One-line definition (source of truth): Using vector margin to assess stability：Using vector margin to assess stability
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → Vector Margin
- Extended explanation (do not paste verbatim on image): Using vector margin to assess stability
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Using vecto…」、「关联·Vector…」
