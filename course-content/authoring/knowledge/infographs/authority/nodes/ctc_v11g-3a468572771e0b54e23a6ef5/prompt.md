...
## Subject
- Title: graphical gain calculation method (graphical_gain_calculation_method)
- Concept kind: analysis_method
- One-line definition (source of truth): graphical gain calculation method：Method to compute the gain K by measuring the lengths of vectors from the poles to a point on the root locus and multip…
- Core intuition: 在图谱邻接中可把握：后续 → 增益。
- Relation summary: 后续 → 增益
- Extended explanation (do not paste verbatim on image): Method to compute the gain K by measuring the lengths of vectors from the poles to a point on the r…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「graphical g…」、「应用于·增益」
