...
- One-line definition (source of truth): 滞后网络相角达到最大滞后值的频率，ωm=1/(T√b)。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 滞后网络相角达到最大滞后值的频率，ωm=1/(T√b)。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「最大滞后角频率」
