...
## Subject
- Title: Matrix Exponenti (Matrix Exponential Method for Solving State Equations)
- Concept kind: unknown
- One-line definition (source of truth): Matrix Exponenti：A method that expresses the solution of linear time-invariant state equations using the matrix exponential.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A method that expresses the solution of linear time-invariant state equations using the matrix expo…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Matrix Expo…」
