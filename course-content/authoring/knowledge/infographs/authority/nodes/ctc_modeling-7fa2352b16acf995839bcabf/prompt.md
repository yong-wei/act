...
## Subject
- Title: representation of the motion (modeling_7fa2352b16acf995839bcabf)
- Concept kind: theoretical_construct
- One-line definition (source of truth): representation of the motion：A representation of the motion of any finite dynamic system as a set of first-order ODEs.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A representation of the motion of any finite dynamic system as a set of first-order ODEs.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「representat…」
