...
- One-line definition (source of truth): Resonant peak：The maximum value of the frequency-response magnitude.
- Core intuition: 在图谱邻接中可把握：后续 → 阻尼比、Resonant frequency。
- Relation summary: 后续 → 阻尼比、Resonant frequency
- Extended explanation (do not paste verbatim on image): The maximum value of the frequency-response magnitude.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Resonant pe…」、「Resonant pea」、「关联·阻尼比」、「关联·Resona…」
