...
## Subject
- Title: Contour evaluation
- Concept kind: unknown
- One-line definition (source of truth): Contour evaluation：The process of evaluating a transfer function H(s) for values of s on a closed contour in the s-plane.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The process of evaluating a transfer function H(s) for values of s on a closed contour in the s-pla…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Contour eva…」、「Contour eval」
