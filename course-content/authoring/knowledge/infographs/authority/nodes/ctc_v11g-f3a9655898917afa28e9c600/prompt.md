...
## Subject
- Title: manual pid tuning methods (manual_pid_tuning_methods)
- Concept kind: design_method
- One-line definition (source of truth): manual pid tuning methods：The process of determining the PID controller gains by trial-and-error with minimal analytic analysis.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The process of determining the PID controller gains by trial-and-error with minimal analytic analys…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「manual pid …」
