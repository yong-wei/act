...
- One-line definition (source of truth): 摩擦特性。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 等效增益、低速爬行现象
- Extended explanation (do not paste verbatim on image): 摩擦特性
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「摩擦特性」、「关联·等效增益」、「关联·低速爬行现象」
