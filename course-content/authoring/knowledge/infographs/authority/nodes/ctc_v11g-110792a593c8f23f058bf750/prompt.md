...
## Subject
- Title: I型系统 (type_one_system)
- Concept kind: system_kind
- One-line definition (source of truth): 开环系统在坐标原点有一个极点的系统类型。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → 静态速度误差系数
- Extended explanation (do not paste verbatim on image): 开环系统在坐标原点有一个极点的系统类型。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「I型系统」、「type one sys」、「关联·静态速度误差…」
