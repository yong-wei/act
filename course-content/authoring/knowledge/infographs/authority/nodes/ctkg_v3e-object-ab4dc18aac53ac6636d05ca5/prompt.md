...
## Subject
- Title: Full-state feedback design procedure
- Concept kind: unknown
- One-line definition (source of truth): Full-state feedback design procedure：The first step in the state variable design process requires us to assume that all the states are available for feedbac…
- Core intuition: 在图谱邻接中可把握：前置 → Separation principle · 后续 → Full-state feedback。
- Relation summary: 前置 → Separation principle · 后续 → Full-state feedback
- Extended explanation (do not paste verbatim on image): The first step in the state variable design process requires us to assume that all the states are a…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Full-state …」、「应用于·Full-…」、「关联·Separa…」
