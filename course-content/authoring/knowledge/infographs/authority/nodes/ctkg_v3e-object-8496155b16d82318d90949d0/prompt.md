...
- One-line definition (source of truth): Frequency Response：A linear system's response to sinusoidal inputs, obtained from knowledge of its pole and zero locations.
- Core intuition: 在图谱邻接中可把握：前置 → Experimental fre、Bode plot techniques。
- Relation summary: 前置 → Experimental fre、Bode plot techniques
- Extended explanation (do not paste verbatim on image): A linear system's response to sinusoidal inputs, obtained from knowledge of its pole and zero locat…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Frequency R…」、「Frequency Re」、「用于分析·Expe…」、「用于分析·Bode…」
