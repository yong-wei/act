...
## Subject
- Title: branches of the root locus (branches_of_the_root_locus)
- Concept kind: representation_kind
- One-line definition (source of truth): branches of the root locus：The individual loci that make up the root locus
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The individual loci that make up the root locus
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「branches of…」
