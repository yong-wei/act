...
- One-line definition (source of truth): 脉冲序列。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 离散模拟信号、理想脉冲、离散时间系统
- Extended explanation (do not paste verbatim on image): 脉冲序列
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「脉冲序列」、「是一种·离散模拟信号」、「是一种·理想脉冲」、「包含组件·离散时间…」、「包含组件·采样控制…」
