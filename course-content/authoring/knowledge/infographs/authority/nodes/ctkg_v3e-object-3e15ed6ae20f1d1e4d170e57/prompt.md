...
## Subject
- Title: equivalent gain
- Concept kind: unknown
- One-line definition (source of truth): equivalent gain：A gain that replaces a memoryless nonlinearity in a root locus analysis, taking on a range of values for a range of inp…
- Core intuition: 在图谱邻接中可把握：前置 → Equivalent gain  · 后续 → Root-Locus Techn。
- Relation summary: 前置 → Equivalent gain  · 后续 → Root-Locus Techn
- Extended explanation (do not paste verbatim on image): A gain that replaces a memoryless nonlinearity in a root locus analysis, taking on a range of value…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「equivalent …」、「equivalent g」、「包含组件·Equi…」、「关联·Root-L…」
