...
## Subject
- Title: 否成立 (Discrete-time controllability rank test)
- Concept kind: unknown
- One-line definition (source of truth): 构造可控性矩阵S1=[h Gh ... G^{n-1}h]，检验rank S1=n是否成立。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 构造可控性矩阵S1=[h Gh ... G^{n-1}h]，检验rank S1=n是否成立。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「否成立」
