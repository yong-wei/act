...
- One-line definition (source of truth): Lag Compensation：Dynamic compensation that approximates proportional-integral (PI) control.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): Dynamic compensation that approximates proportional-integral (PI) control.
