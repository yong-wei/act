...
## Subject
- Title: integral controller (integral_controller)
- Concept kind: system_component
- One-line definition (source of truth): integral controller：A controller with transfer function G_c(s) = K/s.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A controller with transfer function G_c(s) = K/s.
