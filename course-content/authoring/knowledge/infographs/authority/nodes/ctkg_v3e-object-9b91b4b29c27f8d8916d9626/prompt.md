...
## Subject
- Title: Full-state observer
- Concept kind: unknown
- One-line definition (source of truth): Full-state observer：A dynamic system that estimates the state vector x(t) of a plant using the system model and output measurements, such t…
- Core intuition: 在图谱邻接中可把握：前置 → Kalman filter、Ackermann's formula for observer gain、Reduced-order observer。
- Relation summary: 前置 → Kalman filter、Ackermann's formula for observer gain、Reduced-order observer
- Extended explanation (do not paste verbatim on image): A dynamic system that estimates the state vector x(t) of a plant using the system model and output…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Full-state …」、「Full-state o」、「一种·Kalman」、「用于分析·Acke…」、「关联·Reduce…」
