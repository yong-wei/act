...
- One-line definition (source of truth): Gain Margin (GM)：The factor by which the gain can be increased (or decreased in certain cases) before instability results.
- Core intuition: 在图谱邻接中可把握：前置 → Determining GM from root locus · 后续 → Determining GM from root locus、Determining GM from Bode plot。
- Relation summary: 前置 → Determining GM from root locus · 后续 → Determining GM from root locus、Determining GM from Bode plot
- Extended explanation (do not paste verbatim on image): The factor by which the gain can be increased (or decreased in certain cases) before instability re…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Gain Margin…」、「Gain Margin 」、「派生自·Deter…」、「关联·Determ…」
