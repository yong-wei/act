...
## Subject
- Title: Control design methods based (modeling_1ca93c10f7b87df4cc16b696)
- Concept kind: analysis_method
- One-line definition (source of truth): Control design methods based：Control design methods based on transfer functions, such as root locus and frequency response.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): Control design methods based on transfer functions, such as root locus and frequency response.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Control des…」
