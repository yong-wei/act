...
- One-line definition (source of truth): Transient response：The component y1(t) is called the transient response as it decays to zero as time goes on.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The component y1(t) is called the transient response as it decays to zero as time goes on.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Transient r…」、「Transient re」
