...
- One-line definition (source of truth): 当调整放大器增益后仍不能满足性能指标时，在系统中增加参数及特性可按需要改变的校正装置，使系统性能全面满足设计要求的设计问题。
- Core intuition: 在图谱邻接中可把握：前置 → 性能指标。
- Relation summary: 前置 → 性能指标
- Extended explanation (do not paste verbatim on image): 当调整放大器增益后仍不能满足性能指标时，在系统中增加参数及特性可按需要改变的校正装置，使系统性能全面满足设计要求的设计问题。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「校正问题」、「应用于·性能指标」
