...
## Subject
- Title: Observer Canonical Form
- Concept kind: unknown
- One-line definition (source of truth): Observer Canonical Form：A canonical form for which the estimator gain design equations are particularly simple and the existence of a solution…
- Core intuition: 在图谱邻接中可把握：前置 → Observability。
- Relation summary: 前置 → Observability
- Extended explanation (do not paste verbatim on image): A canonical form for which the estimator gain design equations are particularly simple and the exis…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Observer Ca…」、「Observer Can」、「关联·Observ…」
