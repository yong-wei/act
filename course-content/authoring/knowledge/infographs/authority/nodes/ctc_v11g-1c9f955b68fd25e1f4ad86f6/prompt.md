...
## Subject
- Title: 调节时间 (settling_time)
- Concept kind: performance_metric
- One-line definition (source of truth): 调节时间：The time for the system response to settle to within 2% of the final value.。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The time for the system response to settle to within 2% of the final value.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「调节时间」、「settling tim」
