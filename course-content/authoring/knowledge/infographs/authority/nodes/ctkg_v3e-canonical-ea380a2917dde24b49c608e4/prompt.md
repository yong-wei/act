...
- One-line definition (source of truth): 利用MATLAB软件，先建立高阶系统模型，再直接调用step命令即可求出单位阶跃响应。
- Core intuition: 在图谱邻接中可把握：后续 → 利用MATLAB进行高阶系统建…。
- Relation summary: 后续 → 利用MATLAB进行高阶系统建…
- Extended explanation (do not paste verbatim on image): 利用MATLAB软件，先建立高阶系统模型，再直接调用step命令即可求出单位阶跃响应。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「可求出单位阶跃响应」、「包含组件·利用MA…」
