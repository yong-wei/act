...
## Subject
- Title: 否包含元素全… (Diagonal canonical form controllability criterion)
- Concept kind: unknown
- One-line definition (source of truth): 当A的特征值两两相异时，通过线性变换化为对角线规范型，检查B矩阵是否包含元素全为零的行。
- Core intuition: 在图谱邻接中可把握：后续 → 对角线规范型判据。
- Relation summary: 后续 → 对角线规范型判据
- Extended explanation (do not paste verbatim on image): 当A的特征值两两相异时，通过线性变换化为对角线规范型，检查B矩阵是否包含元素全为零的行。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「否包含元素全…」、「一种·对角线规范型…」
