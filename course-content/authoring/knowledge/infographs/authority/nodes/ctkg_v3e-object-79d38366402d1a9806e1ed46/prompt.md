...
## Subject
- Title: 输出可控性矩阵 (output…
- Concept kind: unknown
- One-line definition (source of truth): 输出可控性矩阵 (output controllability matrix)。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 否成立
- Extended explanation (do not paste verbatim on image): 输出可控性矩阵 (output controllability matrix)
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「输出可控性矩阵 (ou…」、「包含组件·否成立」
