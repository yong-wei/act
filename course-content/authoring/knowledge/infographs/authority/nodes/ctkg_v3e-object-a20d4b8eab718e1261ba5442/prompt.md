...
- One-line definition (source of truth): 特殊的相轨迹，它将相平面划分为具有不同运动特点的各个区域。
- Core intuition: 最常见的奇线是极限环。
- Relation summary: 前置 → 极限环 · 后续 → 相轨迹、相平面
- Extended explanation (do not paste verbatim on image): 特殊的相轨迹，它将相平面划分为具有不同运动特点的各个区域。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「奇线」、「是一种·极限环」、「是一种·相轨迹」、「属于·相平面」
