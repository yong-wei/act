...
- One-line definition (source of truth): Phase-Lead Compensator：A first-order compensator where |z|<|p|, providing a phase-lead characteristic.
- Core intuition: 在图谱邻接中可把握：前置 → 串联滞后-超前校正、Phase-Lead Design Using the Bode Plot、Phase-lead compe。
- Relation summary: 前置 → 串联滞后-超前校正、Phase-Lead Design Using the Bode Plot、Phase-lead compe
- Extended explanation (do not paste verbatim on image): A first-order compensator where |z|<|p|, providing a phase-lead characteristic.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Phase-Lead …」、「Phase-Lead C」、「包含组件·串联滞后…」、「应用于·Phase…」
