...
## Subject
- Title: 当所有闭环极点位于实轴上时 (overdamped_system)
- Concept kind: system_kind
- One-line definition (source of truth): 当所有闭环极点位于实轴上时，系统为过阻尼系统，单位阶跃响应为非周期过程。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 当所有闭环极点位于实轴上时，系统为过阻尼系统，单位阶跃响应为非周期过程。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「当所有闭环极点位于实轴…」、「overdamped s」
