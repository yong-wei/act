...
## Subject
- Title: 将微分方程或积分方程通过拉氏变… (modeling_c0359c7c7e3a044d422b43d4)
- Concept kind: analysis_method
- One-line definition (source of truth): 将微分方程或积分方程通过拉氏变换转换为s的代数方程，对每个变量指定一个节点，按因果关系从左向右排列，用标明支路增益的支路连接各节点变量。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 将微分方程或积分方程通过拉氏变换转换为s的代数方程，对每个变量指定一个节点，按因果关系从左向右排列，用标明支路增益的支路连接各节点变量。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「将微分方程或积分方程通…」
