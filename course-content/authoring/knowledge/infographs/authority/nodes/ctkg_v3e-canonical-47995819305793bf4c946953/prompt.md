...
- One-line definition (source of truth): 极点：Values of s such that a(s)=0 will represent points where H(s) is infinity. These s-values are called poles of H(s).。
- Core intuition: 在图谱邻接中可把握：前置 → Undamped natural frequency omega n、Fast pole and slow pole。
- Relation summary: 前置 → Undamped natural frequency omega n、Fast pole and slow pole
- Extended explanation (do not paste verbatim on image): Values of s such that a(s)=0 will represent points where H(s) is infinity. These s-values are calle…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「极点」、「Pole」、「派生自·Undam…」、「关联·Fast p…」
