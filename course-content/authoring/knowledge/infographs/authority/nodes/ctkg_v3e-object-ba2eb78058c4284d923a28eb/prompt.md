...
## Subject
- Title: Ackermann's formula for observer gain
- Concept kind: unknown
- One-line definition (source of truth): Ackermann's formula for observer gain：A formula to compute the observer gain matrix L given a desired characteristic polynomial p(lambda).
- Core intuition: 在图谱邻接中可把握：后续 → Full-state observer。
- Relation summary: 后续 → Full-state observer
- Extended explanation (do not paste verbatim on image): A formula to compute the observer gain matrix L given a desired characteristic polynomial p(lambda).
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Ackermann's…」、「用于分析·Full…」
