...
- One-line definition (source of truth): Sinusoidal steady-state response：The steady-state output of a stable linear system when excited by a sinusoidal input, consisting solely of a sinusoid a…
- Core intuition: 在图谱邻接中可把握：前置 → Natural unforced response。
- Relation summary: 前置 → Natural unforced response
- Extended explanation (do not paste verbatim on image): The steady-state output of a stable linear system when excited by a sinusoidal input, consisting so…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Sinusoidal …」、「关联·Natura…」
