...
- One-line definition (source of truth): Break point：The frequency at which the asymptotes of a first-order or second-order term cross
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The frequency at which the asymptotes of a first-order or second-order term cross
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Break point」
