...
## Subject
- Title: system that is both (modeling_26ca57c1d48b4588cb72f6a0)
- Concept kind: theoretical_construct
- One-line definition (source of truth): system that is both：A system that is both linear and time-invariant, forming the basis for many analytical techniques.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A system that is both linear and time-invariant, forming the basis for many analytical techniques.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「system that…」
