...
## Subject
- Title: 前向通路上各支路增益之乘积 (modeling_v2r_recovered_73552d8da29fbd2073b5f21a)
- Concept kind: theoretical_construct
- One-line definition (source of truth): 前向通路上各支路增益之乘积，一般用 p_k 表示。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 前向通路上各支路增益之乘积，一般用 p_k 表示。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「前向通路上各支路增益之…」
