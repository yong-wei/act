...
## Subject
- Title: 表征系统内部变量及输入变量和输… (modeling_3d5825588c001e4faf3af7bc)
- Concept kind: theoretical_construct
- One-line definition (source of truth): 表征系统内部变量及输入变量和输出变量间转换关系的数学表达式，具有代数方程的形式。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 表征系统内部变量及输入变量和输出变量间转换关系的数学表达式，具有代数方程的形式。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「表征系统内部变量及输入…」
