...
## Subject
- Title: three way catalyst (three_way_catalyst)
- Concept kind: system_component
- One-line definition (source of truth): three way catalyst：A catalyst used for controlling HC, CO, and NOx emissions in conjunction with a closed-loop engine-control system.
- Core intuition: 在图谱邻接中可把握：前置 → exhaust gas sensor。
- Relation summary: 前置 → exhaust gas sensor
- Extended explanation (do not paste verbatim on image): A catalyst used for controlling HC, CO, and NOx emissions in conjunction with a closed-loop engine-…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「three way c…」、「three way ca」、「关联·exhaus…」
