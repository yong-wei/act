...
## Subject
- Title: 系统的输入-输出描述 (modeling_6ba251594a75698f1a092f8d)
- Concept kind: theoretical_construct
- One-line definition (source of truth): 系统的输入-输出描述，将系统看作黑箱，只反映外部变量间的因果关系。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → input
- Extended explanation (do not paste verbatim on image): 系统的输入-输出描述，将系统看作黑箱，只反映外部变量间的因果关系。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「系统的输入-输出描述」、「关联·input」
