...
- One-line definition (source of truth): 串级控制系统中的副调节器，传递函数为Gc2(s)。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 副回路 · 后续 → 副回路
- Extended explanation (do not paste verbatim on image): 串级控制系统中的副调节器，传递函数为Gc2(s)。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「副调节器」、「属于·副回路」、「包含组件·副回路」
