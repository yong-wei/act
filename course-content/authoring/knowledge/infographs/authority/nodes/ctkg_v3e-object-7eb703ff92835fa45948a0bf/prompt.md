...
- One-line definition (source of truth): 控制器中的脉冲元件，其输人和输出为脉冲序列，即时间上离散而幅值上连续的信号，称为离散模拟信号。
- Core intuition: 在图谱邻接中可把握：前置 → 采样控制系统 · 后续 → 脉冲序列。
- Relation summary: 前置 → 采样控制系统 · 后续 → 脉冲序列
- Extended explanation (do not paste verbatim on image): 控制器中的脉冲元件，其输人和输出为脉冲序列，即时间上离散而幅值上连续的信号，称为离散模拟信号。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「离散模拟信号」、「是一种·脉冲序列」、「包含组件·采样控制…」
