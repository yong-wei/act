...
## Subject
- Title: Steady-state error e ss (Steady-state error e_ss)
- Concept kind: theoretical_construct
- One-line definition (source of truth): Steady-state error e ss：The difference between the final value of the response and the desired input magnitude.
- Core intuition: 在图谱邻接中可把握：后续 → Closed-loop system transfer function。
- Relation summary: 后续 → Closed-loop system transfer function
- Extended explanation (do not paste verbatim on image): The difference between the final value of the response and the desired input magnitude.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Steady-stat…」、「Steady-state」、「关联·Closed…」
