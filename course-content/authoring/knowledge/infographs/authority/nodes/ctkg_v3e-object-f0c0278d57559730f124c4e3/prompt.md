...
- One-line definition (source of truth): D/A 转换的第一个过程：把离散数字信号转换为离散的模拟信号。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → D/A转换器
- Extended explanation (do not paste verbatim on image): D/A 转换的第一个过程：把离散数字信号转换为离散的模拟信号。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「解码过程」、「属于·D/A转换器」
