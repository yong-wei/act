...
## Subject
- Title: 脉冲传递函数 (modeling_f6fc65484fb95cf89b67364f)
- Concept kind: theoretical_construct
- One-line definition (source of truth): G(z)称为脉冲传递函数，由差分方程经z变换得到。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): G(z)称为脉冲传递函数，由差分方程经z变换得到。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「脉冲传递函数」
