...
## Subject
- Title: angle of departure (angle_of_departure)
- Concept kind: mathematical_object
- One-line definition (source of truth): angle of departure：The angle at which a locus leaves a complex pole in the s-plane.
- Core intuition: 在图谱邻接中可把握：后续 → root of the denominator。
- Relation summary: 后续 → root of the denominator
- Extended explanation (do not paste verbatim on image): The angle at which a locus leaves a complex pole in the s-plane.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「angle of de…」、「angle of dep」、「关联·root o…」
