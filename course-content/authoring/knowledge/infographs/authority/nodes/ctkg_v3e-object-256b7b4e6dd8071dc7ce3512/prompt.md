...
- One-line definition (source of truth): z变换是从拉氏变换直接引申出来的一种变换方法，它实际上是采样函数拉氏变换的变形，是研究线性离散系统的重要数学工具。
- Core intuition: 在图谱邻接中可把握：前置 → z变换算子 · 后续 → 级数求和法。
- Relation summary: 前置 → z变换算子 · 后续 → 级数求和法
- Extended explanation (do not paste verbatim on image): z变换是从拉氏变换直接引申出来的一种变换方法，它实际上是采样函数拉氏变换的变形，是研究线性离散系统的重要数学工具。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「z变换」、「是一种·z变换算子」、「包含组件·级数求和法」、「关联·z变换算子」
