...
- One-line definition (source of truth): 采用反演积分法求取z反变换，适用于E(z)为超越函数的情况，通过柯西留数定理计算积分值。
- Core intuition: 在图谱邻接中可把握：前置 → 部分分式法、幂级数法。
- Relation summary: 前置 → 部分分式法、幂级数法
- Extended explanation (do not paste verbatim on image): 采用反演积分法求取z反变换，适用于E(z)为超越函数的情况，通过柯西留数定理计算积分值。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「反演积分法」、「关联·部分分式法」、「关联·幂级数法」
