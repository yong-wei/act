...
## Subject
- Title: Estimator Error Poles
- Concept kind: unknown
- One-line definition (source of truth): Estimator Error Poles：The desired poles of the estimator error dynamics, specified as β₁, β₂, …, βₙ.
- Core intuition: 在图谱邻接中可把握：前置 → Estimator Gain S · 后续 → Estimator Error。
- Relation summary: 前置 → Estimator Gain S · 后续 → Estimator Error
- Extended explanation (do not paste verbatim on image): The desired poles of the estimator error dynamics, specified as β₁, β₂, …, βₙ.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Estimator E…」、「Estimator Er」、「包含组件·Esti…」、「应用于·Estim…」
