...
- One-line definition (source of truth): 多极双通道旋转变压器。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 绝对式轴角编码
- Extended explanation (do not paste verbatim on image): 多极双通道旋转变压器
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「多极双通道旋转变压器」、「包含组件·绝对式轴…」
