...
## Subject
- Title: 求解齐次状态方程的一种方法 (modeling_fc65ded76a1e17c5fddcbd66)
- Concept kind: analysis_method
- One-line definition (source of truth): 求解齐次状态方程的一种方法，对状态方程取拉普拉斯变换，得到 $\boldsymbol{X}(s)=(s \boldsymbol{I}-\boldsymbol{A})^{-1} \boldsymbol{x}(0)$，再取反变换得到 $\bo…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 求解齐次状态方程的一种方法，对状态方程取拉普拉斯变换，得到 $\boldsymbol{X}(s)=(s \boldsymbol{I}-\boldsymbol{A})^{-1} \boldsymbol…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「求解齐次状态方程的一种…」
