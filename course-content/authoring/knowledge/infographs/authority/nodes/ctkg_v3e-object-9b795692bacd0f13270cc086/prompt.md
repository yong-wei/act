...
- One-line definition (source of truth): 线性定常连续系统完全可控的充分必要条件是 rank[B AB ... A^{n-1}B] = n，其中 n 为矩阵 A 的维数。
- Core intuition: 在图谱邻接中可把握：后续 → 可控性判别阵、系统完全可控。
- Relation summary: 后续 → 可控性判别阵、系统完全可控
- Extended explanation (do not paste verbatim on image): 线性定常连续系统完全可控的充分必要条件是 rank[B AB ... A^{n-1}B] = n，其中 n 为矩阵 A 的维数。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「秩判据」、「包含组件·可控性判…」、「包含组件·系统完全…」
