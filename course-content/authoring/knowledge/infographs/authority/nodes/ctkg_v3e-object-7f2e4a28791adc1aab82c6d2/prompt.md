...
- One-line definition (source of truth): read-only memory (ROM)：read-only memory (ROM)
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → flash memory、programmable read-only memory (EPROM)、random-access memory (RAM)
- Extended explanation (do not paste verbatim on image): read-only memory (ROM)
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「read-only m…」、「read-only me」、「一种·flash」、「一种·progra…」、「关联·random…」
