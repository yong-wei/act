...
## Subject
- Title: 控制系统中包含有正反馈内回路的… (positive_feedback_system)
- Concept kind: system_kind
- One-line definition (source of truth): 控制系统中包含有正反馈内回路的系统。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 控制系统中包含有正反馈内回路的系统。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「控制系统中包含有正反馈…」、「positive fee」
