...
- One-line definition (source of truth): 具有比例-积分控制规律的控制器，称PI控制器，其输出信号u(t)同时成比例地反映输入信号e(t)及其积分。
- Core intuition: 在图谱邻接中可把握：前置 → PI控制器。
- Relation summary: 前置 → PI控制器
- Extended explanation (do not paste verbatim on image): 具有比例-积分控制规律的控制器，称PI控制器，其输出信号u(t)同时成比例地反映输入信号e(t)及其积分。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「比例-积分控制规律」、「关联·PI控制器」
