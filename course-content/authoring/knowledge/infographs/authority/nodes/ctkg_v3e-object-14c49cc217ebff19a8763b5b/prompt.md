...
- One-line definition (source of truth): z是在复数平面上定义的一个复变量，通常称为z变换算子。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → z变换
- Extended explanation (do not paste verbatim on image): z是在复数平面上定义的一个复变量，通常称为z变换算子。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「z变换算子」、「是一种·z变换」、「关联·z变换」
