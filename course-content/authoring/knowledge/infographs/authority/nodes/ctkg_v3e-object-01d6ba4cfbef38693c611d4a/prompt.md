...
- One-line definition (source of truth): argument principle：A contour map of a complex function will encircle the origin Z-P times, where Z is the number of zeros and P is the num…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A contour map of a complex function will encircle the origin Z-P times, where Z is the number of ze…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「argument pr…」、「argument pri」
