...
## Subject
- Title: point in block diagram (modeling_cba517bb72cccbe819db88e9)
- Concept kind: theoretical_construct
- One-line definition (source of truth): point in block diagram：A point in a block diagram where any number of signals may be added together, represented by a circle with the symbol Σ…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A point in a block diagram where any number of signals may be added together, represented by a circ…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「point in bl…」
