...
- One-line definition (source of truth): 具有比例-微分控制规律的控制器，称为PD控制器，其输出u(t)与输入e(t)的关系为u(t)=Kp e(t) ± Kp Td de(t)/dt。
- Core intuition: 在图谱邻接中可把握：前置 → PD控制器。
- Relation summary: 前置 → PD控制器
- Extended explanation (do not paste verbatim on image): 具有比例-微分控制规律的控制器，称为PD控制器，其输出u(t)与输入e(t)的关系为u(t)=Kp e(t) ± Kp Td de(t)/dt。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「比例-微分控制规律」、「包含组件·PD控制器」、「关联·PD控制器」
