...
- One-line definition (source of truth): 所谓自激振荡是指没有外界信号的作用时，系统内产生的具有固定振幅和频率的稳定周期运动，简称自振。
- Core intuition: 在图谱邻接中可把握：前置 → 描述函数法、非线性系统、继电特性。
- Relation summary: 前置 → 描述函数法、非线性系统、继电特性
- Extended explanation (do not paste verbatim on image): 所谓自激振荡是指没有外界信号的作用时，系统内产生的具有固定振幅和频率的稳定周期运动，简称自振。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「自激振荡」、「用于分析·描述函数法」、「关联·非线性系统」、「关联·继电特性」、「关联·正阻尼」
