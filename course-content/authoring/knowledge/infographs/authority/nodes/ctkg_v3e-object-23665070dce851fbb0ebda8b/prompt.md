...
## Subject
- Title: Auxiliary polynomial
- Concept kind: unknown
- One-line definition (source of truth): Auxiliary polynomial：The polynomial which immediately precedes the zero entry in the Routh array. The order of the auxiliary polynomial is a…
- Core intuition: 在图谱邻接中可把握：后续 → Routh array、Symmetrical root pairs。
- Relation summary: 后续 → Routh array、Symmetrical root pairs
- Extended explanation (do not paste verbatim on image): The polynomial which immediately precedes the zero entry in the Routh array. The order of the auxil…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Auxiliary p…」、「Auxiliary po」、「属于·Routh …」、「包含组件·Symm…」、「关联·Symmet…」
