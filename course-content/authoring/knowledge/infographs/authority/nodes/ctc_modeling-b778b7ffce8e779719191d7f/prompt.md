...
## Subject
- Title: construct that estimates the (modeling_b778b7ffce8e779719191d7f)
- Concept kind: theoretical_construct
- One-line definition (source of truth): construct that estimates the：A construct that estimates the state based on the sensed output.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A construct that estimates the state based on the sensed output.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「construct t…」
