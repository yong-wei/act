...
- One-line definition (source of truth): Marginally Stable System：A system where the characteristic equation has simple roots on the imaginary axis (jω-axis) with all other roots in the…
- Core intuition: 在图谱邻接中可把握：后续 → Stable System。
- Relation summary: 后续 → Stable System
- Extended explanation (do not paste verbatim on image): A system where the characteristic equation has simple roots on the imaginary axis (jω-axis) with al…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Marginally …」、「Marginally S」、「一种·Stable」
