...
## Subject
- Title: Two loops that share (modeling_a0578e35574b6bf2399d6a93)
- Concept kind: theoretical_construct
- One-line definition (source of truth): Two loops that share：Two loops that share one or more common nodes.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): Two loops that share one or more common nodes.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Two loops t…」
