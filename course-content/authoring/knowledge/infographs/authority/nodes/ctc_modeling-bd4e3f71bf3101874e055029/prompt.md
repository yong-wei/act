...
## Subject
- Title: system in which one (modeling_bd4e3f71bf3101874e055029)
- Concept kind: theoretical_construct
- One-line definition (source of truth): system in which one：A system in which one or more parameters may vary as a function of time.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A system in which one or more parameters may vary as a function of time.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「system in w…」
