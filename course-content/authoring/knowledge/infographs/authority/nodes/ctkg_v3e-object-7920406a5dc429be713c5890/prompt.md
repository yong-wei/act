...
## Subject
- Title: Feedback compensation (as a scheme)
- Concept kind: unknown
- One-line definition (source of truth): Feedback compensation (as a scheme)：A compensation scheme where the compensator is placed in the feedback path of a single-loop feedback control system.
- Core intuition: 在图谱邻接中可把握：后续 → Feedback Compensation。
- Relation summary: 后续 → Feedback Compensation
- Extended explanation (do not paste verbatim on image): A compensation scheme where the compensator is placed in the feedback path of a single-loop feedbac…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Feedback co…」、「一种·Feedba…」
