...
- One-line definition (source of truth): 记 ω_min 为最小交接频率，称 ω<ω_min 的频率范围为低频段。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → 对数幅频渐近特性曲线
- Extended explanation (do not paste verbatim on image): 记 ω_min 为最小交接频率，称 ω<ω_min 的频率范围为低频段。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「低频段」、「应用于·对数幅频渐…」
