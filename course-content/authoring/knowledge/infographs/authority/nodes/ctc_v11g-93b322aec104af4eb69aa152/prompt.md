...
## Subject
- Title: pid tuning (pid_tuning)
- Concept kind: design_method
- One-line definition (source of truth): pid tuning：The process of determining acceptable values of the PID gains.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The process of determining acceptable values of the PID gains.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「pid tuning」
