...
- One-line definition (source of truth): decade：An interval of two frequencies with a ratio equal to 10 is called a decade, so that the range of frequencies from ω1 to…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): An interval of two frequencies with a ratio equal to 10 is called a decade, so that the range of fr…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「decade」
