...
## Subject
- Title: 一个由反馈闭合的整体 (modeling_8aa475ed6514adc11f9b5c8d)
- Concept kind: theoretical_construct
- One-line definition (source of truth): 由一些相互制约的部分构成的整体，可能是一个由反馈闭合的整体，也可能是某一控制装置或被控对象。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 由一些相互制约的部分构成的整体，可能是一个由反馈闭合的整体，也可能是某一控制装置或被控对象。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「一个由反馈闭合的整体」
