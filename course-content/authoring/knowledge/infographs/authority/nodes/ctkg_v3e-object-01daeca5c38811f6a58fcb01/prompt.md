...
## Subject
- Title: Equivalent gain  (Equivalent gain analysis using root locus)
- Concept kind: unknown
- One-line definition (source of truth): Equivalent gain ：A technique to study the stability of systems with memoryless nonlinearities by replacing the nonlinearity with an equi…
- Core intuition: 在图谱邻接中可把握：后续 → equivalent gain、memoryless nonlinearity、Root-Locus Techn。
- Relation summary: 后续 → equivalent gain、memoryless nonlinearity、Root-Locus Techn
- Extended explanation (do not paste verbatim on image): A technique to study the stability of systems with memoryless nonlinearities by replacing the nonli…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Equivalent …」、「包含组件·equi…」、「用于分析·memo…」、「关联·Root-L…」
