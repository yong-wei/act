...
## Subject
- Title: 当闭环两个实数极点重合时 (critically_damped_system)
- Concept kind: system_kind
- One-line definition (source of truth): 当闭环两个实数极点重合时，系统为临界阻尼系统，单位阶跃响应仍为非周期过程，但响应速度较快。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 当闭环两个实数极点重合时，系统为临界阻尼系统，单位阶跃响应仍为非周期过程，但响应速度较快。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「当闭环两个实数极点重合时」、「critically d」
