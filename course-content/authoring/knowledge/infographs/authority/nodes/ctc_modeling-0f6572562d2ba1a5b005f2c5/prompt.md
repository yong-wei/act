...
## Subject
- Title: pole zero gain form (pole_zero_gain_form)
- Concept kind: representation_kind
- One-line definition (source of truth): pole zero gain form：v3T accepted candidate for pole zero gain form.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): v3T accepted candidate for pole zero gain form.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「pole zero g…」、「pole zero ga」
