...
## Subject
- Title: sample and hold device
- Concept kind: unknown
- One-line definition (source of truth): sample and hold device：sample and hold device
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → D/A converter · 后续 → D/A converter
- Extended explanation (do not paste verbatim on image): sample and hold device
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「sample and …」、「sample and h」、「一种·D/A」、「有表示·D/A c…」
