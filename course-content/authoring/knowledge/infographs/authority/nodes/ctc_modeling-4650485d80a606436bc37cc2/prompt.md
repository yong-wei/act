...
## Subject
- Title: 系统的输入和输出变量 (modeling_4650485d80a606436bc37cc2)
- Concept kind: theoretical_construct
- One-line definition (source of truth): 系统的输入和输出变量。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 系统的输入和输出变量。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「系统的输入和输出变量」
