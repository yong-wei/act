...
## Subject
- Title: 当 $b_n=0$ 时 (modeling_c800e57165a6a2552cbac118)
- Concept kind: analysis_method
- One-line definition (source of truth): 当 $b_n=0$ 时，通过选择状态变量 $x_n = y$, $x_i = \dot{x}_{i+1} + a_i y - b_i u$ 来建立状态空间表达式的方法。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 当 $b_n=0$ 时，通过选择状态变量 $x_n = y$, $x_i = \dot{x}_{i+1} + a_i y - b_i u$ 来建立状态空间表达式的方法。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「当 $b_n=0$ 时」
