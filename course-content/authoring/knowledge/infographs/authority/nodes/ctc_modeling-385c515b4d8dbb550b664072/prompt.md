...
## Subject
- Title: Input and output points (modeling_385c515b4d8dbb550b664072)
- Concept kind: theoretical_construct
- One-line definition (source of truth): Input and output points：Input and output points or junctions in a signal-flow graph.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): Input and output points or junctions in a signal-flow graph.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Input and o…」
