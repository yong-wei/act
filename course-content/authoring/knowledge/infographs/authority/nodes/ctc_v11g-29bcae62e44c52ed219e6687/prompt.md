...
## Subject
- Title: logarithmic sensitivity (logarithmic_sensitivity)
- Concept kind: performance_metric
- One-line definition (source of truth): logarithmic sensitivity：A measure of the sensitivity of the system performance to specific parameter changes, given by S_K^T(s) = (∂T(s)/T(s))…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A measure of the sensitivity of the system performance to specific parameter changes, given by S_K^…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「logarithmic…」、「logarithmic 」
