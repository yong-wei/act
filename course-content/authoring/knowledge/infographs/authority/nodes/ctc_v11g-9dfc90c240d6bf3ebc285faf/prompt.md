...
## Subject
- Title: frequency of oscillation (frequency_of_oscillation)
- Concept kind: performance_metric
- One-line definition (source of truth): frequency of oscillation：The frequency at which the system oscillates at the borderline stability gain.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The frequency at which the system oscillates at the borderline stability gain.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「frequency o…」、「frequency of」
