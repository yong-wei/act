...
- One-line definition (source of truth): 当 t → ∞ 时，起始于极限环内（外）部的相轨迹卷向极限环，而起始于极限环外（内）部的相轨迹卷离极限环，则这种极限环称为半稳定的极限环。
- Core intuition: 在图谱邻接中可把握：后续 → 极限环。
- Relation summary: 后续 → 极限环
- Extended explanation (do not paste verbatim on image): 当 t → ∞ 时，起始于极限环内（外）部的相轨迹卷向极限环，而起始于极限环外（内）部的相轨迹卷离极限环，则这种极限环称为半稳定的极限环。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「半稳定极限环」、「是一种·极限环」
