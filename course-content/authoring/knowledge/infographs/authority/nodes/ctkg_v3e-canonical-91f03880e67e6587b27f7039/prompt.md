...
- One-line definition (source of truth): 在位置控制系统中，K = K_1 / F，称为开环增益。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 在位置控制系统中，K = K_1 / F，称为开环增益。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「开环增益」
