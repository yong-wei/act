...
## Subject
- Title: Amplitude quantization error
- Concept kind: unknown
- One-line definition (source of truth): Amplitude quantization error：The error introduced when the converted signal m(kT) is stored in digital logic with a finite number of binary digits.
- Core intuition: 在图谱邻接中可把握：后续 → Quantization error、Analog-to-digital converter。
- Relation summary: 后续 → Quantization error、Analog-to-digital converter
- Extended explanation (do not paste verbatim on image): The error introduced when the converted signal m(kT) is stored in digital logic with a finite numbe…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Amplitude q…」、「一种·Quanti…」、「关联·Analog…」
