...
## Subject
- Title: 描述离散时间系统状态随时间演化… (modeling_c0913d0807ab432c65d73f9d)
- Concept kind: theoretical_construct
- One-line definition (source of truth): 描述离散时间系统状态随时间演化的方程，形式为 x(k+1) = G x(k) + h u(k) 或 x(k+1) = G x(k) + H u(k)。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 描述离散时间系统状态随时间演化的方程，形式为 x(k+1) = G x(k) + h u(k) 或 x(k+1) = G x(k) + H u(k)。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「描述离散时间系统状态随…」
