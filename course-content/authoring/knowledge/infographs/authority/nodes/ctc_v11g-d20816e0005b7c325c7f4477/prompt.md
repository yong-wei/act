...
## Subject
- Title: 等效开环传递函数 (equivalent_open_loop_transfer_function)
- Concept kind: representation_kind
- One-line definition (source of truth): 根据闭环特征方程等效变换得到的开环传递函数，用于绘制参数根轨迹。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 在绘制参数根轨迹之前引入的概念、参数根轨迹 · 后续 → The transfer function from
- Extended explanation (do not paste verbatim on image): 根据闭环特征方程等效变换得到的开环传递函数，用于绘制参数根轨迹。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「等效开环传递函数」、「一种·The」、「关联·在绘制参数根…」、「关联·参数根轨迹」
