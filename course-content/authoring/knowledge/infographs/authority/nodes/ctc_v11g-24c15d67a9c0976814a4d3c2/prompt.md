...
## Subject
- Title: exhaust gas sensor (exhaust_gas_sensor)
- Concept kind: system_component
- One-line definition (source of truth): exhaust gas sensor：A sensor that gives an indication of a rich or lean exhaust and compares it to a reference.
- Core intuition: 在图谱邻接中可把握：后续 → three way catalyst。
- Relation summary: 后续 → three way catalyst
- Extended explanation (do not paste verbatim on image): A sensor that gives an indication of a rich or lean exhaust and compares it to a reference.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「exhaust gas…」、「exhaust gas 」、「关联·three …」
