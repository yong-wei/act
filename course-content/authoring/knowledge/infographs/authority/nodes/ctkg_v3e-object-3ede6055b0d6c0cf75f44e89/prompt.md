...
## Subject
- Title: Natural unforced response
- Concept kind: unknown
- One-line definition (source of truth): Natural unforced response：The part of the system response due to its own dynamics (poles), which decays to zero if all poles have negative real p…
- Core intuition: 在图谱邻接中可把握：后续 → Sinusoidal steady-state response。
- Relation summary: 后续 → Sinusoidal steady-state response
- Extended explanation (do not paste verbatim on image): The part of the system response due to its own dynamics (poles), which decays to zero if all poles…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Natural unf…」、「关联·Sinuso…」
