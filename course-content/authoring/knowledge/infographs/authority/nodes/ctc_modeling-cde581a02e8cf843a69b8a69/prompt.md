...
- One-line definition (source of truth): input：v3T accepted candidate for input.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 系统的输入-输出描述
- Extended explanation (do not paste verbatim on image): v3T accepted candidate for input.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「input」、「关联·系统的输入-…」
