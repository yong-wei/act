...
## Subject
- Title: 由初始条件产生的输出分量 (modeling_caf967347827c5f93ed59795)
- Concept kind: theoretical_construct
- One-line definition (source of truth): 由初始条件产生的输出分量，与输入电压无关。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 由初始条件产生的输出分量，与输入电压无关。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「由初始条件产生的输出分量」
