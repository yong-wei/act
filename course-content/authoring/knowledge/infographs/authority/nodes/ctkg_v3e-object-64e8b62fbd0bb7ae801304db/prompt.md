...
## Subject
- Title: 特征方程 (Characteristic equation)
- Concept kind: unknown
- One-line definition (source of truth): 特征方程：Characteristic equation。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → Routh-Hurwitz stability criterion method、Eigenvalue、Routh-Hurwitz stability criterion
- Extended explanation (do not paste verbatim on image): Characteristic equation
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「特征方程」、「Characterist」、「用于分析·Rout…」、「关联·Eigenv…」、「关联·Routh-…」
