...
## Subject
- Title: The path of the (modeling_1d8759ac1188e2de38f75ed3)
- Concept kind: theoretical_construct
- One-line definition (source of truth): The path of the：The path of the state vector in state-space as a solution of the differential equations.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The path of the state vector in state-space as a solution of the differential equations.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「The path of…」
