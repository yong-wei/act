...
## Subject
- Title: Zero assignment  (Zero assignment using Truxal's formula for lag compensation)
- Concept kind: unknown
- One-line definition (source of truth): Zero assignment ：A method to increase the velocity constant K_v by placing a zero near a slow estimator pole, using Truxal's formula to…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A method to increase the velocity constant K_v by placing a zero near a slow estimator pole, using…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Zero assign…」
