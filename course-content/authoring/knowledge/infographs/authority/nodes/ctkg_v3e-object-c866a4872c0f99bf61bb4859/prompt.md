...
- One-line definition (source of truth): 当 t → ∞ 时，起始于极限环内部或外部的相轨迹均卷离极限环，则该极限环称为不稳定的极限环。
- Core intuition: 极限环内部的相轨迹收敛至环内的奇点，说明极限环的内部是稳定区域。
- Relation summary: 前置 → 双极限环 · 后续 → 极限环
- Extended explanation (do not paste verbatim on image): 当 t → ∞ 时，起始于极限环内部或外部的相轨迹均卷离极限环，则该极限环称为不稳定的极限环。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「不稳定极限环」、「是一种·极限环」、「包含组件·双极限环」
