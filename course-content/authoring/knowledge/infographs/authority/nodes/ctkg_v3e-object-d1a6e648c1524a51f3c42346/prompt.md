...
- One-line definition (source of truth): 正弦输入信号作用下，非线性环节的稳态输出中一次谐波分量和输入信号的复数比，用 N(A) 表示。
- Core intuition: 在图谱邻接中可把握：前置 → 描述函数法、谐波线性化、负倒描述函数。
- Relation summary: 前置 → 描述函数法、谐波线性化、负倒描述函数
- Extended explanation (do not paste verbatim on image): 正弦输入信号作用下，非线性环节的稳态输出中一次谐波分量和输入信号的复数比，用 N(A) 表示。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「描述函数」、「包含组件·描述函数法」、「用于分析·谐波线性化」、「派生自·负倒描述函数」
