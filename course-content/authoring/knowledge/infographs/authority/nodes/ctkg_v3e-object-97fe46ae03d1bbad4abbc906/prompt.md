...
- One-line definition (source of truth): 对于式（9－76）所示线性时变系统，如果状态空间中的所有非零状态都是在 t0(t0∈Tt) 时刻可控的，则称系统在时刻 t0 是完全可控或一致可控的，简称系统在时刻 t0可控。
- Core intuition: 在图谱邻接中可把握：前置 → 状态可控。
- Relation summary: 前置 → 状态可控
- Extended explanation (do not paste verbatim on image): 对于式（9－76）所示线性时变系统，如果状态空间中的所有非零状态都是在 t0(t0∈Tt) 时刻可控的，则称系统在时刻 t0 是完全可控或一致可控的，简称系统在时刻 t0可控。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「系统可控」、「是一种·状态可控」
