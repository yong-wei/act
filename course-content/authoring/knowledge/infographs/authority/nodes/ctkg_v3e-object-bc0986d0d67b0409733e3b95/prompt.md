...
- One-line definition (source of truth): 信号复现滤波器。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 信号复现过程 · 后续 → 保持器
- Extended explanation (do not paste verbatim on image): 信号复现滤波器
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「信号复现滤波器」、「是一种·保持器」、「包含组件·信号复现…」
