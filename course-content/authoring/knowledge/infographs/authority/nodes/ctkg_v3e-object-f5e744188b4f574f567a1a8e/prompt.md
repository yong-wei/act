...
- One-line definition (source of truth): 矩阵指数函数 e^{A t}，记为 \Phi(t)，用于描述线性定常系统状态从初始时刻到任意时刻的转移。
- Core intuition: 在图谱邻接中可把握：后续 → 矩阵指数函数。
- Relation summary: 后续 → 矩阵指数函数
- Extended explanation (do not paste verbatim on image): 矩阵指数函数 e^{A t}，记为 \Phi(t)，用于描述线性定常系统状态从初始时刻到任意时刻的转移
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「状态转移矩阵」、「是一种·矩阵指数函数」
