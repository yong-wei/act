...
- One-line definition (source of truth): 前馈校正又称顺馈校正，是在系统主反馈回路之外采用的校正方式。
- Core intuition: 前馈校正装置接在系统给定值（或指令、参考输入信号）之后及主反馈作用点之前的前向通道上。
- Relation summary: 前置 → 前置滤波器 · 后续 → 校正装置、前置滤波器
- Extended explanation (do not paste verbatim on image): 前馈校正又称顺馈校正，是在系统主反馈回路之外采用的校正方式。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「前馈校正」、「是一种·校正装置」、「是一种·前置滤波器」、「关联·前置滤波器」
