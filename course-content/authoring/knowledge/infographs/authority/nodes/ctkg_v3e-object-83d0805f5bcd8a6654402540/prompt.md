...
## Subject
- Title: Cascade compensation
- Concept kind: unknown
- One-line definition (source of truth): Cascade compensation：A compensation scheme where the compensator is placed in the feedforward path of a single-loop feedback control system.
- Core intuition: 在图谱邻接中可把握：前置 → Cascade Compensator。
- Relation summary: 前置 → Cascade Compensator
- Extended explanation (do not paste verbatim on image): A compensation scheme where the compensator is placed in the feedforward path of a single-loop feed…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Cascade com…」、「Cascade comp」、「关联·Cascad…」
