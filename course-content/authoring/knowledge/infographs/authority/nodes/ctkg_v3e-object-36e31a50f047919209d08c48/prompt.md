...
## Subject
- Title: Output Compensation
- Concept kind: unknown
- One-line definition (source of truth): Output Compensation：A compensation scheme where the compensator is placed at the output or load.
- Core intuition: 在图谱邻接中可把握：后续 → Output compensation (as a scheme)、Compensation。
- Relation summary: 后续 → Output compensation (as a scheme)、Compensation
- Extended explanation (do not paste verbatim on image): A compensation scheme where the compensator is placed at the output or load.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Output Comp…」、「Output Compe」、「一种·Output」、「一种·Compen…」
