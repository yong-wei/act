...
## Subject
- Title: 过阻尼二阶系统动态性能指标工程…
- Concept kind: analysis_method
- One-line definition (source of truth): 利用数值解法求出不同 ζ 值下的无因次时间制成曲线以供查用，或者利用曲线拟合法给出近似计算公式。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 利用数值解法求出不同 ζ 值下的无因次时间制成曲线以供查用，或者利用曲线拟合法给出近似计算公式。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「过阻尼二阶系统动态性能…」
