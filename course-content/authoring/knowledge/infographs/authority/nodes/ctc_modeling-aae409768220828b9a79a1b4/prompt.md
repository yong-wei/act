...
## Subject
- Title: 对于输入量中不含导数项的单输入… (modeling_aae409768220828b9a79a1b4)
- Concept kind: analysis_method
- One-line definition (source of truth): 对于输入量中不含导数项的单输入-单输出线性定常连续系统，选取n个状态变量为 x1=y, x2=dy/dt, ..., xn=y^(n-1)。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 对于输入量中不含导数项的单输入-单输出线性定常连续系统，选取n个状态变量为 x1=y, x2=dy/dt, ..., xn=y^(n-1)。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「对于输入量中不含导数项…」
