...
## Subject
- Title: root contours (root_contours)
- Concept kind: representation_kind
- One-line definition (source of truth): root contours：A family of root loci for two parameters, illustrating the effect of both parameters on the roots of the characteristic…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A family of root loci for two parameters, illustrating the effect of both parameters on the roots o…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「root contou…」、「root contour」
