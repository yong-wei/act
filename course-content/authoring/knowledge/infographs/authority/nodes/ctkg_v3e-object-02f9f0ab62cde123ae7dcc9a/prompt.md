...
## Subject
- Title: Left Companion Matrix
- Concept kind: unknown
- One-line definition (source of truth): Left Companion Matrix：The matrix Aₒ in observer canonical form, where the coefficients of the characteristic equation appear on the left side.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The matrix Aₒ in observer canonical form, where the coefficients of the characteristic equation app…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Left Compan…」、「Left Compani」
