...
## Subject
- Title: Phase-lead compensator
- Concept kind: unknown
- One-line definition (source of truth): Phase-lead compensator：A compensator that adds a phase-lead angle to provide an adequate phase margin, used to alter the performance of a cont…
- Core intuition: 在图谱邻接中可把握：前置 → Phase-lead design using the root locus。
- Relation summary: 前置 → Phase-lead design using the root locus
- Extended explanation (do not paste verbatim on image): A compensator that adds a phase-lead angle to provide an adequate phase margin, used to alter the p…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Phase-lead …」、「Phase-lead c」、「应用于·Phase…」
