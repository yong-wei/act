...
## Subject
- Title: 根轨迹法 (Root locus method)
- Concept kind: unknown
- One-line definition (source of truth): 根轨迹法：A method for designing compensators by altering the root locus of the closed-loop system to achieve desired root locati…
- Core intuition: 在图谱邻接中可把握：后续 → Compensator、根轨迹。
- Relation summary: 后续 → Compensator、根轨迹
- Extended explanation (do not paste verbatim on image): A method for designing compensators by altering the root locus of the closed-loop system to achieve…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「根轨迹法」、「Root locus m」、「应用于·Compe…」、「应用于·根轨迹」、「关联·Compen…」、「关联·根轨迹」
