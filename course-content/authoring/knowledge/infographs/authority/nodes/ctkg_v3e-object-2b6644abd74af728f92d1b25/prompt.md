...
- One-line definition (source of truth): 当相轨迹经过该等倾线上任一点时，其切线的斜率都相等，均为 α。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 等倾线法 · 后续 → 等倾线法、相平面
- Extended explanation (do not paste verbatim on image): 当相轨迹经过该等倾线上任一点时，其切线的斜率都相等，均为 α。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「等倾线」、「属于·等倾线法」、「属于·相平面」、「包含组件·等倾线法」
