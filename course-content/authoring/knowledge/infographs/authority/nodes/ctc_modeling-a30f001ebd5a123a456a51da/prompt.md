...
## Subject
- Title: 网络的单位阶跃响应 (modeling_a30f001ebd5a123a456a51da)
- Concept kind: theoretical_construct
- One-line definition (source of truth): 零初始条件响应和零输入响应统称为网络的单位阶跃响应。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 零初始条件响应和零输入响应统称为网络的单位阶跃响应。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「网络的单位阶跃响应」
