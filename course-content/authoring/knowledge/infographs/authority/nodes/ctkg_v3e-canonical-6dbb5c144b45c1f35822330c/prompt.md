...
- One-line definition (source of truth): 比例控制时的二阶系统则称为无零点的二阶系统。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 比例控制时的二阶系统则称为无零点的二阶系统。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「无零点的二阶系统」
