...
## Subject
- Title: Zero-Order Hold (ZOH) Method
- Concept kind: unknown
- One-line definition (source of truth): Zero-Order Hold (ZOH) Method：A digitization method that assumes the input to the controller remains constant throughout the sample period. The discr…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A digitization method that assumes the input to the controller remains constant throughout the samp…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Zero-Order …」
