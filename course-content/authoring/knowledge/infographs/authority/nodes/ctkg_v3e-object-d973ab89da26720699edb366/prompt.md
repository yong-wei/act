...
- One-line definition (source of truth): 量化单位。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 量化单位
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「量化单位」
