...
## Subject
- Title: relay with dead zone
- Concept kind: unknown
- One-line definition (source of truth): relay with dead zone：relay with dead zone
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → memoryless nonlinearity
- Extended explanation (do not paste verbatim on image): relay with dead zone
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「relay with …」、「relay with d」、「一种·memory…」
