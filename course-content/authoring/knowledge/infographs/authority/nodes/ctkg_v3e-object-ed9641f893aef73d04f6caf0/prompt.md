...
## Subject
- Title: Maximum magnitud (Maximum magnitude of closed-loop frequency response M_pω)
- Concept kind: unknown
- One-line definition (source of truth): Maximum magnitud：M_pω is the maximum magnitude of the closed-loop frequency response.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): M_pω is the maximum magnitude of the closed-loop frequency response.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Maximum mag…」
