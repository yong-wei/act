...
- One-line definition (source of truth): Eigenvalue：A scalar p_i such that A t_i = p_i t_i
- Core intuition: 在图谱邻接中可把握：前置 → Computing poles  · 后续 → Eigenvector、特征方程、Computing poles 。
- Relation summary: 前置 → Computing poles  · 后续 → Eigenvector、特征方程、Computing poles
- Extended explanation (do not paste verbatim on image): A scalar p_i such that A t_i = p_i t_i
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Eigenvalue」、「用于分析·Comp…」、「关联·Eigenv…」、「关联·特征方程」、「关联·Comput…」
