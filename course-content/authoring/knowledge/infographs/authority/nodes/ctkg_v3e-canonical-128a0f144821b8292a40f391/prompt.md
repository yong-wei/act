...
- One-line definition (source of truth): 在复合控制系统中，前馈控制与反馈控制相结合，用于抑制可量测扰动，包括低频强扰动。
- Core intuition: 在图谱邻接中可把握：后续 → 前馈通路。
- Relation summary: 后续 → 前馈通路
- Extended explanation (do not paste verbatim on image): 在复合控制系统中，前馈控制与反馈控制相结合，用于抑制可量测扰动，包括低频强扰动。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「前馈控制」、「包含组件·前馈通路」
