...
- One-line definition (source of truth): linearization：v3T accepted candidate for linearization.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): v3T accepted candidate for linearization.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「linearizati…」、「linearizatio」
