...
- One-line definition (source of truth): D/A 转换器是把离散的数字信号转换为连续模拟信号的装置。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 复现过程、解码过程、模拟信号
- Extended explanation (do not paste verbatim on image): D/A 转换器是把离散的数字信号转换为连续模拟信号的装置。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「D/A转换器」、「属于·复现过程」、「属于·解码过程」、「关联·模拟信号」、「关联·数字信号」
