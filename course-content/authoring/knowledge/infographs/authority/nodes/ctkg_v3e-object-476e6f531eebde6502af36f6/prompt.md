...
- One-line definition (source of truth): 通过w变换将z平面上的特征方程转换为w平面上的特征方程，然后应用劳斯表判断离散系统的稳定性。
- Core intuition: 在图谱邻接中可把握：后续 → w变换（双线性变换）。
- Relation summary: 后续 → w变换（双线性变换）
- Extended explanation (do not paste verbatim on image): 通过w变换将z平面上的特征方程转换为w平面上的特征方程，然后应用劳斯表判断离散系统的稳定性。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「w域中的劳斯稳定判据」、「派生自·w变换（双…」
