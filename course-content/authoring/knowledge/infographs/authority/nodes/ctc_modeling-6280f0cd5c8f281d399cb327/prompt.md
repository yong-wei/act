...
## Subject
- Title: 系统在时间域中的行为或运动信息… (modeling_6280f0cd5c8f281d399cb327)
- Concept kind: theoretical_construct
- One-line definition (source of truth): 系统在时间域中的行为或运动信息的集合。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 系统在时间域中的行为或运动信息的集合。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「系统在时间域中的行为或…」
