...
## Subject
- Title: An integral transform that (modeling_cf35c2ab1653eb95eecdb745)
- Concept kind: theoretical_construct
- One-line definition (source of truth): An integral transform that：An integral transform that converts a time-domain function f(t) into a complex frequency-domain function F(s), defined…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): An integral transform that converts a time-domain function f(t) into a complex frequency-domain fun…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「An integral…」
