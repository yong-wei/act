...
- One-line definition (source of truth): 采样开关。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 采样控制系统、线性采样系统
- Extended explanation (do not paste verbatim on image): 采样开关
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「采样开关」、「包含组件·采样控制…」、「包含组件·线性采样…」
