...
## Subject
- Title: 利用MATLAB进行高阶系统建…
- Concept kind: analysis_method
- One-line definition (source of truth): 利用MATLAB进行高阶系统建模和单位阶跃响应计算。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 可求出单位阶跃响应
- Extended explanation (do not paste verbatim on image): 利用MATLAB进行高阶系统建模和单位阶跃响应计算
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「利用MATLAB进行高…」、「包含组件·可求出单…」
