...
- One-line definition (source of truth): Internal model design：A technique for tracking reference inputs with guaranteed steady-state tracking errors.
- Core intuition: 在图谱邻接中可把握：后续 → Steady-state tracking error、Asymptotic tracking、Zero steady-state error。
- Relation summary: 后续 → Steady-state tracking error、Asymptotic tracking、Zero steady-state error
- Extended explanation (do not paste verbatim on image): A technique for tracking reference inputs with guaranteed steady-state tracking errors.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Internal mo…」、「Internal mod」、「用于分析·Stea…」、「关联·Asympt…」、「关联·Zero s…」
