...
## Subject
- Title: Estimator Gain S (Estimator Gain Selection via Pole Placement)
- Concept kind: unknown
- One-line definition (source of truth): Estimator Gain S：Specify desired estimator error poles, form the desired characteristic equation, and solve for L by comparing coefficie…
- Core intuition: 在图谱邻接中可把握：前置 → Estimator Gain S · 后续 → Estimator Gain S、Estimator Error Poles。
- Relation summary: 前置 → Estimator Gain S · 后续 → Estimator Gain S、Estimator Error Poles
- Extended explanation (do not paste verbatim on image): Specify desired estimator error poles, form the desired characteristic equation, and solve for L by…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Estimator G…」、「包含组件·Esti…」、「用于分析·Esti…」、「应用于·Estim…」
