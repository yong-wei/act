...
## Subject
- Title: 在绘制参数根轨迹之前引入的概念 (equivalent_unity_feedback_system)
- Concept kind: system_kind
- One-line definition (source of truth): 在绘制参数根轨迹之前引入的概念，使得常规根轨迹的所有绘制法则均适用于参数根轨迹的绘制。
- Core intuition: 在图谱邻接中可把握：后续 → 参数根轨迹、等效开环传递函数。
- Relation summary: 后续 → 参数根轨迹、等效开环传递函数
- Extended explanation (do not paste verbatim on image): 在绘制参数根轨迹之前引入的概念，使得常规根轨迹的所有绘制法则均适用于参数根轨迹的绘制。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「在绘制参数根轨迹之前引…」、「关联·参数根轨迹」、「关联·等效开环传递…」
