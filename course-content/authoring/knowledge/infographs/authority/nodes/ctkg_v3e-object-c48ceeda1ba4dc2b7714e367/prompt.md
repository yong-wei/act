...
## Subject
- Title: Discrete design
- Concept kind: unknown
- One-line definition (source of truth): Discrete design：A technique for finding difference equations directly without designing D_c(s) first.
- Core intuition: 在图谱邻接中可把握：前置 → Difference equations、Discrete equivalent · 后续 → Difference equations。
- Relation summary: 前置 → Difference equations、Discrete equivalent · 后续 → Difference equations
- Extended explanation (do not paste verbatim on image): A technique for finding difference equations directly without designing D_c(s) first.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Discrete de…」、「Discrete des」、「属于·Differ…」、「包含组件·Diff…」、「包含组件·Disc…」、「派生自·Discr…」
