...
## Subject
- Title: 格拉姆矩阵判据 (Gramia…
- Concept kind: unknown
- One-line definition (source of truth): 格拉姆矩阵判据 (Gramian matrix criterion for controllability)。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 格拉姆矩阵判据 (Gramian matrix criterion for controllability)
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「格拉姆矩阵判据 (Gr…」
