...
## Subject
- Title: variable that is measured (modeling_c2650bdf6df39c39c8ac378c)
- Concept kind: theoretical_construct
- One-line definition (source of truth): variable that is measured：A variable that is measured at a single point in a system element, such as torque, force, current, or heat flow rate, a…
- Core intuition: 在图谱邻接中可把握：前置 → variable that is measured。
- Relation summary: 前置 → variable that is measured
- Extended explanation (do not paste verbatim on image): A variable that is measured at a single point in a system element, such as torque, force, current,…
