...
## Subject
- Title: 某一输出量仅受某一输入量控制的… (modeling_698adafe19696afc3498a321)
- Concept kind: theoretical_construct
- One-line definition (source of truth): 某一输出量仅受某一输入量控制的系统，其传递矩阵为对角阵。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 某一输出量仅受某一输入量控制的系统，其传递矩阵为对角阵。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「某一输出量仅受某一输入…」
