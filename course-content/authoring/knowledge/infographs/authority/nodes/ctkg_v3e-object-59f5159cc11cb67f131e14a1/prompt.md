...
## Subject
- Title: Transformation t (Transformation to modal form using eigenvectors)
- Concept kind: unknown
- One-line definition (source of truth): Transformation t：The transformation matrix T is composed of the eigenvectors of A. The transformed system matrices are A_m = T^{-1} A T,…
- Core intuition: 在图谱邻接中可把握：后续 → Eigenvector、Modal form。
- Relation summary: 后续 → Eigenvector、Modal form
- Extended explanation (do not paste verbatim on image): The transformation matrix T is composed of the eigenvectors of A. The transformed system matrices a…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Transformat…」、「关联·Eigenv…」、「关联·Modal …」
