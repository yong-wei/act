...
## Subject
- Title: 一对电位器组成误差检测器时 (modeling_b5aa1e9d744b24db0fdc4884)
- Concept kind: theoretical_construct
- One-line definition (source of truth): 一对电位器组成误差检测器时，两个电位器电刷角位移之差，记为Δθ(t)。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 一对电位器组成误差检测器时，两个电位器电刷角位移之差，记为Δθ(t)。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「一对电位器组成误差检测…」
