...
- One-line definition (source of truth): State transition matrix：A matrix Phi(t) that represents the response of the system
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A matrix Phi(t) that represents the response of the system
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「State trans…」、「State transi」
