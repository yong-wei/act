...
## Subject
- Title: Zero-Memory Nonlinear Element
- Concept kind: unknown
- One-line definition (source of truth): Zero-Memory Nonlinear Element：A nonlinear element whose output depends only on the instantaneous input, without any memory or dynamics.
- Core intuition: 在图谱邻接中可把握：前置 → Root-Locus Techn。
- Relation summary: 前置 → Root-Locus Techn
- Extended explanation (do not paste verbatim on image): A nonlinear element whose output depends only on the instantaneous input, without any memory or dyn…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Zero-Memory…」、「应用于·Root-…」
