...
- One-line definition (source of truth): 按照平衡状态的定义，在无外作用且系统输出的各阶导数等于零时，系统处于平衡状态。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 平衡点附近增量线性化方法
- Extended explanation (do not paste verbatim on image): 按照平衡状态的定义，在无外作用且系统输出的各阶导数等于零时，系统处于平衡状态。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「平衡状态」、「用于分析·平衡点附…」
