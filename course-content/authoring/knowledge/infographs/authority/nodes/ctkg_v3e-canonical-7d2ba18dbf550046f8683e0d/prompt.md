...
- One-line definition (source of truth): Transmission zeros：Transmission zeros
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → Transmission zero、Using tzero to c
- Extended explanation (do not paste verbatim on image): Transmission zeros
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Transmissio…」、「Transmission」、「一种·Transm…」、「用于分析·Usin…」
