...
- One-line definition (source of truth): Nichols chart：Nichols chart
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → Frequency response method
- Extended explanation (do not paste verbatim on image): Nichols chart
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Nichols cha…」、「Nichols char」、「关联·Freque…」
