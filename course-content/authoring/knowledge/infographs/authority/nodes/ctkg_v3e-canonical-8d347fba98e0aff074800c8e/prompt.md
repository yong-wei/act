...
## Subject
- Title: Insignificant pole
- Concept kind: theoretical_construct
- One-line definition (source of truth): Insignificant pole：A pole with a negative real part that is much more negative than the other poles, expected to affect the transient resp…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A pole with a negative real part that is much more negative than the other poles, expected to affec…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Insignifica…」、「Insignifican」
