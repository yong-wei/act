...
## Subject
- Title: 领域概念 (modeling_0ae282f545b3c7aad3f3b142)
- Concept kind: analysis_method
- One-line definition (source of truth): 领域概念：To find state-space equations, define a state vector of appropriate variables, then derive the A, B, C, D matrices from…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): To find state-space equations, define a state vector of appropriate variables, then derive the A, B…
