...
- One-line definition (source of truth): 超调量：The maximum amount the system overshoots its final value divided by its final value (and is often expressed as a percen…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The maximum amount the system overshoots its final value divided by its final value (and is often e…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「超调量」、「Overshoot」
