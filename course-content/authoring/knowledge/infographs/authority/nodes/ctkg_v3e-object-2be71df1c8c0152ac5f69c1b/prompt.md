...
## Subject
- Title: Steady-state tracking error
- Concept kind: unknown
- One-line definition (source of truth): Steady-state tracking error：The error between the output and the reference input after transients have died out.
- Core intuition: 在图谱邻接中可把握：前置 → Internal model design。
- Relation summary: 前置 → Internal model design
- Extended explanation (do not paste verbatim on image): The error between the output and the reference input after transients have died out.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Steady-stat…」、「用于分析·Inte…」
