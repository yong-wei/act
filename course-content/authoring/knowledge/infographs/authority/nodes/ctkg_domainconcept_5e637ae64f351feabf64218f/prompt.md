...
- One-line definition (source of truth): 李雅普诺夫函数候选是自动控制原理权威图谱中的领域概念。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → 能量函数（李雅普诺夫）、state space model、Equilibrium Point
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「李雅普诺夫函数候选」、「一种·能量函数（李…」、「用于分析·stat…」、「应用于·Equil…」
