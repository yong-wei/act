...
## Subject
- Title: block diagram (block_diagram)
- Concept kind: representation_kind
- One-line definition (source of truth): block diagram：v3T accepted candidate for block diagram.
- Core intuition: 在图谱邻接中可把握：前置 → block diagram reduction。
- Relation summary: 前置 → block diagram reduction
- Extended explanation (do not paste verbatim on image): v3T accepted candidate for block diagram.
