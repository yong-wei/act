...
## Subject
- Title: tau RC the time (modeling_3d919906545f3c2891c3f444)
- Concept kind: theoretical_construct
- One-line definition (source of truth): tau RC the time：tau = RC, the time constant of the network.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): tau = RC, the time constant of the network.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「tau RC the …」
