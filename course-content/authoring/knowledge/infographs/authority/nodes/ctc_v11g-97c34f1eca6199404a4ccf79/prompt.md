...
## Subject
- Title: 反应曲线 (reaction_curve)
- Concept kind: representation_kind
- One-line definition (source of truth): 反应曲线：The measured output from an open-loop step input, assumed to have the general shape of a first-order system with a tran…
- Core intuition: 在图谱邻接中可把握：前置 → 反应速率 · 后续 → 反应曲线参数、传输延迟。
- Relation summary: 前置 → 反应速率 · 后续 → 反应曲线参数、传输延迟
- Extended explanation (do not paste verbatim on image): The measured output from an open-loop step input, assumed to have the general shape of a first-orde…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「反应曲线」、「reaction cur」、「关联·反应速率」、「关联·反应曲线参数」、「关联·传输延迟」
