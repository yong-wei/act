...
## Subject
- Title: Undamped natural frequency omega n (Undamped natural frequency omega_n)
- Concept kind: theoretical_construct
- One-line definition (source of truth): Undamped natural frequency omega n：The undamped natural frequency is the radius at which the poles are located in the s-plane.
- Core intuition: 在图谱邻接中可把握：前置 → Band center · 后续 → 极点。
- Relation summary: 前置 → Band center · 后续 → 极点
- Extended explanation (do not paste verbatim on image): The undamped natural frequency is the radius at which the poles are located in the s-plane.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Undamped na…」、「一种·Band」、「派生自·极点」
