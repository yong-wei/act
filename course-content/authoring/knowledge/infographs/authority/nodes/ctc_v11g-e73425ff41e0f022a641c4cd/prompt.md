...
## Subject
- Title: K_G* (forward_path_root_locus_gain)
- Concept kind: mathematical_object
- One-line definition (source of truth): 前向通路传递函数中分子多项式最高项系数归一化后的增益，记为K_G*。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → 增益
- Extended explanation (do not paste verbatim on image): 前向通路传递函数中分子多项式最高项系数归一化后的增益，记为K_G*。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「K G*」、「是一种·增益」
