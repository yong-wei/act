...
- One-line definition (source of truth): Two-sided Laplace transform：If Eq. (3.32) is a one-sided transform, then by extension, Eq. (3.30) is a two-sided Laplace transform.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): If Eq. (3.32) is a one-sided transform, then by extension, Eq. (3.30) is a two-sided Laplace transf…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Two-sided L…」
