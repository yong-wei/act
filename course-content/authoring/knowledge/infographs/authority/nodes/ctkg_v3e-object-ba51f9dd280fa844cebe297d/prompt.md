...
- One-line definition (source of truth): 开环零极点均位于s左半平面的典型环节，包括比例环节(K>0)、惯性环节、一阶微分环节、振荡环节、二阶微分环节、积分环节、微分环节七种。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 开环零极点均位于s左半平面的典型环节，包括比例环节(K>0)、惯性环节、一阶微分环节、振荡环节、二阶微分环节、积分环节、微分环节七种。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「最小相位环节」
