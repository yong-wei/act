...
- One-line definition (source of truth): Nyquist rate：Nyquist rate
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → Nyquist-Shannon sampling theorem
- Extended explanation (do not paste verbatim on image): Nyquist rate
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Nyquist rate」、「包含组件·Nyqu…」
