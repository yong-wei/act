...
- One-line definition (source of truth): 由离散系统闭环特征方程D(z)=0的系数构造的(2n-3)行、(n+1)列阵列，用于朱利稳定判据。
- Core intuition: 在图谱邻接中可把握：前置 → 朱利稳定判据。
- Relation summary: 前置 → 朱利稳定判据
- Extended explanation (do not paste verbatim on image): 由离散系统闭环特征方程D(z)=0的系数构造的(2n-3)行、(n+1)列阵列，用于朱利稳定判据。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「朱利阵列」、「包含组件·朱利稳定…」
