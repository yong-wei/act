...
## Subject
- Title: 离散系统状态方程中矩阵 G 和… (modeling_342138cd12f0e7fce63c7dc3)
- Concept kind: theoretical_construct
- One-line definition (source of truth): 离散系统状态方程中矩阵 G 和 h 的一种标准形式，具有可控性。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 离散系统状态方程中矩阵 G 和 h 的一种标准形式，具有可控性。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「离散系统状态方程中矩阵…」
