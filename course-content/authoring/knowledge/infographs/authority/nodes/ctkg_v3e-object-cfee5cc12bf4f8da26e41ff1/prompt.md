...
- One-line definition (source of truth): Vector Margin：The distance to the -1 point from the closest approach of the Nyquist plot.
- Core intuition: 在图谱邻接中可把握：前置 → Using vector margin to assess stability。
- Relation summary: 前置 → Using vector margin to assess stability
- Extended explanation (do not paste verbatim on image): The distance to the -1 point from the closest approach of the Nyquist plot.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Vector Marg…」、「Vector Margi」、「关联·Using …」
