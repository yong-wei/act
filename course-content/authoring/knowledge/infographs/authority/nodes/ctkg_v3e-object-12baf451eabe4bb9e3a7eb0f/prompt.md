...
- One-line definition (source of truth): Reference input：A signal r(t) that represents the desired output or command to be followed by the system.
- Core intuition: 在图谱邻接中可把握：前置 → Tracking error、Asymptotic tracking · 后续 → Command following。
- Relation summary: 前置 → Tracking error、Asymptotic tracking · 后续 → Command following
- Extended explanation (do not paste verbatim on image): A signal r(t) that represents the desired output or command to be followed by the system.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Reference i…」、「Reference in」、「应用于·Track…」、「应用于·Asymp…」、「关联·Comman…」
