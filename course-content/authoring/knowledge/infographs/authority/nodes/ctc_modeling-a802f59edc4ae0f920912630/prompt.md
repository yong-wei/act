...
## Subject
- Title: 带有箭头的直线 (modeling_a802f59edc4ae0f920912630)
- Concept kind: theoretical_construct
- One-line definition (source of truth): 带有箭头的直线，箭头表示信号的流向，在直线旁标记信号的时间函数或象函数。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 带有箭头的直线，箭头表示信号的流向，在直线旁标记信号的时间函数或象函数。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「带有箭头的直线」
