...
- One-line definition (source of truth): 积分时间常数。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → PID控制器
- Extended explanation (do not paste verbatim on image): 积分时间常数
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「积分时间常数」、「包含组件·PID控…」
