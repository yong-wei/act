...
## Subject
- Title: 反馈通路传递函数H(s)=1的… (unity_feedback_system)
- Concept kind: system_kind
- One-line definition (source of truth): 反馈通路传递函数H(s)=1的反馈控制系统。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 反馈通路传递函数H(s)=1的反馈控制系统。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「反馈通路传递函数H(s…」、「unity feedba」
