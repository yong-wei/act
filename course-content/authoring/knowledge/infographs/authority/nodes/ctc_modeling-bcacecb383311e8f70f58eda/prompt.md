...
## Subject
- Title: 当bn=0时 (modeling_bcacecb383311e8f70f58eda)
- Concept kind: analysis_method
- One-line definition (source of truth): 当bn=0时，按规则选择状态变量：xn = y, xi = dx_{i+1}/dt + a_i y - b_i u (i=1,2,...,n-1)。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 当bn=0时，按规则选择状态变量：xn = y, xi = dx_{i+1}/dt + a_i y - b_i u (i=1,2,...,n-1)。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「当bn=0时」
