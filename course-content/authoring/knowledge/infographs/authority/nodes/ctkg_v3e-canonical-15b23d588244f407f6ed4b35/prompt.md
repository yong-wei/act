...
- One-line definition (source of truth): Undamped Natural Frequency：The frequency at which a system would oscillate if damping were zero, denoted by ω_n.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The frequency at which a system would oscillate if damping were zero, denoted by ω_n.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Undamped Na…」
