...
## Subject
- Title: distributed rc network (distributed_rc_network)
- Concept kind: system_component
- One-line definition (source of truth): distributed rc network：A network composed of distributed R and C elements, with an attenuation slope of 10n dB/decade, where n is the order of…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A network composed of distributed R and C elements, with an attenuation slope of 10n dB/decade, whe…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「distributed…」、「distributed 」
