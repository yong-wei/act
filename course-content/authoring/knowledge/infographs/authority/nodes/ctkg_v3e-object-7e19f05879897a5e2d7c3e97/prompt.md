...
## Subject
- Title: Ackermann's form (Ackermann's formula for state feedback gain)
- Concept kind: unknown
- One-line definition (source of truth): Ackermann's form：For a single-input, single-output system, Ackermann's formula is useful for determining the state variable feedback mat…
- Core intuition: 在图谱邻接中可把握：后续 → Controllability matrix。
- Relation summary: 后续 → Controllability matrix
- Extended explanation (do not paste verbatim on image): For a single-input, single-output system, Ackermann's formula is useful for determining the state v…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Ackermann's…」、「包含组件·Cont…」
