...
## Subject
- Title: 闭环极点 (closed_loop_poles)
- Concept kind: mathematical_object
- One-line definition (source of truth): 闭环系统特征方程式的根，即闭环传递函数的极点。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 根轨迹 · 后续 → root of the denominator、合同明确要求 composit…、根轨迹法
- Extended explanation (do not paste verbatim on image): 闭环系统特征方程式的根，即闭环传递函数的极点。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「闭环极点」、「closed loop 」、「一种·root」、「属于·合同明确要求…」、「关联·根轨迹法」、「关联·稳定性」、「有表示·根轨迹」
