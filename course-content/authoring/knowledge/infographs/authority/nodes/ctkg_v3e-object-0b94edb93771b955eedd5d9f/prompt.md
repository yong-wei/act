...
- One-line definition (source of truth): 在已知系统开环传递函数条件下，直接调用命令 feedback 和 bode，可立即得到闭环对数幅频和相频曲线，然后可判读出系统谐振频率 ω_r、谐振峰值 M_r(dB) 及带宽频率 ω_b。
- Core intuition: 在图谱邻接中可把握：前置 → 谐振峰值 · 后续 → 谐振峰值、谐振频率、带宽频率。
- Relation summary: 前置 → 谐振峰值 · 后续 → 谐振峰值、谐振频率、带宽频率
- Extended explanation (do not paste verbatim on image): 在已知系统开环传递函数条件下，直接调用命令 feedback 和 bode，可立即得到闭环对数幅频和相频曲线，然后可判读出系统谐振频率 ω_r、谐振峰值 M_r(dB) 及带宽频率 ω_b。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「得到闭环对数幅频和相频…」、「用于分析·谐振峰值」、「用于分析·谐振频率」、「用于分析·带宽频率」、「关联·谐振峰值」、「关联·谐振频率」
