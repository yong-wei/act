...
## Subject
- Title: variable that is measured (modeling_2326548c33d3005ed44b3f18)
- Concept kind: theoretical_construct
- One-line definition (source of truth): variable that is measured：A variable that is measured across an element, representing a difference (e.g., angular velocity difference, voltage di…
- Core intuition: 在图谱邻接中可把握：后续 → variable that is measured。
- Relation summary: 后续 → variable that is measured
- Extended explanation (do not paste verbatim on image): A variable that is measured across an element, representing a difference (e.g., angular velocity di…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「variable th…」、「关联·variab…」
