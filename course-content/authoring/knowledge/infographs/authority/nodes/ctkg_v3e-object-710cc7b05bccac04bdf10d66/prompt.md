...
## Subject
- Title: Asymptotic tracking
- Concept kind: unknown
- One-line definition (source of truth): Asymptotic tracking：The property of a control system where the tracking error approaches zero as time goes to infinity.
- Core intuition: 在图谱邻接中可把握：前置 → Internal model design · 后续 → Reference input、Tracking error、Zero steady-state error。
- Relation summary: 前置 → Internal model design · 后续 → Reference input、Tracking error、Zero steady-state error
- Extended explanation (do not paste verbatim on image): The property of a control system where the tracking error approaches zero as time goes to infinity.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Asymptotic …」、「Asymptotic t」、「应用于·Refer…」、「应用于·Track…」、「关联·Intern…」、「关联·Zero s…」
