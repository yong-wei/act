...
- One-line definition (source of truth): 系统在扰动消失后，由初始偏差状态恢复到原平衡状态的性能。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 平衡状态稳定性、大范围稳定系统、Neutral Stability
- Extended explanation (do not paste verbatim on image): 系统在扰动消失后，由初始偏差状态恢复到原平衡状态的性能。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「稳定性」、「一种·平衡状态稳定性」、「一种·大范围稳定系统」、「一种·Neutra…」、「一种·Margin…」、「一种·BIBO」
