...
- One-line definition (source of truth): 形如 \dot{x}(t) = A x(t) + B u(t) 的状态方程，包含输入项。
- Core intuition: 在图谱邻接中可把握：前置 → 积分法求解非齐次状态方程、拉普拉斯变换法求解非齐次状态方程。
- Relation summary: 前置 → 积分法求解非齐次状态方程、拉普拉斯变换法求解非齐次状态方程
- Extended explanation (do not paste verbatim on image): 形如 \dot{x}(t) = A x(t) + B u(t) 的状态方程，包含输入项
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「非齐次状态方程」、「用于分析·积分法求…」、「用于分析·拉普拉斯…」
