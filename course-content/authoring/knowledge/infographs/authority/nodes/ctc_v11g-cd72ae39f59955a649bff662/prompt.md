...
## Subject
- Title: exhaust gas recirculation egr (exhaust_gas_recirculation_egr)
- Concept kind: design_method
- One-line definition (source of truth): exhaust gas recirculation egr：A technique used to meet nitrogen oxides (NOx) emissions standards by recirculating exhaust gas.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A technique used to meet nitrogen oxides (NOx) emissions standards by recirculating exhaust gas.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「exhaust gas…」
