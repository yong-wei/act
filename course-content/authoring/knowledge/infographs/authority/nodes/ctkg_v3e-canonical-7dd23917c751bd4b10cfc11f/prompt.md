...
- One-line definition (source of truth): c_2(t) 为非零初始条件响应分量。
- Core intuition: 当 0<ζ<1 时，有 c_2(t)=√(c²(0)+[(c(0)ζω_n+ċ(0))/(ω_n√(1-ζ²))]²) e^{-ζω_n t} sin(ω_d t+θ), t≥0。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): c_2(t) 为非零初始条件响应分量。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「非零初始条件响应分量」
