...
- One-line definition (source of truth): Rise Time：The time it takes the system to reach the vicinity of its new set point.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The time it takes the system to reach the vicinity of its new set point.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Rise Time」
