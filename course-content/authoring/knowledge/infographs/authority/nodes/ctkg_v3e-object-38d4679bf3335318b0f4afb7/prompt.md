...
- One-line definition (source of truth): 谐波输入下，输出响应中与输入同频率的谐波分量与谐波输入的幅值之比A(ω)为幅频特性，相位之差φ(ω)为相频特性，并称其指数表达形式G(jω)=A(ω)e^{jφ(ω)}为系统的频率特性。
- Core intuition: 在图谱邻接中可把握：前置 → 绘制概略开环幅相特性曲线方法、频率特性实验确定方法 · 后续 → 相频特性、幅频特性。
- Relation summary: 前置 → 绘制概略开环幅相特性曲线方法、频率特性实验确定方法 · 后续 → 相频特性、幅频特性
- Extended explanation (do not paste verbatim on image): 谐波输入下，输出响应中与输入同频率的谐波分量与谐波输入的幅值之比A(ω)为幅频特性，相位之差φ(ω)为相频特性，并称其指数表达形式G(jω)=A(ω)e^{jφ(ω)}为系统的频率特性。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「频率特性」、「用于分析·绘制概略…」、「应用于·频率特性实…」、「关联·相频特性」、「关联·幅频特性」
