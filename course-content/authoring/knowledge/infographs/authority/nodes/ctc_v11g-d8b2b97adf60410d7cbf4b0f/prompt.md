...
## Subject
- Title: root sensitivity (root_sensitivity)
- Concept kind: performance_metric
- One-line definition (source of truth): root sensitivity：The sensitivity of the roots as a parameter changes from its normal value.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The sensitivity of the roots as a parameter changes from its normal value.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「root sensit…」、「root sensiti」
