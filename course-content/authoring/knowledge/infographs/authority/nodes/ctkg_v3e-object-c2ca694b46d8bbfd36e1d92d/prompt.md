...
## Subject
- Title: Design by Discre (Design by Discrete Equivalent (Emulation))
- Concept kind: unknown
- One-line definition (source of truth): Design by Discre：A design approach that proceeds through stages: 1) design continuous compensation, 2) find discrete equivalent approxim…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A design approach that proceeds through stages: 1) design continuous compensation, 2) find discrete…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Design by D…」
