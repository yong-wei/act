...
- One-line definition (source of truth): 在线性控制系统中，常用的校正装置设计方法有分析法和综合法两种。
- Core intuition: 分析法又称试探法，综合法又称期望特性法。
- Relation summary: 前置 → 串联超前校正 · 后续 → 综合法（期望特性法）、分析法（试探法）
- Extended explanation (do not paste verbatim on image): 在线性控制系统中，常用的校正装置设计方法有分析法和综合法两种。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「频率响应法校正设计」、「属于·串联超前校正」、「包含组件·综合法（…」、「包含组件·分析法（…」
