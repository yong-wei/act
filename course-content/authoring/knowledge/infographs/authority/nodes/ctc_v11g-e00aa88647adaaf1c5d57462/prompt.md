...
## Subject
- Title: asymptote centroid (asymptote_centroid)
- Concept kind: mathematical_object
- One-line definition (source of truth): asymptote centroid：The center of the linear asymptotes, σ_A.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The center of the linear asymptotes, σ_A.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「asymptote c…」、「asymptote ce」
