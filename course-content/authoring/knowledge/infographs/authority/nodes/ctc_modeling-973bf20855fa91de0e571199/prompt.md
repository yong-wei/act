...
## Subject
- Title: The voltage across an (modeling_973bf20855fa91de0e571199)
- Concept kind: theoretical_construct
- One-line definition (source of truth): The voltage across an：The voltage across an inductor is proportional to the rate of change of current through it, v = L di/dt.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The voltage across an inductor is proportional to the rate of change of current through it, v = L d…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「The voltage…」
