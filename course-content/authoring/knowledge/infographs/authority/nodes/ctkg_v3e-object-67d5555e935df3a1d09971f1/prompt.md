...
- One-line definition (source of truth): D/A 转换的第二个过程：因为离散的模拟信号无法直接控制连续的被控对象，需要经过保持器将离散模拟信号复现为连续的模拟信号。
- Core intuition: 在图谱邻接中可把握：前置 → 信号复现过程 · 后续 → D/A转换器、保持器。
- Relation summary: 前置 → 信号复现过程 · 后续 → D/A转换器、保持器
- Extended explanation (do not paste verbatim on image): D/A 转换的第二个过程：因为离散的模拟信号无法直接控制连续的被控对象，需要经过保持器将离散模拟信号复现为连续的模拟信号。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「复现过程」、「是一种·信号复现过程」、「属于·D/A转换器」、「包含组件·保持器」
