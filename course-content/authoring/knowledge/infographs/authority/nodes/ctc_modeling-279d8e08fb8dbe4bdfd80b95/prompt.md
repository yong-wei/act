...
## Subject
- Title: row matrix in the (modeling_279d8e08fb8dbe4bdfd80b95)
- Concept kind: theoretical_construct
- One-line definition (source of truth): row matrix in the：A 1 x n row matrix in the state-variable form.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A 1 x n row matrix in the state-variable form.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「row matrix …」
