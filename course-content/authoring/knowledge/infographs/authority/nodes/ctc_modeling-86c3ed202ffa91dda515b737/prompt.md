...
## Subject
- Title: 用拉普拉斯变换法求解线性定常微… (modeling_86c3ed202ffa91dda515b737)
- Concept kind: analysis_method
- One-line definition (source of truth): 用拉普拉斯变换法求解线性定常微分方程，并分析解的组成，为引出传递函数概念奠定基础。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 用拉普拉斯变换法求解线性定常微分方程，并分析解的组成，为引出传递函数概念奠定基础。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「用拉普拉斯变换法求解线…」
