...
- One-line definition (source of truth): 格拉姆矩阵判据 (Gramian matrix criterion for observability)。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 格拉姆矩阵判据 (Gramian matrix criterion for observability)
