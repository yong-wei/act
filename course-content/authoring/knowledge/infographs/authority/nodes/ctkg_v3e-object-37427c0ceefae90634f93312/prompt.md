...
- One-line definition (source of truth): Sector conditions：Conditions defining a function f(x) belonging to the sector [k1, k2] if k1 x^2 ≤ f(x) x ≤ k2 x^2 for all inputs x.
- Core intuition: 在图谱邻接中可把握：后续 → Circle Criterion。
- Relation summary: 后续 → Circle Criterion
- Extended explanation (do not paste verbatim on image): Conditions defining a function f(x) belonging to the sector [k1, k2] if k1 x^2 ≤ f(x) x ≤ k2 x^2 fo…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Sector cond…」、「Sector condi」、「关联·Circle…」
