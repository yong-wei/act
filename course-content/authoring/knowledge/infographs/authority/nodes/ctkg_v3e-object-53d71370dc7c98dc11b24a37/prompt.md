...
## Subject
- Title: Notch Compensation
- Concept kind: unknown
- One-line definition (source of truth): Notch Compensation：Dynamic compensation with special characteristics for dealing with resonances.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): Dynamic compensation with special characteristics for dealing with resonances.
