...
## Subject
- Title: 传递函数因式分解为一次因子和二… (modeling_a18617af9cc8138f30c71543)
- Concept kind: theoretical_construct
- One-line definition (source of truth): 传递函数因式分解为一次因子和二次因子连乘积时，一次因子和二次因子中出现的常数，记为τ_i和T_j。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 传递函数因式分解为一次因子和二次因子连乘积时，一次因子和二次因子中出现的常数，记为τ_i和T_j。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「传递函数因式分解为一次…」
