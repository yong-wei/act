...
- One-line definition (source of truth): 设sEn(s)的极点位于s左半平面，则可用终值定理法求得稳态误差。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 设sEn(s)的极点位于s左半平面，则可用终值定理法求得稳态误差。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「终值定理法求稳态误差」
