...
- One-line definition (source of truth): 系统阶跃动态响应中的振荡频率，记为ω_d。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → 穿越频率、截止频率
- Extended explanation (do not paste verbatim on image): 系统阶跃动态响应中的振荡频率，记为ω_d
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「阻尼自然频率」、「关联·穿越频率」、「关联·截止频率」
