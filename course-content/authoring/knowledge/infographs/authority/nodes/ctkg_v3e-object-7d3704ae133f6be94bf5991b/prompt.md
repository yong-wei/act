...
- One-line definition (source of truth): 线性定常离散系统的脉冲传递函数定义为系统输出采样信号的z变换与输入采样信号的z变换之比。
- Core intuition: 在图谱邻接中可把握：后续 → 线性定常离散系统。
- Relation summary: 后续 → 线性定常离散系统
- Extended explanation (do not paste verbatim on image): 线性定常离散系统的脉冲传递函数定义为系统输出采样信号的z变换与输入采样信号的z变换之比。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「脉冲传递函数」、「关联·线性定常离散…」
