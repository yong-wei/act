...
## Subject
- Title: Constant that relates motor (modeling_4deaeff05024e26119625b3f)
- Concept kind: theoretical_construct
- One-line definition (source of truth): Constant that relates motor：Constant K_m that relates motor torque to the input current (field current or armature current) in a linearized DC moto…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): Constant K_m that relates motor torque to the input current (field current or armature current) in…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Constant th…」
