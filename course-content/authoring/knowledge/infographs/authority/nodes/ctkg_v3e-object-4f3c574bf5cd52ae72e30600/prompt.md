...
- One-line definition (source of truth): Describing Function：A frequency-dependent equivalent gain used to approximate the behavior of a nonlinear element under sinusoidal excitati…
- Core intuition: 在图谱邻接中可把握：前置 → Nonlinear Element with Sinusoidal Input · 后续 → Describing Function Method、Limit Cycle、Nonlinear Element with Sinusoidal Input。
- Relation summary: 前置 → Nonlinear Element with Sinusoidal Input · 后续 → Describing Function Method、Limit Cycle、Nonlinear Element with Sinusoidal Input
- Extended explanation (do not paste verbatim on image): A frequency-dependent equivalent gain used to approximate the behavior of a nonlinear element under…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Describing …」、「Describing F」、「属于·Descri…」、「用于分析·Limi…」、「应用于·Nonli…」、「关联·Nonlin…」
