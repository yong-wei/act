...
- One-line definition (source of truth): 阶梯信号。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 保持器、信号复现过程 · 后续 → 模拟信号
- Extended explanation (do not paste verbatim on image): 阶梯信号
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「阶梯信号」、「是一种·模拟信号」、「包含组件·保持器」、「包含组件·信号复现…」
