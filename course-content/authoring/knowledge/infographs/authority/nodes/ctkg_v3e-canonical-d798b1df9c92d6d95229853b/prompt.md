...
- One-line definition (source of truth): Amplitude ratio：M is usually referred to as the amplitude ratio and is a function of the input frequency ω.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): M is usually referred to as the amplitude ratio and is a function of the input frequency ω.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Amplitude r…」、「Amplitude ra」
