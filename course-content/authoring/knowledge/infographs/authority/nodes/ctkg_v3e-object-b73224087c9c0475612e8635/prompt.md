...
## Subject
- Title: Phase-Lag Compensator
- Concept kind: unknown
- One-line definition (source of truth): Phase-Lag Compensator：A compensator providing a phase-lag characteristic, often called an integrating compensator.
- Core intuition: 在图谱邻接中可把握：前置 → 串联滞后-超前校正 · 后续 → Cascade Compensator。
- Relation summary: 前置 → 串联滞后-超前校正 · 后续 → Cascade Compensator
- Extended explanation (do not paste verbatim on image): A compensator providing a phase-lag characteristic, often called an integrating compensator.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Phase-Lag C…」、「Phase-Lag Co」、「一种·Cascad…」、「包含组件·串联滞后…」
