...
## Subject
- Title: dominant roots (dominant_roots)
- Concept kind: system_property
- One-line definition (source of truth): dominant roots：Complex conjugate roots near the origin of the s-plane relative to other closed-loop system roots, which dominate the t…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): Complex conjugate roots near the origin of the s-plane relative to other closed-loop system roots,…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「dominant ro…」、「dominant roo」
