...
## Subject
- Title: quarter amplitude decay (quarter_amplitude_decay)
- Concept kind: criterion
- One-line definition (source of truth): quarter amplitude decay：A condition where the amplitude of the closed-loop response is reduced approximately to one-fourth of the maximum value…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A condition where the amplitude of the closed-loop response is reduced approximately to one-fourth…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「quarter amp…」、「quarter ampl」
