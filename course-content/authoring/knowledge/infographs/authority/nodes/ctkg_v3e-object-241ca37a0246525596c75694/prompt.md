...
- One-line definition (source of truth): Zero-order hold：A circuit that takes the value r(kT) and holds it constant for kT ≤ t < (k+1)T.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A circuit that takes the value r(kT) and holds it constant for kT ≤ t < (k+1)T.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Zero-order …」、「Zero-order h」
