...
- One-line definition (source of truth): Undamped oscillation：The response of the uncompensated system is an undamped oscillation because T(s) = 10/(s^2+10).
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The response of the uncompensated system is an undamped oscillation because T(s) = 10/(s^2+10).
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Undamped os…」、「Undamped osc」
