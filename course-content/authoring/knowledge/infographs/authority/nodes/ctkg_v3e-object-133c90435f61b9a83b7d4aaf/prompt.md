...
- One-line definition (source of truth): Controllability：A system is completely controllable if there exists an unconstrained control u(t) that can transfer any initial state x…
- Core intuition: 在图谱邻接中可把握：前置 → Controllability matrix。
- Relation summary: 前置 → Controllability matrix
- Extended explanation (do not paste verbatim on image): A system is completely controllable if there exists an unconstrained control u(t) that can transfer…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Controllabi…」、「Controllabil」、「用于分析·Cont…」
