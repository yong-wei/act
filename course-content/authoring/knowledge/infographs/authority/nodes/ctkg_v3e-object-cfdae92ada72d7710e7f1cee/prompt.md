...
- One-line definition (source of truth): Routh-Hurwitz stability criterion：Routh-Hurwitz stability criterion
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 朱利稳定判据、Routh array · 后续 → Routh array、特征方程、稳定性
- Extended explanation (do not paste verbatim on image): Routh-Hurwitz stability criterion
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Routh-Hurwi…」、「是一种·朱利稳定判据」、「属于·Routh …」、「包含组件·Rout…」、「关联·特征方程」
