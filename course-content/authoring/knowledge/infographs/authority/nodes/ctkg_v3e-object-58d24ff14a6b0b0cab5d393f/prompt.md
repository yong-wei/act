...
- One-line definition (source of truth): 复数c_k，表示周期信号第k次谐波的幅值和相位信息。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 复数c_k，表示周期信号第k次谐波的幅值和相位信息
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「傅里叶系数」
