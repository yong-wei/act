...
- One-line definition (source of truth): 如果在有规律的间隔上，系统取到了离散信息，则这种采样称为周期采样。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → 非周期采样
- Extended explanation (do not paste verbatim on image): 如果在有规律的间隔上，系统取到了离散信息，则这种采样称为周期采样。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「周期采样」、「关联·非周期采样」
