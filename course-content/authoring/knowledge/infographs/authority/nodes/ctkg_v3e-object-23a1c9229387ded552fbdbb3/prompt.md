...
- One-line definition (source of truth): 由波波夫、贝尔维奇和豪塔斯提出的可控性和可观测性判据，基于对矩阵A的特征值检验矩阵的秩。
- Core intuition: 在图谱邻接中可把握：后续 → 否成立。
- Relation summary: 后续 → 否成立
- Extended explanation (do not paste verbatim on image): 由波波夫、贝尔维奇和豪塔斯提出的可控性和可观测性判据，基于对矩阵A的特征值检验矩阵的秩。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「PBH秩判据」、「是一种·否成立」
