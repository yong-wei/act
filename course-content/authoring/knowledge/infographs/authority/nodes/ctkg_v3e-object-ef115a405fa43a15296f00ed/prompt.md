...
## Subject
- Title: Composite gain Nbar
- Concept kind: unknown
- One-line definition (source of truth): Composite gain Nbar：The constant coefficient of r in the control law u = -Kx + Nbar r, computed as Nbar = N_u + K N_x.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The constant coefficient of r in the control law u = -Kx + Nbar r, computed as Nbar = N_u + K N_x.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Composite g…」、「Composite ga」
