...
## Subject
- Title: Method 2: Using  (Method 2: Using tracking error as compensator input)
- Concept kind: unknown
- One-line definition (source of truth): Method 2: Using ：Select N=0 and M=-L so that the observer is driven by the tracking error y(t)-r(t).
- Core intuition: 在图谱邻接中可把握：后续 → Tracking error。
- Relation summary: 后续 → Tracking error
- Extended explanation (do not paste verbatim on image): Select N=0 and M=-L so that the observer is driven by the tracking error y(t)-r(t).
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Method 2」、「包含组件·Trac…」、「应用于·Track…」
