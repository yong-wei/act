...
## Subject
- Title: Observer poles location
- Concept kind: unknown
- One-line definition (source of truth): Observer poles location：Observer poles are located furthest to the left for faster convergence, approximately at (-16, 21) and (-16, -22).
- Core intuition: 在图谱邻接中可把握：后续 → Observer poles。
- Relation summary: 后续 → Observer poles
- Extended explanation (do not paste verbatim on image): Observer poles are located furthest to the left for faster convergence, approximately at (-16, 21)…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Observer po…」、「Observer pol」、「有表示·Obser…」
