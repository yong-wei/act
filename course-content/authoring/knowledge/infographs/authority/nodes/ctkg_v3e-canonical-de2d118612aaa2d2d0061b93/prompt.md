...
## Subject
- Title: Finding transfer (Finding transfer function via Laplace transform of ODE)
- Concept kind: analysis_method
- One-line definition (source of truth): Finding transfer：Take the Laplace transform of both sides of the differential equation assuming zero initial conditions, then solve for…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): Take the Laplace transform of both sides of the differential equation assuming zero initial conditi…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Finding tra…」
