...
- One-line definition (source of truth): 可控性判别阵。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 秩判据
- Extended explanation (do not paste verbatim on image): 可控性判别阵
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「可控性判别阵」、「包含组件·秩判据」
