...
- One-line definition (source of truth): 前馈信号。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → PWM放大器
- Extended explanation (do not paste verbatim on image): 前馈信号
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「前馈信号」、「关联·PWM放大器」
