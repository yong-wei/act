...
## Subject
- Title: State transition (State transition matrix in Laplace domain)
- Concept kind: unknown
- One-line definition (source of truth): State transition：Φ(s) = [sI - A]^{-1} is the Laplace transform of the state transition matrix.
- Core intuition: 在图谱邻接中可把握：前置 → Signal-flow graph state model。
- Relation summary: 前置 → Signal-flow graph state model
- Extended explanation (do not paste verbatim on image): Φ(s) = [sI - A]^{-1} is the Laplace transform of the state transition matrix.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「State trans…」、「用于分析·Sign…」
