...
- One-line definition (source of truth): 零点：v3T accepted candidate for zero.。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 开环零点、zeros of l s、nonminimum phase system · 后续 → 传递函数
- Extended explanation (do not paste verbatim on image): v3T accepted candidate for zero.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「零点」、「zero」、「是一种·开环零点」、「一种·zeros」、「派生自·传递函数」、「关联·nonmin…」、「关联·angle …」
