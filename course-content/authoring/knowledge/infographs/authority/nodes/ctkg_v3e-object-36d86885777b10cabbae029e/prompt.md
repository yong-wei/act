...
## Subject
- Title: Sampled-data second-order system
- Concept kind: unknown
- One-line definition (source of truth): Sampled-data second-order system：A second-order continuous process with a zero-order hold and sampler, analyzed in the discrete-time domain using the z-…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A second-order continuous process with a zero-order hold and sampler, analyzed in the discrete-time…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Sampled-dat…」
