...
- One-line definition (source of truth): 通过z=e^{sT}将s域映射到z域，s平面虚轴映射为单位圆，左半s平面映射为单位圆内，右半s平面映射为单位圆外。
- Core intuition: 在图谱邻接中可把握：前置 → 主要带与次要带。
- Relation summary: 前置 → 主要带与次要带
- Extended explanation (do not paste verbatim on image): 通过z=e^{sT}将s域映射到z域，s平面虚轴映射为单位圆，左半s平面映射为单位圆内，右半s平面映射为单位圆外。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「s平面到z平面的映射」、「属于·主要带与次要带」
