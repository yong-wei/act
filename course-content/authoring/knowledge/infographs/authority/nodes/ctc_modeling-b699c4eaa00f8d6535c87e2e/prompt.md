...
## Subject
- Title: 离散化状态方程中与输入 u(k… (modeling_b699c4eaa00f8d6535c87e2e)
- Concept kind: theoretical_construct
- One-line definition (source of truth): 离散化状态方程中与输入 u(k) 相乘的矩阵，记为 G(T)，计算公式为 G(T) = ∫_0^T Φ(τ') B dτ'。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 离散化状态方程中与输入 u(k) 相乘的矩阵，记为 G(T)，计算公式为 G(T) = ∫_0^T Φ(τ') B dτ'。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「离散化状态方程中与输入…」
