...
## Subject
- Title: Root-Locus Techn (Root-Locus Technique for Nonlinear Systems)
- Concept kind: unknown
- One-line definition (source of truth): Root-Locus Techn：A heuristic method that combines the root-locus technique with the observation that many nonlinear elements can be view…
- Core intuition: 在图谱邻接中可把握：前置 → Equivalent gain 、equivalent gain · 后续 → memoryless nonlinearity、Nonlinear System、Zero-Memory Nonlinear Element。
- Relation summary: 前置 → Equivalent gain 、equivalent gain · 后续 → memoryless nonlinearity、Nonlinear System、Zero-Memory Nonlinear Element
- Extended explanation (do not paste verbatim on image): A heuristic method that combines the root-locus technique with the observation that many nonlinear…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Root-Locus …」、「用于分析·memo…」、「用于分析·Nonl…」、「应用于·Zero-…」、「派生自·根轨迹法」、「关联·Equiva…」
