...
## Subject
- Title: Root locus z-plane method
- Concept kind: unknown
- One-line definition (source of truth): Root locus z-plane method是自动控制原理权威图谱中的领域概念。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → 根轨迹法
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Root locus …」、「派生自·根轨迹法」
