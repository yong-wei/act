...
## Subject
- Title: 两相伺服电动机堵转转矩Ms (modeling_b78ed64321ecf6db80e791bc)
- Concept kind: theoretical_construct
- One-line definition (source of truth): 两相伺服电动机堵转转矩Ms，可由Ms=Cm ua确定，其中Cm可用额定电压时的堵转转矩确定。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 两相伺服电动机堵转转矩Ms，可由Ms=Cm ua确定，其中Cm可用额定电压时的堵转转矩确定。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「两相伺服电动机堵转转矩…」
