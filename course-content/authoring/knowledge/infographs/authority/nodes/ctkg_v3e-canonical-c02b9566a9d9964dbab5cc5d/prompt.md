...
## Subject
- Title: Computing response via convolution
- Concept kind: analysis_method
- One-line definition (source of truth): Computing response via convolution：Decompose the input signal into a sum of elementary components (impulses or exponentials), find the response to each, a…
- Core intuition: 在图谱邻接中可把握：后续 → Convolution。
- Relation summary: 后续 → Convolution
- Extended explanation (do not paste verbatim on image): Decompose the input signal into a sum of elementary components (impulses or exponentials), find the…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Computing r…」、「包含组件·Conv…」
