...
## Subject
- Title: One can often linearize (modeling_f6875bd8fd4ad1250d8980d8)
- Concept kind: analysis_method
- One-line definition (source of truth): One can often linearize：One can often linearize nonlinear elements assuming small-signal conditions, which is the normal approach used to obtai…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): One can often linearize nonlinear elements assuming small-signal conditions, which is the normal ap…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「One can oft…」
