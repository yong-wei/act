...
- One-line definition (source of truth): 非线性特性曲线的折线的各转折点，构成了相平面区域的分界线，称为开关线。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 死区特性 · 后续 → 相平面法分析非线性系统、相平面
- Extended explanation (do not paste verbatim on image): 非线性特性曲线的折线的各转折点，构成了相平面区域的分界线，称为开关线。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「开关线」、「属于·相平面法分析…」、「属于·相平面」、「关联·死区特性」
