...
## Subject
- Title: Symmetric Root Locus (SRL) Method
- Concept kind: unknown
- One-line definition (source of truth): Symmetric Root Locus (SRL) Method：A technique for linear control systems design based on the optimal linear quadratic regulator (LQR) problem. The optima…
- Core intuition: 在图谱邻接中可把握：后续 → Linear Quadratic Regulator (LQR)。
- Relation summary: 后续 → Linear Quadratic Regulator (LQR)
- Extended explanation (do not paste verbatim on image): A technique for linear control systems design based on the optimal linear quadratic regulator (LQR)…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Symmetric R…」、「应用于·Linea…」
