...
- One-line definition (source of truth): 用分析法设计校正装置比较直观，在物理上易于实现，但要求设计者有一定的工程设计经验，设计过程带有试探性。
- Core intuition: 在图谱邻接中可把握：前置 → 频率响应法校正设计。
- Relation summary: 前置 → 频率响应法校正设计
- Extended explanation (do not paste verbatim on image): 用分析法设计校正装置比较直观，在物理上易于实现，但要求设计者有一定的工程设计经验，设计过程带有试探性。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「分析法（试探法）」、「包含组件·频率响应…」
