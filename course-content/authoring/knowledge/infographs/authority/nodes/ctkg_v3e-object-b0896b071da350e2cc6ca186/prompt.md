...
- One-line definition (source of truth): 间隙特性。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 等效增益、死区 · 后续 → 自振荡
- Extended explanation (do not paste verbatim on image): 间隙特性
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「间隙特性」、「关联·等效增益」、「关联·死区」、「关联·自振荡」
