...
## Subject
- Title: method to derive state (modeling_0cb360fdf4bd5a71b47de347)
- Concept kind: analysis_method
- One-line definition (source of truth): method to derive state：A method to derive a state model from a transfer function by introducing an intermediate variable Z(s), equating numera…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A method to derive a state model from a transfer function by introducing an intermediate variable Z…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「method to d…」
