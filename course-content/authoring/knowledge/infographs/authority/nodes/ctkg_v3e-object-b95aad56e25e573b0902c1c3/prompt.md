...
## Subject
- Title: Input compensation (as a scheme)
- Concept kind: unknown
- One-line definition (source of truth): Input compensation (as a scheme)：A compensation scheme where the compensator is placed before the summing junction of the main feedback loop in a single…
- Core intuition: 在图谱邻接中可把握：前置 → Input Compensation。
- Relation summary: 前置 → Input Compensation
- Extended explanation (do not paste verbatim on image): A compensation scheme where the compensator is placed before the summing junction of the main feedb…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Input compe…」、「一种·Input」
