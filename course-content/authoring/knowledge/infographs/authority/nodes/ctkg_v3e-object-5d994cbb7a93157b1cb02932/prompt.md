...
## Subject
- Title: Path cofactor in signal-flow graph
- Concept kind: unknown
- One-line definition (source of truth): Path cofactor in signal-flow graph：The path cofactor Δ₁(s) is the determinant of the part of the graph not touching the forward path.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The path cofactor Δ₁(s) is the determinant of the part of the graph not touching the forward path.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Path cofact…」
