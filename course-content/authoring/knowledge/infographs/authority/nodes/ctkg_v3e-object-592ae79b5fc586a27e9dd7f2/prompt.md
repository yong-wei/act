...
- One-line definition (source of truth): Unstable System：A system where the characteristic equation has at least one root in the right half of the s-plane or repeated jω roots
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A system where the characteristic equation has at least one root in the right half of the s-plane o…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Unstable Sy…」、「Unstable Sys」
