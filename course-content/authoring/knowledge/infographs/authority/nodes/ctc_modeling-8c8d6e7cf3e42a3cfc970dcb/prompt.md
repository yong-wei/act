...
## Subject
- Title: system with several input (modeling_8c8d6e7cf3e42a3cfc970dcb)
- Concept kind: theoretical_construct
- One-line definition (source of truth): system with several input：A system with several input and output signals.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A system with several input and output signals.
