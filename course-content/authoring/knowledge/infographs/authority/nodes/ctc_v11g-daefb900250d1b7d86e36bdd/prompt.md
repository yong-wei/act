...
## Subject
- Title: ultimate gain (ultimate_gain)
- Concept kind: performance_metric
- One-line definition (source of truth): ultimate gain：The gain on the border of instability, denoted by K_U, used in the closed-loop Ziegler-Nichols tuning method.
- Core intuition: 在图谱邻接中可把握：后续 → 增益。
- Relation summary: 后续 → 增益
- Extended explanation (do not paste verbatim on image): The gain on the border of instability, denoted by K_U, used in the closed-loop Ziegler-Nichols tuni…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「ultimate ga…」、「ultimate gai」、「是一种·增益」
