...
## Subject
- Title: Output compensation (as a scheme)
- Concept kind: unknown
- One-line definition (source of truth): Output compensation (as a scheme)：A compensation scheme where the compensator is placed after the process (at the output or load) in a single-loop feedba…
- Core intuition: 在图谱邻接中可把握：前置 → Output Compensation。
- Relation summary: 前置 → Output Compensation
- Extended explanation (do not paste verbatim on image): A compensation scheme where the compensator is placed after the process (at the output or load) in…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Output comp…」、「一种·Output」
