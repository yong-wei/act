...
- One-line definition (source of truth): 数字控制器。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → 脉冲控制器
- Extended explanation (do not paste verbatim on image): 数字控制器
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「数字控制器」、「关联·脉冲控制器」
