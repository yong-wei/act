...
## Subject
- Title: Determining PM from Bode plot
- Concept kind: unknown
- One-line definition (source of truth): Determining PM from Bode plot：Measure angle of G(jω) as it crosses the magnitude=1 circle.
- Core intuition: 在图谱邻接中可把握：后续 → Crossover frequency。
- Relation summary: 后续 → Crossover frequency
- Extended explanation (do not paste verbatim on image): Measure angle of G(jω) as it crosses the magnitude=1 circle.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Determining…」、「用于分析·Cros…」
