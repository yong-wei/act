...
- One-line definition (source of truth): Full-state feedback：A control design approach where all state variables are assumed available for feedback, and the control input is a line…
- Core intuition: 在图谱邻接中可把握：前置 → Full-state feedback design procedure。
- Relation summary: 前置 → Full-state feedback design procedure
- Extended explanation (do not paste verbatim on image): A control design approach where all state variables are assumed available for feedback, and the con…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Full-state …」、「Full-state f」、「应用于·Full-…」
