...
- One-line definition (source of truth): 复合控制信号。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → PWM放大器
- Extended explanation (do not paste verbatim on image): 复合控制信号
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「复合控制信号」、「包含组件·PWM放…」
