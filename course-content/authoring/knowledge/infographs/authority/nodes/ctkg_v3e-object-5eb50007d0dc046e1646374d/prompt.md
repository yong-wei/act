...
- One-line definition (source of truth): D/A converter：A device to convert the digital word from the computer to an analog voltage.
- Core intuition: 在图谱邻接中可把握：前置 → sample and hold device · 后续 → A/D converter、sample and hold device。
- Relation summary: 前置 → sample and hold device · 后续 → A/D converter、sample and hold device
- Extended explanation (do not paste verbatim on image): A device to convert the digital word from the computer to an analog voltage.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「D/A convert…」、「D/A converte」、「一种·sample」、「关联·A/D co…」、「有表示·sampl…」
