...
- One-line definition (source of truth): 当 Im[G(jω_x)H(jω_x)]=0 或 φ(ω_x)=∠[G(jω_x)H(jω_x)]=kπ 时，称 ω_x 为穿越频率。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 当 Im[G(jω_x)H(jω_x)]=0 或 φ(ω_x)=∠[G(jω_x)H(jω_x)]=kπ 时，称 ω_x 为穿越频率。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「穿越频率」
