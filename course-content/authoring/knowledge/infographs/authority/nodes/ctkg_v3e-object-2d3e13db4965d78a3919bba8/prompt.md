...
- One-line definition (source of truth): 应用MATLAB软件，调用命令ode45及plot，可解出系统微分方程高阶数值，并在线性坐标平面上方便地绘出相应的相轨迹。
- Core intuition: 在图谱邻接中可把握：后续 → 相轨迹、相平面法。
- Relation summary: 后续 → 相轨迹、相平面法
- Extended explanation (do not paste verbatim on image): 应用MATLAB软件，调用命令ode45及plot，可解出系统微分方程高阶数值，并在线性坐标平面上方便地绘出相应的相轨迹。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「MATLAB绘制相轨迹」、「用于分析·相轨迹」、「关联·相平面法」
