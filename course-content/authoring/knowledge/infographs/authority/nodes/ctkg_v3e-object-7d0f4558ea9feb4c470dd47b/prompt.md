...
- One-line definition (source of truth): 对于式（9－81）所示线性时变系统，如果取定初始时刻 t0∈Tt，存在一个有限时刻 t1∈Tt, t1>t0，对于所有 t∈[t0, t1]，系统的输出 y(t) 能唯一确定状态向量的初值 x(t0)，则称系统在 [t0, t1] 内是完…
- Core intuition: 如果对于一切 t1>t0系统都是可观测的，则称系统在 [t0, ∞) 内完全可观测。
- Relation summary: 后续 → 可观测性
- Extended explanation (do not paste verbatim on image): 对于式（9－81）所示线性时变系统，如果取定初始时刻 t0∈Tt，存在一个有限时刻 t1∈Tt, t1>t0，对于所有 t∈[t0, t1]，系统的输出 y(t) 能唯一确定状态向量的初值 x(t0…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「系统完全可观测」、「是一种·可观测性」、「属于·可观测性」
