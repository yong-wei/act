...
- One-line definition (source of truth): Modal form：A state-space realization where the A matrix is diagonal (or block diagonal for complex poles).
- Core intuition: 在图谱邻接中可把握：前置 → Transformation t。
- Relation summary: 前置 → Transformation t
- Extended explanation (do not paste verbatim on image): A state-space realization where the A matrix is diagonal (or block diagonal for complex poles).
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Modal form」、「关联·Transf…」
