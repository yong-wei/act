...
- One-line definition (source of truth): Bryson's Rule：A method to start the LQR design iteration by initially choosing diagonal matrices Q and R such that Q_ii = 1 / maximum…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A method to start the LQR design iteration by initially choosing diagonal matrices Q and R such tha…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Bryson's Ru…」、「Bryson's Rul」
