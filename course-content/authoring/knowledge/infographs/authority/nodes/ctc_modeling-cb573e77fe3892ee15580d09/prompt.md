...
## Subject
- Title: 状态空间表达式中q×n的系数矩阵 (modeling_cb573e77fe3892ee15580d09)
- Concept kind: theoretical_construct
- One-line definition (source of truth): 状态空间表达式中q×n的系数矩阵，也称输出矩阵。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 状态空间表达式中q×n的系数矩阵，也称输出矩阵。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「状态空间表达式中q×n…」
