...
## Subject
- Title: Unit circle in z-plane
- Concept kind: unknown
- One-line definition (source of truth): Unit circle in z-plane是自动控制原理权威图谱中的领域概念。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → 朱利稳定判据
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Unit circle…」、「Unit circle 」、「用于分析·朱利稳定…」
