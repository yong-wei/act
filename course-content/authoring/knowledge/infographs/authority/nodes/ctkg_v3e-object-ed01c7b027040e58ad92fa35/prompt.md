...
- One-line definition (source of truth): 输入与输出关系不随时间而改变的线性离散系统，称为线性定常离散系统。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 迭代法、z变换法、加权序列 · 后续 → 线性离散系统
- Extended explanation (do not paste verbatim on image): 输入与输出关系不随时间而改变的线性离散系统，称为线性定常离散系统。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「线性定常离散系统」、「是一种·线性离散系统」、「用于分析·迭代法」、「用于分析·z变换法」、「关联·加权序列」、「关联·脉冲传递函数」
