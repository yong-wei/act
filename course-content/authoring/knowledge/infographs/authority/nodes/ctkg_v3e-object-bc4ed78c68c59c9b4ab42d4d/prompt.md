...
## Subject
- Title: Axis Shift Method for Relative Stability
- Concept kind: unknown
- One-line definition (source of truth): Axis Shift Method for Relative Stability：A method that extends the Routh-Hurwitz criterion to ascertain relative stability by shifting the s-plane axis using a…
- Core intuition: 在图谱邻接中可把握：后续 → Relative Stability。
- Relation summary: 后续 → Relative Stability
- Extended explanation (do not paste verbatim on image): A method that extends the Routh-Hurwitz criterion to ascertain relative stability by shifting the s…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Axis Shift …」、「用于分析·Rela…」
