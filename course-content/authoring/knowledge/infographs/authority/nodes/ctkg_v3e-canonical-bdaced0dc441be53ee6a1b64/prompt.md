...
- One-line definition (source of truth): Time invariance：If the input is delayed or shifted in time, then the output is unchanged except also being shifted by exactly the same…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): If the input is delayed or shifted in time, then the output is unchanged except also being shifted…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Time invari…」、「Time invaria」
