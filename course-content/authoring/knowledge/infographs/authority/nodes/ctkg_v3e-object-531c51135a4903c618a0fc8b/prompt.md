...
- One-line definition (source of truth): 闭环幅频特性达到极大值时的频率，记为ω_r。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 得到闭环对数幅频和相频曲线、谐振峰值 · 后续 → 带宽频率
- Extended explanation (do not paste verbatim on image): 闭环幅频特性达到极大值时的频率，记为ω_r
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「谐振频率」、「用于分析·得到闭环…」、「关联·谐振峰值」、「关联·得到闭环对数…」、「关联·带宽频率」
