...
## Subject
- Title: Normalized characteristic equation
- Concept kind: unknown
- One-line definition (source of truth): Normalized characteristic equation：Normalized characteristic equation
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → 特征方程
- Extended explanation (do not paste verbatim on image): Normalized characteristic equation
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Normalized …」、「派生自·特征方程」
