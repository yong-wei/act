...
## Subject
- Title: Truxal's Formula
- Concept kind: theoretical_construct
- One-line definition (source of truth): Truxal's Formula：A formula for the velocity constant of a Type 1 system in terms of the closed-loop poles and zeros.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A formula for the velocity constant of a Type 1 system in terms of the closed-loop poles and zeros.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Truxal's Fo…」、「Truxal's For」
