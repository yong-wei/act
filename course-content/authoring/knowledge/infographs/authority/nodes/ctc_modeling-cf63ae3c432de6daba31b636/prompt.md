...
## Subject
- Title: 通过选择状态变量 $x_1 =… (modeling_cf63ae3c432de6daba31b636)
- Concept kind: analysis_method
- One-line definition (source of truth): 通过选择状态变量 $x_1 = y - h_0 u$, $x_i = \dot{x}_{i-1} - h_{i-1} u$ 来避免状态方程中出现输入导数项，并确定待定常数 $h_i$ 的方法。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 通过选择状态变量 $x_1 = y - h_0 u$, $x_i = \dot{x}_{i-1} - h_{i-1} u$ 来避免状态方程中出现输入导数项，并确定待定常数 $h_i$ 的方法。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「通过选择状态变量 $x…」
