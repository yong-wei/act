...
## Subject
- Title: Symmetric Root Locus (SRL)
- Concept kind: unknown
- One-line definition (source of truth): Symmetric Root Locus (SRL)：A root-locus problem with respect to the parameter ρ, which weighs the relative cost of tracking error z^2 with respect…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A root-locus problem with respect to the parameter ρ, which weighs the relative cost of tracking er…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Symmetric R…」
