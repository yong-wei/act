...
## Subject
- Title: Qualitative anal (Qualitative analysis using pole-zero locations)
- Concept kind: analysis_method
- One-line definition (source of truth): Qualitative anal：Rather than inverting Y(s), one can use prior knowledge and intuition about the effects of pole and zero locations in Y…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): Rather than inverting Y(s), one can use prior knowledge and intuition about the effects of pole and…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Qualitative…」
