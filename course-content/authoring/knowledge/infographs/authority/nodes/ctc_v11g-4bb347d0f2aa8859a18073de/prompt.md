...
## Subject
- Title: seven steps for sketching a root locus (seven_steps_for_sketching_a_root_locus)
- Concept kind: analysis_method
- One-line definition (source of truth): seven steps for sketching a root locus：A seven-step procedure for sketching a root locus, including preparing the sketch, locating real-axis segments, determi…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A seven-step procedure for sketching a root locus, including preparing the sketch, locating real-ax…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「seven steps…」
