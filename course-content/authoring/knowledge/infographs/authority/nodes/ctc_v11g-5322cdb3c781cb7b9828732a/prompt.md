...
## Subject
- Title: nonminimum phase system (nonminimum_phase_system)
- Concept kind: system_kind
- One-line definition (source of truth): nonminimum phase system：A system whose transfer function has a zero in the right half-plane (RHP).
- Core intuition: 在图谱邻接中可把握：后续 → 零点、negative gain root locus。
- Relation summary: 后续 → 零点、negative gain root locus
- Extended explanation (do not paste verbatim on image): A system whose transfer function has a zero in the right half-plane (RHP).
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「nonminimum …」、「nonminimum p」、「关联·零点」、「关联·negati…」
