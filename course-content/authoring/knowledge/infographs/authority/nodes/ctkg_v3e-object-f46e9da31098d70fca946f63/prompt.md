...
## Subject
- Title: programmable read-only memory (EPROM)
- Concept kind: unknown
- One-line definition (source of truth): programmable read-only memory (EPROM)：programmable read-only memory (EPROM)
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → read-only memory (ROM)
- Extended explanation (do not paste verbatim on image): programmable read-only memory (EPROM)
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「programmabl…」、「一种·read-o…」
