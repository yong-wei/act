...
## Subject
- Title: Acceleration Error Constant (K a) (Acceleration Error Constant (K_a))
- Concept kind: theoretical_construct
- One-line definition (source of truth): Acceleration Error Constant (K a)：For a Type 2 system (two integrators in the loop transfer function), the constant defined as K_a = lim_{s->0} s^2 G D_c…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): For a Type 2 system (two integrators in the loop transfer function), the constant defined as K_a =…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Acceleratio…」
