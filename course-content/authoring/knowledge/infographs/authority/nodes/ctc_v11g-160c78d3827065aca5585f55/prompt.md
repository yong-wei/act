...
## Subject
- Title: evans root locus construction method (evans_root_locus_construction_method)
- Concept kind: analysis_method
- One-line definition (source of truth): evans root locus construction method：A set of rules to construct the root-locus graph, used for selecting parameter values and designing dynamic compensatio…
- Core intuition: 在图谱邻接中可把握：后续 → 传递函数。
- Relation summary: 后续 → 传递函数
- Extended explanation (do not paste verbatim on image): A set of rules to construct the root-locus graph, used for selecting parameter values and designing…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「evans root …」、「应用于·传递函数」
