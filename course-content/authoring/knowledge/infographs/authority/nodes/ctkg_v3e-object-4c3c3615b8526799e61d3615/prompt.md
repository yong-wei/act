...
## Subject
- Title: Closed-Loop Estimator
- Concept kind: unknown
- One-line definition (source of truth): Closed-Loop Estimator：An estimator that feeds back the difference between measured and estimated outputs to correct the model continuously.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): An estimator that feeds back the difference between measured and estimated outputs to correct the m…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Closed-Loop…」、「Closed-Loop 」
