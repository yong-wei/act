...
## Subject
- Title: 根轨迹法 (root_locus_method)
- Concept kind: analysis_method
- One-line definition (source of truth): 根轨迹法：A graphical method for sketching the locus of roots in the s-plane as a parameter is varied, used to analyze the relati…
- Core intuition: 在图谱邻接中可把握：前置 → Root locus z-plane method、Root-Locus Techn、闭环极点 · 后续 → root of the denominator、特征方程、传递函数。
- Relation summary: 前置 → Root locus z-plane method、Root-Locus Techn、闭环极点 · 后续 → root of the denominator、特征方程、传递函数
- Extended explanation (do not paste verbatim on image): A graphical method for sketching the locus of roots in the s-plane as a parameter is varied, used t…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「根轨迹法」、「root locus m」、「用于分析·root…」、「用于分析·特征方程」、「应用于·传递函数」、「应用于·根轨迹」、「派生自·Root …」
