...
- One-line definition (source of truth): 完全可达。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 状态完全可达
- Extended explanation (do not paste verbatim on image): 完全可达
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「完全可达」、「是一种·状态完全可达」
