...
- One-line definition (source of truth): 在采样控制系统中，把连续信号转变为脉冲序列的过程称为采样过程，简称采样。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 采样控制系统 · 后续 → 采样控制系统、模拟信号、保持器
- Extended explanation (do not paste verbatim on image): 在采样控制系统中，把连续信号转变为脉冲序列的过程称为采样过程，简称采样。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「采样过程」、「属于·采样控制系统」、「包含组件·采样控制…」、「关联·模拟信号」、「关联·保持器」、「关联·采样器」
