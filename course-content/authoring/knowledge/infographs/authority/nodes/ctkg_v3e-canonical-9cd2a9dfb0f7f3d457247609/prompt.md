...
- One-line definition (source of truth): 响应曲线达到并保持在稳态值允许误差范围内所需的最短时间。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 动态性能指标
- Extended explanation (do not paste verbatim on image): 响应曲线达到并保持在稳态值允许误差范围内所需的最短时间。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「调节时间」、「包含组件·动态性能…」
