...
- One-line definition (source of truth): Estimation error：The difference between the actual state x(t) and the estimated state hat{x}(t), denoted e(t) = x(t) - hat{x}(t).
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The difference between the actual state x(t) and the estimated state hat{x}(t), denoted e(t) = x(t)…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Estimation …」、「Estimation e」
