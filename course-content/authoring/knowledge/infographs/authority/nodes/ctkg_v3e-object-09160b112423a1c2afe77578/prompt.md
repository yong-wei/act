...
## Subject
- Title: Maximum phase lead frequency ω m (Maximum phase lead frequency ω_m)
- Concept kind: unknown
- One-line definition (source of truth): Maximum phase lead frequency ω m：The frequency at which maximum additional phase lead is obtained, located at ω_m = √(z p), halfway between the pole and…
- Core intuition: 在图谱邻接中可把握：后续 → Phase-lead compensator。
- Relation summary: 后续 → Phase-lead compensator
- Extended explanation (do not paste verbatim on image): The frequency at which maximum additional phase lead is obtained, located at ω_m = √(z p), halfway…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Maximum pha…」、「应用于·Phase…」
