...
- One-line definition (source of truth): Phase stabilization：A stabilization method that uses specially tailored compensation (e.g., a notch filter) to change the phase at a specif…
- Core intuition: 在图谱邻接中可把握：前置 → Phase stabilizat。
- Relation summary: 前置 → Phase stabilizat
- Extended explanation (do not paste verbatim on image): A stabilization method that uses specially tailored compensation (e.g., a notch filter) to change t…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Phase stabi…」、「Phase stabil」、「包含组件·Phas…」
