...
- One-line definition (source of truth): 串级控制系统在结构上比单回路控制系统多了一个副回路，因而对进入副回路的二次扰动有很强的抑制能力。
- Core intuition: 在图谱邻接中可把握：后续 → 副回路。
- Relation summary: 后续 → 副回路
- Extended explanation (do not paste verbatim on image): 串级控制系统在结构上比单回路控制系统多了一个副回路，因而对进入副回路的二次扰动有很强的抑制能力。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「串级控制方法」、「包含组件·副回路」
