...
## Subject
- Title: state space model (state_space_model)
- Concept kind: representation_kind
- One-line definition (source of truth): state space model：v3T accepted candidate for state space model.
- Core intuition: 在图谱邻接中可把握：前置 → Lyapunov function、李雅普诺夫函数候选、李雅普诺夫函数。
- Relation summary: 前置 → Lyapunov function、李雅普诺夫函数候选、李雅普诺夫函数
- Extended explanation (do not paste verbatim on image): v3T accepted candidate for state space model.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「state space…」、「state space 」、「用于分析·Lyap…」、「用于分析·李雅普诺…」
