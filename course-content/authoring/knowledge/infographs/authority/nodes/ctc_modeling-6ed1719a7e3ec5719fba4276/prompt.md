...
## Subject
- Title: An matrix in the (modeling_6ed1719a7e3ec5719fba4276)
- Concept kind: theoretical_construct
- One-line definition (source of truth): An matrix in the：An n x n matrix in the state-variable form.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): An n x n matrix in the state-variable form.
