...
## Subject
- Title: 利用开环频域指标估算时域性能方法
- Concept kind: unknown
- One-line definition (source of truth): 通过相角裕度γ和截止频率ω_c估算系统时域性能指标。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 相角裕度、截止频率 · 后续 → 相角裕度、截止频率
- Extended explanation (do not paste verbatim on image): 通过相角裕度γ和截止频率ω_c估算系统时域性能指标
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「利用开环频域指标估算时…」、「用于分析·相角裕度」、「用于分析·截止频率」、「关联·相角裕度」、「关联·截止频率」
