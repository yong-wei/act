...
- One-line definition (source of truth): Prefilter：A prefilter is a transfer function G_p(s) placed before the compensator in a control system to shape the reference inpu…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A prefilter is a transfer function G_p(s) placed before the compensator in a control system to shap…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Prefilter」
