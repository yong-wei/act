...
- One-line definition (source of truth): 当采样开关和系统其余部分的传递函数都具有线性特性时，这样的系统就称为线性采样系统。
- Core intuition: 在图谱邻接中可把握：后续 → 采样控制系统、采样开关。
- Relation summary: 后续 → 采样控制系统、采样开关
- Extended explanation (do not paste verbatim on image): 当采样开关和系统其余部分的传递函数都具有线性特性时，这样的系统就称为线性采样系统。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「线性采样系统」、「是一种·采样控制系统」、「包含组件·采样开关」
