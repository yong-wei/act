...
- One-line definition (source of truth): Neutral Stability：A position where if a cone rests on its side and is displaced slightly, it rolls with no tendency to leave the position…
- Core intuition: 在图谱邻接中可把握：前置 → Stability boundary · 后续 → Stable System、稳定性。
- Relation summary: 前置 → Stability boundary · 后续 → Stable System、稳定性
- Extended explanation (do not paste verbatim on image): A position where if a cone rests on its side and is displaced slightly, it rolls with no tendency t…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Neutral Sta…」、「Neutral Stab」、「一种·Stable」、「是一种·稳定性」、「关联·Stabil…」
