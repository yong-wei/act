...
- One-line definition (source of truth): ω_n = √(K/T_m) 为自然频率（或无阻尼振荡频率）。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): ω_n = √(K/T_m) 为自然频率（或无阻尼振荡频率）。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「自然频率」
