...
- One-line definition (source of truth): Absolute Stability：A type of stability characterization where a closed-loop feedback system is either stable or not stable. A system posse…
- Core intuition: 在图谱邻接中可把握：后续 → Stable System、稳定性。
- Relation summary: 后续 → Stable System、稳定性
- Extended explanation (do not paste verbatim on image): A type of stability characterization where a closed-loop feedback system is either stable or not st…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Absolute St…」、「Absolute Sta」、「一种·Stable」、「是一种·稳定性」
