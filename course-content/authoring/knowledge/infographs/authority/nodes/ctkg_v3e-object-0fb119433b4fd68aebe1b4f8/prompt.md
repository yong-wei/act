...
## Subject
- Title: 离散系统的可控性矩阵 (dis…
- Concept kind: unknown
- One-line definition (source of truth): 离散系统的可控性矩阵 (discrete-time controllability matrix S1)。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 离散系统的可控性矩阵 (discrete-time controllability matrix S1)
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「离散系统的可控性矩阵 …」
