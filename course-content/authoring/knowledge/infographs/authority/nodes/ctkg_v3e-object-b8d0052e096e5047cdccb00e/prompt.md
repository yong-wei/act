...
- One-line definition (source of truth): Linear Quadratic Regulator (LQR)：Linear Quadratic Regulator (LQR)
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → Symmetric Root Locus (SRL) Method
- Extended explanation (do not paste verbatim on image): Linear Quadratic Regulator (LQR)
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Linear Quad…」、「应用于·Symme…」
