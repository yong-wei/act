...
- One-line definition (source of truth): Phase (frequency response)：φ is referred to as the phase and is a function of the input frequency ω.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): φ is referred to as the phase and is a function of the input frequency ω.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Phase (freq…」
