...
- One-line definition (source of truth): Estimator Error：The difference between the true state and the estimated state, defined as x̃ ≜ x - x̂.
- Core intuition: 在图谱邻接中可把握：前置 → Estimator Error Poles。
- Relation summary: 前置 → Estimator Error Poles
- Extended explanation (do not paste verbatim on image): The difference between the true state and the estimated state, defined as x̃ ≜ x - x̂.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Estimator E…」、「Estimator Er」、「应用于·Estim…」
