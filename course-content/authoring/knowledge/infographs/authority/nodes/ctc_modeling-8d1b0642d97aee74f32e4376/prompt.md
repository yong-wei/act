...
## Subject
- Title: 考虑初始条件 (modeling_8d1b0642d97aee74f32e4376)
- Concept kind: analysis_method
- One-line definition (source of truth): 考虑初始条件，对微分方程中的每一项分别进行拉氏变换，将微分方程转换为变量s的代数方程；由代数方程求出输出量拉氏变换函数的表达式；对输出量拉氏变换函数求反变换，得到输出量的时域表达式，即为所求微分方程的解。
- Core intuition: 由代数方程求出输出量拉氏变换函数的表达式。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 考虑初始条件，对微分方程中的每一项分别进行拉氏变换，将微分方程转换为变量s的代数方程；由代数方程求出输出量拉氏变换函数的表达式；对输出量拉氏变换函数求反变换，得到输出量的时域表达式，即为所求微分方程…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「考虑初始条件」
