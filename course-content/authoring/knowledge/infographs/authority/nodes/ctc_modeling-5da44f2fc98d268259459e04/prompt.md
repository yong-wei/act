...
## Subject
- Title: impulse response (impulse_response)
- Concept kind: theoretical_construct
- One-line definition (source of truth): impulse response：v3T accepted candidate for impulse response.
- Core intuition: 在图谱邻接中可把握：前置 → 传递函数 · 后续 → Natural response。
- Relation summary: 前置 → 传递函数 · 后续 → Natural response
- Extended explanation (do not paste verbatim on image): v3T accepted candidate for impulse response.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「impulse res…」、「impulse resp」、「一种·Natura…」、「关联·传递函数」
