...
- One-line definition (source of truth): Compensation：The alteration or adjustment of a control system in order to provide a suitable performance
- Core intuition: 在图谱邻接中可把握：前置 → Output Compensation、Input Compensation、Feedback Compensation。
- Relation summary: 前置 → Output Compensation、Input Compensation、Feedback Compensation
- Extended explanation (do not paste verbatim on image): The alteration or adjustment of a control system in order to provide a suitable performance
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Compensation」、「一种·Output」、「一种·Input」、「一种·Feedba…」、「关联·Compen…」
