...
## Subject
- Title: Signal-flow graph state model
- Concept kind: unknown
- One-line definition (source of truth): Signal-flow graph state model：A graphical representation of the state model that includes initial conditions of each state variable at the output of…
- Core intuition: 在图谱邻接中可把握：后续 → State transition。
- Relation summary: 后续 → State transition
- Extended explanation (do not paste verbatim on image): A graphical representation of the state model that includes initial conditions of each state variab…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Signal-flow…」、「用于分析·Stat…」
