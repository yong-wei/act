...
- One-line definition (source of truth): 能量函数（李雅普诺夫）是自动控制原理权威图谱中的领域概念。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 李雅普诺夫函数候选、李雅普诺夫函数
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「能量函数（李雅普诺夫）」、「一种·李雅普诺夫函…」、「一种·李雅普诺夫函数」
