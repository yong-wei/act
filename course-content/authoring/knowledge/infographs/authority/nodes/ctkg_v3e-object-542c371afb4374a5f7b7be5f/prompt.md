...
- One-line definition (source of truth): 由幂级数 \sum_{k=0}^{\infty} \frac{1}{k。
- Core intuition: 在图谱邻接中可把握：前置 → 状态转移矩阵。
- Relation summary: 前置 → 状态转移矩阵
- Extended explanation (do not paste verbatim on image): 由幂级数 \sum_{k=0}^{\infty} \frac{1}{k
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「矩阵指数函数」、「是一种·状态转移矩阵」
