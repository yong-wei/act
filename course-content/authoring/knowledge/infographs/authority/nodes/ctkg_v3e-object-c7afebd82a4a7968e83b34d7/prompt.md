...
- One-line definition (source of truth): Lur'e problem：A nonlinear system with only one single-input-single-output nonlinearity, studied by the Soviet scientist Lur'e.
- Core intuition: 在图谱邻接中可把握：前置 → Aizermann's conjecture · 后续 → Aizermann's conjecture。
- Relation summary: 前置 → Aizermann's conjecture · 后续 → Aizermann's conjecture
- Extended explanation (do not paste verbatim on image): A nonlinear system with only one single-input-single-output nonlinearity, studied by the Soviet sci…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Lur'e probl…」、「Lur'e proble」、「应用于·Aizer…」、「关联·Aizerm…」
