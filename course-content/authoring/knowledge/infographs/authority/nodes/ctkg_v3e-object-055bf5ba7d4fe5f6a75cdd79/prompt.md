...
## Subject
- Title: Type-two system
- Concept kind: unknown
- One-line definition (source of truth): Type-two system：A type-two system has satisfactory steady-state error for both step and ramp input signals.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A type-two system has satisfactory steady-state error for both step and ramp input signals.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Type-two sy…」、「Type-two sys」
