...
## Subject
- Title: Settling time T s (Settling time T_s)
- Concept kind: theoretical_construct
- One-line definition (source of truth): Settling time T s：The time required for the system to settle within a certain percentage δ of the input amplitude.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The time required for the system to settle within a certain percentage δ of the input amplitude.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Settling ti…」、「Settling tim」
