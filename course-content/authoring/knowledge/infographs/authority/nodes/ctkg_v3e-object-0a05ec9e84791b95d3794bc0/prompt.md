...
- One-line definition (source of truth): Compensator：An additional component that is inserted into a control system to compensate for a deficient performance.
- Core intuition: 在图谱邻接中可把握：前置 → Integration compensator、Cascade Compensator、Phase-lead compensator · 后续 → 根轨迹、Compensation。
- Relation summary: 前置 → Integration compensator、Cascade Compensator、Phase-lead compensator · 后续 → 根轨迹、Compensation
- Extended explanation (do not paste verbatim on image): An additional component that is inserted into a control system to compensate for a deficient perfor…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Compensator」、「一种·Integr…」、「一种·Cascad…」、「一种·Phase-…」、「应用于·根轨迹法」、「应用于·Frequ…」
