...
- One-line definition (source of truth): relay：relay
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → memoryless nonlinearity
- Extended explanation (do not paste verbatim on image): relay
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「relay」、「一种·memory…」
