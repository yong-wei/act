...
- One-line definition (source of truth): 零阶保持器是一种按常值外推的保持器，它把前一采样时刻nT的采样值e(nT)一直保持到下一采样时刻(n+1)T到来之前。
- Core intuition: 在图谱邻接中可把握：后续 → 保持器、一阶保持器。
- Relation summary: 后续 → 保持器、一阶保持器
- Extended explanation (do not paste verbatim on image): 零阶保持器是一种按常值外推的保持器，它把前一采样时刻nT的采样值e(nT)一直保持到下一采样时刻(n+1)T到来之前。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「零阶保持器」、「是一种·保持器」、「关联·一阶保持器」
