...
## Subject
- Title: When the sensor and (modeling_352b9bfe27ba1ab6b9c9672e)
- Concept kind: theoretical_construct
- One-line definition (source of truth): When the sensor and：When the sensor and actuator are rigidly attached to one another, the situation is referred to as the collocated case.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): When the sensor and actuator are rigidly attached to one another, the situation is referred to as t…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「When the se…」
