...
- One-line definition (source of truth): Relative Stability：Given that a closed-loop system is stable, the degree of stability can be further characterized. This is referred to as…
- Core intuition: 在图谱邻接中可把握：前置 → Axis Shift Method for Relative Stability。
- Relation summary: 前置 → Axis Shift Method for Relative Stability
- Extended explanation (do not paste verbatim on image): Given that a closed-loop system is stable, the degree of stability can be further characterized. Th…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Relative St…」、「Relative Sta」、「用于分析·Axis…」
