...
- One-line definition (source of truth): 系统受到有界扰动作用偏离了原平衡状态，不论扰动引起的初始偏差有多大，当扰动取消后，系统都能以足够的准确度恢复到初始平衡状态。
- Core intuition: 在图谱邻接中可把握：后续 → 稳定性。
- Relation summary: 后续 → 稳定性
- Extended explanation (do not paste verbatim on image): 系统受到有界扰动作用偏离了原平衡状态，不论扰动引起的初始偏差有多大，当扰动取消后，系统都能以足够的准确度恢复到初始平衡状态。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「大范围稳定系统」、「是一种·稳定性」
