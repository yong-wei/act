...
- One-line definition (source of truth): PWM放大器。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 复合控制信号 · 后续 → 前馈信号
- Extended explanation (do not paste verbatim on image): PWM放大器
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「PWM放大器」、「包含组件·复合控制…」、「关联·前馈信号」
