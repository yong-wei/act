...
## Subject
- Title: Pole placement using place() function
- Concept kind: unknown
- One-line definition (source of truth): Pole placement using place() function：A method to compute state feedback gain K or estimator gain L given desired pole locations.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A method to compute state feedback gain K or estimator gain L given desired pole locations.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Pole placem…」
