...
## Subject
- Title: Controllability  (Controllability Gramian matrix criterion method)
- Concept kind: unknown
- One-line definition (source of truth): Controllability ：Controllability Gramian matrix criterion method
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): Controllability Gramian matrix criterion method
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Controllabi…」
