...
## Subject
- Title: 通过变量替换检验给定稳定度的方法
- Concept kind: unknown
- One-line definition (source of truth): 通过变量替换检验给定稳定度的方法。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 给定稳定度检验方法
- Extended explanation (do not paste verbatim on image): 通过变量替换检验给定稳定度的方法
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「通过变量替换检验给定稳…」、「包含组件·给定稳定…」
