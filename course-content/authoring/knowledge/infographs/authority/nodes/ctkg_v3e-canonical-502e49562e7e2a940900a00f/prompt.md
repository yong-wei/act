...
## Subject
- Title: Finding transfer (Finding transfer function by assuming exponential solution)
- Concept kind: analysis_method
- One-line definition (source of truth): Finding transfer：Assume a solution of the form y(t)=H(s)e^{st}, substitute into the differential equation, and solve for H(s).
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): Assume a solution of the form y(t)=H(s)e^{st}, substitute into the differential equation, and solve…
