...
- One-line definition (source of truth): 增广系统可控的条件是 rank 矩阵等于 n+1（阶跃输入）或 n+2（斜坡输入），其中矩阵包含系统矩阵 A、输入矩阵 b 和输出矩阵 c 的乘积项。
- Core intuition: 在图谱邻接中可把握：后续 → 增广系统。
- Relation summary: 后续 → 增广系统
- Extended explanation (do not paste verbatim on image): 增广系统可控的条件是 rank 矩阵等于 n+1（阶跃输入）或 n+2（斜坡输入），其中矩阵包含系统矩阵 A、输入矩阵 b 和输出矩阵 c 的乘积项。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「增广系统可控性」、「应用于·增广系统」
