...
## Subject
- Title: set of state variables (modeling_8a0ff75844bf161eeea3c542)
- Concept kind: theoretical_construct
- One-line definition (source of truth): set of state variables：A set of state variables \(x_1(t), x_2(t), \ldots, x_n(t)\) that are the outputs of integrators in a chain, used in the…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A set of state variables \(x_1(t), x_2(t), \ldots, x_n(t)\) that are the outputs of integrators in…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「set of stat…」
