...
## Subject
- Title: Various levels of simulation (modeling_30d05b970a0d4a71c9d243c3)
- Concept kind: theoretical_construct
- One-line definition (source of truth): Various levels of simulation：Various levels of simulation fidelity (accuracy) are available to the control engineer: low-fidelity simulations in ear…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): Various levels of simulation fidelity (accuracy) are available to the control engineer: low-fidelit…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Various lev…」
