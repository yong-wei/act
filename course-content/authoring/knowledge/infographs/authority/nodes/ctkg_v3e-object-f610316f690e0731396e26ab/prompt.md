...
## Subject
- Title: Airplane control system
- Concept kind: unknown
- One-line definition (source of truth): Airplane control system：Airplane control system
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): Airplane control system
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Airplane co…」、「Airplane con」
