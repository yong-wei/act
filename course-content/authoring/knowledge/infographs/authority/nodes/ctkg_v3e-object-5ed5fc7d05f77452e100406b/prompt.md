...
- One-line definition (source of truth): 双极限环。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → 稳定极限环、不稳定极限环
- Extended explanation (do not paste verbatim on image): 双极限环
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「双极限环」、「包含组件·稳定极限环」、「包含组件·不稳定极…」
