...
## Subject
- Title: Type-one system
- Concept kind: unknown
- One-line definition (source of truth): Type-one system：A system with one pole at the origin in the loop transfer function.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A system with one pole at the origin in the loop transfer function.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Type-one sy…」、「Type-one sys」
