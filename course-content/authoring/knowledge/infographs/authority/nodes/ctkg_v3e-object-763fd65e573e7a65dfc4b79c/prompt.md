...
## Subject
- Title: Nyquist图 (Nyquist plot)
- Concept kind: unknown
- One-line definition (source of truth): Nyquist图：A plot of the magnitude of KG(s) versus the angle of KG(s) used to evaluate the open-loop transfer function for stabili…
- Core intuition: 在图谱邻接中可把握：前置 → Frequency response method。
- Relation summary: 前置 → Frequency response method
- Extended explanation (do not paste verbatim on image): A plot of the magnitude of KG(s) versus the angle of KG(s) used to evaluate the open-loop transfer…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Nyquist图」、「Nyquist plot」、「关联·Freque…」
