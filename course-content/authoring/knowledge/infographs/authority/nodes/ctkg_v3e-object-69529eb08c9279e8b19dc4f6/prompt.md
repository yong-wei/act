...
- One-line definition (source of truth): 惯性环节的对数幅频渐近特性曲线低频部分和高频部分两条直线交于 ω=1/T 处，称频率 1/T 为惯性环节的交接频率。
- Core intuition: 在图谱邻接中可把握：后续 → 对数幅频渐近特性曲线。
- Relation summary: 后续 → 对数幅频渐近特性曲线
- Extended explanation (do not paste verbatim on image): 惯性环节的对数幅频渐近特性曲线低频部分和高频部分两条直线交于 ω=1/T 处，称频率 1/T 为惯性环节的交接频率。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「交接频率」、「属于·对数幅频渐近…」
