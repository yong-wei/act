...
- One-line definition (source of truth): Kalman filter：A time-varying optimal observer that solves the observer problem in the presence of measurement noise and process noise.
- Core intuition: 在图谱邻接中可把握：后续 → Full-state observer。
- Relation summary: 后续 → Full-state observer
- Extended explanation (do not paste verbatim on image): A time-varying optimal observer that solves the observer problem in the presence of measurement noi…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Kalman filt…」、「Kalman filte」、「一种·Full-s…」
