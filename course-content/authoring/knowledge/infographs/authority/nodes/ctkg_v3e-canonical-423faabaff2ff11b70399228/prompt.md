...
- One-line definition (source of truth): F_c = 2 sqrt(J K_1)，为阻尼比ζ=1时的阻尼系数。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): F_c = 2 sqrt(J K_1)，为阻尼比ζ=1时的阻尼系数。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「临界阻尼系数」
