...
- One-line definition (source of truth): 将系统按可观测性分解为可观测子系统和不可观测子系统的过程。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → 可观测性规范分解方法
- Extended explanation (do not paste verbatim on image): 将系统按可观测性分解为可观测子系统和不可观测子系统的过程
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「可观测性规范分解」、「一种·可观测性规范…」、「应用于·可观测性规…」
