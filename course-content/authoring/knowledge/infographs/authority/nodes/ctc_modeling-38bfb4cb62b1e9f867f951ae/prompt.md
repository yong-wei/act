...
## Subject
- Title: 输入向量至偏差向量之间的传递矩阵 (modeling_38bfb4cb62b1e9f867f951ae)
- Concept kind: theoretical_construct
- One-line definition (source of truth): 输入向量至偏差向量之间的传递矩阵。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 输入向量至偏差向量之间的传递矩阵。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「输入向量至偏差向量之间…」
