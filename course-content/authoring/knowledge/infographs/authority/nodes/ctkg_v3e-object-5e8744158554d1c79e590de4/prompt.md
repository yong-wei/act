...
- One-line definition (source of truth): Phase Margin (PM)：The amount by which the phase of G(jω) exceeds -180° when |KG(jω)|=1.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The amount by which the phase of G(jω) exceeds -180° when |KG(jω)|=1.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Phase Margi…」、「Phase Margin」
