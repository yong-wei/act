...
## Subject
- Title: Open-loop system poles location
- Concept kind: unknown
- One-line definition (source of truth): Open-loop system poles location：Open-loop system poles are located at approximately (10, 0), (-1, 0), and (-10, 0).
- Core intuition: 在图谱邻接中可把握：后续 → Open-loop system poles。
- Relation summary: 后续 → Open-loop system poles
- Extended explanation (do not paste verbatim on image): Open-loop system poles are located at approximately (10, 0), (-1, 0), and (-10, 0).
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Open-loop s…」、「有表示·Open-…」
