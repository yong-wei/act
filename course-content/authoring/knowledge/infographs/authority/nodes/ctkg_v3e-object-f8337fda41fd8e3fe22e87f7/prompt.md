...
- One-line definition (source of truth): multiplexer：A device used to sequentially connect a single A/D converter into multiple channels being sampled, rather than using mu…
- Core intuition: 在图谱邻接中可把握：后续 → A/D converter。
- Relation summary: 后续 → A/D converter
- Extended explanation (do not paste verbatim on image): A device used to sequentially connect a single A/D converter into multiple channels being sampled,…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「multiplexer」、「关联·A/D co…」
