...
## Subject
- Title: Pole-zero constellation
- Concept kind: theoretical_construct
- One-line definition (source of truth): Pole-zero constellation：Information about y(t) can be obtained from the pole-zero constellation of Y(s) without actually inverting it.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): Information about y(t) can be obtained from the pole-zero constellation of Y(s) without actually in…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Pole-zero c…」、「Pole-zero co」
