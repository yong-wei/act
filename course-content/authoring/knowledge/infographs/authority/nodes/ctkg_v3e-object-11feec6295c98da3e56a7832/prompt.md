...
## Subject
- Title: Estimator Gain S (Estimator Gain Selection via Coefficient Comparison)
- Concept kind: unknown
- One-line definition (source of truth): Estimator Gain S：Solve for L by comparing coefficients of the estimator error characteristic equation (det[sI-(A-LC)]=0) with the desire…
- Core intuition: 在图谱邻接中可把握：前置 → Estimator Gain S · 后续 → Estimator Gain S。
- Relation summary: 前置 → Estimator Gain S · 后续 → Estimator Gain S
- Extended explanation (do not paste verbatim on image): Solve for L by comparing coefficients of the estimator error characteristic equation (det[sI-(A-LC)…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Estimator G…」、「包含组件·Esti…」、「用于分析·Esti…」
