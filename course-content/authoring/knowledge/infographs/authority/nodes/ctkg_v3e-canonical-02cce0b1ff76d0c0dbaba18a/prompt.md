...
- One-line definition (source of truth): 一阶系统在单位加速度函数输入下的输出响应，表达式为c(t)=1/2 t^2 - T t + T^2(1-e^{-t/T})。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 一阶系统在单位加速度函数输入下的输出响应，表达式为c(t)=1/2 t^2 - T t + T^2(1-e^{-t/T})
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「一阶系统的单位加速度响应」
