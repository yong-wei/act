...
## Subject
- Title: 反应速率 (reaction_rate_r)
- Concept kind: performance_metric
- One-line definition (source of truth): 反应速率：The slope parameter characterizing the reaction curve, computed as R = M/τ where M is the steady-state magnitude and τ…
- Core intuition: 在图谱邻接中可把握：后续 → 反应曲线、反应曲线参数。
- Relation summary: 后续 → 反应曲线、反应曲线参数
- Extended explanation (do not paste verbatim on image): The slope parameter characterizing the reaction curve, computed as R = M/τ where M is the steady-st…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「反应速率」、「reaction rat」、「关联·反应曲线」、「关联·反应曲线参数」
