...
## Subject
- Title: 相距很近的闭环零点和极点 (pole_zero_dipole)
- Concept kind: mathematical_object
- One-line definition (source of truth): 相距很近的闭环零点和极点，若距离比模值小一个数量级则构成偶极子。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 相距很近的闭环零点和极点，若距离比模值小一个数量级则构成偶极子。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「相距很近的闭环零点和极点」、「pole zero di」
