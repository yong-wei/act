...
- One-line definition (source of truth): 由比例单元、微分单元和积分单元组合而成的有源串联校正装置，可以实现各种要求的控制规律，在工业自动化设备中经常采用。
- Core intuition: 在图谱邻接中可把握：后续 → 校正装置、比例系数、微分时间常数。
- Relation summary: 后续 → 校正装置、比例系数、微分时间常数
- Extended explanation (do not paste verbatim on image): 由比例单元、微分单元和积分单元组合而成的有源串联校正装置，可以实现各种要求的控制规律，在工业自动化设备中经常采用。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「PID控制器」、「是一种·校正装置」、「包含组件·比例系数」、「包含组件·微分时间…」、「包含组件·积分时间…」
