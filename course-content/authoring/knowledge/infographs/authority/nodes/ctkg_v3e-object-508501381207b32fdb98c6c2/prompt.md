...
- One-line definition (source of truth): 比例系数。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 前置 → PID控制器、比例控制规律、衰减曲线法
- Extended explanation (do not paste verbatim on image): 比例系数
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「比例系数」、「包含组件·PID控…」、「包含组件·比例控制…」、「包含组件·衰减曲线法」
