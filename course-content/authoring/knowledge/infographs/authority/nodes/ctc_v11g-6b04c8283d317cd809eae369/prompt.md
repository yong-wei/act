...
## Subject
- Title: notch compensation (notch_compensation)
- Concept kind: design_method
- One-line definition (source of truth): notch compensation：A compensation scheme used to achieve stability for systems with lightly damped flexible modes, particularly with nonco…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): A compensation scheme used to achieve stability for systems with lightly damped flexible modes, par…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「notch compe…」、「notch compen」
