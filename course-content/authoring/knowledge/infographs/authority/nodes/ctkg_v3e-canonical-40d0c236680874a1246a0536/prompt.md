...
## Subject
- Title: Damping ratio zeta
- Concept kind: theoretical_construct
- One-line definition (source of truth): Damping ratio zeta：The damping ratio reflects the level of damping as a fraction of the critical damping value where the poles become real.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The damping ratio reflects the level of damping as a fraction of the critical damping value where t…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Damping rat…」、「Damping rati」
