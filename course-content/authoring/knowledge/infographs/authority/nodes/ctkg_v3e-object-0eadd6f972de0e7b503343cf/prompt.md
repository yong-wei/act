...
## Subject
- Title: PID controller d (PID controller design procedure for spacecraft attitude control)
- Concept kind: unknown
- One-line definition (source of truth): PID controller d：The design procedure involves: 1) Adjusting T_D to achieve the desired phase margin (PM) at a reasonably high frequency…
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): The design procedure involves: 1) Adjusting T_D to achieve the desired phase margin (PM) at a reaso…
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「PID control…」
