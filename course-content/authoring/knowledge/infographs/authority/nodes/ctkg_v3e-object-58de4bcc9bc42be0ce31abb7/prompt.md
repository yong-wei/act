...
## Subject
- Title: Bounded Input Bo (Bounded Input Bounded Output (BIBO) Stability)
- Concept kind: unknown
- One-line definition (source of truth): Bounded Input Bo：A system is stable if, for any bounded input, the output remains bounded.
- Core intuition: 在图谱邻接中可把握：后续 → Stable System。
- Relation summary: 后续 → Stable System
- Extended explanation (do not paste verbatim on image): A system is stable if, for any bounded input, the output remains bounded.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Bounded Inp…」、「一种·Stable」
