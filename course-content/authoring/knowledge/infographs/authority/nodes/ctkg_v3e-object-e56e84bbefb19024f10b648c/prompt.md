...
- One-line definition (source of truth): 系统受到有界扰动作用后，只有当扰动引起的初始偏差小于某一范围时，系统才能在取消扰动后恢复到初始平衡状态，否则就不能恢复到初始平衡状态。
- Core intuition: 在图谱邻接中可把握：后续 → Stable System、稳定性。
- Relation summary: 后续 → Stable System、稳定性
- Extended explanation (do not paste verbatim on image): 系统受到有界扰动作用后，只有当扰动引起的初始偏差小于某一范围时，系统才能在取消扰动后恢复到初始平衡状态，否则就不能恢复到初始平衡状态。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「小范围稳定系统」、「一种·Stable」、「是一种·稳定性」
