...
- One-line definition (source of truth): z变换法。
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → 线性定常离散系统
- Extended explanation (do not paste verbatim on image): z变换法
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「z变换法」、「用于分析·线性定常…」
