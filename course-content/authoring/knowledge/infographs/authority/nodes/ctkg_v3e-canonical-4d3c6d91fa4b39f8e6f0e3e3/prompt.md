...
## Subject
- Title: Left half-plane (LHP)
- Concept kind: theoretical_construct
- One-line definition (source of truth): Left half-plane (LHP)：Left half-plane (LHP)
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: 后续 → Right half-plane (RHP)
- Extended explanation (do not paste verbatim on image): Left half-plane (LHP)
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Left half-p…」、「Left half-pl」、「关联·Right …」
