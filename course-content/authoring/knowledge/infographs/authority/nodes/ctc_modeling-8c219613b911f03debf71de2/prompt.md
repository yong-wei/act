...
## Subject
- Title: 先列写网络的微分方程 (modeling_8c219613b911f03debf71de2)
- Concept kind: analysis_method
- One-line definition (source of truth): 求取无源网络传递函数的两种方法：一是先列写网络的微分方程，然后在零初始条件下进行拉氏变换；二是引用复数阻抗直接列写网络的代数方程，然后求其传递函数。
- Core intuition: 二是引用复数阻抗直接列写网络的代数方程，然后求其传递函数。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): 求取无源网络传递函数的两种方法：一是先列写网络的微分方程，然后在零初始条件下进行拉氏变换；二是引用复数阻抗直接列写网络的代数方程，然后求其传递函数。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「先列写网络的微分方程」
