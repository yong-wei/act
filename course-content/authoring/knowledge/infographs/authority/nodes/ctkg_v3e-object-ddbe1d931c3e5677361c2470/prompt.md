...
## Subject
- Title: Position error constant
- Concept kind: unknown
- One-line definition (source of truth): Position error constant：K=K_p (the position error constant) that appears as the low-frequency gain on the Bode plot.
- Core intuition: 抓住定义核心与邻接关系，不扩展未给出的事实。
- Relation summary: （权威图邻接待补充）
- Extended explanation (do not paste verbatim on image): K=K_p (the position error constant) that appears as the low-frequency gain on the Bode plot.
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「Position er…」、「Position err」
