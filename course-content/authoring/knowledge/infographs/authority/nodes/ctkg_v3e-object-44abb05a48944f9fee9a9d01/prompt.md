...
- One-line definition (source of truth): 根据非线性的分段特点，将相平面分成若干区域进行研究，可使非线性微分方程在各个区域表现为线性微分方程，再应用线性系统的相平面分析方法。
- Core intuition: 在图谱邻接中可把握：前置 → 开关线 · 后续 → 非线性系统。
- Relation summary: 前置 → 开关线 · 后续 → 非线性系统
- Extended explanation (do not paste verbatim on image): 根据非线性的分段特点，将相平面分成若干区域进行研究，可使非线性微分方程在各个区域表现为线性微分方程，再应用线性系统的相平面分析方法。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「相平面法分析非线性系统」、「属于·开关线」、「用于分析·非线性系统」
