...
- One-line definition (source of truth): 相平面法是推广应用时域分析法的一种图解分析方法。
- Core intuition: 该方法通过在相平面上绘制相轨迹曲线，确定非线性微分方程在不同初始条件下解的运动形式。
- Relation summary: 前置 → MATLAB绘制相轨迹 · 后续 → 相平面、非线性系统、相轨迹
- Extended explanation (do not paste verbatim on image): 相平面法是推广应用时域分析法的一种图解分析方法。
...
- Allowed visible labels (max ~10, each preferably ≤10 Chinese characters): 「相平面法」、「包含组件·相平面」、「用于分析·非线性系统」、「应用于·非线性系统」、「关联·MATLAB…」、「有表示·相轨迹」
