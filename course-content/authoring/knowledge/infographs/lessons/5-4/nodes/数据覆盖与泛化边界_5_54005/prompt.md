...
一句话定义：
数据覆盖与泛化边界判断样本是否覆盖关键工况，以及方法在未见但合理可能出现的场景中是否仍可信。

核心直觉：


可用公式（内部依据，图面公式区标题只能写“公式”）：
\mathcal{D}_{\mathrm{train}}\supset \mathcal{B}_{\mathrm{critical}}

本课关系：
- 数据覆盖与泛化边界 --引出--> 模型数据责任分配
- 数据驱动进入条件 --包含--> 数据覆盖与泛化边界
- 数据驱动进入条件 --包含--> 数据覆盖与泛化边界
- 数据覆盖与泛化边界 --引出--> 模型数据责任分配

建议视觉骨架：
用训练数据覆盖区、关键边界工况和未见相邻场景表达样本覆盖不足时的泛化风险。
...
text_contract: 只允许围绕这些标签组织图面文字：数据覆盖与泛化边界、数据覆盖、泛化、边界工况、故障注入、数据质量、泛化风险；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：数据覆盖与泛化边界、数据覆盖、泛化、边界工况、故障注入、数据质量、泛化风险。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：\mathcal{D}_{\mathrm{train}}\supset \mathcal{B}_{\mathrm{critical}}
