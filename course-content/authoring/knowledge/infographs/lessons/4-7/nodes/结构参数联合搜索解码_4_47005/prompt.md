...
一句话定义：
把结构编号和连续控制参数统一编码，通过粗搜与精修得到候选，再解码回可解释的经典控制结构。

核心直觉：
优化设计不是把控制器黑箱化，而是让“选结构”和“定参数”在同一证据链中接受比较。
...
- 工程指标代价函数翻译 --引出--> 结构参数联合搜索解码
- 工程指标代价函数翻译 --引出--> 结构参数联合搜索解码: 优化设计必须在同一目标函数和边界条件下搜索结构与参数。
...
text_contract: 只允许围绕这些标签组织图面文字：结构参数联合搜索解码、结构编码、参数搜索、解码、遗传算法；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：结构参数联合搜索解码、结构编码、参数搜索、解码、遗传算法。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：\delta_c[k]=k_pe[k]+k_iI[k]+k_dD[k]+a_{\ell}\frac{e[k]-e[k-1]}{\Delta t}+a_gz[k]
