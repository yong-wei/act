...
一句话定义：
分别辨识舵机惯性、船体艏摇惯性和扰动等效惯性，再把可信的舵机段与船体段组合为名义航向模型。

核心直觉：
每个参数都应能回到一个物理位置，而不是只在总模型里做黑箱拟合。

可用公式（内部依据，图面公式区标题只能写“公式”）：
G_r(s)=\frac{1}{1.10s+1},\qquad G_h(s)=\frac{0.0700}{42.00s+1}

本课关系：
- 分段辨识模型组合 --引出--> 工程指标代价函数翻译
- 高保真任务降阶辨识 --引出--> 分段辨识模型组合
- 高保真任务降阶辨识 --引出--> 分段辨识模型组合: 高保真任务需要先拆成舵机、船体和扰动段，才能组合出可用于经典设计的名义模型。
- 分段辨识模型组合 --引出--> 工程指标代价函数翻译: 模型结构确定后，航向、航迹、舵角和舵速等高保真观察量才能被写成目标函数与约束。
...
text_contract: 只允许围绕这些标签组织图面文字：分段辨识模型组合、分段辨识、舵机惯性、船体惯性、扰动等效、舵机；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：分段辨识模型组合、分段辨识、舵机惯性、船体惯性、扰动等效、舵机。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：G_r(s)=\frac{1}{1.10s+1},\qquad G_h(s)=\frac{0.0700}{42.00s+1}
