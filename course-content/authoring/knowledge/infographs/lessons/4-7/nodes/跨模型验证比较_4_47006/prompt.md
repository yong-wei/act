...
一句话定义：
把传统设计、辨识模型优化和高保真模型优化放进同一方波与回转任务，判断名义模型上的好结果能否转化为真实任务可用。

核心直觉：
设计结果必须同时回答“模型上成立”和“任务中可交付”两个问题。

可用公式（内部依据，图面公式区标题只能写“公式”）：
\text{可交付判断}=\text{名义模型证据}+\text{高保真验证证据}+\text{边界检查}

本课关系：
- 结构参数联合搜索解码 --引出--> 跨模型验证比较
- 传统设计四联图校正 --引出--> 跨模型验证比较
- 跨模型验证比较 --引出--> 扰动噪声设计边界
- 传统设计四联图校正 --引出--> 跨模型验证比较: 传统设计得到的名义控制器必须同时接受辨识模型和高保真模型验证。
- 结构参数联合搜索解码 --引出--> 跨模型验证比较: 搜索得到的优化控制器只有经过跨模型验证，才能从算法结果变成工程设计结论。
...
text_contract: 只允许围绕这些标签组织图面文字：跨模型验证比较、跨模型验证、高保真验证、方波任务、回转任务、跨模型比较；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：跨模型验证比较、跨模型验证、高保真验证、方波任务、回转任务、跨模型比较。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：\text{可交付判断}=\text{名义模型证据}+\text{高保真验证证据}+\text{边界检查}
