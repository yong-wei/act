...
一句话定义：
用时域、根轨迹、Bode 和 Nyquist 四类证据诊断未校正模型，再选择 $PI+\text{超前}$ 并按频域指标计算参数。

核心直觉：
传统设计不是凭经验套结构，而是从三域证据看清当前对象的问题后再定型。

可用公式（内部依据，图面公式区标题只能写“公式”）：
C_{trad}(s)=23.11\left(\frac{42.00s+1}{42.00s}\right)\left(\frac{48.02s+1}{13.01s+1}\right)

本课关系：
- 工程指标代价函数翻译 --引出--> 传统设计四联图校正
- 传统设计四联图校正 --引出--> 跨模型验证比较
- 工程指标代价函数翻译 --引出--> 传统设计四联图校正: 目标和约束明确后，传统设计才能从三域诊断中选择结构并计算参数。
- 传统设计四联图校正 --引出--> 跨模型验证比较: 传统设计得到的名义控制器必须同时接受辨识模型和高保真模型验证。
...
text_contract: 只允许围绕这些标签组织图面文字：传统设计四联图校正、四联图、PI超前、Bode、根轨迹；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：传统设计四联图校正、四联图、PI超前、Bode、根轨迹。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：C_{trad}(s)=23.11\left(\frac{42.00s+1}{42.00s}\right)\left(\frac{48.02s+1}{13.01s+1}\right)
