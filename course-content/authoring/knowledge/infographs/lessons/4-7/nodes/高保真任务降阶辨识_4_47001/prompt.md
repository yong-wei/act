...
一句话定义：
把高保真舰船航向任务拆成舵机、船体艏摇、航向积分和航迹响应这些可解释主通道，用于经典设计与高保真验证之间的连接。

核心直觉：
降阶不是把高保真模型“变简单就算完”，而是提取能解释控制器职责的主链条。
...
- 统一结构编码与解码 --引出--> 高保真任务降阶辨识: 4-6 建立结构编码入口后，4-7 需要把编码搜索放回高保真任务与可解释辨识模型之间。
...
text_contract: 只允许围绕这些标签组织图面文字：高保真任务降阶辨识、高保真模型、降阶辨识、驱逐舰航向；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：高保真任务降阶辨识、高保真模型、降阶辨识、驱逐舰航向。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：P_{id}(s)=\frac{K}{s(T_rs+1)(T_hs+1)}
