...
一句话定义：
六维归一化编码先决定结构编号，再按当前结构解释有效参数槽位和忽略槽位。

核心直觉：
同一个向量只有先判定结构，才能被正确翻译回工程参数。

可用公式（内部依据，图面公式区标题只能写“公式”）：
[z_0,\ldots,z_5]\in[0,1]^6,\qquad k=\max(1,\lceil 5z_0\rceil)

本课关系：
- 参数约束翻译表 --相关--> 统一结构编码与解码
- 统一结构编码与解码 --应用于--> 四类方案比较矩阵
- 结构搜索引入条件 --引出--> 统一结构编码与解码
- 统一结构编码与解码 --引出--> 高保真任务降阶辨识
- 结构搜索引入条件 --引出--> 统一结构编码与解码: 当结构选择正式进入问题本身，就需要统一的编码与解码机制来承载结构和参数。
- 参数约束翻译表 --相关--> 统一结构编码与解码: 编码后的参数槽位仍需落回可解释范围，因而与参数边界和槽位职责紧密相关。
...
text_contract: 只允许围绕这些标签组织图面文字：统一结构编码与解码、统一编码、解码、有效槽位、逐槽映射；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：统一结构编码与解码、统一编码、解码、有效槽位、逐槽映射。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：$[z_0,\ldots,z_5]\in[0,1]^6$，$k=\max(1,\mathrm{ceil}(5z_0))$。不要把 `ceil(5z_0)` 画成普通方括号。
