...
一句话定义：
把旧目标/新目标与旧结构/变结构交叉组合，才能分清“目标错位”和“结构边界”。

核心直觉：
四类方案不是为了凑图，而是为了把两道判断拆开。

可用公式（内部依据，图面公式区标题只能写“公式”）：
\{\text{旧目标},\text{新目标}\}\times\{\text{旧结构},\text{变结构}\}

本课关系：
- 四类方案比较矩阵 --引出--> 组合结构专项验证边界判断
- 优化前后证据比较表 --相关--> 四类方案比较矩阵
- 统一结构编码与解码 --应用于--> 四类方案比较矩阵
- 统一结构编码与解码 --应用于--> 四类方案比较矩阵: 只有把结构和参数放进统一编码，四类方案中的变结构结果才可被稳定比较。
- 优化前后证据比较表 --相关--> 四类方案比较矩阵: 四类方案比较延续了统一证据框架，只是把比较对象扩展到目标语言与结构边界。
...
text_contract: 只允许围绕这些标签组织图面文字：四类方案比较矩阵、四类方案、比较矩阵、目标错位、结构边界；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：四类方案比较矩阵、四类方案、比较矩阵、目标错位、结构边界。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：\{\text{旧目标},\text{新目标}\}\times\{\text{旧结构},\text{变结构}\}
