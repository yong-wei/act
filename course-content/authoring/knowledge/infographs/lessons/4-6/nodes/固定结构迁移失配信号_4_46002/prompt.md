...
一句话定义：
旧代价函数和固定结构迁移到新任务后，若出现持续拖尾、航迹偏离和职责过载，就说明问题已不再适合写成单纯调参。

核心直觉：
这里暴露的不是某次试参失手，而是原判断链开始对不上题。

可用公式（内部依据，图面公式区标题只能写“公式”）：
\text{失配}=(\text{切换段拖尾},\ \text{航迹偏离},\ \text{职责过载})

本课关系：
- 权重偏好与无约束候选解 --应用于--> 固定结构迁移失配信号
- 固定结构迁移失配信号 --引出--> 驱逐舰专用代价函数改写
- 迁移场景任务重排 --引出--> 固定结构迁移失配信号
- 迁移场景任务重排 --引出--> 固定结构迁移失配信号: 任务排序一旦变化，原有固定结构解释链就需要在新场景中重新接受证据检验。
- 权重偏好与无约束候选解 --应用于--> 固定结构迁移失配信号: 旧偏好写出的候选解迁移到新任务后，能够直接暴露目标错位带来的失配。
...
text_contract: 只允许围绕这些标签组织图面文字：固定结构迁移失配信号、失配信号、固定结构、拖尾、航迹偏离；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：固定结构迁移失配信号、失配信号、固定结构、拖尾、航迹偏离。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：\text{失配}=(\text{切换段拖尾},\ \text{航迹偏离},\ \text{职责过载})
