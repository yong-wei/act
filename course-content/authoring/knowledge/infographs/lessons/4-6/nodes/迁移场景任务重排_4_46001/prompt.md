...
一句话定义：
对象参数、参考轨迹和筛选线一起变化时，原方案中的收益与代价排序也会跟着变化。

核心直觉：
迁移不是“同题加速版”，而是把原来的判断链重新排了一次序。

可用公式（内部依据，图面公式区标题只能写“公式”）：
\text{迁移判断}=\text{对象变化}+\text{任务变化}+\text{筛选线重排}

本课关系：
- 剩余风险说明 --引出--> 迁移场景任务重排
- 迁移场景任务重排 --引出--> 固定结构迁移失配信号
- 剩余风险说明 --引出--> 迁移场景任务重排: `4-5` 中仍贴边的固定结构可用解，正是 `4-6` 重新辨认任务排序的入口。
...
text_contract: 只允许围绕这些标签组织图面文字：迁移场景任务重排、任务重排、场景迁移、筛选线；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：迁移场景任务重排、任务重排、场景迁移、筛选线。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：\text{迁移判断}=\text{对象变化}+\text{任务变化}+\text{筛选线重排}
