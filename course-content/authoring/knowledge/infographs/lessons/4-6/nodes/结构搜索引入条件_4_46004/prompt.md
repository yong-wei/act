...
一句话定义：
当固定结构已经无法同时解释速度收益、动作边界和稳健性余量时，结构搜索才成为合理入口。

核心直觉：
结构搜索不是“升级版算法”，而是固定结构问题已经写不全时的补救。

可用公式（内部依据，图面公式区标题只能写“公式”）：
\text{引入条件}=\text{职责过载}+\text{可用域收缩}+\text{解释链断层}
...
text_contract: 只允许围绕这些标签组织图面文字：结构搜索引入条件、结构搜索、引入条件、可用域；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：结构搜索引入条件、结构搜索、引入条件、可用域。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：\text{引入条件}=\text{职责过载}+\text{可用域收缩}+\text{解释链断层}
