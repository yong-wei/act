...
一句话定义：
当设计任务从给定跟踪转为扰动抑制时，复合结构可能不再主要来自频段叠加，而是来自对控制通道、扰动通道和物理分量的重新组织。

核心直觉：
复合结构不只回答“哪个频段还要补”，还会回答“哪条通道需要重新写”。
...
- 反馈保底与前馈补偿边界 --引出--> 扰动抑制中的通道重写边界
- 任务通道变化下的自由目标改写 --相关--> 扰动抑制中的通道重写边界
- 任务通道变化下的自由目标改写 --相关--> 扰动抑制中的通道重写边界
- 反馈保底与前馈补偿边界 --引出--> 扰动抑制中的通道重写边界: 当任务进入扰动抑制时，前馈和反馈的边界会进一步扩展成通道重写问题。
- 单结构与复合结构分流条件 --概括--> 扰动抑制中的通道重写边界: 4-3 在边界案例中进一步说明，结构分流不只发生在频段职责之间，也会发生在控制通道重写层面。
...
text_contract: 只允许围绕这些标签组织图面文字：扰动抑制中的通道重写边界、扰动抑制、通道重写、边界案例、减摇鳍；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：扰动抑制中的通道重写边界、扰动抑制、通道重写、边界案例、减摇鳍。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：u(s)=u_{\mathrm{feedback}}(s)+u_{\mathrm{channel\ rewrite}}(s)
