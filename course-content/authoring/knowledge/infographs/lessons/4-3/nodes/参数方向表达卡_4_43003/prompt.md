...
所属分组：复合结构 A/B、复合结构 C 与客船入口、客船落地与验证、例题与工作区
知识类型：D

事实真源如下，禁止添加未出现的事实、公式、术语或工程案例：

一句话定义：
把首轮方案写成“先改哪段行为、预期先改善什么、最可能先透支什么”的设计表达，用来替代只报参数大小的经验描述。

核心直觉：
参数方向不是参数表，而是方案的设计意图和风险提示。

可用公式（内部依据，图面公式区标题只能写“公式”）：
\text{参数方向}=(\text{先改哪段行为},\text{预期改善},\text{最可能透支})

本课关系：
- 低频补偿稳态改善路径 --应用于--> 参数方向表达卡
- 参数方向表达卡 --引出--> 首轮验证最小读数逻辑
- 单结构与复合结构分流条件 --引出--> 参数方向表达卡
- 目标驱动超前校正 --应用于--> 参数方向表达卡
- 参数方向表达卡 --应用于--> 优化起点与归一化来源
- 单结构与复合结构分流条件 --引出--> 参数方向表达卡: 一旦结构分流确定下来，下一步就要把方案写成参数方向，而不是直接跳到最后参数值。
...
text_contract: 只允许围绕这些标签组织图面文字：参数方向表达卡、参数方向、设计语言、首轮方案、方案表达；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：参数方向表达卡、参数方向、设计语言、首轮方案、方案表达。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：\text{参数方向}=(\text{先改哪段行为},\text{预期改善},\text{最可能透支})
