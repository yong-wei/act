...
一句话定义：
首轮验证至少同时读取时域、频域和控制量边界中的关键证据，用来判断方案是否接住主矛盾、是否触碰硬约束，以及下一轮应优先调整什么。

核心直觉：
只报一个最好看的指标，不足以说明方案真的可继续推进。
...
text_contract: 只允许围绕这些标签组织图面文字：首轮验证最小读数逻辑、首轮验证、时域、频域、控制峰值、读数逻辑；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：首轮验证最小读数逻辑、首轮验证、时域、频域、控制峰值、读数逻辑。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：\text{首轮验证}= \{\text{时域过程},\text{截止频率/相角裕度},\text{控制峰值或高频代价}\}
