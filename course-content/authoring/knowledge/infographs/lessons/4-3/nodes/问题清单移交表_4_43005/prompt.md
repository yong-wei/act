...
一句话定义：
把首轮验证结论整理成“已满足目标、已暴露代价、下一轮优先调整项”三类句子，用来把首轮证据正式移交给后续权衡和优化阶段。

核心直觉：
问题清单不是一句“效果一般”，而是一份能推动下一轮设计的证据摘要。

可用公式（内部依据，图面公式区标题只能写“公式”）：
\text{问题清单}=(\text{已满足目标},\text{已暴露代价},\text{下一轮优先调整项})

本课关系：
- 首轮验证最小读数逻辑 --引出--> 问题清单移交表
- 问题清单移交表 --引出--> 经典试凑中的多目标拉扯
- 问题清单移交表 --引出--> 经典试凑中的多目标拉扯
- 首轮验证最小读数逻辑 --引出--> 问题清单移交表: 4-3 的首轮证据只有被整理成问题清单，才真正成为 4-4 可继续使用的输入。
...
text_contract: 只允许围绕这些标签组织图面文字：问题清单移交表、问题清单、移交、下一轮调整、移交表；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：问题清单移交表、问题清单、移交、下一轮调整、移交表。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：\text{问题清单}=(\text{已满足目标},\text{已暴露代价},\text{下一轮优先调整项})
