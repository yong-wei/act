...
所属分组：主场景联读、对照案例与排序重排、任务表达与收束
知识类型：X

事实真源如下，禁止添加未出现的事实、公式、术语或工程案例：

一句话定义：
按“时域响应 → 根轨迹/可行域 → Bode 幅频 → 相频/裕度”的顺序联读同一对象证据，再据此重排任务优先级。

核心直觉：
同一套图不直接给控制器答案，它先告诉我们“当前最紧的矛盾到底排在什么顺序”。
...
text_contract: 只允许围绕这些标签组织图面文字：跨域联读任务重排、跨域联读、四联图、任务排序、案例对照、案例分析；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：跨域联读任务重排、跨域联读、四联图、任务排序、案例对照、案例分析。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：\text{读图顺序}=\text{时域响应}\rightarrow\text{根轨迹/可行域}\rightarrow\text{Bode 幅频}\rightarrow\text{相频/裕度}
