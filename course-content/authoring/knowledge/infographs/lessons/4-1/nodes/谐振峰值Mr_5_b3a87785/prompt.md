...
一句话定义：
闭环幅频特性曲线的最大峰值，用来表征系统的峰化风险和相对稳定性倾向。

核心直觉：
在 `4-1` 的对照案例里，`Mr` 更像“速度提升后要持续盯住的代价证据”。
...
text_contract: 只允许围绕这些标签组织图面文字：谐振峰值Mr、相对稳定性、阻尼比、相角裕度、幅频特性、频域指标、峰化风险；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：谐振峰值Mr、相对稳定性、阻尼比、相角裕度、幅频特性、频域指标、峰化风险。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：M_r=\frac{1}{2\zeta\sqrt{1-\zeta^2}}
