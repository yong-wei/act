...
一句话定义：
把时域、频域和积分误差指标，从“描述系统怎样”改写成“分别回答过程接受度、储备边界和累计代价”的设计语言。

核心直觉：
同一个指标到了设计课，不再只是一个读数，而要回答“它在任务书里到底负责什么”。

可用公式（内部依据，图面公式区标题只能写“公式”）：
J_{\mathrm{ISE}}=\int_0^{\infty} e^2(t)\,\mathrm{d}t,\quad J_{\mathrm{IAE}}=\int_0^{\infty} |e(t)|\,\mathrm{d}t,\quad J_{\mathrm{ITAE}}=\int_0^{\infty} t|e(t)|\,\mathrm{d}t
...
text_contract: 只允许围绕这些标签组织图面文字：指标角色重组、时域指标、频域指标、积分误差、指标分工、指标重组、设计语言；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：指标角色重组、时域指标、频域指标、积分误差、指标分工、指标重组、设计语言。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：J_{\mathrm{ISE}}=\int_0^{\infty} e^2(t)\,\mathrm{d}t,\quad J_{\mathrm{IAE}}=\int_0^{\infty} |e(t)|\,\mathrm{d}t,\quad J_{\mathrm{ITAE}}=\int_0^{\infty} t|e(t)|\,\mathrm{d}t
