...
一句话定义：
同时满足所有性能约束的参数允许范围，在三域各有投影——这是从"分析"走向"设计"的核心工具。
...
- 设计可行域 --包含--> 可行域-满意域-最优域分层: 4-1 用设计可行域承接“先排除不能做的，再比较可用方案”的入口逻辑。
...
text_contract: 只允许围绕这些标签组织图面文字：设计可行域、可行域、参数范围、约束映射、三域投影、设计决策、层0速通、框架概念、设计型；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：设计可行域、可行域、参数范围、约束映射、三域投影、设计决策、层0速通、框架概念、设计型。
