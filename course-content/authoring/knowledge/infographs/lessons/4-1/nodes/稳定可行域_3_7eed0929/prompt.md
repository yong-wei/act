...
一句话定义：
由劳斯表第一列全正条件联立得到的参数范围，就是稳定可行域。

核心直觉：
稳定可行域把“能否稳定”写成一段明确的参数区间，边界点对应极点压到虚轴或原点的临界状态。
...
text_contract: 只允许围绕这些标签组织图面文字：稳定可行域、稳定区间、参数范围、临界边界、可行域、层2深化、设计型、参数约束；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：稳定可行域、稳定区间、参数范围、临界边界、可行域、层2深化、设计型、参数约束。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：\text{稳定可行域}=\{\theta \mid \text{劳斯表第一列全正}\}
