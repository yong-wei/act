...
一句话定义：
把入口判断分成“先排除不能做的可行域筛选、再保留当前可接受的满意域、最后才比较更优的最优域”三层。

核心直觉：
`4-1` 只负责把边界写清楚，不把筛边界和求最优混成一步。

可用公式（内部依据，图面公式区标题只能写“公式”）：
\mathcal{O}\subseteq\mathcal{S}\subseteq\mathcal{F}

本课关系：
- 可行域-满意域-最优域分层 --前置--> 跨域联读任务重排
- 硬约束-软目标-观察指标分类 --引出--> 可行域-满意域-最优域分层
- 稳定可行域 --前置--> 可行域-满意域-最优域分层
- 复平面可行域 --应用于--> 可行域-满意域-最优域分层
- 设计可行域 --包含--> 可行域-满意域-最优域分层
...
text_contract: 只允许围绕这些标签组织图面文字：可行域-满意域-最优域分层、可行域、满意域、最优域、分层筛选、分层判断；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：可行域-满意域-最优域分层、可行域、满意域、最优域、分层筛选、分层判断。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：\mathcal{O}\subseteq\mathcal{S}\subseteq\mathcal{F}
