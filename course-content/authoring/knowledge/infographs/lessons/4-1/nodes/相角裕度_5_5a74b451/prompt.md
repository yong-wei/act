...
一句话定义：
在截止频率 $\omega_c$ 处，开环相角距离 $-180^\circ$ 还剩下的角度余量。

核心直觉：
它回答的是“在增益刚好等于 1 的那个频率上，相位离危险边界还有多远”。
...
text_contract: 只允许围绕这些标签组织图面文字：相角裕度、截止频率、相位滞后、临界稳定、稳定性、开环频率特性、层1精化、频域分析、2-4；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：相角裕度、截止频率、相位滞后、临界稳定、稳定性、开环频率特性、层1精化、频域分析、2-4。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：\gamma=180^\circ+\angle G(j\omega_c)
