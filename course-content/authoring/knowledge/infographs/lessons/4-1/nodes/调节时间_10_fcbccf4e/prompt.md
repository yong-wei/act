...
一句话定义：
响应进入并始终保持在稳态值允许误差带内所需的最短时间。

核心直觉：
在设计入口里，调节时间不是单纯的“越短越好”，而是要和场景允许的恢复时限绑定。
...
text_contract: 只允许围绕这些标签组织图面文字：调节时间、稳态误差带、极点实部、响应速度、性能指标、稳态、调节时间、瞬态性能、时域指标；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：调节时间、稳态误差带、极点实部、响应速度、性能指标、稳态、调节时间、瞬态性能、时域指标。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：t_s\approx \frac{4}{\zeta\omega_n}=\frac{4}{\sigma}
