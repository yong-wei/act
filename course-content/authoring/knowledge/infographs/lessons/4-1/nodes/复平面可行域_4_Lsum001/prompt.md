...
一句话定义：
把超调量和调节时间等时域指标翻译成复平面几何约束后得到的允许极点区域。

核心直觉：
它让“响应曲线要满足什么要求”变成“极点允许落在哪里”的图形问题。
...
text_contract: 只允许围绕这些标签组织图面文字：复平面可行域、可行域、扇形区域、阻尼比射线、实部垂线、极点约束、层0速通、层1精化、根轨迹分析；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：复平面可行域、可行域、扇形区域、阻尼比射线、实部垂线、极点约束、层0速通、层1精化、根轨迹分析。
