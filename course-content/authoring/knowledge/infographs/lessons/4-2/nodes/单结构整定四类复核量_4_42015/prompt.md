...
一句话定义：
单结构参数初算后，必须同时检查阶跃响应、扰动响应、相角裕度和控制量。

核心直觉：
单一指标改善只说明结构有作用，不说明它已经适合交付。

可用公式（内部依据，图面公式区标题只能写“公式”）：
\text{复核}=(step,disturbance,PM,u_{peak})

本课关系：
- 相角裕度 --应用于--> 单结构整定四类复核量: 相角裕度是参数初算后必须复核的频域储备量。
- 幅值裕度 --相关--> 单结构整定四类复核量: 幅值裕度帮助补充判断系统离不稳定边界还有多远。
- 客船候选结构整定比较 --supports--> 单结构整定四类复核量: 客船候选结果证明单结构初算后必须同时复核动态、扰动、裕度和控制量。
- 单结构整定四类复核量 --应用于--> 单结构首轮起步卡: 起步卡在新版讲义中不仅记录理由，还应记录参数初算后要复核的证据。

建议视觉骨架：
用同一单结构参数初算结果同时进入时域响应、扰动响应、相角裕度和控制量四个复核仪表，表达不能凭单指标宣布有效；图面用“时域响应”而不是“阶跃响应”。
...
text_contract: 只允许围绕这些标签组织图面文字：单结构整定四类复核量、复核、时域响应、扰动响应、相角裕度、控制量、设计判断；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：单结构整定四类复核量、复核、时域响应、扰动响应、相角裕度、控制量、设计判断。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：\text{复核}=(step,disturbance,PM,u_{peak})
