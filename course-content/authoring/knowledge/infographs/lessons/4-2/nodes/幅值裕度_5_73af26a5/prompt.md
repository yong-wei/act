...
一句话定义：
在穿越频率 $\omega_g$ 处，开环幅值距离 `0 dB` 还剩下的增益余量。
...
text_contract: 只允许围绕这些标签组织图面文字：幅值裕度、穿越频率、开环幅频特性、临界稳定、频域分析；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：幅值裕度、穿越频率、开环幅频特性、临界稳定、频域分析。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：K_g=\frac{1}{|G(j\omega_g)|}
