...
一句话定义：
把无约束候选重新放回统一证据中复核，确认它究竟在哪些边界上失守。

核心直觉：
只看“更快、更小”的收益会误判候选质量，越界证据让代价重新可见。
...
text_contract: 只允许围绕这些标签组织图面文字：越界证据记录单、越界证据、无约束候选、统一复核、工程失真、越界复核、主案例；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：越界证据记录单、越界证据、无约束候选、统一复核、工程失真、越界复核、主案例。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：\mathcal{E}_{vio}=(M_p,\ u_{\max},\ \phi_m,\ t_s,\ ITAE,\ ITSE,\ E_u)
