...
一句话定义：
同一固定结构、同一组目标项下，只要权重偏好不同，求解器就会给出不同风格的候选解。

核心直觉：
候选解的差异不是算出来的噪声，而是偏好被显性写进模型后的必然结果。
...
text_contract: 只允许围绕这些标签组织图面文字：权重偏好与无约束候选解、权重、偏好、候选解、权重偏好、无约束优化；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：权重偏好与无约束候选解、权重、偏好、候选解、权重偏好、无约束优化。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：\theta^\star=\arg\min_{\theta}J_{\mathrm{free}}(\theta)
