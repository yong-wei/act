...
一句话定义：
把输出侧硬约束和参数范围分别写成求解器可消费的规则。

核心直觉：
不是所有不等式都在回答同一个问题。

可用公式（内部依据，图面公式区标题只能写“公式”）：
c(\theta)= \begin{bmatrix} M_p(\theta)-20\\ u_{\max}(\theta)-7\\ 45-\phi_m(\theta) \end{bmatrix}\le 0,\qquad lb\le \theta\le ub

本课关系：
- 优化起点与归一化来源 --应用于--> 参数约束翻译表
- 越界证据记录单 --引出--> 参数约束翻译表
- 参数约束翻译表 --引出--> 罚函数表达表
- 参数约束翻译表 --相关--> 统一结构编码与解码
- 参数约束翻译表 --应用于--> 工程指标代价函数翻译
- 越界证据记录单 --引出--> 参数约束翻译表: 只有先确认候选在哪些边界上失守，约束翻译才不会流于口头提醒。
...
text_contract: 只允许围绕这些标签组织图面文字：参数约束翻译表、约束翻译、nonlcon、上下界、职责分工；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：参数约束翻译表、约束翻译、nonlcon、上下界、职责分工。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：c(\theta)= \begin{bmatrix} M_p(\theta)-20\\ u_{\max}(\theta)-7\\ 45-\phi_m(\theta) \end{bmatrix}\le 0,\qquad lb\le \theta\le ub
