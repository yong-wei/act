...
一句话定义：
把 `4-3` 已经验证过的首轮方案当作参数起点，并把任务书读数或首轮读数当作归一化基准，让后续搜索始终回接真实证据。
...
text_contract: 只允许围绕这些标签组织图面文字：优化起点与归一化来源、优化起点、归一化、参数范围；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：优化起点与归一化来源、优化起点、归一化、参数范围。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：\theta_0=[2.796,\ 10,\ 0.406]^{\mathsf T}
