...
一句话定义：
把越界代价显性写进总代价函数，让求解器主动远离不可用区域。

核心直觉：
边界若只停留在注释里，搜索就会继续奖励失真解。
...
- 罚函数表达表 --引出--> 求解记录单
- 参数约束翻译表 --引出--> 罚函数表达表
- 参数约束翻译表 --引出--> 罚函数表达表: 边界一旦被翻译成可计算约束，就可以继续写成显式罚项或等价约束表达。
- 罚函数表达表 --引出--> 求解记录单: 总代价函数和约束表达写清之后，求解记录单才能完整记录 `fmincon` 的输入与输出。
...
text_contract: 只允许围绕这些标签组织图面文字：罚函数表达表、罚函数、总代价、越界代价、显性边界；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：罚函数表达表、罚函数、总代价、越界代价、显性边界。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：J_{con}(\theta)=J_{free}(\theta)+50\max(0,M_p-20)^2+20\max(0,u_{\max}-7)^2+5\max(0,45-\phi_m)^2
