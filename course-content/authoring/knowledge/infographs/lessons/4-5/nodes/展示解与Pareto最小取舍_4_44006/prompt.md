...
一句话定义：
Pareto前沿不是替我们宣布唯一最优，而是把一组互不支配的候选一起呈现出来，让偏好取舍变成可读对象。

核心直觉：
展示解是一族同样值得继续讨论的候选，不是已经完成工程复核的最终答案。

可用公式（内部依据，图面公式区标题只能写“公式”）：
\mathcal{P}=\{\theta\mid \nexists \theta' \text{ 使所有目标不劣且至少一项目标更优}\}

本课关系：
- 权重偏好与无约束候选解 --引出--> Pareto前沿与展示解取舍
...
建议视觉骨架：
用二维目标平面中的 Pareto 前沿、多个非支配候选点和展示解标记表达“候选族可读、取舍未结束”的判断，不把展示解画成唯一最终最优。
...
text_contract: 只允许围绕这些标签组织图面文字：Pareto前沿与展示解取舍、Pareto前沿、非支配候选、展示解、候选族、多目标取舍；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：Pareto前沿与展示解取舍、Pareto前沿、非支配候选、展示解、候选族、多目标取舍。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：\mathcal{P}=\{\theta\mid \nexists \theta' \text{ 使所有目标不劣且至少一项目标更优}\}
