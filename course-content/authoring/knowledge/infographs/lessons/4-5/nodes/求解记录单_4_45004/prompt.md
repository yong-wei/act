...
text_contract: 只允许围绕这些标签组织图面文字：求解记录单、求解记录、fmincon、初值、复现；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：求解记录单、求解记录、fmincon、初值、复现。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：\mathcal{R}_{solve}=(x_0,\ lb,\ ub,\ J(\theta),\ c(\theta),\ x^*,\ f(x^*))
