...
一句话定义：
先判闭环稳定与信号通道，再在“终值定理直接求”和“型别/误差系数快速判”之间选择求解路径的判断框架。
...
text_contract: 只允许围绕这些标签组织图面文字：稳态误差双路径判断、终值定理、型别、静态误差系数、路径选择、适用边界、求解路径、稳态误差；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：稳态误差双路径判断、终值定理、型别、静态误差系数、路径选择、适用边界、求解路径、稳态误差。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：e_{ss}=\lim_{s\to0}sE(s)
