...
所属分组：低频补偿与稳态改善路径、频域设计与速度优先方案
知识类型：X

事实真源如下，禁止添加未出现的事实、公式、术语或工程案例：

一句话定义：
通过比例作用配合积分作用，既保留可调增益，又能通过提高型别改善稳态精度的控制器。

核心直觉：
`PI` 的价值不是“多了一个参数”，而是“把系统型别真的改了”。
...
- 系统特性计算 --应用于--> 比例-积分控制器
- 极点配置 --相关--> 比例-积分控制器
- LQ/LQG最优控制 --相关--> 比例-积分控制器
- SISO系统设计工具 --应用于--> 比例-积分控制器
- 鲁棒控制 --应用于--> 比例-积分控制器
- 时域分析法 --应用于--> 比例-积分控制器
...
text_contract: 只允许围绕这些标签组织图面文字：比例-积分控制器、PI控制器、稳态误差、积分控制、PI、低频补偿；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：比例-积分控制器、PI控制器、稳态误差、积分控制、PI、低频补偿。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：G_{PI}(s)=K\left(1+\frac{1}{T_i s}\right)
