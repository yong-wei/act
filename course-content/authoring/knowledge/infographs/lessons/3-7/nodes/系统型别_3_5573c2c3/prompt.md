...
一句话定义：
按开环传函在原点的积分个数对系统进行分类，用来判断典型给定输入下的跟踪能力。

核心直觉：
型别回答的是“哪一类输入的误差可以先天更小或归零”。
...
text_contract: 只允许围绕这些标签组织图面文字：系统型别、开环增益、跟踪能力、无差系统、型别、积分环节、无差度、0型系统、I型系统、II型系统；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：系统型别、开环增益、跟踪能力、无差系统、型别、积分环节、无差度、0型系统、I型系统、II型系统。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：G(s)H(s)=\frac{K_0}{s^v}G_0(s),\qquad G_0(0)\neq 0
