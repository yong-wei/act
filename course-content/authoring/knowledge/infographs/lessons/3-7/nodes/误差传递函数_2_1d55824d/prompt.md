...
一句话定义：
以误差信号为输出量，把给定输入或扰动与误差联系起来的传递函数。

核心直觉：
求稳态误差时，真正该盯的是误差通道，不只是输出通道。
...
text_contract: 只允许围绕这些标签组织图面文字：误差传递函数、误差信号、E(s)、反馈系统、误差、结构表达；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：误差传递函数、误差信号、E(s)、反馈系统、误差、结构表达。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：\frac{E(s)}{R(s)}=\frac{1}{1+G(s)H(s)}
