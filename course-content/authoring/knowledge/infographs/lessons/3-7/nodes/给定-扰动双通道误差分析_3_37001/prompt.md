...
一句话定义：
把给定输入和扰动分别沿闭环通道列式，再用总输出与总误差统一处理稳态问题的分析框架。

核心直觉：
先问“信号从哪里进来”，再问“误差最后停在哪”。

可用公式（内部依据，图面公式区标题只能写“公式”）：
C(s)=\Phi_r(s)R(s)+\Phi_d(s)D(s)

本课关系：
- 误差传递函数 --引出--> 给定-扰动双通道误差分析
- 给定-扰动双通道误差分析 --引出--> 稳态误差双路径判断
- 扰动作用下的稳态误差 --跨域连接--> 给定-扰动双通道误差分析
- 误差传递函数 --引出--> 给定-扰动双通道误差分析: 3-7 把已有的误差传函概念推进为给定/扰动并列列式的双通道分析框架。
- 扰动作用下的稳态误差 --跨域连接--> 给定-扰动双通道误差分析: 只有把扰动看成独立通道进入闭环，抗扰稳态误差才不会被误当成另一种输入型别。
- 给定-扰动双通道误差分析 --引出--> 稳态误差双路径判断: 先把通道写对，后面才有资格在“直接求”和“快速判”之间选路径。
...
text_contract: 只允许围绕这些标签组织图面文字：给定-扰动双通道误差分析、给定输入、扰动通道、总输出、总误差、分母相同、误差通道、稳态改善；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：给定-扰动双通道误差分析、给定输入、扰动通道、总输出、总误差、分母相同、误差通道、稳态改善。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：C(s)=\Phi_r(s)R(s)+\Phi_d(s)D(s)
