...
一句话定义：
把典型给定输入下的稳态误差快速判断压成 `K_p / K_v / K_a` 三个低频指标。

核心直觉：
它们是快判工具，不是所有稳态误差问题的通用替身。

可用公式（内部依据，图面公式区标题只能写“公式”）：
K_p=\lim_{s\to0}G(s)H(s),\quad K_v=\lim_{s\to0}sG(s)H(s),\quad K_a=\lim_{s\to0}s^2G(s)H(s)

本课关系：
- 系统特性计算 --相关--> 静态误差系数
- 极点配置 --相关--> 静态误差系数
- SISO系统设计工具 --应用于--> 静态误差系数
- 时域分析法 --包含--> 静态误差系数
- 典型输入信号 --引出--> 静态误差系数
- 单位阶跃函数 --引出--> 静态误差系数
...
text_contract: 只允许围绕这些标签组织图面文字：静态误差系数、位置误差系数、速度误差系数、加速度误差系数、静态误差系数、快速判断；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：静态误差系数、位置误差系数、速度误差系数、加速度误差系数、静态误差系数、快速判断。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：K_p=\lim_{s\to0}G(s)H(s),\quad K_v=\lim_{s\to0}sG(s)H(s),\quad K_a=\lim_{s\to0}s^2G(s)H(s)
