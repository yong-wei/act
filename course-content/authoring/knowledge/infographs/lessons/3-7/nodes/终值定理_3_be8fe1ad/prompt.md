...
一句话定义：
通过象函数直接求原函数最终极限值的工具，是稳态误差直接求法的统一出口。
...
text_contract: 只允许围绕这些标签组织图面文字：终值定理、终值、拉普拉斯变换、极限、终值定理、直接求；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：终值定理、终值、拉普拉斯变换、极限、终值定理、直接求。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：\lim_{t\to\infty}e(t)=\lim_{s\to0}sE(s)
