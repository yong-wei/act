...
一句话定义：
在进入非线性工具、复杂系统链路、数据驱动或策略学习之前，先写清边界证据、假设破坏、线性内处理和迁移理由。

核心直觉：
方法迁移不是追新，而是已有主干已经不能解释当前对象时的有证据转向。
...
- 线性预测失真对比 --引出--> 方法迁移前边界说明: 只有把预测失真说清楚，后续进入边界工具、复杂系统链路或数据驱动才有工程理由。
...
text_contract: 只允许围绕这些标签组织图面文字：方法迁移前边界说明、方法迁移、边界说明、工程证据、风险说明、工程判断；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：方法迁移前边界说明、方法迁移、边界说明、工程证据、风险说明、工程判断。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：\text{边界说明}=\text{证据}+\text{假设破坏}+\text{线性内处理}+\text{迁移理由}
