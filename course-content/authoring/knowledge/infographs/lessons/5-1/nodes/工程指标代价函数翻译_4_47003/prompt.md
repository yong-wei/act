...
一句话定义：
把航向误差、航迹偏离、舵角、舵速、饱和占比和稳定裕度翻译为主目标、次目标、硬约束与罚项。

核心直觉：
优化目标不是越多越好，而是每一项都要回答真实任务中的一个具体风险。
...
- 工程指标代价函数翻译 --跨域连接--> 线性预测失真对比: 4-7 中舵角、舵速和饱和占比的代价语言，在 5-1 中转化为边界响应失真的观察证据。
...
text_contract: 只允许围绕这些标签组织图面文字：工程指标代价函数翻译、代价函数、硬约束、航迹偏离、舵速、工程权衡、工程指标；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：工程指标代价函数翻译、代价函数、硬约束、航迹偏离、舵速、工程权衡、工程指标。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：J=w_eJ_e+w_yJ_y+w_uJ_u+w_{\dot u}J_{\dot u}+w_sJ_s+J_{penalty}
