...
一句话定义：
在工作点附近用切线斜率近似平滑非线性，同时说明该近似只对小扰动邻域有效。

核心直觉：
能线性化的不是“所有非线性”，而是工作点附近足够平滑、扰动范围足够小的对象。

可用公式（内部依据，图面公式区标题只能写“公式”）：
u=f(v)\approx f(v_0)+f'(v_0)(v-v_0),\qquad \frac{Y(s)}{V(s)}\approx \frac{f'(v_0)}{2s+1}
...
text_contract: 只允许围绕这些标签组织图面文字：局部线性化边界判断、局部线性化、工作点、传递函数、不连续环节、边界判断；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：局部线性化边界判断、局部线性化、工作点、传递函数、不连续环节、边界判断。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：u=f(v)\approx f(v_0)+f'(v_0)(v-v_0),\qquad \frac{Y(s)}{V(s)}\approx \frac{f'(v_0)}{2s+1}
