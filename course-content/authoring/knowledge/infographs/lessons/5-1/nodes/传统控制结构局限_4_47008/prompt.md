...
一句话定义：
固定低阶传递函数、固定结构、线性反馈和单传感器反馈在高保真任务中存在表达边界，越过边界后需要更强的前沿方法。

核心直觉：
传统控制不是失效，而是给出可解释基线；高保真验证告诉我们什么时候必须引入更强结构。
...
text_contract: 只允许围绕这些标签组织图面文字：传统控制结构局限、传统控制局限、鲁棒控制、扰动观测器、MPC、传统控制、前沿入口、方法迁移；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：传统控制结构局限、传统控制局限、鲁棒控制、扰动观测器、MPC、传统控制、前沿入口、方法迁移。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：\text{传统结构边界}=\text{模型固定}+\text{结构固定}+\text{线性反馈}+\text{单传感器}
