请使用 GPT Image 2 / Codex 最新图片生成能力，制作一张横版中文教学信息图。

主题：链路责任诊断
所属课程单元：5-3《从单回路控制到复杂自主系统链路：MASS 的感知—估计—规划—控制协同》
所属分组：责任诊断与工程收束
知识类型：X

事实真源如下，禁止添加未出现的事实、公式、术语或工程案例：

一句话定义：
链路责任诊断按感知、估计、规划、控制、执行和监督逐层定位偏差来源，避免把复杂系统问题直接压给控制器参数。

核心直觉：


可用公式（内部依据，图面公式区标题只能写“公式”）：
\text{诊断顺序}:\text{感知}\rightarrow\text{估计}\rightarrow\text{规划}\rightarrow\text{控制}\rightarrow\text{执行}\rightarrow\text{监督}

本课关系：
- 上游信息质量 --引出--> 链路责任诊断
- MASS自动化等级责任边界 --相关--> 链路责任诊断
- 规划参考可实现性 --引出--> 链路责任诊断
- 执行约束反馈 --引出--> 链路责任诊断
- 上游信息质量 --引出--> 链路责任诊断
- 规划参考可实现性 --引出--> 链路责任诊断

建议视觉骨架：
用从感知到监督的证据链诊断顺序表达偏差来源逐层定位，避免把复杂系统问题直接压给控制器参数。

视觉资产 brief：
主视觉对象：一张工程证据墙，六列依次为感知、估计、规划、控制、执行、监督，每列有一个日志或曲线证据。
工程场景：同一轨迹偏差被拆成丢帧、估计滞后、急转参考、控制振荡、舵角饱和、降级接管证据。
核心图示：诊断顺序箭头、证据链、责任边界、优先检查四个元素。
构图方式：从左到右流程板，右下角放“不要直接调参”边界提示，但不写长段落。

结构化视觉架构：
visual_archetype: 工程评审板（把链路、证据窗口和责任边界组织成工程评审台，强调分层归因和边界判断。）
layout_contract: 16:9 横版；采用工程评审台或证据墙布局；3-4 个证据窗口按当前节点的曲线、轨迹、交点、微扰或工程波形组织；右下角给边界判断。
text_contract: 只允许围绕这些标签组织图面文字：链路责任诊断、故障诊断、责任边界、证据链、监督、链路诊断；全图最多 12 个可见标签；不放正文段落。
technical_insets_contract: 1-3 个技术小窗，每个小窗必须承担读图、机理解释或边界判断功能。
negative_constraints:
- 不要生成电子讲义截图或白底多卡片堆叠。
- 不要把长定义、长例题或整段说明塞进图面。
- 不要用无关装饰替代工程对象、技术曲线或物理隐喻。
- 不要渲染 visual_archetype、layout_contract、text_contract 等提示词字段名。

图像要求：
- 横版信息图，适合放在互动课程入口的知识点详情中；画面应像一张教学视觉资产，而不是电子知识卡片截图。
- 必须包含一个具体主视觉对象或工程场景，并包含一个数学/工程图示嵌图；不要只画卡片、图标和箭头。
- 主视觉对象、示意图、公式和短标签要共同解释机制；不要用无关装饰填充画面。
- 中文为主，不使用英文大标题；英文只允许作为小号副标题。
- 不要把“课程单元、所属分组、知识类型、事实真源、可用公式、图像要求、visual_archetype、layout_contract、text_contract”等元数据或提示词字段画进图面；公式区域如需标题，只能写“公式”。
- 不要整句复刻“一句话定义”；把定义压缩成 3-5 个短标签或短判断。
- 全图可见中文标签控制在 12 个以内；单个标签尽量不超过 10 个汉字，不放解释段落。
- 图中的关系标签统一使用中文，不要显示 contains、leads_to、cross_domain 等英文关系类型。
- 采用“核心直觉 -> 机理路径 -> 边界提醒”的三段式视觉结构。
- 只使用少量短中文标签，优先使用这些词：链路责任诊断、故障诊断、责任边界、证据链、监督、链路诊断。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：\text{诊断顺序}:\text{感知}\rightarrow\text{估计}\rightarrow\text{规划}\rightarrow\text{控制}\rightarrow\text{执行}\rightarrow\text{监督}
- 不要出现教师、课堂、流程说明、软件界面、二维码、水印。
- 字体清晰，文字少而准确，避免密集段落。
- 不要使用纯蓝或纯紫单色风格；使用白底、深墨色文字、青绿/橙色强调和少量红色边界提示。
