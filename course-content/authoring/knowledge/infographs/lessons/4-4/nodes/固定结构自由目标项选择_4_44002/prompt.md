...
一句话定义：
在固定结构已经确定的前提下，先选出能够表达当前偏好的自由目标项，再决定如何把它们合成为同一个加权目标。

核心直觉：
本课先比较“想让搜索更愿意服务什么”，而不是先宣布“谁绝对不能越界”。

可用公式（内部依据，图面公式区标题只能写“公式”）：
\Phi(\theta)=\bigl(t_s(\theta),\ \mathrm{ITAE}(\theta),\ \mathrm{ITSE}(\theta),\ E_u(\theta)\bigr)

本课关系：
- 固定结构自由目标项选择 --引出--> 优化起点与归一化来源
- 经典试凑中的多目标拉扯 --引出--> 固定结构自由目标项选择
...
text_contract: 只允许围绕这些标签组织图面文字：固定结构自由目标项选择、自由目标、指标选择、固定结构；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：固定结构自由目标项选择、自由目标、指标选择、固定结构。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：\Phi(\theta)=\bigl(t_s(\theta),\ \mathrm{ITAE}(\theta),\ \mathrm{ITSE}(\theta),\ E_u(\theta)\bigr)
