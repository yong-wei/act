...
一句话定义：
当固定结构下的人工试凑同时牵动速度、拖尾、误差强度和控制动作时，设计者看到的不再是单一改进，而是一组彼此拉扯的收益与代价。

核心直觉：
经典试凑没有失效，真正暴露出来的是我们还缺一套统一的比较语言。
...
text_contract: 只允许围绕这些标签组织图面文字：经典试凑中的多目标拉扯、经典试凑、多目标拉扯、固定结构、多目标、拉扯；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：经典试凑中的多目标拉扯、经典试凑、多目标拉扯、固定结构、多目标、拉扯。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：\text{试凑读数}=(\text{速度收益},\text{拖尾变化},\text{误差强度},\text{动作代价})
