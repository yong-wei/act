...
一句话定义：
二状态冷链热模型用空气温度和货品核心温度两个状态区分快速传感器读数与慢速本体热风险。
...
text_contract: 只允许围绕这些标签组织图面文字：二状态冷链热模型、空气温度、货品核心温度、热模型、时间尺度、冷链温控；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：二状态冷链热模型、空气温度、货品核心温度、热模型、时间尺度、冷链温控。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：T_p(k+1)=T_p(k)+\frac{T_a(k)-T_p(k)}{250}+0.0062q_{\mathrm{load}}(k)
