...
一句话定义：
方法选择说明卡把推荐路线、收益、风险、验证路径和暂不采用其他路线的依据写成可审查的工程判断。
...
text_contract: 只允许围绕这些标签组织图面文字：方法选择说明卡、说明卡、路线选择、验证路径、暂不采用、实践产出、方法迁移；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：方法选择说明卡、说明卡、路线选择、验证路径、暂不采用、实践产出、方法迁移。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：\text{路线选择}=\text{收益}\land\text{风险边界}\land\text{验证路径}
