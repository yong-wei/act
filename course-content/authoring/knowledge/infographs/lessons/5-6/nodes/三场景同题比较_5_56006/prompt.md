...
一句话定义：
三场景同题比较用常规日、入库高峰和未见扰动检验三条路线的性能收益、能耗、切换、恢复和验证责任。
...
text_contract: 只允许围绕这些标签组织图面文字：三场景同题比较、常规日、入库高峰、未见扰动、恢复时间、指标读解、方法比较；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：三场景同题比较、常规日、入库高峰、未见扰动、恢复时间、指标读解、方法比较。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：E=\sum_k u(k)/60
