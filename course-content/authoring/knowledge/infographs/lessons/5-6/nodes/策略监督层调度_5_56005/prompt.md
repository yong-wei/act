...
一句话定义：
策略监督层调度只在预冷、正常、节能和保守恢复四种模式间选择，不直接输出压缩机占空比。
...
本课关系：
- 显式控制律与学习策略 --引出--> 策略监督层调度
- 策略学习进入条件 --前置--> 策略监督层调度
- 策略学习安全外壳 --应用于--> 策略监督层调度
- 策略监督层调度 --应用于--> 三场景同题比较
...
text_contract: 只允许围绕这些标签组织图面文字：策略监督层调度、策略监督层、模式调度、安全壳、回退、策略学习、安全边界；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：策略监督层调度、策略监督层、模式调度、安全壳、回退、策略学习、安全边界。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：x(k)=[T_m(k),T_p(k),\hat H_{60}(k),T_{\mathrm{amb}}(k)]^T,\qquad m_k\in\{\text{预冷},\text{正常},\text{节能},\text{保守恢复}\}
