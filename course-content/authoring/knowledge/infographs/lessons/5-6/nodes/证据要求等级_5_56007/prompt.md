...
一句话定义：
证据要求等级表示路线选择还需要补充多少模型、数据、压力测试、安全壳和运行监控证据。
...
text_contract: 只允许围绕这些标签组织图面文字：证据要求等级、验证负担、证据要求、安全壳、部署监控、工程验证、证据责任；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：证据要求等级、验证负担、证据要求、安全壳、部署监控、工程验证、证据责任。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：\text{证据要求}=\text{模型说明}+\text{数据覆盖}+\text{压力测试}+\text{安全壳}+\text{部署监控}
