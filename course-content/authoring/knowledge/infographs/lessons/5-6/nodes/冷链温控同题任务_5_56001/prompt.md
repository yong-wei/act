...
一句话定义：
冷链温控同题任务把同一冷藏仓、同一温区约束、同一扰动日志和同一评价指标交给多条控制路线比较。
...
本课关系：
- 方法迁移前边界说明 --引出--> 冷链温控同题任务
- 冷链温控同题任务 --引出--> 二状态冷链热模型
...
text_contract: 只允许围绕这些标签组织图面文字：冷链温控同题任务、冷链温控、同题比较、温区约束、方法迁移、方法比较；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：冷链温控同题任务、冷链温控、同题比较、温区约束、方法迁移、方法比较。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：2\leq T_a(k)\leq 8,\qquad 2\leq T_p(k)\leq 8,\qquad 0\leq u(k)\leq 1
