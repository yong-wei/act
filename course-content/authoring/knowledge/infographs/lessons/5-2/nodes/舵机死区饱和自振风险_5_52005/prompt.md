...
一句话定义：
船舶航向舵机中的阀口死区、驱动限幅和机械间隙可能与线性航向回路共同形成自持振荡，带来能耗、磨损和航向安全裕度问题。
...
建议视觉骨架：
用舵机死区/饱和非线性曲线、闭环舵角反馈和航向振荡波形表达工程自振风险。
...
text_contract: 只允许围绕这些标签组织图面文字：舵机死区饱和自振风险、舵机、死区、饱和、自振风险、抗积分饱和、船舶案例；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：舵机死区饱和自振风险、舵机、死区、饱和、自振风险、抗积分饱和、船舶案例。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：G(s)=\frac{K}{s(s+1)(0.2s+1)}
