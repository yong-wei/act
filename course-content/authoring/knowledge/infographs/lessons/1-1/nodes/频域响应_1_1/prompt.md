...
一句话定义：
频域响应描述系统对不同频率正弦输入的稳态幅值变化和相位变化。

核心直觉：
频域图回答系统更怕慢变化、快变化还是某些特定节奏的扰动。
...
- 频域响应 --图示为--> Bode图: Bode图是频域响应的图形表示
- 时域响应 --互补--> 频域响应: 时域和频域是同一系统的两种互补表征

建议视觉骨架：
用频率轴上的低频、中频、高频响应面板表达幅值变化和相位变化，配一个正弦输入输出小窗。
...
版式约束：
- 视觉类型：频域仪表盘。以 Bode/闭环幅频/三频段读图为主图，配合裕度、边界和工程读回小窗。
- 画布与布局：16:9 横版；左侧或上方为 Bode/闭环幅频主图；2-3 个仪表式小窗显示裕度、带宽或三频段读回；用细线连接读图位置和判断面板。
- 文字范围：只允许围绕这些标签组织图面文字：频域响应、频域响应；全图最多 12 个可见标签；不放正文段落。
