...
一句话定义：
性能指标是用超调量 $M_p$、调节时间 $t_s$、上升时间 $t_r$ 等数字评价响应表现的语言。

核心直觉：
工程判断不能只说“看起来不错”，需要用可比较的数字描述。
...
建议视觉骨架：
用阶跃响应曲线直接标注 M_p、t_s、t_r 三个读数位置，表达工程判断从直觉变成可比较数字。
...
- 文字范围：只允许围绕这些标签组织图面文字：性能指标、性能指标、时域响应；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：性能指标、性能指标、时域响应。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：M_p；t_s
