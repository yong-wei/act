...
一句话定义：
校正设计是根据诊断结果在回路中加入控制器，以改写系统行为的过程。

核心直觉：
校正不是一次性套公式，而是诊断、修改、再诊断、比较和调整的循环。

可用公式（内部依据，图面公式区标题只能写“公式”）：
无公式，图中只放与主题直接相关的短符号。

本课关系：
- 控制器 --支持--> 校正设计: 控制器是执行校正设计的工具
- 反馈 --支持--> 校正设计: 反馈使得校正成为可能——通过修改回路改变系统行为

建议视觉骨架：
用诊断、加控制器、重新诊断、比较、调整的循环流程表达校正设计，主图必须包含三域证据小窗。
...
- 文字范围：只允许围绕这些标签组织图面文字：校正设计、校正设计、控制器；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：校正设计、校正设计、控制器。
