...
一句话定义：
课程总图是整门课程的知识结构地图，以五个模块对应五个连续追问。

核心直觉：
1-1 先让学生看见全貌，后续单元再逐步把每个工具讲深。
...
建议视觉骨架：
用五个模块追问串成课程地图，表达真实问题到统一对象、多种表征、结构机理、约束设计和边界识别的主线。
...
- 文字范围：只允许围绕这些标签组织图面文字：课程总图、课程总图、模块1；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：课程总图、课程总图、模块1。
