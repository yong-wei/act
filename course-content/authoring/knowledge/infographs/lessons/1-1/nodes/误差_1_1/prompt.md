...
一句话定义：
误差信号是期望输出与测量输出的差值，通常记为 $e(t)=r(t)-y_m(t)$。

核心直觉：
控制器真正关心的不是输出本身，而是目标与测量结果之间还差多少。
...
建议视觉骨架：
用期望航向与测量航向在比较点相减形成 e(t) 的结构图表达误差来源，主视觉应突出 r(t)、y_m(t)、e(t) 三个信号。
...
- 文字范围：只允许围绕这些标签组织图面文字：误差信号、误差、反馈；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：误差信号、误差、反馈。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：e(t)=r(t)-y_m(t)
