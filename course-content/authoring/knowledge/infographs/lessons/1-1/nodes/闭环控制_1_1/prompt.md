...
一句话定义：
闭环控制是利用输出信息持续修正控制动作的方式。

核心直觉：
闭环系统持续测量、比较和修正，因此能把偏差纳入控制过程。

可用公式（内部依据，图面公式区标题只能写“公式”）：
无公式，图中只放与主题直接相关的短符号。

本课关系：
- 反馈 --支持--> 闭环控制: 反馈是闭环控制的核心机制
- 闭环控制 --使用--> 误差信号: 闭环控制通过比较期望输出与测量输出形成误差信号，并据此驱动修正
- 闭环控制 --contrasts_with--> 开环控制: 闭环与开环的根本区别在于是否使用输出信息修正动作

建议视觉骨架：
用船舶航向偏差经测量回送后自动修正的闭环结构表达“持续测量、比较、修正”，同时给出开环与闭环轨迹对照小窗。
...
- 文字范围：只允许围绕这些标签组织图面文字：闭环控制、闭环控制、反馈；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：闭环控制、闭环控制、反馈。
