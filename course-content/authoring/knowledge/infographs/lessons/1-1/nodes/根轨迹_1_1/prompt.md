...
一句话定义：
根轨迹是参数变化时闭环极点在复平面上的迁移路径。

核心直觉：
它把“调参数”变成一张可以读的极点迁移地图。
...
- 根轨迹 --提示--> 稳定性: 根轨迹揭示参数变化时系统何时逼近稳定边界

建议视觉骨架：
用 s 平面根轨迹曲线表达增益变化时闭环极点迁移，配一个响应变快或振荡增强的小窗。
...
- 文字范围：只允许围绕这些标签组织图面文字：根轨迹、根轨迹、极点；全图最多 12 个可见标签；不放正文段落。
