...
一句话定义：
反馈是把输出信息送回比较与决策环节的机制，是闭环控制的核心思想。

核心直觉：
系统不是只执行预设动作，而是持续观察结果，并根据偏差修正动作。
...
- 反馈 --支持--> 校正设计: 反馈使得校正成为可能——通过修改回路改变系统行为

建议视觉骨架：
用船舶航向输出回送到比较端的闭合信息回路表达“观察结果、发现偏差、修正动作”，主图必须包含闭环结构和输出响应小窗。
...
- 文字范围：只允许围绕这些标签组织图面文字：反馈、反馈、闭环控制；全图最多 12 个可见标签；不放正文段落。
