...
一句话定义：
比例控制是最简单的控制器，通常记为 $C(s)=K_p$。

核心直觉：
误差越大，修正动作越大；误差越小，修正动作越小。

可用公式（内部依据，图面公式区标题只能写“公式”）：
C(s)=K_p

本课关系：
- 比例控制 --实例--> 控制器: 比例控制是最简单的控制器形式

建议视觉骨架：
用 C(s)=K_p 和误差大小到控制力度的比例关系表达“误差大用力大、误差小用力小”，配闭环响应小窗。
...
- 文字范围：只允许围绕这些标签组织图面文字：比例控制、比例控制、控制器；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：比例控制、比例控制、控制器。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 请准确渲染这些公式本身，放在留白充足的位置；公式区标题只能写“公式”：C(s)=K_p
