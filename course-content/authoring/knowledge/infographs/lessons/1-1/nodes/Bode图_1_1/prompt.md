...
一句话定义：
Bode 图是频域响应的图形表示，包含幅频曲线和相频曲线。

核心直觉：
幅频图看放大或衰减，相频图看滞后程度，两者共同提示稳定裕度和响应节奏。
...
建议视觉骨架：
用 Bode 幅频/相频双图作为主视觉，标出对数频率轴、幅值变化、相位变化和穿越读数位置。
...
- 文字范围：只允许围绕这些标签组织图面文字：Bode图、Bode图、频域响应；全图最多 12 个可见标签；不放正文段落。
...
- 只使用少量短中文标签，优先使用这些词：Bode图、Bode图、频域响应。
