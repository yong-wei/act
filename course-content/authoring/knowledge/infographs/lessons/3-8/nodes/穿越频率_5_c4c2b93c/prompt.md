...
一句话定义：
开环相频特性曲线穿越-180°线时的频率，记为ω_g，是计算幅值裕度的关键频率。

核心直觉：
它是“相位刚好拖到危险边界”的那一点，也是读幅值裕度的固定锚点。
...
- 只使用少量短中文标签，优先使用这些词：穿越频率、-180°穿越、开环相频特性。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 不要绘制密集数学公式；不要让公式变形。若需要公式，只放关键公式锚点中的短符号：\varphi(\omega_g) = -180^\circ
