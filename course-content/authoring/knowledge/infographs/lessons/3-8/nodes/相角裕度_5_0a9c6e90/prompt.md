...
一句话定义：
在开环截止频率 $\omega_c$ 处，相位距离 $-180^\circ$ 还剩下的角度余量。

核心直觉：
它回答的是“系统在最危险的增益交叉点附近，还剩多少相位缓冲”。
...
- 只使用少量短中文标签，优先使用这些词：相角裕度。
- 右侧用简洁小面板表达判断结果或边界，不写长段落。
- 不要绘制密集数学公式；不要让公式变形。若需要公式，只放关键公式锚点中的短符号：\gamma=180^\circ+\angle L(j\omega_c)
