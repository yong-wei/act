...
async function main() {
  ...
  try {
    ...
    await page.getByRole('button', { name: '结构图绘制规范步骤' }).first().waitFor({ timeout: 60000 });
    await page.waitForLoadState('networkidle');
    await page.getByRole('button', { name: '结构图绘制规范步骤', exact: true }).press('Enter');
    ...
    await page.waitForLoadState('networkidle');
    await page.locator('button[data-active-resource-title="结构图绘制规范步骤"][data-active-resource-launch="viewer-shell"]').click();
    await page.waitForLoadState('networkidle');
    ...
    writeFileSync(join(batch, 'browser-verification.json'), JSON.stringify({status: 'passed', method: 'authenticated Playwright Chromium, graph domain -> precise block diagram modeling node -> resource viewer', keyboardGraphNavigation: true, displayedSelfCheck: true, readingCompletions: 0, representativeCard: '结构图绘制规范步骤'}, null, 2) + '\n');
    console.log('PASS graph -> block diagram modeling card viewer -> authored self-check');
