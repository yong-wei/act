"""Bind the reviewed nonlinear source-specific topics to existing semantically suitable goals."""
import hashlib,json
...
orders={'nonlinear-control-foundations':list(range(1,5))}
by_order={c['order']:c for c in inventory['cards']};assert set(by_order)==set(range(1,5))
assert sorted(n for values in orders.values() for n in values)==list(range(1,5))
...
print('Registered 4 reviewed targets in the existing nonlinear-control goal')
