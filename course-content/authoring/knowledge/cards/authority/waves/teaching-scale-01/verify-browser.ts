...
async function main() {
  ...
  try {
    ...
    await page.waitForLoadState('networkidle');
    await page.getByRole('button', { name: '增益', exact: true }).press('Enter');
    ...
    await page.waitForLoadState('networkidle');
    await page.locator('button[data-active-resource-title="增益"]').click();
    await page.waitForLoadState('networkidle');
    ...
    writeFileSync(join(batch, 'browser-verification.json'), JSON.stringify({status: 'passed', method: 'authenticated Playwright Chromium, graph domain -> precise gain node -> resource viewer', keyboardGraphNavigation: true, displayedSelfCheck: true, readingCompletions: 0, representativeCard: '增益'}, null, 2) + '\n');
    console.log('PASS graph -> gain card viewer -> authored self-check');
