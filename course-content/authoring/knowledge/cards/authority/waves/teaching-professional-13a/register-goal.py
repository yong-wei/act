"""Register the reviewed nonlinear-control cards through existing goal catalogs."""
import hashlib
import json
...
goal='nonlinear-control-foundations'
title='非线性控制基础'
description='理解逆系统、状态反馈与耗散方法，核验可逆性、内部动态和输入约束。'
...
text=text.replace('covers the thirty preset goals','covers the thirty-one preset goals')
if "      '"+goal+"'," not in text:
    needle="      'robust-control-foundations',\n";assert text.count(needle)==1
    text=text.replace(needle,needle+"      '"+goal+"',\n")
pending[p]=text
p=ROOT/'src/lib/__tests__/adaptive-path-goal-options.test.ts';text=p.read_text().replace('toHaveLength(30)','toHaveLength(31)')
if "      '"+goal+"'," not in text:
    marker="      'robust-control-foundations',\n";assert text.count(marker)==1
...
print('Registered six accepted targets and nonlinear-control goal in server/client catalogs')
