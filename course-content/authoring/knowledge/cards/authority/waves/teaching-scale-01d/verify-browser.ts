...
async function main() {
  ...
  try {
    ...
    await page.waitForLoadState('networkidle');
    await page.getByRole('button', { name: '直流增益', exact: true }).press('Enter');
    ...
    await page.waitForLoadState('networkidle');
    await page.locator('button[data-active-resource-title="直流增益"]').click();
    await page.waitForLoadState('networkidle');
    ...
    writeFileSync(join(batch, 'browser-verification.json'), JSON.stringify({status: 'passed', method: 'authenticated Playwright Chromium, graph domain -> precise DC gain node -> resource viewer', keyboardGraphNavigation: true, displayedSelfCheck: true, readingCompletions: 0, representativeCard: '直流增益'}, null, 2) + '\n');
    console.log('PASS graph -> DC gain card viewer -> authored self-check');
