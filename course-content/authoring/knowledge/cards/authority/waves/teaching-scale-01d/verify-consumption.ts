...
const inventories = ['teaching-batch-01', 'teaching-batch-02', 'teaching-scale-01', 'teaching-scale-01b', 'teaching-scale-01d'].map((name) => read(join(batch, '..', name, 'inventory.json')));
const cards = inventories.flatMap((d) => d.cards);
const acceptedScope = read(join(batch, 'accepted-scope.json'));
assert.deepEqual(inventories[4].cards.map((c: any) => c.canonicalId).sort(), acceptedScope.cards.map((c: any) => c.canonicalId).sort());
assert.equal(cards.length, 48 + acceptedScope.cards.length);
...
let retiredCount = 0;
for (const name of ['teaching-batch-01', 'teaching-batch-02', 'teaching-scale-01', 'teaching-scale-01b', 'teaching-scale-01d']) {
  const retirement = read(join(batch, '..', name, 'retirement.json'));
  if (name === 'teaching-scale-01d') {
    ...
    assert.deepEqual(receipt.batchCanonicalIds, inventories[4].cards.map((c: any) => c.canonicalId).sort());
  }
