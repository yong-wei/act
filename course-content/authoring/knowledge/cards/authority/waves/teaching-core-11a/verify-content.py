"""Verify teaching-core-11a authoring cards without runtime writes."""
...
EXPECTED_IDS = ['ctc:v11g-11d00d2120a83f5e05efdb6e', 'ctc:v12g-b536959708e2d0fe3c8c791a', 'ctkg:v3e-canonical-15b23d588244f407f6ed4b35', 'ctkg:v3e-canonical-3311578f3f7796d3b9ff0830', 'ctkg:v3e-canonical-40d0c236680874a1246a0536', 'ctkg:v3e-canonical-6bb580c242f908566ab53b18', 'ctkg:v3e-canonical-7ee407860ee485f1e1b1f3d1', 'ctkg:v3e-canonical-9c5f83612d3255aa5805a9cb', 'ctkg:v3e-canonical-ad988c74286cfcad2c3fdf98', 'ctkg:v3e-canonical-b9fa7b3ae0d8d7044619ac55', 'ctkg:v3e-canonical-c6fafdb175fcf88882f8b91f']
...
def model_claim_checks(cards, models):
    snippets=[
        ['16.3033', '1.81380', '1.16303'],
        ['omega_n=2', 'zeta=0.5', 's^2+2s+4'],
        ['cos(2t)', 'E=\\frac12', '周期为 $\\pi$'],
        ['1.89636', '7.82405', '4T=8'],
        ['c_{\\mathrm{crit}}', 'c/4', 'm=1', 'k=4'],
        ['1.94486', '4.43571', '4t e^{-2t}'],
        ['T_m=2', 'K=8', '1.98431', 'frac18=0.125'],
        ['3\\sqrt3/2', 'frac9{s^2+3s+9}', '2/3'],
        ['zeta=-0.5', 's=1\\pm j\\sqrt3', 'pm2j'],
        ['p_s=-4+2\\sqrt3', '4.43571', '1.94486'],
        ['3.62760', '1.81380', '3\\pi/\\sqrt3', '严格周期函数'],
    ]
    true('eleven model-backed topics',len(models)==11)
...
def verify_parser_and_katex(cards: list[dict[str, object]]) -> dict[str, object]:
    with tempfile.TemporaryDirectory(prefix="teaching-core-11a-parser-") as temp:
        env = os.environ.copy()
        env.update({
...
def main() -> None:
    ...
    true("exact eleven scope cards", len(scope["cards"]) == 11)
    ...
    true("source backup count", len(list(BATCH.glob("*-source.json"))) == 11)
    true("previous copy count", len(list((BATCH / "previous").glob("*.md"))) == 11)
