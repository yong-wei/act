...
async function main() {
  ...
  try {
    ...
    await page.getByRole('button', { name: '动态系统建模规范步骤' }).first().waitFor({ timeout: 60000 });
    await page.waitForLoadState('networkidle');
    await page.getByRole('button', { name: '动态系统建模规范步骤', exact: true }).press('Enter');
    ...
    await page.waitForLoadState('networkidle');
    await page.locator('button[data-active-resource-title="动态系统建模规范步骤"]').click();
    await page.waitForLoadState('networkidle');
    ...
    writeFileSync(join(batch, 'browser-verification.json'), JSON.stringify({status: 'passed', method: 'authenticated Playwright Chromium, graph domain -> precise modeling procedure node -> resource viewer', keyboardGraphNavigation: true, displayedSelfCheck: true, readingCompletions: 0, representativeCard: '动态系统建模规范步骤'}, null, 2) + '\n');
    console.log('PASS graph -> modeling procedure card viewer -> authored self-check');
