"""Verify teaching-core-12a authoring cards without runtime writes."""
...
EXPECTED_IDS = ['ctc:v11g-1c9f955b68fd25e1f4ad86f6', 'ctc:v11g-9dfc90c240d6bf3ebc285faf', 'ctkg:v3e-canonical-0fa44601675cb175f5df9562', 'ctkg:v3e-canonical-9988510f6127d3086f4aa265', 'ctkg:v3e-canonical-9cd2a9dfb0f7f3d457247609', 'ctkg:v3e-canonical-a9f8ff29f0e4550a22bfcc67', 'ctkg:v3e-canonical-b1fa48122cb30019661f9eaa', 'ctkg:v3e-canonical-bd871a5da052c90d142e285a', 'ctkg:v3e-canonical-bf7bc53bf8286e07fa6035cc', 'ctkg:v3e-canonical-c8eb5072bc1783d917169c9a', 'ctkg:v3e-canonical-d1d8f9f3725b9a8440cdd486', 'ctkg:v3e-canonical-f9718484925341d6ce1dec78']
...
def model_claim_checks(cards, models):
    snippets=[
        ['1.17674', '4.03817', '4.055864', '0.02'],
        ['K=6', '(s+3)(s^2+2)', 'K=5', 'K=7'],
        ['0.4\\sin(2t)-0.8\\cos(2t)', '0.8e^{-t}', 'arctan(2)'],
        ['frac4{\\sqrt3}', '1.81380', '1.16303'],
        ['2.64455', '3.139573', '4.03817'],
        ['0.818786', '1.20920', '0.244115', '1.062901'],
        ['H(t-0.5)', '2.313799', '1.813799'],
        ['0.8e^{-t}', '实际初值0', '解为 $y(t)=0$'],
        ['1.678954', '1.944860', '(1+2t)e^{-2t}>0'],
        ['t-1+e^{-t}', '1-e^{-t}', '自然模态 $e^{-t}$'],
        ['4.43571', '0.535898', 'dot y(0^+)=0'],
        ['ddot y(0^+)=4', '1.20920', '1.81380', '4.03817'],
    ]
    true('twelve model-backed topics',len(models)==12)
    for row,expected in zip(cards,snippets):
        text=(ROOT/row['authoringPath']).read_text()
        for snippet in expected:true(f"model-backed claim {row['cardId']}: {snippet}",snippet in text)
        for formula in re.findall(r'\$\$([\s\S]*?)\$\$|\$([^$\n]+)\$',text):
            true('no bare command: '+row['cardId'],not re.search(r'(?<!\\)\b(?:qquad|pm)\b',''.join(formula)))
...
def verify_parser_and_katex(cards: list[dict[str, object]]) -> dict[str, object]:
    with tempfile.TemporaryDirectory(prefix="teaching-core-12a-parser-") as temp:
        env = os.environ.copy()
        env.update({
...
def main() -> None:
    ...
    true("exact twelve scope cards", len(scope["cards"]) == 12)
    ...
    true("source backup count", len(list(BATCH.glob("*-source.json"))) == 12)
    true("previous copy count", len(list((BATCH / "previous").glob("*.md"))) == 12)
