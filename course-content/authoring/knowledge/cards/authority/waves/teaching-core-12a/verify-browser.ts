...
async function main() {
  ...
  try {
    ...
    await page.getByRole('button', { name: '稳定时间' }).first().waitFor({ timeout: 60000 });
    await page.waitForLoadState('networkidle');
    await page.getByRole('button', { name: '稳定时间', exact: true }).press('Enter');
    ...
    await page.waitForLoadState('networkidle');
    await page.locator('button[data-active-resource-title="稳定时间"][data-active-resource-launch="viewer-shell"]').click();
    await page.waitForLoadState('networkidle');
    ...
    writeFileSync(join(batch, 'browser-verification.json'), JSON.stringify({status: 'passed', method: 'authenticated Playwright Chromium, graph domain -> precise two-percent settling node -> resource viewer', keyboardGraphNavigation: true, displayedSelfCheck: true, readingCompletions: 0, representativeCard: '稳定时间'}, null, 2) + '\n');
    console.log('PASS graph -> two-percent settling card viewer -> authored self-check');
