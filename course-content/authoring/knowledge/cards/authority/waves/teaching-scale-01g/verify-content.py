"""Verify teaching-scale-01g authoring cards without runtime writes."""
...
EXPECTED_IDS = [
    "ctkg:v3e-object-3fe181aa7c0967f58d1dbfee",
    "ctkg:v3e-object-7d20806108956a0f52c08f3a",
    "ctkg:v3e-object-17cae71babbbfe193c0124a4",
    "ctkg:v3e-object-0a05ec9e84791b95d3794bc0",
    "ctkg:v3e-object-c437a7c8ef65d5677ad5c387",
    "ctkg:v3e-object-e1caec9f68da5c573be699d5",
    "ctkg:v3e-object-d80dfe6ce979515d21845c81",
]
...
def relation_checks(card_text: str, row: dict[str, object], neighborhood: dict[str, object],
    ...
    for entry in declared:
        ...
        rendered = f"- **{objects[entry['otherId']]}**（{relative}，关系：{predicates[relation['predicate']]}）"
...
def model_claim_checks(cards: list[dict[str, object]], models: dict[str, object]) -> None:
    by_id = {row["canonicalId"]: (ROOT / row["authoringPath"]).read_text(encoding="utf-8") for row in cards}
    expected = {
        EXPECTED_IDS[0]: [
            "K=1", "K=4", "-2", "-5", "0.5", "0.2",
            str(models["P_1"]["initialControl"]), str(models["P_4"]["initialControl"]),
        ],
        EXPECTED_IDS[1]: [
            "K_p=2", "K_i=1", "0.9999999360059044", "2.0",
            "-2.618033988749895", "-0.38196601125010515",
        ],
        EXPECTED_IDS[2]: [
            "1.0000000021336248", "1.1630330651827194", "1.298435677012138",
            "0.9999999999868622", "1.0000000000515648", "1.0",
        ],
        EXPECTED_IDS[3]: ["0.1s+2", "0.05", "0.5", "-1", "10"],
        EXPECTED_IDS[4]: ["K=9", "9", "3", "K_p=2", "K_i=1", "2", "误差为 $0$"],
        EXPECTED_IDS[5]: ["4/(s+5)", "4/(s+2)", "(s+3)^2", "(s+2)^2"],
        EXPECTED_IDS[6]: [
            "0.7861513777573741", "51.82729237298949", "0.9999509697194077",
            "79.15178050016591", "0.1", "K_v=10", "-8.886051522703896",
            "-0.11110937749575278",
        ],
    }
...
def verify_parser_and_katex(cards: list[dict[str, object]]) -> dict[str, object]:
    with tempfile.TemporaryDirectory(prefix="teaching-scale-01g-parser-") as temp:
        env = os.environ.copy()
        env.update({
...
def main() -> None:
    ...
    for row in source_inventory["cards"]:
        ...
        true(f"body materialized: {card_id}", body_path.read_text(encoding="utf-8") == body)
        item = dict(row)
