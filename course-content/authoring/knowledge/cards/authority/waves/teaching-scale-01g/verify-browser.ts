...
async function main() {
  ...
  try {
    ...
    await page.getByRole('button', { name: '经典控制设计', exact: true }).press('Enter');
    await page.getByText('正在加载知识图谱…', { exact: true }).waitFor({ state: 'hidden', timeout: 60000 });
    await page.getByRole('button', { name: '串联滞后-超前校正' }).first().waitFor({ timeout: 60000 });
    await page.waitForLoadState('networkidle');
    await page.getByRole('button', { name: '串联滞后-超前校正', exact: true }).press('Enter');
    ...
    await page.waitForLoadState('networkidle');
    await page.locator('button[data-active-resource-title="串联滞后-超前校正"]').click();
    await page.waitForLoadState('networkidle');
    ...
    writeFileSync(join(batch, 'browser-verification.json'), JSON.stringify({status: 'passed', method: 'authenticated Playwright Chromium, graph domain -> precise lead-lag compensation node -> resource viewer', keyboardGraphNavigation: true, displayedSelfCheck: true, readingCompletions: 0, representativeCard: '串联滞后-超前校正'}, null, 2) + '\n');
    console.log('PASS graph -> lead-lag compensation card viewer -> authored self-check');
