...
async function main() {
  ...
  try {
    ...
    await page.getByRole('button', { name: '稳定性分析', exact: true }).press('Enter');
    await page.getByText('正在加载知识图谱…', { exact: true }).waitFor({ state: 'hidden', timeout: 60000 });
    await page.getByRole('button', { name: '劳斯-赫尔维茨稳定性判据法' }).first().waitFor({ timeout: 60000 });
    await page.waitForLoadState('networkidle');
    await page.getByRole('button', { name: '劳斯-赫尔维茨稳定性判据法', exact: true }).press('Enter');
    ...
    await page.waitForLoadState('networkidle');
    await page.locator('button[data-active-resource-title="劳斯-赫尔维茨稳定性判据法"]').click();
    await page.waitForLoadState('networkidle');
    ...
    writeFileSync(join(batch, 'browser-verification.json'), JSON.stringify({status: 'passed', method: 'authenticated Playwright Chromium, graph domain -> precise Routh array method node -> resource viewer', keyboardGraphNavigation: true, displayedSelfCheck: true, readingCompletions: 0, representativeCard: '劳斯-赫尔维茨稳定性判据法'}, null, 2) + '\n');
    console.log('PASS graph -> Routh array method card viewer -> authored self-check');
