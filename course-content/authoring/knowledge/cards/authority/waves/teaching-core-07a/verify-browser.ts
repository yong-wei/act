...
async function main() {
  ...
  try {
    ...
    await page.getByRole('button', { name: '梅森增益公式' }).first().waitFor({ timeout: 60000 });
    await page.waitForLoadState('networkidle');
    await page.getByRole('button', { name: '梅森增益公式', exact: true }).press('Enter');
    ...
    await page.waitForLoadState('networkidle');
    await page.locator('button[data-active-resource-title="梅森增益公式"][data-active-resource-launch="viewer-shell"]').click();
    await page.waitForLoadState('networkidle');
    ...
    writeFileSync(join(batch, 'browser-verification.json'), JSON.stringify({status: 'passed', method: 'authenticated Playwright Chromium, graph domain -> precise Mason gain formula node -> resource viewer', keyboardGraphNavigation: true, displayedSelfCheck: true, readingCompletions: 0, representativeCard: '梅森增益公式'}, null, 2) + '\n');
    console.log('PASS graph -> Mason gain formula card viewer -> authored self-check');
