"""Bind the reviewed linear-quadratic topics to existing semantically suitable goals."""
import hashlib,json
...
orders={'optimal-control-foundations':list(range(1,9))}
by_order={c['order']:c for c in inventory['cards']};assert set(by_order)==set(range(1,9))
assert sorted(n for values in orders.values() for n in values)==list(range(1,9))
...
print('Registered 8 reviewed targets in the existing optimal-control goal')
