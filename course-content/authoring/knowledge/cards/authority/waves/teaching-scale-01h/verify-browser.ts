...
async function main() {
  ...
  try {
    ...
    await page.getByRole('button', { name: '全状态反馈设计步骤' }).first().waitFor({ timeout: 60000 });
    await page.waitForLoadState('networkidle');
    await page.getByRole('button', { name: '全状态反馈设计步骤', exact: true }).press('Enter');
    ...
    await page.waitForLoadState('networkidle');
    await page.locator('button[data-active-resource-title="全状态反馈设计步骤"]').click();
    await page.waitForLoadState('networkidle');
    ...
    writeFileSync(join(batch, 'browser-verification.json'), JSON.stringify({status: 'passed', method: 'authenticated Playwright Chromium, graph domain -> precise full-state feedback node -> resource viewer', keyboardGraphNavigation: true, displayedSelfCheck: true, readingCompletions: 0, representativeCard: '全状态反馈设计步骤'}, null, 2) + '\n');
    console.log('PASS graph -> full-state feedback card viewer -> authored self-check');
