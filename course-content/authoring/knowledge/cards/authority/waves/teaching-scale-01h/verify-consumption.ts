...
const inventories = ['teaching-batch-01', 'teaching-batch-02', 'teaching-scale-01', 'teaching-scale-01b', 'teaching-scale-01d', 'teaching-scale-01c', 'teaching-scale-01f', 'teaching-scale-01e', 'teaching-scale-01h'].map((name) => read(join(batch, '..', name, 'inventory.json')));
const cards = inventories.flatMap((d) => d.cards);
const recoveryFiles = ['teaching-batch-01', 'teaching-batch-02', 'teaching-scale-01', 'teaching-scale-01b', 'teaching-scale-01d', 'teaching-scale-01c', 'teaching-scale-01f', 'teaching-scale-01e', 'teaching-scale-01h']
...
assert.deepEqual(inventories[8].cards.map((c: any) => c.canonicalId).sort(), acceptedScope.cards.map((c: any) => c.canonicalId).sort());
assert.equal(cards.length, 78 + acceptedScope.cards.length);
...
let retiredCount = 0;
for (const name of ['teaching-batch-01', 'teaching-batch-02', 'teaching-scale-01', 'teaching-scale-01b', 'teaching-scale-01d', 'teaching-scale-01c', 'teaching-scale-01f', 'teaching-scale-01e', 'teaching-scale-01h']) {
  const retirement = read(join(batch, '..', name, 'retirement.json'));
  if (name === 'teaching-scale-01h') {
    ...
    assert.deepEqual(receipt.batchCanonicalIds, inventories[8].cards.map((c: any) => c.canonicalId).sort());
  }
