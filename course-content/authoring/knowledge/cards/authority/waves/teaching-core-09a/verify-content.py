"""Verify teaching-core-09a authoring cards without runtime writes."""
...
EXPECTED_IDS = ['ctc:v11g-2d0a8b878047bb6a5effc783', 'ctc:v11g-4aec82cd79a002c70fb185b3', 'ctc:v11g-5a7e22337cb506e673b9affe', 'ctc:v11g-69118dc66489d66b1732901b', 'ctc:v11g-c7228e5d5d171b0633b246e3', 'ctkg:domainconcept:cb7da716d9229f0af132e0c1', 'ctkg:v3e-canonical-91f03880e67e6587b27f7039']
...
def model_claim_checks(cards, models):
    snippets=[
        ['s^2+4s+6', '1/6', '2/3', '分子不同'],
        ['0.7-1.1j', '(s+1)(s+2)', '不含 $K$'],
        ['s+1-h', '-0.5', '+1', '2.5', '0.8'],
        ['y=x+d', 's+1}{s+2', '0.5+0.5e^{-2t}', '初始立即跳到 1'],
        ['frac2{s+3}', 'frac13', 'e=r-y'],
        ['u=r', 'd=0.5', 'frac13(1-e^{-3t})'],
        ['K=2', 'K=5', '1.41421', '3.53553', '-135', '无界'],
    ]
    true('seven model-backed topics',len(models)==7)
...
def verify_parser_and_katex(cards: list[dict[str, object]]) -> dict[str, object]:
    with tempfile.TemporaryDirectory(prefix="teaching-core-09a-parser-") as temp:
        env = os.environ.copy()
        env.update({
