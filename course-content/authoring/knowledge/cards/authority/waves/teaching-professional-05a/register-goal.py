"""Bind the reviewed Lyapunov stability topics to existing semantically suitable goals."""
import hashlib,json
...
orders={'stability-concepts-foundations':list(range(1,16))}
by_order={c['order']:c for c in inventory['cards']};assert set(by_order)==set(range(1,16))
assert sorted(n for values in orders.values() for n in values)==list(range(1,16))
...
print('Registered 15 reviewed targets in the existing stability-concepts goal')
