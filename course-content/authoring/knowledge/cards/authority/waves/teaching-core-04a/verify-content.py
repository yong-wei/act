"""Verify teaching-core-04a authoring cards without runtime writes."""
...
EXPECTED_IDS = ['ctc:modeling-023f560241f0e47081f0f59e', 'ctc:modeling-348746457d644126ff8ecf77', 'ctc:modeling-352b9bfe27ba1ab6b9c9672e', 'ctkg:v3e-object-1f331a007aa3ca6c2c95d297', 'ctkg:v3e-object-239fecfdb347e1a48fb9d7cc', 'ctkg:v3e-object-23a1c9229387ded552fbdbb3', 'ctkg:v3e-object-9b795692bacd0f13270cc086', 'ctkg:v3e-object-f1ec085ab5a77428affa50eb', 'ctkg:v3e-object-fac170446815fe168f9a5469', 'ctkg:v3e-object-ff560d5d53d578d0bc57baeb']
...
def model_claim_checks(cards, models):
    snippets = [
        ['q(t)=t', 'v(t)=1', '初始输出'],
        ['N^2=0', 't e^{-t}', '-I'],
        ['-2\\dot q', '-3\\dot q^2', '-1,-2'],
        ['9', '20', 's^2+9s+20', '-Ln'],
        ['6-12t', '3t^2-2t^3', '6t-6t^2', '1/12'],
        ['秩为 1', 'lambda=-2', '完全可观测'],
        ['1<2', 'e^{-2t}', 'det'],
        ['k_1=12,k_2=7', 's^2+7s+12', '-12&-7'],
        ['特征值互异', 'A=-I', '只有秩 1'],
        ['D=0', '1=p', '秩为 0'],
    ]
    true('ten independently verified models', len(models) == 10)
...
def verify_parser_and_katex(cards: list[dict[str, object]]) -> dict[str, object]:
    with tempfile.TemporaryDirectory(prefix="teaching-core-04a-parser-") as temp:
        env = os.environ.copy()
        env.update({
...
def main() -> None:
    ...
    true("exact ten scope cards", len(scope["cards"]) == 10)
    ...
    true("source backup count", len(list(BATCH.glob("*-source.json"))) == 10)
    true("previous copy count", len(list((BATCH / "previous").glob("*.md"))) == 10)
