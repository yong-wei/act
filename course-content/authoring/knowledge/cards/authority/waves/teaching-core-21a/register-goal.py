...
orders={'control-correction':[1,4,5,6,7,9],'feedback-structure-foundations':[2,3,10],
        'steady-state-control-foundations':[8,11]}
by_order={c['order']:c for c in inventory['cards']};assert set(by_order)==set(range(1,12))
assert sorted(n for values in orders.values() for n in values)==list(range(1,12))
...
print('Registered 11 reviewed targets across existing design, feedback-structure and tracking-precision goals')
