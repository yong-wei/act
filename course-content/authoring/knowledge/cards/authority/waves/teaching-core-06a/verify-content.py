"""Verify teaching-core-06a authoring cards without runtime writes."""
...
EXPECTED_IDS = ['ctc:modeling-385c515b4d8dbb550b664072', 'ctc:modeling-4c7a0114a15c9e4039f6d193', 'ctc:modeling-5e62391de61ac1af304601cb', 'ctc:modeling-9121dc2c97afa77f52ce0220', 'ctc:modeling-a2b2e4c67cee79eef87f3b81', 'ctc:modeling-v2r-5585bf06f0e579bc1c362ac2']
...
def model_claim_checks(cards, models):
    snippets = [
        ['没有入支路', '不是纯汇点', '19}{13', '70}{13'],
        ['L=3', '-0.3', '1.3', '不是 $3-0.1'],
        ['x=2r-0.1y', '-7/13', '57/13'],
        ['五条支路', '行列式为 $1.3$', '70}{13'],
        ['2\\times3\\times1=6', '1\\times1=1', '重复经过 $y$'],
        ['唯一基本回路', '(-0.3)^2=0.09', '闭合游走'],
    ]
...
def verify_parser_and_katex(cards: list[dict[str, object]]) -> dict[str, object]:
    with tempfile.TemporaryDirectory(prefix="teaching-core-06a-parser-") as temp:
        env = os.environ.copy()
        env.update({
