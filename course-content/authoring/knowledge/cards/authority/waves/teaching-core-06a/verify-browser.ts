...
async function main() {
  ...
  try {
    ...
    await page.getByRole('button', { name: '信号流图输入输出节点' }).first().waitFor({ timeout: 60000 });
    await page.waitForLoadState('networkidle');
    await page.getByRole('button', { name: '信号流图输入输出节点', exact: true }).press('Enter');
    ...
    await page.waitForLoadState('networkidle');
    await page.locator('button[data-active-resource-title="信号流图输入输出节点"][data-active-resource-launch="viewer-shell"]').click();
    await page.waitForLoadState('networkidle');
    ...
    writeFileSync(join(batch, 'browser-verification.json'), JSON.stringify({status: 'passed', method: 'authenticated Playwright Chromium, graph domain -> precise signal-flow nodes node -> resource viewer', keyboardGraphNavigation: true, displayedSelfCheck: true, readingCompletions: 0, representativeCard: '信号流图输入输出节点'}, null, 2) + '\n');
    console.log('PASS graph -> signal-flow nodes card viewer -> authored self-check');
