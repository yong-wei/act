"""Verify teaching-core-13a authoring cards without runtime writes."""
...
def numeric_forms(value: object) -> set[str]:
    ...
    return forms | {
        form.rstrip("0").rstrip(".") if "." in form else form
        for form in forms
    }
...
def contains_number(text: str, value: object) -> bool:
    return any(
        re.search(rf"(?<![\d.]){re.escape(form)}(?!\d|\.\d)", text)
        for form in numeric_forms(value)
    )


def model_claim_checks(cards, models):
    true("model report has expected sections", all(
        key in models
        for key in ("motor_time_constant", "itae", "second_order_A", "second_order_B", "design", "minimum_beat")
    ))
    card_texts = {
        row["name"]: (ROOT / row["authoringPath"]).read_text(encoding="utf-8")
        for row in cards
    }

    def text_for(label: str, *names: str) -> str:
        selected = "\n".join(card_texts[name] for name in names if name in card_texts)
        true(f"model-backed card present: {label}", bool(selected))
        return selected

    claims = [
        (
            "motor_time_constant",
            text_for("motor time constant", "时间常数τ1"),
            ("tauSeconds", "steadySpeedRadPerSecond", "speedAtTau"),
        ),
        (
            "itae",
            text_for("ITAE", "ITAE性能指标"),
            ("T", "infiniteHorizonITAE"),
        ),
        (
            "second_order_A",
            text_for("second-order A", "动态性能指标", "动态性能", "使用时域指标的设计综合"),
            ("zeta", "wn", "overshootPercent", "peakTime", "zeroTo100Rise",
             "twoPercentSettling", "twoPercentEnvelopeUpperBound"),
        ),
        (
            "second_order_B",
            text_for("second-order B", "动态性能"),
            ("zeta", "wn", "overshootPercent", "zeroTo100Rise",
             "twoPercentSettling"),
        ),
        (
            "design",
            text_for("time-domain design", "使用时域指标的设计综合"),
            ("zetaMinimumFromOvershoot",),
        ),
        (
            "minimum_beat",
            text_for("continuous-time minimum-beat response", "最小节拍响应"),
            ("overshootPercent", "numericallyLocatedTwoPercentSettling", "sourceRoundedSettling"),
        ),
    ]
    for model_name, body, fields in claims:
        for field in fields:
            true(
                f"model-backed claim {model_name}.{field}",
                field in models[model_name] and contains_number(body, models[model_name][field]),
            )
    for row in cards:
        text = card_texts[row["name"]]
        for formula in re.findall(r'\$\$([\s\S]*?)\$\$|\$([^$\n]+)\$', text):
            true(
                "no bare command: " + row["cardId"],
                not re.search(r'(?<!\\)\b(?:qquad|pm)\b', "".join(formula)),
            )
