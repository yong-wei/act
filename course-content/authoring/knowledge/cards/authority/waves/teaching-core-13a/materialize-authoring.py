"""Materialize the captured bodies with exact source and relation metadata."""
...
scope=json.loads((BATCH/'scope.json').read_text())
assert scope['batchId'] == s['batchId']
assert [row['canonicalId'] for row in scope['cards']] == [row['canonicalId'] for row in s['cards']]
assert [row['order'] for row in s['cards']] == list(range(1, len(s['cards']) + 1))
...
scope['status']='authoring';write(BATCH/'scope.json',scope)
write(BATCH/'accepted-scope.json',{**scope,'status':'pending-independent-review','accepted':False})
print(f"Materialized {len(pending)} candidate cards with exact relations")
