...
try {
  for (const name of [batch]) {
    const inventory = JSON.parse(readFileSync(join(name, 'inventory.json'), 'utf8'));
    ...
    process.chdir(root);
    results.push({ batchId: inventory.batchId, cards: inventory.cards.length, status: 'passed' });
  }
} finally {
  process.chdir(root);
  rmSync(temporary, { recursive: true, force: true });
}
writeFileSync(join(batch, 'author-markdown-verification.json'), JSON.stringify({ status: 'passed', runtimeWritten: false, results }, null, 2) + '\n');
