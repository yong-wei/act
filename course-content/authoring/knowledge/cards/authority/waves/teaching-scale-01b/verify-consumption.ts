...
const inventories = ['teaching-batch-01', 'teaching-batch-02', 'teaching-scale-01', 'teaching-scale-01b'].map((name) => read(join(batch, '..', name, 'inventory.json')));
const cards = inventories.flatMap((d) => d.cards);
const acceptedScope = read(join(batch, 'accepted-scope.json'));
assert.deepEqual(inventories[3].cards.map((c: any) => c.canonicalId).sort(), acceptedScope.cards.map((c: any) => c.canonicalId).sort());
assert.equal(cards.length, 40 + acceptedScope.cards.length);
...
for (const name of ['teaching-batch-01', 'teaching-batch-02', 'teaching-scale-01', 'teaching-scale-01b']) {
  ...
  if (name === 'teaching-scale-01b') {
    ...
    assert.deepEqual(receipt.batchCanonicalIds, inventories[3].cards.map((c: any) => c.canonicalId).sort());
  }
