"""Verify teaching-core-10a authoring cards without runtime writes."""
...
EXPECTED_IDS = ['ctc:modeling-5da44f2fc98d268259459e04', 'ctkg:m3-v2w:source-object:2107b134aa2e942c147b9594', 'ctkg:v3e-canonical-0e5d22ed0afee37f65584d36', 'ctkg:v3e-canonical-16ddaebf2fe19d65ecc35886', 'ctkg:v3e-canonical-2ec135b0a8506867adbe4bf1', 'ctkg:v3e-canonical-303314b2f1d601cdba5e3f99', 'ctkg:v3e-canonical-b03bdd7d909a44a2830459ca', 'ctkg:v3e-canonical-f8bf6b65acbd8dc67f1032e3', 'ctkg:v3e-object-12baf451eabe4bb9e3a7eb0f']
...
def model_claim_checks(cards, models):
    snippets=[
        ['y(0^+)=2', 'h(t)=2e^{-t}', '单位面积'],
        ['frac12e^{-t}', 'frac12e^{-3t}', 't e^{-t}'],
        ['2\\sin(3t+\\pi/6)', 's+3\\sqrt3', 'r(0^+)', '3/(2\\pi)'],
        ['3e^{-t}', '2+e^{-t}', '系数为 1'],
        ['2(t-1+e^{-t})', '3e^{-t}', 'tau-1'],
        ['3H(t-1)', '3e^{-s}', '6(1-e^{-(t-1)})'],
        ['tfrac12t^2H(t)', '1/s^3', '3/(s^2+9)'],
        ['0.773804', '0.739450', '0.735759', '1/\\varepsilon'],
        ['frac43e^{-3t}', '跟踪误差为 $1/3$', '控制量为 2'],
    ]
...
def verify_parser_and_katex(cards: list[dict[str, object]]) -> dict[str, object]:
    with tempfile.TemporaryDirectory(prefix="teaching-core-10a-parser-") as temp:
        env = os.environ.copy()
        env.update({
...
def main() -> None:
    ...
    true("ambiguous response taxonomy not projected", all(r["relationId"] != "ctr:m1f-bridge-relation-ad5c8e4d866cbe9847b254cd" for c in relation_input["cards"] for r in c["relations"]))
