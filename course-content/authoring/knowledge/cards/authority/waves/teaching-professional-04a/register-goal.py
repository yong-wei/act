...
orders={'state-space-controllability-foundations':[1,2,3], 'state-space-analysis-foundations':[4,5,6,7,8,9], 'steady-state-control-foundations':[10]}
by_order={c['order']:c for c in inventory['cards']};assert set(by_order)==set(range(1,11))
assert sorted(n for values in orders.values() for n in values)==list(range(1,11))
...
print('Registered 10 reviewed targets across existing state-space and steady-state goals')
