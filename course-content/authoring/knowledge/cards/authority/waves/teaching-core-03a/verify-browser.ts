...
async function main() {
  ...
  try {
    ...
    await page.getByRole('button', { name: '平衡点泰勒级数线性化' }).first().waitFor({ timeout: 60000 });
    await page.waitForLoadState('networkidle');
    await page.getByRole('button', { name: '平衡点泰勒级数线性化', exact: true }).press('Enter');
    ...
    await page.waitForLoadState('networkidle');
    await page.locator('button[data-active-resource-title="平衡点泰勒级数线性化"][data-active-resource-launch="viewer-shell"]').click();
    await page.waitForLoadState('networkidle');
    ...
    writeFileSync(join(batch, 'browser-verification.json'), JSON.stringify({status: 'passed', method: 'authenticated Playwright Chromium, graph domain -> precise equilibrium linearization node -> resource viewer', keyboardGraphNavigation: true, displayedSelfCheck: true, readingCompletions: 0, representativeCard: '平衡点泰勒级数线性化'}, null, 2) + '\n');
    console.log('PASS graph -> equilibrium linearization card viewer -> authored self-check');
