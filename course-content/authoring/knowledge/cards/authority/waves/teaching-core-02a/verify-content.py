"""Verify teaching-core-02a authoring cards without runtime writes."""
...
EXPECTED_IDS = ['ctc:909e732d40f972bf760fa8ff', 'ctc:modeling-137c3394c7a146209fe2cd37', 'ctc:modeling-1b36aff609b370e792d9dee6', 'ctc:modeling-3abaeb7a2cbf8d77276bf23c', 'ctc:modeling-afa39b57a646673e84b55f09', 'ctc:modeling-bb52102ba5f63982e27e329a', 'ctc:modeling-ebf5a771fd114720de11f6e3', 'ctkg:domainconcept:0a8044ae6cb529e98bc20bca']
...
def model_claim_checks(cards, models):
    expected = [
        ...
        ['0.632121', '0.5', '2', '1-e^{-'],
        ['0.5', '0.25', '-4&-2', '0&1'],
        ['s^2+3s+1', '1/3', '1/2', '0.4'],
        ['3s+4', '2-e^{-t}-e^{-2t}', 'y_\\infty=2'],
        ['1-2e^{-t}+e^{-2t}', '2x-2y', '终值为 1'],
        ['0.126424', '1.990074', '-5.710593', '0.1s+1'],
    ]
    true('eight independently calculated models', len(models) == 8)
    for row, snippets in zip(cards, expected):
...
def verify_parser_and_katex(cards: list[dict[str, object]]) -> dict[str, object]:
    with tempfile.TemporaryDirectory(prefix="teaching-core-02a-parser-") as temp:
        env = os.environ.copy()
        env.update({
...
def main() -> None:
    ...
    true("exact eight scope cards", len(scope["cards"]) == 8)
    ...
    true("source backup count", len(list(BATCH.glob("*-source.json"))) == 8)
