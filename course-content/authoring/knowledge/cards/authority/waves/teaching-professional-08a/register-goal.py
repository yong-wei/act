"""Bind the reviewed discrete control topics to existing semantically suitable goals."""
import hashlib,json
...
orders={'discrete-control-foundations':list(range(1,8))}
by_order={c['order']:c for c in inventory['cards']};assert set(by_order)==set(range(1,8))
assert sorted(n for values in orders.values() for n in values)==list(range(1,8))
...
print('Registered 7 reviewed targets in the existing discrete-control goal')
