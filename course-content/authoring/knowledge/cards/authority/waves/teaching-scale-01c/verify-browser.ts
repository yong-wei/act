...
async function main() {
  ...
  try {
    ...
    await page.getByRole('button', { name: '时域分析', exact: true }).press('Enter');
    await page.getByText('正在加载知识图谱…', { exact: true }).waitFor({ state: 'hidden', timeout: 60000 });
    await page.getByRole('button', { name: '终值定理' }).first().waitFor({ timeout: 60000 });
    await page.waitForLoadState('networkidle');
    await page.getByRole('button', { name: '静态误差系数法求稳态误差', exact: true }).press('Enter');
    ...
    await page.waitForLoadState('networkidle');
    await page.locator('button[data-active-resource-title="静态误差系数法求稳态误差"]').click();
    await page.waitForLoadState('networkidle');
    ...
    writeFileSync(join(batch, 'browser-verification.json'), JSON.stringify({status: 'passed', method: 'authenticated Playwright Chromium, graph domain -> precise static error coefficient method node -> resource viewer', keyboardGraphNavigation: true, displayedSelfCheck: true, readingCompletions: 0, representativeCard: '静态误差系数法求稳态误差'}, null, 2) + '\n');
    console.log('PASS graph -> static error coefficient method card viewer -> authored self-check');
