...
orders={'control-correction':[1,3,4,5,7,8],'feedback-structure-foundations':[2,6,9]}
by_order={c['order']:c for c in inventory['cards']};assert set(by_order)==set(range(1,10))
assert sorted(n for values in orders.values() for n in values)==list(range(1,10))
...
print('Registered 9 reviewed targets across existing design and feedback-structure goals')
