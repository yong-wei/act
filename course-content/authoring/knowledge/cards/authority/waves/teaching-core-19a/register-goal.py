...
orders={'control-correction':[1,2,3,5,6,7,10],'steady-state-control-foundations':[4,8,11],
        'feedback-loop-concept-foundations':[9],'state-space-controllability-foundations':[12]}
by_order={c['order']:c for c in inventory['cards']};assert set(by_order)==set(range(1,13))
assert sorted(n for values in orders.values() for n in values)==list(range(1,13))
...
print('Registered 12 reviewed targets across controller design, PI precision, feedback, and state-space goals')
