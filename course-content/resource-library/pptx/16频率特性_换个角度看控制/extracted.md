...
这一段课件中混入了视频、GIF 和分步揭示页面，本次不逐页复刻，而保留其稳定结论。
...
课件第 17 页原始对象层保留了一段 Matlab 代码，作用是通过时域仿真验证正弦稳态响应。原代码骨架为：
...
其教学定位不是“学 Matlab 语法”，而是“用仿真回看频率响应的时域结果”。

## 5. 频率特性的图解表示与 Bode 图（第 19-23 页）
...
- 横轴：频率取对数刻度，以 decade（十倍频程）组织；
- 纵轴：幅值用 dB 表示，相角仍以角度表示；
...
![Bode图的坐标定义](tikz/rendered/04-bode-axes.png)

## 6. 典型环节的 Bode 图（第 24-28 页）
...
- 幅频渐近线斜率为 $-20\,\mathrm{dB/dec}$；
- 相角恒为 $-90^\circ$。

更一般地，若为 $\frac{1}{s^\alpha}$，则斜率为 $-20\alpha\,\mathrm{dB/dec}$，相角恒为 $-90\alpha^\circ$。
...
- 低频段幅值近似为常数；
- 当频率越过固有频率附近后，幅值逐渐转为 $-40\,\mathrm{dB/dec}$；
- 当阻尼比较小时，会出现明显谐振峰；
...
![典型环节Bode图摘要卡](tikz/rendered/05-typical-elements-card.png)
...
1. 频率响应来自正弦稳态响应。
2. Bode 图是最适合手工叠加分析的表示方式。
3. 典型环节的 Bode 图是后续组合对象分析的基础。
...
- Rake (1979). Step response and frequency response methods.
- Wang et al. (2020). Fast and accurate frequency response estimation for large power system disturbances.
- B 站资源：`DR_CAN 频率响应详细数学推导 G(jw)`
- 图书：《控制之美》
- 在线视频：088、089
