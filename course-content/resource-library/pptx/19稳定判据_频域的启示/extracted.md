...
| 6 | 稳定判据比较与频域判据的价值 |
| 7-9 | 幅角原理与零点、极点的几何映射 |
| 10-14 | `F(s)=1+G(s)H(s)`、奈氏路径与完整 Nyquist 曲线 |
| 15-18 | 典型对象的 Nyquist 图与正式稳定判据 |
| 19-25 | 半闭合 Nyquist 曲线、穿越计数与简化判据 |
| 26-29 | 简化判据注意事项与应用题 |
| 30-32 | 对数稳定判据 |
| 33-34 | 总结与课后思考 |
...
- 对数稳定判据：把频域判据继续翻译到 Bode 图上。

课件在这里给出的关键判断不是“谁更高级”，而是“谁更适合设计”：

- Routh 判据适合回答“稳不稳”；
- Nyquist 与对数判据适合回答“参数怎么调、结构怎么改、稳定性会怎样变化”。
...
## 6. 奈氏路径与完整 Nyquist 曲线如何构造（第 12-14 页）
...
![奈氏路径与完整 Nyquist 曲线构造示意](tikz/rendered/02-nyquist-path-overview.png)

## 7. 正式 Nyquist 判据（第 18 页）

第 18 页把上面的构造收拢成正式结论。

若右半平面开环极点数为 `P`，右半平面闭环极点数为 `Z`，完整 Nyquist 曲线对点 `(-1,j0)` 的逆时针包围周数为 `R`，则：
...
> Nyquist 曲线不通过 `(-1,j0)`，并且逆时针包围 `(-1,j0)` 的周数等于右半平面开环极点数 `P`，则闭环稳定。
...
课件用几类典型对象反复演示 Nyquist 图的起点、终点和原点附近行为：
...
![典型对象在 Nyquist 图中的起点、终点与原点附近行为](tikz/rendered/03-typical-type-summary.png)

## 9. 半闭合 Nyquist 曲线：把包围计数改成穿越计数（第 22-27 页）

第 22-27 页把完整 Nyquist 曲线进一步简化为“只看一半曲线”的实用版本。
...
![半闭合 Nyquist 曲线的穿越计数与稳定性关系](tikz/rendered/04-half-nyquist-crossing.png)
...
![简化 Nyquist 判据的结论与注意事项](tikz/rendered/05-simplified-criterion-card.png)

## 11. 应用题告诉我们什么（第 28-29 页）

第 28-29 页的应用题都在训练同一件事：

- 随着参数 `K` 变化，Nyquist 曲线整体缩放；
- 曲线与 `(-1,j0)` 的相对位置发生改变；
- 因此稳定区间对应某个“临界增益”。
...
这说明 Nyquist 判据不仅能“判稳”，还天然能用来找稳定边界和临界参数。

## 12. 对数稳定判据：把穿越规则翻译到 Bode 图（第 30-32 页）

第 30-32 页把 Nyquist 曲线上的穿越规则转换为 Bode 图上的联合判断。

课件的核心意思是：

- 当 `|G(j\omega)H(j\omega)|>1` 时，Nyquist 曲线位于单位圆外，更有可能穿越 `(-\infty,-1)`；
- 在 Bode 图上，这对应 `L(\omega)=20\log_{10}|G(j\omega)H(j\omega)|>0`；
...
![Nyquist 穿越计数到 Bode 图表达的对应关系](tikz/rendered/06-bode-criterion-card.png)
...
3. 闭环稳定时，完整 Nyquist 曲线应逆时针包围 `(-1,j0)` 共 `P` 周；
4. 对数判据只是把同一个穿越问题换到 Bode 图上表达。

第 34 页留下的思考题是：

> 试证右侧给出的 Nyquist 图总是不稳定。
...
4. 最后把穿越计数继续翻译成 Bode 图上的幅值-相位联合判据。
