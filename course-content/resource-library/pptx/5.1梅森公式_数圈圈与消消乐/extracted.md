...
1. 在 Simulink 中建立结构图或信号流图模型。
2. 用 `linmod` 得到线性状态空间模型。
3. 用 `ss2tf` 把状态空间模型转成传递函数。
...
- 第 18 页：基于图论的方法与参考论文
  ...
  - V. C. Prasad，2010
- 第 19 页：浏览器求解器
  - Chrome Web Store 的 `Signal Flow Graph` 扩展
- 第 20 页：第三方 MATLAB 函数
  - MATLAB Central File Exchange 上的自动流图分析工具
- 第 21 页：Github 项目
