...
- 延迟时间 `Delay Time`，记为 $t_d$
- 上升时间 `Rise Time`，记为 $t_r$
- 峰值时间 `Peak Time`，记为 $t_p$
...
- 调节时间 `Settling Time`，记为 $t_s$
- 超调量 `Overshoot`，记为 $\sigma\%$
- 稳态误差 `Steady-state Error`，记为 $e_{ss}$
...
第 17 页明确引入了 MATLAB / Octave 的 `stepinfo`。对象层保留了完整示例，说明课件希望学生把“图上读指标”与“软件直接算指标”建立对应。
