...
\begin{tikzpicture}[
  ...
  \node[stage] (s1) at (0,0) {基于模型的频域设计\\超前 / 滞后 / 超前滞后校正};
  \node[stage, right=1.4cm of s1] (s2) {工程控制器形式\\将比例积分微分控制器看作实用的超前滞后补偿器};
  \node[stage, right=1.4cm of s2] (s3) {实验驱动整定\\阶跃响应法 / 频率响应法};
  \draw[flow] (s1) -- (s2);
  \draw[flow] (s2) -- (s3);
\end{tikzpicture}
\end{document}
