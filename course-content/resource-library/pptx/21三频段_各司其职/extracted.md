...
- 中频段希望以 `-20\,\mathrm{dB/dec}` 斜率穿越 `0\,\mathrm{dB}`，并保持较宽频带，以兼顾较大的相角裕度；
...
3. 不能仅凭“是否以 `-20\,\mathrm{dB/dec}` 过 `0\,\mathrm{dB}`”机械判稳；
...
- 对高阶系统，仍然希望中频段以 `-20\,\mathrm{dB/dec}` 穿越 `0\,\mathrm{dB}`；
- 保持足够的相角裕度，可以控制超调量；
...
- 第 32 页回顾了实验测试、闭环频率特征量、Nyquist 判据、对数判据与稳定裕度，说明这些内容并不是割裂的章节；
