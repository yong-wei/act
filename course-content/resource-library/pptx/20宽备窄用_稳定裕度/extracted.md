...
![稳定裕度在极坐标图与 Bode 图中的对应定义](tikz/rendered/01-margin-definition.png)
...
## 7. Bode 图近似：为什么手算会有误差（第 19 页）

第 19 页把同一个例题改用 Bode 图近似读取。
...
![Bode 图近似读取稳定裕度及其误差来源](tikz/rendered/05-bode-estimate-card.png)

## 8. 计算机辅助验证（第 20 页）

第 20 页把全部结论压缩成一行 MATLAB 命令：
...
1. 先在 Nyquist 图或 Bode 图上找到两类关键穿越频率；
