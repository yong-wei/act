...
| 320px mobile | `fail-closed-generation-mobile-320.png` | `AB7CF54B8305C5B75EBCCF0FA96094785BF7A615790A16C3C8DBCAC8FA0F2447` |

## Browser Observation

The authenticated local student page rendered at both target widths. It remained fail-closed with the generation action disabled because the local historical student data could not produce a governed learner-safe summary. The server reported `Portrait v2 derivation must use a governed learner-safe summary.` No database state was changed and no successful browser generation request was fabricated.
