...
- 仅清理主报告中 finding 132/133/328 的关闭映射。
- 不重新审查 `audit-remediation-p0-stability` 的实现证据。
- 不关闭 finding 134、135 或 136；这些仍分别属于入口动作、近场连接或恢复体验完整性的后续整改范围。
...
| 328 | `report.md` 仍列为 P0 真实班级课前包路由返回 500；`chapters/57-function-state-flows-batch49.md` 已有 2026-06-21 修复说明 | `audit-remediation-p0-stability` 任务 3.2 明确覆盖 class-scoped 课前包恢复态；evidence 记录 Playwright 覆盖 root、cluster 和 class-scoped 路由 | 在主报告条目下标记 mapping-cleaned / closed by archived evidence |
...
- 章节内联状态：`artifacts/product-design-audits/full-system-page-function-audit-2026-06-20/chapters/57-function-state-flows-batch49.md`
