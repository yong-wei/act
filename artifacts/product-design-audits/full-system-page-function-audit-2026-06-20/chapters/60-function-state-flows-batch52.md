...
2026-06-22 / #618：`GET /api/teacher/preset-lessons/clone` 返回带 `recoveryHref` 与 POST 方法要求的产品化 405 JSON，并新增 API 回归测试。
...
2026-06-22 / #618：共享 `LessonPlanList` 新增搜索输入、结果数量、清除条件和无匹配恢复态，教师与管理员教案页共同受益。
...
2026-06-22 / #618：新建页将 `returnTo` 与 `templateId` 传入编排器，编排器显示模板来源提示并保留作者态返回路径。
...
2026-06-22 / #618：资源管理页新增搜索结果数量、清除按钮、无匹配恢复提示；资源编辑弹窗新增使用影响说明。
...
2026-06-22 / #618：ResourceNode 管理支持 URL 查询初始化、`status=blocked` 阻断筛选说明和 25 条分页加载，降低长列表一次性暴露。
...
2026-06-22 / #618：管理员教案页复用可搜索 `LessonPlanList`；管理员新建与编辑页保留返回上下文，坏对象进入恢复页。
...
2026-06-22 / #618：治理页识别 authoring surface 并默认进入报告视图，status API 回传 `authoringContext`，页面显示质量报告提示和返回教案管理动作。
...
2026-06-22 / #618：互动课程目录读取 `q`，提供搜索输入、结果数量、无匹配空态和清除链接。
...
2026-06-22 / #618：课程流构建器默认分页知识节点、显示已选数量、防重复添加，保存成功进入保留播放 intent 的课程流启动页；API 写入知识节点环节。
