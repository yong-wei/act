...
2026-06-22 / #618：教师编辑页保留返回上下文；缺失对象进入 `LessonPlanMissingRecovery`，提供返回列表和新建教案动作。
...
2026-06-22 / #618：教师新建页识别 `templateId` 并在编排器显示模板来源提示，空标题错误也保留模板上下文和作者态返回路径。
...
2026-06-22 / #618：`status=blocked` 初始化为阻断/排除筛选，显示阻断上下文并分页 ResourceNode 列表，保留单节点详情治理入口。
...
2026-06-22 / #618：管理员编辑页保留返回上下文，管理员教案列表新增搜索和结果状态，坏对象进入恢复页而非默认 404。
...
2026-06-22 / #618：管理员坏教案编辑返回产品化恢复页，展示请求 ID、缺失说明、返回教案管理和新建教案动作。
...
2026-06-22 / #618：治理页与 status API 显式识别 authoring deep link，显示目标教案 ID、质量报告视图、返回教案管理和恢复动作。
...
2026-06-22 / #618：播放列表 play 路由展示课程流启动页，保留 playlistId、标题、环节数和 `intent=start-class`，通过 `/api/session` 开始课堂。
...
2026-06-22 / #618：知识图谱识别 `nodeId` deep link，检查器提供加入课程流、创建学习任务、查看证据和返回路径动作；教师知识节点管理也提供加入课程流入口。
