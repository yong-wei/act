/** Direct v0.18 → v0.22 migration impact evidence.
 *
 * This module deliberately computes from complete snapshots.  The upstream
 * v0.21 → v0.22 release-diff is parsed and retained only as a cross-check.
 */
...
function fail(message: string): never {
  throw new Error(`ActKG v0.22 impact report rejected: ${message}`);
}
 ...
 */
export function buildV022DeltaProjections(bundle: ValidatedActKGBundleV2): DeltaProjectionRecord[] {
...
function snapshotFromV2(bundle: ValidatedActKGBundleV2, releaseSetId: string): DeltaSemanticSnapshot {
  const runtimeNodes = array(bundle.selectedRuntimeProjection.payload.nodes, 'v0.22 runtime nodes');
  const runtimeLinks = array(bundle.selectedRuntimeProjection.payload.links, 'v0.22 runtime links');
  const entries = array(bundle.release.entries, 'v0.22 release entries');
...
export async function computeV022ImpactReport(input: {
  repoRoot?: string;
  ...
  const v018Root = path.resolve(repoRoot, V018_BASELINE_DEFAULT_PATH);
  if (input.v018Root !== undefined && path.resolve(input.v018Root) !== v018Root) {
    fail('explicit --v018-root must resolve to the tracked v0.18 release directory');
  }
  ...
  const captureRevision = resolveTrustedCaptureRevision({
    ...
    trackedPaths: [V018_BASELINE_DEFAULT_PATH],
  ...
  for (const relativePath of V018_BASELINE_FILES) {
    captured.set(relativePath, await readCapturedFile(repoRoot, input.captureRevision, `${V018_BASELINE_DEFAULT_PATH}/${relativePath}`));
  }
  ...
  const body = {
    protocol: V022_IMPACT_REPORT_PROTOCOL,
...
export function serializeV022ImpactReport(report: V022ImpactReport): string {
