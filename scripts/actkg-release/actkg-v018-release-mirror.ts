/**
 * Immutable ActKG v0.18 publication mirror.
 *
...
export const V018_PUBLICATION_PATH =
  'releases/control-theory-engineering-v0.18' as const;
export const V018_ACT_CONTROLLED_PATH =
...
const RECEIPT_CONTRACT = 'actkg-v018-release-mirror/1' as const;
...
export interface ActkgV018MirrorReceipt {
  ...
  publicationPath: typeof V018_PUBLICATION_PATH;
...
export interface MirrorPinnedV018ReleaseOptions {
...
export interface MirrorPinnedV018ReleaseResult {
  outputPath: string;
  receiptPath: string;
  receipt: ActkgV018MirrorReceipt;
  reused: boolean;
}

function fail(message: string): never {
  throw new Error(`ActKG v0.18 mirror rejected: ${message}`);
}
...
function parseTree(root: string, commit: string): MirroredGitTreeEntry[] {
  const raw = execFileSync(
    'git',
    ['ls-tree', '-r', '-l', '-z', commit, '--', V018_PUBLICATION_PATH],
    {
      cwd: root,
      encoding: 'utf8',
      stdio: ['ignore', 'pipe', 'pipe'],
  ...
  for (const record of raw.split('\0').filter(Boolean)) {
    ...
    const relativePath = absolutePath.slice(`${V018_PUBLICATION_PATH}/`.length);
...
function expectedReceiptBytes(receipt: ActkgV018MirrorReceipt): Buffer {
...
async function assertExistingMirror(
  ...
  const stored = JSON.parse(await readFile(receiptPath, 'utf8')) as ActkgV018MirrorReceipt;
...
export async function mirrorPinnedV018Release(
  options: MirrorPinnedV018ReleaseOptions,
): Promise<MirrorPinnedV018ReleaseResult> {
  ...
  const controlledPath = options.controlledPath ?? V018_ACT_CONTROLLED_PATH;
  ...
  if (publicationCommit !== registry.publicationCommit || !COMMIT.test(publicationCommit)) {
    fail('publication commit is not the registered v0.18 commit');
  }
  ...
  const treeDigest = createHash('sha256')
    ...
    .digest('hex');
  const receipt: ActkgV018MirrorReceipt = {
    ...
    publicationPath: V018_PUBLICATION_PATH,
    controlledPath,
...
export const V018_MIRROR_IDENTITIES = Object.freeze({
...
async function main(): Promise<void> {
  ...
  const result = await mirrorPinnedV018Release({
