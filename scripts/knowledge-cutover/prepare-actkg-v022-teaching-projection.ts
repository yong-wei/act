 ...
 * against the admitted v0.22 Authority snapshot (#1443).
 *
...
import { loadAndValidateRegisteredPublicBundleV2 } from '../actkg-release/public-bundle-v2';
import { REVIEWED_V0_22_V2_REGISTRY } from '../actkg-release/bundle-compatibility-registry-v022';
...
const DEFAULT_OUTPUT = 'course-content/authoring/knowledge/teaching-projection/candidates/control-theory-engineering-v0.22';
const CAPTURE_MANIFEST = 'course-content/authoring/knowledge/teaching-projection/capture-bound-input-manifest-v022.json';
export const V022_TEACHING_PROJECTION_POINTER_PATHS = V022_CURRENT_POINTER_PATHS;
...
function targetAuthorityNodes(engineering: AuthorityEngineeringBody): V022AuthorityNodeRecord[] {
...
function sourceAuthorityNodes(engineering: AuthorityEngineeringBody, referencedIds: ReadonlySet<string>): V022AuthorityNodeRecord[] {
...
function readEngineering(repoRoot: string, manifest: JsonObject): AuthorityEngineeringBody {
  ...
  const pathForEngineering = path.join(
    repoRoot,
    'course-content/authoring/knowledge/authority/candidates/control-theory-engineering-v0.22/replay-1/authority/releases',
    snapshotId,
    'engineering.json',
  );
...
function sealedAuthorityRelativePath(repoRoot: string, value: string): string {
  if (!value || path.isAbsolute(value) || value.includes('\\') || /(^|\/)\.\.(\/|$)/u.test(value)) {
    fail(`v0.22 Authority sealed path is unsafe: ${value || '<missing>'}`);
  }
  const normalized = value.split('/').filter((part) => part && part !== '.').join('/');
  const resolved = path.resolve(repoRoot, normalized);
  const root = path.resolve(repoRoot);
  if (resolved === root || !resolved.startsWith(`${root}${path.sep}`)) {
    fail(`v0.22 Authority sealed path escapes repository: ${value}`);
  }
  return path.relative(root, resolved).split(path.sep).join('/');
}

export function assertV022AdmittedAuthorityCandidate(input: {
  repoRoot: string;
  receipt: JsonObject;
  receiptPath: string;
}): void {
  const outputs = Array.isArray(input.receipt.outputs) ? input.receipt.outputs : [];
  if (outputs.length === 0) fail('v0.22 Authority candidate receipt is missing sealed outputs');
  ...
  if (replays.length !== 2) fail('v0.22 Authority candidate requires exactly two replay receipts');
  const replayNames = replays.map((raw) => String(asRecord(raw).name ?? '')).sort();
  if (replayNames[0] !== 'replay-1' || replayNames[1] !== 'replay-2') {
    fail('v0.22 Authority candidate requires distinct replay-1 and replay-2 admissions');
  }
  const replayRoots = new Set<string>();
  for (const raw of replays) {
    const replay = asRecord(raw);
    const name = String(replay.name ?? '');
    const expectedPrefix = `course-content/authoring/knowledge/authority/candidates/control-theory-engineering-v0.22/${name}/`;
    for (const key of ['manifestPath', 'engineeringPath', 'stageReceiptPath'] as const) {
      ...
      if (!replayPath.startsWith(expectedPrefix)) {
        fail(`v0.22 Authority ${name} path is not under its own sealed replay root: ${replayPath}`);
      }
      required.add(replayPath);
    }
    replayRoots.add(name);
  }
  if (replayRoots.size !== 2) fail('v0.22 Authority candidate replay roots are not independent');
  for (const requiredPath of [...required].sort()) {
    if (!outputPaths.has(requiredPath)) {
      fail(`v0.22 Authority sealed output list is missing ${requiredPath}`);
    }
  }
  for (const raw of outputs) {
    ...
    if (!relativePath || !/^[a-f0-9]{64}$/u.test(expected)) {
      fail(`v0.22 Authority sealed output is malformed: ${relativePath || '<missing>'}`);
    }
    const abs = path.join(input.repoRoot, relativePath);
    if (!existsSync(abs)) fail(`v0.22 Authority sealed output is missing: ${relativePath}`);
    ...
    const actual = scope === 'receipt-body-without-outputs'
      ...
      : sha256(bytes);
    if (actual !== expected) fail(`v0.22 Authority sealed output drifted: ${relativePath}`);
  }
}
...
function loadCandidateAuthority(repoRoot: string): {
  ...
  const root = path.join(repoRoot, 'course-content/authoring/knowledge/authority/candidates/control-theory-engineering-v0.22');
  ...
  if (receipt.status !== 'staged' || receipt.nonActivation !== true || receipt.mode !== 'local-disposable-non-activation') {
    fail('v0.22 Authority candidate receipt is not staged/nonActivation');
  }
  assertV022AdmittedAuthorityCandidate({ repoRoot, receipt, receiptPath });
  ...
  for (const replay of [first, second]) {
    ...
    if (imported.some((value) => value.candidateState !== 'ACCEPTED_CANDIDATE')) fail('v0.22 replay is not ACCEPTED_CANDIDATE');
    if (String(replay.snapshotId) !== String(first.snapshotId) || String(replay.snapshotHash) !== String(first.snapshotHash)) fail('v0.22 replay snapshot identity drift');
    if (String(replay.manifestPath).includes('replay-')) {
      const manifest = readJson(path.join(repoRoot, String(replay.manifestPath)));
      if (manifest.lifecycle !== 'staged') fail('v0.22 Authority candidate manifest is not staged');
    }
  }
  ...
  if (authorityDigest(engineering) !== String(manifest.engineeringDigest ?? '')) {
    fail('v0.22 Authority engineeringDigest does not match engineering body');
  }
  if (String(manifest.snapshotId) !== String(first.snapshotId) || String(manifest.snapshotHash) !== String(first.snapshotHash)) {
    fail('v0.22 Authority manifest snapshot identity drifted from sealed replay');
  }
  if (String(manifest.snapshotId) !== `snap-${String(manifest.snapshotHash)}`) {
    fail('v0.22 Authority snapshotId does not match snapshotHash');
  }
  ...
  if (!existsSync(stageReceiptPath)) fail('v0.22 Authority stage receipt is missing');
  return {
    receipt,
    manifest,
    engineering,
...
function loadOrBuildCaptureManifest(repoRoot: string, inventory: ReturnType<typeof buildActiveCourseInventory>, captureRevision: string): V022CaptureManifest {
  const manifestPath = path.join(repoRoot, CAPTURE_MANIFEST);
  if (existsSync(manifestPath)) {
    const manifest = readJson(manifestPath) as unknown as V022CaptureManifest;
    ...
    if (manifest.policy === 'file-membership-execution-bound/v2'
      ...
      assertV022CaptureBound(repoRoot, manifest);
      return manifest;
    }
  }
  ...
  const manifest = buildV022CaptureManifest({
    repoRoot,
    captureRevision,
...
function makeAuthorityBinding(candidate: ReturnType<typeof loadCandidateAuthority>): V022AuthorityBinding {
...
async function generateDatabaseObservation(input: {
...
}): Promise<V022DatabaseObservation> {
  const sourceUrl = process.env.DATABASE_URL?.trim();
  if (!sourceUrl) {
    throw new V022CaptureError('database-observation-source-rejected', 'DATABASE_URL is not configured');
  }
  const safety = assertV022DisposableDatabaseUrl(sourceUrl);
  const loadedBundle = await loadAndValidateRegisteredPublicBundleV2({
    root: input.repoRoot,
    bundlePath: 'course-content/authoring/knowledge/releases/control-theory-engineering-v0.22-r5',
  ...
  try {
    ...
    return buildV022DatabaseObservation({
      ...
      schemaIdentity: expectedV022AdmissionSchemaIdentity({
...
function blockedDatabaseObservation(error: unknown): JsonObject {
  if (error instanceof V022CaptureError) {
    return {
      status: 'BLOCKED',
...
export async function prepareActKgV022TeachingProjection(
  ...
  if (authority.releaseId !== 'ctr:release:control-theory-engineering-v0.22' || authority.snapshotId !== 'snap-9c4b2c1c2c976b4903bb979889d8b74a8dd306bc718cd4c9d06ac97b14172151') {
    fail('v0.22 rebase requires the admitted snap-9c4b2c1c candidate Authority root');
  }
  ...
  const pointersBefore = readV022PointerSnapshots(repoRoot, V022_TEACHING_PROJECTION_POINTER_PATHS);
  ...
  if (inventory.packageCount <= 0 || inventory.resourceCount <= 0) fail(`active inventory is empty: ${inventory.packageCount}/${inventory.resourceCount}`);
  const captureManifest = loadOrBuildCaptureManifest(repoRoot, inventory, authority.captureRevision);
  const references = buildV022ReferenceDenominator({
    inventory,
    priorArtifacts: priorProjection,
    captureRevision: authority.captureRevision,
    repoRoot,
  });
  const capture = buildV022CaptureReceipt({ manifest: captureManifest, inventory, references, excludedHistoricalCount: 4880 });
  ...
  const upstreamImpactEvidence = {
    ...
    path: String(upstreamImpact.path ?? 'course-content/authoring/knowledge/authority/candidates/control-theory-engineering-v0.22/impact-report.json'),
    digest: String(upstreamImpact.digest ?? ''),
  };
  const first = buildV022IdentityRebase({ authority, sourceNodes, targetNodes, priorArtifacts: priorProjection, prerequisiteInput: priorPrerequisiteInput, references, upstreamImpactEvidence });
  const second = buildV022IdentityRebase({ authority, sourceNodes, targetNodes, priorArtifacts: priorProjection, prerequisiteInput: priorPrerequisiteInput, references, upstreamImpactEvidence });
  ...
  assertV022CandidateSelectorSafety({ mode: 'local-disposable-non-activation', unqualified: true, nonActivation: true, selectorConsumption: false, outputRoot: relative(repoRoot, outputRoot), currentPointerPaths: V022_TEACHING_PROJECTION_POINTER_PATHS });
  ...
  const pointersAfter = readV022PointerSnapshots(repoRoot, V022_TEACHING_PROJECTION_POINTER_PATHS);
  const dualBuild = buildV022DualBuildIdentity({
  ...
  let observation: V022DatabaseObservation | null = null;
  let blockedObservation: JsonObject | null = null;
  if (observationPath) {
    observation = JSON.parse(readFileSync(path.resolve(repoRoot, observationPath), 'utf8')) as V022DatabaseObservation;
  } else {
    try {
  ...
  const databaseObservation = validateV022DatabaseObservation({
    observation,
    ...
    expectedSchemaIdentity: expectedV022AdmissionSchemaIdentity({
      snapshotId: authority.snapshotId,
      releaseId: authority.releaseId,
  ...
  const referenceKindCounts = Object.fromEntries(V022_REFERENCE_KINDS.map((kind) => [kind, references.filter((reference) => reference.kind === kind).length])) as Record<(typeof V022_REFERENCE_KINDS)[number], number>;
  ...
  writeCanonical(path.join(outputRoot, 'candidate-manifest.json'), {
    contract: 'act-teaching-projection-v022-candidate/v1',
...
if (import.meta.url === pathToFileURL(process.argv[1] ?? '').href) {
  prepareActKgV022TeachingProjection().catch((error: unknown) => {
