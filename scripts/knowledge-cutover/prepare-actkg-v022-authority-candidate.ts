...
import {
  mirrorPinnedV022Release,
  V022_ACT_CONTROLLED_PATH,
  type ActkgV022MirrorReceipt,
} from '../actkg-release/actkg-v022-release-mirror';
import {
  computeV022ImpactReport,
  serializeV022ImpactReport,
  type V022ImpactReport,
} from '../actkg-release/actkg-v022-impact';
import {
...
export const V022_CANDIDATE_RECEIPT_PROTOCOL =
  'actkg-v022-authority-candidate/1' as const;

export const V022_MIRROR_RECEIPT_PATH =
  `${V022_ACT_CONTROLLED_PATH}.mirror-receipt.json` as const;
...
export const V022_CANDIDATE_CAPTURE_PATHS = [
  'package.json',
  'package-lock.json',
  ...PUBLIC_BUNDLE_V2_ADAPTER_CAPTURE_PATHS,
  'prisma.config.ts',
  ...
  V022_ACT_CONTROLLED_PATH,
  V022_MIRROR_RECEIPT_PATH,
] as const;

export const V022_CANDIDATE_MIGRATIONS_PATH = 'prisma/migrations' as const;
 ...
 */
export const V022_CANDIDATE_SOURCE_CONTRACT_VERSION =
  'actkg-v022-authority-candidate/source/1' as const;
export const V022_CANDIDATE_EVIDENCE_CONTRACT =
  'actkg-v022-authority-candidate/evidence-capture/2' as const;

const GIT_FILE_MODES = new Set(['100644', '100755']);

export interface V022CandidateSourceEntry {
  path: string;
  mode: string;
  objectType: string;
  gitObject: string;
...
type CandidateGitEntry = V022CandidateSourceEntry;

export interface V022CandidateOutputSummary {
  path: string;
  mode: '100644' | '100755';
  byteLength: number;
  sha256: string;
...
export interface V022CandidateEvidenceCaptureContract {
  contract: typeof V022_CANDIDATE_EVIDENCE_CONTRACT;
  sourceContractVersion: typeof V022_CANDIDATE_SOURCE_CONTRACT_VERSION;
  ...
  outputs: V022CandidateOutputSummary[];
}
...
export interface V022AuthorityCandidateReceipt {
  protocol: typeof V022_CANDIDATE_RECEIPT_PROTOCOL;
  status: 'staged';
  ...
  sourceContractVersion: typeof V022_CANDIDATE_SOURCE_CONTRACT_VERSION;
  sourceManifestDigest: string;
  sourceEntries: V022CandidateSourceEntry[];
  derivedPaths: string[];
  outputs: V022CandidateOutputSummary[];
  stagedAt: string;
  mirror: ActkgV022MirrorReceipt;
  validated: {
  ...
  impact: {
    path: string;
    digest: string;
    directDenominator: 'complete-v0.18-snapshot';
    upstreamDiffStatus: V022ImpactReport['upstreamCrosscheck']['status'];
  };
  publicSchema: {
...
function fail(message: string): never {
  throw new Error(`ActKG v0.22 Authority candidate rejected: ${message}`);
}
...
async function candidateOutputSummaries(
  ...
  derivedRoots: readonly string[],
  allowEmpty = false,
): Promise<V022CandidateOutputSummary[]> {
 ...
 */
export async function assertV022CandidateEvidenceCaptureContract(options: {
  repoRoot: string;
  captureRevision: string;
  generationRevision?: string;
...
}): Promise<V022CandidateEvidenceCaptureContract> {
  const repoRoot = path.resolve(options.repoRoot);
  const sourcePaths = [...(options.sourcePaths ?? V022_CANDIDATE_CAPTURE_PATHS)]
  ...
  return {
    contract: V022_CANDIDATE_EVIDENCE_CONTRACT,
    sourceContractVersion: V022_CANDIDATE_SOURCE_CONTRACT_VERSION,
    captureRevision,
    generationRevision,
    sourcePaths,
    derivedPaths,
    sourceManifestDigest: sha256(canonicalJson({
      contract: V022_CANDIDATE_SOURCE_CONTRACT_VERSION,
      paths: sourcePaths,
 ...
 * receipt.
 */
export async function verifyV022CandidateEvidence(input: {
  repoRoot: string;
  captureRevision: string;
  generationRevision: string;
  outputRoot: string;
  ...
  const contract = await assertV022CandidateEvidenceCaptureContract({
    repoRoot,
  ...
  const receipt = parsed as Partial<V022AuthorityCandidateReceipt>;
  if (
  ...
  await assertV022MirrorReceiptMatchesCapture({
    gitRoot: repoRoot,
 ...
 * bytes and executable mode. This is intentionally local to the v0.22
...
export async function assertV022MirrorReceiptMatchesCapture(options: {
  gitRoot: string;
  revision: string;
  expected: ActkgV022MirrorReceipt;
  fail: (message: string) => never;
}): Promise<ActkgV022MirrorReceipt> {
  const bytes = await assertGitFileMatchesWorkingTree({
    gitRoot: options.gitRoot,
    revision: options.revision,
    relativePath: V022_MIRROR_RECEIPT_PATH,
    fail: options.fail,
  });
  let parsed: unknown;
  try {
    ...
    options.fail('ACT v0.22 mirror receipt is not valid JSON');
  }
  if (canonicalJson(parsed) !== canonicalJson(options.expected)) {
    options.fail('ACT v0.22 mirror receipt semantic drift');
  }
  return parsed as ActkgV022MirrorReceipt;
}
 ...
 * rejection.
 */
export async function writeV022CandidateReceiptAfterCaptureCheck(options: {
  repoRoot: string;
  captureRevision: string;
  expectedMirrorReceipt: ActkgV022MirrorReceipt;
  outputRoot: string;
  receipt: V022AuthorityCandidateReceipt;
  captureContract?: V022CandidateEvidenceCaptureContract;
  allowUncommittedDerived?: boolean;
}): Promise<V022AuthorityCandidateReceipt> {
  let outputPublished = false;
  try {
    const mirror = await assertV022MirrorReceiptMatchesCapture({
      gitRoot: options.repoRoot,
      revision: options.captureRevision,
      expected: options.expectedMirrorReceipt,
      fail,
    });
    let receipt: V022AuthorityCandidateReceipt = { ...options.receipt, mirror };
    ...
    if (options.captureContract) {
      const verified = await assertV022CandidateEvidenceCaptureContract({
...
export function assertV022CandidateBundleCounts(bundle: ValidatedActKGBundleV2): void {
  if (bundle.statistics.releaseNodes <= 0) fail('v0.22 release membership was not resolved');
  if (bundle.statistics.projectionNodes <= 0) fail('v0.22 runtime Projection membership was not resolved');
  if (bundle.statistics.publishedRelations <= 0) fail('v0.22 runtime relations were not resolved');
  if (bundle.statistics.terminologyAssertions < 0) fail('v0.22 terminology assertions are invalid');
  if (bundle.projectionProfiles.length !== 3) fail('v0.22 must retain all three typed projection profiles');
  if (bundle.rawArtifacts.length < 2) fail('v0.22 raw Artifact evidence is incomplete');
}
...
async function cleanupFailureProbe(repoRoot: string, baseClient: Client): Promise<void> {
  ...
  try {
    ...
    throw new Error('forced-v022-cleanup-probe');
  } catch (error) {
    if (!(error instanceof Error && error.message === 'forced-v022-cleanup-probe')) throw error;
  } finally {
...
export async function prepareV022AuthorityCandidate(input: {
  repoRoot: string;
  ...
  if (path.resolve(repoRoot, V022_ACT_CONTROLLED_PATH) === outputRoot) fail('outputRoot cannot be the controlled release path');
  ...
  const mirror = await mirrorPinnedV022Release({
    upstreamGitRoot: input.upstreamGitRoot,
    repoRoot,
    receiptPath: path.join(repoRoot, V022_MIRROR_RECEIPT_PATH),
  });
  const captureRevision = resolveTrustedCaptureRevision({
    gitRoot: repoRoot,
    trackedPaths: [...V022_CANDIDATE_CAPTURE_PATHS],
  ...
  if (relativePath(repoRoot, mirror.receiptPath) !== V022_MIRROR_RECEIPT_PATH) {
    fail('v0.22 mirror receipt path drifted from the capture contract');
  }
  await assertV022MirrorReceiptMatchesCapture({
    gitRoot: repoRoot,
  ...
  await assertGitDirectoryMatchesWorkingTree({
    gitRoot: repoRoot,
    revision: captureRevision,
    relativeDirectory: V022_CANDIDATE_MIGRATIONS_PATH,
    fail: (reason) => fail(reason),
  });
  ...
  if (
    bundle.schemaIdentity.version !== REVIEWED_V0_22_V2_REGISTRY.schemaVersion
    || bundle.schemaIdentity.rawSha256 !== REVIEWED_V0_22_V2_REGISTRY.schemaRawSha256
  ) {
    fail('v0.22 schema identity is not the admitted v0.18 Schema 0.3.0 hash');
  }
  ...
  await assertV022CandidateEvidenceCaptureContract({
    repoRoot,
    captureRevision,
  ...
  try {
    ...
    const impact1 = await computeV022ImpactReport({
      repoRoot,
      captureGitRoot: repoRoot,
      v018Root: input.v018Root,
      candidate: bundle,
      candidateReleaseSetId: identity.releaseSetId,
      captureRevision,
    });
    const impact2 = await computeV022ImpactReport({
      repoRoot,
      captureGitRoot: repoRoot,
      v018Root: input.v018Root,
      candidate: bundle,
    ...
    const impactBytes1 = Buffer.from(serializeV022ImpactReport(impact1));
    const impactBytes2 = Buffer.from(serializeV022ImpactReport(impact2));
    if (!impactBytes1.equals(impactBytes2)) fail('clean replay impact reports differ');
    const finalMirrorReceipt = await assertV022MirrorReceiptMatchesCapture({
      gitRoot: repoRoot,
      revision: captureRevision,
      expected: mirror.receipt,
      fail,
    ...
    const captureContract = await assertV022CandidateEvidenceCaptureContract({
      repoRoot,
      captureRevision,
      generationRevision: captureRevision,
      derivedPaths: [relativePath(repoRoot, outputRoot)],
    ...
    const receipt: V022AuthorityCandidateReceipt = {
      protocol: V022_CANDIDATE_RECEIPT_PROTOCOL,
      status: 'staged',
      mode: 'local-disposable-non-activation',
      deterministic: true,
      nonActivation: true,
    ...
    const publishedReceipt = await writeV022CandidateReceiptAfterCaptureCheck({
      repoRoot,
      captureRevision,
...
async function main(): Promise<void> {
  ...
  if (argv.includes('--verify-derived')) {
    const contract = await verifyV022CandidateEvidence({
  ...
  const receipt = await prepareV022AuthorityCandidate({
    repoRoot: requiredOption(argv, '--repo-root'),
    upstreamGitRoot: requiredOption(argv, '--upstream-git-root'),
    captureRevision: requiredOption(argv, '--capture-revision'),
    outputRoot: requiredOption(argv, '--output-root'),
    v018Root: optionalOption(argv, '--v09-root'),
