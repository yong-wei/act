...
const entryRoute = fs.readFileSync(
  path.join(root, 'src/app/interactive-learning/courses/unit-1-2-block-diagram-simplification/page.tsx'),
  'utf8',
);
const entryPage = fs.readFileSync(
  path.join(root, 'src/features/interactive/unit-1-2-structure-graph/entry-page.tsx'),
  'utf8',
);
...
assert.equal(
  entryRoute.includes("loadLessonRuntimeEntry('1-2')"),
  true,
  '1-2 入口路由应加载 runtime lesson 数据并传给首页组件',
);

assert.equal(
  entryPage.includes('lessonRuntime'),
  true,
  '1-2 首页组件应接收 runtime lesson bundle',
);

assert.equal(
  entryPage.indexOf('教师入口') < entryPage.indexOf('<LessonEntryRuntimeSections'),
  true,
  '1-2 教师入口、自由浏览、学生入口模块应位于 runtime 导学模块之前',
);

assert.equal(
  entryPage.includes('1-1') && entryPage.includes('1-3'),
  true,
  '1-2 首页摘要应明确承接 1-1 到 1-3 的课程链',
);
...
console.log('test-1-2-entry-runtime-content passed');
