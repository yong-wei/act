...
const aiContextPath = 'src/lib/unit-1-1-ai-contexts.ts';
...
assert.equal(
  ...
  true,
  '1-1 学生页应接入 useStudentLessonSession、useCourseEventTracking 与全局 AI page context',
);
...
assert.equal(
  ...
  true,
  '1-1 教师页应接入 useTeacherLessonSession 与 useCourseEventTracking',
);

assert.equal(
  studentPage.includes('fetch(`/api/session/${sessionId}/state?scope=student-view`)'),
  false,
  '1-1 学生页应通过统一 state hook 读取 student-view，而不是页面内直接 fetch',
);

assert.equal(
  teacherPage.includes('fetch(`/api/session/${sessionId}/state`)'),
  false,
  '1-1 教师页应通过统一 state hook 读取 teacher-view，而不是页面内直接 fetch',
);
...
assert.equal(
  ...
  true,
  '1-1 课程适配层应显式声明 stateKey，并提供 teacher-sync payload builder',
);

assert.equal(
  aiContextFile.includes('getUnit11StepAIContext') &&
...
console.log('test-1-1-session-framework-adoption passed');
