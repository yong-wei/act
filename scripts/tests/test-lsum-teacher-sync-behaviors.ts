...
const shouldPostLSUMTeacherSync = loadFunction(coursePath, 'shouldPostLSUMTeacherSync');
const resolveLSUMTeacherSyncDraft = loadFunction(coursePath, 'resolveLSUMTeacherSyncDraft');
const finalizeLSUMTeacherSession = loadFunction(coursePath, 'finalizeLSUMTeacherSession');

assert.equal(
  shouldPostLSUMTeacherSync({ loadingSession: true, teacherViewHydrated: false }),
  false,
  'session 仍在加载时不应写回 L-sum teacher-sync'
);

assert.equal(
  shouldPostLSUMTeacherSync({ loadingSession: false, teacherViewHydrated: false }),
  false,
  'teacher-view 尚未回填完成时不应写回 L-sum teacher-sync'
);

assert.equal(
  shouldPostLSUMTeacherSync({ loadingSession: false, teacherViewHydrated: true }),
  true,
  '只有 session 已加载且 teacher-view 已回填后，才允许首次写回 L-sum teacher-sync'
);

assert.deepEqual(
  resolveLSUMTeacherSyncDraft({
    localRevealedAnswers: null,
    localReleasedActivities: null,
    teacherSyncState: {
  ...
  '本地未编辑前，应优先沿用服务端已保存的 revealedAnswers/releasedActivities'
);
...
assert.deepEqual(
  finalizeTrace,
  ['finish', 'finalize:step-13'],
  'L-sum finalize 事件必须在 finish 成功后发送'
);
...
assert.equal(
  teacherPage.includes('teacherViewHydrated') &&
    teacherPage.includes('resolveLSUMTeacherSyncDraft') &&
    teacherPage.includes('shouldPostLSUMTeacherSync') &&
    teacherPage.includes('finalizeLSUMTeacherSession'),
  true,
  'L-sum 教师页应显式使用 hydrated gate、teacher-sync draft 合并与 finalize helper'
);
...
assert.equal(
  ...
  true,
  '共享 session/state hooks 应暴露 teacher-view hydrated 语义供 L-sum 复用'
);

console.log('lsum teacher sync behavior test passed');
