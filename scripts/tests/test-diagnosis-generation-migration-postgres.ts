...
async function main() {
  ...
  try {
    ...
    `);
    assert(indexes.rows.length === 4, 'diagnosis generation unique indexes are missing');

    const foreignKeys = await client.query(`
      SELECT conname, pg_get_constraintdef(oid) AS definition FROM pg_constraint
      WHERE conname IN ('DiagnosisGenerationAttempt_jobId_fkey', 'DiagnosisReport_generationJobId_fkey')
    `);
    assert(foreignKeys.rows.length === 2, 'diagnosis generation foreign keys are missing');
