...
async function main() {
  ...
  const [
    ...
    prisma.simulationLog.findMany({ select: { id: true } }),
    prisma.userAnswer.findMany({ select: { id: true } }),
    prisma.aIIntervention.findMany({ select: { id: true } }),
    prisma.promptAssessment.findMany({ select: { id: true } }),
