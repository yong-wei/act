...
- 指标：每臂结构通过率（按口径）、盲审判定率与均分、综合（结构∧盲审，
  ...
  直接支撑判据＝已核验 + 有锚点 + href 可访问 + 快照冻结答案相关性
  证据且分级在白名单内（`answerRelevanceBasis` ∈ 显式引用/查询直接命中；
  纯语义相似 `semantic-score`、缺失或未知值一律按「仅相关」保守拒绝）；判据上限是生产端引用核验 + 直接选中
  证据，主张级蕴含验证超出确定性审计范围（非目标）。逐回答输出失败
  分桶：无标记/标记未分配/引用未核验/引用无锚点或不可访问/仅相关无
  直接支撑证据（含纯语义相似），漂移计数为唯一编号口径（不与出现
  次数口径叠加）；臂间差值输出绝对值、百分点差、配对 bootstrap 95% CI，
