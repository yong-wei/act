 ...
 向 stdout 输出 JSON：{
   "status": "ok" | "error",
   "result": str,       # LaTeX 结果
   "steps": [{ "step", "description", "input", "output" }],
   "error": str | null
 }
 ...
 def _step(seq: int, description: str, inp: str, out: str) -> dict:
     return {
         "step": seq,
     ...
     if operation == "simplify":
         ...
         steps.append(
             _step(
                 ...
                 "写出拉普拉斯变换定义式",
                 "definition",
                 f"F(s) = \mathcal{{L}}\{{f(t)\}} = \int_{{0}}^{{\infty}} f(t) e^{{-s t}} dt",
                 f"F(s) = \int_{{0}}^{{\infty}} {latex(expr)} e^{{-s t}} dt",
             )
         )
