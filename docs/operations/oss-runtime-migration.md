...
- ECS RAM Role metadata endpoint 当前返回 404；`ossfs2`、`ossfs`、`ossutil` 都不存在。此状态下禁止 OSS 发布、挂载、切换和删除。
...
发布器在受控、已绑定发布身份的计算环境运行；source runtime 必须来自主工作树的实际目录，而不是 Git tracked 文件集合。
...
npx tsx scripts/runtime-release/act-runtime-release.ts publish \
  --runtime-root <main-worktree-runtime> \
  --source-revision <40-char-git-sha> \
  --release-id <release-id-from-plan> \
  --bucket act-course-assets \
  --region oss-cn-hangzhou \
  --role-name <publisher-role> \
...
npx tsx scripts/runtime-release/act-runtime-release.ts verify \
  ...
  --role-name <publisher-role> --output <verification-receipt.json>
...
`plan` 从 source revision 和 tree SHA-256 导出 content-addressed release id；不同 source identity 不会共享前缀。`publish` 先上传业务对象，manifest 最后写入；随后重新读取并逐项校验对象集合、size 与 SHA-256，并要求 receipt 与发布方提交的 manifest/tree identity 一致。任一中断、额外对象或不匹配都会失败，且不得生成 selection。Release prefix 从不覆盖、从不原地修复。
...
未解决风险：RAM Role 未绑定、ossfs 候选挂载未验证、真实 OSS revalidation 未执行、ossfs retrieval 性能未知、19 项媒体输入 unresolved。它们不允许生产切换，但不会改变本地 Release 工具和 legacy filesystem fallback 的可用性。
