...
`runtime:publish` 不激活。本机 publish 只把 Blob 与 manifest 写入 OSS；生产 ossfs 只挂 `runtime/blobs/sha256/` 到 `data/runtime/ossfs/blobs`。激活在 ECS 上用系统 `python3`（3.6）读取该 blob 根，并用事先 GetObject 下来的单份 v2 manifest（`--manifest` 或 `cas-store/runtime/blob-releases/<id>/manifest.json`）。读指针以 `current`/`previous` 符号链接为准，`pointers.json` 只是投影；若宿主机还没有该投影，激活必须从现有已验证的 `current` 符号链接（或与之一致的 `act-runtime-selection.json`）读出 outgoing release，不得把仍在服务的 current 当成空。旧 v2 manifest 的可选文件 `source` 必须参与 `manifestSha256` 校验。`blob-views` 且提供 `--blob-root` 时，激活先把 ossfs blob 根只读 bind 到候选 `.act-runtime-blobs`（可用 `--bind-helper` 覆盖），再把逻辑文件链到 helper 叶，不得在 bind 前对 FUSE blob 做整树复制。激活只比较 current manifest 的 Δ 路径、确认这些 Blob 可见、检查固定 sentinel，对候选视图跑 runtime smoke，然后在 `.act-runtime-selection.lock` 下先改符号链接再写 `pointers.json`，并在同一事务写出 `data/runtime/act-runtime-active-receipt.json`（可用 `--active-receipt` / `ACT_RUNTIME_ACTIVE_RECEIPT_PATH` 覆盖）。生产 `--state-dir` 必须是 `data/runtime/blob-views`。物化视图必须写出 `.act-runtime-release.v2.json`、`.act-runtime-release-materialization.v1.json` 和真实目录 `.act-runtime-blobs`。`blob-views` 上激活若未提供 `--smoke` / `ACT_RUNTIME_SMOKE` 会失败关闭。`--reload-consumers` 失败时必须把指针和 active receipt 恢复到切换前。`runtime:doctor` 与 `runtime:gc` 不得由 publish、activate 或 `deploy:app` 隐式调用。`runtime:gc --execute` 必须发现 `CourseBundleRevision.runtimeReleaseId`（`DATABASE_URL` + `psql`）并与显式 `--session-release` 取并集；发现不到课堂引用时失败关闭，夹具才允许 `--no-session-refs`。对象键保持 `runtime/blobs/sha256/<sha256>` 与 `runtime/blob-releases/<releaseId>/manifest.json`。
