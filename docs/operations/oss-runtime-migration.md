...
合作者开发接入使用独立 RAM 用户 `act-runtime-dev-read` 和杭州**公网** Endpoint，只挂载 readiness 投影的生产 active v2 Release。同一 Linux 运行层上的多个 worktree 共享一份只读 Blob 挂载与 ossfs2 数据缓存，Release pin 仍按 checkout 独立。它不是 Publisher，也不是 ECS `act-runtime-oss-read`。共享凭据无法做个人审计，项目结束必须删钥匙。接入说明见 [developer-oss-runtime-access.md](developer-oss-runtime-access.md)，共享缓存见 [developer-oss-shared-cache.md](developer-oss-shared-cache.md)，RAM 创建/撤销见 [developer-oss-runtime-ram-setup.md](developer-oss-runtime-ram-setup.md)。开发启动走 `npm run startup:oss-runtime`，不要把密钥写入 `.env`。
