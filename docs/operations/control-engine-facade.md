...
消费者约束由实际调用与 façade 导入约束、数值及路由行为测试保证。官方 Arena 评分仍由 evaluator 拥有；缓存最小键仍是 `taskId + artifactHash + protocolVersion`。命中后还须核对写入 metadata 的 runtime/model/spec 绑定，冲突视为 miss，不得把旧分数当成新 runtime 的官方结果。
