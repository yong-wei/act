...
`--input-dir` 可放 `service-role.json`、`object.json`、`dns.json`、`transport.json`、`isolation.json`、`cost.json`。缺项不会静默当成功。默认写入 `docs/operations/static-esa-delivery/observations/<qualificationId>/`；已存在则失败关闭。Git dirty 与 commit/tree 漂移以探测结果为准。
...
- `qualified`：角色已接受、对象与缓存规则匹配、只改了 static DNS、传输/隔离/两个计费面都证明。
- `incomplete`：证据缺失或账单 delayed。
- `blocked`：dirty/mixed worktree、Authority origin、未接受角色、覆盖对象、DNS 越界、隔离失败或拦截型 DNS 观测。
