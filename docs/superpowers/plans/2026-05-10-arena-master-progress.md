...
| S | Teacher challenge templates and editable configuration | `docs/superpowers/plans/2026-05-11-arena-plan-s-teacher-templates.md` | Complete | Targeted tests, lint, smoke, build, Hegel re-review passed | `47498fca` |
| T | Code controller safety gate | `docs/superpowers/plans/2026-05-11-arena-plan-t-code-controller-safety.md` | Complete | Targeted tests, lint, smoke, build, Hegel re-review passed | `6d1a92ad` |
| U | White-box workbench preview and scheme comparison | `docs/superpowers/plans/2026-05-11-arena-plan-u-whitebox-workbench-preview.md` | Complete | Targeted tests, lint, smoke, build, Hegel re-review passed | `8a8a5b2d` |
| V | Robust disturbance challenge | `docs/superpowers/plans/2026-05-11-arena-plan-v-robust-disturbance-challenge.md` | Complete | Targeted tests, lint, smoke, build, Hegel re-review passed | `f31a7484` |
| W | Unstable typical object coverage | `docs/superpowers/plans/2026-05-11-arena-plan-w-unstable-object-coverage.md` | Complete | Targeted tests, runtime repro, lint, smoke, build, Hegel re-review passed | `14a543d8` |
...
- 2026-05-11: Plan S added teacher-facing challenge templates and editable configuration for target signals, disturbances, initial conditions, allowed methods, hard constraints, scoring weights, Pareto ranking, hidden tests, public leaderboard visibility, telemetry level, and homework grade binding. Hegel review found medium issues in grade-binding consistency, read-only template fields, and leaderboard policy override validation; fixes were applied and Hegel re-review found no blocker, high, or medium issues. Implementation commit `47498fca` recorded.
- 2026-05-11: Plan T added the safe pre-sandbox layer for code-type controller artifacts. `code-controller` is a known method but is excluded from current official student submission methods; metadata-only artifacts reject inline source, enforce language and sha256 lock hashes, and official evaluation fails closed without external sandbox verification. Hegel review found one medium API bypass risk in persistence; the fix now validates task/method and sanitizes code-controller metadata before hash, evaluation, artifact, or submission storage. Hegel re-review found no blocker, high, or medium issues. Implementation commit `6d1a92ad` recorded.
- 2026-05-11: Plan U closed the white-box student workbench gap by adding local deterministic workbench simulation preview and scheme comparison before official submission. The preview composes the same controller artifact builder with the white-box evaluator, compares candidate scores and primary metrics against the latest real submission, and does not create `ArenaSubmission` or leaderboard data. Hegel review found one medium issue in missing metric-level UI comparison; the fix renders metric deltas per primary metric. Hegel re-review found no blocker, high, or medium issues. Implementation commit `8a8a5b2d` recorded.
- 2026-05-11: Plan V added a dedicated white-box robust disturbance challenge for the ship-roll object. The new profile ranks hidden-scenario worst case, settling time, control energy, and overshoot, and explicitly declares `control_not_saturated` plus `hidden_scenarios_passed` as hard constraints. Hegel review found one medium issue in the initial hard-constraint declaration; the fix aligned the profile with the evaluator's actual validity gate. Hegel re-review found no blocker, high, or medium issues. Implementation commit `f31a7484` recorded.
- 2026-05-11: Plan W closed the typical-object gap for unstable systems by adding `plant-unstable-first-order`, a stabilization challenge, and a dedicated stabilization metric profile whose ranked metrics align with task primary metrics. The same phase fixed a runtime crash when `/arena` read submissions from a local database missing Arena tables: `listSubmissions` now degrades to an empty real-data list for Prisma `P2021` only, while write paths still fail normally. Hegel review found one high issue in the first stabilization scoring profile; the fix added metric-alignment invariant tests and positive-score evaluation coverage. Hegel re-review found no blocker, high, or medium issues. Implementation commit `14a543d8` recorded.
