...
Cruise 课程转向场景（`sim/cruise` / `cruise-comfort-course-turn` / `fleet-cruise-adora`）在完成态增加只读“控制效果复盘”：事实层只陈述当前运行已计算指标，任务层仅在课程任务给出权威阈值时逐项判断满足或未满足。复盘不重算物理量、不把结束时刻舵角或功率当作峰值或能量、也不改 Arena 评分、官方评价、榜单或学习证据。其他船型保持原完成态。
