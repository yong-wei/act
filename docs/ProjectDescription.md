...
框图工作台首版采用复合校正参数化模板，统一管理前置滤波、前馈通道、局部反馈、扰动补偿和控制量限幅，并通过统一提交面板生成 `composite-compensation` 官方评测工件。预测控制工作台采用有界线性 MPC 与优化辅助 PID 两类参数化模板，分别管理预测/控制时域、权重、限幅、采样时间和优化目标权重；隐藏场景最差表现等指标只标注为官方评测项，不在本地预览中伪造数值。
