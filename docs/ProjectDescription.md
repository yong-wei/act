...
📅 **最后更新**：2026-01-11
🧩 **整改进展**：课程编排资源模型统一进行中（编排器支持 TeachingResource / KnowledgeNode，互动资源已拆分为单页组件，旧内容迁移脚本已补齐）；Control Odyssey 通关结算指标与积分体系升级（超调/稳态/平均相对误差 + 分支得分基准），榜单分支标识与指标展示优化，飞船全息壳层外观联动控制器状态
...
- **定位**：仅用于单页互动资源浏览，不作为课程入口；课程编排与播放入口统一在管理员课堂流程
- **新增**：趣味探索分类与十滴水关卡进度/排行榜支持
- **Control Odyssey**：关卡青铜/白银/黄金分级与等级解锁、控制商店积分换购与控制器升级体系、测速反馈/前馈/史密斯预估器复合控制、难度滑块与积分倍率、配置面板分区与控制框图高亮、暗流扰动（含惯性滤波）与通道包络生成、通关结算三指标（超调/稳态/平均相对误差）与分支得分基准、榜单分支标识与指标展示、全息能量壳层飞船外形与动态尾迹、AI 控制建议（20 积分调用，关卡/配置/指标上下文，关卡内最新建议共享与高分配置上下文，建议历史落库）、失败结算详情曲线入口
