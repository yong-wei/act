...
| 浏览器 WASM 运行时 | `src/lib/control-engine/client.ts` | `preloadBrowserControlEngine`, `computeVirtualSimulationStepBrowserSync` | 浏览器侧通过 façade 调用 `compute_virtual_simulation_step`；若 runtime 未加载完成会抛错，而不是自动回退到本地模型。 |
| 服务端 WASM 运行时 | `src/lib/control-engine/server.ts` | `preloadServerControlEngine`, `computeVirtualSimulationServerStep` | 服务器端通过 façade 加载 `index_bg.wasm`，用于 API 路由侧的数值计算。 |
