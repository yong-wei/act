...
### 裁决三：模态独立绑定，禁止二级映射（2026-08-24）

课程负责人裁决（原文摘录）：

> 本来就应该是各自绑定，讲义和音频、视频各自视为不同的来源，不要搞讲义绑定到图谱，视频和音频再绑定到讲义这种二级映射。音视频虽然源于讲义，但是有自己的语言和逻辑体系。讲述的都是一个主题，但是涉及到的概念并不是完全一样的。

实施含义：

1. 讲义、音频、视频是三种独立资源模态，各自直接绑定 Canonical 节点；禁止"讲义→图谱、音视频→讲义"的间接结构。
2. 音频原子 = 转写文本按 VAD/语义边界分段（口语原子），节点映射基于音频自身的语言与概念体系；与讲义原子仅在节点层相遇（图串联）。
3. 讲义段落↔音频直接对齐（已证伪的 0.1–0.8% 路线）就此关闭，不投入模糊对齐（方向 B 否决）。
