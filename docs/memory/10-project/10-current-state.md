...
- OpenSpec/Buddy 仍是功能推进主线，但当前 active changes 已经从 6 月初的控制校正和智能助教提案扩展阶段，收束到二级页面迁移、知识图谱壳层迁移、数据中心角色可见性和二级导航视觉治理。React Doctor 错误清理系列已归档，覆盖 server、aria role、shared state/effect、interactive state/effect 和 resource state/effect；AppShell 折叠导航合同 #413 已归档，桌面收起态固定为 72px 图标栏并带 aria/title 标签。
...
- 平台 UI 重构系列已经把 Mission workspace shell、AppShell 折叠导航合同、数据中心、教师工作台、状态证据组件和角色导航推进到实现与归档阶段；当前需要继续迁移二级页面和知识图谱页面，并固化治理门禁，避免页面族回退。
...
- Active OpenSpec 数量不多，当前主要风险集中在统一 UI 壳层、二级导航迁移、角色可见性和视觉治理门禁。改动时应按页面族和治理规则限域，不要顺手重构无关业务页面。
