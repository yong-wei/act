...
- OpenSpec/Buddy 仍是功能推进主线，但当前 active changes 已经从 6 月初的控制校正和智能助教提案扩展阶段，收束到 React Doctor 错误清理和统一 UI 治理门禁。`eliminate-react-doctor-shared-state-effect-errors` 已归档；当前 active changes 是 `eliminate-react-doctor-interactive-state-effect-errors`、`eliminate-react-doctor-resource-state-effect-errors` 和 `harden-unified-ui-governance-gates`。
...
- 平台 UI 重构系列已经把 Mission workspace shell、数据中心、教师工作台、状态证据组件和角色导航推进到实现与归档阶段；当前需要继续固化治理门禁，避免页面族回退。
...
- Active OpenSpec 数量不多，但 React Doctor 剩余 interactive 与 resource state/effect 清理仍容易牵动课程 runtime、资源层和测试 harness。改动时应按错误族限域，不要顺手重构页面。
