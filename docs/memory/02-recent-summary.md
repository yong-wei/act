...
- 2026-06-12 如果当前会话已经在 `act-dev1`、`act-dev2`、`act-resource` 或 Codex 派生出的永久隔离工作树中进行开发，不要再为同一 change 新建第二层临时 worktree；这些永久工作树本身就是隔离边界。只有在主协调工作树中需要保护当前分支、或用户明确要求额外隔离时，才创建新的临时 worktree。
