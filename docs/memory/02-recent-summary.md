...
状态: active
最后更新: 2026-08-17
摘要: 初始化时优先读取的最近上下文入口。当前平台主线已经覆盖统一壳层、标准互动课、控制工作台、Arena、资源语义治理、知识图谱、数据治理和智能助教；31 课闲聊自控导入片已落到作者态 `media/processed/` 并由 `export-runtime.sh` 同步到本地 runtime。Issue #1125 已完成 CTKG 0.2 聚合协议变基的实现与文档同步，候选底座锁定 `control-theory-engineering-v0.2`，Legacy 仍为生产权威。
上游:
