...
审查发现共享物化器默认拒绝所有未授权事件后，历史回填中的服务端来源也会失去既有能力贡献。该问题成立：历史物化器是同一回填脚本的唯一候选事实构造边界，不能以“历史任务”整体放行，因为其中的 `InteractionLog` 仍保存原始客户端遥测。

修正决策：仅对已通过来源、真实性与资格门禁的服务端历史来源显式授予内存授权标记；`InteractionLog` 始终不授予该标记。这样保留服务端已核验历史测评、作答、仿真、设计和 Arena 提交的贡献语义，同时维持客户端互动记录的空贡献约束。
