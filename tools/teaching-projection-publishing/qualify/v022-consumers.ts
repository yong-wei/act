...
import { loadPrerequisitePublication, resolvePrerequisiteStorePaths, type PrerequisiteStorePaths } from '../../../src/lib/teaching-projection/prerequisites/store';
import {
...
import { V022_NAMED_CONSUMERS } from './v022-qualify-contract';
import {
  V022_RELEASE_ID,
  V022_SNAPSHOT,
  leakInDisplay,
...
export interface ConsumerShadowResult {
  consumerId: (typeof V022_NAMED_CONSUMERS)[number];
...
function runGraphReads(input: CandidateConsumerReadInput): ConsumerShadowRead[] {
  ...
  const domainId = input.catalog?.domains[0]?.domainId;
  if (!domainId) {
    pushRead(reads, 'engineering-graph', false, {
      authoritySnapshotId: input.authorityManifest.snapshotId,
    }, false, 'isolated-catalog-domain-missing');
    return reads;
  }
...
export function runNamedConsumerShadowReads(
  ...
  const byConsumer: Record<(typeof V022_NAMED_CONSUMERS)[number], ConsumerShadowRead[]> = {
  ...
  return V022_NAMED_CONSUMERS.map((consumerId) => {
...
export function runLiveNamedConsumerShadowReads(repoRoot: string): {
  ...
  try {
    ...
    if (authorityManifest.snapshotId !== V022_SNAPSHOT || authorityManifest.releaseId !== V022_RELEASE_ID) {
    ...
    if (results.length !== V022_NAMED_CONSUMERS.length) blockers.push('consumer-shadow-reads-incomplete');
