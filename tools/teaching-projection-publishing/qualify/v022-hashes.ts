...
import { authorityCanonicalJson, authoritySha256 } from '../../../src/lib/authoritative-knowledge/authority-snapshot';
...
export function collectDeclaredCandidateHashes(repoRoot: string): DeclaredCandidateHash[] {
  ...
  for (const row of mirrorFiles) {
    const rec = asRecord(row);
    declared.push({
      path: `${V022_RELEASE_RELATIVE}/${String(rec.path ?? '')}`,
      sha256: String(rec.rawSha256 ?? rec.sha256 ?? ''),
      source: 'authority-receipt.mirror',
    });
  }

  const bundleManifestPath = path.join(repoRoot, V022_RELEASE_RELATIVE, 'bundle-manifest.json');
  ...
  const validationPath = path.join(repoRoot, V022_RELEASE_RELATIVE, 'validation-report.json');
