...
fn compute(request: Value) -> Value {
    let result = compute_nonlinear_analysis(&request.to_string())
        .expect("nonlinear analysis should compute");
...
fn computes_phase_plane_trajectories_for_unit_5_2() {
    ...
    let trajectory = result["phasePlane"]["trajectories"][0]["points"]
        .as_array()
        .unwrap();
    ...
    assert!(
        vector_field
            .iter()
            .any(|point| point["x"].as_f64().unwrap() == -3.0)
    );
    assert!(
        vector_field
            .iter()
    ...
    assert!(
        vector_field
            .iter()
            .any(|point| point["y"].as_f64().unwrap() == -3.0)
    );
    assert!(
        vector_field
            .iter()
    ...
    assert_eq!(
        result["summary"]["outcome"].as_str().unwrap(),
        "阻尼二阶轨迹逐步收敛到原点"
    );
    ...
    assert!(
        vdp_field
            .iter()
            .any(|point| point["y"].as_f64().unwrap() == -4.0)
    );
    assert!(
        vdp_field
            .iter()
...
fn computes_negative_inverse_family_with_complex_branch() {
    ...
    let points = result["negativeInverse"]["curves"][0]["points"]
        .as_array()
        .unwrap();
    ...
    assert!(
        points
            .iter()
            .any(|point| point["im"].as_f64().unwrap().abs() > 0.01)
    );
    assert!(
        result["negativeInverse"]["curves"][0]["selectedPoint"]["amplitude"]
            .as_f64()
            .unwrap()
            > 1.9
    );
    assert_eq!(
        result["negativeInverse"]["curves"][0]["marks"]["direction"]
            .as_str()
            .unwrap(),
        "arrow_for_increasing_A"
    );
}
...
fn computes_harmonic_lowpass_with_output_comparison_and_spectrum() {
    ...
    assert!(
        result["harmonic"]["filteredOutput"]
            .as_array()
            .unwrap()
            .len()
            >= 100
    ...
    assert!(
        result["harmonic"]["describingFunctionApproximation"]
            .as_array()
            .unwrap()
            .len()
            >= 100
...
fn computes_distinct_characteristics_for_unit_5_2_tabs() {
    ...
    assert_ne!(
        saturation["characteristic"]["curve"],
        deadzone["characteristic"]["curve"]
    );
    assert!(
    ...
    assert!(
        saturation["characteristic"]["signalComparison"]["input"]
            .as_array()
            .unwrap()
            .len()
            >= 9
    ...
    assert!(
        saturation["characteristic"]["signalComparison"]["output"]
            .as_array()
            .unwrap()
            .len()
            >= 9
    ...
    assert!(
        hysteresis["characteristic"]["describingFunction"]["im"]
            .as_f64()
            .unwrap()
            .abs()
            > 0.01
...
fn computes_turning_radius_constraints_for_unit_5_3() {
    ...
    assert_eq!(
        tight["turningRadius"]["saturationActive"]
            .as_bool()
            .unwrap(),
        true
    );
    assert_eq!(
        wide["turningRadius"]["saturationActive"].as_bool().unwrap(),
        false
    );
    assert_eq!(
        tight["turningRadius"]["collisionActive"].as_bool().unwrap(),
        true
    );
    assert_eq!(
        medium["turningRadius"]["collisionActive"]
            .as_bool()
            .unwrap(),
        false
    );
    assert_eq!(
        wide["turningRadius"]["collisionActive"].as_bool().unwrap(),
        false
    );
    assert_eq!(
        medium["turningRadius"]["safetyConstraintSatisfied"]
            .as_bool()
            .unwrap(),
        false
    );
    assert_eq!(
        wide["turningRadius"]["safetyConstraintSatisfied"]
            .as_bool()
            .unwrap(),
        true
    );
    ...
    assert!(
        medium["turningRadius"]["dStartM"].as_f64().unwrap()
            > tight["turningRadius"]["dStartM"].as_f64().unwrap()
    );
    ...
    assert_eq!(
        tight["turningRadius"]["headingCurves"]
            .as_array()
            .unwrap()
            .len(),
        2
    ...
    assert!(
        wide["turningRadius"]["path"]["actual"]
            .as_array()
            .unwrap()
            .len()
            >= 120
    ...
    assert!(
        wide["summary"]["metrics"]
            .as_array()
            .unwrap()
            .iter()
            .any(|item| { item.as_str().unwrap_or("").contains("R=140") })
    );
}
