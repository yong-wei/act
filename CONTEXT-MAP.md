...
- [课程知识基座治理来源注册表](./docs/proposals/course-knowledge-base-governance-source-registry.yaml) — 机器可读地规定仓库 glob、数据库字段与 JSON selector、快照边界、namespace 和直接写入口。
 - [KA-Q 公式推导工具](./docs/contexts/kaq-formula-derivation/CONTEXT.md) — 管理 KA-Q 系统中 SymPy 公式推导组件的术语与架构决策。
...
- **共同约束**：两个上下文共享教学资源身份和课程运行能力，但分别维护备课决策与规范知识真相。学习者可见界面不得暴露系统内部标识，必须使用中文名称或通用中文回退。
 - **KA-Q 公式推导工具 → 课程知识基座**：公式推导涉及的知识概念（如拉普拉斯变换、传函）引用规范知识概念，但推导逻辑和展示不属于知识基座治理范围。
 - **KA-Q 公式推导工具 → 教师备课**：公式推导 API 可在课件/教案中被引用，但推导引擎本身独立于备课工作流。
