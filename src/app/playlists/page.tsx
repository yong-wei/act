...
export default async function PlaylistsPage() {
  ...
  const viewerRole = resolvePlaylistViewerRole(session?.user?.role);
  const canCreatePlaylist = viewerRole === 'teacher' || viewerRole === 'admin';
  ...
  return (
    <AppShell
      ...
      activeHref="/playlists"
      activeNavigationHref={getPlaylistNavigationParent(viewerRole)}
      accountHref={getPlatformCockpitHref(session?.user?.role)}
      title="课程播放列表"
      subtitle="管理和播放互动课程流。"
      breadcrumbs={[{ label: '首页', href: '/' }, { label: '课程播放列表' }]}
    >
      <div className="mb-8 flex items-center justify-between gap-4">
        <div>
          <h2 className="text-3xl font-bold text-platform-fg-primary">课程播放列表</h2>
          <p className="mt-2 text-platform-fg-secondary">管理和播放您的互动课程流</p>
        </div>
        {canCreatePlaylist ? (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {playlists.map((playlist) => (
          <Card key={playlist.id} className="bg-[#0F172A] border-slate-700 hover:border-blue-500 transition-all group">
            <CardHeader>
              <CardTitle className="text-white group-hover:text-blue-400 transition-colors">{playlist.title}</CardTitle>
            ...
            <CardContent>
              <p className="text-sm text-slate-400 line-clamp-2 min-h-[40px]">{playlist.description || '暂无描述'}</p>
            ...
            <CardFooter className="border-t border-slate-800 pt-4">
              <Link href={`/playlists/${playlist.id}/play`} className="w-full">
                <button
                  type="button"
                  className="w-full flex items-center justify-center gap-2 bg-slate-800 hover:bg-blue-600 text-slate-200 hover:text-white py-2 rounded transition-colors group-hover:bg-blue-600 group-hover:text-white"
                >
        ...
        {playlists.length === 0 && (
          <div className="col-span-full rounded-lg border border-dashed border-platform-border bg-platform-surface py-20 text-center text-platform-fg-muted">
            暂无公开课程流。
          </div>
        )}
      </div>
...
function resolvePlaylistViewerRole(role?: string | null): PlatformRole {
