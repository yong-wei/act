...
export async function PUT(request: Request) {
  ...
  const { saved, operationLedger } = await prisma.$transaction(async (tx) => {
    const savedSettings = await setAIProviderSettings(settings, tx);
    const completedAt = new Date().toISOString();
    const activeProvider = savedSettings.providers.find((provider) => provider.id === savedSettings.activeProvider);
    const idempotencyKey = buildAdminOperationIdempotencyKey([
      'admin-config-save',
      'ai-settings',
      JSON.stringify({
        activeProvider: savedSettings.activeProvider,
        providers: savedSettings.providers.map((provider) => ({
          id: provider.id,
          name: provider.name,
    ...
    const ledger = buildAdminOperationLedgerEntry({
      ...
      scope: `admin-config-ai-settings:${savedSettings.activeProvider}`,
      startedAt: completedAt,
      completedAt,
      outcome: 'completed',
      idempotencyKey,
      artifactRefs: [{
        id: `admin-config-ai-settings:${savedSettings.activeProvider}`,
        kind: 'config-diff',
        label: 'AI 供应商配置',
      ...
      auditSummary: `AI 设置已保存：activeProvider=${savedSettings.activeProvider}，selectedModel=${activeProvider?.selectedModel ?? 'none'}。`,
      recoveryState: {
        status: 'available',
    ...
    return { saved: savedSettings, operationLedger: ledger };
  });
  return NextResponse.json({
    ...saved,
    operationLedger,
