...
export const KONLING_TEACHING_ASSISTANT_MODE_REGISTRY: Record<KonlingTeachingAssistantModeId, KonlingTeachingAssistantModeContract> = {
  ...
  'prep-coauthor': teachingAssistantMode({
    ...
    permittedTools: ['get_page_context', 'get_learner_state', 'get_plan_context', 'search_knowledge_graph', 'propose_smart_lesson_task_change'],
    ...
    getStudentRiskFlags: async (args: { userId?: string }) =>
      runKonlingRuntimeTool(input, 'get_student_risk_flags', args, async () => {
        ...
        return flags.map((f) => ({
          ...
          evidenceSummary: f.evidenceJson,
        }));
      }),
    getClassCompetencySummary: async (args: { classId?: string }) =>
      runKonlingRuntimeTool(input, 'get_class_competency_summary', args, async () => {
        const { prisma } = await import('@/lib/prisma');
        const classId = args.classId ?? input.scope.classId;
        if (!classId) return { unavailable: true, reason: 'no-class-scope' };
        const snapshot = await prisma.classCompetencySnapshot.findFirst({
          where: { classId },
          orderBy: { snapshotAt: 'desc' },
        });
        if (!snapshot) return { unavailable: true, reason: 'no-snapshot' };
        return {
          classId: snapshot.classId,
          snapshotAt: snapshot.snapshotAt.toISOString(),
          aggregated: snapshot.aggregateJson,
          distribution: snapshot.distributionJson,
          trend: snapshot.trendJson,
          riskSummary: snapshot.riskSummaryJson,
          levelDistribution: snapshot.levelDistribution,
          activeStudentCount: snapshot.activeStudentCount,
          totalStudentCount: snapshot.totalStudentCount,
        };
      }),
    getStudentKnowledgeProgress: async (args: { userId?: string; nodeIds?: string[] }) =>
      runKonlingRuntimeTool(input, 'get_student_knowledge_progress', args, async () => {
        const { prisma } = await import('@/lib/prisma');
        const userId = args.userId ?? input.scope.targetUserId;
        const progress = await prisma.knowledgeProgress.findMany({
          where: {
            userId,
            ...(args.nodeIds?.length ? { nodeId: { in: args.nodeIds } } : {}),
          },
...
export function buildScopedKonlingAiTools(runtime: ReturnType<typeof buildKonlingToolRuntime>) {
  const tools = {
    ...
    get_student_risk_flags: tool({
      description: '获取学生的风险标记列表（participation, stagnation, ai_misuse, constraint, cross_domain）',
      inputSchema: z.object({
        userId: z.string().optional().describe('目标学生ID，如不提供则使用当前会话的学生'),
      }),
      execute: (args) => runtime.getStudentRiskFlags(args as { userId?: string }),
    }),
    get_class_competency_summary: tool({
      description: '获取班级的能力聚合摘要（均值、分布、趋势、风险摘要）',
      inputSchema: z.object({
        classId: z.string().optional().describe('班级ID，如不提供则使用当前会话的班级'),
      }),
      execute: (args) => runtime.getClassCompetencySummary(args as { classId?: string }),
    }),
    get_student_knowledge_progress: tool({
      description: '获取学生逐知识点掌握度（status, progress 0-100, timeSpent），最多返回50条',
      inputSchema: z.object({
        userId: z.string().optional().describe('目标学生ID'),
        nodeIds: z.array(z.string()).optional().describe('限定知识点ID列表'),
      }),
      execute: (args) => runtime.getStudentKnowledgeProgress(args as { userId?: string; nodeIds?: string[] }),
    }),
  };
