...
export const UNIT_3_5_STEP_AI_CONTEXTS: Record<string, AIContextConfig> = {
  ...
  'step-10': context(
    'step-10',
    'PD 单独装置的频域指纹：中高频抬升与高频代价',
    'practice',
    ['让 PD 的频域原理先以频带分工出现。', 'PD 更像持续抬升中高频。'],
    [
      { label: '本页目标', question: '为什么这一步要先从频带分工理解 PD，而不是只记“它会变快”？' },
      { label: '允许范围', question: '这一步允许我从哪些角度理解或检查自己的判断？' },
    ],
    '允许范围：中高频抬升；交叉频率；噪声放大。禁止范围：把 PD 直接等同为更强的超前。关键事实：PD 更像持续抬升中高频。',
  ),
  'step-11': context(
    'step-11',
    '超前单独装置的频域指纹：相位峰、补角与作用频带',
    'practice',
    ['把超前理解成关键频带补相角。', '超前更像在截止频率附近补相角。'],
    [
      { label: '本页目标', question: '为什么超前更像关键频带补相角，而不是更强的 PD？' },
      { label: '允许范围', question: '这一步允许我从哪些角度理解或检查自己的判断？' },
    ],
    '允许范围：相位峰；补相角；关键频带。禁止范围：只拿零点位置做唯一比较。关键事实：超前更像在截止频率附近补相角。',
  ),
  'step-12': context(
    'step-12',
    '频域对照：什么时候优先想 PD，什么时候优先想超前',
    'practice',
    ['把频域原理收束成初步选项判断。', '本页只做初步判断，不做完整设计。'],
    [
      { label: '本页目标', question: '如果面对新情境，这一步要我优先判断哪些约束？' },
      { label: '允许范围', question: '这一步允许我从哪些角度理解或检查自己的判断？' },
    ],
    '允许范围：速度；裕度；高频代价；模型不确定性。禁止范围：提前进入模块 4 的多目标排序。关键事实：本页只做初步判断，不做完整设计。',
  ),
  'step-13': context(
    'step-13',
    '非最小相入口：镜像零点、逆响应与“先反向动”',
    'practice',
    ['把非最小相先压成现象加结构来源。', '非最小相不只是名字不同。'],
    [
      { label: '本页目标', question: '为什么要先把镜像对象、逆响应和名称来源放在一起看？' },
      { label: '允许范围', question: '这一步允许我从哪些角度理解或检查自己的判断？' },
    ],
    '允许范围：镜像对象；逆响应；名称来源。禁止范围：只给术语定义，不回到现象图。关键事实：非最小相不只是名字不同。',
  ),
  'step-14': context(
    'step-14',
    '频域边界：右半平面零点为何限制带宽',
    'practice',
    ['把非最小相边界从现象收束到控制动作。', '非最小相对象通常先保守带宽。'],
    [
      { label: '本页目标', question: '为什么右半平面零点会让我们先保守带宽，而不是继续把带宽往上推？' },
      { label: '允许范围', question: '这一步允许我从哪些角度理解或检查自己的判断？' },
    ],
    '允许范围：额外相位滞后；带宽上限；保守交叉频率。禁止范围：把边界表述成完整校正设计流程。关键事实：非最小相对象通常先保守带宽。',
  ),
  'step-15': context(
    'step-15',
    '后测与收束：四个观察量、风险边界与 3-6 入口',
    'quiz',
    ['把本课收束为可带入 3-6 的判断地图。', '本课输出是判断地图，不是完整设计答案。'],
    [
      { label: '本页目标', question: '这一步结束时，为什么要把四个观察量一起带走，而不是只记某一个结论？' },
      { label: '允许范围', question: '这一步允许我从哪些角度理解或检查自己的判断？' },
    ],
    '允许范围：四个观察量；风险边界；后续实验入口。禁止范围：AI 直接替学生完成后测解释题。关键事实：本课输出是判断地图，不是完整设计答案。',
  ),
};
