...
export const UNIT_3_4_STEP_AI_CONTEXTS: Record<string, AIContextConfig> = {
  ...
  'step-11': context(
    ...
    '频域代价',
    'practice',
    ['写清版本 C 的频域收益与频域代价', '把 Bode 图读成一条权衡句'],
    [
      { label: '为什么不能只看收益', question: '为什么版本 C 的频域判断不能只说带宽提高，而必须同时写出裕量和共振峰代价？' },
      { label: '频域句怎么写', question: '一个完整的频域收益-代价句，至少应包含哪两类指标？' },
    ],
  ...
  'step-12': context(
    ...
    '连续跟踪中的收益与代价',
    'practice',
    ['同时保留 B/C 两张连续跟踪图', '写出完整收益-代价句'],
    [
      { label: '为什么必须双图并列', question: '为什么 4.6 需要把 B 和 C 两张斜坡响应与航迹图并排放，而不是只保留 C？' },
      { label: '收益代价句怎么写', question: '一个完整的连续跟踪收益-代价判断句，至少应把哪两类证据放在一起？' },
    ],
  ...
  'step-13': context(
    ...
    '广义根轨迹与等效改写',
    'practice',
    ['识别 a 不是再调一次 K', '把局部反馈结构改写成等效根轨迹问题'],
    [
      { label: '为什么不是再调一次 K', question: '为什么局部反馈系数 a 不能被理解成“再调一次 K”？' },
      { label: '改写链怎么串', question: '从局部反馈结构走到等效根轨迹，关键的几步改写链是什么？' },
    ],
    '只能解释改写链和变量意义，不代替学生完成推导记录。',
  ),
  'step-14': context(
    'step-14',
    '非增益参数窗口记录',
