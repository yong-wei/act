/** Capture-bound denominator and database-observation helpers for #1443. */
...
import type { ActiveCourseInventory } from '../migration-contracts';
import type {
...
export class V022CaptureError extends Error {
  readonly code: string;

  constructor(code: string, message: string) {
    super(message);
    this.name = 'V022CaptureError';
    this.code = code;
  }
}
...
function normalizeRelative(repoRoot: string, filePath: string): string {
  ...
  if (!value || value === '.' || value.startsWith('../') || value.includes('/../')) {
    throw new V022CaptureError('path-outside-repository', `capture path escapes repository: ${filePath}`);
  }
  return value;
}
...
function gitModeAtRevision(repoRoot: string, revision: string, path: string): string {
  ...
  if (!mode) throw new V022CaptureError('capture-file-missing', `capture revision has no mode for ${path}`);
  return mode;
}

function membershipDigest(repoRoot: string, revision: string, collection: V022CaptureCollection): string {
...
function executionDigest(repoRoot: string, revision: string, collection: V022CaptureCollection): string {
...
function currentCollectionEntries(repoRoot: string, collection: V022CaptureCollection): {
  paths: string[];
  execution: string[];
} {
 ...
 */
export function buildV022CaptureManifest(input: {
  repoRoot: string;
  captureRevision: string;
  files: readonly string[];
  collections?: readonly Omit<V022CaptureCollection, 'membershipDigest' | 'executionDigest'>[];
}): V022CaptureManifest {
  if (!SHA.test(input.captureRevision)) {
    throw new V022CaptureError('invalid-capture-revision', 'captureRevision must be a full Git SHA');
  }
  const files: V022CaptureFile[] = [...new Set(input.files.map((file) => normalizeRelative(input.repoRoot, file)))]
    .sort()
    .map((path) => {
  ...
  const collections: V022CaptureCollection[] = [...(input.collections ?? [])]
    .map((collection) => ({
      ...collection,
      membershipDigest: membershipDigest(input.repoRoot, input.captureRevision, collection as V022CaptureCollection),
      executionDigest: executionDigest(input.repoRoot, input.captureRevision, collection as V022CaptureCollection),
    }))
 ...
 */
export function assertV022CaptureBound(
  repoRoot: string,
  manifest: V022CaptureManifest,
): void {
  if (manifest.contract !== 'act-teaching-projection-capture-bound-inputs/v2') {
    throw new V022CaptureError('manifest-contract-invalid', 'unsupported v0.22 capture manifest contract');
  }
  ...
  if (manifest.inputDigest !== projectionDigest(body)) {
    throw new V022CaptureError('manifest-digest-drift', 'v0.22 capture manifest inputDigest drift');
  }
  for (const file of manifest.files) {
    const path = normalizeRelative(repoRoot, file.path);
    const abs = resolve(repoRoot, path);
    if (!existsSync(abs)) throw new V022CaptureError('file-drift', `captured file is missing: ${path}`);
    const workingBytes = readFileSync(abs);
    if (sha256(workingBytes) !== file.blobSha256) {
      throw new V022CaptureError('file-drift', `captured file bytes drifted: ${path}`);
    }
    if (gitModeAtRevision(repoRoot, manifest.captureRevision, path) !== file.mode) {
      throw new V022CaptureError('file-mode-drift', `captured file mode drifted: ${path}`);
    }
  }
  for (const collection of manifest.collections) {
    ...
    if (projectionDigest(current.paths) !== collection.membershipDigest) {
      throw new V022CaptureError('membership-drift', `capture membership drifted: ${collection.collectionId}`);
    }
    if (projectionDigest(current.execution) !== collection.executionDigest) {
      throw new V022CaptureError('execution-drift', `capture execution set drifted: ${collection.collectionId}`);
    }
  }
}

function resourceKind(resourceType: TeachingResourceType, sourcePath: string): V022ReferenceKind {
...
export function collectOverlayInfographReferences(input: {
  ...
  const records: V022ReferenceRecord[] = [];
  for (const pkg of input.inventory.packages) {
    ...
    if (!existsSync(abs)) {
      throw new V022CaptureError('infograph-overlay-missing', `captured graph overlay is missing: ${overlayPath}`);
    }
 ...
 */
export function buildV022ReferenceDenominator(input: {
  inventory: ActiveCourseInventory;
  priorArtifacts: TeachingProjectionArtifacts;
  captureRevision: string;
  repoRoot?: string;
}): V022ReferenceRecord[] {
  const records = new Map<string, V022ReferenceRecord>();
  const add = (record: V022ReferenceRecord) => {
...
export function buildV022CaptureReceipt(input: {
  manifest: V022CaptureManifest;
  inventory: ActiveCourseInventory;
...
export const V022_DATABASE_QUERY_CONTRACT = {
  contract: 'actkg-v022-candidate-admission-db-query/v1',
...
export const V022_DATABASE_QUERY_CONTRACT_HASH = projectionDigest(V022_DATABASE_QUERY_CONTRACT);

export function expectedV022AdmissionSchemaIdentity(input: {
  snapshotId: string;
  releaseId: string;
  bundleDigest: string;
  prerequisiteInputDigest: string;
}): string {
  return `actkg_admission_v022_${projectionDigest({
...
export interface V022DatabaseUrlSafety {
  accepted: true;
 ...
 */
export function assertV022DisposableDatabaseUrl(databaseUrl: string): V022DatabaseUrlSafety {
  let url: URL;
  try {
    url = new URL(databaseUrl);
  } catch {
    throw new V022CaptureError('database-observation-source-rejected', 'DATABASE_URL is not a valid PostgreSQL URL');
  }
  if (url.protocol !== 'postgres:' && url.protocol !== 'postgresql:') {
    throw new V022CaptureError('database-observation-source-rejected', 'DATABASE_URL must use a PostgreSQL URL scheme');
  }
...
export function buildV022DatabaseObservation(input: {
  schemaIdentity: string;
  environmentIdentity: string;
  authoritySnapshotId: string;
  parameters: Record<string, string | number | boolean | null>;
  objectRows: readonly V022DatabaseObservation['objectRows'][number][];
  prerequisiteRows: readonly V022DatabaseObservation['prerequisiteRows'][number][];
}): V022DatabaseObservation {
...
export function validateV022DatabaseObservation(input: {
  observation: V022DatabaseObservation | null | undefined;
  authoritySnapshotId: string;
  ...
  if (observation.contract !== 'actkg-v022-candidate-admission-db-observation/v1') findings.push('database-contract-invalid');
  ...
  if (observation.queryContractHash !== V022_DATABASE_QUERY_CONTRACT_HASH) findings.push('database-query-contract-drift');
...
export function resourceTypeFromV022Id(resourceId: string): TeachingResourceType {
