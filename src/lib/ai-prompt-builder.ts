...
function buildAdaptiveRuntimeSection(runtime: KonlingPromptRuntimeContext): string {
  ...
  if (runtime.teachingAssistantMode) {
    ...
    if (mode.mode.id === 'path-advisor') {
      ...
      lines.push('  - 候选选路边界: 用户明确选择已有候选时，只能调用 select_learning_path，并传入当前上下文提供的 authorized-candidate-batch 标识；有明确 candidateId 时一并传入，否则把用户原话放入 naturalLanguageIntent。不得提交标题、序号或 selectedStyleId 代替持久化身份。工具返回 clarification_required 时必须按原顺序展示 alternatives 并提问，不得自行选择。');
