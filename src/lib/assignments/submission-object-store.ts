...
export interface StoredObjectMetadata { key: string; ownerId: string; answerId: string; sizeBytes: number; mimeType: string; checksum: string; scanState: 'CLEAN' | 'PENDING' | 'UNSAFE' }
...
export class S3CompatibleSubmissionObjectStore implements SubmissionObjectStore {
  ...
  async signUpload(intent: Omit<StoredObjectMetadata, 'key' | 'scanState'>, ttlSeconds = 600, objectKey?: string) {
    ...
    const command = new PutObjectCommand({ Bucket: this.config.bucket, Key: key, ContentType: intent.mimeType, ContentLength: intent.sizeBytes, ChecksumSHA256: checksumBase64(intent.checksum), Metadata: { owner: intent.ownerId, answer: intent.answerId }, Tagging: 'scan-state=PENDING' });
  ...
  async head(key: string): Promise<StoredObjectMetadata | null> {
    try {
      ...
      return { key, ownerId: head.Metadata?.owner ?? '', answerId: head.Metadata?.answer ?? '', sizeBytes: head.ContentLength ?? -1, mimeType: head.ContentType ?? '', checksum: checksumHex(head.ChecksumSHA256), scanState: scanTag === 'CLEAN' ? 'CLEAN' : scanTag === 'UNSAFE' ? 'UNSAFE' : 'PENDING' };
    } catch (error) { if ((error as { name?: string }).name === 'NotFound' || (error as { $metadata?: { httpStatusCode?: number } }).$metadata?.httpStatusCode === 404) return null; throw new SubmissionError('object-store-head-failed', 502); }
  }
  ...
  async delete(key: string) { try { await this.client.send(new DeleteObjectCommand({ Bucket: this.config.bucket, Key: key })); } catch { throw new SubmissionError('object-store-delete-failed', 502); } }
}
...
export class MemorySubmissionObjectStore implements SubmissionObjectStore {
  ...
  async delete(key: string) { this.objects.delete(key); this.payloads.delete(key); }
}
...
function normalizeScannerS3Error(error: unknown): SubmissionError {
  ...
  if (status === 404 || name === 'NoSuchKey' || name === 'NotFound') return new SubmissionError('scanner-object-disappeared', 409);
