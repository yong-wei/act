...
function clamCommand(config: { host: string; port: number }, payload: Buffer): Promise<string> { return new Promise((resolve, reject) => { const socket = createConnection(config); const chunks: Buffer[] = []; let settled = false; const finish = () => { if (settled) return; settled = true; const response = Buffer.concat(chunks).toString('utf8'); socket.end(); resolve(response); }; socket.setTimeout(30_000); socket.on('connect', () => socket.write(payload)); socket.on('data', (chunk) => { chunks.push(chunk); if (chunk.includes(0)) finish(); }); socket.on('end', finish); socket.on('timeout', () => socket.destroy(new Error('clamav-timeout'))); socket.on('error', (error) => { if (!settled) { settled = true; reject(error); } }); }); }
