...
export const UNIT_2_2_COURSE_META = {
...
function context(
  ...
  return {
    enabled: true,
    courseId: UNIT_2_2_COURSE_META.courseId,
    courseTitle: UNIT_2_2_COURSE_META.courseTitle,
    pageType,
    stepId,
    topic,
    learningObjectives,
...
export const UNIT_2_2_STEP_AI_CONTEXTS: Record<string, AIContextConfig> = {
  'step-01': context(
    'step-01',
    '课程定位：从闭环传递函数走向响应曲线',
    'theory',
...
export function getUnit22StepAIContext(stepId: string): AIContextConfig | null {
  return UNIT_2_2_STEP_AI_CONTEXTS[stepId] ?? null;
}

export function getUnit22StepQuickQuestions(stepId: string): Array<{ label: string; question: string }> {
  return UNIT_2_2_STEP_AI_CONTEXTS[stepId]?.quickQuestions ?? [];
}

export const getUNIT_2_2StepAIContext = getUnit22StepAIContext;
export const getUNIT_2_2StepQuickQuestions = getUnit22StepQuickQuestions;
