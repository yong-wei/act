...
const STEP_SOURCE = [
  ...
  ['step-03', 'P1', '前置基础快测', '本页考察闭环误差、反馈质量和执行器限幅三类基础知识，只诊断闭环误差、反馈质量和执行约束基础。', 'quiz_group'],
  ...
  ['step-07', 'P2', '规划参考可实现性', '规划层面对任务空间，控制层拿到的是参考航迹、航向或速度剖面；本页把几何路径、航向参考、控制输出和实际舵角连成证据链。', 'single_choice'],
  ['step-08', 'P2', '执行约束与可实现反馈', '说明执行约束会改变真实运动并反馈给规划与控制。', 'activity_card_set'],
  ['step-09', 'P2', '链路责任诊断顺序', '建立从信息到监督的链路诊断顺序。', 'activity_card_set'],
  ['step-10', 'P2', '自主避碰中的偏差传播场景', '一艘自主水面船在狭窄航道保持计划航线，并遇到横穿目标船，需要完成安全避让；本页把避碰偏差分解为信息、参考、控制和执行证据。', 'activity_card_set'],
  ['step-11', 'P2', '避碰转弯半径与舵角可行域分析', '通过单情形互动面板判断规划半径、舵角上限和安全约束之间的联动。', 'activity_card_set'],
  ['step-12', 'P2', 'MASS 自动化等级与责任边界', '自动化等级描述运行形态和责任主体差异，但技术诊断仍需回到链路证据；本页区分自动化等级、责任主体和技术链路证据。', 'single_choice'],
