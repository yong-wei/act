...
export const UNIT_3_2_STEP_AI_CONTEXTS: Record<string, AIContextConfig> = {
  ...
  'step-02': context(
    ...
    [
      { label: '闭环特征方程', question: '为什么只盯闭环特征方程的极点迁移图，还不足以直接写出参数可行域？' },
      { label: '系数判稳', question: '从闭环特征方程系数直接判稳，究竟补上了哪一层证据？' },
    ],
  ...
  'step-04': context(
    ...
    [
      { label: '固定 k=4', question: '把 k=4 固定以后，为什么只看第一列符号变化就已经能回答右半平面根数问题？' },
      { label: '为什么还没求根', question: 'k=4 这一页已经能判稳，但为什么仍没有显式根表达式？' },
    ],
  ...
  'step-08': context(
    ...
    [
      { label: '全零行含义', question: '全零行为什么会暴露对称根结构，从而必须引入辅助方程？' },
      { label: '导数替换', question: '为什么全零行对应的辅助方程求导后可以回填劳斯表，而不是任意换一行？' },
    ],
  ...
  'step-11': context(
    ...
    [
      { label: 'Re(s)<-0.5', question: '为什么把 Re(s)<-0.5 这样的竖线约束写成变量平移后，问题又能回到普通劳斯判稳？' },
