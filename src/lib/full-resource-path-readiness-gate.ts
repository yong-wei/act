...
export function buildFullResourcePathReadinessGate(input: {
  ...
  const findings = [
    ...
    findingIf('yang-fan-fixture-blockers', input.evidenceLineageSummary.yangFanFixtureBlockers.blockerCount, 'blocking', 'Yang Fan fixture generation remains blocked by fixture-owned lineage limitations.', ['resource-evidence-lineage-readiness-summary.json']),
    findingIf('yang-fan-fixture-limited-coverage', input.evidenceLineageSummary.yangFanFixtureBlockers.globalLimitationCount, 'warning', 'Yang Fan fixture-owned resources are scoped separately; unrelated global resource backlog remains visible as limited coverage.', ['resource-evidence-lineage-readiness-summary.json']),
...
export function renderFullResourcePathReadinessGateEvidence(report: FullResourcePathReadinessGateReport): string {
  const lines = [
    ...
    `Yang Fan fixture global limitations: ${report.resourceCoverage.yangFanFixtureBlockers.globalLimitationCount}`,
    '',
