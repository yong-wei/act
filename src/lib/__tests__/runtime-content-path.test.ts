...
describe('runtime content path resolver', () => {
  it('accepts project-relative runtime and content paths inside fixed roots', async () => {
    ...
    expect(resolveReadableContentPath('content/demo/example.mdx').projectPath)
      .toBe('content/demo/example.mdx');
  });
