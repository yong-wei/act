...
import { prepareActKgV022CutoverQualification } from '../../../scripts/knowledge-cutover/qualify-actkg-v022-cutover-candidate';
import { CTKG_SCHEMA_V2_RAW_SHA256 } from '../../../scripts/actkg-release/bundle-compatibility-registry-v2';
import {
...
describe('v0.22 cutover qualification', () => {
  it('qualifies the sealed compound candidate without moving production selectors', async () => {
    ...
    const outputRoot = mkdtempSync(path.join(tmpdir(), 'act-v022-qualify-'));
    roots.push(outputRoot);
    const result = await prepareActKgV022CutoverQualification([
      '--repo-root',
      REPO_ROOT,
      '--output-root',
      outputRoot,
    ...
    const report = JSON.parse(readFileSync(path.join(outputRoot, 'qualification-readiness.json'), 'utf8')) as {
      ...
      inputHashes: { schemaRawSha256?: string };
      audit: { teachingCoverageGap: number; reviewRequiredCount: number };
    };
    ...
    expect(report.consumerResults.map((row) => row.consumerId).sort()).toEqual([...V022_NAMED_CONSUMERS].sort());
    ...
    expect(report.inputHashes.schemaRawSha256).toBe(CTKG_SCHEMA_V2_RAW_SHA256);
    expect(report.audit.reviewRequiredCount).toBe(0);
    expect(report.audit.teachingCoverageGap).toBeGreaterThan(0);
  ...
  it('binds the declared label index hash so a mutated row cannot stay READY', () => {
    ...
    const scratch = mkdtempSync(path.join(tmpdir(), 'act-v022-label-'));
    roots.push(scratch);
  ...
  it('refuses to write qualification output onto a runtime selector path', async () => {
    await expect(prepareActKgV022CutoverQualification([
      '--repo-root',
      REPO_ROOT,
      '--output-root',
