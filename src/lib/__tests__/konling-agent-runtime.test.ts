...
vi.mock('@/lib/course-bundle', () => ({
  ...
  // #2047：konling 运行时经工程教材引用链导入 textbook-reader，其模块级
  // 依赖需要这两个导出存在（不会被该测试实际调用）。
  loadStructuredTextbookBook: vi.fn(),
  STRUCTURED_TEXTBOOK_TITLES: {
    ...
    'hu-shousong-auto-control-7th': '自动控制原理（胡寿松）',
...
describe('konling agent runtime', () => {
  ...
  it('search_textbook keeps Legacy foreground unchanged when no layered payload supplies the composed sidecar', async () => {
    // #2047：#1112 影子上下文输入已删除（无生产调用方可供版本绑定证据集），
    // 组合通道由分层 payload 驱动；无 payload 时前台结果保持 Legacy 原样，
    // 不产生 composed 影子诊断。组合/影子语义见 konling-consumes-graphs-and-resources 测试。
    const productionForeground = {
      candidates: [{
        identity: { kind: 'unit' as const, bookId: 'dorf-modern-control-systems', edition: '14th Global Edition', sourceRevision: 'rev-1', unitId: 'textbook-unit:production-only-foreground' },
        title: '生产前台候选',
        href: '/textbooks/dorf-modern-control-systems/14th Global Edition/chapter-chapter-03',
        limitation: null,
      }],
    };
    mocks.retrieveTextbookSourcePackV2Progressive.mockResolvedValue({
    ...
    const result = await runtime.searchTextbook({ query: '请解释根轨迹的基本概念' }) as {
      candidates?: unknown[];
      composedRagShadowDiagnostic?: unknown;
      teachingResourceReferences?: unknown;
    };
    expect(result.candidates?.[0]).toMatchObject({
      identity: { unitId: 'textbook-unit:production-only-foreground' },
      // 服务器分配的引用编号与 Legacy 前台语义保持一致。
      displayNumber: 1,
    });
    expect(result.composedRagShadowDiagnostic).toBeUndefined();
