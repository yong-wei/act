...
const QUIZ_ITEMS: QuizItem[] = [
  {
    ...
    answerId: 'B',
    explanation: '右半平面极点对应指数发散，系统不稳定。',
  },
  {
    id: 'zero-pole',
    title: '概念复盘',
    prompt: '传递函数分子与分母的根分别称为：',
    options: [
      { id: 'A', label: '极点/零点' },
      { id: 'B', label: '零点/极点' },
      { id: 'C', label: '特征根/特征根' },
    ...
    answerId: 'B',
    explanation: '分子根为零点，分母根为极点。',
  },
  {
    id: 'matlab',
    title: '工具复盘',
    prompt: 'MATLAB 中构造传递函数常用的命令是：',
    options: [
      { id: 'A', label: 'eig' },
      { id: 'B', label: 'tf' },
      { id: 'C', label: 'fft' },
    ...
    answerId: 'B',
    explanation: 'tf 是 MATLAB 控制工具箱中创建传递函数的核心命令。',
  },
];

interface TransferExitQuizProps extends BaseWidgetProps {}

export default function TransferExitQuiz({ onComplete, onStateChange }: TransferExitQuizProps) {
    ...
    <div className="w-full max-w-4xl mx-auto">
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-sm text-slate-500">后测 · 传递函数</p>
            <h2 className="text-2xl font-bold text-slate-900">传函速测</h2>
          </div>
