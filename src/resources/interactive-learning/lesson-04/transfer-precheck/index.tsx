...
const QUIZ_ITEMS: QuizItem[] = [
  {
    ...
    prompt: '传递函数的正确定义是：',
    options: [
      { id: 'A', label: '输出与输入的时域比值' },
      { id: 'B', label: '零初始条件下输出拉氏变换与输入拉氏变换之比' },
      { id: 'C', label: '输出与输入的微分关系' },
      { id: 'D', label: '输出的傅里叶变换' },
    ],
    answerId: 'B',
    explanation: '传递函数在零初始条件下定义，为 Y(s)/U(s)。',
  },
  {
    id: 'assumption',
    title: '前提条件',
    ...
    options: [
      { id: 'A', label: '系统是非线性系统' },
      { id: 'B', label: '系统处于零初始条件' },
      { id: 'C', label: '系统无阻尼' },
      { id: 'D', label: '输入必须是阶跃信号' },
    ],
    answerId: 'B',
    explanation: '零初始条件是传递函数定义成立的前提。',
  },
  {
    id: 'order',
    title: '系统阶次',
    prompt: '传递函数分母多项式的阶次代表：',
    options: [
      { id: 'A', label: '输入信号阶次' },
      { id: 'B', label: '系统阶次' },
      { id: 'C', label: '零点数量' },
    ...
    answerId: 'B',
    explanation: '分母最高阶次就是系统阶次。',
  },
  {
    id: 'rlc',
    title: '典型环节',
    prompt: 'RLC 串联电路一般可归类为：',
    options: [
      { id: 'A', label: '比例环节' },
      { id: 'B', label: '一阶惯性环节' },
      { id: 'C', label: '二阶振荡环节' },
    ...
    answerId: 'C',
    explanation: 'RLC 电路对应二阶系统，具有振荡特性。',
  },
];

interface TransferPrecheckProps extends BaseWidgetProps {}

export default function TransferPrecheck({ onComplete, onStateChange }: TransferPrecheckProps) {
    ...
    <div className="w-full max-w-4xl mx-auto">
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-sm text-slate-500">前测 · 传递函数</p>
