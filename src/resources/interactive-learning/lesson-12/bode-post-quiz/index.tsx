...
const QUIZ_ITEMS: QuizItem[] = [
  {
    ...
    answerId: 'A',
    explanation: '完成转折频率与低频特性后，进入叠加作图阶段。',
  },
  {
    id: 'break-frequency',
    title: '转折频率',
    prompt: '一阶环节 (1 + s/20) 的转折频率为：',
    options: [
      { id: 'A', label: '0.2 rad/s' },
      { id: 'B', label: '2 rad/s' },
      { id: 'C', label: '20 rad/s' },
      { id: 'D', label: '200 rad/s' },
    ],
    answerId: 'C',
    explanation: '标准型 (1 + s/ω) 的转折频率即 ω。',
  },
  {
    id: 'slope-accumulation',
    title: '斜率叠加',
    prompt: '在同一频段内，两个一阶极点叠加后的斜率为：',
    options: [
      { id: 'A', label: '-20 dB/dec' },
      { id: 'B', label: '-40 dB/dec' },
      { id: 'C', label: '+20 dB/dec' },
      { id: 'D', label: '0 dB/dec' },
    ],
    answerId: 'B',
    explanation: '每个一阶极点贡献 -20 dB/dec，总斜率为 -40 dB/dec。',
  },
];

interface BodePostQuizProps extends BaseWidgetProps {}

export default function BodePostQuiz({ onComplete, onStateChange }: BodePostQuizProps) {
    ...
    <div className="w-full max-w-4xl mx-auto">
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-sm text-slate-500">后测 · 伯德图掌握度</p>
