...
const QUIZ_ITEMS: QuizItem[] = [
  {
    id: 'initial-value',
    prompt: '初值定理给出的 f(0+) 表达式是：',
    options: [
      { id: 'A', label: 'lim_{s→0} sF(s)' },
      { id: 'B', label: 'lim_{s→∞} sF(s)' },
      { id: 'C', label: 'lim_{s→∞} F(s)/s' },
      { id: 'D', label: 'lim_{s→0} F(s)/s' },
    ],
    answerId: 'B',
    explanation: '初值定理：f(0+) = lim_{s→∞} sF(s)。',
  },
  {
    id: 'final-value',
    prompt: '终值定理在稳定条件下给出的表达式是：',
    options: [
      { id: 'A', label: 'lim_{s→∞} sF(s)' },
      { id: 'B', label: 'lim_{s→0} sF(s)' },
      { id: 'C', label: 'lim_{s→0} F(s)' },
      { id: 'D', label: 'lim_{s→∞} F(s)' },
    ],
    answerId: 'B',
    explanation: '终值定理：f(∞) = lim_{s→0} sF(s)，需满足稳定条件。',
  },
  {
    id: 'shift',
    prompt: 'L^{-1}{e^{-as}F(s)} 表示：',
    options: [
      { id: 'A', label: 'f(t+a)' },
      { id: 'B', label: 'f(t-a)u(t-a)' },
      { id: 'C', label: 'e^{-at} f(t)' },
      { id: 'D', label: 'f(t) * e^{-at}' },
    ],
    answerId: 'B',
    explanation: '位移定理：e^{-as} 对应时域延迟 f(t-a)u(t-a)。',
  },
];

interface LaplaceExitQuizProps extends BaseWidgetProps {}

export default function LaplaceExitQuiz({ onComplete, onStateChange }: LaplaceExitQuizProps) {
    ...
    <div className="w-full max-w-4xl mx-auto">
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-sm text-slate-500">后测 · 拉氏变换关键定理</p>
