...
const QUIZ_ITEMS: QuizItem[] = [
  {
    id: 'intersection-meaning',
    title: '交点含义',
    prompt: 'G(jω) 与 -1/N(A) 的交点意味着：',
    options: [
      { id: 'A', label: '可能存在自持振荡' },
      { id: 'B', label: '系统必然稳定' },
      { id: 'C', label: '系统必然不稳定' },
    ...
    answerId: 'A',
    explanation: '交点满足 N(A)G(jω)=-1，是自振存在的必要条件。',
  },
  {
    id: 'stability-rule',
    title: '稳定判别',
    prompt: '判断交点自振是否稳定的核心思路是：',
    options: [
      { id: 'A', label: '扰动后是否回到交点' },
      { id: 'B', label: '只看交点数量' },
      { id: 'C', label: '只看负倒描述函数是否在实轴上' },
      { id: 'D', label: '只看相角裕度' },
    ],
    answerId: 'A',
    explanation: '微小扰动分析判断振幅是否回到交点，从而确定稳定性。',
  },
  {
    id: 'solve-parameters',
    title: '参数求解',
    prompt: '自振参数求解通常需要：',
    options: [
      { id: 'A', label: '同时满足幅值方程与相位方程' },
      { id: 'B', label: '只满足幅值方程' },
      { id: 'C', label: '只满足相位方程' },
      { id: 'D', label: '无需计算' },
    ],
    answerId: 'A',
    explanation: 'N(A)G(jω)=-1 包含幅值与相位约束，需要联立求解 A 与 ω。',
  },
];

interface DfExitQuizProps extends BaseWidgetProps {}

export default function DfExitQuiz({ onComplete, onStateChange }: DfExitQuizProps) {
    ...
    <div className="w-full max-w-4xl mx-auto">
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-sm text-slate-500">后测 · 描述函数分析</p>
            <h2 className="text-2xl font-bold text-slate-900">关键判别速测</h2>
          </div>
