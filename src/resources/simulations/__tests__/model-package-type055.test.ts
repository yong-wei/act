...
describe('type055-nanchang-101 v2.1.3 received package integrity', () => {
  it('verifies the complete seven-role denominator, hashes, sizes and manifest identity', () => {
    const receipt = validateReceivedModelPackage(TYPE055_NANCHANG_101_V2_1_3, realIo());
    ...
    expect(receipt.manifestSha256).toBe(TYPE055_NANCHANG_101_V2_1_3.releaseManifestSha256);
  });

  it('registers each role exactly once with unique files including interactive-systems', () => {
    const roleFiles = Object.values(TYPE055_NANCHANG_101_V2_1_3.roles).map((role) => role.file);
    expect(new Set(roleFiles).size).toBe(7);
    expect(Object.keys(TYPE055_NANCHANG_101_V2_1_3.roles).sort()).toEqual(
  ...
  it('rejects a duplicate-role denominator and a missing role', () => {
    const duplicate = {
      ...TYPE055_NANCHANG_101_V2_1_3,
      roles: {
        ...TYPE055_NANCHANG_101_V2_1_3.roles,
        demo: TYPE055_NANCHANG_101_V2_1_3.roles.payload,
      },
    };
    expect(() => validateReceivedModelPackage(duplicate, realIo())).toThrow('incomplete-role-denominator');
    const io = tamperedIo((files) => {
      files.delete(TYPE055_NANCHANG_101_V2_1_3.roles['interactive-systems'].file);
    });
    expect(() => validateReceivedModelPackage(TYPE055_NANCHANG_101_V2_1_3, io)).toThrow('missing-file');
  });
  ...
  it('binds a receive receipt with the same identities as the descriptor on a clean revision', () => {
    ...
    expect(receipt.manifestSha256).toBe(TYPE055_NANCHANG_101_V2_1_3.releaseManifestSha256);
    expect(receipt.sourceBlendSha256).toBe(TYPE055_NANCHANG_101_V2_1_3.sourceBlendSha256);
    ...
    for (const [role, artifact] of Object.entries(TYPE055_NANCHANG_101_V2_1_3.roles)) {
  ...
  it('rejects the whole package when one artifact hash drifts', () => {
    const io = tamperedIo((files) => {
      const entry = files.get(TYPE055_NANCHANG_101_V2_1_3.roles['ship-lod1'].file);
      if (!entry) throw new Error('missing lod1 for tamper test');
      entry.bytes[0] ^= 0xff;
    });
    expect(() => validateReceivedModelPackage(TYPE055_NANCHANG_101_V2_1_3, io)).toThrow('sha-mismatch');
  });

  it('rejects a partially received package (missing file)', () => {
    const io = tamperedIo((files) => {
      files.delete(TYPE055_NANCHANG_101_V2_1_3.roles.collision.file);
    });
    expect(() => validateReceivedModelPackage(TYPE055_NANCHANG_101_V2_1_3, io)).toThrow('missing-file');
  });
  ...
  it('rejects size drift and undeclared files inside the candidate directory', () => {
    ...
    expect(() => validateReceivedModelPackage(TYPE055_NANCHANG_101_V2_1_3, io)).toThrow('undeclared-file');
  });
  ...
  it('rejects when the release manifest itself is replaced', () => {
    ...
    expect(() => validateReceivedModelPackage(TYPE055_NANCHANG_101_V2_1_3, io)).toThrow('manifest-sha-mismatch');
  });
});
...
describe('type055-nanchang-101 v2.1.3 semantic interface contract', () => {
  ...
  it('validates every declared interface across the real six GLBs', () => {
    const glbJson = {
      'ship-lod0': glbJsonOf(TYPE055_NANCHANG_101_V2_1_3.roles['ship-lod0'].file),
      'ship-lod1': glbJsonOf(TYPE055_NANCHANG_101_V2_1_3.roles['ship-lod1'].file),
      'ship-lod2': glbJsonOf(TYPE055_NANCHANG_101_V2_1_3.roles['ship-lod2'].file),
      demo: glbJsonOf(TYPE055_NANCHANG_101_V2_1_3.roles.demo.file),
      payload: glbJsonOf(TYPE055_NANCHANG_101_V2_1_3.roles.payload.file),
    };
    expect(validateModelPackageInterface(TYPE055_NANCHANG_101_V2_1_3, glbJson)).toEqual([]);
  });

  it('detects a wrong ship animation count and an unreachable animation target', () => {
    const ship = glbJsonOf(TYPE055_NANCHANG_101_V2_1_3.roles['ship-lod2'].file);
    const truncated = { ...ship, animations: (ship.animations as unknown[]).slice(0, 8) };
    const violations = validateModelPackageInterface(TYPE055_NANCHANG_101_V2_1_3, { 'ship-lod2': truncated });
    ...
    const unreachable = validateModelPackageInterface(TYPE055_NANCHANG_101_V2_1_3, { 'ship-lod2': redirected });
  ...
  it('resolves runtime bindings by semantic name, rejecting duplicates and indices', () => {
    const ship = glbJsonOf(TYPE055_NANCHANG_101_V2_1_3.roles['ship-lod0'].file) as unknown as {
    ...
    const payload = glbJsonOf(TYPE055_NANCHANG_101_V2_1_3.roles.payload.file) as unknown as {
      animations: { name?: unknown }[];
      nodes: { name?: unknown; extras?: unknown }[];
    };
    const loaded = listLoadedInstanceNames(payload);
    expect(loaded.vls).toHaveLength(TYPE055_NANCHANG_101_V2_1_3.interfaceContract.vlsLoadedCount);
    expect(loaded.hq10).toHaveLength(TYPE055_NANCHANG_101_V2_1_3.interfaceContract.hq10LoadedCount);
...
describe('v2.1.3 declared waterline anchor, propulsors and semantic bindings', () => {
  ...
  it('declares the design waterline and twin propulsors consistent across all LODs', () => {
    expect(TYPE055_NANCHANG_101_V2_1_3.verticalAnchor?.designWaterlineY).toBe(6.6);
    expect(TYPE055_NANCHANG_101_V2_1_3.propulsors?.map((propulsor) => propulsor.node)).toEqual([
      'PROP_PORT',
      'PROP_STARBOARD',
    ]);
    for (const role of ['ship-lod0', 'ship-lod1', 'ship-lod2'] as const) {
      const nodeNames = new Set((glbJsonOf(TYPE055_NANCHANG_101_V2_1_3.roles[role].file).nodes ?? []).map((node) => node.name));
      for (const propulsor of TYPE055_NANCHANG_101_V2_1_3.propulsors ?? []) {
  ...
  it('converts propulsor model-local positions onto the scene +Z-bow basis', () => {
    const anchors = propulsorSceneAnchors(TYPE055_NANCHANG_101_V2_1_3, 180);
  ...
  it('resolves every declared semantic binding against the real ship GLBs', () => {
    const bindings = TYPE055_NANCHANG_101_V2_1_3.semanticBindings ?? [];
    expect(bindings.length).toBeGreaterThan(0);
    for (const role of ['ship-lod0', 'ship-lod1', 'ship-lod2'] as const) {
      const json = glbJsonOf(TYPE055_NANCHANG_101_V2_1_3.roles[role].file);
      ...
      for (const patrol of TYPE055_NANCHANG_101_V2_1_3.easterEgg?.patrolClips ?? []) {
        expect(clipNames.has(patrol.clip), `${role} must contain patrol clip ${patrol.clip}`).toBe(true);
      }
    }
    const demo = glbJsonOf(TYPE055_NANCHANG_101_V2_1_3.roles.demo.file);
    const demoClipNames = new Set((demo.animations ?? []).map((clip) => clip.name));
    for (const clipName of TYPE055_NANCHANG_101_V2_1_3.interfaceContract.demoAnimations) {
  ...
  it('keeps antifouling material below the declared waterline in every LOD', () => {
    ...
    for (const role of ['ship-lod0', 'ship-lod1', 'ship-lod2'] as const) {
      const json = glbJsonOf(TYPE055_NANCHANG_101_V2_1_3.roles[role].file) as { materials?: { name?: string }[] };
...
describe('quality tier to LOD mapping and coordinate basis adapter', () => {
  it('maps high/medium/low uniquely to LOD0/1/2', () => {
    ...
    expect(shipLodUrlForQualityTier(TYPE055_NANCHANG_101_V2_1_3, 'high')).toBe(TYPE055_NANCHANG_101_V2_1_3.roles['ship-lod0'].url);
    expect(shipLodUrlForQualityTier(TYPE055_NANCHANG_101_V2_1_3, 'low')).toBe(TYPE055_NANCHANG_101_V2_1_3.roles['ship-lod2'].url);
  });
...
describe('type055-nanchang-101 v2.2.0 hero activation', () => {
  const packageDir = path.join(process.cwd(), 'public/assets/model-releases/type055-nanchang-101/v2.2.0');

  it('activates 2.2.0 with the declared model-to-scene matrix and no extra waterline', () => {
    expect(TYPE055_NANCHANG_101_V2.modelVersion).toBe('2.2.0');
    expect(TYPE055_NANCHANG_101_V2.baseUrl).toBe('/assets/model-releases/type055-nanchang-101/v2.2.0');
    ...
    expect(TYPE055_NANCHANG_101_V2_1_3.modelVersion).toBe('2.1.3');
  });

  it('verifies on-disk hashes and prefers OSS then the image copy', () => {
    ...
    const receipt = validateReceivedModelPackage(TYPE055_NANCHANG_101_V2, io);
    ...
    expect(oss).toBe(`https://static.adapt-learn.online/assets/${TYPE055_NANCHANG_101_V2.roles['ship-lod0'].sha256}/${TYPE055_NANCHANG_101_V2.roles['ship-lod0'].file}`);
