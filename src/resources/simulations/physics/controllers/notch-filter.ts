...
export function notchFilterStep(
  ...
  if (!state.enabled) {
    return {
      ...
      newState: {
        ...state,
        inputHistory: [input, state.inputHistory[0]],
        outputHistory: [input, state.outputHistory[0]],
        currentGainDb: 0,
      },
    };
  }
  ...
  // 计算滤波器系数
  const { b0, b1, b2, a1, a2 } = computeNotchCoefficients(params, sampleRate);

  // 差分方程
  const output =
    b0 * input +
    b1 * state.inputHistory[0] +
    b2 * state.inputHistory[1] -
    a1 * state.outputHistory[0] -
    a2 * state.outputHistory[1];

  // 计算当前增益 (dB)
  const currentGainDb =
    Math.abs(input) > 1e-9
      ? 20 * Math.log10(Math.abs(output) / Math.abs(input))
      : 0;

  // 更新状态
  const newState: NotchFilterState = {
    inputHistory: [input, state.inputHistory[0]],
    outputHistory: [output, state.outputHistory[0]],
...
export function filterSignal(
  ...
  let state = createNotchFilterState(true);
  const output: number[] = [];

  for (const input of inputSignal) {
    const result = notchFilterStep(input, state, params, sampleRate);
