import rawCaseData from '../../../../course-content/runtime/lessons/4-1/media/generated-data/4-1-case-data.json';
...
type Unit42CaseKey = 'ship_heading' | 'platform_pitch';
...
const CASE_MAP: Record<Unit42CaseId, Unit42CaseKey> = {
  ship: 'ship_heading',
  platform: 'platform_pitch',
};
...
export function getUnit42FallbackResult(caseId: Unit42CaseId): ControlAnalysisResult {
  const payload = (rawCaseData as unknown as { cases: Record<Unit42CaseKey, RawCasePayload> }).cases[CASE_MAP[caseId]];
  ...
  return {
    ...
    isFallback: true,
    fallbackMessage: 'Wasm 计算内核暂不可用，当前显示 4-2 离线基线结果。',
  };
}
