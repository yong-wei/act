...
export function L2DCourseEntryPage({
  initialRole,
  lessonRuntime,
}: {
  ...
  const createClassroom = async () => {
    ...
    try {
      const cloneRes = await fetch('/api/teacher/preset-lessons/clone', {
        ...
        body: JSON.stringify({ presetKey: L2D_PRESET_KEY }),
      });
      ...
      router.push(`/interactive-learning/courses/${L2D_ROUTE_SEGMENT}/teacher/${createData.id}`);
    } catch (requestError) {
    ...
    <div className="premium-lesson-shell">
      <header className="premium-lesson-topbar">
        <div className="mx-auto flex max-w-[1180px] items-center justify-between px-4 py-4 sm:px-6">
          ...
          <div className="text-right">
            <div className="premium-lesson-kicker text-[10px] tracking-[0.24em]">Practice Runtime Course</div>
            <h1 className="premium-lesson-title text-base font-semibold sm:text-lg">{L2D_COURSE_TITLE}</h1>
          </div>
        </div>
      </header>
      ...
      <main className="premium-lesson-main mx-auto max-w-[1180px] px-3 py-4 sm:px-6 sm:py-8">
        <div className="mt-4 grid gap-4 md:grid-cols-3">
          {showTeacherSection ? (
            <section className="premium-lesson-panel p-5">
              ...
              <p className="premium-lesson-muted mt-2">自动克隆 L-2d 预置教案，生成课堂码并进入三域联动实践课堂。</p>
              <button
                type="button"
          ...
          <section className="premium-lesson-panel p-5">
            ...
            <p className="premium-lesson-muted mt-2">以演示模式进入学生端，自由浏览 14 个环节、三面板工作区、任务一/二/三和总结页。</p>
            <Link
              href={`/interactive-learning/courses/${L2D_ROUTE_SEGMENT}/student/demo`}
        ...
        <section className="premium-lesson-panel mt-4 px-5 py-5">
          <div className="premium-lesson-kicker">L-2d · Practice Guide Runtime</div>
          <h2 className="premium-lesson-title mt-2 text-3xl font-semibold sm:text-4xl">实践任务书驱动的三域联动课堂</h2>
          <p className="premium-lesson-muted mt-3 max-w-3xl sm:text-base">{L2D_COURSE_DESCRIPTION}</p>
          <p className="premium-lesson-muted mt-3 max-w-3xl">
            首页讲义入口直接读取 runtime 中由实践任务书导出的 Markdown，因此这里看到的内容就是课堂实际使用的《实践任务书》。
          </p>
        </section>
