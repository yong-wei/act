...
export function useInteractiveTracking(
  ...
  const { resourceId, userId, sessionId, syncInterval = SYNC_INTERVAL, onSync } = options;

  const eventsRef = useRef<InteractiveEvent[]>([]);
  const syncTimerRef = useRef<NodeJS.Timeout | null>(null);
  const debounceTimerRef = useRef<NodeJS.Timeout | null>(null);
  const storageKey = `${STORAGE_KEY_PREFIX}${resourceId}`;
  ...
  }, [resourceId, userId, sessionId, saveToStorage, syncEvents]);
