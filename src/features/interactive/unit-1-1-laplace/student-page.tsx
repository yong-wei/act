...
import { UNIT_1_1CourseHeader } from './course-header';
import {
  UNIT_1_1KnowledgeMapVisual,
  UNIT_1_1StepAiAssistant,
...
import { getUnit11StepAIContext } from '@/lib/course-ai-contexts';
...
export function UNIT_1_1StudentPage({
  ...
  // 当步骤变化时，更新AI上下文
  useEffect(() => {
    const stepContext = getUnit11StepAIContext(step.id);
    if (stepContext) {
      updatePageContext({
  ...
  return (
    <div className="premium-lesson-shell">
      ...
      <main className="premium-lesson-main mx-auto max-w-[1180px] px-3 py-4 sm:px-6 sm:py-6">
        ...
        {step.pageType === 'ai' ? (
          <div className="mt-4">
            <UNIT_1_1StepAiAssistant step={step} onAiEvent={handleAiEvent} />
          </div>
        ) : null}
