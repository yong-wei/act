...
export function L2CCourseEntryPage({
  initialRole,
  lessonRuntime,
}: {
  ...
  const createClassroom = async () => {
    ...
    try {
      const cloneRes = await fetch('/api/teacher/preset-lessons/clone', {
        ...
        body: JSON.stringify({ presetKey: L2C_PRESET_KEY }),
      });
      ...
      router.push(`/interactive-learning/courses/${L2C_ROUTE_SEGMENT}/teacher/${createData.id}`);
    } catch (requestError) {
    ...
    <div className="premium-lesson-shell">
      <header className="premium-lesson-topbar">
        <div className="mx-auto flex max-w-[1180px] items-center justify-between px-4 py-4 sm:px-6">
          ...
          <div className="text-right">
            ...
            <h1 className="premium-lesson-title text-base font-semibold sm:text-lg">{L2C_COURSE_TITLE}</h1>
          </div>
        </div>
      </header>
      ...
      <main className="premium-lesson-main mx-auto max-w-[1180px] px-3 py-4 sm:px-6 sm:py-8">
        <div className="mt-4 grid gap-4 md:grid-cols-3">
          {showTeacherSection ? (
            <section className={entryCardClassName}>
              ...
              <p className="premium-lesson-muted mt-2">自动克隆 L-2c 预置教案，生成课堂码并进入频域直觉精品课堂。</p>
              <button
                type="button"
          ...
          <section className={entryCardClassName}>
            ...
            <p className="premium-lesson-muted mt-2">以演示模式进入学生端，自由浏览全部 17 个环节以及页内 AI、测验和知识卡片。</p>
            <Link
              href={`/interactive-learning/courses/${L2C_ROUTE_SEGMENT}/student/demo`}
        ...
        <section className="premium-lesson-panel mt-4 px-5 py-5">
          <div className="premium-lesson-kicker">L-2c · Frequency Intuition</div>
          <h2 className="premium-lesson-title mt-2 text-3xl font-semibold sm:text-4xl">Bode 图、稳定裕度与三域联动直觉</h2>
          <p className="premium-lesson-muted mt-3 max-w-3xl sm:text-base">{L2C_COURSE_DESCRIPTION}</p>
          <p className="premium-lesson-muted mt-3 max-w-3xl">
            课程首页已接入本课知识点网络、知识卡片预览与讲义入口，帮助教师在开课前快速梳理本课的频域主线。
          </p>
        </section>
