...
export function UNIT_3_7StudentPage({
  sessionId,
  lessonRuntime,
}: {
  sessionId: string;
  ...
  useEffect(() => {
    const stepContext = getUnit37StepAIContext(step.id);
    if (stepContext) {
      updatePageContext({
  ...
  const answerVisible =
    teacherSyncState?.activeStepId === step.id
      ...
      : false;
  const released =
    isDemo || !isUNIT_3_7InteractivePageType(step.pageType)
      ? true
      : teacherSyncState?.activeStepId === step.id
  ...
  const handleSubmitResponse = (response: UNIT_3_7StepResponse) => {
    const isResubmit = Boolean(savedResponse);
    void saveCourseState((prev) => {
      const nextState: UNIT_3_7StudentCourseState = {
        ...prev,
  ...
  return (
    <LessonRuntimeShell
        ...
        onIndexChange={(index) => {
          trackStepLeave(step.id, { nextStepId: UNIT_3_7_LESSON_STEPS[index]?.id });
        ...
        runtimeAttributes={{
          ...
          'data-return-target': `/interactive-learning/courses/${UNIT_3_7_ROUTE_SEGMENT}`,
        ...
        <div className="space-y-4">
          ...
          <div className="premium-lesson-tone-block premium-tone-amber mb-4 flex flex-wrap items-center justify-between gap-3">
            ...
            <button
              type="button"
              onClick={() => {
                trackStepView(UNIT_3_7_LESSON_STEPS[teacherIndex]?.id, {
                  pageType: UNIT_3_7_LESSON_STEPS[teacherIndex]?.pageType,
