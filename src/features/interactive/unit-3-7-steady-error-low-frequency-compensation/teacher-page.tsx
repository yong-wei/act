...
import {
  ...
  UNIT_3_7_RESOURCE_KEY,
  UNIT_3_7_SESSION_ADAPTER,
  UNIT_3_7_STAGE_MAP,
  UNIT_3_7_COURSE_TITLE,
  UNIT_3_7_COURSE_SUBTITLE,
  UNIT_3_7_ROUTE_SEGMENT,
  UNIT_3_7_STAGE_LABEL,
  type UNIT_3_7StudentCourseState,
  type UNIT_3_7TeacherCourseSyncState,
...
export function UNIT_3_7TeacherPage({
  sessionId,
  lessonRuntime,
}: {
  sessionId: string;
  ...
  const interactiveTracking = useInteractiveTracking({
    resourceId: UNIT_3_7_RESOURCE_KEY,
    resourceKey: UNIT_3_7_RESOURCE_KEY,
    sessionId,
  });
  ...
  const {
    ...
    adapter: UNIT_3_7_SESSION_ADAPTER,
  });
  ...
  const step = UNIT_3_7_LESSON_STEPS[activeIndex];

  const teacherSyncState = useMemo(() => {
    const latestRecord = [...teacherStates].reverse().find((record) => isUNIT_3_7TeacherSyncState(record.data));
    return (latestRecord?.data as UNIT_3_7TeacherCourseSyncState | null) ?? null;
  }, [teacherStates]);
  ...
  useEffect(() => {
    if (!shouldPostUNIT_3_7TeacherSync({ loadingSession, teacherViewHydrated })) return;
  ...
  const studentStates = useMemo(() => {
    return courseStates
      .map((record) => {
        ...
        if (data.kind !== 'unit37_student_state' || data.version !== 1) return null;
        return {
          studentName: data.studentName || record.user?.name?.trim() || '未命名学生',
          state: data as UNIT_3_7StudentCourseState,
        };
      })
      .filter(Boolean) as Array<{ studentName: string; state: UNIT_3_7StudentCourseState }>;
  }, [courseStates]);
  ...
  const currentResponses = useMemo(() => {
    return studentStates
      ...
      .filter(Boolean) as Array<{ studentName: string; response: UNIT_3_7StudentCourseState['responses'][string] }>;
  }, [step.id, studentStates]);

  const handlePatchCurrentStep = useCallback(
    async (nextIndex: number) => {
      const nextStep = UNIT_3_7_LESSON_STEPS[nextIndex];
      trackStepLeave(step.id, { nextStepId: nextStep.id });
      await patchCurrentStep(nextIndex, {
        currentItemId: nextStep.id,
        currentStage: UNIT_3_7_STAGE_MAP[nextStep.stage],
      });
  ...
  const handleEndSession = useCallback(async () => {
    ...
    try {
      await finalizeUNIT_3_7TeacherSession({
        finishSession,
  ...
  if (loadingSession) {
    return (
      <LessonRuntimeLoadingShell
        ...
        subtitle={UNIT_3_7_COURSE_SUBTITLE}
  ...
  const teacherSummaryPanel = isDisplayStep ? null : (
    <div className="mt-4">
      <UNIT_3_7TeacherActivitySummary
        step={step}
  ...
  return (
    <LessonRuntimeShell
        ...
        stageLabel={UNIT_3_7_STAGE_LABEL}
        ...
        localTools={
          ...
          <div className="premium-lesson-panel-soft px-4 py-4">
            ...
            currentStepId={step.id}
            orderedStepIds={UNIT_3_7_LESSON_STEPS.map((item) => item.id)}
            title="页面知识卡片"
            inlineTool
          />
          </>
        ...
        runtimeAttributes={{
          ...
          'data-runtime-manifest-truth': lessonRuntime.interactiveManifest?.lessonId ?? UNIT_3_7_LESSON_KEY,
        }}
      >
        ...
        {activityFirst && teacherSummaryPanel ? teacherSummaryPanel : null}
        {contentPanel}
        {!activityFirst && teacherSummaryPanel ? teacherSummaryPanel : null}
        </div>
      </LessonRuntimeShell>
