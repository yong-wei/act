  ...
  const createClassroom = async () => {
    ...
    try {
      const cloneRes = await fetch('/api/teacher/preset-lessons/clone', {
        ...
        body: JSON.stringify({ presetKey: UNIT_3_7_PRESET_KEY }),
      });
      ...
      router.push(`/interactive-learning/courses/${UNIT_3_7_ROUTE_SEGMENT}/teacher/${createData.id}`);
    } catch (requestError) {
      ...
      <header className="premium-lesson-topbar">
        <div className="mx-auto flex max-w-[1180px] items-center justify-between px-4 py-4 sm:px-6">
          ...
          <div className="text-right">
            ...
            <h1 className="premium-lesson-title text-base font-semibold sm:text-lg">{UNIT_3_7_COURSE_TITLE}</h1>
          </div>
        </div>
      </header>
      ...
      <main className="premium-lesson-main mx-auto max-w-[1180px] px-3 py-4 sm:px-6 sm:py-8">
        <div className="mt-4 grid gap-4 md:grid-cols-3">
          {showTeacherSection ? (
            <section className="premium-lesson-panel p-5">
              ...
              <p className="premium-lesson-muted mt-2">
                自动克隆 3-7 预置教案，生成课堂码并进入《型别、积分环节与稳态改善》精品课堂。
              </p>
              <button
                type="button"
          ...
          <section className="premium-lesson-panel p-5">
            ...
            <p className="premium-lesson-muted mt-2">
              以演示模式进入学生端，先预览 {UNIT_3_7_LESSON_STEPS.length} 个课堂环节与 runtime 导学资源。
            </p>
            <Link
              href={`/interactive-learning/courses/${UNIT_3_7_ROUTE_SEGMENT}/student/demo`}
        ...
        <section className="premium-lesson-panel mt-4 px-5 py-5">
          <div className="premium-lesson-kicker">3-7 · Steady Error And Low-Frequency Compensation</div>
          <h2 className="premium-lesson-title mt-2 text-3xl font-semibold sm:text-4xl">{UNIT_3_7_COURSE_TITLE}</h2>
          <p className="premium-lesson-muted mt-3 max-w-3xl sm:text-base">{UNIT_3_7_COURSE_DESCRIPTION}</p>
          <p className="premium-lesson-muted mt-3 max-w-3xl">
            本课承接 <strong>3-6</strong> 已经建立的动态改善语言，把“更快更稳”推进成“为什么还能更准”，
            并为 <strong>3-8</strong> 的统一频域判别语言做铺垫。
          </p>
          <div className="mt-4 flex flex-wrap gap-2 text-xs">
            {['双通道误差', '终值定理', '系统型别', '静态误差系数', 'PI', '滞后校正'].map((item) => (
        ...
        <LessonEntryMediaHub lessonRuntime={lessonRuntime} courseLabel="3-7 · Pre-study" />
