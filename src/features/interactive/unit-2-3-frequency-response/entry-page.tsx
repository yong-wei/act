  ...
  const createClassroom = async () => {
    ...
    try {
      const cloneRes = await fetch('/api/teacher/preset-lessons/clone', {
        ...
        body: JSON.stringify({ presetKey: UNIT_2_3_PRESET_KEY }),
      });
      ...
      router.push(`/interactive-learning/courses/${UNIT_2_3_ROUTE_SEGMENT}/teacher/${createData.id}`);
    } catch (requestError) {
    ...
    <div className="premium-lesson-shell">
      <header className="premium-lesson-topbar">
        <div className="mx-auto flex max-w-[1180px] items-center justify-between px-4 py-4 sm:px-6">
          ...
          <div className="text-right">
            ...
            <h1 className="premium-lesson-title text-base font-semibold sm:text-lg">{UNIT_2_3_COURSE_TITLE}</h1>
          </div>
        </div>
      </header>
      ...
      <main className="premium-lesson-main mx-auto max-w-[1180px] px-3 py-4 sm:px-6 sm:py-8">
        <div className="mt-4 grid gap-4 md:grid-cols-3">
          {showTeacherSection ? (
            <section className="premium-lesson-panel p-5">
              ...
              <p className="premium-lesson-muted mt-2">自动克隆 2-3 预置教案，生成课堂码并进入“频率响应基础与 Bode 图初步”精品课堂。</p>
              <button
                type="button"
          ...
          <section className="premium-lesson-panel p-5">
            ...
            <p className="premium-lesson-muted mt-2">以演示模式进入学生端，先预览 17 个课堂环节与 runtime 导学内容。</p>
            <Link
              href={`/interactive-learning/courses/${UNIT_2_3_ROUTE_SEGMENT}/student/demo`}
        ...
        <section className="premium-lesson-panel mt-4 px-5 py-5">
          <div className="premium-lesson-kicker">2-3 · Frequency Response</div>
          <h2 className="premium-lesson-title mt-2 text-3xl font-semibold sm:text-4xl">{UNIT_2_3_COURSE_TITLE}</h2>
          <p className="premium-lesson-muted mt-3 max-w-3xl sm:text-base">{UNIT_2_3_COURSE_DESCRIPTION}</p>
          <p className="premium-lesson-muted mt-3 max-w-3xl">
            本课承接 <strong>2-2</strong> 已经建立的时间响应语言，把视角转向“系统如何处理不同频率成分”。
            页面先立住频率分量、正弦稳态响应、G(jω) 和 Bode 首轮骨架，再把最关键的位置升级为互动任务。
          </p>
          <div className="mt-4 flex flex-wrap gap-2 text-xs">
            {['频率分量', '正弦稳态响应', 'G(jω)', '幅频特性', '相频特性', 'Bode 图'].map((item) => (
