...
import {
  finalizeUNIT_2_3TeacherSession,
  getUNIT_2_3MediaSrc,
  isUNIT_2_3AiPageType,
  isUNIT_2_3TeacherSyncState,
  resolveUNIT_2_3TeacherSyncDraft,
  shouldPostUNIT_2_3TeacherSync,
  UNIT_2_3_LESSON_KEY,
  UNIT_2_3_LESSON_STEPS,
  UNIT_2_3_RESOURCE_KEY,
  UNIT_2_3_SESSION_ADAPTER,
  UNIT_2_3_STAGE_MAP,
  UNIT_2_3_COURSE_TITLE,
  UNIT_2_3_COURSE_SUBTITLE,
  UNIT_2_3_ROUTE_SEGMENT,
  UNIT_2_3_STAGE_LABEL,
  type UNIT_2_3StudentCourseState,
  type UNIT_2_3TeacherCourseSyncState,
} from '@/lib/unit-2-3-course';
import {
  UNIT_2_3KnowledgeMapVisual,
  UNIT_2_3StepAiAssistant,
  UNIT_2_3StepContentPanel,
  UNIT_2_3TeacherActivitySummary,
} from './step-panels';
import type { WorkspaceParameterChange } from './workspace';

export function UNIT_2_3TeacherPage({
  sessionId,
  lessonRuntime,
}: {
  sessionId: string;
  ...
  const interactiveTracking = useInteractiveTracking({
    resourceId: UNIT_2_3_RESOURCE_KEY,
    resourceKey: UNIT_2_3_RESOURCE_KEY,
    sessionId,
  });
  ...
  const {
    ...
    sessionId,
    steps: UNIT_2_3_LESSON_STEPS,
    adapter: UNIT_2_3_SESSION_ADAPTER,
  });
  ...
  const step = UNIT_2_3_LESSON_STEPS[activeIndex];

  const teacherSyncState = useMemo(() => {
    const latestRecord = [...teacherStates].reverse().find((record) => isUNIT_2_3TeacherSyncState(record.data));
    return (latestRecord?.data as UNIT_2_3TeacherCourseSyncState | null) ?? null;
  }, [teacherStates]);
  ...
  useEffect(() => {
    if (!shouldPostUNIT_2_3TeacherSync({ loadingSession, teacherViewHydrated })) return;
  ...
  const studentStates = useMemo(() => {
    return courseStates
      .map((record) => {
        ...
        if (data.kind !== 'unit23_student_state' || data.version !== 1) return null;
        return {
          studentName: data.studentName || record.user?.name?.trim() || '未命名学生',
          state: data as UNIT_2_3StudentCourseState,
        };
      })
      .filter(Boolean) as Array<{ studentName: string; state: UNIT_2_3StudentCourseState }>;
  }, [courseStates]);
  ...
  const currentResponses = useMemo(() => {
    return studentStates
      ...
      .filter(Boolean) as Array<{ studentName: string; response: UNIT_2_3StudentCourseState['responses'][string] }>;
  }, [step.id, studentStates]);

  const handlePatchCurrentStep = useCallback(
    async (nextIndex: number) => {
      const nextStep = UNIT_2_3_LESSON_STEPS[nextIndex];
      trackStepLeave(step.id, { nextStepId: nextStep.id });
      await patchCurrentStep(nextIndex, {
        currentItemId: nextStep.id,
        currentStage: UNIT_2_3_STAGE_MAP[nextStep.stage],
      });
  ...
  const handleEndSession = useCallback(async () => {
    ...
    try {
      await finalizeUNIT_2_3TeacherSession({
        finishSession,
  ...
  if (loadingSession) {
    return (
      <LessonRuntimeLoadingShell
        ...
        subtitle={UNIT_2_3_COURSE_SUBTITLE}
  ...
  return (
    <LessonRuntimeShell
        ...
        stageLabel={UNIT_2_3_STAGE_LABEL}
        ...
        localTools={
          ...
          <div className="premium-lesson-panel-soft px-4 py-4">
            ...
            currentStepId={step.id}
            orderedStepIds={UNIT_2_3_LESSON_STEPS.map((item) => item.id)}
            title="页面知识卡片"
            inlineTool
          />
          </>
        ...
        runtimeAttributes={{
          ...
          'data-runtime-manifest-truth': lessonRuntime.interactiveManifest?.lessonId ?? UNIT_2_3_LESSON_KEY,
        }}
      >
        <div className="space-y-4">
          {error ? <div className="premium-lesson-tone-block premium-tone-rose mb-4">{error}</div> : null}

        {step.id === 'step-01' ? <UNIT_2_3KnowledgeMapVisual /> : null}

        <UNIT_2_3StepContentPanel
          step={step}
          mediaSrc={getUNIT_2_3MediaSrc(step.id)}
          mediaAlt={step.title}
          ...
          <UNIT_2_3TeacherActivitySummary
            ...
            onToggleRelease={() =>
              setLocalReleasedActivities((prev) => ({
                ...(prev ?? (teacherSyncState as UNIT_2_3TeacherCourseSyncState | null)?.releasedActivities ?? {}),
                [step.id]: !(prev?.[step.id] ?? (teacherSyncState as UNIT_2_3TeacherCourseSyncState | null)?.releasedActivities?.[step.id]),
              }))
            }
            onToggleAnswerVisible={() =>
              setLocalRevealedAnswers((prev) => ({
                ...(prev ?? (teacherSyncState as UNIT_2_3TeacherCourseSyncState | null)?.revealedAnswers ?? {}),
                [step.id]: !(prev?.[step.id] ?? (teacherSyncState as UNIT_2_3TeacherCourseSyncState | null)?.revealedAnswers?.[step.id]),
              }))
            }
          />
        </div>
