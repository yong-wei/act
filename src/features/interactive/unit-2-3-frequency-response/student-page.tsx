...
import { getUnit23StepAIContext } from '@/lib/course-ai-contexts';
import {
  UNIT_2_3KnowledgeMapVisual,
  UNIT_2_3StepAiAssistant,
  UNIT_2_3StepContentPanel,
  UNIT_2_3StudentActivityForm,
  UNIT_2_3StudentSummaryPanel,
} from './step-panels';
import type { WorkspaceParameterChange } from './workspace';

export function UNIT_2_3StudentPage({
  sessionId,
  lessonRuntime,
}: {
  sessionId: string;
  ...
  useEffect(() => {
    const stepContext = getUnit23StepAIContext(step.id);
    if (stepContext) {
      updatePageContext({
  ...
  const answerVisible =
    teacherSyncState?.activeStepId === step.id
      ...
      : false;
  const released =
    isDemo || !isUNIT_2_3InteractivePageType(step.pageType)
      ? true
      : teacherSyncState?.activeStepId === step.id
  ...
  const handleSubmitResponse = (response: UNIT_2_3StepResponse) => {
    const isResubmit = Boolean(savedResponse);
    void saveCourseState((prev) => {
      const nextState: UNIT_2_3StudentCourseState = {
        ...prev,
  ...
  return (
    <LessonRuntimeShell
        ...
        onIndexChange={(index) => {
          trackStepLeave(step.id, { nextStepId: UNIT_2_3_LESSON_STEPS[index]?.id });
        ...
        runtimeAttributes={{
          ...
          'data-return-target': `/interactive-learning/courses/${UNIT_2_3_ROUTE_SEGMENT}`,
      ...
      >
        <div className="space-y-4">
          ...
          <div className="premium-lesson-tone-block premium-tone-amber mb-4 flex flex-wrap items-center justify-between gap-3">
            ...
            <button
              type="button"
              onClick={() => {
                trackStepView(UNIT_2_3_LESSON_STEPS[teacherIndex]?.id, {
                  pageType: UNIT_2_3_LESSON_STEPS[teacherIndex]?.pageType,
        ...
        {step.pageType === 'summary' ? (
          <div className="mt-4">
            <UNIT_2_3StudentSummaryPanel responses={courseState.responses} />
          </div>
        ) : null}
        </div>
      </LessonRuntimeShell>
