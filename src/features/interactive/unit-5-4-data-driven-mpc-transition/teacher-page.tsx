...
import {
  finalizeUNIT_5_4TeacherSession,
  isUNIT_5_4StudentState,
  isUNIT_5_4TeacherSyncState,
  resolveUNIT_5_4TeacherSyncDraft,
  shouldPostUNIT_5_4TeacherSync,
  UNIT_5_4_LESSON_KEY,
...
const UNIT_5_4_MISCONCEPTION_LABELS: Record<string, string> = {
...
function normalizeAnswerList(value: string, comparison: Unit54ObjectiveCard['comparison']) {
  const items = value
    .split(/[|/,，、]/)
...
function collectObjectiveCards(manifest: InteractiveRuntimeManifest | null | undefined): Unit54ObjectiveCard[] {
  if (!manifest) return [];

  const cards: Unit54ObjectiveCard[] = [];
...
function isObjectiveAnswerCorrect(item: Unit54ObjectiveCard, value: string) {
...
function misconceptionLabel(tag: string) {
  return UNIT_5_4_MISCONCEPTION_LABELS[tag] ?? tag;
}
...
export function UNIT_5_4TeacherPage({
  sessionId,
  lessonRuntime,
}: {
  sessionId: string;
  ...
  const teacherSyncState = useMemo(() => {
    const latestRecord = [...teacherStates].reverse().find((record) => isUNIT_5_4TeacherSyncState(record.data));
    return (latestRecord?.data as UNIT_5_4TeacherCourseSyncState | null) ?? null;
  }, [teacherStates]);
  ...
  useEffect(() => {
    if (!shouldPostUNIT_5_4TeacherSync({ loadingSession, teacherViewHydrated })) return;
    const payload = {
  ...
  const studentStates = useMemo(() => {
    return courseStates
      .map((record) => {
        if (!isUNIT_5_4StudentState(record.data)) return null;
        return {
          studentName: record.data.studentName || record.user?.name?.trim() || '未命名学生',
          state: record.data,
        };
      })
      .filter(Boolean) as Array<{ studentName: string; state: UNIT_5_4StudentCourseState }>;
  }, [courseStates]);
  ...
  const figureCoverage = useMemo(() => {
    if (!studentStates.length) return 0;
    const withParameters = studentStates.filter((item) =>
      Object.values(item.state.responses).some((response) => Boolean(response.answers.__figure_parameters)),
    ).length;
  ...
  const currentResponses = useMemo(() => {
    return studentStates
      ...
      .filter(Boolean) as Array<{ studentName: string; response: UNIT_5_4StudentCourseState['responses'][string] }>;
  ...
  const handlePatchCurrentStep = useCallback(
    async (nextIndex: number) => {
      const nextStep = UNIT_5_4_LESSON_STEPS[nextIndex];
      await patchCurrentStep(nextIndex, {
        currentItemId: nextStep.id,
        currentStage: UNIT_5_4_STAGE_MAP[nextStep.stage],
      });
    },
    [patchCurrentStep],
  );
  ...
  const handleEndSession = useCallback(async () => {
    ...
    try {
      await finalizeUNIT_5_4TeacherSession({
        finishSession,
