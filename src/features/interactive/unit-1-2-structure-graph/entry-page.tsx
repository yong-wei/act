  ...
  const createClassroom = async () => {
    ...
    try {
      const cloneRes = await fetch('/api/teacher/preset-lessons/clone', {
        ...
        body: JSON.stringify({ presetKey: UNIT_1_2_PRESET_KEY }),
      });
      ...
      router.push(`/interactive-learning/courses/${UNIT_1_2_ROUTE_SEGMENT}/teacher/${createData.id}`);
    } catch (requestError) {
      ...
      <header className="premium-lesson-topbar">
        <div className="mx-auto flex max-w-[1180px] items-center justify-between px-4 py-4 sm:px-6">
          ...
          <div className="text-right">
            ...
            <h1 className="premium-lesson-title text-base font-semibold sm:text-lg">{UNIT_1_2_COURSE_TITLE}</h1>
          </div>
        </div>
      </header>
      ...
      <main className="premium-lesson-main mx-auto max-w-[1180px] px-3 py-4 sm:px-6 sm:py-8">
        <div className="mt-4 grid gap-4 md:grid-cols-3">
          {showTeacherSection ? (
            <section className="premium-lesson-panel p-5">
              ...
              <p className="premium-lesson-muted mt-2">
                自动克隆 1-2 预置教案，生成课堂码并进入“结构图与化简”精品课堂。
              </p>
              <button
                type="button"
          ...
          <section className="premium-lesson-panel p-5">
            ...
            <Link
              href={`/interactive-learning/courses/${UNIT_1_2_ROUTE_SEGMENT}/student/demo`}
        ...
        <section className="premium-lesson-panel mt-4 px-5 py-5">
          <div className="premium-lesson-kicker">1-2 · Block Diagram &amp; Mason</div>
          <h2 className="premium-lesson-title mt-2 text-3xl font-semibold sm:text-4xl">{UNIT_1_2_COURSE_TITLE}</h2>
          <p className="premium-lesson-muted mt-3 max-w-3xl sm:text-base">{UNIT_1_2_COURSE_DESCRIPTION}</p>
          <p className="premium-lesson-muted mt-3 max-w-3xl">
            本课位于 <strong>1-1</strong> 与 <strong>1-3</strong> 之间：上一课先学会提取单个环节的传递函数，这一课学习如何把积木块组装成完整系统蓝图，下一课再进入闭环系统的时域响应分析。
          </p>
          <div className="mt-4 flex flex-wrap gap-2 text-xs">
            {['四元素', '三连接', '六规则', '代数化简', '梅森公式'].map((item) => (
