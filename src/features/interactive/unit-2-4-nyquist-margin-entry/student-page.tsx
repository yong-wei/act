...
export function UNIT_2_4StudentPage({
  ...
  useEffect(() => {
    const stepContext = getUnit24StepAIContext(step.id);
    if (stepContext) {
      updatePageContext({
  ...
  const answerVisible =
    teacherSyncState?.activeStepId === step.id
      ...
      : false;
  const released =
    isDemo || !isUNIT_2_4InteractivePageType(step.pageType)
      ? true
      : teacherSyncState?.activeStepId === step.id
  ...
  const handleSubmitResponse = (response: UNIT_2_4StepResponse) => {
    ...
    void saveCourseState((prev) => {
      const nextState: UNIT_2_4StudentCourseState = {
        ...prev,
  ...
  if (loadingSession) {
    return (
      <LessonRuntimeLoadingShell
        ...
        subtitle={UNIT_2_4_COURSE_SUBTITLE}
  ...
  return (
    <LessonRuntimeShell
        ...
        steps={UNIT_2_4_LESSON_STEPS}
        activeIndex={activeIndex}
        stageLabel={UNIT_2_4_STAGE_LABEL}
        notice={isOutOfSync ? `当前页面与教师不同步，教师正在第 ${teacherIndex + 1} 页` : step.hint}
        onIndexChange={(index) => {
          trackStepLeave(step.id, { nextStepId: UNIT_2_4_LESSON_STEPS[index]?.id });
        ...
        runtimeAttributes={{
          ...
          'data-return-target': `/interactive-learning/courses/${UNIT_2_4_ROUTE_SEGMENT}`,
          'data-runtime-manifest-truth': lessonRuntime.interactiveManifest?.lessonId ?? UNIT_2_4_LESSON_KEY,
      ...
      >
        <div className="space-y-4">
          ...
          <div className="premium-lesson-tone-block premium-tone-amber mb-4 flex flex-wrap items-center justify-between gap-3">
            ...
            <button
              type="button"
              onClick={() => {
                trackStepView(UNIT_2_4_LESSON_STEPS[teacherIndex]?.id, {
                  pageType: UNIT_2_4_LESSON_STEPS[teacherIndex]?.pageType,
        ...
        {step.pageType === 'summary' ? (
          <div className="mt-4">
            <UNIT_2_4StudentSummaryPanel responses={courseState.responses} />
          </div>
        ) : null}
        </div>
      </LessonRuntimeShell>
