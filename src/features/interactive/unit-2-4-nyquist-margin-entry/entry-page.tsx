  ...
  const createClassroom = async () => {
    ...
    try {
      const cloneRes = await fetch('/api/teacher/preset-lessons/clone', {
        ...
        body: JSON.stringify({ presetKey: UNIT_2_4_PRESET_KEY }),
      });
      ...
      router.push(`/interactive-learning/courses/${UNIT_2_4_ROUTE_SEGMENT}/teacher/${createData.id}`);
    } catch (requestError) {
    ...
    <div className="premium-lesson-shell">
      <header className="premium-lesson-topbar">
        <div className="mx-auto flex max-w-[1180px] items-center justify-between px-4 py-4 sm:px-6">
          ...
          <div className="text-right">
            ...
            <h1 className="premium-lesson-title text-base font-semibold sm:text-lg">{UNIT_2_4_COURSE_TITLE}</h1>
          </div>
        </div>
      </header>
      ...
      <main className="premium-lesson-main mx-auto max-w-[1180px] px-3 py-4 sm:px-6 sm:py-8">
        <div className="mt-4 grid gap-4 md:grid-cols-3">
          {showTeacherSection ? (
            <section className="premium-lesson-panel p-5">
              ...
              <p className="premium-lesson-muted mt-2">自动克隆 2-4 预置教案，生成课堂码并进入“Nyquist 图与频域指标入口”精品课堂。</p>
              <button
                type="button"
          ...
          <section className="premium-lesson-panel p-5">
            ...
            <p className="premium-lesson-muted mt-2">以演示模式进入学生端，先预览 17 个课堂环节与导学资料。</p>
            <Link
              href={`/interactive-learning/courses/${UNIT_2_4_ROUTE_SEGMENT}/student/demo`}
        ...
        <section className="premium-lesson-panel mt-4 px-5 py-5">
          <div className="premium-lesson-kicker">2-4 · Nyquist & Margins</div>
          <h2 className="premium-lesson-title mt-2 text-3xl font-semibold sm:text-4xl">{UNIT_2_4_COURSE_TITLE}</h2>
          <p className="premium-lesson-muted mt-3 max-w-3xl sm:text-base">{UNIT_2_4_COURSE_DESCRIPTION}</p>
          <p className="premium-lesson-muted mt-3 max-w-3xl">
            本课承接 <strong>2-3</strong> 已经建立的 Bode 骨架语言，把同一个 <strong>G(jω)</strong> 继续收束为 Nyquist 轨迹、
            频域指标锚点、手工绘图入口与基础反向识别。
          </p>
          <div className="mt-4 flex flex-wrap gap-2 text-xs">
            {['Nyquist 图', '双图表达', '相位裕度', '增益裕度', '手工绘图入口', '反向识别'].map((item) => (
