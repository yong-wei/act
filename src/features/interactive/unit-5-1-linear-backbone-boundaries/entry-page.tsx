  ...
  const createClassroom = async () => {
    ...
    try {
      const cloneRes = await fetch('/api/teacher/preset-lessons/clone', {
        ...
        body: JSON.stringify({ presetKey: UNIT_5_1_PRESET_KEY }),
      });
      ...
      router.push(`/interactive-learning/courses/${UNIT_5_1_ROUTE_SEGMENT}/teacher/${createData.id}`);
    } catch (requestError) {
    ...
    <div className="premium-lesson-shell">
      <header className="premium-lesson-topbar">
        <div className="mx-auto flex max-w-[1180px] items-center justify-between px-4 py-4 sm:px-6">
          ...
          <div className="text-right">
            ...
            <h1 className="premium-lesson-title text-base font-semibold sm:text-lg">{UNIT_5_1_COURSE_TITLE}</h1>
          </div>
        </div>
      </header>
      ...
      <main className="premium-lesson-main mx-auto max-w-[1180px] px-3 py-4 sm:px-6 sm:py-8">
        <div className="mt-4 grid gap-4 md:grid-cols-3">
          {showTeacherSection ? (
            <section className="premium-lesson-panel p-5">
              ...
              <p className="premium-lesson-muted mt-2">自动克隆 5-1 预置教案，生成课堂码并进入线性主干边界识别课堂。</p>
          ...
          <section className="premium-lesson-panel p-5">
            ...
            <p className="premium-lesson-muted mt-2">以演示模式进入学生端，预览 14 个课堂环节与导学资料。</p>
            <Link href={`/interactive-learning/courses/${UNIT_5_1_ROUTE_SEGMENT}/student/demo`} className="premium-lesson-action-secondary mt-5 flex w-full">
              进入演示模式
            </Link>
          </section>
        ...
        <section className="premium-lesson-panel mt-4 px-5 py-5">
          <div className="premium-lesson-kicker">5-1 · Linear Backbone Boundaries</div>
          <h2 className="premium-lesson-title mt-2 text-3xl font-semibold sm:text-4xl">{UNIT_5_1_COURSE_TITLE}</h2>
          <p className="premium-lesson-muted mt-3 max-w-3xl sm:text-base">{UNIT_5_1_COURSE_DESCRIPTION}</p>
          <div className="mt-4 flex flex-wrap gap-2 text-xs">
            {['线性边界', '饱和', '死区', '滞回', '局部线性化'].map((item) => (
              <span key={item} className="premium-lesson-tone-pill premium-tone-cyan px-3 py-1">{item}</span>
            ))}
          </div>
        </section>

        <LessonEntryMediaHub lessonRuntime={lessonRuntime} courseLabel="5-1" />
