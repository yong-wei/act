...
export function UNIT_3_4StudentPage({
  sessionId,
  lessonRuntime,
}: {
  sessionId: string;
  ...
  useEffect(() => {
    const stepContext = getUnit34StepAIContext(step.id);
    if (stepContext) {
      updatePageContext({
  ...
  const handleSubmitResponse = (response: UNIT_3_4StepResponse) => {
    const isResubmit = Boolean(savedResponse);
    void saveCourseState((prev) => {
      const nextState: UNIT_3_4StudentCourseState = {
        ...prev,
  ...
  return (
    <LessonRuntimeShell
        ...
        onIndexChange={(index) => {
          trackStepLeave(step.id, { nextStepId: UNIT_3_4_LESSON_STEPS[index]?.id });
        ...
        runtimeAttributes={{
          ...
          'data-return-target': `/interactive-learning/courses/${UNIT_3_4_ROUTE_SEGMENT}`,
      ...
      >
        <div className="space-y-4">
          ...
          <div className="premium-lesson-tone-block premium-tone-amber mb-4 flex flex-wrap items-center justify-between gap-3">
            ...
            <button
              type="button"
              onClick={() => {
                trackStepView(UNIT_3_4_LESSON_STEPS[teacherIndex]?.id, {
                  pageType: UNIT_3_4_LESSON_STEPS[teacherIndex]?.pageType,
        ...
        {step.pageType === 'summary' ? (
          <div className="mt-4">
            <UNIT_3_4StudentSummaryPanel responses={courseState.responses} />
          </div>
        ) : null}
        </div>
      </LessonRuntimeShell>
