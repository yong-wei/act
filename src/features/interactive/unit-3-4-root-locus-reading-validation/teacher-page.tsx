...
import {
  ...
  UNIT_3_4_RESOURCE_KEY,
  UNIT_3_4_SESSION_ADAPTER,
  UNIT_3_4_STAGE_MAP,
  UNIT_3_4_COURSE_TITLE,
  UNIT_3_4_COURSE_SUBTITLE,
  UNIT_3_4_ROUTE_SEGMENT,
  UNIT_3_4_STAGE_LABEL,
  type UNIT_3_4StudentCourseState,
  type UNIT_3_4TeacherCourseSyncState,
...
export function UNIT_3_4TeacherPage({
  sessionId,
  lessonRuntime,
}: {
  sessionId: string;
  ...
  useEffect(() => {
    if (!shouldPostUNIT_3_4TeacherSync({ loadingSession, teacherViewHydrated })) return;
  ...
  const studentStates = useMemo(() => {
    return courseStates
      .map((record) => {
        ...
        if (data.kind !== 'unit34_student_state' || data.version !== 1) return null;
        return {
          studentName: data.studentName || record.user?.name?.trim() || '未命名学生',
          state: data as UNIT_3_4StudentCourseState,
        };
      })
      .filter(Boolean) as Array<{ studentName: string; state: UNIT_3_4StudentCourseState }>;
  }, [courseStates]);
  ...
  const currentResponses = useMemo(() => {
    return studentStates
      ...
      .filter(Boolean) as Array<{ studentName: string; response: UNIT_3_4StudentCourseState['responses'][string] }>;
  }, [step.id, studentStates]);

  const handlePatchCurrentStep = useCallback(
    async (nextIndex: number) => {
      const nextStep = UNIT_3_4_LESSON_STEPS[nextIndex];
      trackStepLeave(step.id, { nextStepId: nextStep.id });
      await patchCurrentStep(nextIndex, {
        currentItemId: nextStep.id,
        currentStage: UNIT_3_4_STAGE_MAP[nextStep.stage],
      });
  ...
  const handleEndSession = useCallback(async () => {
    ...
    try {
      await finalizeUNIT_3_4TeacherSession({
        finishSession,
  ...
  if (loadingSession) {
    return (
      <LessonRuntimeLoadingShell
        ...
        subtitle={UNIT_3_4_COURSE_SUBTITLE}
  ...
  return (
    <LessonRuntimeShell
        ...
        stageLabel={UNIT_3_4_STAGE_LABEL}
        ...
        localTools={
          ...
          <div className="premium-lesson-panel-soft px-4 py-4">
            ...
            currentStepId={step.id}
            orderedStepIds={UNIT_3_4_LESSON_STEPS.map((item) => item.id)}
            title="页面知识卡片"
            inlineTool
          />
          </>
        ...
        runtimeAttributes={{
          ...
          'data-runtime-manifest-truth': lessonRuntime.interactiveManifest?.lessonId ?? UNIT_3_4_LESSON_KEY,
        }}
      >
        <div className="space-y-4">
          {error ? <div className="premium-lesson-tone-block premium-tone-rose mb-4">{error}</div> : null}

        {step.id === 'step-01' ? <UNIT_3_4KnowledgeMapVisual /> : null}
          ...
          <UNIT_3_4TeacherActivitySummary
            ...
            onToggleAnswerVisible={() =>
              setLocalRevealedAnswers((prev) => ({
                ...(prev ?? (teacherSyncState as UNIT_3_4TeacherCourseSyncState | null)?.revealedAnswers ?? {}),
                [step.id]: !(prev?.[step.id] ?? (teacherSyncState as UNIT_3_4TeacherCourseSyncState | null)?.revealedAnswers?.[step.id]),
              }))
            }
          />
        </div>
