...
export function UNIT_3_1CourseEntryPage({
  initialRole,
  lessonRuntime,
}: {
  ...
  const createClassroom = async () => {
    ...
    try {
      const cloneRes = await fetch('/api/teacher/preset-lessons/clone', {
        ...
        body: JSON.stringify({ presetKey: UNIT_3_1_PRESET_KEY }),
      });
      ...
      router.push(`/interactive-learning/courses/${UNIT_3_1_ROUTE_SEGMENT}/teacher/${createData.id}`);
    } catch (requestError) {
    ...
    <div className="premium-lesson-shell">
      <header className="premium-lesson-topbar">
        <div className="mx-auto flex max-w-[1180px] items-center justify-between px-4 py-4 sm:px-6">
          ...
          <div className="text-right">
            ...
            <h1 className="premium-lesson-title text-base font-semibold sm:text-lg">{UNIT_3_1_COURSE_TITLE}</h1>
          </div>
        </div>
      </header>
      ...
      <main className="premium-lesson-main mx-auto max-w-[1180px] px-3 py-4 sm:px-6 sm:py-8">
        <div className="mt-4 grid gap-4 md:grid-cols-3">
          {showTeacherSection ? (
            <section className="premium-lesson-panel p-5">
              ...
              <p className="premium-lesson-muted mt-2">自动克隆 3-1 预置教案，生成课堂码并进入“纯极点视角下的稳定、模态与双域近似”精品课堂。</p>
              <button
                type="button"
          ...
          <section className="premium-lesson-panel p-5">
            ...
            <p className="premium-lesson-muted mt-2">以演示模式进入学生端，先预览 15 个课堂环节与 runtime 导学资源。</p>
            <Link
              href={`/interactive-learning/courses/${UNIT_3_1_ROUTE_SEGMENT}/student/demo`}
        ...
        <section className="premium-lesson-panel mt-4 px-5 py-5">
          <div className="premium-lesson-kicker">3-1 · Pure Poles & Modes</div>
          <h2 className="premium-lesson-title mt-2 text-3xl font-semibold sm:text-4xl">{UNIT_3_1_COURSE_TITLE}</h2>
          <p className="premium-lesson-muted mt-3 max-w-3xl sm:text-base">{UNIT_3_1_COURSE_DESCRIPTION}</p>
          <p className="premium-lesson-muted mt-3 max-w-3xl">
            本课作为模块 3 的入口页，先把系统从“对象语言”推进到“机理语言”，围绕稳定底线、模态展开、主导极点近似与
            Bode 证据完成第一轮结构化判断。
          </p>
          <div className="mt-4 flex flex-wrap gap-2 text-xs">
            {['稳定底线', '模态语言', '主导极点', '附加模态', 'Bode 证据', '卷积激发'].map((item) => (
        ...
        <LessonEntryMediaHub lessonRuntime={lessonRuntime} courseLabel="3-1 · Pre-study" />
