...
export function UNIT_3_6CourseHeader({
  steps,
  activeIndex,
  onIndexChange,
  middleNotice,
  ...
  steps: readonly UNIT_3_6StepDefinition[];
  activeIndex: number;
      ...
      <div className="mx-auto max-w-[1280px] px-3 py-2 sm:px-4 sm:py-2.5">
        <div className="flex items-center gap-2 sm:gap-3">
          <div className="flex min-w-0 flex-1 items-center gap-3">
            <Link href={`/interactive-learning/courses/${UNIT_3_6_ROUTE_SEGMENT}`} className="premium-lesson-nav-button shrink-0">
              返回
            </Link>
            <div className="min-w-0">
              <div className="premium-lesson-kicker truncate text-[10px] tracking-[0.24em]">{UNIT_3_6_COURSE_SUBTITLE}</div>
              <h1 className="premium-lesson-title truncate text-sm font-semibold md:text-base">{UNIT_3_6_COURSE_TITLE}</h1>
            </div>
          </div>
          ...
          <div className="hidden min-w-0 flex-1 md:block">
            <div className="flex items-center justify-center gap-2">
              <span className="premium-lesson-tone-pill premium-tone-cyan px-3 py-0.5 text-[11px]">
                {UNIT_3_6_STAGE_LABEL[currentStep.stage]}
              </span>
