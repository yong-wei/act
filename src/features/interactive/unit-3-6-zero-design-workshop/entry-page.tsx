...
export function UNIT_3_6CourseEntryPage({
  initialRole,
  lessonRuntime,
}: {
  ...
  const createClassroom = async () => {
    ...
    try {
      const cloneRes = await fetch('/api/teacher/preset-lessons/clone', {
        ...
        body: JSON.stringify({ presetKey: UNIT_3_6_PRESET_KEY }),
      });
      ...
      router.push(`/interactive-learning/courses/${UNIT_3_6_ROUTE_SEGMENT}/teacher/${createData.id}`);
    } catch (requestError) {
      ...
      <header className="premium-lesson-topbar">
        <div className="mx-auto flex max-w-[1180px] items-center justify-between px-4 py-4 sm:px-6">
          ...
          <div className="text-right">
            ...
            <h1 className="premium-lesson-title text-base font-semibold sm:text-lg">{UNIT_3_6_COURSE_TITLE}</h1>
          </div>
        </div>
      </header>
      ...
      <main className="premium-lesson-main mx-auto max-w-[1180px] px-3 py-4 sm:px-6 sm:py-8">
        <div className="mt-4 grid gap-4 md:grid-cols-3">
          {showTeacherSection ? (
            <section className="premium-lesson-panel p-5">
              ...
              <p className="premium-lesson-muted mt-2">
                自动克隆 3-6 预置教案，生成课堂码并进入“从性能目标到校正设计”的教师课堂台。
              </p>
              <button
                type="button"
          ...
          <section className="premium-lesson-panel p-5">
            ...
            <p className="premium-lesson-muted mt-2">
              以演示模式进入学生端，先预览 {UNIT_3_6_LESSON_STEPS.length} 个课堂环节与 runtime 导学资源。
            </p>
            <Link
              href={`/interactive-learning/courses/${UNIT_3_6_ROUTE_SEGMENT}/student/demo`}
        ...
        <section className="premium-lesson-panel mt-4 px-5 py-5">
          <div className="premium-lesson-kicker">3-6 · Zero Design Workshop</div>
          <h2 className="premium-lesson-title mt-2 text-3xl font-semibold sm:text-4xl">{UNIT_3_6_COURSE_TITLE}</h2>
          <p className="premium-lesson-muted mt-3 max-w-3xl sm:text-base">{UNIT_3_6_COURSE_DESCRIPTION}</p>
          <p className="premium-lesson-muted mt-3 max-w-3xl">
            本课承接 <strong>3-5</strong> 的零点机理课，把关注点正式推进到“目标如何驱动结构和参数选择”，
            并为 <strong>3-7</strong> 的稳态改善主线准备入口判断语言。
          </p>
          <div className="mt-4 flex flex-wrap gap-2 text-xs">
            {['目标分类', '时域指标翻译', 'PD 设计', '测速反馈', '超前校正', '非最小相边界'].map((item) => (
        ...
        <LessonEntryMediaHub lessonRuntime={lessonRuntime} courseLabel="3-6 · Pre-study" />
