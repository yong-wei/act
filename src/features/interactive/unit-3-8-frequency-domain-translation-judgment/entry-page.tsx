...
export function UNIT_3_8CourseEntryPage({
  initialRole,
  lessonRuntime,
}: {
  ...
  const createClassroom = async () => {
    ...
    try {
      const cloneRes = await fetch('/api/teacher/preset-lessons/clone', {
        ...
        body: JSON.stringify({ presetKey: UNIT_3_8_PRESET_KEY }),
      });
      ...
      router.push(`/interactive-learning/courses/${UNIT_3_8_ROUTE_SEGMENT}/teacher/${createData.id}`);
    } catch (requestError) {
      ...
      <header className="premium-lesson-topbar">
        <div className="mx-auto flex max-w-[1180px] items-center justify-between px-4 py-4 sm:px-6">
          ...
          <div className="text-right">
            ...
            <h1 className="premium-lesson-title text-base font-semibold sm:text-lg">{UNIT_3_8_COURSE_TITLE}</h1>
          </div>
        </div>
      </header>
      ...
      <main className="premium-lesson-main mx-auto max-w-[1180px] px-3 py-4 sm:px-6 sm:py-8">
        <div className="mt-4 grid gap-4 md:grid-cols-3">
          {showTeacherSection ? (
            <section className="premium-lesson-panel p-5">
              ...
              <p className="premium-lesson-muted mt-2">
                自动克隆 3-8 预置教案，生成课堂码并进入《频域判别与跨域综合语言》精品课堂。
              </p>
              <button
                type="button"
          ...
          <section className="premium-lesson-panel p-5">
            ...
            <p className="premium-lesson-muted mt-2">
              以演示模式进入学生端，先预览 {UNIT_3_8_LESSON_STEPS.length} 个课堂环节与 runtime 导学资源。
            </p>
            <Link
              href={`/interactive-learning/courses/${UNIT_3_8_ROUTE_SEGMENT}/student/demo`}
        ...
        <section className="premium-lesson-panel mt-4 px-5 py-5">
          <div className="premium-lesson-kicker">3-8 · Frequency-Domain Translation And Judgment</div>
          <h2 className="premium-lesson-title mt-2 text-3xl font-semibold sm:text-4xl">{UNIT_3_8_COURSE_TITLE}</h2>
          <p className="premium-lesson-muted mt-3 max-w-3xl sm:text-base">{UNIT_3_8_COURSE_DESCRIPTION}</p>
          <p className="premium-lesson-muted mt-3 max-w-3xl">
            本课承接 <strong>3-5</strong> 的结构变化线和 <strong>3-7</strong> 的稳态改善线，把它们统一翻译为
            <strong>频域判断地图</strong>，再把这张地图接到 <strong>3-9</strong> 综合映射实验与 <strong>4-1</strong> 设计入口。
          </p>
          <div className="mt-4 flex flex-wrap gap-2 text-xs">
            {['频域指纹', 'Nyquist', 'Bode', '三频段分工', '中频超前', '工程读回'].map((item) => (
        ...
        <div className="mt-4 grid gap-4">
          <LessonEntryMediaHub lessonRuntime={lessonRuntime} courseLabel="3-8 · Pre-study" />
