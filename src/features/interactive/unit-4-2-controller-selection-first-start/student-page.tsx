...
import {
  ...
  UNIT_4_2StudentSummaryPanel,
} from './step-panels';
import type { WorkspaceParameterChange } from './workspace';

export function UNIT_4_2StudentPage({
  sessionId,
  lessonRuntime,
}: {
  sessionId: string;
  ...
  useEffect(() => {
    const stepContext = getUnit42StepAIContext(step.id);
    if (stepContext) {
      updatePageContext({
  ...
  const handleSubmitResponse = (response: UNIT_4_2StepResponse) => {
    const isResubmit = Boolean(savedResponse);
    void saveCourseState((prev) => {
      const nextState: UNIT_4_2StudentCourseState = {
        ...prev,
  ...
  return (
    <LessonRuntimeShell
        ...
        onIndexChange={(index) => {
          trackStepLeave(step.id, { nextStepId: UNIT_4_2_LESSON_STEPS[index]?.id });
        ...
        runtimeAttributes={{
          ...
          'data-return-target': `/interactive-learning/courses/${UNIT_4_2_ROUTE_SEGMENT}`,
        ...
        <div className="space-y-4">
          ...
          <div className="premium-lesson-tone-block premium-tone-amber mb-4 flex flex-wrap items-center justify-between gap-3">
            ...
            <button
              type="button"
              onClick={() => {
                trackStepView(UNIT_4_2_LESSON_STEPS[teacherIndex]?.id, {
                  pageType: UNIT_4_2_LESSON_STEPS[teacherIndex]?.pageType,
