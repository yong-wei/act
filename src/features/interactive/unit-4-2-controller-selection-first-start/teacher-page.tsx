...
import {
  finalizeUNIT_4_2TeacherSession,
  getUNIT_4_2MediaSrc,
  isUNIT_4_2AiPageType,
  isUNIT_4_2TeacherSyncState,
  resolveUNIT_4_2TeacherSyncDraft,
  shouldPostUNIT_4_2TeacherSync,
  UNIT_4_2_LESSON_KEY,
  UNIT_4_2_LESSON_STEPS,
  UNIT_4_2_RESOURCE_KEY,
  UNIT_4_2_SESSION_ADAPTER,
  UNIT_4_2_STAGE_MAP,
  UNIT_4_2_COURSE_TITLE,
  UNIT_4_2_COURSE_SUBTITLE,
  UNIT_4_2_ROUTE_SEGMENT,
  UNIT_4_2_STAGE_LABEL,
  type UNIT_4_2StudentCourseState,
  type UNIT_4_2TeacherCourseSyncState,
} from '@/lib/unit-4-2-course';
import {
  UNIT_4_2KnowledgeMapVisual,
  UNIT_4_2StepAiAssistant,
  UNIT_4_2StepContentPanel,
  UNIT_4_2TeacherActivitySummary,
} from './step-panels';
import type { WorkspaceParameterChange } from './workspace';

export function UNIT_4_2TeacherPage({
  sessionId,
  lessonRuntime,
}: {
  sessionId: string;
  ...
  const interactiveTracking = useInteractiveTracking({
    resourceId: UNIT_4_2_RESOURCE_KEY,
    resourceKey: UNIT_4_2_RESOURCE_KEY,
    sessionId,
  });
  ...
  const {
    ...
    adapter: UNIT_4_2_SESSION_ADAPTER,
  });
  ...
  const teacherSyncState = useMemo(() => {
    ...
    return (latestRecord?.data as UNIT_4_2TeacherCourseSyncState | null) ?? null;
  }, [teacherStates]);
  ...
  useEffect(() => {
    if (!shouldPostUNIT_4_2TeacherSync({ loadingSession, teacherViewHydrated })) return;
  ...
  const studentStates = useMemo(() => {
    return courseStates
      .map((record) => {
        ...
        if (data.kind !== 'unit42_student_state' || data.version !== 1) return null;
        return {
          studentName: data.studentName || record.user?.name?.trim() || '未命名学生',
          state: data as UNIT_4_2StudentCourseState,
        };
      })
      .filter(Boolean) as Array<{ studentName: string; state: UNIT_4_2StudentCourseState }>;
  }, [courseStates]);
  ...
  const currentResponses = useMemo(() => {
    return studentStates
      ...
      .filter(Boolean) as Array<{ studentName: string; response: UNIT_4_2StudentCourseState['responses'][string] }>;
  }, [step.id, studentStates]);

  const handlePatchCurrentStep = useCallback(
    async (nextIndex: number) => {
      const nextStep = UNIT_4_2_LESSON_STEPS[nextIndex];
      trackStepLeave(step.id, { nextStepId: nextStep.id });
      await patchCurrentStep(nextIndex, {
        currentItemId: nextStep.id,
        currentStage: UNIT_4_2_STAGE_MAP[nextStep.stage],
      });
  ...
  const handleEndSession = useCallback(async () => {
    ...
    try {
      await finalizeUNIT_4_2TeacherSession({
        finishSession,
  ...
  if (loadingSession) {
    return (
      <LessonRuntimeLoadingShell
        ...
        subtitle={UNIT_4_2_COURSE_SUBTITLE}
  ...
  return (
    <LessonRuntimeShell
        ...
        stageLabel={UNIT_4_2_STAGE_LABEL}
        ...
        localTools={
          ...
          <div className="premium-lesson-panel-soft px-4 py-4">
            ...
            currentStepId={step.id}
            orderedStepIds={UNIT_4_2_LESSON_STEPS.map((item) => item.id)}
            title="页面知识卡片"
            inlineTool
          />
          </>
        ...
        runtimeAttributes={{
          ...
          'data-runtime-manifest-truth': lessonRuntime.interactiveManifest?.lessonId ?? UNIT_4_2_LESSON_KEY,
        }}
      >
        <div className="space-y-4">
          {error ? <div className="premium-lesson-tone-block premium-tone-rose mb-4">{error}</div> : null}

        {step.id === 'step-01' ? <UNIT_4_2KnowledgeMapVisual /> : null}
        ...
        {isUNIT_4_2AiPageType(step.pageType) ? (
          <div className="mt-4">
            <UNIT_4_2StepAiAssistant step={step} onAiEvent={handleAiEvent} />
          </div>
        ) : null}
          ...
          <UNIT_4_2TeacherActivitySummary
            ...
            onToggleRelease={() =>
              setLocalReleasedActivities((prev) => ({
                ...(prev ?? (teacherSyncState as UNIT_4_2TeacherCourseSyncState | null)?.releasedActivities ?? {}),
                [step.id]: !(prev?.[step.id] ?? (teacherSyncState as UNIT_4_2TeacherCourseSyncState | null)?.releasedActivities?.[step.id]),
              }))
            }
            onToggleBrowse={() =>
              setLocalBrowseEnabled((prev) => ({
                ...(prev ?? (teacherSyncState as UNIT_4_2TeacherCourseSyncState | null)?.browseEnabled ?? {}),
                [step.id]: !(prev?.[step.id] ?? (teacherSyncState as UNIT_4_2TeacherCourseSyncState | null)?.browseEnabled?.[step.id]),
              }))
            }
            onToggleAnswerVisible={() =>
              setLocalRevealedAnswers((prev) => ({
                ...(prev ?? (teacherSyncState as UNIT_4_2TeacherCourseSyncState | null)?.revealedAnswers ?? {}),
                [step.id]: !(prev?.[step.id] ?? (teacherSyncState as UNIT_4_2TeacherCourseSyncState | null)?.revealedAnswers?.[step.id]),
              }))
            }
            onAdvanceReveal={() =>
              setLocalTeacherRevealProgress((prev) => ({
                ...(prev ?? (teacherSyncState as UNIT_4_2TeacherCourseSyncState | null)?.teacherRevealProgress ?? {}),
                [step.id]: (prev?.[step.id] ?? (teacherSyncState as UNIT_4_2TeacherCourseSyncState | null)?.teacherRevealProgress?.[step.id] ?? 0) + 1,
              }))
            }
            onResetReveal={() =>
              setLocalTeacherRevealProgress((prev) => ({
