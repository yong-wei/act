...
import {
  ...
  type UNIT_4_6StepResponse,
} from '@/lib/unit-4-6-course';
import { UNIT_4_6StepContentPanel, UNIT_4_6StudentActivityForm } from './step-panels';

export function UNIT_4_6StudentPage({
  sessionId,
  lessonRuntime,
}: {
  sessionId: string;
  ...
  useEffect(() => {
    const stepContext = getUnit46StepAIContext(step.id);
    if (stepContext) {
      updatePageContext({
  ...
  const handleSubmitResponse = (response: UNIT_4_6StepResponse) => {
    const isResubmit = Boolean(savedResponse);
    void saveCourseState((prev) => {
      const nextState: UNIT_4_6StudentCourseState = {
        ...prev,
  ...
  return (
    <LessonRuntimeShell
        ...
        onIndexChange={(index) => {
          trackStepLeave(step.id, { nextStepId: UNIT_4_6_LESSON_STEPS[index]?.id });
        ...
        runtimeAttributes={{
          ...
          'data-return-target': `/interactive-learning/courses/${UNIT_4_6_ROUTE_SEGMENT}`,
