...
import {
  finalizeUNIT_4_6TeacherSession,
  isUNIT_4_6TeacherSyncState,
  resolveUNIT_4_6TeacherSyncDraft,
  shouldPostUNIT_4_6TeacherSync,
  UNIT_4_6_LESSON_KEY,
  UNIT_4_6_LESSON_STEPS,
  UNIT_4_6_RESOURCE_KEY,
  UNIT_4_6_SESSION_ADAPTER,
  UNIT_4_6_STAGE_MAP,
  UNIT_4_6_COURSE_TITLE,
  UNIT_4_6_COURSE_SUBTITLE,
  UNIT_4_6_ROUTE_SEGMENT,
  UNIT_4_6_STAGE_LABEL,
  type UNIT_4_6StudentCourseState,
  type UNIT_4_6TeacherCourseSyncState,
} from '@/lib/unit-4-6-course';
import { UNIT_4_6StepContentPanel, UNIT_4_6TeacherActivitySummary } from './step-panels';

export function UNIT_4_6TeacherPage({
  sessionId,
  lessonRuntime,
}: {
  sessionId: string;
  ...
  const interactiveTracking = useInteractiveTracking({
    resourceId: UNIT_4_6_RESOURCE_KEY,
    resourceKey: UNIT_4_6_RESOURCE_KEY,
    sessionId,
  });
  ...
  const {
    ...
    adapter: UNIT_4_6_SESSION_ADAPTER,
  });
  ...
  const teacherSyncState = useMemo(() => {
    const latestRecord = [...teacherStates].reverse().find((record) => isUNIT_4_6TeacherSyncState(record.data));
    return (latestRecord?.data as UNIT_4_6TeacherCourseSyncState | null) ?? null;
  }, [teacherStates]);
  ...
  useEffect(() => {
    if (!shouldPostUNIT_4_6TeacherSync({ loadingSession, teacherViewHydrated })) return;
  ...
  const studentStates = useMemo(() => {
    return courseStates
      .map((record) => {
        ...
        if (data.kind !== 'unit46_student_state' || data.version !== 1) return null;
        return {
          studentName: data.studentName || record.user?.name?.trim() || '未命名学生',
          state: data as UNIT_4_6StudentCourseState,
        };
      })
      .filter(Boolean) as Array<{ studentName: string; state: UNIT_4_6StudentCourseState }>;
  }, [courseStates]);
  ...
  const currentResponses = useMemo(() => {
    return studentStates
      ...
      .filter(Boolean) as Array<{ studentName: string; response: UNIT_4_6StudentCourseState['responses'][string] }>;
  }, [step.id, studentStates]);

  const handlePatchCurrentStep = useCallback(
    async (nextIndex: number) => {
      const nextStep = UNIT_4_6_LESSON_STEPS[nextIndex];
      trackStepLeave(step.id, { nextStepId: nextStep.id });
      await patchCurrentStep(nextIndex, {
        currentItemId: nextStep.id,
        currentStage: UNIT_4_6_STAGE_MAP[nextStep.stage],
      });
  ...
  const handleEndSession = useCallback(async () => {
    ...
    try {
      await finalizeUNIT_4_6TeacherSession({
        finishSession,
  ...
  if (loadingSession) {
    return (
      <LessonRuntimeLoadingShell
        ...
        subtitle={UNIT_4_6_COURSE_SUBTITLE}
  ...
  return (
    <LessonRuntimeShell
        ...
        stageLabel={UNIT_4_6_STAGE_LABEL}
        ...
        localTools={
          ...
          <div className="premium-lesson-panel-soft px-4 py-4">
            ...
            currentStepId={step.id}
            orderedStepIds={UNIT_4_6_LESSON_STEPS.map((item) => item.id)}
            title="页面知识卡片"
            inlineTool
          />
          </>
        ...
        runtimeAttributes={{
          ...
          'data-runtime-manifest-truth': lessonRuntime.interactiveManifest?.lessonId ?? UNIT_4_6_LESSON_KEY,
        }}
      >
        ...
        <div className="mt-4">
          <UNIT_4_6TeacherActivitySummary
            step={step}
            manifest={runtimeManifest}
