...
export function UNIT_4_7CourseEntryPage({
  initialRole,
  lessonRuntime,
}: {
  ...
  const createClassroom = async () => {
    ...
    try {
      const cloneRes = await fetch('/api/teacher/preset-lessons/clone', {
        ...
        body: JSON.stringify({ presetKey: UNIT_4_7_PRESET_KEY }),
      });
      ...
      router.push(`/interactive-learning/courses/${UNIT_4_7_ROUTE_SEGMENT}/teacher/${createData.id}`);
    } catch (requestError) {
    ...
    <div className="premium-lesson-shell">
      <header className="premium-lesson-topbar">
        <div className="mx-auto flex max-w-[1180px] items-center justify-between px-4 py-4 sm:px-6">
          <Link href="/interactive-learning/courses" className="premium-lesson-nav-button inline-flex items-center gap-1 px-3 py-1">
            <ArrowLeft className="h-3.5 w-3.5" />
            返回课程总览
          </Link>
          <div className="text-right">
            <div className="premium-lesson-kicker text-[10px] tracking-[0.24em]">Premium Classroom</div>
            <h1 className="premium-lesson-title text-base font-semibold sm:text-lg">{UNIT_4_7_COURSE_TITLE}</h1>
          </div>
        </div>
      </header>
      ...
      <main className="premium-lesson-main mx-auto max-w-[1180px] px-3 py-4 sm:px-6 sm:py-8">
        <div className="mt-4 grid gap-4 md:grid-cols-3">
          {showTeacherSection ? (
            <section className="premium-lesson-panel p-5">
              ...
              <p className="premium-lesson-muted mt-2">自动克隆 4-7 预置教案，生成课堂码并进入工程设计闭环精品课堂。</p>
              <button type="button" onClick={() => void createClassroom()} disabled={isCreating} className="premium-lesson-action-primary mt-5 flex w-full">
          ...
          <section className="premium-lesson-panel p-5">
            ...
            <p className="premium-lesson-muted mt-2">以演示模式进入学生端，预览 12 个课堂环节与导学资料。</p>
            <Link href={`/interactive-learning/courses/${UNIT_4_7_ROUTE_SEGMENT}/student/demo`} className="premium-lesson-action-secondary mt-5 flex w-full">
              进入演示模式
            </Link>
          </section>
          ...
          {showStudentSection ? (
            <section className="premium-lesson-panel p-5">
              ...
              <button type="button" onClick={() => void joinClassroom()} disabled={isJoining} className="premium-lesson-action-secondary mt-4 flex w-full">
        ...
        <section className="premium-lesson-panel mt-4 px-5 py-5">
          <div className="premium-lesson-kicker">4-7 · Destroyer Hifi Design Closure</div>
          <h2 className="premium-lesson-title mt-2 text-3xl font-semibold sm:text-4xl">{UNIT_4_7_COURSE_TITLE}</h2>
          <p className="premium-lesson-muted mt-3 max-w-3xl sm:text-base">{UNIT_4_7_COURSE_DESCRIPTION}</p>
          <div className="mt-4 flex flex-wrap gap-2 text-xs">
            {['高保真辨识', '传统设计', '优化解码', '跨模型验证', '扰动噪声边界'].map((item) => (
              <span key={item} className="premium-lesson-tone-pill premium-tone-cyan px-3 py-1">{item}</span>
            ))}
          </div>
        </section>

        <LessonEntryMediaHub lessonRuntime={lessonRuntime} courseLabel="4-7" />
