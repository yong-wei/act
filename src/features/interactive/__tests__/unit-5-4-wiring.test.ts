...
const manifest = normalizeInteractiveRuntimeManifest(
  JSON.parse(readFileSync(join(repoRoot, 'course-content/runtime/lessons/5-4/interactive-manifest.json'), 'utf8')),
);

if (!manifest) throw new Error('5-4 interactive manifest is invalid');

const UNIT_5_4_ROUTE_SEGMENT = 'unit-5-4-data-driven-mpc-transition';
const UNIT_5_4_PRESET_KEY = 'unit-5-4-data-driven-mpc-transition-v1';
const UNIT_5_4_COURSE_ID = 'unit-5-4-data-driven-mpc-transition';
const UNIT_5_4_COURSE_TITLE = manifest.courseTitle;

describe('unit 5-4 platform wiring', () => {
  it('registers the 5-4 AI context registry entry and quick questions from manifest page goals', () => {
    const registry = COURSE_AI_CONTEXT_REGISTRY[UNIT_5_4_PRESET_KEY];

    expect(registry).toBeDefined();
    expect(registry?.courseMeta.courseTitle).toBe(UNIT_5_4_COURSE_TITLE);

    const quickQuestions = getStepQuickQuestions(UNIT_5_4_PRESET_KEY, 'step-11');
  ...
  it('registers the course in the featured catalog, module 5, and classroom route resolver', () => {
    const lesson = FEATURED_LESSONS.find((item) => item.id === UNIT_5_4_COURSE_ID);
    const module5 = INTERACTIVE_COURSE_MODULES.find((module) => module.id === 'module-5');

    expect(lesson).toMatchObject({
      id: UNIT_5_4_COURSE_ID,
    ...
    expect(resolveSessionRouteFromPlanTitle(UNIT_5_4_COURSE_TITLE)).toEqual({
      routeSegment: UNIT_5_4_ROUTE_SEGMENT,
      isPremiumCourse: true,
    });
    expect(resolveSessionRouteFromPlanTitle('5-4：从模型驱动到数据驱动')).toEqual({
      routeSegment: UNIT_5_4_ROUTE_SEGMENT,
      isPremiumCourse: true,
    });
  });
  ...
  it('exposes a dedicated preset generated from the 17-step runtime manifest flow', async () => {
    ...
    const preset = presetsModule.UNIT_5_4_DATA_DRIVEN_MPC_TRANSITION_PRESET;

    expect(preset).toBeDefined();
    expect(preset?.key).toBe(UNIT_5_4_PRESET_KEY);
    ...
    expect(ALL_PRESETS.some((item) => item.key === UNIT_5_4_PRESET_KEY)).toBe(true);
  });
});
