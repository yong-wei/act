...
describe('PREMIUM_LESSONS', () => {
  it('includes unit 2-1 in the premium interactive lesson entry', () => {
    expect(PREMIUM_LESSONS.some((lesson) => lesson.id === 'unit-2-1-modeling-language')).toBe(true);
  });

  it('does not leave unit 2-1 in the legacy lesson list', () => {
    expect(LEGACY_LESSONS.some((lesson) => lesson.id === 'unit-2-1-modeling-language')).toBe(false);
  });
