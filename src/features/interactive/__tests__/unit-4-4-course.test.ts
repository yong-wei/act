...
describe('unit 4-4 interactive course', () => {
  ...
  it('keeps the local page contracts aligned with the authoring interactive contract for all 14 steps', async () => {
    const contract = parse(
      readFileSync(
        join(repoRoot, 'course-content/authoring/lessons/4-4/design/4-4-interactive-contract.yaml'),
        'utf8',
      ),
    ) as {
      steps: Record<string, {
    ...
    const courseModule = await import('@/lib/unit-4-4-course');
    const interactiveSteps = new Map(courseModule.UNIT_4_4_LESSON_STEPS.map((step) => [step.id, step]));
