...
import {
  UNIT_3_9_COURSE_DESCRIPTION,
  UNIT_3_9_COURSE_TITLE,
  UNIT_3_9_LESSON_STEPS,
  UNIT_3_9_PRESET_KEY,
  UNIT_3_9_ROUTE_SEGMENT,
...
export function UNIT_3_9CourseEntryPage({
  initialRole,
  lessonRuntime,
}: {
  ...
  const createClassroom = async () => {
    ...
    try {
      const cloneRes = await fetch('/api/teacher/preset-lessons/clone', {
        ...
        body: JSON.stringify({ presetKey: UNIT_3_9_PRESET_KEY }),
      });
      ...
      router.push(`/interactive-learning/courses/${UNIT_3_9_ROUTE_SEGMENT}/teacher/${createData.id}`);
    } catch (requestError) {
      ...
      <header className="premium-lesson-topbar">
        <div className="mx-auto flex max-w-[1180px] items-center justify-between px-4 py-4 sm:px-6">
          ...
          <div className="text-right">
            ...
            <h1 className="premium-lesson-title text-base font-semibold sm:text-lg">{UNIT_3_9_COURSE_TITLE}</h1>
          </div>
        </div>
      </header>
      ...
      <main className="premium-lesson-main mx-auto max-w-[1180px] px-3 py-4 sm:px-6 sm:py-8">
        <div className="mt-4 grid gap-4 md:grid-cols-3">
          {showTeacherSection ? (
            <section className="premium-lesson-panel p-5">
              ...
              <p className="premium-lesson-muted mt-2">
                自动克隆 3-9 预置教案，生成课堂码并进入《稳定—动态—稳态综合映射实验》精品课堂。
              </p>
              <button
                type="button"
          ...
          <section className="premium-lesson-panel p-5">
            ...
            <p className="premium-lesson-muted mt-2">
              以演示模式进入学生端，先预览 {UNIT_3_9_LESSON_STEPS.length} 个课堂环节与 runtime 导学资源。
            </p>
            <Link
              href={`/interactive-learning/courses/${UNIT_3_9_ROUTE_SEGMENT}/student/demo`}
        ...
        <section className="premium-lesson-panel mt-4 px-5 py-5">
          <div className="premium-lesson-kicker">3-9 · Cross-Domain Mapping Lab</div>
          <h2 className="premium-lesson-title mt-2 text-3xl font-semibold sm:text-4xl">{UNIT_3_9_COURSE_TITLE}</h2>
          <p className="premium-lesson-muted mt-3 max-w-3xl sm:text-base">{UNIT_3_9_COURSE_DESCRIPTION}</p>
          <p className="premium-lesson-muted mt-3 max-w-3xl">
            本课承接 <strong>3-8</strong> 的统一判断地图，把 <strong>基准</strong>、<strong>零点线补强</strong>、
            <strong>积分家族</strong> 与 <strong>滞后对照</strong> 写回同一张综合映射表，再把它接到
            <strong>4-1</strong> 的任务表达入口。
          </p>
          <div className="mt-4 flex flex-wrap gap-2 text-xs">
            {['统一对象', '任务标签', '零点线补强', '积分家族', '滞后对照', '综合映射'].map((item) => (
        ...
        <div className="mt-4 grid gap-4">
          <LessonEntryMediaHub lessonRuntime={lessonRuntime} courseLabel="3-9 · Pre-study" />
