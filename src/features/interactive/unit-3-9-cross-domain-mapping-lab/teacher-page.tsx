...
import {
  ...
  UNIT_3_9_STAGE_LABEL,
  type UNIT_3_9StudentCourseState,
  type UNIT_3_9TeacherCourseSyncState,
...
export function UNIT_3_9TeacherPage({
  ...
  useEffect(() => {
    if (!shouldPostUNIT_3_9TeacherSync({ loadingSession, teacherViewHydrated })) return;
  ...
  const studentStates = useMemo(() => {
    return courseStates
      .map((record) => {
        ...
        if (data.kind !== 'unit39_student_state' || data.version !== 1) return null;
        return {
          studentName: data.studentName || record.user?.name?.trim() || '未命名学生',
          state: data as UNIT_3_9StudentCourseState,
        };
      })
      .filter(Boolean) as Array<{ studentName: string; state: UNIT_3_9StudentCourseState }>;
  }, [courseStates]);
  ...
  const currentResponses = useMemo(() => {
    return studentStates
      ...
      .filter(Boolean) as Array<{ studentName: string; response: UNIT_3_9StudentCourseState['responses'][string] }>;
  ...
  const handlePatchCurrentStep = useCallback(
    async (nextIndex: number) => {
      ...
      await patchCurrentStep(nextIndex, {
        ...
        currentStage: UNIT_3_9_STAGE_MAP[nextStep.stage],
      });
  ...
  const handleEndSession = useCallback(async () => {
    ...
    try {
      await finalizeUNIT_3_9TeacherSession({
        finishSession,
  ...
  if (loadingSession) {
    return (
      <LessonRuntimeLoadingShell
        ...
        subtitle={UNIT_3_9_COURSE_SUBTITLE}
  ...
  return (
    <LessonRuntimeShell
        ...
        stageLabel={UNIT_3_9_STAGE_LABEL}
        ...
        runtimeAttributes={{
          ...
          'data-runtime-manifest-truth': lessonRuntime.interactiveManifest?.lessonId ?? UNIT_3_9_LESSON_KEY,
        }}
      >
