...
export function UNIT_4_3CourseEntryPage({
  initialRole,
  lessonRuntime,
}: {
  ...
  const createClassroom = async () => {
    ...
    try {
      const cloneRes = await fetch('/api/teacher/preset-lessons/clone', {
        ...
        body: JSON.stringify({ presetKey: UNIT_4_3_PRESET_KEY }),
      });
      ...
      router.push(`/interactive-learning/courses/${UNIT_4_3_ROUTE_SEGMENT}/teacher/${createData.id}`);
    } catch (requestError) {
      ...
      <header className="premium-lesson-topbar">
        <div className="mx-auto flex max-w-[1180px] items-center justify-between px-4 py-4 sm:px-6">
          ...
          <div className="text-right">
            ...
            <h1 className="premium-lesson-title text-base font-semibold sm:text-lg">{UNIT_4_3_COURSE_TITLE}</h1>
          </div>
        </div>
      </header>
      ...
      <main className="premium-lesson-main mx-auto max-w-[1180px] px-3 py-4 sm:px-6 sm:py-8">
        <div className="mt-4 grid gap-4 md:grid-cols-3">
          {showTeacherSection ? (
            <section className="premium-lesson-panel p-5">
              ...
              <p className="premium-lesson-muted mt-2">
                自动克隆 4-3 预置教案，生成课堂码并进入“初始方案落地实践：从对象分析到结构组合与首轮验证”精品课堂。
              </p>
              <button
                type="button"
          ...
          <section className="premium-lesson-panel p-5">
            ...
            <Link
              href={`/interactive-learning/courses/${UNIT_4_3_ROUTE_SEGMENT}/student/demo`}
        ...
        <section className="premium-lesson-panel mt-4 px-5 py-5">
          <div className="premium-lesson-kicker">4-3 · Initial Scheme</div>
          <h2 className="premium-lesson-title mt-2 text-3xl font-semibold sm:text-4xl">{UNIT_4_3_COURSE_TITLE}</h2>
          <p className="premium-lesson-muted mt-3 max-w-3xl sm:text-base">{UNIT_4_3_COURSE_DESCRIPTION}</p>
          <p className="premium-lesson-muted mt-3 max-w-3xl">
            本课承接 <strong>4-2</strong> 的起步卡，把对象分析、结构分流、参数方向、首轮验证与问题清单压成可继续交给 <strong>4-4</strong> 的第一版方案。
          </p>
          <div className="mt-4 flex flex-wrap gap-2 text-xs">
            {['对象分析', '结构分流', '复合结构', '客船验证', '边界案例', '问题清单'].map((item) => (
