...
import {
  ...
  type UNIT_4_3TeacherCourseSyncState,
...
export function UNIT_4_3TeacherPage({
  sessionId,
  lessonRuntime,
}: {
  sessionId: string;
  ...
  const teacherSyncState = useMemo(() => {
    ...
    return (latestRecord?.data as UNIT_4_3TeacherCourseSyncState | null) ?? null;
  }, [teacherStates]);
  ...
  useEffect(() => {
    if (!shouldPostUNIT_4_3TeacherSync({ loadingSession, teacherViewHydrated })) return;
  ...
  const studentStates = useMemo(() => {
    return courseStates
      .map((record) => {
        ...
        if (data.kind !== 'unit43_student_state' || data.version !== 1) return null;
        return {
          studentName: data.studentName || record.user?.name?.trim() || '未命名学生',
          state: data as UNIT_4_3StudentCourseState,
        };
      })
      .filter(Boolean) as Array<{ studentName: string; state: UNIT_4_3StudentCourseState }>;
  }, [courseStates]);
  ...
  const currentResponses = useMemo(() => {
    return studentStates
      ...
      .filter(Boolean) as Array<{ studentName: string; response: UNIT_4_3StudentCourseState['responses'][string] }>;
  ...
  const handlePatchCurrentStep = useCallback(
    async (nextIndex: number) => {
      ...
      await patchCurrentStep(nextIndex, {
        ...
        currentStage: UNIT_4_3_STAGE_MAP[nextStep.stage],
      });
  ...
  const handleEndSession = useCallback(async () => {
    ...
    try {
      await finalizeUNIT_4_3TeacherSession({
        finishSession,
          ...
          <UNIT_4_3TeacherActivitySummary
            ...
            onToggleRelease={() =>
              setLocalReleasedActivities((prev) => ({
                ...(prev ?? (teacherSyncState as UNIT_4_3TeacherCourseSyncState | null)?.releasedActivities ?? {}),
                [step.id]: !(prev?.[step.id] ?? (teacherSyncState as UNIT_4_3TeacherCourseSyncState | null)?.releasedActivities?.[step.id]),
              }))
            }
            onToggleBrowse={() =>
              setLocalBrowseEnabled((prev) => ({
                ...(prev ?? (teacherSyncState as UNIT_4_3TeacherCourseSyncState | null)?.browseEnabled ?? {}),
                [step.id]: !(prev?.[step.id] ?? (teacherSyncState as UNIT_4_3TeacherCourseSyncState | null)?.browseEnabled?.[step.id]),
              }))
            }
            onToggleAnswerVisible={() =>
              setLocalRevealedAnswers((prev) => ({
                ...(prev ?? (teacherSyncState as UNIT_4_3TeacherCourseSyncState | null)?.revealedAnswers ?? {}),
                [step.id]: !(prev?.[step.id] ?? (teacherSyncState as UNIT_4_3TeacherCourseSyncState | null)?.revealedAnswers?.[step.id]),
              }))
            }
            onAdvanceReveal={() =>
              setLocalTeacherRevealProgress((prev) => ({
                ...(prev ?? (teacherSyncState as UNIT_4_3TeacherCourseSyncState | null)?.teacherRevealProgress ?? {}),
                [step.id]: (prev?.[step.id] ?? (teacherSyncState as UNIT_4_3TeacherCourseSyncState | null)?.teacherRevealProgress?.[step.id] ?? 0) + 1,
              }))
            }
            onResetReveal={() =>
              setLocalTeacherRevealProgress((prev) => ({
