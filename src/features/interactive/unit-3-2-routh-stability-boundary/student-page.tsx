...
export function UNIT_3_2StudentPage({
  sessionId,
  lessonRuntime,
}: {
  sessionId: string;
  ...
  useEffect(() => {
    const stepContext = getUnit32StepAIContext(step.id);
    if (stepContext) {
      updatePageContext({
  ...
  const handleSubmitResponse = (response: UNIT_3_2StepResponse) => {
    const isResubmit = Boolean(savedResponse);
    void saveCourseState((prev) => {
      const nextState: UNIT_3_2StudentCourseState = {
        ...prev,
  ...
  if (loadingSession) {
    return (
      <LessonRuntimeLoadingShell
        ...
        subtitle={UNIT_3_2_COURSE_SUBTITLE}
  ...
  return (
    <LessonRuntimeShell
        ...
        steps={UNIT_3_2_LESSON_STEPS}
        activeIndex={activeIndex}
        stageLabel={UNIT_3_2_STAGE_LABEL}
        notice={isOutOfSync ? `当前页面与教师不同步，教师正在第 ${teacherIndex + 1} 页` : step.hint}
        onIndexChange={(index) => {
          trackStepLeave(step.id, { nextStepId: UNIT_3_2_LESSON_STEPS[index]?.id });
        ...
        runtimeAttributes={{
          ...
          'data-return-target': `/interactive-learning/courses/${UNIT_3_2_ROUTE_SEGMENT}`,
          'data-runtime-manifest-truth': lessonRuntime.interactiveManifest?.lessonId ?? UNIT_3_2_LESSON_KEY,
        ...
        <div className="space-y-4">
          ...
          <div className="premium-lesson-tone-block premium-tone-amber mb-4 flex flex-wrap items-center justify-between gap-3">
            ...
            <button
              type="button"
              onClick={() => {
                trackStepView(UNIT_3_2_LESSON_STEPS[teacherIndex]?.id, {
                  pageType: UNIT_3_2_LESSON_STEPS[teacherIndex]?.pageType,
