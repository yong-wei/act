...
import {
  UNIT_3_2_COURSE_DESCRIPTION,
  UNIT_3_2_COURSE_TITLE,
  UNIT_3_2_LESSON_STEPS,
  UNIT_3_2_PRESET_KEY,
  UNIT_3_2_ROUTE_SEGMENT,
...
export function UNIT_3_2CourseEntryPage({
  initialRole,
  lessonRuntime,
}: {
  ...
  const createClassroom = async () => {
    ...
    try {
      const cloneRes = await fetch('/api/teacher/preset-lessons/clone', {
        ...
        body: JSON.stringify({ presetKey: UNIT_3_2_PRESET_KEY }),
      });
      ...
      router.push(`/interactive-learning/courses/${UNIT_3_2_ROUTE_SEGMENT}/teacher/${createData.id}`);
    } catch (requestError) {
    ...
    <div className="premium-lesson-shell">
      <header className="premium-lesson-topbar">
        <div className="mx-auto flex max-w-[1180px] items-center justify-between px-4 py-4 sm:px-6">
          ...
          <div className="text-right">
            ...
            <h1 className="premium-lesson-title text-base font-semibold sm:text-lg">{UNIT_3_2_COURSE_TITLE}</h1>
          </div>
        </div>
      </header>
      ...
      <main className="premium-lesson-main mx-auto max-w-[1180px] px-3 py-4 sm:px-6 sm:py-8">
        <div className="mt-4 grid gap-4 md:grid-cols-3">
          {showTeacherSection ? (
            <section className="premium-lesson-panel p-5">
              ...
              <p className="premium-lesson-muted mt-2">自动克隆 3-2 预置教案，生成课堂码并进入《劳斯判据——从高阶系统稳定判定到参数可行域》精品课堂。</p>
              <button
                type="button"
          ...
          <section className="premium-lesson-panel p-5">
            ...
            <p className="premium-lesson-muted mt-2">
              以演示模式进入学生端，先预览 {UNIT_3_2_LESSON_STEPS.length} 个课堂环节与 runtime 导学资源。
            </p>
            <Link
              href={`/interactive-learning/courses/${UNIT_3_2_ROUTE_SEGMENT}/student/demo`}
        ...
        <section className="premium-lesson-panel mt-4 px-5 py-5">
          <div className="premium-lesson-kicker">3-2 · Routh Stability Boundary</div>
          <h2 className="premium-lesson-title mt-2 text-3xl font-semibold sm:text-4xl">{UNIT_3_2_COURSE_TITLE}</h2>
          <p className="premium-lesson-muted mt-3 max-w-3xl sm:text-base">{UNIT_3_2_COURSE_DESCRIPTION}</p>
          <p className="premium-lesson-muted mt-3 max-w-3xl">
            本课承接 <strong>3-1</strong> 已经建立的纯极点语言，把高阶系统稳定判断推进成劳斯表、边界类型与参数可行域表达，
            并为 <strong>3-3</strong> 的迁移机制与 <strong>4-1</strong> 的区域约束设计打地基。
          </p>
          <div className="mt-4 flex flex-wrap gap-2 text-xs">
            {['劳斯判据', '稳定边界', '参数区间', '特殊情况', '三域翻译', '区域约束'].map((item) => (
        ...
        <LessonEntryMediaHub lessonRuntime={lessonRuntime} courseLabel="3-2 · Pre-study" />
