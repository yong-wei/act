...
import { getUnit13StepAIContext } from '@/lib/unit-1-3-ai-contexts';
import {
  buildUNIT_1_3ParameterSnapshots,
  getUNIT_1_3ManifestStepFromManifest,
  getUNIT_1_3PageContractFromManifest,
  UNIT_1_3_LESSON_KEY,
  UNIT_1_3_LESSON_STEPS,
  UNIT_1_3_RESOURCE_KEY,
  UNIT_1_3_SESSION_ADAPTER,
  UNIT_1_3_COURSE_TITLE,
  UNIT_1_3_COURSE_SUBTITLE,
...
export function UNIT_1_3StudentPage({
  sessionId,
  lessonRuntime,
}: {
  sessionId: string;
  ...
  const pageContract = getUNIT_1_3PageContractFromManifest(runtimeManifest, step.id);
  ...
  useEffect(() => {
    ...
    const demoIndex = UNIT_1_3_LESSON_STEPS.findIndex((item) => item.id === demoStepId);
  ...
  const enqueueCourseStateSave = useCallback(
    (updater: (prev: UNIT_1_3StudentCourseState) => UNIT_1_3StudentCourseState) => {
  ...
  useEffect(() => {
    const stepContext = getUnit13StepAIContext(step.id);
  ...
  const handleSubmitResponse = (response: UNIT_1_3StepResponse) => {
    ...
    void enqueueCourseStateSave((prev) => {
      ...
      const stepManifest = getUNIT_1_3ManifestStepFromManifest(runtimeManifest, targetStepId);
      const structuredAnswer = firstStructuredAnswer(answers);
      const parameterSnapshots = buildUNIT_1_3ParameterSnapshots({
        answers,
  ...
  if (loadingSession) {
    return (
      <LessonRuntimeLoadingShell
        ...
        subtitle={UNIT_1_3_COURSE_SUBTITLE}
  ...
  return (
    <LessonRuntimeShell
        ...
        steps={UNIT_1_3_LESSON_STEPS}
        activeIndex={activeIndex}
        stageLabel={UNIT_1_3_STAGE_LABEL}
        ...
        runtimeAttributes={{
          ...
          'data-return-target': `/interactive-learning/courses/${UNIT_1_3_ROUTE_SEGMENT}`,
          'data-runtime-manifest-truth': lessonRuntime.interactiveManifest?.lessonId ?? UNIT_1_3_LESSON_KEY,
