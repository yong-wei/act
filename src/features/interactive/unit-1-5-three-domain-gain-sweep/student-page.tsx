...
import { createUNIT_1_5AIContext } from '@/lib/unit-1-5-ai-contexts';
import {
  buildUNIT_1_5ParameterSnapshots,
  ...
  getUNIT_1_5PageContractFromManifest,
  UNIT_1_5_LESSON_KEY,
  ...
  type UNIT_1_5StepResponse,
} from '@/lib/unit-1-5-course';
import {
  UNIT_1_5StepContentPanel,
  UNIT_1_5StudentActivityForm,
} from './step-panels';
...
export function UNIT_1_5StudentPage({
  sessionId,
  lessonRuntime,
}: {
  sessionId: string;
  ...
  const interactiveTracking = useInteractiveTracking({
    resourceId: UNIT_1_5_RESOURCE_KEY,
    resourceKey: UNIT_1_5_RESOURCE_KEY,
  ...
  const {
    ...
    adapter: UNIT_1_5_SESSION_ADAPTER,
    currentStudentName,
    currentUserId,
    isDemo,
    demoStepId,
  ...
  const step = UNIT_1_5_LESSON_STEPS[activeIndex];
  const runtimeManifest = lessonRuntime.interactiveManifest;
  const currentStepManifest = getUNIT_1_5ManifestStepFromManifest(runtimeManifest, step.id);
  const pageContract = getUNIT_1_5PageContractFromManifest(runtimeManifest, step.id);
  ...
  const viewedStepIds = useMemo(
    () => Array.from(new Set([...(courseState.viewedStepIds ?? []), ...localViewedStepIds])),
    [courseState.viewedStepIds, localViewedStepIds],
  );
  const analyticsSummary = useMemo(
    () => buildUNIT_1_5StudentAnalyticsItems({ ...courseState, viewedStepIds }, runtimeManifest),
    [courseState, runtimeManifest, viewedStepIds],
  );
  ...
  useEffect(() => {
    ...
    const demoIndex = UNIT_1_5_LESSON_STEPS.findIndex((item) => item.id === demoStepId);
  ...
  const enqueueCourseStateSave = useCallback(
    (updater: (prev: UNIT_1_5StudentCourseState) => UNIT_1_5StudentCourseState) => {
      const run = saveQueueRef.current.then(async () => {
        const confirmation: {
          previousState?: UNIT_1_5StudentCourseState;
          confirmedState?: UNIT_1_5StudentCourseState;
        } = {};
  ...
  const teacherStep = UNIT_1_5_LESSON_STEPS[teacherIndex] ?? step;
  const teacherPageContract = getUNIT_1_5PageContractFromManifest(runtimeManifest, teacherStep.id);
  ...
  useEffect(() => {
    updatePageContext(createUNIT_1_5AIContext(currentStepManifest, {
  ...
  const handleSubmitResponse = async (response: UNIT_1_5StepResponse) => {
    ...
    try {
      const confirmation = await enqueueCourseStateSave((prev) => {
        ...
        return {
          ...
          responseHistory: appendUNIT_1_5ResponseHistory(prev.responseHistory, persistedResponse),
        };
      });
      const persistedResponse = confirmation.confirmedState.responses[targetStepId];
      const stepManifest = getUNIT_1_5ManifestStepFromManifest(runtimeManifest, targetStepId);
      const structuredAnswer = firstStructuredAnswer(response.answers);
      const parameterSnapshots = buildUNIT_1_5ParameterSnapshots({
      ...
      submitManifestStepResponse({
        ...
        dataOverrides: buildUNIT_1_5SubmissionDataOverrides({
          stepManifest,
          structuredAnswer: isControlWorkbenchEvidenceDraft(structuredAnswer) ? structuredAnswer : null,
        }),
      });
    } catch (requestError) {
  ...
  return (
    <LessonRuntimeShell
        ...
        runtimeAttributes={{
          ...
          'data-return-target': `/interactive-learning/courses/${UNIT_1_5_ROUTE_SEGMENT}`,
      ...
      >
        ...
        {submissionError ? (
          <div className="premium-lesson-tone-block premium-tone-rose mb-4" role="alert">
            <span>提交失败：{submissionError}。请在当前作答区确认最新参数或答案后重试。</span>
          </div>
        ) : null}
        ...
        <UNIT_1_5StepContentPanel
          ...
          onPanelSubmit={released ? (response) => handleSubmitResponse(response) : undefined}
        />
        ...
        {step.pageType !== 'interactive_figure_submit' || savedResponse || answerVisible ? (
          ...
          <UNIT_1_5StudentActivityForm
            ...
            responseHistory={courseState.responseHistory ?? courseState.responses}
            released={released}
