...
import {
  ...
  isUNIT_1_5StudentState,
  isUNIT_1_5TeacherSyncState,
  resolveUNIT_1_5TeacherSyncDraft,
  UNIT_1_5_LESSON_KEY,
...
function revealLayerCount(lessonRuntime: RuntimeLessonEntryBundle, stepId: string) {
  ...
  return getInteractiveRevealLayerCount(
    getUNIT_1_5ManifestStepFromManifest(lessonRuntime.interactiveManifest, stepId),
  );
}

export function UNIT_1_5TeacherPage({
  sessionId,
  lessonRuntime,
}: {
  sessionId: string;
  ...
  if (sessionId === 'demo') return <UNIT_1_5TeacherDemoPage lessonRuntime={lessonRuntime} />;
  return <UNIT_1_5LiveTeacherPage sessionId={sessionId} lessonRuntime={lessonRuntime} />;
}

function UNIT_1_5LiveTeacherPage({
  sessionId,
  lessonRuntime,
}: {
  sessionId: string;
  ...
  const teacherSyncState = useMemo(() => {
    const latestRecord = [...teacherStates].reverse().find((record) => isUNIT_1_5TeacherSyncState(record.data));
    return (latestRecord?.data as UNIT_1_5TeacherCourseSyncState | null) ?? null;
  }, [teacherStates]);
  ...
  const studentStates = useMemo(() => {
    const recordsByUserId = new Map<string, { studentName: string; state: UNIT_1_5StudentCourseState }>();
    courseStates.forEach((record, index) => {
      if (!isUNIT_1_5StudentState(record.data)) return;
  ...
  const analyticsSummary = useMemo(
    () => buildUNIT_1_5TeacherAnalyticsItems(studentStates.map((item) => item.state), runtimeManifest),
    [runtimeManifest, studentStates],
  );
  const currentResponses = useMemo(() => {
    return studentStates
      ...
      .filter(Boolean) as Array<{ studentName: string; response: UNIT_1_5StudentCourseState['responses'][string] }>;
  ...
  const handlePatchCurrentStep = useCallback(
    async (nextIndex: number) => {
      const nextStep = UNIT_1_5_LESSON_STEPS[nextIndex];
      setPendingStepIndex(nextIndex);
      setControlError(null);
      try {
        await patchCurrentStep(nextIndex, {
          currentItemId: nextStep.id,
          currentStage: UNIT_1_5_STAGE_MAP[nextStep.stage],
        });
      } catch (requestError) {
  ...
  const handleEndSession = useCallback(async () => {
    ...
    try {
      await finalizeUNIT_1_5TeacherSession({
        finishSession,
...
function UNIT_1_5TeacherDemoPage({ lessonRuntime }: { lessonRuntime: RuntimeLessonEntryBundle }) {
