...
export function UNIT_1_5CourseHeader({
  steps,
  activeIndex,
  onIndexChange,
  ...
  steps: readonly UNIT_1_5StepDefinition[];
  activeIndex: number;
      ...
      <div className="w-full px-3 py-2 sm:px-4 sm:py-2.5">
        <div className="flex items-center gap-2 sm:gap-3">
          <div className="flex min-w-0 flex-1 items-center gap-3">
            <Link href={`/interactive-learning/courses/${UNIT_1_5_ROUTE_SEGMENT}`} className="premium-lesson-nav-button shrink-0">
              返回
            </Link>
          ...
          <div className="hidden min-w-0 flex-1 md:block">
            <div className="flex items-center justify-center gap-2">
              <span className="premium-lesson-tone-pill premium-tone-cyan px-3 py-0.5 text-[11px]">
                {UNIT_1_5_STAGE_LABEL[currentStep.stage]}
              </span>
          ...
          <div className="ml-auto flex shrink-0 flex-col items-end gap-1">
            ...
            <div className="flex items-center gap-1.5">
              <button type="button" aria-label="上一页" disabled={!navigationEnabled || activeIndex <= 0} onClick={() => onIndexChange(Math.max(0, activeIndex - 1))} className="premium-lesson-control inline-flex items-center disabled:opacity-40">
                <ChevronLeft className="h-3.5 w-3.5" />
              </button>
              <button type="button" aria-label="下一页" disabled={!navigationEnabled || activeIndex >= steps.length - 1} onClick={() => onIndexChange(Math.min(steps.length - 1, activeIndex + 1))} className="premium-lesson-control inline-flex items-center disabled:opacity-40">
                <ChevronRight className="h-3.5 w-3.5" />
              </button>
              <span className="premium-lesson-caption text-xs">{activeIndex + 1} / {steps.length}</span>
            </div>
            {!navigationEnabled ? (
              <span className="premium-lesson-caption text-[10px]">教师开放浏览后可切换环节</span>
            ) : null}
          </div>
        </div>
      </div>
