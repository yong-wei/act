...
import {
  finalizeUNIT_3_3TeacherSession,
  getUNIT_3_3MediaSrc,
  isUNIT_3_3TeacherSyncState,
  resolveUNIT_3_3TeacherSyncDraft,
  shouldPostUNIT_3_3TeacherSync,
  UNIT_3_3_LESSON_KEY,
...
export function UNIT_3_3TeacherPage({
  ...
  const teacherSyncState = useMemo(() => {
    const latestRecord = [...teacherStates].reverse().find((record) => isUNIT_3_3TeacherSyncState(record.data));
    return (latestRecord?.data as UNIT_3_3TeacherCourseSyncState | null) ?? null;
  }, [teacherStates]);
  ...
  useEffect(() => {
    if (!shouldPostUNIT_3_3TeacherSync({ loadingSession, teacherViewHydrated })) return;
  ...
  const studentStates = useMemo(() => {
    return courseStates
      .map((record) => {
        ...
        if (data.kind !== 'unit33_student_state' || data.version !== 1) return null;
        return {
          studentName: data.studentName || record.user?.name?.trim() || '未命名学生',
          state: data as UNIT_3_3StudentCourseState,
        };
      })
      .filter(Boolean) as Array<{ studentName: string; state: UNIT_3_3StudentCourseState }>;
  }, [courseStates]);
  ...
  const currentResponses = useMemo(() => {
    return studentStates
      ...
      .filter(Boolean) as Array<{ studentName: string; response: UNIT_3_3StudentCourseState['responses'][string] }>;
  }, [step.id, studentStates]);

  const handlePatchCurrentStep = useCallback(
    async (nextIndex: number) => {
      const nextStep = UNIT_3_3_LESSON_STEPS[nextIndex];
      trackStepLeave(step.id, { nextStepId: nextStep.id });
      await patchCurrentStep(nextIndex, {
        currentItemId: nextStep.id,
        currentStage: UNIT_3_3_STAGE_MAP[nextStep.stage],
      });
  ...
  const handleEndSession = useCallback(async () => {
    ...
    try {
      await finalizeUNIT_3_3TeacherSession({
        finishSession,
  ...
  if (loadingSession) {
    return (
      <LessonRuntimeLoadingShell
        ...
        subtitle={UNIT_3_3_COURSE_SUBTITLE}
  ...
  return (
    <LessonRuntimeShell
        ...
        stageLabel={UNIT_3_3_STAGE_LABEL}
        ...
        runtimeAttributes={{
          ...
          'data-runtime-manifest-truth': lessonRuntime.interactiveManifest?.lessonId ?? UNIT_3_3_LESSON_KEY,
        }}
      >
          ...
          <UNIT_3_3TeacherActivitySummary
            ...
            onToggleRelease={() =>
              setLocalReleasedActivities((prev) => ({
                ...(prev ?? (teacherSyncState as UNIT_3_3TeacherCourseSyncState | null)?.releasedActivities ?? {}),
                [step.id]: !(prev?.[step.id] ?? (teacherSyncState as UNIT_3_3TeacherCourseSyncState | null)?.releasedActivities?.[step.id]),
              }))
            }
            onToggleBrowse={() =>
              setLocalBrowseEnabled((prev) => ({
                ...(prev ?? (teacherSyncState as UNIT_3_3TeacherCourseSyncState | null)?.browseEnabled ?? {}),
                [step.id]: !(prev?.[step.id] ?? (teacherSyncState as UNIT_3_3TeacherCourseSyncState | null)?.browseEnabled?.[step.id]),
              }))
            }
            onToggleAnswerVisible={() =>
              setLocalRevealedAnswers((prev) => ({
                ...(prev ?? (teacherSyncState as UNIT_3_3TeacherCourseSyncState | null)?.revealedAnswers ?? {}),
                [step.id]: !(prev?.[step.id] ?? (teacherSyncState as UNIT_3_3TeacherCourseSyncState | null)?.revealedAnswers?.[step.id]),
              }))
            }
            onAdvanceReveal={() =>
              setLocalTeacherRevealProgress((prev) => ({
                ...(prev ?? (teacherSyncState as UNIT_3_3TeacherCourseSyncState | null)?.teacherRevealProgress ?? {}),
                [step.id]:
                  (prev?.[step.id] ??
                    (teacherSyncState as UNIT_3_3TeacherCourseSyncState | null)?.teacherRevealProgress?.[step.id] ??
                    0) + 1,
              }))
            }
            onResetReveal={() =>
