...
import {
  ...
  UNIT_1_3StudentSummaryPanel,
} from './step-panels';
import type { WorkspaceParameterChange } from './workspace';
import { useGlobalAI } from '@/components/providers/global-ai-provider';
import { getUnit13StepAIContext } from '@/lib/course-ai-contexts';
...
export function UNIT_1_3StudentPage({
  ...
  const handleSubmitResponse = (response: UNIT_1_3StepResponse) => {
    ...
    void saveCourseState((prev) => {
      const nextState: UNIT_1_3StudentCourseState = {
        ...prev,
    ...
    <div className="premium-lesson-shell">
      <UNIT_1_3CourseHeader
        ...
        onIndexChange={(index) => {
          ...
          trackStepLeave(step.id, { nextStepId: UNIT_1_3_LESSON_STEPS[index]?.id });
      ...
      <main className="premium-lesson-main mx-auto max-w-[1180px] px-3 py-4 sm:px-6 sm:py-6">
        {isOutOfSync ? (
          <div className="premium-lesson-tone-block premium-tone-amber mb-4 flex flex-wrap items-center justify-between gap-3">
            ...
            <button
              type="button"
              onClick={() => {
                trackStepView(UNIT_1_3_LESSON_STEPS[teacherIndex]?.id, {
                  pageType: UNIT_1_3_LESSON_STEPS[teacherIndex]?.pageType,
        ...
        {step.pageType === 'summary' ? (
          <div className="mt-4">
            <UNIT_1_3StudentSummaryPanel responses={courseState.responses} />
          </div>
        ) : null}
      </main>
    </div>
