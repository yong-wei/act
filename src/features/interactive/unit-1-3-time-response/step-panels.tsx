...
import {
  ...
  UNIT_1_3_STAGE_LABEL,
  type UNIT_1_3StepDefinition,
  type UNIT_1_3StepResponse,
...
export interface UNIT_1_3TeacherResponseItem {
  studentName: string;
  response: UNIT_1_3StepResponse;
}
...
function getStepBlueprint(step: UNIT_1_3StepDefinition): StepBlueprint {
  switch (step.id) {
    case 'step-01':
      return {
        kicker: 'Roadmap',
    ...
    case 'step-14':
      return {
        kicker: 'AI Compare',
        intro: '这一步必须坚持“先个人判断、后 AI 对照”。你要先把指标要求翻译成参数区域，再用 AI 帮你核对推理链有没有漏约束、错方向。',
        sections: [
          {
            title: '本页 AI 的职责',
            tone: 'violet',
        ...
        prompts: [
          '我先给出自己的判断，请你不要直接重算，而是逐步检查我的推理：若要求超调量不超过 10%、调节时间不超过 1s，我把它翻译成 zeta 与 wn 的哪些约束？请指出我是否漏掉了条件。',
    ...
    case 'step-15':
      return {
        ...
        intro: '时域指标不是孤立存在的，它们最终都会投影到极点位置上：更快看实部，更小超调看阻尼比，更稳定的品质看区域约束而不是单点。',
        sections: [
          {
            title: '三句翻译',
            tone: 'amber',
...
function getStepActivity(step: UNIT_1_3StepDefinition): ActivitySpec {
  switch (step.id) {
    case 'step-02':
      return {
        kind: 'quiz',
...
function getAiPrompts(step: UNIT_1_3StepDefinition) {
  return getStepBlueprint(step).prompts ?? [];
}

function buildInteractiveAiConfig(step: UNIT_1_3StepDefinition): InteractiveConfig {
  return {
    resourceId: `unit13:${step.id}`,
    registryId: 'unit13-inline-ai',
...
function getDefaultDraft(activity: ActivitySpec, savedResponse?: UNIT_1_3StepResponse) {
  if (savedResponse) {
...
function getWordCloudEntries(responses: UNIT_1_3TeacherResponseItem[]) {
  ...
  return (
    <section className="premium-lesson-panel-soft mt-4 px-4 py-4">
      ...
      <label className="premium-lesson-caption mt-4 block text-xs">
        ...
        <input
          id="unit-1-3-time-constant-slider"
          name="timeConstant"
    ...
    <section className="premium-lesson-panel-soft mt-4 px-4 py-4">
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-2">
        ...
        <label className="premium-lesson-caption text-xs">
          zeta = {zeta.toFixed(2)}
          <input
            id="unit-1-3-example-zeta-slider"
            name="exampleZeta"
...
export function UNIT_1_3KnowledgeMapVisual() {
  return (
  ...
  return (
    <section className="premium-lesson-panel px-4 py-5">
      <div className="flex flex-wrap items-center gap-2">
        <span className="premium-lesson-tone-pill premium-tone-slate">{UNIT_1_3_STAGE_LABEL[step.stage]}</span>
...
export function UNIT_1_3StudentActivityForm({
  ...
  step: UNIT_1_3StepDefinition;
  savedResponse?: UNIT_1_3StepResponse;
  released: boolean;
  answerVisible: boolean;
  onSubmit: (response: UNIT_1_3StepResponse) => void;
}) {
...
export function UNIT_1_3TeacherActivitySummary({
  ...
  step: UNIT_1_3StepDefinition;
  responses: UNIT_1_3TeacherResponseItem[];
  released: boolean;
  answerVisible: boolean;
...
export function UNIT_1_3StudentSummaryPanel({
  responses,
}: {
  responses: Record<string, UNIT_1_3StepResponse>;
}) {
  const finishedSteps = UNIT_1_3_LESSON_STEPS.filter((step) => responses[step.id]);

  return (
    <section className="premium-lesson-panel-soft px-4 py-4">
      <div className="premium-lesson-title text-sm font-medium">本课学习小结</div>
      <div className="premium-lesson-muted mt-2 text-sm">
        你已完成 {finishedSteps.length} / {UNIT_1_3_LESSON_STEPS.length} 个互动环节。
      </div>
...
export function UNIT_1_3StepAiAssistant({
  step,
  onAiEvent,
}: {
  step: UNIT_1_3StepDefinition;
      ...
      <Dialog open={ai.isPanelOpen} onOpenChange={ai.togglePanel}>
        <DialogContent className="max-w-5xl border-border bg-background p-0 text-foreground">
          <DialogHeader className="border-b border-border px-6 py-4">
            <DialogTitle>{UNIT_1_3_COURSE_TITLE} · 页内 AI 助手</DialogTitle>
            <DialogDescription>
