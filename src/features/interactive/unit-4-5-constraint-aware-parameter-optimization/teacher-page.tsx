...
import {
  ...
  type UNIT_4_5TeacherCourseSyncState,
} from '@/lib/unit-4-5-course';
import { UNIT_4_5CourseHeader } from './course-header';
import {
  UNIT_4_5StepContentPanel,
  UNIT_4_5TeacherActivitySummary,
} from './step-panels';

export function UNIT_4_5TeacherPage({
  sessionId,
  lessonRuntime,
}: {
  sessionId: string;
  ...
  const teacherSyncState = useMemo(() => {
    ...
    return (latestRecord?.data as UNIT_4_5TeacherCourseSyncState | null) ?? null;
  }, [teacherStates]);
  ...
  useEffect(() => {
    if (!shouldPostUNIT_4_5TeacherSync({ loadingSession, teacherViewHydrated })) return;
  ...
  const studentStates = useMemo(() => {
    return courseStates
      .map((record) => {
        ...
        if (data.kind !== 'unit45_student_state' || data.version !== 1) return null;
        return {
          studentName: data.studentName || record.user?.name?.trim() || '未命名学生',
          state: data as UNIT_4_5StudentCourseState,
        };
      })
      .filter(Boolean) as Array<{ studentName: string; state: UNIT_4_5StudentCourseState }>;
  }, [courseStates]);

  const joinedStudents = useMemo(
    () => Array.from(new Set(studentStates.map((item) => item.studentName))),
    [studentStates],
  );
  ...
  const currentResponses = useMemo(() => {
    return studentStates
      ...
      .filter(Boolean) as Array<{ studentName: string; response: UNIT_4_5StudentCourseState['responses'][string] }>;
  }, [step.id, studentStates]);

  const handlePatchCurrentStep = useCallback(
    async (nextIndex: number) => {
      const nextStep = UNIT_4_5_LESSON_STEPS[nextIndex];
      trackStepLeave(step.id, { nextStepId: nextStep.id });
      await patchCurrentStep(nextIndex, {
        currentItemId: nextStep.id,
        currentStage: UNIT_4_5_STAGE_MAP[nextStep.stage],
      });
  ...
  const handleEndSession = useCallback(async () => {
    ...
    try {
      await finalizeUNIT_4_5TeacherSession({
        finishSession,
          ...
          <UNIT_4_5TeacherActivitySummary
            ...
            onAdvanceReveal={() =>
              setLocalTeacherRevealProgress((prev) => ({
                ...
                [step.id]:
                  (prev?.[step.id] ?? teacherSyncState?.teacherRevealProgress?.[step.id] ?? 0) + 1,
              }))
            }
            onResetReveal={() =>
              setLocalTeacherRevealProgress((prev) => ({
