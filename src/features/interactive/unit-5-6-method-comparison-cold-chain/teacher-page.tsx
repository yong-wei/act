...
import {
  finalizeUNIT_5_6TeacherSession,
  isUNIT_5_6StudentState,
  isUNIT_5_6TeacherSyncState,
  resolveUNIT_5_6TeacherSyncDraft,
  shouldPostUNIT_5_6TeacherSync,
  UNIT_5_6_LESSON_KEY,
...
function normalizeAnswerList(value: string, comparison: Unit56ObjectiveCard['comparison']) {
  const items = value
    .split(/[|/,，、]/)
...
function expectedObjectiveAnswer(card: InteractiveRuntimeActivityCardManifest) {
  if (card.responseKind === 'drag_match' && card.referenceMatches?.length && card.matchItems?.length) {
    return {
  ...
  if ((card.responseKind === 'drag_sort' || card.responseKind === 'card_sort') && card.options.length) {
    return {
  ...
  if (card.responseKind === 'multi_select' || card.responseKind === 'multi_choice') {
    return { expected: referenceAnswer, comparison: 'set' as const };
  }
  if (card.responseKind === 'single_choice' || card.responseKind === 'binary_choice') {
    return { expected: referenceAnswer, comparison: 'single' as const };
  }
  return null;
}

function collectObjectiveCards(manifest: InteractiveRuntimeManifest | null | undefined): Unit56ObjectiveCard[] {
  if (!manifest) return [];
  const cards: Unit56ObjectiveCard[] = [];
...
function isObjectiveAnswerCorrect(item: Unit56ObjectiveCard, value: string) {
  if (!value.trim()) return false;
  if (item.comparison === 'single') return normalizeAnswerToken(value) === normalizeAnswerToken(item.expected);
...
export function UNIT_5_6TeacherPage({
  sessionId,
  lessonRuntime,
}: {
  sessionId: string;
  ...
  if (sessionId === 'demo') return <UNIT_5_6TeacherDemoPage lessonRuntime={lessonRuntime} />;
  return <UNIT_5_6LiveTeacherPage sessionId={sessionId} lessonRuntime={lessonRuntime} />;
}

function UNIT_5_6LiveTeacherPage({
  sessionId,
  lessonRuntime,
}: {
  sessionId: string;
  ...
  const teacherSyncState = useMemo(() => {
    const latestRecord = [...teacherStates].reverse().find((record) => isUNIT_5_6TeacherSyncState(record.data));
    return (latestRecord?.data as UNIT_5_6TeacherCourseSyncState | null) ?? null;
  }, [teacherStates]);
  ...
  useEffect(() => {
    if (!shouldPostUNIT_5_6TeacherSync({ loadingSession, teacherViewHydrated })) return;
  ...
  const studentStates = useMemo(() => {
    return courseStates
      .map((record) => {
        if (!isUNIT_5_6StudentState(record.data)) return null;
        return {
          studentName: record.data.studentName || record.user?.name?.trim() || '未命名学生',
          state: record.data,
        };
      })
      .filter(Boolean) as Array<{ studentName: string; state: UNIT_5_6StudentCourseState }>;
  }, [courseStates]);
  ...
  const routeObservationCoverage = useMemo(() => {
    if (!studentStates.length) return 0;
    const withRouteObservation = studentStates.filter((item) =>
      Object.values(item.state.responses).some((response) => Boolean(response.answers.cold_chain_route_observation)),
    ).length;
    return Math.round((withRouteObservation / studentStates.length) * 100);
  }, [studentStates]);
  ...
  const currentResponses = useMemo(() => {
    return studentStates
      ...
      .filter(Boolean) as Array<{ studentName: string; response: UNIT_5_6StudentCourseState['responses'][string] }>;
  ...
  const handlePatchCurrentStep = useCallback(
    async (nextIndex: number) => {
      const nextStep = UNIT_5_6_LESSON_STEPS[nextIndex];
      await patchCurrentStep(nextIndex, {
        currentItemId: nextStep.id,
        currentStage: UNIT_5_6_STAGE_MAP[nextStep.stage],
      });
    },
    [patchCurrentStep],
  );
  ...
  const handleEndSession = useCallback(async () => {
    ...
    try {
      await finalizeUNIT_5_6TeacherSession({
        finishSession,
    ...
    <div className="premium-lesson-shell">
      ...
      <main className="premium-lesson-main mx-auto max-w-[1180px] px-3 py-4 sm:px-6 sm:py-6">
        ...
        <UNIT_5_6StepContentPanel
          ...
          routeObservationCoverage={routeObservationCoverage}
...
function UNIT_5_6TeacherDemoPage({ lessonRuntime }: { lessonRuntime: RuntimeLessonEntryBundle }) {
