...
import {
  finalizeUNIT_2_1TeacherSession,
  getUNIT_2_1MediaSrc,
  isUNIT_2_1TeacherSyncState,
  resolveUNIT_2_1TeacherSyncDraft,
  shouldPostUNIT_2_1TeacherSync,
  UNIT_2_1_LESSON_KEY,
...
import {
  ...
  UNIT_2_1TeacherActivitySummary,
} from './step-panels';
import type { WorkspaceParameterChange } from './workspace';

export function UNIT_2_1TeacherPage({
  sessionId,
  lessonRuntime,
}: {
  sessionId: string;
  ...
  const teacherSyncState = useMemo(() => {
    const latestRecord = [...teacherStates].reverse().find((record) => isUNIT_2_1TeacherSyncState(record.data));
    return (latestRecord?.data as UNIT_2_1TeacherCourseSyncState | null) ?? null;
  }, [teacherStates]);
  ...
  useEffect(() => {
    if (!shouldPostUNIT_2_1TeacherSync({ loadingSession, teacherViewHydrated })) return;
  ...
  const studentStates = useMemo(() => {
    return courseStates
      .map((record) => {
        ...
        if (data.kind !== 'unit21_student_state' || data.version !== 1) return null;
        return {
          studentName: data.studentName || record.user?.name?.trim() || '未命名学生',
          state: data as UNIT_2_1StudentCourseState,
        };
      })
      .filter(Boolean) as Array<{ studentName: string; state: UNIT_2_1StudentCourseState }>;
  }, [courseStates]);
  ...
  const currentResponses = useMemo(() => {
    return studentStates
      ...
      .filter(Boolean) as Array<{ studentName: string; response: UNIT_2_1StudentCourseState['responses'][string] }>;
  }, [step.id, studentStates]);

  const handlePatchCurrentStep = useCallback(
    async (nextIndex: number) => {
      const nextStep = UNIT_2_1_LESSON_STEPS[nextIndex];
      trackStepLeave(step.id, { nextStepId: nextStep.id });
      await patchCurrentStep(nextIndex, {
        currentItemId: nextStep.id,
        currentStage: UNIT_2_1_STAGE_MAP[nextStep.stage],
      });
  ...
  const handleEndSession = useCallback(async () => {
    ...
    try {
      await finalizeUNIT_2_1TeacherSession({
        finishSession,
          ...
          <UNIT_2_1TeacherActivitySummary
            ...
            onToggleRelease={() =>
              setLocalReleasedActivities((prev) => ({
                ...(prev ?? (teacherSyncState as UNIT_2_1TeacherCourseSyncState | null)?.releasedActivities ?? {}),
                [step.id]: !(prev?.[step.id] ?? (teacherSyncState as UNIT_2_1TeacherCourseSyncState | null)?.releasedActivities?.[step.id]),
              }))
            }
            onToggleAnswerVisible={() =>
              setLocalRevealedAnswers((prev) => ({
                ...(prev ?? (teacherSyncState as UNIT_2_1TeacherCourseSyncState | null)?.revealedAnswers ?? {}),
                [step.id]: !(prev?.[step.id] ?? (teacherSyncState as UNIT_2_1TeacherCourseSyncState | null)?.revealedAnswers?.[step.id]),
              }))
            }
          />
        </div>
