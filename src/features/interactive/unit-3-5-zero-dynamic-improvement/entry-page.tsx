  ...
  const createClassroom = async () => {
    ...
    try {
      const cloneRes = await fetch('/api/teacher/preset-lessons/clone', {
        ...
        body: JSON.stringify({ presetKey: UNIT_3_5_PRESET_KEY }),
      });
      ...
      router.push(`/interactive-learning/courses/${UNIT_3_5_ROUTE_SEGMENT}/teacher/${createData.id}`);
    } catch (requestError) {
      ...
      <header className="premium-lesson-topbar">
        <div className="mx-auto flex max-w-[1180px] items-center justify-between px-4 py-4 sm:px-6">
          ...
          <div className="text-right">
            ...
            <h1 className="premium-lesson-title text-base font-semibold sm:text-lg">{UNIT_3_5_COURSE_TITLE}</h1>
          </div>
        </div>
      </header>
      ...
      <main className="premium-lesson-main mx-auto max-w-[1180px] px-3 py-4 sm:px-6 sm:py-8">
        <div className="mt-4 grid gap-4 md:grid-cols-3">
          {showTeacherSection ? (
            <section className="premium-lesson-panel p-5">
              ...
              <p className="premium-lesson-muted mt-2">
                自动克隆 3-5 预置教案，生成课堂码并进入“零点引入与动态改善”的教师课堂台。
              </p>
              <button
                type="button"
          ...
          <section className="premium-lesson-panel p-5">
            ...
            <p className="premium-lesson-muted mt-2">
              以演示模式进入学生端，先预览 {UNIT_3_5_LESSON_STEPS.length} 个课堂环节与 runtime 导学资源。
            </p>
            <Link
              href={`/interactive-learning/courses/${UNIT_3_5_ROUTE_SEGMENT}/student/demo`}
        ...
        <section className="premium-lesson-panel mt-4 px-5 py-5">
          <div className="premium-lesson-kicker">3-5 · Zero Dynamic Improvement</div>
          <h2 className="premium-lesson-title mt-2 text-3xl font-semibold sm:text-4xl">{UNIT_3_5_COURSE_TITLE}</h2>
          <p className="premium-lesson-muted mt-3 max-w-3xl sm:text-base">{UNIT_3_5_COURSE_DESCRIPTION}</p>
          <p className="premium-lesson-muted mt-3 max-w-3xl">
            本课承接 <strong>3-4</strong> 的“沿既有轨迹读图判断”，第一次把关注点从“轨迹怎么读”推进到“结构一改，轨迹、响应和频域为什么一起变”，
            并为 <strong>3-6</strong> 的统一对象实验准备判断语言。
          </p>
          <div className="mt-4 flex flex-wrap gap-2 text-xs">
            {['零点重排', 'PD 与测速反馈', '超前频域原则', '非最小相边界', '保守带宽', '三域判断链'].map((item) => (
        ...
        <LessonEntryMediaHub lessonRuntime={lessonRuntime} courseLabel="3-5 · Pre-study" />
