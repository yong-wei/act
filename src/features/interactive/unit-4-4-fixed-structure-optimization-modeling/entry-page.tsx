...
export function UNIT_4_4CourseEntryPage({
  initialRole,
  lessonRuntime,
}: {
  ...
  const createClassroom = async () => {
    ...
    try {
      const cloneRes = await fetch('/api/teacher/preset-lessons/clone', {
        ...
        body: JSON.stringify({ presetKey: UNIT_4_4_PRESET_KEY }),
      });
      ...
      router.push(`/interactive-learning/courses/${UNIT_4_4_ROUTE_SEGMENT}/teacher/${createData.id}`);
    } catch (requestError) {
      ...
      <header className="premium-lesson-topbar">
        <div className="mx-auto flex max-w-[1180px] items-center justify-between px-4 py-4 sm:px-6">
          ...
          <div className="text-right">
            ...
            <h1 className="premium-lesson-title text-base font-semibold sm:text-lg">{UNIT_4_4_COURSE_TITLE}</h1>
          </div>
        </div>
      </header>
      ...
      <main className="premium-lesson-main mx-auto max-w-[1180px] px-3 py-4 sm:px-6 sm:py-8">
        <div className="mt-4 grid gap-4 md:grid-cols-3">
          {showTeacherSection ? (
            <section className="premium-lesson-panel p-5">
              ...
              <p className="premium-lesson-muted mt-2">
                自动克隆 4-4 预置教案，生成课堂码并进入“多目标权衡与控制器优化设计”精品课堂。
              </p>
              <button
                type="button"
          ...
          <section className="premium-lesson-panel p-5">
            ...
            <p className="premium-lesson-muted mt-2">以演示模式进入学生端，先预览 14 个课堂环节与导学资料。</p>
            <Link
              href={`/interactive-learning/courses/${UNIT_4_4_ROUTE_SEGMENT}/student/demo`}
        ...
        <section className="premium-lesson-panel mt-4 px-5 py-5">
          <div className="premium-lesson-kicker">4-4 · Fixed-Structure Optimization</div>
          <h2 className="premium-lesson-title mt-2 text-3xl font-semibold sm:text-4xl">{UNIT_4_4_COURSE_TITLE}</h2>
          <p className="premium-lesson-muted mt-3 max-w-3xl sm:text-base">{UNIT_4_4_COURSE_DESCRIPTION}</p>
          <p className="premium-lesson-muted mt-3 max-w-3xl">
            本课承接 <strong>4-3</strong> 的第一版方案，把多目标拉扯、自由目标表达、候选族比较与 <strong>Pareto front</strong> 显性化，再把候选移交给 <strong>4-5</strong> 做工程复核。
          </p>
          <div className="mt-4 flex flex-wrap gap-2 text-xs">
            {['多目标拉扯', '自由目标', '归一化基准', '候选族', 'Pareto front', '边界案例'].map((item) => (
