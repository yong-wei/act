...
export function UNIT_4_4TeacherPage({
  ...
  const teacherSyncState = useMemo(() => {
    ...
    return (latestRecord?.data as UNIT_4_4TeacherCourseSyncState | null) ?? null;
  }, [teacherStates]);
  ...
  useEffect(() => {
    if (!shouldPostUNIT_4_4TeacherSync({ loadingSession, teacherViewHydrated })) return;
  ...
  const studentStates = useMemo(() => {
    return courseStates
      .map((record) => {
        ...
        if (data.kind !== 'unit44_student_state' || data.version !== 1) return null;
        return {
          studentName: data.studentName || record.user?.name?.trim() || '未命名学生',
          state: data as UNIT_4_4StudentCourseState,
        };
      })
      .filter(Boolean) as Array<{ studentName: string; state: UNIT_4_4StudentCourseState }>;
  }, [courseStates]);
  ...
  const currentResponses = useMemo(() => {
    return studentStates
      ...
      .filter(Boolean) as Array<{ studentName: string; response: UNIT_4_4StudentCourseState['responses'][string] }>;
  }, [step.id, studentStates]);

  const handlePatchCurrentStep = useCallback(
    async (nextIndex: number) => {
      const nextStep = UNIT_4_4_LESSON_STEPS[nextIndex];
      trackStepLeave(step.id, { nextStepId: nextStep.id });
      await patchCurrentStep(nextIndex, {
        currentItemId: nextStep.id,
        currentStage: UNIT_4_4_STAGE_MAP[nextStep.stage],
      });
  ...
  const handleEndSession = useCallback(async () => {
    ...
    try {
      await finalizeUNIT_4_4TeacherSession({
        finishSession,
          ...
          <UNIT_4_4TeacherActivitySummary
            ...
            onToggleRelease={() =>
              setLocalReleasedActivities((prev) => ({
                ...(prev ?? (teacherSyncState as UNIT_4_4TeacherCourseSyncState | null)?.releasedActivities ?? {}),
                [step.id]: !(prev?.[step.id] ?? (teacherSyncState as UNIT_4_4TeacherCourseSyncState | null)?.releasedActivities?.[step.id]),
              }))
            }
            onToggleBrowse={() =>
              setLocalBrowseEnabled((prev) => ({
                ...(prev ?? (teacherSyncState as UNIT_4_4TeacherCourseSyncState | null)?.browseEnabled ?? {}),
                [step.id]: !(prev?.[step.id] ?? (teacherSyncState as UNIT_4_4TeacherCourseSyncState | null)?.browseEnabled?.[step.id]),
              }))
            }
            onToggleAnswerVisible={() =>
              setLocalRevealedAnswers((prev) => ({
                ...(prev ?? (teacherSyncState as UNIT_4_4TeacherCourseSyncState | null)?.revealedAnswers ?? {}),
                [step.id]: !(prev?.[step.id] ?? (teacherSyncState as UNIT_4_4TeacherCourseSyncState | null)?.revealedAnswers?.[step.id]),
              }))
            }
            onAdvanceReveal={() =>
              setLocalTeacherRevealProgress((prev) => ({
                ...(prev ?? (teacherSyncState as UNIT_4_4TeacherCourseSyncState | null)?.teacherRevealProgress ?? {}),
                [step.id]: (prev?.[step.id] ?? (teacherSyncState as UNIT_4_4TeacherCourseSyncState | null)?.teacherRevealProgress?.[step.id] ?? 0) + 1,
              }))
            }
            onResetReveal={() =>
              setLocalTeacherRevealProgress((prev) => ({
