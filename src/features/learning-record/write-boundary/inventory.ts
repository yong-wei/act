...
export const WRITE_BOUNDARY_ROWS: readonly WriteBoundaryRow[] = [
  ...
  row('producer.control-correction.path', 'path-rounds', 'src/lib/control-correction-path-rounds.ts', 'personalization', 'direct', 'writeKnowledgeScopedLearningFacts', 'control-correction-path:/learning-path:', 'path choice identity', 'server choice time', 'student-private', 'none in this change', 'control-correction plugin', 'control-correction-path-rounds.test.ts', 'C6 migrates adapter', 'exception-c6-c7'),
...
export const WRITE_BOUNDARY_DELETION_LEDGER = [
  ...
  { id: 'redis.core-shouldMaterialize', status: 'isolated-this-change', replacement: 'API direct ingest owns those identities' },
] as const;
