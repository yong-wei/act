...
function normalizeGrading(value: unknown): Submission['grading'] { if (!value || typeof value !== 'object') return null; const row = value as Record<string, unknown>; const snapshotId = stringValue(row.snapshotId); return snapshotId ? { snapshotId, state: stringValue(row.state), operationState: stringValue(row.operationState) || null, confirmationId: stringValue(row.confirmationId) || null, releaseId: stringValue(row.releaseId) || null } : null; }
function stringValue(value: unknown) { return typeof value === 'string' ? value : ''; }
function numberValue(value: unknown) { return typeof value === 'number' && Number.isFinite(value) ? value : 0; }
export function isAiGradingCandidate(submission: Pick<Submission, 'state' | 'submittedRequiredCount' | 'grading' | 'gradingDiagnostic'>) { return ['SUBMITTED', 'IN_PROGRESS'].includes(submission.state) && submission.submittedRequiredCount > 0 && !submission.grading && (submission.gradingDiagnostic?.canStartAi ?? true); }
