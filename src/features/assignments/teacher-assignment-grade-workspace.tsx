...
type QuestionProjection = { questionId?: string; snapshotItemId?: string; status?: string; score?: number | null; comment?: string | null; failureReason?: string | null };
...
type GradePayload = {
  grade?: { version?: number; state?: string; totalScore?: number | null; questionProjection?: unknown[] };
  questions?: Array<{ snapshotItemId?: string; questionId?: string; promptSnapshot?: unknown; sourceHistory?: Array<{ source?: string; state?: string; draftTotalScore?: number | null; overallComment?: string }>; approvalHistory?: Array<{ source?: string; questionTotal?: number; overallComment?: string }> }>;
};
...
export function TeacherAssignmentGradeWorkspace({ assignmentId, submissionId, snapshotId, initialConfirmationId }: { assignmentId: string; submissionId: string; snapshotId: string; initialConfirmationId: string | null }) {
  ...
  return <main className="surface-page min-h-screen px-4 py-6 md:px-8" data-assignment-submission-grade-workspace="true"><div className="mx-auto max-w-5xl space-y-6">
    ...
    {loadState === 'ready' && grade ? <><section className="rounded-xl border border-slate-800 bg-slate-900/60 p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm text-slate-400">当前状态</p><p className="mt-1 text-lg font-semibold text-white">{gradeStateLabel(grade.state)}</p></div><p className="rounded-xl bg-cyan-500/10 px-4 py-2 text-lg font-semibold text-cyan-200">{grade.totalScore == null ? '总分待定' : `总分 ${grade.totalScore}`}</p></div></section><section className="space-y-4">{payload.questions?.map((question, index) => { const projection = projections.find((item) => item.questionId === question.questionId); const itemId = question.snapshotItemId ?? ''; const conclusion = conclusions[itemId] ?? { kind: 'UNANSWERED' as const, scoreEffect: '0', reason: '' }; const returnDraft = returns[itemId] ?? { reason: '', deadlineAt: defaultReturnDeadline() }; const unresolved = projection?.status !== 'COMPLETE'; const canReturn = itemId && projection?.status === 'COMPLETE' && ['CONFIRMED', 'RELEASED'].includes(grade.state ?? ''); return <article key={itemId || index} className="rounded-xl border border-slate-800 bg-slate-900/60 p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="font-semibold text-white">第 {index + 1} 题</h2><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-slate-300">{promptText(question.promptSnapshot)}</p></div><span className="rounded-full bg-slate-800 px-3 py-1 text-sm text-slate-200">{projection?.score == null ? projection?.failureReason ? String(projection.failureReason) : '待处理' : `${projection.score} 分`}</span></div>{projection?.comment ? <p className="mt-4 whitespace-pre-wrap text-sm text-slate-300">{String(projection.comment)}</p> : null}<div className="mt-4 grid gap-3 md:grid-cols-2"><div className="rounded-lg bg-slate-950/60 p-3"><p className="text-xs text-slate-500">评分来源</p><p className="mt-1 text-sm text-slate-200">{question.approvalHistory?.map((item) => `${gradingSourceLabel(item.source)} · ${item.questionTotal ?? 0} 分`).join('；') || '尚无已确认评分'}</p></div><div className="rounded-lg bg-slate-950/60 p-3"><p className="text-xs text-slate-500">批改运行</p><p className="mt-1 text-sm text-slate-200">{question.sourceHistory?.map((item) => `${gradingSourceLabel(item.source)} · ${gradingRunStateLabel(item.state)}`).join('；') || '尚未运行'}</p></div></div>{canReturn ? <div className="mt-4 rounded-lg border border-cyan-500/30 bg-cyan-950/20 p-4"><p className="text-sm font-medium text-cyan-100">退回该题补交后，学生的新版本将重新进入批改、确认和发布流程。</p><div className="mt-3 grid gap-3 md:grid-cols-[1fr_13rem_auto]"><input aria-label={`第 ${index + 1} 题补交原因`} value={returnDraft.reason} onChange={(event) => setReturns((current) => ({ ...current, [itemId]: { ...returnDraft, reason: event.target.value } }))} placeholder="请填写退回补交原因" className="min-h-10 rounded-lg border border-slate-600 bg-slate-950 px-3 text-sm text-slate-100 placeholder:text-slate-500" /><input aria-label={`第 ${index + 1} 题补交截止时间`} type="datetime-local" value={returnDraft.deadlineAt} onChange={(event) => setReturns((current) => ({ ...current, [itemId]: { ...returnDraft, deadlineAt: event.target.value } }))} className="min-h-10 rounded-lg border border-slate-600 bg-slate-950 px-3 text-sm text-slate-100" /><button type="button" onClick={() => void returnQuestion(itemId)} disabled={busy !== null} className="min-h-10 rounded-lg border border-cyan-400/70 px-3 text-sm text-cyan-100 disabled:opacity-50">退回补交</button></div></div> : null}{unresolved && itemId ? <div className="mt-4 rounded-lg border border-amber-500/30 bg-amber-950/20 p-4"><p className="text-sm font-medium text-amber-100">该题尚未形成可确认评分。可转人工批改，或记录明确结论。</p><div className="mt-3 grid gap-3 md:grid-cols-[auto_9rem_1fr_auto]"><select aria-label={`第 ${index + 1} 题结论类型`} value={conclusion.kind} onChange={(event) => setConclusions((current) => ({ ...current, [itemId]: { ...conclusion, kind: event.target.value as ConclusionDraft['kind'] } }))} className="min-h-10 rounded-lg border border-slate-600 bg-slate-950 px-3 text-sm text-slate-100"><option value="UNANSWERED">未作答</option><option value="EXEMPT">不计分</option></select><input aria-label={`第 ${index + 1} 题结论分值`} type="number" min="0" step="0.1" value={conclusion.scoreEffect} onChange={(event) => setConclusions((current) => ({ ...current, [itemId]: { ...conclusion, scoreEffect: event.target.value } }))} className="min-h-10 rounded-lg border border-slate-600 bg-slate-950 px-3 text-sm text-slate-100" /><input aria-label={`第 ${index + 1} 题结论原因`} value={conclusion.reason} onChange={(event) => setConclusions((current) => ({ ...current, [itemId]: { ...conclusion, reason: event.target.value } }))} placeholder="请填写教师结论依据" className="min-h-10 rounded-lg border border-slate-600 bg-slate-950 px-3 text-sm text-slate-100 placeholder:text-slate-500" /><button type="button" onClick={() => void conclude(itemId)} disabled={busy !== null} className="min-h-10 rounded-lg border border-amber-400/70 px-3 text-sm text-amber-100 disabled:opacity-50">记录结论</button></div></div> : null}</article>; })}</section></> : null}
  </div></main>;
}
