...
function TaskBadge({ task }: { task: AuthoringApiTask }) {
  const className = "inline-flex max-w-full items-center gap-1 rounded-md border border-slate-700 bg-slate-900/60 px-2 py-1 text-[11px] text-slate-300";
  const commonProps = {
    className,
  ...
  const content = (
    <>
      ...
      <span className="text-slate-500">{authoringApiTaskStatusLabel(task.status)}</span>
    </>
  );
  return task.href ? (
