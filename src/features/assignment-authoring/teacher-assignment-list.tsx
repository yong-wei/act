...
function AssignmentCard({ item }: { item: ReviewAwareAssignment }) {
...
export function normalizeTeacherAssignmentListItem(value: unknown): ReviewAwareAssignment {
  ...
  return { id: String(row.id), state: String(row.state) as TeacherAssignmentListItem['state'], updatedAt: String(row.updatedAt), latestRevision: revision ? { id: String(revision.id), revisionNumber: Number(revision.revisionNumber), version: Number(revision.version), title: String(revision.title), state: String(revision.state) as 'DRAFT' | 'PUBLISHED', audiences: audiences.map((entry) => ({ classId: String(entry.classId), availableAt: String(entry.availableAt), dueAt: String(entry.dueAt) })) } : null, submissionCount: safeCount(reviewSummary.submissionCount ?? row.submissionCount), pendingReviewCount: safeCount(reviewSummary.pendingReviewCount ?? row.pendingReviewCount), reviewedCount: safeCount(reviewSummary.reviewedCount ?? row.reviewedCount), nextReview: nextReview && nextReview.submissionId && nextReview.questionId ? { submissionId: String(nextReview.submissionId), questionId: String(nextReview.questionId), reviewId: nullableId(nextReview.reviewId), gradingRunId: nullableId(nextReview.gradingRunId) } : null, submissionCountsAvailable: reviewSummary.submissionCount !== undefined || row.submissionCount !== undefined };
}
