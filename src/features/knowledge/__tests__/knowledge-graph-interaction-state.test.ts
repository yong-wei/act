...
describe('knowledge graph interaction state stability', () => {
  ...
  it('keeps hover and selection out of graph filtering, membership, and layout derivation', () => {
    ...
    expect(rendererSource).toContain('}, [nodes, links, relayoutVersion, layoutState, expandedNodeIds, expandedDirectLinks, activationSequenceByCenterId, materializedNodeIds]);');
    ...
    expect(canvasSource).toContain('}, [nodes, links, relayoutVersion, layoutState, expandedNodeIds, expandedDirectLinks, activationSequenceByCenterId, materializedNodeIds]);');
