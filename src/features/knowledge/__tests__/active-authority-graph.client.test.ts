...
describe('active Authority knowledge workspace client boundary', () => {
  ...
  it('keeps the active component browser-safe and free of server or Legacy resolver imports', () => {
    ...
    expect(captureSource).toContain('__ACT_KNOWLEDGE_PRODUCT_QA_ACTIVE_IDENTITY_TOKENS__');
    ...
    expect(captureSource).toContain('createSensitiveValueMatcher(await readActiveSurfaceIdentityTokens(page))');
