...
新建整课或改变教学结构时读取以下相关真源；局部修订只读取目标内容、对应术语和直接受影响的约束，未变证据可复用：
...
1. 涉及知识节点、卡片身份、图谱或 authoring/runtime 同步时才运行全量知识同步检查；纯文风、排版或局部文字修改不触发同步：
   ```bash
   python3 .agents/skills/lesson/scripts/sync_runtime_knowledge.py --check
   ```
2. 若发现影响本次写入的冲突，停止该写入并展示证据；无关冲突单列，不阻塞独立工作，不覆盖作者态真源。
...
4. 本次已授权同步且仅有可安全合并项时，可先运行 `python3 .agents/skills/lesson/scripts/sync_runtime_knowledge.py --apply`，再重新运行 `--check` 确认无冲突。
...
1. 确认本次需要的前置检查已完成；不涉及同步时无需运行同步命令。
...
> **读取参考文件**：`step3-handout.md`

固定顺序：
1. 先生成学生版 `handout.md`（按逻辑分批写入，见 step3-handout.md）
2. 导出并抽查 `handout.pdf`
...
- 学生版讲义按逻辑段落分批写入，保证例题、案例和收尾完整。用户要求阶段确认时等待确认；已授权整篇创作时连续完成，不默认在中途暂停。
- 教师版 `teacher-handout.md` 必须以学生版 clean brief 和定稿内容为基础重写，不得把导出门槛、检查表、流程提示直接露出成课堂讲义句子。

### Step 3｜知识图谱与知识卡片草案

> **读取参考文件**：`step4-knowledge-graph.md`
...
> **读取参考文件**
> - `step7-multimedia.md`
> - `step7-multimedia-codegen.md`
> - `step7-multimedia-ai.md`
