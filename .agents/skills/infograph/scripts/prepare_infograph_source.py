...
def visual_focus_for_node(node_name: str, groups: list[str], lesson_id: str) -> str:
    ...
    if '经典复合控制边界' in text:
        return '用一条反馈主结构贯穿画面，左右分出参考前馈、扰动前馈和实现层保护三类支路，表达本课只处理经典复合控制边界，不把串级、MPC 或扰动观测器混入主体。'
    if '参考前馈与给定滤波分工' in text:
        return '用参考信号 r 同时进入参考前馈支路和给定滤波 Q_f 支路，反馈主结构只看到平滑后的 r_f，表达参考前馈负责提前动作，给定滤波负责平顺反馈命令。'
    if '扰动前馈保守落地' in text:
        return '用可测扰动 d、理想对消 F_d^*、75% 强度和一阶低通四个层次，表达扰动前馈从理想模型转为保守可落地方案，并保留反馈修正余量。'
    if '复合方案首轮记录表' in text:
        return '用一张工程记录表连接反馈主结构、参考前馈、扰动前馈、给定滤波、执行器保护、抗饱和和验证证据，表达首轮记录是后续参数优化的输入。'
    if '实现层复合保护' in text:
        return '用控制器原始输出经过限幅、斜率限制和抗积分饱和回算后进入执行器，表达实现层保护决定复合控制方案能否在真实执行器约束下恢复。'
    if '按输入补偿前馈' in text:
...
def visual_asset_brief_for_node(node_name: str, groups: list[str], lesson_id: str) -> str:
    ...
    if '实现层复合保护' in text:
        return (
            '主视觉对象：u_raw 从控制器输出后依次经过限幅、斜率限制和执行器，再把 u_act-u_raw 回送到抗积分饱和回算通道。\n'
            '工程场景：舵角幅值边界、舵速边界、饱和恢复曲线三联小窗，显示线性控制律必须被实现层保护包住。\n'
            '核心图示：u_raw、u_act、sat、rateLimit、T_aw 五个元素。\n'
            '构图方式：左侧控制器，中央保护层剖面，右侧恢复证据小窗；不要画成泛泛的安全盾牌或纯流程图。'
        )
    if '按输入补偿前馈' in text:
        return (
