...
首次实现或改变教学内容、契约与导出链路前，必须确认对应内容已经经过 `lesson-content-review` 课程审查技能处理；若审查报告、runtime 导出或媒体处理缺失，先回到课程审查链路补齐。

默认迁移路径：
1. 能脚本化迁移的内容，优先补到 `course-content/scripts/export-runtime.sh`（内部调用 `python3` 脚本）完成导出。
2. 不能自动迁移的内容在作者态补齐并修复导出入口；不得在 runtime 手写独立教学真源。
...
续接既有工作时读取相关 `notes/<lesson>.md`；笔记缺失不阻塞明确的局部修复，需要交接时再记录。

## 工作流

### 1. 锁定设计源

完整实现时读取以下相关资料；局部修复按受影响内容选择输入，复用未变证据：
...
更多规则见 [references/media-and-path-rules.md](media-and-path-rules.md)。
...
详细规则与常见问题见 [references/runtime-code-generated-media.md](runtime-code-generated-media.md)。
...
- 设计稿对照与课程笔记更新规则见 [references/verification-and-note-update.md](verification-and-note-update.md)
- 浏览器闭环验收、双子代理逐页流程、测试账号与详细覆盖项见 [references/closed-loop-browser-validation.md](closed-loop-browser-validation.md)
- 入口页模式与 runtime 媒体索引契约见 [references/runtime-entry-page-pattern.md](runtime-entry-page-pattern.md) 与 [references/runtime-media-index-contract.md](runtime-media-index-contract.md)
...
- 完整课程验收须有真实教师/学生角色验证记录，再运行严格脚本。只有用户已授权且独立验证有收益时采用子代理；否则主线程使用隔离的浏览器身份执行。接受文件如实记录执行者、范围和证据，不伪造独立审核。局部修复复用仍有效的记录，更新受影响部分。
- 若实际验收指出契约漏实现、旧口径回潮、埋点缺失、页内 AI 入口或曲线/示意图静态降级，必须先修复，再写 `notes/interactive-implementation-acceptance.json`，更不得宣称完成
- 若采用双子代理浏览器验收，主代理只需读取总则，并按角色给子代理分发各自规范文件：
  - 教师端子代理读取 [references/browser-validation-teacher-subagent.md](browser-validation-teacher-subagent.md)
  - 学生端子代理读取 [references/browser-validation-student-subagent.md](browser-validation-student-subagent.md)
- 委派时按角色加载规范；主线程自行验收时按目标角色读取对应规范，不因未委派省略角色隔离。
...
- `media-and-path-rules.md`
  - 媒体缺失时的占位符、路径、命名与输出规范
- `runtime-code-generated-media.md`
  - 代码直出 / SVG 绘制资源的目录、生成、字体、验证与常见问题
- `verification-and-note-update.md`
  - 设计核对流程、验证清单和课程笔记更新规则
- `closed-loop-browser-validation.md`
  - 设计对齐后的浏览器闭环验收流程、双子代理分工、测试账号与逐页验收规则
- `browser-validation-teacher-subagent.md`
  - 教师端浏览器验收子代理规范：登录、建课、翻页、显示答案、结束课堂与对话框处理
- `browser-validation-student-subagent.md`
