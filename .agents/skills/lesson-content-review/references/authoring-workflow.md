...
- 完整课程审查中，所有课型都必须审 `design/{unit}-interactive-page.md` 与 `design/{unit}-interactive-contract.yaml`（若已建立）；互动页不是可有可无的附属稿，而是课堂 PPT 的互动延伸板。
...
- 完整课程审查结论分层输出，且每层都要给出 `通过 / 需修订 / 阻塞` 状态：**结构正确性**、**互动页覆盖**、**事实正确性**、**科学合理性**、**确定性结论验证**。
...
完整课程审查读取以下相关输入；用户指定局部修订时只检查受影响内容和直接依赖，不把局部检查声称为整课放行：
...
详细核对项见 `boppps-coverage-review.md`。
...
- 重点检查：
  ...
  - manifest-first 课程导出后是否能通过模块消费审计：
    - 审计入口为 `python3 .agents/skills/interactive-design/scripts/audit_interactive_manifest.py --lesson <lesson> --write-review`
...
- 详细规则与元件级核对表见 `interactive-implementation-review.md`
...
详细规则见 `knowledge-card-review.md`。
...
runtime 契约见 `runtime-output-contract.md`。
...
- [ ] 若为 manifest-first 课程，已运行 `.agents/skills/interactive-design/scripts/audit_interactive_manifest.py` 并完成模块消费审计：content 模块非空、activity 模块不进正文、活动题面不重复、公式/媒体/图后说明均被消费
