...
图形与缓存适配的复现应包含真实运行协议：当前 ForceGraph `onZoom` 的 `x/y` 已是世界坐标中的视图中心，不能再次按原始 D3 平移换算；DOM 标签存在也不能证明 WebGL mesh 存在。相机测试应覆盖切换维度和改变视口后的恢复，必要时比较真实投影。版本缓存的失败关闭检查必须覆盖构建、内存命中、磁盘命中的共同返回边界。
