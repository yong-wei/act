...
1. **Select the change**
   ...
   **Load current archive inputs before the existing archive checks:**
   ...
   ```bash
   rtk proxy npx --yes --package @fission-ai/openspec@1.13.0 openspec instructions archive --change "<name>" --json
   ```
...
2. **Check artifact completion status**
   ...
   Run `rtk proxy npx --yes --package @fission-ai/openspec@1.13.0 openspec status --change "<name>" --json` to check artifact completion.
...
4. **Assess delta spec sync state**
   ...
   **When the user has not already chosen or authorized the normal sync-and-archive operation, prompt options:**
   - If changes needed: "Sync now (recommended)", "Archive without syncing"
   - If already synced: "Archive now", "Sync anyway", "Cancel"

   If normal complete archive is already authorized, sync when needed and archive when already synced. For an explicit exceptional choice, route on the answer:
   - "Cancel" — stop, do not archive
   - "Archive without syncing" or "Archive now" — proceed to archive
   - "Sync now" or "Sync anyway" — sync, then verify (below)
   - An unresolved choice about an exception — ask rather than assuming the exception

   Before a selected sync writes any main spec, run
   `rtk proxy npx --yes --package @fission-ai/openspec@1.13.0 openspec instructions specs --change "<name>" --json` once with the same
...
5. **Perform the archive**
   ...
   Generate the target name: use the change name as-is when it already starts with a `YYYY-MM-DD-` prefix; otherwise prepend the current date as `YYYY-MM-DD-<change-name>`. Never stack a second date (same rule as `rtk proxy npx --yes --package @fission-ai/openspec@1.13.0 openspec archive`).
...
- Use artifact graph (rtk proxy npx --yes --package @fission-ai/openspec@1.13.0 openspec status --json) for completion checking
