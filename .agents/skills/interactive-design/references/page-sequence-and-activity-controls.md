...
3. **步骤 3：前测**
   - 固定为客观题。
   - 可混合单选、多选、排序、判断等客观题型。
   - 只考察进入本单元学习所必需的基础知识点，例如前序概念、数学工具、读图规则或基本计算。
   - 禁止考察本单元将要讲解的新概念、新方法、新结论或本单元例题中的核心推理。
   - 不得设置主观简答、开放论述或需要本单元新知识才能完成的综合题。
4. **中间步骤**
   - 按讲义章节顺序逐渐展开。
...
5. **倒数第二步：后测**
   ...
   - 总题数不超过 3 题，最多 1 题为主观题；其余题目采用单选、多选、排序、判断等客观题。
6. **最后一步：总结**
   - 固定独立成页。
   - 顶部放课程信息图。
   - 讨论主要内容、局限性、重点难点，并可留下拓展思考。
   - 学生页面必须给出学生个人课堂表现统计数据。
   - 教师页面必须给出班级整体表现统计数据。
...
- 步骤名称必须采用一般教科书式陈述标题，直接概括对象、方法、原理、分析过程或结论范围。
- 禁止使用问句作为步骤名称，例如“为什么……”“XX如何……”“XX是否……”。
- 禁止使用否定句作为步骤名称，例如“XX不是XX”“不能……”“不要……”。
- 页面正文、导入漫画和作答题面可以保留问题意识；步骤名称本身必须是稳定的内容型标题。
- 推荐形式：
  - `单位阶跃响应与超调量读图`
  - `频域裕度的图形读取`
  - `根轨迹主导极点分析`
...
- 每个页面顶端必须单独设置页面标题模块，显示步骤名称，并用一句或一小段文案说明本页主要内容。
- 页面标题模块不计入讲义章节覆盖，也不替代本页正文证据、公式、图形或作答题面。
...
- 图片下方文案不得直接使用页面标题、模块标题、图片标题或文件名。可以不写下方文案；一旦写，就必须描述图片具体内容、指出可观察证据、说明读图顺序，或给出基于图片的分析判断。
...
- 后测题目不超过 `3` 题，最多 `1` 题为主观题；其余题目必须为单选、多选、排序、判断等客观题。
- 总结页顶部必须放课程信息图；信息图之后再总结主要内容、局限性、重点难点和拓展思考。
- 总结页学生端必须显示个人课堂表现统计，例如已浏览页面、已提交互动、前后测完成情况、参数探索提交或关键能力达成摘要。
- 总结页教师端必须显示班级整体表现统计，例如提交率、题目正确率或选项分布、参数探索覆盖、常见误判标签和目标达成概况。
