...
name: homework
description: >
  当用户按题号（如 T1-1）请求生成作业题或考试题时调用，尤其当需要另起 Codex 实例运行"出题-裁判-作答"多智能体工作流，并带反作弊文件访问限制，最终产出题面、答案、评分标准和参考作答时。
  同样适用于以下场景，即使用户未明确说"出题"：
  - 用户说"T1系列完成了"、"汇总T2系列作业"、"生成T3作业文档"等，触发系列汇编工作流；
  - 用户说"检查T2-3"、"更新T3-1题面"、"T1-2图片有问题"等，触发题目检查/更新工作流。
---
