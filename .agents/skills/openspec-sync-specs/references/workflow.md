...
4. **For each delta spec, apply changes to main specs**
   ...
   - If archive invoked this workflow inline and supplied a valid snapshot from
     `rtk proxy npx --yes --package @fission-ai/openspec@1.13.0 openspec instructions specs --change "<name>" --json`, reuse it and do not
   ...
   c. **Apply changes intelligently**:
      ...
      **`## Purpose` in the delta:**
      - The main spec already has one and it is authoritative - leave it alone
        (this is what `rtk proxy npx --yes --package @fission-ai/openspec@1.13.0 openspec archive` does; it warns and moves on)

   d. **Create new main spec** if capability doesn't exist yet:
      - Create `<planningHome.root>/openspec/specs/<capability-path>/spec.md`
      - Add Purpose section: copy the delta's `## Purpose` body verbatim when it has one
        (this is what `rtk proxy npx --yes --package @fission-ai/openspec@1.13.0 openspec archive` does); only write a brief TBD placeholder when it does not
      - Add Requirements section with the ADDED requirements
      - Follow the **Main Spec Format Reference** below

5. **Validate updated main specs**

   Run `rtk proxy npx --yes --package @fission-ai/openspec@1.13.0 openspec validate --specs` with the same selected-root flags used earlier.
...
- A MODIFIED block carries the whole requirement - body plus every scenario that survives the change. `rtk proxy npx --yes --package @fission-ai/openspec@1.13.0 openspec validate` and `rtk proxy npx --yes --package @fission-ai/openspec@1.13.0 openspec archive` both reject one that drops a scenario the main spec still has.
...
- Resolve routine details from the selected artifacts; ask only for material ambiguity.
