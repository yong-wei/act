...
**Planning boundary**: A propose-only request authorizes planning only. If the user explicitly authorized planning followed by implementation, finish and validate the artifacts, then enter apply within that authorization. Do not infer deployment, publication, or activation permission.
...
Enter apply when implementation is explicitly authorized, including authorization already provided in the same request.
...
2. **Load project context**
   ...
   Run `rtk proxy npx --yes --package @fission-ai/openspec@1.13.0 openspec context --json` from the current working directory (or `rtk proxy npx --yes --package @fission-ai/openspec@1.13.0 openspec context --json --store "<store-id>"` when a registered store was explicitly selected). Use the returned `root.path` as the authoritative OpenSpec root. If context reports `no_openspec_root`, stop without creating or changing any files. Offer `rtk proxy npx --yes --package @fission-ai/openspec@1.13.0 openspec init` and wait for the user to request initialization. Do not initialize automatically or run `rtk proxy npx --yes --package @fission-ai/openspec@1.13.0 openspec new change`. After initialization, rerun this context check before continuing. For any other context failure, stop and report the error; do not fall back to the current directory or run later OpenSpec commands without the selected store.
...
3. **Determine the workflow schema**
   ...
   **Use a different schema only if the user:**
   ...
   - Asks to "show workflows" or asks "what workflows" exist → resolve the authoritative root by running `rtk proxy npx --yes --package @fission-ai/openspec@1.13.0 openspec context --json` from the current working directory. If the user explicitly selected a registered store, use `rtk proxy npx --yes --package @fission-ai/openspec@1.13.0 openspec context --json --store "<store-id>"`. Then run `rtk proxy npx --yes --package @fission-ai/openspec@1.13.0 openspec schemas --json` with its working directory set to the returned `root.path` and let them choose. This preserves roots selected by a local `store:` pointer or the global `defaultStore`; when a registered store was explicitly selected, append `--store "<store-id>"` to `rtk proxy npx --yes --package @fission-ai/openspec@1.13.0 openspec schemas --json` as well. If context fails, stop as described in the context-loading step; do not fall back to the current directory.
...
4. **Create the change directory**
   ...
   ```bash
   rtk proxy npx --yes --package @fission-ai/openspec@1.13.0 openspec new change "<name>"
   ```

   Using an explicitly requested schema:
   ```bash
   rtk proxy npx --yes --package @fission-ai/openspec@1.13.0 openspec new change "<name>" --schema "<schema-name>"
   ```
...
6. **Create every artifact in the required set**
   ...
   b. **Continue until every artifact in the required set exists (not just `apply.requires`)**
      - After creating each artifact, re-run `rtk proxy npx --yes --package @fission-ai/openspec@1.13.0 openspec status --change "<name>" --json`
...
7. **Show final status**
   ```bash
   rtk proxy npx --yes --package @fission-ai/openspec@1.13.0 openspec status --change "<name>"
   ```
...
- Follow the `instruction` field from `rtk proxy npx --yes --package @fission-ai/openspec@1.13.0 openspec instructions` for each artifact type - it is the authoritative guidance, even for familiar artifact names
...
**Guardrails**
- A propose-only request authorizes planning only. If planning and implementation were both explicitly requested, complete and validate planning before entering apply. Do not infer claim, publication, deployment, activation or other external-action authority.
- Create every artifact the apply phase transitively depends on, not just the ids listed in `apply.requires`
- Read required dependency artifacts; reuse content known to be unchanged and refresh when another writer or newer revision may have changed it.
