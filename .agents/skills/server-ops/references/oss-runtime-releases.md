...
- `runtime:publish` 只处理工作树扫描、Δ 哈希、条件 PUT 与不可变 manifest；条件 PUT 只有解析出结构化 OSS 冲突（JSON `statusCode` 412/409 或 `errorCode`/`Code` 为 `FileAlreadyExists`/`PreconditionFailed`，或 XML `<Code>`）才算 CAS 命中，任意诊断文本不算。`runtime:activate` / `runtime:rollback` 在 ECS 的 Python 3.6 上读取 ossfs blob 根（`--blob-root`）和单份已下载 manifest，核对 Δ Blob 与 sentinel，先 bind helper 再按 manifest 把逻辑文件链到 helper 叶（不对未改路径做 FUSE `stat`），对候选视图跑 smoke，再在 `.act-runtime-selection.lock` 下先改 `current`/`live`/`previous` 符号链接、再写 `pointers.json` 投影，并在同一事务写出 `data/runtime/act-runtime-active-receipt.json`。读指针以符号链接为准。缺少投影时必须从现有 `current` 符号链接迁移 outgoing release；旧 v2 可选 `source` 必须计入 manifest 摘要。`blob-views` 加 `--blob-root` 时，populate 前要把 ossfs blob 根只读 bind 到目标 helper。生产 `--state-dir` 必须是 `blob-views` 根。`--reload-consumers` 失败必须恢复原指针和 active receipt。二者都不得构建镜像、传输 image tar、处理数据库、Prisma、Nginx、systemd 或完整 runtime `rsync`。
