...
- [x] 1.3 Verify the declared protocol is `actkg-public-bundle/2` and Schema
  ...
  当前结果：已 pin `control-theory-engineering-v0.22-r5` / source-r8（bundle digest `98f2d5b183…`）。Schema 哈希与 v0.18 相同；r5 只刷新 validation-report / manifest / SHA256SUMS，`relation_type_count=9` 与三个投影谓词并集一致。v2 适配器 shadow admission 通过。生产选择器未改，候选 snapshot 尚未写出。
