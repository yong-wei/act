...
The system SHALL rebuild app and worker from the same declared image and explicit `ACT_KNOWLEDGE_DEPLOYMENT_MODE=cutover` after verifying the transferred image identity. It SHALL reject mixed image revisions or mixed knowledge deployment modes.
...
The system SHALL retain a refresh receipt containing the new and previous image identities, provenance references, verified selector identities, pre/post marker/receipt/journal digests and post-refresh health evidence.
