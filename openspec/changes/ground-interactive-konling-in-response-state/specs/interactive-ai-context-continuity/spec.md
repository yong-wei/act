...
#### Scenario: Client submits forged tutoring state
- **WHEN** a client sends altered tools, prompt extensions, response fields, completion flags or answer-disclosure flags
- **THEN** the server SHALL reject or ignore those values before model invocation
- **AND** the values SHALL NOT enable answer checking, disclose answers or expand the learner's authorized context.

#### Scenario: Response state changes between turns
- **WHEN** the learner saves, submits or receives teacher disclosure before sending the next question
- **THEN** the next request SHALL use the latest server-validated response state
