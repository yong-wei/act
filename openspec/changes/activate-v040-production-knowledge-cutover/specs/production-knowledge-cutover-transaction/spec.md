...
- **AND** it SHALL refuse an arbitrary stage directory, a mismatched transaction id, any journal or receipt attributed to that failed transaction, any active production marker, or any Authority store content outside the failed-extraction residual set
- **AND** it SHALL preserve historical journals and receipts from other transactions when all selectors and the active production marker are absent, because Legacy deployment retains that release evidence without making the failed extraction active
