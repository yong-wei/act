...
- [ ] 1.1 Lock the `control-theory-engineering-v0.22` composite release
  envelope and record the Aggregate v0.22 Component Manifest as the sole
  version truth for this qualification run.
- [ ] 1.2 Verify manifest completeness and identity: Integration v0.20,
...
- [ ] 2.1 Audit the candidate values of all five production selectors
...
- [ ] 2.2 Verify domain catalog membership completeness from materialized
...
- [ ] 2.3 Verify Chinese display coverage against terminology v0.5 and check
...
- [ ] 3.1 Rebuild the Authority domain shard set twice from the same locked
  envelope and prove the rebuilds match.
- [ ] 3.2 Preserve a snapshot of the previous production composite release
  envelope and demonstrate it is restorable as rollback evidence.
- [ ] 3.3 Emit the qualification verdict as candidate-auditable evidence only
  and prove every production selector still equals its pre-run value.
- [ ] 3.4 Run affected checks (lint, typecheck, relevant tests) and
