...
Runtime lesson steps SHALL become path-planning units only after step-level human review.

#### Scenario: Runtime step is promoted
- **WHEN** a runtime lesson step is promoted to a PlanningUnit
- **THEN** it SHALL have a verified route target, graph bindings, capability or quality contribution where applicable, estimated time, path role, prerequisite relation, evidence contract, privacy policy, source version, and human review metadata
