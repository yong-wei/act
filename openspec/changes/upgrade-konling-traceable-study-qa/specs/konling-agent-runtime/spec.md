...
- **THEN** Konling SHALL require assumptions or symbol definitions, material transformations, applicable conditions, and a result check.

#### Scenario: Code debugging is requested

- **WHEN** a learner requests help locating or fixing a code problem
- **THEN** Konling SHALL require observed failure, likely cause, minimal correction, and a verification method.

### Requirement: Konling keeps expression preferences below evidence governance

Konling SHALL accept supported explanation-depth, example, formatting, and hint-strength preferences only as presentation constraints. It SHALL NOT relax evidence, citation, or normative-content requirements.
...
Konling SHALL treat standards, prescribed formats, official rules, legal requirements, and other normative content as authoritative only when server-verified eligible authority citations support the answer.
...
Konling SHALL bind material conclusions, derivation transformations, and code repair recommendations to server-verified citation targets when available. A marker that cannot map to an eligible server-verified target SHALL NOT become a verified evidence binding.
