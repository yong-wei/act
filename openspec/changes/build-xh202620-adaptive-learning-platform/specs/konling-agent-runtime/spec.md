...
Konling SHALL build its runtime context from server-owned page context, learner state, plan context, and scoped memory rather than default or client-provided profile values.
...
- **AND** each tool SHALL enforce user, class, resource, and privacy scope.

### Requirement: Konling memory is layered and persistent
Konling SHALL define layered memory at working-summary, session-summary, episodic, semantic learner, and strategy levels, with Stage 1 persistence limited to working, session, and episodic summaries unless later privacy and evaluation gates enable long-term semantic and strategy memory.
...
#### Scenario: Memory retrieval is scoped
- **WHEN** Konling searches learning memory
- **THEN** it SHALL retrieve only memories authorized for the current user and context
- **AND** it SHALL include source, confidence, freshness, and privacy level in the tool result.

#### Scenario: Long-term memory is gated after MVP
- **WHEN** Stage 1 MVP is active
...
Konling SHALL support corrective, remedial, load-reducing, and challenge interventions grounded in learner state and path context.

#### Scenario: Intervention is generated
- **WHEN** Konling intervenes because of risk, stagnation, path deviation, repeated failure, or high mastery
...
- **THEN** the system SHALL persist the outcome and make it available to learner state, path evaluation, and strategy memory within privacy limits.
