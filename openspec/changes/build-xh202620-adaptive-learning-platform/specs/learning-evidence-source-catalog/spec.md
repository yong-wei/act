...
The system SHALL maintain a machine-readable catalog of learning evidence sources that identifies each source table or event family, its provenance policy, learning scope, value level, profile eligibility, materialization readiness, learner-state role, path-planning role, and privacy level.
...
#### Scenario: Catalog covers adaptive-loop evidence families
- **WHEN** the adaptive-learning platform capabilities are enabled
- **THEN** the catalog SHALL classify adaptive assessment records, ResourceNode execution events, media progress events, path plan events, path feedback events, Konling tool calls, Konling memory summaries, and intervention outcomes
- **AND** each family SHALL state whether it can contribute to learner state, path planning, teacher insight, Konling memory, or only context.
...
The system SHALL provide a dry-run evidence coverage report that summarizes current source coverage, eligibility, exclusions, users, source windows, learner-state readiness, and path-planning readiness without mutating data.
...
- **AND** it SHALL NOT create, update, or delete `LearningFact`, snapshot, summary, feature, path, memory, or raw source rows.

#### Scenario: Coverage report explains exclusions
- **WHEN** a source row family is excluded from profile, learner-state, path-planning, or Konling memory eligibility
