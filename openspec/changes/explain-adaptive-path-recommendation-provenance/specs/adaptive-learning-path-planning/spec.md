...
### Requirement: Candidate paths preserve recommendation provenance
The adaptive learning path planner SHALL persist a student-safe recommendation provenance snapshot for each formally generated candidate path without changing candidate selection, ranking, or scoring.
...
- **THEN** the candidate path SHALL preserve one or more provenance entries that connect an evidence summary to a capability or knowledge judgment and the path resources affected by that judgment
...
- **WHEN** recommendation provenance is serialized for a student-facing candidate path
