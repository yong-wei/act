...
1. 绑定实体记录 KAQ role ID、Canonical ID、聚合 ReleaseSet/Release、绑定角色、证据和审核状态。
2. 绑定依据 KAQ 语义重新审核，不从 Legacy ID 或同名节点继承。
3. `govern-aggregate-course-coverage-and-resource-bindings` 产出的聚合 CourseCoverage 限制可进入当前课程 KAQ 的 Canonical 对象。
