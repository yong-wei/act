...
- [ ] 1.1 Read the admitted v0.22 composite candidate receipt from
...
- [ ] 1.2 Add a fail-closed consistency check that the catalog, zh-CN, and
...
- [ ] 2.1 Load the v0.22 domain catalog data and derive the top-level domain
  entries, reviewed order, and aggregate entry without any hard-coded count.
- [ ] 2.2 Rebuild many-to-many membership for every published v0.22 concept
...
- [ ] 2.3 Replace the v0.9-bound manual membership source in candidate space
  while leaving the currently served production catalog untouched.
- [ ] 2.4 Verify presentation safety: reviewed Chinese names and summaries
...
- [ ] 4.2 Resolve every captured reference by unchanged stable ID or explicit
...
- [ ] 4.3 Build the complete inactive Teaching Projection and prerequisite
  candidates twice with the candidate-import database observation; verify
  byte-identical outputs and `nonActivation` receipts.
- [ ] 4.4 Verify that new v0.22 nodes without teaching relations do not block
  rebase completion and that later published relations can be added through a
  new complete Projection release.

## 5. Verification

- [ ] 5.1 Run the catalog, resolver, and rebase unit/integration suites plus
  lint and typecheck.
- [ ] 5.2 Snapshot all current production pointer bytes before and after each
