...
- Students can distinguish low confidence from precise diagnosis.
- Missing cache produces explicit evidence-insufficient state.
- Recommendation language does not overstate low-confidence evidence.
...
- Use feature cache as approved aggregate cache, not the sole truth source.
- Fallback states are explicit instead of hidden.
- Avoid exposing raw sensitive answers by default.

## Risks / Trade-offs

- Adding status text can become verbose. Mitigate by compact UI summaries and drilldown links.
