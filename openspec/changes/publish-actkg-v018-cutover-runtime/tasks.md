...
- [ ] 2.1 Configure Docker Desktop for 24 GiB RAM and 8 GiB swap, verify at least
  20 GiB through `docker info`, and confirm no residual parallel build owns it.
- [ ] 2.2 Build through `scripts/build.sh` with a 12 GiB Node heap and require
  container Next compilation, TypeScript, image export, OCI labels, and provenance.
- [ ] 2.3 Verify the exported image digest and release sidecars against the frozen revision.
...
- [ ] 3.2 Refresh app and worker to the frozen cutover-capable OCI revision while
