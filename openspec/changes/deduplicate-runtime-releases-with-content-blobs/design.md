...
The v2 lifecycle `active` record is the single authority for an activation. The legacy host active receipt is a deterministic compatibility projection, never an authority from which lifecycle state is reconstructed. Activation records a same-directory transaction intent, commits the lifecycle generation, then projects and rereads the host receipt. Recovery first repairs the lifecycle journal and then either completes or recreates the receipt from the committed lifecycle identity. A crash at any boundary therefore leaves the prior lifecycle identity or a recoverable projection of the committed identity; it never silently rolls a committed lifecycle state back to an older receipt.
