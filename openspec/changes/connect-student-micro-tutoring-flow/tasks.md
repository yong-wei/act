...
- [ ] 3.2 Run targeted tests, typecheck, strict OpenSpec validation, relevant browser acceptance, build and diff checks. Browser acceptance is recorded in `artifacts/commercial-ui/issue-1330-student-micro-tutoring-flow/evidence-manifest.json` for code commit `35c0bf0030370235252034b396eae0c4f11c2d1a`; it covers 1440px and 320px available, fail-closed unavailable, and 503 recovery states with screenshot hashes and no-overflow checks. Do not mark complete until the scoped final checkpoint passes the required build/bundle gates without importing an unrelated build repair.
