...
- [x] 3.2 Run targeted tests, typecheck, strict OpenSpec validation, relevant browser acceptance, build and diff checks. Browser acceptance is recorded in `artifacts/commercial-ui/issue-1330-student-micro-tutoring-flow/evidence-manifest.json` for the clean code checkpoint recorded by that manifest; it covers a reviewed catalog-backed wrong-answer entry, available full loop, `REFERENCE_DRIFT` with stale details hidden and explicit retry, and retry recovery at 1440px and 320px, with screenshot hashes and no-overflow checks. Capture rejects all dirty tracked runtime inputs (except the evidence output directory) and HEAD/content drift before publishing evidence. Targeted tests, typecheck, strict OpenSpec validation, and diff checks are rerun for each final checkpoint.
