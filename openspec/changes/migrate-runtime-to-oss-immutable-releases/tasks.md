...
- [ ] 2.4 Add a source-authoritative, no-staging streaming publisher transport as the only production write entrypoint, with per-release ECS `flock`, resumable exact-object checks, wire-digest separation and failure-path regression tests.
