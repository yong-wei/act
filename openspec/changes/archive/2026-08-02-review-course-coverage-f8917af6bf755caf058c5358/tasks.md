...
- [x] Preserve the raw Primary/Challenger source artifacts byte-for-byte and bind normalized stage documents to their source SHA-256 and writer sessions.
- [x] Treat the member's aggregate `priorDecisionRef` as historical provenance only; it cannot supply independent course evidence or authorize a role.
...
- [x] Map the agreed role-free `DEFER` to `DEFERRED_EVIDENCE_BLOCKED` while keeping CourseCoverage authority unresolved and the aggregate gate blocked.
