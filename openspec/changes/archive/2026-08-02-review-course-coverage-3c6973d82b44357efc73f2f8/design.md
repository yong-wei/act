...
The receipt is deterministic and machine-mergeable: it contains the exact batch binding, ordered member references, Primary/Challenger/Third stage outcomes, conflict resolutions, terminal per-member decisions, provenance binding, and proof that no out-of-slice member was written. The completed immutable pair is recorded by `receiptDigest` `2309285ee5acef97d6076135c8d9da29fbd642d1053e314b8f867e737a83c347` and detached `attestationDigest` `1fdf6b0398f01aa590f01e83cb48d834191d8ba9958fad77b603dc0b6a8c5d83`. It uses the v3 production-boundary proof and v2 detached-attestation schemas; an identical replay completed in `content-equivalent` mode without capture-commit ancestry dependence. A bound receipt is valid only with the current v3/v2 pair. A missing provenance binding is accepted only for the fifteen tracked historical receipt/attestation path-and-digest pairs across v2/v1 and v3/v2; deleting or retaining the binding while downgrading protocol fields, or resealing either digest, is rejected.

### Current-head Sol medium O1 accepted decision

The frozen public-boundary policy is activated only after `assertCurrentCourseCoverageProductionBoundaryBundle` has verified the exact repository-relative receipt path `course-content/authoring/knowledge/issue-1200-course-coverage-review/batch-receipt.json`. The policy then compares the complete ten-field batch binding, ordered Challenger coverage, fixed Primary/Challenger sessions, provenance SHA-256, protected-path order, and review scopes. Because each recorded `sourceWriterScope` equals its corresponding `reviewSessionScope`, both scope fields are frozen and tested independently. This path gate prevents mutable batch metadata from selecting the #1200 contract.
