...
- [x] 1.1 Verify from live Issue state that #1876 is closed, has
...
- [x] 1.2 Obtain and schema-validate A's immutable successor subject identity,
...
- [x] 1.3 Add fixtures for absent, mismatched, and locally substituted A inputs
  and verify that no qualified output is written and no working-tree fallback is
  accepted.

## 2. Scope Denominator and Family Discovery

- [x] 2.1 Consume A's exact captured entry set and enumerate the required
...
- [x] 2.2 Implement stable per-slice discovered/qualified/
...
- [x] 2.3 Keep ignored, untracked, generated, and runtime-created inputs outside
...
- [x] 2.4 Implement family grouping from member evidence and require primary
...
- [x] 3.1 Create one clean classifier/validator tool checkpoint and record
...
- [x] 3.2 Freeze the A subject, tool, schema, and consumer/provenance evidence
  digests and reject subject/tool/schema/input drift before serialization.
- [x] 3.3 Build read-only evidence adapters for production, test, tool/script,
  ...
  as unresolved.
- [x] 3.4 Record canonical source, production authority, candidate/staged,
...
- [x] 4.1 Implement the typed record contract for path/family, member or family
  ...
  retention facets.
- [x] 4.2 Assign exactly one primary class to qualified records using the fixed
  ...
  non-primary facets.
- [x] 4.3 Preserve missing or conflicting evidence as `unresolved` with
  ...
  is blocked.
- [x] 4.4 Bind explicit retention, privacy, materialization, recovery, and
...
- [x] 5.1 Keep every denominator member independently disposed as
...
- [x] 5.2 Evaluate future eligibility only when canonical source, complete
...
- [x] 5.3 Keep any missing, unknown, or drifted future-eligibility proof
  unresolved and verify that `futureEligible` never performs or authorizes an
  action.

## 6. Compact Outputs and Privacy Boundary

- [x] 6.1 Generate deterministic `summary.md`, `index.json`,
...
- [x] 6.2 Define the policy matrix for Git retention, approved external storage,
  ...
  unexecuted.
- [x] 6.3 Enforce stable ordering and repository-relative/logical locators and
...
- [x] 6.4 Inject credential, cookie, user ID, raw answer, absolute path, and
...
- [x] 7.1 Add a read-only mutation guard and pre/post subject/input hash check;
...
- [x] 7.2 Run focused tests for denominator closure, member dispositions,
...
- [x] 7.3 Run existing read-only contract validators for QA evidence lifecycle,
...
- [x] 8.1 Review every unresolved record and future-eligibility gate against its
...
- [x] 8.2 Produce the digest-bound handoff containing only the exact A subject
  ...
  review.
- [x] 8.3 Confirm the final worktree diff is confined to this change's
