...
表达式只允许 ASCII 数学字符且长度 ≤300，变量只允许 ASCII 标识符；共享执行器在启动子进程前执行与 Python 一致的 Zod 白名单校验。Python 按输入格式确定性分流：含反斜杠或花括号等明确 LaTeX 标记的输入只调用 `parse_latex(strict=True)`，否则只调用使用最小安全全局构造器的受限 `parse_expr`，两种格式之间禁止回退；`parse_expr` 的 transformations 不重复添加 `auto_symbol`，且 `__builtins__` 仍为空。Python 进程内设置 `RLIMIT_CPU`；共享执行器统一限制 API 与 KAQ 共用的 4 并发 + 8 排队，超限由 API 投影为 429、由 KAQ 投影为受治理工具错误；子进程 10 秒超时。每个步骤固定包含 `step`、`description`、`operation`、`input`、`output` 五字段。
