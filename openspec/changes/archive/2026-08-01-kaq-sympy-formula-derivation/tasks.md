...
- [x] 2.1 新增 `src/lib/math-calc.ts`，在 `spawn` 前执行与 Python 一致的表达式/变量白名单校验；保留非零退出携带的合法结构化计算错误，仅将无合法错误信封的异常退出投影为稳定 unavailable 错误（不泄露 stderr）；共享执行器统一封装 10 秒超时、结构化结果解析与 4 并发 + 8 排队限制，使 API 与 KAQ 不存在旁路。
- [x] 2.2 新增 `POST /api/math/calculate`，强制登录、Zod 校验，并将共享执行器容量超限投影为 429。
