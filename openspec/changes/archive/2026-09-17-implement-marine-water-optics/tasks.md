...
- [x] 4. （泡沫为受光表面）受光泡沫：泡沫颜色乘与基面一致照明因子（含同源太阳辐照归一：light = N·L × sunIllumination，七包经 useEnvironmentWaterColors 透传 preset.sun.intensity/2.0），暗预设下泡沫整体变暗不改变色相（litFoamColor 纯函数测试钉住各通道同因子缩放）；本项自然泡沫为无状态场（crest×噪声调制），无持久泡沫状态——持久泡沫/衰减属 #2101 船体尾流范围。
