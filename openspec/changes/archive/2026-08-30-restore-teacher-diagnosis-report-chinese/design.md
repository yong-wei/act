...
2. **确定性校验规则（不调用模型判语言）**：每个自然语言字段必须 (a) 至少包含一个中文字符；(b) 不含日文假名；(c) 不含高频繁体专用字（约 120 字黑名单，简化字对应不同码点，例如"課程學習進度良好"会被拒绝）；(d) 中文字符数 ≥ 拉丁字母数。规则允许正文内嵌英文技术词（如 "Risk Flags"），拒绝英文主导段落、繁体/日文文本与 limitation 技术码直出。黑名单仅覆盖高频繁体字，生僻繁体字混排存在理论绕过空间（残余披露）。
