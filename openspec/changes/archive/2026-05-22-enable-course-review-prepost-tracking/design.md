...
- Teacher review no longer needs per-unit state-kind branches.
- Legacy evidence is shown as limited rather than falsified.
- 5-3 to 5-6 completion tracking is ready before post-class review.
...
- Use CourseEvidenceSpec for state kind and step ids.
- Use manifest scoring metadata to rescore final-state answers only when source answers exist.
- Do not invent question summaries for legacy envelopes.

## Risks / Trade-offs

- Old review paths may regress. Mitigate with compatibility tests for course_review and showcase_review.
