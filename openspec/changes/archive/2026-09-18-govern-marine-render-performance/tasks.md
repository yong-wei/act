...
- [x] 4. （QA 采集入口）MarinePerformanceEvidenceProbe（?qa=marine-performance 或既有帧契约入口 ?qa=marine-frame → window.__marinePerformanceEvidence.read()）：滚动 ~60s 帧样本环形缓冲 + 完整报告构建（drawingBufferSize；口径诚实——rAF 帧间隔恒 method=frame-intervals，timer-query 扩展存在性单独记录，未经实际 query 采集不标 GPU 口径）；船包/镜头/海况经 contextInput 从各场景真实状态传入（缺省 DOM 探测/unknown，不占位假值）；七包 Canvas 挂载。
