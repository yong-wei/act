...
- [x] Verify the `batches[21]` manifest slice, exact order, 284-member count, and all bound digests before any review.
- [x] Fail closed if a member, digest, canonical revision, or selected manifest file changes.

## 2. Produce independent conclusions

- [x] Primary reviews each exact member against current course evidence and records a rationale; all 284 conclusions are role-free `DEFER`.
- [x] Challenger independently reviews all 284 profileOnly/highRisk KnowledgeStatement members and records separate rationales; all 284 conclusions are role-free `DEFER`.
...
- [x] Map all 284 agreed role-free `DEFER` decisions to `DEFERRED_EVIDENCE_BLOCKED`, leave CourseCoverage authority unresolved, and keep the aggregate gate blocked.
