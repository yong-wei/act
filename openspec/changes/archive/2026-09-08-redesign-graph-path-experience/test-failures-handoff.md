...
GitHub 首次审查另发现资源版本混入全局发布身份的 P1，该场景未在原 73 项失败清单中出现。后续整改将自身内容/后端配置/绑定版本与全局引用身份分离，并新增回归测试。继续归因时，请查询 PR #2065 最终 HEAD 和其验证评论；本文件统计表保留的是 88e8ad0 的原始证据，不冒充后续提交的结果。
