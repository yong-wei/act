...
- Builds the hard latest-cutover readiness check on the existing `act-knowledge-surface/v1` server-resolved envelope and `readKnowledgeSurface`; it does not add a second read model, client-selectable release, or parallel cache authority.
