...
- RAG 接线：`teaching-resource-rag` 作为 composed 的教学资源通道接入控灵检索链（承载投影绑定资源的检索职责，不退役）；接线 `canonicalRagShadow` 死代码为影子诊断；`selectRagAuthority` 的 `PRODUCTION_ANSWER` 从 LEGACY 切到 composed，按 cutover 治理执行（影子对比 → 门禁指标 → 授权切换 → 回滚预案）。
