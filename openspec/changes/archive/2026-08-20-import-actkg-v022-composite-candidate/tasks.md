...
- [x] 1.3 Verify the declared protocol is `actkg-public-bundle/2` and Schema
  ...
  当前结果：已 pin `control-theory-engineering-v0.22-r5` / source-r8（bundle digest `98f2d5b183…`）。Schema 哈希与 v0.18 相同；r5 只刷新 validation-report / manifest / SHA256SUMS，`relation_type_count=9` 与三个投影谓词并集一致。v2 适配器 shadow admission 通过。

## 2. Candidate import and impact evidence

- [x] 2.1 Reuse the standard v2 import boundary for the validated v0.22 result
  without changing v1 or v0.18 database or file import behavior.
- [x] 2.2 Materialize the staged Authority snapshot whose visible membership is
  ...
  records.
  记录计数：releaseNodes=7300，runtimeNodes=7082，runtimeLinks=2932，labels=2148。snapshot `snap-16c0b75362fa…`。
- [x] 2.3 Compute the complete v0.18 to v0.22 object, type, relation, endpoint,
  and identity impact report directly from the current v0.18 production
  snapshot and the full v0.22 candidate snapshot.
  直接审计：+239 objects、+121 relations、+239 crosswalk；分类 `SEMANTIC_CONTENT_UPDATE`。
- [x] 2.4 Retain and cross-check the upstream intermediate release diffs
  ...
  上游 `release-diff.json` 是 v0.21→v0.22，crosscheck 记为 `DISAGREED`；分母仍是完整 v0.18 snapshot。
...
- [x] 3.1 Rebuild the mirror, candidate, and impact evidence twice from a clean
  ...
  源 E=`aa775d7a005c`，派生 F=`5a242c716065`，`--verify-derived` 通过；replay-1/2 字节相同。
- [x] 3.2 Prove idempotent re-import of the same envelope and disposable-schema
  cleanup on success and failure.
- [x] 3.3 Assert Authority, Teaching Projection, prerequisite, Chinese-display,
  ...
  本分支 `authority/current.json` 仍为 `snap-7f4cdd1084af…` / v0.9，sha256 `086f14793fbf2aa3…`。
- [x] 3.4 Run focused adapter/import/snapshot tests, typecheck, lint, strict
  OpenSpec validation, and stable-revision review.
  已跑 v0.22 boundary + authority snapshot 测试、typecheck、changed-file eslint、`openspec validate import-actkg-v022-composite-candidate --type change --strict`。清场审查在推送后按 latest head 请求。
