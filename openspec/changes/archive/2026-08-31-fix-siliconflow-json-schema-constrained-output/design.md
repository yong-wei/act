...
- 逐字节重放同一生产请求观察到 **24s–300s+ 的延迟方差**（SiliconFlow 服务端负载相关）：升级后截断根因确定性消除（finish=stop、输出完整合法，多次重放 14–58s），但 120s 生产窗口在服务端高负载时段仍可能出现个别超时——该部分由既有重试预算与失败分类正确处理，属服务端特性而非本修复缺口。
- 既有 `withQwen35JsonObjectFallback`（显式 json_schema 调用方的降级）保持原行为，不在本变更范围；升级叠加在降级之后，两条路径互不干扰（降级注入的英文 system 指令不含中文标记，不会触发升级）。
