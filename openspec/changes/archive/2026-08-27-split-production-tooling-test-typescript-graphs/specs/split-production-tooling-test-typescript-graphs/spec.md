...
`capture-modular-monolith-refactor-baseline` 与 `restore-trustworthy-test-command-contracts` 是本 capability 的硬前置，必须同时 qualified 且 source identity 可追溯；后续 CI gate 消费本 capability，不得形成回指依赖。
