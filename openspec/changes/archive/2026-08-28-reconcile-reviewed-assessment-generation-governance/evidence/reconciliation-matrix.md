...
| torn hydration 回退静态 sidecar 导致旧发布物在 DB 证据不完整时恢复治理阶段权限（PR #1664 第二轮 Codex P1） | 修复前 `hydrateGeneratedCatalogFromPersistence` 检测到断裂 lineage 时静默返回，overlay 不就绪，`loadGeneratedCatalog` 回退读取上一次发布的静态 items/reviews | torn 快照安装空的就绪 overlay 并置 torn 标记，禁止静态回退；后续 ensure 调用重读 DB，lineage 恢复后自动恢复服务；测试覆盖 torn 阻断与恢复 |
