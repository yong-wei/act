...
- [x] 1.1 Before any claim, apply, implementation (including validator or test
...
- [x] 1.2 Receive change A's exact owner-residue subject identity and
...
- [x] 1.3 Re-read the current modular-monolith charter/current-head contracts,
  ...
  ledger.
- [x] 1.4 If an adjudicator or validator is implemented, create a clean tool
...
- [x] 1.5 Freeze A subject, tool identity, schema, relevant spec inputs and
...
- [x] 2.1 Enumerate the exact current A member set for
...
- [x] 2.2 Discover and classify every member's production, test-only,
  ...
  relationship kinds.
- [x] 2.3 Reconcile static imports, dynamic imports, barrel exports, shell/path
...
- [x] 2.4 Define one normalized record schema with stable member/family ID,
...
- [x] 2.5 Prove every member has one primary observation or a justified
...
- [x] 2.6 Form a family only when all members share outcome-relevant owner,
  authority, consumer, privacy, lifecycle, retention, trust and deletion
  evidence; split or leave unresolved on any difference.
- [x] 2.7 Verify that all capability, route, API, model, worker, script,
...
- [x] 3.1 Characterize Learning Record paths against the sole online writer
...
- [x] 3.2 Apply the archived C16 map: Assignment remains the owner of approved
...
- [x] 3.3 Preserve LearningFact eligibility as independent from assignment
  ownership and preserve all existing event, revision, replay, retention and
  backfill isolation requirements.
- [x] 3.4 Preserve `ArenaSubmission` as official score authority; classify
...
- [x] 3.5 Preserve Portrait V2 as primary and StudentCompetency as compatibility;
  retain freshness, confidence, fallback and recommendation eligibility.
- [x] 3.6 Preserve teacher/class authorization, redaction and independent-
  learner small-sample suppression; keep backfill separate from online writers
  and current pointers.
- [x] 3.7 Scan the evidence contract for raw answers, event payloads,
...
- [x] 4.1 Preserve exactly one accountable target domain owner for every
...
- [x] 4.2 For an exact business-domain outcome, select one existing charter
  ...
  the outcome.
- [x] 4.3 For a kernel outcome, independently prove all conditions: at least
...
- [x] 4.4 Reject a kernel label when any condition is missing or supported only
  ...
  `unresolved`.
- [x] 4.5 For operator/backfill outcomes, record operator authority,
...
- [x] 4.6 For fixture/demo/test/generated outcomes, record non-production role,
...
- [x] 4.7 For compatibility/retirement outcomes, record exact replacement,
...
- [x] 4.8 Preserve all multi-owner, no-owner, incomplete-caller and
...
- [x] 4.9 Emit the compact decision matrix and owner/kernel/tool/fixture/
...
- [x] 5.1 Generate the scoped owner/deprecation projection from the existing
  normalized charter record vocabulary and one record set; do not create a
  second owner catalog, charter, or ledger.
- [x] 5.2 Mark the scoped charter projection qualified only when the denominator
...
- [x] 5.3 Keep the complete ledger as a local/CI artifact outside Git.  Publish
...
- [x] 5.4 For every future slice, record exact paths, public boundary, paths
  ...
  the design.
- [x] 5.5 Require every reader of a slice to verify subject/tool/schema and
...
- [x] 6.1 Add privacy/path checks over every compact projection and full ledger
  locator artifact; fail closed with only a safe record ID and violation code
  for forbidden content or absolute paths.
- [x] 6.2 Add deterministic fixtures proving identical output for identical A
...
- [x] 6.3 Add rejection fixtures for the #1876 gate, missing/mismatched A
...
- [x] 6.4 Add scope tests proving no source movement, import/export change,
...
- [x] 6.5 Run targeted contract/privacy/determinism/denominator validators and
...
- [x] 6.6 Produce the final handoff with COMPLETE or non-qualified BLOCKER
