...
4. **Each upstream artifact is pinned.** The new generation records path, original byte digest, original published/semantic digest, resealed digest, the fixed snapshot binding, and an explicit conversion protocol. Any upstream, Authority, source or semantic drift fails closed. Worklists actually read by `collectCrossDomainBoundaryCandidates` are pinned per role/path to the shared generation-3 cross-domain worklist identity digest. Multiple reviewer inputs may share that one empty-review output; do not invent one-to-one reseals or new cross-domain edges.
