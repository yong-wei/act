...
This reconciliation SHALL consume the qualified command/discovery contract and the immutable A subject identity/digest plus the distinct generator identity from `capture-post-convergence-repository-and-architecture-baseline`. It SHALL remain blocked until the parent coordination layer verifies Issue #1876 is closed, carries `status:archived`, and has its native `blockedBy` dependencies resolved.
