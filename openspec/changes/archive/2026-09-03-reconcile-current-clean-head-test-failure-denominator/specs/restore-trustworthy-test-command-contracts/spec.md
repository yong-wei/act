...
The current-head reconciliation SHALL consume the immutable successor capture from `capture-post-convergence-repository-and-architecture-baseline` and the existing qualified command-contract inputs. The successor is an investigation input only; it SHALL NOT be treated as `REQUIRED_BASELINE`, `REQUIRED_FITNESS_BUDGET`, or any active selector.
...
#### Scenario: The same subject and generator are qualified again
