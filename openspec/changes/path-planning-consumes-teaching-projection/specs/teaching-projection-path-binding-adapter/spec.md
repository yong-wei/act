...
The teaching-projection path binding adapter SHALL map projection resource ids to existing ResourceNode ids using only deterministic rules: `act:handout:<lessonKey>` to `runtime-handout:<lessonKey>`, `act:card:<stableCardId>` to `knowledge-card:<stableCardId>`, `act:video|act:audio:<lessonKey>:<mediaId>` to `runtime-media:<lessonKey>:<mediaId>`, `act:simulation:arena-task-<taskId>` to `arena-task:<taskId>`, `act:textbook-section:<sectionId>` to the matching textbook section node, and `act:exercise:*` to exercise nodes. Classroom simulations whose ids encode a lesson unit SHALL resolve only through an explicit mapping table. Fuzzy matching, label similarity, and vector similarity MUST NOT create a mapping. `act:textbook` and `act:textbook-chapter` containers MUST NOT produce path nodes.
