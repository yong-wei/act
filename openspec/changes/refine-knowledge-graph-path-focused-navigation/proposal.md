...
- Register the 11 runtime types `causes`, `demonstrates`, `equivalent_to`, `exemplifies`, `extends`, `has_stage`, `precedes`, `produces`, `provides_context`, `refined_by`, and `refines` as association semantics in the shared TypeScript/Python contracts, with zero-instance fixtures and honest unavailable-evidence behavior.
...
- Fail relation export when one ID resolves to different normalized endpoints or types; permit deterministic coalescing only for repeated records with the same normalized key.
- Invalidate runtime lesson/media semantic review item by item from reviewed-versus-current source, manifest, and evidence hashes, retaining auditable provenance and removing stale items from path eligibility without creating replacement human confirmation.
...
- Correct the evidenced 1-3 authoring relation from `refines` to `provides_foundation`; retain `extends` as authored `source → target` because its description explicitly extends the static pole concept into parameter-dependent pole migration. Runtime output is regenerated through the review/export pipeline.
