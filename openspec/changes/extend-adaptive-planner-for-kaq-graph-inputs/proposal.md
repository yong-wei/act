...
After first-class LearningGoals, ResourceNode graph profiles, goal subgraphs, and artifact versions exist, the adaptive planner must consume them directly. Keeping planning on old target arrays would leave the graph work as documentation rather than path-generation input.

## What Changes

- Extend the existing adaptive planner contract instead of creating a parallel planner.
- Allow planner requests to consume LearningGoal metadata, ExpandedGoalSubgraph, ResourceNode graph profile metadata, ResourceCoverage overlay, Learner/Class overlay, and version refs.
...
- `adaptive-learning-path-planning`: planner inputs and outputs must support graph-driven LearningGoal context.
...
- Depends on first-class LearningGoals, resource graph profiles, goal subgraph expansion, artifact versioning, the archived `graph-resource-coverage-overlay` capability, and active change `add-learner-class-graph-overlays`.
