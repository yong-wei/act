...
- [ ] 1.1 Freeze the source revision/tree and enumerate assignment-bound and
...
- [ ] 1.2 Enumerate related `GradingRun`, approval snapshot, derivative,
...
- [ ] 1.3 Inventory every legacy route, alias/deep link, flag, component,
...
- [ ] 1.4 Define the usage/data/log observation window, privacy-safe queries,
...
- [ ] 2.1 Resolve each valid assignment-bound record to Assignment, published
...
- [ ] 2.2 Migrate assignment-bound review/feedback callers and records through
  the Assignment public API and existing approval snapshot/outbox contract.
- [ ] 2.3 Block or migrate unapproved/unverifiable rows with explicit reasons;
  do not infer approval from a draft, route response, or `reviewerState` alone.
- [ ] 2.4 Retain approved unbound history only with an immutable approval
...
- [ ] 2.5 Fence legacy producers and drain, cancel, or account for all legacy
  worker/outbox jobs and retries without deleting governed audit history.
- [ ] 2.6 Create a reviewable one-to-one mapping from each legacy draft to an
...
- [ ] 2.7 Preserve existing non-null `sourceEventId` values and use only a
...
- [ ] 2.8 Classify materialized versus unmaterialized criterion evidence and
...
- [ ] 3.4 Preserve migration/repair scripts, generic `LearningEvidenceDraft`,
  ...
  still require them.
- [ ] 3.5 Add a source-level no-facade/no-hidden-fallback proof covering
  imports, routes, flags, scripts, workers, dynamic links, and test fixtures.

## 4. Closure Evidence And Verification

- [ ] 4.1 Re-run data, route, caller, worker, script, capture, and spec
...
- [ ] 4.6 Run focused data-governance/assignment tests, typecheck, strict
