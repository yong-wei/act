...
- `npx vitest run src/app/__tests__/portfolio-reflection-drafts-route.test.ts src/lib/__tests__/portfolio-reflection-draft-contracts.test.ts src/lib/__tests__/ai-task-boundary-ui-source.test.ts` (15 tests passed, including retryable conflict and discarded replay regressions)
- `tsx scripts/tests/test-portfolio-reflection-draft-postgres.ts` (migration, concurrent idempotent save, learner scoping, discard, discarded replay rejection, and zero `LearningFact` writeback passed)
...
- `node artifacts/commercial-ui/issue-1321-portfolio-reflection-drafts/capture-portfolio-reflection-drafts-evidence.mjs` (passed; capture revision `a25feb05f90be931e952e9c0bb1bbb69c1057af4`, 1440px and 320px screenshots with SHA-256 manifest)
