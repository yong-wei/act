...
- **THEN** the system retains one active draft rather than creating duplicate draft rows or changing its original provenance

#### Scenario: A discarded draft receives an old save replay
- **WHEN** a student replays a candidate save request whose identity belongs to a discarded draft
- **THEN** the system rejects the replay, keeps the draft discarded, and does not restore it to the active list

### Requirement: Draft provenance is immutable after save
The system SHALL preserve the saved draft's source, optional assignment, intent, title, and idempotency identity. The student may edit only the draft content.

#### Scenario: Student submits changed provenance to an item update
- **WHEN** a student sends source, assignment, intent, title, or request identity along with an item update
- **THEN** the system rejects the request and leaves the saved provenance unchanged
