...
The developer adapter SHALL serialize mount state per checkout, record only credential-free identities and exact owned mount paths, start consumers only after selection, and stop consumers before unmounting the checkout view and Blob FUSE. Cleanup SHALL affect only mounts proven to belong to that checkout and SHALL preserve unknown paths and other worktrees.

#### Scenario: Normal shutdown releases the runtime
- **WHEN** the developer invokes the OSS-aware shutdown command
- **THEN** it SHALL stop frontend, worker and scheduler processes before unmounting the exact checkout bind and Blob FUSE mounts

#### Scenario: Prior startup crashed
- **WHEN** a new startup finds a receipt or mount left by an interrupted run
- **THEN** it SHALL verify process ownership, mount source and identity, then safely reuse or report the exact cleanup required without recursive deletion
