...
系统 SHALL 从可审计的 `micro-tutoring-validation-purpose-reviews-v1.jsonl` 读取每条验证题的用途决定，并同时核对其 catalog 身份、sourceId、contentHash 与规范知识节点。生成器不得根据普通 assessment approval 或阶段标签自行合成用途审核。缺失或不匹配时 MUST fail closed，该题不得进入正式 v2 注册表。
