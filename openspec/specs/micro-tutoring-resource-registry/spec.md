...
系统 SHALL 从 `micro-tutoring-option-attributions-v2`（工件版本 `micro-tutoring-option-attributions.v3`）派生当前资源投影。投影 MUST 覆盖该目录中每个唯一 `knowledgeNodeId` 与 `misconceptionTag` 对，使 v2 分母中每个错误选项都能解析到至少一个当前有效、学生可见、可启动的学习资源。v1 资源投影与资格回执 MUST 保持只读历史兼容，不得被原地扩写。
