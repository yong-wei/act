...
The system SHALL persist adaptive assessment sessions and answers instead of relying on process memory, and path-owned requests MUST have no process-memory fallback.
...
- **AND** the submission SHALL remain readable after application restart without consulting process memory.
...
The system SHALL preserve existing assessment response compatibility during persistence migration while routing all path-owned behavior through the durable Assessment public API.
