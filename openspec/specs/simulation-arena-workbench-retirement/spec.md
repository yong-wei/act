# simulation-arena-workbench-retirement Specification

## Purpose
Record that the `simulation-arena-workbench` transition feature is not a production owner: launch, session, status and route contracts live on existing Platform UI, Control Workbench, Arena, simulation and lesson modules.
## Requirements
