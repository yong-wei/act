# trusted-learning-fact-filter Specification

## Purpose
Define the versioned trusted LearningFact policy used by Portrait v2 materialization. Trust is decided from existing server-side evidence anchors, not from a new `trusted` field or an application-side marker.
## Requirements
