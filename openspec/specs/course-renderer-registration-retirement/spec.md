...
After reusable capabilities and central consumers use the typed registry, old
central renderer branches and compatibility entrypoints SHALL be removed only
when production, test, dynamic, and bundle imports have zero required callers.
No permanent forwarding facade or second global registry SHALL remain.

#### Scenario: The legacy inventory is empty

- **WHEN** all required old-renderer callers have a recorded replacement
- **THEN** the old branches and entrypoints SHALL be deleted in the qualified
  revision
- **AND** the ledger SHALL bind the replacement and deletion proof.

#### Scenario: A hidden caller remains

- **WHEN** a route, test, or bundle still reaches the old renderer
- **THEN** deletion SHALL remain blocked
- **AND** the path SHALL be reported as transitional rather than retired.
