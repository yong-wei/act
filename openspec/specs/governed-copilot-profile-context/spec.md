# governed-copilot-profile-context Specification

## Purpose

普通 Copilot 的学生画像事实只能来自认证用户的服务端受治理学习状态。客户端提交的身份、学习风格、认知水平和能力向量不得进入提示词；画像缺失、过期、低置信或服务不可用必须显式降级为通用课程辅导，不得用默认值冒充个人事实，也不得把聊天写回 LearningFact 或学习画像。
