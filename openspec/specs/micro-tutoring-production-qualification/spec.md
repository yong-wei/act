# micro-tutoring-production-qualification Specification

## Purpose
定义微辅导全题覆盖的生产资格候选合同：严格审计摘要、服务端契约、代表性学生闭环与不可变回执绑定同一 Git 修订。资格通过只产生 candidate，不得自动激活生产 canary 或改写生产 selector。
## Requirements
