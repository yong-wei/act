...
**Coordinated v0.37-r4 cutover (2026-08-26):** `fix/v037-coordinated-cutover-capture@4cedf619d` 已捕获 `r4` 并完成一次性历史基线闭合：7,476 个 Authority 对象、22,428 条教学处置、1,058 条正式资源信封与 2,296 条连续性义务均保留可复验输入；零项未闭合正式教学义务。基线以正式教学、显式不适用、运行时支持和仅目录四类义务分别处理，不将支持性资源伪造成 Canonical 绑定，也不建立逐条人工审核队列。r4 语义缓存已确认 10,523 条输入可复用、零条需因图谱语义重算；运行时选择器候选与 v0.22 前任证据已在本地静态树中封存。生产当前仍为 v0.22；Authority current、Runtime lifecycle、DB 与服务均未改动。下一步是将此静态候选合入 `integration`，在生产以不可选择 Runtime release 完成物化后，重捕获前任并生成 c5 candidate，再执行停止服务的 journaled transaction。不得用本地旧 v0.9 指针、标签匹配或人工“批量审核”代替该链路。
