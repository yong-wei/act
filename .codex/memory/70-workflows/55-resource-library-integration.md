...
## 最近稳定样例：`3-8` 频域课的资源选材口径

- `3-8（理论） 频域判别与跨域综合语言` 已形成一条可复用的资源驱动判断链：
  - `pptx/16频率特性_换个角度看控制` 负责“为什么需要频域语言”的导入；
  - `pptx/19稳定判据_频域的启示` 负责 Nyquist / Bode 判稳主体；
  - `pptx/21三频段_各司其职` 负责三频段分工与频域量到动态品质的读回；
  - 船舶 `5.1` 与 `5.3` 负责把裕度、带宽、谐振峰值与阶跃性能、工程判断接起来。
- 该样例同时明确：
  - `pptx/17近似叠加_绘制伯德图` 与 `pptx/18幅相特性_换个角度看频域` 在 `3-8` 中应排除；
  - 排除原因不是资源不好，而是它们的主价值属于模块2的作图/读图基础，不应侵蚀 `3-8` 的“翻译优先”定位。
- 后续若遇到“模块3后段频域课到底该读哪些资源”的问题，可直接把该样例当作边界检查参考，而不是重新从全部频域旧课件里筛选。
