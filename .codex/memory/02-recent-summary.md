...
状态: active
最后更新: 2026-04-05
摘要: 这是智能体初始化时优先读取的最近上下文入口，浓缩最近几次会话中最值得先知道的稳定变化、当前风险与建议下一跳；当前除 `1-1`、`1-2`、`1-3` runtime-first 精品互动课主线外，模块 2 的 `2-1 / 2-2 / 2-3` 也已进入作者态双轨真源驱动的精品互动课实现阶段，必须注意互动设计文档已进入“人读 `interactive-page.md` + 机读 `interactive-contract.yaml`”双轨真源阶段，`slides-ref` 已启动 `pptx -> Markdown + TikZ` 资源库长期转换任务、`resource-library` 已正式接入大纲重构与课程制作技能并出现了 `3-7` 误差分析总入口课与 `3-8` 频域课的稳定边界样例、新落地的 `course-content/questions/` 结构化习题库，以及数据治理 worker 故障收敛后的稳定运维边界。
上游:
