...
- `gpt-5.6-luna`：高频低成本任务，包括路由、读扫、路径映射、UI 轻审、复盘和极窄小补丁。
- `gpt-5.6-terra`：日常工程任务，包括实现、测试、复杂排障、性能和可靠性审查。
- `gpt-5.6-sol`：高语义或高正确性风险任务，包括规划、领域审查、安全、发布和关键终审。
- 常设角色只使用 `low`、`medium`、`high`、`xhigh`；`max` 与 `ultra` 由主线程在一次性自由派发时按风险显式选择。
