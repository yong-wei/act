...
- 至少包含 1 个具名例题和 3 道分层练习；例题章节标题必须根据所解决的具体问题命名，例如“XX求解示例”“XX设计过程示例”“XX分析示例”，不得使用“最小例题”这类泛称
- 例题之后必须安排案例章节；案例必须在具体工程场景下重新表述问题，不能直接甩出公式开始计算
- 以总结和扩展思考收束
...
媒体制作补充：
- 对响应曲线、Bode/Nyquist、根轨迹、稳定域、参数扫描等可计算图像，仍优先采用 Octave + Python/matplotlib 的代码直出流程。
- 对方框图、信号流图、电路图、机械结构图等线框图，仍必须调用 `tikz-control-draw`。
- 对场景导入、具体对象图形、实物/设备意象、类比解释、框图中的具象对象嵌入，以及代码直出难以表达的视觉教学资产，可调用 `imagen` 技能生成图像，并将图片与提示词按媒体命名规则归档。
- 生成式图片插入讲义后，正文必须给出工程语境、图像解读和概念边界，保持讲义严谨性。
