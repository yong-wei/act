...
16. 学生版讲义默认需要规划封面漫画资源，但**不强制在讲义正文中预插图片占位**；Step 3 默认只产出 `media/raw/cover-comic-prompt.md`，不要求当场生成 `cover-comic.png`。若用户明确要求“先只交提示词”，则讲义中不放封面占位图，后续成图后再决定是否插回。
17. 学生版讲义在正文结束、附录开始之前，必须固定预留 `info.png` 信息图占位；该图默认不要求在 Step 3 同步生成提示词，待后续多媒体制作或人工制作完成后回写。
18. 学生版讲义在导出 PDF 前，必须确认所有**正文中已实际引用**的图片都已落盘。若封面漫画当前仅交提示词、未插入正文，则它不构成 PDF 导出的阻断项；`info.png` 若已在正文中引用，则必须先回写对应文件。
...
5. 学生版讲义默认必须规划一幅封面漫画资源；多媒体阶段默认先交付 `cover-comic-prompt.md`。若用户要求先只交提示词，则暂不在正文中插入封面图；若后续成图再接入，默认位置仍为讲义信息节后、正文前。
6. 封面漫画应延续已确认的多格漫画导入思路：允许格子不绝对整齐，但必须用箭头标出阅读顺序；所有对话必须使用简洁中文，不得出现英文气泡或大段说明。
7. 学生版讲义正文末尾、附录前的 `info.png` 信息图默认保留占位；多媒体阶段优先给出制作规格，不强制同步生成提示词。
8. 若讲义中的代码直出图脚本已经具备，多媒体阶段优先补足封面漫画、导入视频、信息图等提示词/制作规格，而不是重复生成已有代码图。
9. 默认还应规划一个符合课程场景的 15 秒导入视频，用于课堂开场或互动课首页导入；视频提示词生成时必须显式调用 `seedance` 技能来组织中文导入视频提示词结构。
10. 设计导入视频提示词时，若用户未指定具体风格，可先从电影级写实、2D 动画、3D 动画、木偶、黏土动画等候选中**随机选择一个最适合本课主线的单一风格**；一旦选定，就必须按该单一风格完成镜头与转场设计，不得把多个风格并列为“任选”。
11. 若用户指定即梦平台，视频提示词默认使用**中文自然语言**，并按“主体 -> 动作链 -> 场景 -> 镜头与构图 -> 风格 -> 光线色彩 -> 画质 -> 负面约束”的顺序写成完整句群；不得只堆关键词。
12. 导入视频的目标是建立问题情境与课程气质，不是复述讲义正文；默认应少用公式、板书和技术细节，尤其在电影感方案中，除非用户明确要求，否则不要把公式推导写成画面主角。
13. 若交付封面漫画提示词，默认优先交付**中文定稿提示词**，直接可用于中文生图平台；不得只给“设计约束清单”，必须给出可直接投喂模型的完整提示词正文。
