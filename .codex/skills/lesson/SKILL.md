---
name: lesson
description: 面向“自动控制原理”课程创作完整教学内容包——讲义、知识图谱节点、BOPPPS 课案、互动页面蓝图与多模态资源；在正式制作前先执行运行时知识图谱与知识卡片到作者态的全量同步、合并与冲突检测，并按 `course-content/syllabus-refactor/module-skeletons.md` 与 `unit-design-details/` 的最新边界生成新编号课次。当用户提到要创作、写、生成某个单元或课次（如“2-2”“4-1”“下一单元”），或要求生成 handout/讲义、BOPPPS/教案、interactive-page/互动课、知识节点、多媒体资源、时域/频域/根轨迹图、方框图/信号流图/电路图/机械结构图中任何一项时，都应激活本技能；创作时需遵守 `$`/`$$` 公式规范、LaTeX 渲染、静态图 `python control` / `Octave` 验证、互动运行时 Rust/WASM 曲线契约与 `tikz-control-draw` 线框图流程。
---
...
5. **静态核验与运行时引擎必须分层**
   - 讲义、教师版讲义、多媒体中的静态数值图核验与成图，统一沿用 `Octave` / `python control` / `Python-matplotlib` 流程。
   - 互动课程运行时的参数联动曲线，统一沿用 Rust/WASM 控制分析引擎与共享请求接口，不再默认回退到 `Octave` 或 `python control` 的前端实时计算。
   ...
   - 根轨迹默认优先用 `rlocus()` 生成与核验；若为了统一排版而转成“`Octave` 导出数据 + `Python/matplotlib` 复绘”，则必须调用 `.codex/skills/lesson/scripts/root_locus_branch_match.py` 做分支连续匹配与审计，不再允许各课次临时手写一套局部匹配逻辑。
   - 上述根轨迹辅助脚本同时服务于独立根轨迹图、局部放大插图、以及 `2×2` / 多联 / 四联图中的根轨迹子图；凡涉及根轨迹子图，都必须共用同一份匹配后分支数据，只允许视窗不同，不允许各自单独重匹配。
   - 根轨迹脚本的输入格式、输出格式、视图配置和审计项，统一遵循 `references/root-locus-branch-match.md`。
   - 跨域 `2×2` 或多联数值图的子图内容与位置，默认严格继承讲义原图布局；设计稿与实现稿都必须把 `subplot_mapping` 写清楚，不得把某种布局误写成课程通用固定模板。子图标题只写基本标题，不写“左上/右上/左下/右下”等方位提示。若根轨迹存在显著尺度差异，可沿讲义原图采用案例 B 一类的异尺度布局，但仍必须逐项写清各子图在原图中的位置与承担的判断任务。
   - 上述规则用于讲义、教师版讲义与媒体中的静态数值图核验与成图，不等同于互动课程运行时的实时计算引擎。凡讲义曲线在互动课中升级为参数联动图，设计稿必须明确其运行时统一接入 Rust/WASM 控制分析引擎，并按共享请求接口写清 `caseId`、`outputs`、`timeRange`、`frequencyRange`、`rootLocus`、`structures`、`referenceProfile`、`feasibleRegion` 等字段，不能只写“有滑块、会联动”。
...
7. **互动课必须先形成完整课件**
   ...
   - 讲义中的核心概念、核心公式、关键图表、关键例题与关键结论，必须以“证据单元”形式逐项落到具体页面，不能以“见讲义”“教师口述”或“实现时再补”替代。证据单元至少覆盖：对象、公式、曲线图、结构图、表格、关键判断、误判点与任务卡。
   - 若讲义中已经出现“由标准形式推出阻尼/带宽/裕度公式”“由目标指标反求参数”“由结构差异推出闭环传函差异”这类推导链，互动设计必须把该推导链写成显式证据单元，不能只保留起点公式和终点答案。
   - 若讲义中的图后还跟有闭环传函、指标表、适用规律表、风险表或保守带宽实例表，互动设计必须继续把这些后续证据落到页面，不能只保留图本身。
   - 允许纯静态页面承担知识展示；凡暂时不值得做互动升级的核心内容，可以直接做静态展示页，不强制每页都带交互控件。
   - 页面总数按教学逻辑决定，不设固定上限；不得为了压到某个页数范围而删减核心内容。
   - 在保留课件骨架的基础上，只把最值得互动升级的位置做成三类主类型：证据联读板、参数联动仿真板、推导显影板。排序、连线、拖拽、前后测或工作区操作只作为局部辅助手段，不再作为默认页面骨架。
   - 设计型案例页默认按“对象/背景 -> 模型与公式 -> 推导或分析 -> 图像与曲线 -> 指标/边界 -> 判断与任务卡”的主阅读顺序组织，不得默认改成“大图先行”或“流程卡先行”。
   - Step 7 开始前，必须先为每个证据单元建立“升级决策表”，至少写清 `证据类型`、`来源锚点`、`目标步骤`、`升级方式`（`静态保留 / 原生重绘 / 参数联动 / 点击显影 / 逐步揭示`）、`保留元素`、`不得删减内容` 与 `验收点`；不得等实现阶段再临时决定图片是否互动化。
   - 除照片、封面图、信息图与承载不可替代外部素材的图片外，讲义中的教学图片默认不得直接以位图原样保留到互动页；结构图、关系图、步骤图、对比图、矩阵图、判断清单和职责图，必须优先升级为前端原生组件、点击显影板或逐步呈现板。
   - 若同一证据单元同时包含图片、公式、表格、结论句与任务卡，设计稿不得只写“主阅读顺序”四个字，必须显式写成 `先出现什么 -> 再出现什么 -> 最后出现什么`，并写清哪些内容必须同屏、哪些内容可折叠到下一块；不得给实现阶段留下“默认把图顶到最前面”的自由度。
   - 若讲义中的图后还跟有公式解释、闭环传函、规律表、风险表或实例表，这些后续证据默认与该图共属同一证据链；互动设计必须继续按原顺序落页，并明确“图后证据”位于同一步骤内还是紧邻下一步骤，不得把图先落页、其余证据后补或省略。
   - 凡属于推导型证据单元，默认升级为“推导显影板”：必须写清 `起点公式`、`中间改写`、`命名公式`、`代入/判定`、`结论句` 的显影顺序；除纯回顾页外，不得只保留最终结果式。
   - 若公式页承担讲义主线中的首次正式推导，默认至少分成 `对象或标准形式 -> 中间推导 -> 目标公式 -> 结果解释` 四层；仅把全部公式一次性平铺，视为设计未完成。
   - 讲义中的曲线图默认需要升级为可调参数的互动图像；互动图像的默认状态必须与讲义静态图一致，默认参数、结构配置、曲线数量与图组排布都要复现原图。
   - 若讲义中的静态图是 `2×2` 曲线图组，互动区域默认也保持 `2×2` 图像格子，仅在窄屏下做响应式重排，不改变图组语义与阅读顺序。
   - 若讲义中的四联图或多联图把幅频特性和相频特性拆成两个子图，则实现时必须严格复现讲义原图中的相对位置与阅读顺序，并同时动态标注相角裕度与增益裕度；不得擅自把其中任一子图换位。
   - 曲线图控件栏默认放在图像模块下方并折叠显示：单一结构只暴露一个参数时，默认使用一个滑块；存在结构变化时，使用结构勾选项与各结构的参数滑块；同类型结构只配置一次，不按每条曲线重复配置。
   - 若静态图对应“同一结构在不同参数下的多条曲线”，则互动图必须能通过滑块覆盖这些曲线位置，默认停在基线参数；若静态图对应“多种结构的并列对照”，则互动图必须提供结构勾选与对应参数控制，默认勾选状态与基线参数均与静态图一致。
   - 互动曲线图默认采用固定绘图范围与固定数轴；范围必须按参数上下限能够覆盖的全区间预先确定，不允许因滑块变化动态重算坐标范围或动态缩放数轴，避免页面重绘闪烁。
   - 四联图或多联图面板默认只保留子图标题，不再额外堆叠说明文案；图内标注应替换为对应指标：时域图显示当前时域性能指标，根轨迹图显示当前闭环极点位置，频域图显示当前双裕度；所有图内标注与 tooltip 数值统一保留 2 位小数。
   - 时域响应图默认需要用虚线给出参考信号；根轨迹图若存在分离点、回折点、交汇点等关键点，设计稿必须把“关键区间需加密采样并保证轨迹交汇连续”写入契约，避免实现后出现分离点断开。
   - 曲线图的子图内容与空间位置必须严格镜像讲义原图，不得为了实现方便随意改换子图类型、上下左右位置或阅读顺序。
   - 除系统框图外，讲义中的步骤示意图、对比图、卡片图、层级图、区域图、职责图、判断清单和原生表格，默认都要映射为前端原生绘制或原生表格组件，以便后续继续做点击显影、逐步呈现或局部交互改造。
   - 对于步骤型、对比型、卡片型示意图，作者态设计默认要写成“按步骤点击逐步呈现”的互动形态，而不是一次性平铺全部信息。
   - 凡讲义中的表格进入互动页，默认实现为前端原生表格，并保留公式与符号的 LaTeX 渲染能力；不得把表格内容烘成位图后直接贴图。
8. **讲义围绕单一主线展开**
...
12. **讲义必须导出正式 PDF**
   ...
   - 若学生版正文已经定稿、但 `cover-comic.png` 与 `info.png` 尚未完成，只允许用 `export_handout_pdf.py --draft-mode` 导出 `handout-draft.pdf` 供 NotebookLM/多媒体制作参考；不得为了让第一次 PDF 导出通过，就在 `media/processed/` 中写入脚本渲染的假封面漫画或假信息图。
   ...
   - Markdown 源稿中的表格默认必须保留“表序号 + 标题”这一行，便于源码阅读；PDF 导出时每个表格都必须生成**居中的正式表题与连续编号**，不得省略表题、不得左对齐悬挂。若正文需要稳定交叉引用，应进一步为该表提供 `#tbl:` 标签或等价 `\label{}`。
   - PDF 导出后的表格应主动调整列宽分配，尽量充分利用页心宽度，优先让长文本列获得更多宽度，避免第一列空白过多而后续列过窄；若导出后仍出现明显失衡，应回到导出模板或预处理脚本调整列宽分配，不要只靠作者在正文中换行补救。
...
16. **多媒体阶段必须产出媒体链接文档**
   ...
   - 该文档的标准节名固定为 5 个课程级资源条目：`[单元编号]-intro-video.mp4`、`[单元编号]-slides.pdf`、`[单元编号]-course.mp4`、`[单元编号]-audio.m4a`、`handout.md`。
   - 若该文档不存在，或缺少上述任一节名，智能体只补齐缺失的空白节名，不得代写其他资源说明、链接或摘要。
   - 若该文档已经存在且某些节下已有人工内容，智能体不得覆盖、改写、重排或清空这些已有内容；只允许补缺失节。
   - 仅当 `intro-video.mp4` 对应节存在但仍为空白时，才允许补 1 句导入视频文案；该文案必须描述视频中实际出现的场景、对象、动作和对比关系，不得写成“用导入情境引出……”这类泛化句式。
   - 若引用现有导入视频提示词，也只在 `intro-video.mp4` 节为空时，才把提示词中的主线情节压缩成 1 句可直接放入媒体链接文档的中文描述。
17. **封面漫画与信息图属于保留资产**
   - `[单元编号]-cover-comic.png` 与 `[单元编号]-info.png` 默认由用户或外部生成流程回写，属于保留资产文件名，不得被 `Octave`、`Python/matplotlib`、批量复绘脚本或临时占位脚本覆盖。
   - 任何代码直出脚本若需要批量生成图片，输出目标必须显式列出允许写入的数值图文件名；允许写入列表中不得包含 `cover-comic.png`、`info.png`、`intro-video.mp4`、`slides.pdf`、`audio.m4a` 等课程级待审查成品名。
...
3. Step 7 的讲义映射单位固定为“证据单元”，不得只按标题摘要或课堂步骤做粗粒度映射。输出 `interactive-page.md` 与 `interactive-contract.yaml` 前，必须先明确每个证据单元是静态承载、互动升级，还是折叠展开。
4. 若证据单元中包含曲线图，则默认进入“参数联动仿真板”设计，不得先假设保留为静态图。只有纯说明性示意图、照片、封面图或普通结构框图，才可以不做动态化。
5. 曲线互动图必须写清“静态基线如何复现”：包括基线参数、默认结构组合、图组排布、控件位置与响应式规则。若 handout 中使用 `2×2` 图组，互动稿中必须保持同一图组逻辑。
6. 若曲线证据对应根轨迹工作区，设计稿必须明确是否统一接入 Rust/WASM 控制分析引擎、是否只保留一个根轨迹面板、是否允许添加零点/极点、是否允许拖动零极点位置，以及默认零极点位置是否与 handout 一致；不得只写“有根轨迹工作区”。
7. Step 7 输出前，必须同时产出“证据单元升级决策表”与“混合证据顺序表”。前者至少覆盖所有图、表、公式链；后者至少写清每一步骤中 `图片/图组/结构图/公式/表格/结论/任务卡` 的先后顺序与同屏关系。
8. 若证据单元对应非曲线教学图，必须在作者态先标记其升级方式：`原生重绘 / 点击显影 / 分步呈现 / 保留位图`；选择 `保留位图` 时必须附一条理由，且只能用于照片、封面图、信息图或不可替代的外部素材图。
9. 若证据单元中存在推导链，`interactive-page.md` 与 `interactive-contract.yaml` 都必须写出逐步揭示节奏；至少要能看出“学生首次看到什么、点击后展开什么、最后收束到什么结论”，不得只保留最终结果式与一句解释。
10. Step 7 完成态默认要求：作者态映射锚点、公式映射、曲线镜像说明与契约解析告警均为 `0`。若 `review_lesson_content.py` 仍报告 `handout_anchor`、公式映射或契约解析告警，不进入实现阶段。
...
13. 若采用跨域 `2×2` 或多联教学图，子图的内容与位置必须严格继承讲义原图布局；子图标题只保留基本标题。若根轨迹存在全范围与关键区域两个明显尺度，可沿讲义原图采用案例 B 一类的异尺度版式，但必须明确各子图在原图中的位置、阅读顺序和承担的判断任务。多联图中若包含多个根轨迹子图，这些子图必须来自同一份 `root_locus_branch_match.py` 处理后的匹配数据与视图配置。
14. 根轨迹类媒体在导出后必须专项检查：开环极点/零点数量是否完整、闭环分支数是否等于开环极点数、共轭分支是否在交汇点真正汇合、远端快速极点附近是否存在被截断或因增益上限不足而缺失的轨迹。专项检查默认读取 `root_locus_branch_match.py` 输出的审计 JSON，不再接受纯人工目测替代。
15. 多媒体阶段结束前，必须在 `media/processed/` 下确保存在 `[单元编号]-media.md` 媒体链接文档；若文档不存在或节名不全，只补齐 5 个课程级资源的空白节名。若文档已有内容，不覆盖、不改写，只允许补缺失节；仅当导入视频节为空时，才允许补 1 句导入视频文案。
