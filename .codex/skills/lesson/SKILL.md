...
13. **资源库只作候选源，不直接拼贴成课**
   ...
   - 应根据当前单元目标，选择性读取 `pptx`、`civics-cases`、`ship-control-cases` 的索引和正文，而不是整包通读。
...
**⚠️ 如课型为“实践”**：确认后读取 `references/practice-lesson.md`，执行其中 5 步工作流，不执行 Step 2 ~ 8。
...
若三类资源都不适合当前单元，必须明确写出“本课不强行接入资源库内容”的理由，而不是机械凑数。
...
5. `pptx` 图示与船舶案例若被接入互动页，必须转写成“预测、比较、读图、验证、诊断”之一，而不是静态堆叠素材卡。
...
6. 学生版讲义默认必须规划一幅封面漫画资源、一个 15 秒导入视频、一个信息图，并在资源总表中显式保留 `{unit}-slides.pdf`、`{unit}-course.mp4`、`{unit}-audio.m4a`。若本轮暂不生成成品，也必须保留这些交付项与命名。
7. 若交付 AI 提示词，默认优先交付可直接投喂中文平台的**中文定稿提示词**；即梦导入视频除常规“分镜图生成提示词”和“图生视频提示词”外，还必须补一段“Agent 模式视频生成提示词”，只保留故事情节主线，不写细风格与配乐。
8. 若资源库中已有高质量提取图或可重绘骨架，可直接在媒体规划中登记“复用原图 / 依据原图重绘 / 仅保留信息结构”三种处理方式之一，但最终仍要经过版面与正确性复核。
...
| 实践课（如 `4-4`、`4-6`、legacy `L-2d` 来源） | 读取 `references/practice-lesson.md`，执行其中的 5 步工作流，不执行标准 Step 2 ~ 8 |
