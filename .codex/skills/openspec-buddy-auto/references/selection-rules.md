...
```bash
node .codex/skills/openspec-buddy/scripts/select-next-change.mjs < selection-input.json
```

For the common case, use the wrapper:

```bash
.codex/skills/openspec-buddy/scripts/select-next-change.sh [current-series]
```
