...
- 学生端图片标题、图注、提示文案禁止泄露 `runtime`、`设计稿`、`实现`、`配套图示`、`默认消费路径` 这类工程口径；学生只应读到对象、证据与判断任务。
...
- 若设计稿要求例题逐步显影，实施测试必须覆盖两项：`点击“显示下一步”可继续显影` 与 `点击当前已显影的最下方步骤也可继续显影下一层`；不要只测页外按钮存在。
...
- 逐步显影组件若已在外层显示步骤编号，步骤标题正文不得再重复写“第 1 步：……”；默认支持点击当前已显影步骤继续显影下一层，不只保留页外总按钮。
...
- 逐步显影组件必须能展开全部设计层级。若教师端只控制初始层数，学生端仍应按契约支持本地点击继续展开或等待教师推进；不得停在第一层没有后续内容。
