...
const closing = new RegExp(String.raw`(^|\n|\s)(close[sd]?|fix(e[sd])?|resolve[sd]?)\s+(#${issueNumber}|\w+/\w+#${issueNumber})`, "i");
if (closing.test(body)) {
  process.stderr.write(`PR body uses a closing keyword for #${issueNumber}; Buddy issues must stay open until archive.\n`);
  process.exit(1);
}
...
if (current.includes(marker)) {
  fs.writeFileSync(process.argv[3], current ? `${current}\n` : "");
  process.exit(2);
}
const note = `${marker}\n\n## OpenSpec Buddy\n\nOrigin issue: #${issueNumber}\nDevelopment source: #${issueNumber}`;
fs.writeFileSync(process.argv[3], current ? `${current}\n\n${note}\n` : `${note}\n`);
' "$pr_file" "$issue_number" "$body_file"
body_status=$?
set -e
