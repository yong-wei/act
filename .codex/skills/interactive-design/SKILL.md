...
0.7 **固定首尾步骤**
   ...
   - 步骤 3 固定为前测：题目必须是客观题，只考察学习本单元所需的基本预备知识，禁止考察本单元将要讲授的新知识点。
   - 收尾固定拆成两个页面：后测一个页面，总结一个页面。
   - 后测使用简明题目验证课程目标达成，必须与目标页呼应，但不得设计成过度复杂的综合大题。
   - 总结页顶部必须附课程信息图；随后讨论本单元主要内容、局限性、重点难点，并可留下拓展思考。
...
- 步骤 3 是否为只考察预备知识的客观题前测页，未提前考察本单元新知识
